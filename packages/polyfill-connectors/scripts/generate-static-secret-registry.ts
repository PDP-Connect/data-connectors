...
function canonicalSlug(connectorId: string): string {
	return connectorId.startsWith(REGISTRY_URL_PREFIX)
		? connectorId.slice(REGISTRY_URL_PREFIX.length)
		: connectorId;
}
...
function descriptorFromManifest(
...
): StaticSecretDescriptor | null {
	const normalized = normalizeStaticSecretCredentialCapture(
		connectorKey,
	...
	const setupFields = fullyBundled
		? []
	...
	if (
		isBundledStaticSecretCredentialKind(credentialKind) &&
		bundleFields.length > 1
	) {
		descriptor.secretFieldEnvVars = Object.fromEntries(
			bundleFields.map((field) => [field.name, [...field.env]]),
		);
		const optional = bundleFields
	...
	if (setupFields.length > 0) {
		descriptor.setupFieldEnvVars = Object.fromEntries(
			setupFields.map((field) => [field.name, [...field.env]]),
		);
	}
	return descriptor;
}
...
function jsonStringArrayMap(
	...
	const lines = keys.map(
		(key) =>
			`${indent}  ${JSON.stringify(key)}: ${jsonStringArray(record[key] ?? [])},`,
	);
	return `{\n${lines.join("\n")}\n${indent}}`;
}
...
function descriptorLiteral(descriptor: StaticSecretDescriptor): string {
	const lines: string[] = [
		`      credentialKind: ${JSON.stringify(descriptor.credentialKind)},`,
	];
	...
	if (descriptor.secretEnvVars) {
		lines.push(
			`      secretEnvVars: ${jsonStringArray(descriptor.secretEnvVars)},`,
		);
	}
	if (descriptor.secretFieldEnvVars) {
		lines.push(
			`      secretFieldEnvVars: ${jsonStringArrayMap(descriptor.secretFieldEnvVars, "      ")},`,
		);
	}
	if (descriptor.optionalSecretBundleFields) {
		lines.push(
			`      optionalSecretBundleFields: ${jsonStringArray(descriptor.optionalSecretBundleFields)},`,
		);
	}
	if (descriptor.setupFieldEnvVars) {
		lines.push(
			`      setupFieldEnvVars: ${jsonStringArrayMap(descriptor.setupFieldEnvVars, "      ")},`,
		);
	}
	return `{\n${lines.join("\n")}\n    }`;
}
...
const registryBody = entries.map(
	([key, descriptor]) =>
		`    ${JSON.stringify(key)}: ${descriptorLiteral(descriptor)},`,
);
...
// Run the freshly-written output through Biome's own formatter so this
// generator's output always matches whatever this package's biome.jsonc
