...
describe("getChangedFiles: real git repository, real rename operations", () => {
	...
	test("a committed rename via `git mv` reports the old path as deleted, not silently dropped", () => {
		...
		git(repoRoot, [
			"mv",
			"connectors/acme/index.ts",
			"connectors/acme/index-renamed.ts",
		]);
	...
	test("a staged rename via `git mv` (uncommitted) reports the old path as deleted", () => {
		...
		git(repoRoot, [
			"mv",
			"connectors/acme/index.ts",
			"connectors/acme/index-staged-renamed.ts",
		]);
		...
		rmSync(join(repoRoot, "connectors", "acme", "index-staged-renamed.ts"), {
			force: true,
		});
	});
	...
	test("an unstaged filesystem rename (plain `mv`, no `git add`) reports the old path as deleted", () => {
		...
		assert.deepEqual(deletedRelativePaths, ["connectors/acme/index.ts"]);
		assert.deepEqual(changedRelativePaths, [
			"connectors/acme/index-fs-renamed.ts",
		]);
	...
	test("a rename staged via `mv` + `git add -A` (the ordinary developer workflow) reports the old path as deleted", () => {
		...
		const statusPorcelain = git(repoRoot, [
			"status",
			"--porcelain",
			"--",
			"connectors/acme",
		...
		assert.deepEqual(deletedRelativePaths, ["connectors/acme/index.ts"]);
		assert.deepEqual(changedRelativePaths, [
			"connectors/acme/index-add-a-renamed.ts",
		]);

		git(repoRoot, ["reset", "-q", "HEAD", "--", "connectors/acme"]);
		git(repoRoot, ["checkout", "--", "connectors/acme/index.ts"]);
		rmSync(join(repoRoot, "connectors", "acme", "index-add-a-renamed.ts"), {
			force: true,
		});
	});
	...
	test("a rename plus a small edit (high similarity, git would classify as a rename) still reports the old path as deleted", () => {
		...
		git(repoRoot, [
			"mv",
			"connectors/acme/index.ts",
			"connectors/acme/index-renamed-edited.ts",
		]);
		writeFileSync(
		...
		const renameStatus = git(repoRoot, [
			"diff",
			"--cached",
			"-M",
			"--diff-filter=R",
		...
		git(repoRoot, ["reset", "-q", "HEAD", "--", "connectors/acme"]);
		git(repoRoot, ["checkout", "--", "connectors/acme/index.ts"]);
		rmSync(join(repoRoot, "connectors", "acme", "index-renamed-edited.ts"), {
			force: true,
		});
	});
	...
	test("a rename to a path with spaces and non-ASCII characters still reports the old path as deleted and the new path is not silently dropped", () => {
		const baseRef = git(repoRoot, ["rev-parse", "HEAD"]).trim();
		mkdirSync(join(repoRoot, "connectors", "acme", "weird dir"), {
			recursive: true,
		});
		git(repoRoot, [
			"mv",
			"connectors/acme/index.ts",
			"connectors/acme/weird dir/idx é.ts",
		]);
		...
		git(repoRoot, ["reset", "-q", "HEAD", "--", "connectors/acme"]);
		git(repoRoot, ["checkout", "--", "connectors/acme/index.ts"]);
		rmSync(join(repoRoot, "connectors", "acme", "weird dir"), {
			recursive: true,
			force: true,
		});
	});
	...
	test("an added file with a non-ASCII name is reported, not silently dropped by path-quoting", () => {
		const baseRef = git(repoRoot, ["rev-parse", "HEAD"]).trim();
		writeFileSync(
			join(repoRoot, "connectors", "acme", "café-módulo.ts"),
			"export const x = 1;\n",
		);
		...
		git(repoRoot, ["reset", "-q", "HEAD", "--", "connectors/acme"]);
		rmSync(join(repoRoot, "connectors", "acme", "café-módulo.ts"), {
			force: true,
		});
	});
...
describe("getChangedFiles: unresolved Git index/worktree entries fail closed", () => {
	...
	test("a real UU conflict is returned as unmerged even when both staged and unstaged U diffs exist", () => {
		...
		assert.equal(
			git(repoRoot, ["status", "--porcelain=v1"]).trim(),
			"UU index.ts",
		);
		assert.equal(
			git(repoRoot, [
		...
		assert.equal(
			git(repoRoot, ["diff", "--name-only", "-z", "--diff-filter=U"]),
			"index.ts\0",
		);
