...
export function countByLocationKey<
	T extends { path: string; line: number; column: number },
 ...
 *
 * Deliberately NOT `extends: ["./biome.jsonc"]`: Biome (2.5.7) replaces
 * rather than merges `files.includes` across an extends chain, so a config
 * that extends biome.jsonc's own extends chain (ultracite/biome/core,
 * ultracite/biome/type-aware) ends up checking only this scratch file
 * itself — confirmed by a minimal repro and by this script silently
 * reporting its entire allowlist as stale (0 live findings) on an
 * unmodified tree. `root: true` with just the one override this script
 * needs sidesteps that: it doesn't inherit biome.jsonc's other rules, but
 * `--only=performance/noAwaitInLoops` below means nothing else is measured
 * anyway.
 */
function collectLiveFindings(): LiveFinding[] {
	...
	const reportPath = join(
		mkdtempSync(join(tmpdir(), "pdpp-noawait-conformance-")),
		"report.json",
	);
...
function main(): void {
	...
	const duplicateAllowlistEntries = [...allowlistKeyCounts.entries()].filter(
		([, count]) => count > 1,
	);
	...
	const repeatedLiveLocations = [...liveKeyCounts.entries()].filter(
		([, count]) => count > 1,
	);
	...
	const stale = NO_AWAIT_IN_LOOPS_ALLOWLIST.filter(
		(entry) => !liveByKey.has(locationKey(entry)),
	);
	...
	if (stale.length > 0) {
		...
		for (const entry of stale) {
			console.error(
				`  ${entry.path}:${entry.line}:${entry.column} (${entry.category}: ${entry.note})`,
			);
		}
		console.error(
