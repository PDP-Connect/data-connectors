...
const CHATS_EMPTY = "[]";
const CHATS_MALFORMED = JSON.stringify([
	{ unexpected_field: "no conversation_id" },
]);
...
const MESSAGES_MALFORMED = JSON.stringify([
...
function customMessagesForThisInvocation() {
	const pool = JSON.parse(
		process.env.FAKE_GMCLI_MESSAGES_JSON ?? MESSAGES_HEALTHY,
	);
...
const isChatsList =
	args.includes("chats") &&
	args.includes("list") &&
	args[args.indexOf("chats") + 1] === "list";
const isMessagesList = args[0] === "messages" && args[1] === "list";

if (mode === "not_paired") {
	process.stderr.write(
		"error: device not paired. Please run `gmcli auth` to pair your Android device.\n",
	);
	process.exit(1);
}
...
process.stderr.write(
	`fake-gmcli: unrecognized invocation: ${args.join(" ")}\n`,
);
process.exit(1);
