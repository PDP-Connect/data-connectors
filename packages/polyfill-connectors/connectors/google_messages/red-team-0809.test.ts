...
function chat(
	overrides: Partial<RawChat> & Pick<RawChat, "conversation_id">,
): RawChat {
	return {
		source_platform: "rcs",
...
function msg(
	overrides: Partial<RawMessage> &
		Pick<RawMessage, "message_id" | "timestamp_ms" | "conversation_id">,
): RawMessage {
	return {
		source_platform: "rcs",
...
test("finding 1 — growing conversation: a conversation past its per-chat limit still surfaces brand-new messages on the next run", async () => {
	const chats = [
		chat({ conversation_id: "chat_alice", last_message_time_ms: 2_000_000 }),
	];
	const limit = 5;
	...
	assert.equal(
		run1Emitted.length,
		limit,
		"run 1 emits every message in the (already full) window",
	);
	assert.ok(
	...
	// window forever.
	assert.ok(
		run2Ids.has("m5") && run2Ids.has("m6") && run2Ids.has("m7"),
		"the three brand-new messages are emitted",
	);
	assert.ok(
	...
	const truncationSkip = skips(run2.messages).find(
		(s) => s.reason === "gmcli_per_chat_limit_reached",
	);
	assert.ok(
		truncationSkip,
...
test("finding 2 — sortChatsByRecency: missing/null last_message_time_ms sorts last, never wins priority over a real timestamp", () => {
	...
	assert.equal(
		sorted[0]?.id,
		"has_timestamp",
		"any real timestamp, however small, outranks a missing one",
	);
	assert.deepEqual(
...
test("finding 2 — exact cap, no gap: chat count equal to GMCLI_MAX_CHATS scans everything, no truncation SKIP_RESULT", async () => {
	...
	const messages = chats.map((c, i) =>
		msg({
			message_id: `msg_${c.conversation_id}`,
			conversation_id: c.conversation_id,
			timestamp_ms: 1000 + i,
		}),
	);
	const run = await runGoogleMessagesCustom(
		chats,
		messages,
		{},
		{ GMCLI_MAX_CHATS: String(maxChats) },
	);
	const emitted = records(run.messages, "messages");
	assert.equal(
		emitted.length,
		3,
		"all three chats' messages are fetched — the count exactly meets the cap",
	);
	assert.ok(
		!skips(run.messages).some(
			(s) => s.reason === "gmcli_chat_scan_limit_reached",
		),
...
test("finding 2 — over cap, gap: chat count exceeding GMCLI_MAX_CHATS keeps the most recently active chats and emits a truncation SKIP_RESULT", async () => {
	...
	const chatScanSkip = skips(run.messages).find(
		(s) => s.reason === "gmcli_chat_scan_limit_reached",
	);
	assert.ok(
		chatScanSkip,
	...
	assert.match(
		...
		/Only the 2 most recently active conversations were scanned/,
	);
	assert.match(
		chatScanSkip?.message ?? "",
		/at least 3 conversation\(s\) exist/,
	);
...
test("finding 3 — chats list always carries an explicit --limit argv, never relies on gmcli's own default", async () => {
	...
	const chatsListInvocation = seenInvocations.find(
		(a) => a[2] === "chats" && a[3] === "list",
	);
...
test("finding 3 — behavioral: an archive with more chats than gmcli's own hidden 50-default is fully scanned when GMCLI_MAX_CHATS is set above 50", async () => {
	...
	const chats = Array.from({ length: chatCount }, (_, i) =>
		chat({
			conversation_id: `chat_${String(i).padStart(3, "0")}`,
			last_message_time_ms: 1_000_000 + i * 1000,
		}),
	);
	const messages = chats.map((c) =>
		msg({
	...
	for (const c of chats) {
		assert.ok(
			emittedChatIds.has(c.conversation_id),
			`${c.conversation_id} must have been scanned`,
		);
	}
...
test("finding 3 — behavioral: an archive genuinely exceeding GMCLI_MAX_CHATS past the 50-row hidden default is still correctly detected as truncated", async () => {
	...
	assert.match(
		chatScanSkip?.message ?? "",
		/Only the 60 most recently active conversations were scanned/,
	);
	...
	assert.match(
		chatScanSkip?.message ?? "",
		/at least 61 conversation\(s\) exist/,
	);
});
...
test("finding 3 — exact boundary: archive size exactly GMCLI_MAX_CHATS + 1 is correctly detected as truncated by exactly one chat", async () => {
	...
	const chats = Array.from({ length: chatCount }, (_, i) =>
		chat({
			conversation_id: `chat_${String(i).padStart(2, "0")}`,
			last_message_time_ms: 1_000_000 + i * 1000,
		}),
	);
	const messages = chats.map((c) =>
	...
	assert.match(
		chatScanSkip?.message ?? "",
		new RegExp(`at least ${String(chatCount)} conversation\\(s\\) exist`),
	);
});
...
test("finding 4 — behavioral: distinct reason code paths still surface their correct, distinct reason/message after skip was flattened to top-level fields", async () => {
	...
	const notInstalledInvoker: GmcliInvoker = () =>
		Promise.reject(
			new GmcliError("gmcli binary not found: gmcli", "not_installed"),
		);
	const notInstalledOutcome =
		await fetchAndParseGmcliMessages(notInstalledInvoker);
	...
	const schemaDriftInvoker: GmcliInvoker = () =>
		Promise.resolve({ exitCode: 0, stderr: "", stdout: "not json" });
	const schemaDriftOutcome =
		await fetchAndParseGmcliMessages(schemaDriftInvoker);
	assert.equal(schemaDriftOutcome.reason, "gmcli_schema_drift");
	assert.ok(
		schemaDriftOutcome.message,
		"gmcli_schema_drift must carry a non-empty message",
	);
});
...
test("finding 2 — the chat-scan-limit reason has vetted display copy declared in this connector's own manifest, not in RI", async () => {
	const manifestModule = await import("../../manifests/google_messages.json", {
		with: { type: "json" },
	});
	...
	assert.ok(
		...
		"manifest must declare a reason_display_messages map",
	);
	assert.ok(
