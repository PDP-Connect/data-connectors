...
function runGoogleMessages(
	streams: string[],
	env: Record<string, string>,
	opts: { allowFailedDone?: boolean } = {},
) {
...
test("empty archive: zero messages, coverage still collected", async () => {
	...
	assert.ok(
		messagesCoverage,
		"messages must emit DETAIL_COVERAGE even for a zero-considered run",
	);
...
test("schema drift: malformed chats output produces a typed error, not a silent wrong-shape emit", async () => {
	...
	assert.ok(
		skip,
		"expected a messages SKIP_RESULT for malformed gmcli chats output",
	);
...
test("schema drift: malformed messages output produces a typed error, not a silent wrong-shape emit", async () => {
	...
	assert.ok(
		skip,
		"expected a messages SKIP_RESULT for malformed gmcli messages output",
	);
