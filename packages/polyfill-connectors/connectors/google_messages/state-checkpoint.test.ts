...
function msg(
	overrides: Partial<RawMessage> &
		Pick<RawMessage, "message_id" | "timestamp_ms">,
): RawMessage {
	return {
		conversation_id: "chat_alice",
		source_platform: "rcs",
...
test("first run: no prior state, every message emitted and a STATE watermark is committed", async () => {
	...
	const cursor = stateFor(run1.messages, "messages") as
		| { fingerprints?: Record<string, string> }
		| undefined;
	assert.ok(
		cursor,
		"first run must commit a STATE watermark for the messages stream",
	);
	assert.equal(
		Object.keys(cursor?.fingerprints ?? {}).length,
		2,
		"cursor remembers both fetched message ids",
	);
});
...
test("no-op resume: identical archive on run 2 emits zero duplicate RECORDs", async () => {
	...
	assert.equal(
		records(run1.messages, "messages").length,
		2,
		"run 1 emits both messages",
	);
	const projected = projectState(run1.messages);
	assert.ok(
		projected.messages,
		"run 1 persisted a messages cursor to carry forward",
	);
	...
	assert.ok(
		...
		"no-op resume still commits a STATE watermark",
	);
});
...
test("new rows: run 2 adds one message to the same chat — only the new message is emitted", async () => {
	...
	assert.equal(
		emitted.length,
		1,
		"only the new message is emitted, not the unchanged one",
	);
	...
	assert.equal(
		coverage?.considered,
		2,
		"both fetched messages were considered, even though only one was emitted",
	);
	assert.equal(
		coverage?.covered,
		2,
...
test("same-timestamp rows: two distinct messages sharing sent_at are both emitted and both idempotent on resume", async () => {
	...
	assert.equal(
		emitted1.length,
		2,
		"distinct ids at the same timestamp are both emitted, not collapsed",
	);
...
test("late rows: a message with an older sent_at than already-seen messages is still emitted when it first appears", async () => {
	...
	assert.equal(
		emitted.length,
		1,
		"only the newly-appearing late message is emitted",
	);
	assert.equal(
		emitted[0]?.id,
		"msg_late",
		"an older-timestamped message is still discoverable on first appearance",
	);
});
...
test("truncation: per-chat limit hit still commits a STATE watermark for the fetched window, plus an honest SKIP_RESULT", async () => {
	...
	const messages = Array.from({ length: limit }, (_, i) =>
		msg({
			message_id: `msg_${String(i)}`,
			timestamp_ms: 1_754_071_000_000 + i * 1000,
		}),
	);
	...
	assert.equal(
		records(run1.messages, "messages").length,
		limit,
		"every fetched (bounded) message is emitted",
	);

	const truncationSkip = skips(run1.messages).find(
...
test("crash before commit: a mid-run failure leaves the prior checkpoint intact, not a partial/fabricated one", async () => {
	...
	assert.ok(
		projected.messages,
		"run 1 committed a real cursor to be at risk of loss",
	);
	...
	assert.ok(
		skip,
		"a failed fetch surfaces an honest SKIP_RESULT instead of a silent empty success",
	);
...
test("external archive replacement: a different (e.g. re-paired) archive's messages are all treated as new, not silently dropped", async () => {
	...
	const replacementMessages = [
		msg({
			message_id: "replaced_msg_1",
			conversation_id: "chat_bob",
			timestamp_ms: 1_754_079_000_000,
		}),
		msg({
			message_id: "replaced_msg_2",
			conversation_id: "chat_bob",
			timestamp_ms: 1_754_079_100_000,
		}),
	];
	const run2 = await runGoogleMessagesCustom(
		[NEW_CHAT],
		replacementMessages,
		projected,
	);
	...
	const cursor = stateFor(run2.messages, "messages") as
		| { fingerprints?: Record<string, string> }
		| undefined;
	assert.ok(
		cursor?.fingerprints?.replaced_msg_1,
		"new archive's messages are recorded into the cursor",
	);
	assert.ok(
		cursor?.fingerprints?.replaced_msg_2,
		"new archive's messages are recorded into the cursor",
	);
});
...
test("STATE is emitted after every messages RECORD this run, never before", async () => {
	...
	const stateIndex = run1.messages.findIndex(
...
test("a message the shape check rejects is considered but NOT covered", async () => {
	...
	const messages = [
		...
		msg({
			message_id: "msg_bad",
			timestamp_ms: 1_754_071_453_000,
			body: `bad${String.fromCharCode(0)}body`,
		}),
	];

	const run = await runGoogleMessagesCustom(chats, messages, {});

	const dropped = skips(run.messages).find(
		(s) => s.reason === "shape_check_failed",
	);
	assert.ok(
		dropped,
		"the poisoned message must be dropped by the runtime's shape check, not stored",
	);
	...
	assert.equal(
		coverage?.covered,
		1,
		"the dropped message must NOT count as covered — this is the shortfall",
	);
});
