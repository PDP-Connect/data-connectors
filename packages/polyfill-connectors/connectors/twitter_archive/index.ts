...
async function runTweetsStream(ctx: TweetsContext): Promise<void> {
	...
	if (!path) {
		await emit({
			...
			message:
				"Twitter archive tweets data was not found in the configured import directory",
		});
		return;
	}
...
async function runDirectMessagesStream(ctx: DmsContext): Promise<void> {
	...
	if (!archiveExists(path)) {
		await emit({
			...
			reason: "archive_not_found",
			message:
				"Twitter archive direct-message data was not found in the configured import directory",
		});
		return;
	}
