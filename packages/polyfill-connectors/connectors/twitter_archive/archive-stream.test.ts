...
test("stripAssignmentPrefix: returns from the array opener onward", () => {
	assert.equal(
		stripAssignmentPrefix("window.YTD.tweets.part0 = [1,2]"),
		"[1,2]",
	);
});
...
test("streamJsArchive: preserves escaped quotes, backslashes, newlines, brackets, unicode", async () => {
	const streamed = (await collect(fixturePath(TWEETS_FIXTURE))) as {
...
test("streamJsArchive: legacy flat tweet.js parses", async () => {
	const streamed = (await collect(fixturePath(LEGACY_FIXTURE))) as {
		id_str: string;
	}[];
...
test("streamJsArchive: absent file yields zero entries", async () => {
	const streamed = await collect(
		join(tmpdir(), "definitely-missing-twitter-archive.js"),
	);
	assert.deepEqual(streamed, []);
});
...
test("twitter_archive end-to-end: streams tweet + DM records and STATE cursors", async () => {
	...
	assert.ok(
		states.some(
			(m) =>
				(m as Extract<EmittedMessage, { type: "STATE" }>).stream === "tweets",
		),
	...
	assert.ok(
		states.some(
			(m) =>
				(m as Extract<EmittedMessage, { type: "STATE" }>).stream ===
				"direct_messages",
		),
