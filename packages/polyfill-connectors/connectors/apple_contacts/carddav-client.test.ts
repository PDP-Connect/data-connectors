...
test("syncCollectionReport: returns resources and a fresh sync-token on initial sync", async () => {
	...
	try {
		...
		assert.equal(
			result.resources[0]?.vcardText.includes("Alice Example"),
			true,
		);
...
test("syncCollectionReport: a subsequent call reports deletions as 404 responses", async () => {
	...
	try {
		...
		assert.deepEqual(second.deletedHrefs, [
			server.url("/addressbooks/owner/card/bob.vcf"),
		]);
	...
	try {
		...
		await assert.rejects(
			...
			(err: unknown) =>
				...
				err.message.startsWith("carddav_response_too_large"),
		);
	} finally {
		await server.close();
	}
...
test("syncCollectionReport: a missing redirect Location header is a CardDavStructuralError", async () => {
	...
	try {
		...
		await assert.rejects(
			...
			(err: unknown) =>
				err instanceof CardDavStructuralError &&
				err.message === "carddav_redirect_missing_location",
		);
	} finally {
		await server.close();
	}
...
test("syncCollectionReport: a redirect to an untrusted origin is a CardDavStructuralError", async () => {
	...
	try {
		...
		await assert.rejects(
			...
			(err: unknown) =>
				err instanceof CardDavStructuralError &&
				err.message.startsWith("carddav_unsafe_redirect"),
		);
	} finally {
		await server.close();
	}
...
test("syncCollectionReport: a redirect loop past the hop cap is a CardDavStructuralError", async () => {
	...
	try {
		...
		await assert.rejects(
			...
			(err: unknown) =>
				err instanceof CardDavStructuralError &&
				err.message.startsWith("carddav_too_many_redirects"),
		);
	} finally {
		await server.close();
	}
	...
	try {
		...
		await assert.rejects(
			listAddressBooks({
				...
				authHeader: AUTH_HEADER,
				fetchImpl: interceptMethod("PROPFIND", 302, {
					location: discovery.addressBookHomeUrl,
				}),
				trustedOrigins: [server.origin],
			}),
			(err: unknown) =>
