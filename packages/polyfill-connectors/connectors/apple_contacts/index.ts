...
function classifyDiscoveryFailure(err: unknown): Error {
	if (err instanceof CardDavRedirectOriginError) {
		return createConnectorFailure(
			APPLE_CONTACTS_ERROR_CODE.UNSAFE_REDIRECT_REFUSED,
			err.message,
			{ cause: err },
		);
	}
	if (err instanceof CardDavDiscoveryError) {
		...
		return createConnectorFailure(
			APPLE_CONTACTS_ERROR_CODE.DISCOVERY_FAILED,
			err.message,
			{
				cause: err,
 ...
 */
function classifyCardDavRequestFailure(
	err: unknown,
	options: { retryableByDefault: boolean },
): Error {
	const message = err instanceof Error ? err.message : String(err);
	if (err instanceof CardDavStructuralError) {
		return createConnectorFailure(
			APPLE_CONTACTS_ERROR_CODE.CARDDAV_REQUEST_FAILED,
			message,
			{
				cause: err,
				retryable: false,
...
export function addressBookRecord(
...
export function deriveGroups(
	bookUrl: string,
	cards: ReadonlyArray<{ card: ParsedVCard; uid: string }>,
): RecordData[] {
	const { contacts, groups } = partitionVCards(cards);

	const membersByGroup = new Map<
		string,
		{ memberUids: string[]; providerUid?: string }
	>();
	...
	for (const { card, uid } of groups) {
		...
		membersByGroup.set(name, {
			memberUids: groupMemberUids(card),
			providerUid: card.uid ?? uid,
		});
	}
...
interface AddressBookCollectionCtx {
	...
	progress: (
		message: string,
		extra?: { count?: number; stream?: string; total?: number },
	) => Promise<void>;
...
async function resolveSyncResult(args: {
	...
	result: Awaited<ReturnType<typeof syncCollectionReport>>;
	fullBoundary: boolean;
}> {
	const { bookUrl, authHeader, fetchImpl, trustedOrigins, priorSyncToken } =
		args;
	...
	if (
		first.supportsSyncCollection &&
		first.syncToken === "" &&
		priorSyncToken
	) {
		return {
			result: await syncCollectionReport({
				bookUrl,
				authHeader,
				fetchImpl,
				trustedOrigins,
...
async function emitContactGroupsIfRequested(args: {
	...
	const derivedCategoryGroups = new Set(
		contacts.flatMap(({ card }) => categoriesOf(card)),
	).size;
	...
	return {
		emitted: groupsEmitted,
		anchor: { ...anchor, emitted: groupsEmitted },
	};
}
...
async function hydrateMissingBodies(args: {
	...
	const fetchedByHref = new Map(
		fetched.map((resource) => [resource.href, resource]),
	);
	let unfetched = 0;
...
async function loadFullGroupSnapshot(args: {
	...
	cards: Array<{ card: ParsedVCard; uid: string }>;
	complete: boolean;
}> {
	...
	for (const resource of resources) {
		...
		cards.push({
			card,
			uid: String(contactRecord(bookUrl, resource, card).id),
		});
	}
 ...
 */
export async function collectAddressBook(
	...
	const bookCovered = await emitAddressBookRecordIfRequested({
		book,
		bookCursor,
		requested,
		emitRecord,
	...
	const emitContactRecord = async (resource: VCardResource): Promise<void> => {
		...
		if (
			...
			entityCursor.shouldEmit(record)
		) {
	...
	groupsBoundaryEstablished =
		groupsBoundaryEstablished && unparseableResources === 0;
	...
	const { emitted: groupsEmitted, anchor: groupAnchor } =
		await emitContactGroupsIfRequested({
			bookUrl: book.url,
	...
	contactsState[bookKey] = {
		...
		initial_sync_completed:
			priorSync?.initial_sync_completed === true || contactsBoundaryEstablished,
		...(supportsSync && resolvedSyncResult.syncToken
			? { sync_token: resolvedSyncResult.syncToken }
			: {}),
	};
...
if (isMainModule(import.meta.url)) {
	runConnector({
		...
		async collect({
			...
			const newState: Record<string, unknown> = JSON.parse(
				JSON.stringify(state),
			);
			const priorBookState =
				(state.address_books as Record<
					string,
					{ fingerprints?: Record<string, string> }
				>) ?? {};
			const bookCursor: FingerprintCursor = openFingerprintCursor({
				fingerprints: priorBookState.fingerprints,
			});
			...
			for (const book of books) {
				...
				contactsConsidered += bookContactsConsidered;
				contactsCovered += bookContactsCovered;
				anyUnparseableResource =
					anyUnparseableResource || hadUnparseableResource;
			}
			...
			if (requested.has("contact_groups") && groupsBoundaryEstablished) {
				await emit(
					buildFullScanCoverageMessage("contact_groups", groupsConsidered),
				);
			}

			if (requested.has("address_books")) {
				bookCursor.dropUnseenIds();
				newState.address_books = {
					fingerprints: bookCursor.toState(),
					fetched_at: nowIso(),
				};
				await emit({
					type: "STATE",
					stream: "address_books",
					cursor: newState.address_books,
				});
				await emitDetailCoverage(
					{ emit },
					{
