...
test("isSafeRedirectTarget: allows the narrow icloud.com subdomain carve-out", () => {
	assert.equal(
		isSafeRedirectTarget(
			"https://contacts.icloud.com/x",
			"https://p05-contacts.icloud.com/y",
			[],
		),
		true,
	);
...
test("isSafeRedirectTarget: allows an explicitly trusted origin", () => {
	assert.equal(
		isSafeRedirectTarget(
			"https://contacts.icloud.com/x",
			"https://totally-different.example/y",
...
test("isSafeRedirectTarget: refuses a malformed URL", () => {
	assert.equal(
		isSafeRedirectTarget("https://contacts.icloud.com/x", "not a url", []),
		false,
	);
});
...
test("isSafeRedirectTarget: refuses an icloud.com lookalike domain", () => {
	...
	assert.equal(
		isSafeRedirectTarget(
			"https://contacts.icloud.com/x",
			"https://icloud.com.attacker.example/y",
			[],
		),
		false,
	);
...
test("isSafeRedirectTarget: refuses icloud.com carve-out with a non-default port on the target", () => {
	assert.equal(
		isSafeRedirectTarget(
			"https://contacts.icloud.com/x",
			"https://p05-contacts.icloud.com:8443/y",
			[],
		),
		false,
	);
...
test("isSafeRedirectTarget: refuses icloud.com carve-out with a non-default port on the source", () => {
	assert.equal(
		isSafeRedirectTarget(
			"https://contacts.icloud.com:8443/x",
			"https://p05-contacts.icloud.com/y",
			[],
		),
		false,
...
test("discoverCardDav: follows a redirect to a regional host under the icloud.com carve-out", async () => {
	...
	try {
		const remap = (url: string): string =>
			url
				.replace(regionalFakeOrigin, server.regionalOrigin as string)
				.replace(primaryFakeOrigin, server.origin);
		const proxyFetch: Parameters<typeof discoverCardDav>[0]["fetchImpl"] =
			async (url, init) => {
				...
				if (res.status >= 300 && res.status < 400) {
					...
					const rewrittenLocation =
						location &&
						server.regionalOrigin &&
						location.startsWith(server.regionalOrigin)
		...
		assert.equal(
			result.principalUrl,
			`${regionalFakeOrigin}/principals/owner/`,
		);
...
test("discoverCardDav: rejects on 401 with a stable auth-rejected error", async () => {
	...
	try {
		await assert.rejects(
			...
			(err: unknown) =>
				err instanceof CardDavDiscoveryError &&
				err.message === "carddav_auth_rejected",
		);
...
test("discoverCardDav: refuses to follow a well-known redirect to an untrusted origin", async () => {
	...
	const attacker = await startFakeCardDavServer({
		username: "owner@example.com",
		password: "app-specific-pw",
	});
	const legit = await startFakeCardDavServer({
		username: "owner@example.com",
		password: "app-specific-pw",
	});
	try {
		...
		const maliciousFetch: Parameters<typeof discoverCardDav>[0]["fetchImpl"] =
			async (url, init) => {
				const res = await nativeFetchAdapter(url, init);
				if (String(url).endsWith("/.well-known/carddav")) {
					return syntheticResponse(302, {
						location: `${attacker.origin}/principals/owner/`,
					});
				}
				return res;
			};
