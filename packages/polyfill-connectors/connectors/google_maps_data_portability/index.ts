...
interface DataPortabilityClientLike {
	...
	getArchiveState: (
		archiveJobId: string,
	) => ReturnType<GoogleDataPortabilityClient["getArchiveState"]>;
	initiateArchive: (
...
function archiveStreamTimeRange(ctx: CollectContext): {
	endTime?: string;
	startTime?: string;
} {
	const stream = ctx.scope.streams.find(
		(entry) => entry.name === "archive_jobs",
	);
...
function mergeAccessType(
	...
	const authorizedFromEnv = csvSet(
		env.GOOGLE_DATAPORTABILITY_AUTHORIZED_RESOURCE_GROUPS,
	);
	const authorized = new Set([...oneTime, ...timeBased, ...authorizedFromEnv]);
	return { authorized, oneTime, timeBased };
}

function cursorForResource(
	state: DataPortabilityState,
	resourceGroup: string,
): ArchiveJobCursor | null {
	const cursor = state.archive_jobs?.[resourceGroup];
	return cursor && typeof cursor.archive_job_id === "string" ? cursor : null;
}

function shouldPollExisting(
	cursor: ArchiveJobCursor | null,
): cursor is ArchiveJobCursor {
	return Boolean(
		cursor?.archive_job_id && !TERMINAL_STATES.has(cursor.state ?? ""),
	);
}
...
async function collectResourceGroup({
	...
	if (!pollExisting) {
		await ctx.progress("Initiating Google Data Portability archive", {
			stream: "archive_jobs",
		});
	...
	return {
		access_type: accessTypeForResource(
			resourceGroup,
			initiated,
			accessType.oneTime,
			accessType.timeBased,
		),
...
export async function collectGoogleMapsDataPortability(
	...
	if (!ctx.requested.has("archive_jobs")) {
		await ctx.progress("No Google Data Portability archive streams requested", {
			stream: "archive_jobs",
		});
		return;
	}
	...
	const client = options.clientFactory
		...
		: new GoogleDataPortabilityClient({
				accessToken: requiredAccessToken(env),
			});
	const state = stateFromContext(ctx);
	const checkedAccessType = mergeAccessType(
		await client.checkAccessType(),
		env,
	);
	const selected = selectedResourceGroups(ctx);
	const resourceGroups = selected.filter((resourceGroup) =>
		checkedAccessType.authorized.has(resourceGroup),
	);
	...
	const nextJobs: Record<string, ArchiveJobCursor> = {
		...(state.archive_jobs ?? {}),
	};
