...
const MANIFEST: ProviderAuthManifestLike = Object.freeze({
	capabilities: {
		auth: {
			...
			deployment_config: [
				{
					env_alias: "GOOGLE_DATAPORTABILITY_CLIENT_ID",
					logical_key: "client_id",
					secret: false,
				},
				{
					env_alias: "GOOGLE_DATAPORTABILITY_CLIENT_SECRET",
...
const readyResolver: DeploymentConfigResolver = async ({ logicalKey }) =>
	READY_VALUES[logicalKey] ?? null;
...
test("registers under the oauth2_access_type_resource_groups kind", async () => {
	const adapter = await resolveProviderAuthAdapter(
		"oauth2_access_type_resource_groups",
	);
	assert.ok(
		adapter,
		"oauth2_access_type_resource_groups adapter is registered",
	);
});
...
test("runInventoryOrTest computes an access-type snapshot and rejects when nothing requested was authorized", async () => {
	...
	assert.ok(adapter);
	const fetchStub = makeFetch([
		jsonResponse({ oneTimeResources: [], timeBasedResources: [] }),
	]);
...
test("runInventoryOrTest returns a fingerprinted account with denied/authorized resource-group sourceBinding and no raw tokens", async () => {
	...
	try {
		const tokens = {
			accessToken: "ya29.inventory-access",
			refreshToken: "refresh-inventory",
			tokenKind: "Bearer",
		};
		...
		assert.deepEqual(account.sourceBinding?.authorized_resource_groups, [
			"maps.starred_places",
		]);
		assert.deepEqual(account.sourceBinding?.denied_resource_groups, [
			"myactivity.maps",
		]);
		assert.doesNotMatch(
			JSON.stringify(result.accounts),
			/ya29\.inventory-access|refresh-inventory/,
		);
	} finally {
		fetchStub.restore();
	}
...
test("runInventoryOrTest's persistenceContext carries only the non-secret access-type snapshot, never a raw token", async () => {
	...
	try {
		const tokens = {
			accessToken: "ya29.context-access",
...
test("storeTokens returns generic (non-prefixed) bundle field names given the persistenceContext from runInventoryOrTest", async () => {
	...
	try {
		...
		const { persistenceContext } = await adapter.runInventoryOrTest({
			manifest: MANIFEST,
			tokens,
		});
		...
		assert.equal(
			bundle.authorized_resource_groups,
			"maps.starred_places,myactivity.maps",
		);
		assert.ok(
			!Object.keys(bundle).some((key) =>
				key.startsWith("google_dataportability_"),
			),
...
test("storeTokens fails closed on a malformed persistenceContext instead of trusting its shape", async () => {
	...
	const malformedContexts = [
		{
			deniedResourceGroups: "not-an-array",
			oneTimeResourceGroups: [],
			timeBasedResourceGroups: [],
		},
		{ oneTimeResourceGroups: [], timeBasedResourceGroups: [] },
		{
			deniedResourceGroups: [1, 2],
			oneTimeResourceGroups: [],
			timeBasedResourceGroups: [],
		},
		{},
...
test("two interleaved flows for different tokens do not cross-talk: each storeTokens only ever sees its own runInventoryOrTest's context", async () => {
	...
	try {
		const tokensA = {
			accessToken: "ya29.flow-a",
			refreshToken: "refresh-a",
			tokenKind: "Bearer",
		};
		const tokensB = {
			accessToken: "ya29.flow-b",
		...
		assert.notEqual(
			...
			bundleB.authorized_resource_groups,
		);
