...
function requireManifestUrl(
	manifest: ProviderAuthManifestLike,
	field: "authorization_url" | "token_url",
): string {
...
function manifestResourceGroups(
	manifest: ProviderAuthManifestLike,
): readonly string[] {
...
function scopesForResourceGroups(
	resourceGroups: readonly string[],
): readonly string[] {
	return resourceGroups.map(
		(resourceGroup) =>
			`https://www.googleapis.com/auth/dataportability.${resourceGroup}`,
	);
}

function tokenAccountFingerprint(
 ...
 */
function readAccessTypeSnapshot(
	context: ProviderAuthPersistenceContext | undefined,
...
export const GOOGLE_DATA_PORTABILITY_EXCHANGER_KIND =
