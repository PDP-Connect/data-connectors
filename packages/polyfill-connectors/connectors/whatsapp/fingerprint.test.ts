...
test("fresh run: all chats and messages emit (no prior cursor)", () => {
	...
	assert.equal(
		chatsCursor.shouldEmit(CHAT_A),
		true,
		"CHAT_A emits on fresh run",
	);
	assert.equal(
		chatsCursor.shouldEmit(CHAT_B),
		true,
		"CHAT_B emits on fresh run",
	);

	const msgsCursor = openFingerprintCursor(undefined);
	assert.equal(
		msgsCursor.shouldEmit(MSG_A1),
		true,
		"MSG_A1 emits on fresh run",
	);
	assert.equal(
		msgsCursor.shouldEmit(MSG_A2),
		true,
		"MSG_A2 emits on fresh run",
	);
...
test("identical second run: all records suppressed (fingerprint match)", () => {
	...
	assert.equal(
		...
		false,
		"CHAT_A not re-emitted — unchanged",
	);
	assert.equal(
		chatsCursor2.shouldEmit(CHAT_B),
		false,
		"CHAT_B not re-emitted — unchanged",
	);

	const msgsCursor2 = openFingerprintCursor(msgsState1);
	assert.equal(
		msgsCursor2.shouldEmit(MSG_A1),
		false,
		"MSG_A1 not re-emitted — unchanged",
	);
	assert.equal(
		msgsCursor2.shouldEmit(MSG_A2),
		false,
		"MSG_A2 not re-emitted — unchanged",
	);
...
test("changed record re-emits; unchanged records are still suppressed", () => {
	...
	const chatsCursor2 = openFingerprintCursor(chatsState1);
	assert.equal(
		chatsCursor2.shouldEmit(CHAT_A_UPDATED),
		true,
		"changed chat re-emits",
	);
	assert.equal(
		chatsCursor2.shouldEmit(CHAT_B),
		false,
		"unchanged chat still suppressed",
	);
});
...
test("pruneStale: chat absent from this run is dropped from next STATE", () => {
	...
	const chatsState1 = { fingerprints: chatsCursor1.toState() };
	assert.equal(
		Object.keys(chatsState1.fingerprints).length,
		2,
		"two fingerprints after run 1",
	);
	...
	assert.equal(
		chatsState2.fingerprints[CHAT_B.id],
		undefined,
		"CHAT_B fingerprint pruned",
	);
	assert.equal(
		Object.keys(chatsState2.fingerprints).length,
		1,
		"only one fingerprint survives",
	);
});
...
test("no run-clock exclusion needed: record is deterministic across runs (no fetched_at)", () => {
	...
	assert.equal(
		cursor2.shouldEmit(MSG_A1),
		false,
		"deterministic record suppressed without clock exclusion",
	);
});
