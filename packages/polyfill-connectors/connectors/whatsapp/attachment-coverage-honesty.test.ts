...
test("WhatsApp attachments - media that fails to extract reads as a coverage shortfall", async () => {
	...
	try {
		...
		const emittedAttachments = result.messages.filter(
			(message) =>
				message.type === "RECORD" && message.stream === "attachments",
		);
		const coverage = attachmentCoverage(result.messages);

		assert.equal(
			emittedAttachments.length,
			1,
			"only the readable media file can be emitted",
		);
		assert.equal(
			coverage?.considered,
			2,
			"both media files were discovered in the export",
		);
		assert.equal(
			coverage?.covered,
			1,
			"the unreadable media file must not count as covered",
		);
		assert.ok(
		...
		const skipResults = result.messages.filter(
			(message) =>
				message.type === "SKIP_RESULT" && message.stream === "attachments",
		);
		assert.equal(
			skipResults.length,
			1,
			"the skipped media file is still disclosed as a SKIP_RESULT",
		);
	} finally {
...
test("WhatsApp attachments - a fully readable export still proves full coverage", async () => {
	...
	try {
		...
		assert.equal(
			coverage?.covered,
			2,
			"no media was dropped, so coverage stays complete",
		);
	} finally {
		await rm(importRoot, { force: true, recursive: true });
	}
