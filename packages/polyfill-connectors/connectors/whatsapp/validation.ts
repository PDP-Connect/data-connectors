...
export type WhatsAppChatExportValidationStatus =
	| "valid"
	| "duplicate"
	| "empty"
	| "unsupported"
...
function remediationFor(
	status: WhatsAppChatExportValidationStatus,
): string | null {
	switch (status) {
		case "duplicate":
...
function baseValidation(
	fileSha256: string,
): Omit<WhatsAppChatExportValidation, "remediation" | "status"> {
	return {
...
function buildValidationFromSummary(
	...
	return {
		...
		estimated_records:
			summary.messageCount + (summary.messageCount > 0 ? 1 : 0),
		file_sha256: fileSha256,
		media_coverage: {
		...
		source_identity:
			summary.messageCount > 0
				? {
						...
						suggested_display_name: summary.title
...
function parseZipArtifactSummary(
	fd: number,
	fileSize: number,
): ParsedArtifactSummary | "too_large" | null {
