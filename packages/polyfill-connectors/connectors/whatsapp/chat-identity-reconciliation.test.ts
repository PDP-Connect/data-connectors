...
test("two DISTINCT chats sharing the same participant list stay distinct, not silently merged", async () => {
	const importRoot = await mkdtemp(
		join(tmpdir(), "pdpp-whatsapp-same-participants-"),
	);
	try {
		...
		assert.equal(
			chats.length,
			2,
			"expected two distinct chat records, not one merged chat",
		);
		...
		assert.equal(
			chatIds.size,
			2,
			`expected two distinct chatIds, got: ${JSON.stringify([...chatIds])}`,
		);
		...
		const chatIdsForTripMessages = new Set(
			msgs
				.filter((m) => String(m.content).includes("trip"))
				.map((m) => m.chat_id),
		);
		const chatIdsForReportMessages = new Set(
			msgs
				.filter((m) => String(m.content).includes("report"))
				.map((m) => m.chat_id),
		);
		assert.equal(chatIdsForTripMessages.size, 1);
		assert.equal(chatIdsForReportMessages.size, 1);
		assert.notEqual(
			[...chatIdsForTripMessages][0],
			[...chatIdsForReportMessages][0],
		);
		...
		assert.match(
			texts,
			/shares its participant list with a different, already-imported chat/i,
		);
	} finally {
...
test("a genuine re-export of the SAME chat (overlapping content, different filename) reuses the same chatId across runs", async () => {
	const importRootRun1 = await mkdtemp(
		join(tmpdir(), "pdpp-whatsapp-reexport-1-"),
	);
	const importRootRun2 = await mkdtemp(
		join(tmpdir(), "pdpp-whatsapp-reexport-2-"),
	);
	try {
		...
		assert.equal(
			run2ChatId,
			run1ChatId,
			"a genuine re-export with overlapping content must reuse the same chatId",
		);
...
test("alias-list growth per identityKey is capped, not unbounded (H4)", async () => {
	...
	try {
		...
		for (let i = 0; i < totalDistinctChats; i += 1) {
			const importRoot = await mkdtemp(
				join(tmpdir(), `pdpp-whatsapp-alias-cap-${i}-`),
			);
		...
		const finalChatState = state?.messages as
			| { chat_identity?: { aliases?: Record<string, unknown[]> } }
			| undefined;
		const aliasLists = Object.values(
			finalChatState?.chat_identity?.aliases ?? {},
		);
		assert.equal(
			aliasLists.length,
			1,
			"expected exactly one identityKey (one shared participant set)",
		);
		...
		assert.ok(
			lastChat,
			"expected the chat past the alias cap to still be imported, not dropped",
		);
		...
		await Promise.all(
			importRoots.map((root) => rm(root, { force: true, recursive: true })),
		);
	}
});
...
test("two distinct chats sharing participants, seen across SEPARATE runs (not one run), still stay distinct", async () => {
	const importRootRun1 = await mkdtemp(
		join(tmpdir(), "pdpp-whatsapp-ambiguous-1-"),
	);
	const importRootRun2 = await mkdtemp(
		join(tmpdir(), "pdpp-whatsapp-ambiguous-2-"),
	);
	try {
		await writeFile(
		...
		assert.notEqual(
			run2ChatId,
			run1ChatId,
			"a distinct chat across separate runs must NOT be merged via STATE",
		);
