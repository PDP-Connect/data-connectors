...
test("static guard: index.ts's collection path does not import node:fs/promises readFile (whole-file read)", async () => {
	...
	const fsPromisesImportLines = source
		...
		.filter(
			(line) =>
				line.includes('from "node:fs/promises"') ||
				line.includes("from 'node:fs/promises'"),
		);
	assert.ok(
		fsPromisesImportLines.length > 0,
		"expected at least one node:fs/promises import (readdir)",
	);
...
test("a large synthetic WhatsApp .txt export (well past the old 1 GiB cap) parses and emits with bounded RSS", async () => {
	...
	const importRoot = await mkdtemp(
		join(largeFixtureBaseDir(), "pdpp-whatsapp-large-txt-"),
	);
	try {
		...
		await writeFile(
			join(importRoot, "WhatsApp Chat - Large.txt"),
			parts.join(""),
		);
...
test("outcome proof (H3): a zip export with 250 MiB of media attachments never pushes peak RSS past 450 MiB", async () => {
	...
	const importRoot = await mkdtemp(
		join(largeFixtureBaseDir(), "pdpp-whatsapp-heap-ceiling-zip-"),
	);
	try {
		const chatText = [
		...
		const entries = [
			{ data: Buffer.from(chatText, "utf8"), name: "WhatsApp Chat - Heap.txt" },
		];
		for (let i = 0; i < 5; i += 1) {
			entries.push({
				data: Buffer.alloc(mediaEntrySize, i + 1),
				name: `IMG-2024010${i}-WA000${i}.jpg`,
			});
		}
		...
		const attachmentRecords = result.messages.filter(
			(m): m is Extract<typeof m, { type: "RECORD" }> =>
				m.type === "RECORD" && m.stream === "attachments",
		);
