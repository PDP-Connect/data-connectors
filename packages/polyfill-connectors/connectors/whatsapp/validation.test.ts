...
test("validateWhatsAppChatExportArtifact reports messages, participants, media, and range", () => {
	const validation = validateWhatsAppChatExportArtifact(VALID_EXPORT, {
	...
	assert.equal(
		validation.source_identity?.suggested_display_name,
		"WhatsApp - Alice",
	);
	assert.deepEqual(validation.source_identity?.participant_preview, [
		"Alice",
		"Bob",
	]);
...
test("validateWhatsAppChatExportArtifact accepts zip exports with media present", () => {
	...
	const validation = validateWhatsAppChatExportArtifact(zip, {
...
test("validateWhatsAppChatExportArtifact rejects malformed zip input without throwing", () => {
	const malformedZip = Buffer.concat([
		Buffer.from("PK\u0003\u0004", "binary"),
		Buffer.from("not a usable zip"),
	]);

	const validation = validateWhatsAppChatExportArtifact(malformedZip, {
		fileName: "WhatsApp Chat - Alice.zip",
	});
...
test("validateWhatsAppChatExportArtifact rejects unsupported and too-large artifacts", () => {
	...
	const tooLarge = validateWhatsAppChatExportArtifact(VALID_EXPORT, {
		maxFileBytes: 4,
	});
...
test("a zip declaring far more entries than the policy allows classifies as too_large, NOT unsupported", () => {
	...
	const entries = Array.from({ length: 25_000 }, (_, i) => ({
		data: "x",
		name: `media-${i}.jpg`,
	}));
...
test("validateWhatsAppChatExportArtifactFromFile: .txt produces the same validation as the buffer-backed API", async () => {
	...
	const bufferResult = validateWhatsAppChatExportArtifact(VALID_EXPORT, {
		fileName: "WhatsApp Chat - Alice.txt",
	});
	await withTempFile(
		"WhatsApp Chat - Alice.txt",
		bytes,
		async (path, fd, size) => {
			const fileResult = await validateWhatsAppChatExportArtifactFromFile(
				fd,
				path,
				size,
				{
			...
			assert.equal(
				fileResult.estimated_messages,
				bufferResult.estimated_messages,
			);
			assert.equal(
				fileResult.estimated_participants,
				bufferResult.estimated_participants,
			);
			...
			assert.equal(
				fileResult.source_identity?.stable_id,
				bufferResult.source_identity?.stable_id,
			);
		},
	);
});
...
test("validateWhatsAppChatExportArtifactFromFile: .zip produces the same validation as the buffer-backed API", async () => {
	...
	const bufferResult = validateWhatsAppChatExportArtifact(zip, {
		fileName: "WhatsApp Chat - Alice.zip",
	});
	await withTempFile(
		"WhatsApp Chat - Alice.zip",
		zip,
		async (path, fd, size) => {
			const fileResult = await validateWhatsAppChatExportArtifactFromFile(
				fd,
			...
			assert.equal(
				fileResult.media_coverage.attached_media_files,
				bufferResult.media_coverage.attached_media_files,
			);
		},
	);
});
...
test("validateWhatsAppChatExportArtifactFromFile: empty .txt file is 'empty', not 'unsupported' (size 0 short-circuit)", async () => {
	await withTempFile("empty.txt", Buffer.alloc(0), async (path, fd, size) => {
		const result = await validateWhatsAppChatExportArtifactFromFile(
			fd,
			path,
			size,
			{
...
test("validateWhatsAppChatExportArtifactFromFile: export-shaped .txt with zero real messages is 'empty', not 'unsupported'", async () => {
	...
	const impossibleDateOnly =
		"[31/02/24, 9:16:00 AM] Bob: impossible date only, no valid messages";
	await withTempFile(
		"impossible.txt",
		Buffer.from(impossibleDateOnly, "utf8"),
		async (path, fd, size) => {
			const result = await validateWhatsAppChatExportArtifactFromFile(
				fd,
				path,
...
test("validateWhatsAppChatExportArtifactFromFile: unrecognizable .txt content is 'unsupported'", async () => {
	await withTempFile(
		"random.txt",
		Buffer.from(
			"just some random text with no date-like structure at all",
			"utf8",
		),
		async (path, fd, size) => {
			const result = await validateWhatsAppChatExportArtifactFromFile(
...
test("validateWhatsAppChatExportArtifactFromFile: a .zip export past WHATSAPP_MAX_MESSAGE_COUNT is 'too_large', not a crash (H1)", async () => {
	...
	try {
		...
		const zip = makeStoredZip([
			{ name: "WhatsApp Chat - Alice.txt", data: lines },
		]);
...
test("countertest: validateWhatsAppChatExportArtifactFromFile's .txt path allocates meaningfully less than a whole-buffer read of the same file", async (t) => {
	...
	const oneMessage =
		"[6/5/24, 9:15:22 AM] Alice: Hello there, this is a test message with some bulk padding text.\n";
	...
	try {
		...
		const bufferResult = validateWhatsAppChatExportArtifact(wholeBuffer, {
			fileName: "bulk.txt",
		});
		...
		try {
			...
			const streamResult = await validateWhatsAppChatExportArtifactFromFile(
				readFd,
				path,
				written,
				{
			...
			assert.equal(
				streamResult.estimated_messages,
				bufferResult.estimated_messages,
			);
