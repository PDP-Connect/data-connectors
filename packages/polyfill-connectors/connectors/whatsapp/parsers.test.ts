...
function parseWhatsAppChatFileForTest(
	...
	const summary = scanWhatsAppChatIdentity(
		filename,
		splitWhatsAppChatLines(content),
	);
...
test("parseWhatsAppChatFile: an unparseable date folds into the prior message, never the run clock", () => {
	...
	const parsed = parseWhatsAppChatFileForTest(
		...
		[
			...
			"[31/02/24, 9:16:00 AM] Bob: impossible date",
		].join("\n"),
	);
	...
	for (const message of parsed.messages) {
		assert.ok(
			...
			`sent_at ${message.sent_at} looks like ingest time`,
		);
	}
});
...
test("parseWhatsAppChatFile: parses well-formed lines into one record each", () => {
	const parsed = parseWhatsAppChatFileForTest(
		"WhatsApp Chat with Alice.txt",
		[
			"[6/5/24, 9:15:22 AM] Alice: first",
			"[6/5/24, 9:16:00 AM] Bob: second",
		].join("\n"),
	);
...
test("chatId is stable across a renamed re-export of the same chat", () => {
	const original = parseWhatsAppChatFileForTest(
		"WhatsApp Chat with Alice.txt",
		EXPORT_TEXT,
	);
	const renamed = parseWhatsAppChatFileForTest(
	...
	assert.equal(
		renamed.chatId,
		original.chatId,
		"chatId must not depend on the export filename",
	);
});
...
test("message id is stable when a re-export prepends earlier history (index would shift, content hash does not)", () => {
	...
	const shiftedFirst = withPrependedHistory.messages.find(
		(m) => m.content === "first",
	);
	...
	assert.equal(
		shiftedFirst?.id,
		originalFirst?.id,
		"message id must not depend on array position",
	);
});
...
test("message id changes if the message content changes, given identical author/time", () => {
	...
	const edited = parseWhatsAppChatFileForTest(
		"export.txt",
		[
			"[6/5/24, 9:15:22 AM] Alice: first EDITED",
			"[6/5/24, 9:16:00 AM] Bob: second",
		].join("\n"),
	);
	const originalFirst = parsed.messages.find((m) => m.content === "first");
	const editedFirst = edited.messages.find((m) =>
...
test("duplicate consecutive messages (same author/time/content) get distinct ids, not a collision", () => {
	...
	assert.notEqual(
		first?.id,
		second?.id,
		"true duplicates must still get distinct, deterministic ids",
	);
});
	...
	try {
		...
		assert.throws(
			...
			(err: unknown) => {
				...
				assert.match(
					err.message,
					/exceeds the maximum supported message count/,
				);
				return true;
			},
		);
...
test("pass 1 (scanWhatsAppChatIdentity): exactly messageCount === cap is accepted, cap+1 throws", async () => {
	...
	try {
		...
		const summary = scanWhatsAppChatIdentity(
			"export.txt",
			splitWhatsAppChatLines(atCap),
		);
		assert.equal(
			summary.messageCount,
			3,
			"exactly at cap is accepted, not rejected",
		);
...
test("pass 2 (streamWhatsAppChatMessagesAsync, the real production call path): exactly messageCount === cap is accepted, cap+1 throws", async () => {
	...
	try {
		...
		await streamWhatsAppChatMessagesAsync(
			splitWhatsAppChatLines(atCap),
			"chat123",
			(m) => {
		...
		await assert.rejects(
			() =>
				streamWhatsAppChatMessagesAsync(
					splitWhatsAppChatLines(overCap),
					"chat123",
					() => {
						/* intentionally empty */
					},
	...
	try {
		...
		streamWhatsAppChatMessages(
			...
			(message) => {
				...
				concurrentlyAlive += 1;
				peakConcurrentlyAlive = Math.max(
					peakConcurrentlyAlive,
					concurrentlyAlive,
				);
		...
		assert.equal(
			peakConcurrentlyAlive,
			1,
			"streamWhatsAppChatMessages must hand off exactly one message at a time",
		);
	} finally {
		if (originalCap === undefined) {
			delete process.env.WHATSAPP_MAX_MESSAGE_COUNT;
...
test("bounded-memory oracle: streamWhatsAppChatMessages emits INTERLEAVED with line consumption, not buffer-then-drain-at-end", () => {
	...
	try {
		...
		assert.equal(totalSeen, 2_000_000);
		assert.ok(
			firstOnMessageAtLinesPulled !== null &&
				firstOnMessageAtLinesPulled < 2_000_000,
