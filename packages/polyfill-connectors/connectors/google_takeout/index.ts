...
const PHOTO_EXTENSIONS_RE =
	/\.(jpg|jpeg|png|gif|bmp|webp|mp4|mov|mts|m4v|3gp|3g2|wmv|avi|mkv|flv|webm)$/i;
...
async function collectLocationHistory(
	...
	if (!json?.locations) {
		await emit({
			...
			message:
				"Google Takeout location records were not found in the configured import directory",
		});
		return;
	}
...
async function collectYoutubeWatchHistory(
	...
	if (!Array.isArray(json)) {
		await emit({
			...
			message:
				"Google Takeout watch history was not found in the configured import directory",
		});
		return;
	}
	const since = streamState?.last_timestamp;
...
async function collectSearchHistory(
	...
	if (!Array.isArray(json)) {
		await emit({
			...
			reason: "history_not_found",
			message:
				"Google Takeout search history was not found in the configured import directory",
		});
		return;
	}
	const since = streamState?.last_timestamp;
...
async function resolveSidecarMetadata(
	...
	return (await readJsonIf(
		join(file.dir, sidecarName),
	)) as PhotoMetadataFile | null;
}
...
async function buildPhotoRecordForFile(
	...
	const base = buildPhotoRecord(
		file.name,
		ts,
		hydration.contentSha256,
		metadata,
	);
	return {
		...base,
...
async function emitPhotos(
	...
	for (const file of mediaEntries) {
		processedItems += 1;
		const record = await buildPhotoRecordForFile(
			file,
			jsonFilenamesByDir,
			maxBytes,
		);
...
async function collectPhotos(
	...
	if (!existsSync(photosDir)) {
		await emit({
			...
			message:
				"Google Takeout Photos directory was not found in the configured import directory",
		});
		return;
	}
...
runConnector({
	...
	async collect(ctx) {
		...
		if (ctx.requested.has("youtube_watch_history")) {
			await collectYoutubeWatchHistory(
				ctx,
				importDir,
				typedState.youtube_watch_history,
			);
		}
