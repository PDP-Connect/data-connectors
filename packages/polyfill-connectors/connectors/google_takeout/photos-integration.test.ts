...
test("photos stream discovers files, matches sidecars, and skips unsupported files", async () => {
	...
	try {
		...
		await writeFile(
			...
			JSON.stringify({
				title: "Sunset",
				photoTakenTime: { timestamp: "1717600000" },
			}),
		);
		...
		assert.equal(
			photos.length,
			3,
			"IMG_0001.jpg, IMG_0001-edited.jpg, and zzz_no_sidecar.png are all discovered",
		);
		...
		const editedVariant = photos.find(
			(p) => p.filename === "IMG_0001-edited.jpg",
		);
...
test("photos stream collapses duplicate album copies of the same photo to one record via content hash", async () => {
	...
	try {
		...
		await writeFile(
			join(albumA, "IMG_9999.jpg.json"),
			JSON.stringify({ creationTime: { timestamp: "1717600000" } }),
		);
		await writeFile(join(albumB, "IMG_9999.jpg"), TINY_JPEG_A);
		await writeFile(
			join(albumB, "IMG_9999.jpg.json"),
			JSON.stringify({ creationTime: { timestamp: "1717600100" } }),
		);
		...
		assert.equal(
			ids.size,
			1,
			"identical bytes in two album folders collapse to one record id",
		);
	} finally {
		await rm(importRoot, { force: true, recursive: true });
	}
...
test("photos stream reports directory-read failures without leaking the local path", async () => {
	...
	try {
		...
		assert.doesNotMatch(
			skips[0]?.message ?? "",
			new RegExp(importRoot.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
		);
	} finally {
		await rm(importRoot, { force: true, recursive: true });
	}
...
test("photos stream marks hydration failed (not fabricated deletion) when blob upload fails", async () => {
	const importRoot = await mkdtemp(
		join(tmpdir(), "pdpp-takeout-hydrate-fail-"),
	);
	try {
		const photosDir = join(importRoot, "Photos", "Photos from 2024");
		await mkdir(photosDir, { recursive: true });
		...
		await withBlobServer(
			...
			async (baseUrl) => {
				...
				assert.equal(
					photos.length,
					1,
					"the record is still emitted even though hydration failed",
				);
				...
				assert.doesNotMatch(
					String(photos[0]?.hydration_error),
					/IMG_2222|Photos from 2024/,
				);
			},
		);
	} finally {
