...
async function makeSyntheticProjectTree(): Promise<{
	baseDir: string;
...
test("scanProjectDirs: pass 1 (buildOnly=true) populates accumulators but emits no child records", async () => {
	...
	try {
		...
		assert.equal(
			harness.emitted.filter((r) => r.stream === "messages").length,
			0,
			"buildOnly=true emits no messages",
		);
		assert.equal(
...
test("scanProjectDirs: full two-pass — sessions emit BEFORE messages (parent-first end-to-end)", async () => {
	...
	try {
		...
		const firstSession = harness.emitted.findIndex(
			(r) => r.stream === "sessions",
		);
		const firstMessage = harness.emitted.findIndex(
			(r) => r.stream === "messages",
		);
		const firstAttachment = harness.emitted.findIndex(
		...
		assert.ok(
			firstSession < firstAttachment,
			"sessions must precede attachments",
		);

		// Exactly one of each child type — no double-emit from pass 1.
		const sessionCount = harness.emitted.filter(
			(r) => r.stream === "sessions",
		).length;
		const messageCount = harness.emitted.filter(
			(r) => r.stream === "messages",
		).length;
...
test("scanProjectDirs: unchanged file mtime skips child records on an incremental pass", async () => {
	...
	try {
		...
		assert.equal(
			firstPass.emitted.filter((r) => r.stream === "messages").length,
			3,
			"first pass emits messages",
		);
		assert.equal(
			Object.keys(newMtimes).length,
			2,
			"top-level and recursive subagent mtimes captured",
		);
		...
		assert.equal(
			secondPass.emitted.length,
			0,
			"unchanged mtimes skip re-emitting child records",
		);
	} finally {
		await cleanup();
	}
...
test("scanProjectDirs: session aggregation can backfill independently from message mtimes", async () => {
	...
	try {
		...
		assert.equal(
			...
			2,
			"message cursor captured JSONL mtimes",
		);
...
test("scanProjectDirs: one top-level agent JSONL can emit summaries for each per-line session id", async () => {
	...
	try {
		...
		const byIdThenCount = (a: [string, number], b: [string, number]): number =>
...
test("scanProjectDirs: an unchanged session does NOT re-emit a sessions record on a later run", async () => {
	...
	try {
		...
		assert.equal(
			run2Accumulators.size,
			0,
			"unchanged session file is skipped — no accumulator rebuilt",
		);
		assert.equal(
...
test("scanProjectDirs: a grown session re-emits once with the higher message_count (real change, not no-op)", async () => {
	...
	try {
		...
		assert.equal(
			firstSession?.data.message_count,
			1,
			"run 1 session has one message",
		);
		...
		assert.equal(
			sessions2[0]?.data.message_count,
			2,
			"re-emitted session carries the higher message_count",
		);
		assert.equal(
			sessions2[0]?.data.last_event_at,
			"2026-04-23T10:05:00Z",
			"last_event_at advances to the new line",
		);
	} finally {
		await rm(root, { recursive: true, force: true });
	}
...
test("scanProjectDirs: three-stream bounded proof — messages + attachments + memory_notes without sessions", async () => {
	...
	try {
		...
		const memoryNotes = harness.emitted.filter(
			(r) => r.stream === "memory_notes",
		);
		const sessions = harness.emitted.filter((r) => r.stream === "sessions");
		...
		assert.ok(
			...
			"memory_notes should emit when requested",
		);
		assert.equal(
			sessions.length,
			0,
			"sessions must not emit when not requested",
		);
	} finally {
		await cleanup();
	}
