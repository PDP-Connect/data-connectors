...
async function runClaudeCode(
	...
	for (const message of result.messages) {
		if (
			message.type === "STATE" &&
			typeof (message as { stream?: string }).stream === "string"
		) {
			carried[(message as { stream: string }).stream] = (
				message as { cursor?: unknown }
			).cursor;
		}
	}
	return {
...
test("a skill whose content changes under a restored mtime is re-emitted", async () => {
	...
	try {
		...
		assert.equal(
			...
			1,
			"the skill should be emitted on a fresh run",
		);
		...
		assert.equal(
			...
			1,
			"changed content under an identical mtime must re-emit, not be skipped",
		);
	} finally {
...
test("an unchanged skill stays suppressed even when its mtime moves", async () => {
	...
	try {
		...
		assert.equal(
			second.records.length,
			0,
			"identical content must stay suppressed regardless of mtime",
		);
	} finally {
		await rm(home, { force: true, recursive: true });
	}
...
test("a slash command whose content changes under a restored mtime is re-emitted", async () => {
	...
	try {
		...
		assert.equal(
			second.records.length,
			1,
			"slash_commands must use the same content gate as skills",
		);
	} finally {
		await rm(home, { force: true, recursive: true });
	}
