...
const FIXTURE_ROOT = join(
	import.meta.dirname,
	"../../fixtures/claude_code/source-home",
);
...
test("claude_code fixture home: coverage diagnostics classify every known store", async () => {
	...
	assert.equal(
		contextMode?.data.stream,
		null,
		"context_mode must not map to a requestable stream",
	);
...
test("claude_code fixture home: risky-store secret sentinels are never emitted", async () => {
	...
	for (const sentinel of SECRET_SENTINELS) {
		assert(
			...
			`sentinel leaked into connector output: ${sentinel}`,
		);
	}
});
...
test("claude_code fixture home: inventory-only stores emit metadata without payload", async () => {
	...
	const cacheRec = recs.find(
		(r) => r.stream === "cache_inventory" && r.data.relative_path === "cache",
	);
	assert(
		cacheRec,
		"cache_inventory should emit a metadata record for the cache dir",
	);
	assert.equal(cacheRec.data.classification, "inventory_only");
	assert.equal(typeof cacheRec.data.path_hash, "string");
	assert(
		!("content" in cacheRec.data),
		"inventory records must not carry payload content",
	);
	...
	assert(
		recs.some(
			(r) =>
				r.stream === "file_history" &&
				r.data.relative_path === "file-history/snapshot.json",
		),
...
test("claude_code inventory: unchanged store does not re-version across runs", async () => {
	...
	const run1 = await runFixtureConnector({
		home: DEVICE_A_HOME,
		streams: [{ name: "backup_inventory" }],
	});
	assert.equal(run1.exitCode, 0);
	const run1Records = records(run1.messages).filter(
		(r) => r.stream === "backup_inventory",
	);
	...
	const cursor = run1States.at(-1)?.cursor as
		| { fingerprints?: Record<string, string> }
		| undefined;
	assert(
		cursor?.fingerprints && Object.keys(cursor.fingerprints).length > 0,
		"STATE carries inventory fingerprints",
	);
	...
	const run2Records = records(run2.messages).filter(
		(r) => r.stream === "backup_inventory",
	);
	assert.equal(
		run2Records.length,
		0,
		"unchanged inventory store does not re-emit on the second run",
	);
	...
	assert(
		run2States.length > 0,
		"second run still writes the carry-forward STATE",
	);
});

test("claude_code fixture homes: two device homes inventory independently", async () => {
	const a = await runFixtureConnector({
		home: DEVICE_A_HOME,
		streams: [{ name: "skills" }],
	});
	const b = await runFixtureConnector({
		home: DEVICE_B_HOME,
		streams: [{ name: "skills" }],
	});
	...
	assert(
		JSON.stringify(bSkills).includes("Device B"),
		"device B skill should reflect device B content",
	);
	assert(
		!JSON.stringify(aSkills).includes("Device B"),
		"device A must not see device B content",
	);
});
