...
async function runSkills(home: string): Promise<{
	...
	const result = await runConnectorProtocolSubprocess({
		...
		start: {
			scope: { streams: [{ name: "skills" }] },
			state: {},
			type: "START",
		},
	});
	return {
		records: result.messages.filter(
			(m) => m.type === "RECORD" && m.stream === "skills",
		).length,
		states: result.messages.filter(
			(m) => m.type === "STATE" && m.stream === "skills",
		).length,
		unreadable: result.messages.filter(
			(m) =>
				m.type === "SKIP_RESULT" &&
				m.stream === "skills" &&
				m.reason === "source_unreadable",
		).length,
	};
}

test("an unreadable skills directory does not checkpoint an empty cursor", {
	...
	try {
		...
		assert.equal(
			outcome.records,
			0,
			"no records can be read from an unreadable directory",
		);
		assert.equal(
			outcome.unreadable,
			1,
			"the failed enumeration must surface as a skip, not silence",
		);
	} finally {
...
test("a genuinely missing skills directory is still an honest empty enumeration", async () => {
	...
	try {
		...
		assert.equal(
			outcome.unreadable,
			0,
			"an absent directory is legitimate, not a source failure",
		);
