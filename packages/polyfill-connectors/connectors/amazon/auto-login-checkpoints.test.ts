...
function makeFakePage(state: FakePageState): Page {
	const page: FakePageShape = {
		url: () =>
			state.urls[Math.min(state.idx, state.urls.length - 1)] ?? "about:blank",
		goto: () => {
		...
		locator: (selector: string) =>
			makeFakeLocator(selector) as ReturnType<Page["locator"]>,
	};
	return page as Page;
}
...
test("ensureAmazonSession invokes the checkpoint hook at each auth phase (non-2FA path)", async () => {
	...
	for (const label of expectedOrder) {
		assert.ok(
			checkpoints.includes(label),
			`missing checkpoint: ${label} (saw: ${checkpoints.join(", ")})`,
		);
	}
