...
const LOCAL_RAW_DIR = join(
	...
	"amazon",
	"raw",
	"2026-04-23T00-13-01-167Z",
	"dom",
);
const SCRUBBED_FIXTURE_DIR = join(
	__dirname,
	"..",
	"..",
	"fixtures",
...
test("itemId: prefers ASIN when present", () => {
	assert.equal(
		itemId("111-2222222-3333333", { asin: "B01ABCDEFG", name: "Widget" }),
		"111-2222222-3333333|B01ABCDEFG",
	);
});
...
test("itemId: falls back to 'unknown' when both ASIN and name are absent/empty", () => {
	assert.equal(
		itemId("111-2222222-3333333", {}),
		"111-2222222-3333333|unknown",
	);
	assert.equal(
		itemId("111-2222222-3333333", { asin: null, name: "" }),
		"111-2222222-3333333|unknown",
	);
	// Whitespace-only name collapses to empty -> falsy -> "unknown"
	assert.equal(
		itemId("111-2222222-3333333", { name: "   " }),
		"111-2222222-3333333|unknown",
	);
});
...
test("parseOrdersListDom: scrubbed real-shape fixture extracts full orders", () => {
	const html = readFileSync(
		join(SCRUBBED_FIXTURE_DIR, "orders-list-2026.html"),
		"utf8",
	);
	...
	assert.equal(
		o.deliveryStatus,
		"Delivered April 20 to [REDACTED_NAME] at [REDACTED_ADDRESS]",
	);
...
test("parseOrdersListDom: empty page returns []", () => {
	assert.deepEqual(
		parseOrdersListDom(
			"<!doctype html><html><body><div>no orders</div></body></html>",
		),
		[],
	);
...
test("countOrderCardsWithoutOrderId: counts a card with no .yohtmlc-order-id that parseOrdersListDom silently drops", () => {
	...
	assert.equal(
		orders.length,
		1,
		"only the normal card survives parseOrdersListDom",
	);
...
test("parseOrdersListDom: cancelled order (no items, no total) parses alongside a normal order on the same page", () => {
	...
	assert.equal(
		orders.length,
		2,
		"both the cancelled card and the normal card must be extracted",
	);
...
test("parseOrderDetailDom: synthetic-minimal fixture parses full OrderDetail", () => {
	...
	assert.equal(
		...
		"Fictional Person, 123 Placeholder Ln, Fakeville, TX 00000",
	);
...
test("parseOrderDetailDom: fopo Whole Foods detail fixture parses alternate detail shape", () => {
	...
	assert.equal(
		d.shipping_address_summary,
		"Whole Foods Market, 123 Placeholder Rd, Fakeville, TX 00000",
	);
	assert.equal(
		d.status_detail,
		"Purchased at Whole Foods Market - Fictional Store",
	);
	assert.equal(d.gift_order, false);
	assert.equal(d.digital_order, false);
...
test("parseOrderDetailDom: ufpo grocery detail fixture parses current alternate detail shape", () => {
	...
	assert.equal(
		d.status_detail,
		"Delivered today · Arrived during the selected window",
	);
...
test("parseOrderDetailDom: shipping address <li>-only fallback (no inner span)", () => {
	...
	assert.equal(
		d.shipping_address_summary,
		"Jane Doe, 456 Main St, Oakland, CA 94607",
	);
});
...
test("parseOrderDetailDom: missing #orderDetails container returns null", () => {
	assert.equal(
		parseOrderDetailDom(
			"<!doctype html><html><body><div>no details</div></body></html>",
		),
		null,
	);
...
test("parseOrderDetailDom: #orderDetails present but no data-components returns empty-ish OrderDetail", () => {
	...
	const html =
		'<!doctype html><html><body><div id="orderDetails"></div></body></html>';
	const d = parseOrderDetailDom(html);
	assert.ok(d);
...
test("parseOrderDetailDom: local real fixtures yield items and grand_total", {
	...
	for (const name of [
		"order-detail-111-1177311-6377828.html",
		"order-detail-111-2841132-0656246.html",
	]) {
