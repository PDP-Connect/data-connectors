...
test("workspace: fetched_at is excluded — same source state across runs does NOT re-emit", async () => {
	...
	assert.equal(
		run2.emitted.length,
		0,
		"unchanged record does not re-emit when only fetched_at moved",
	);
...
test("wiring: channel_memberships is in the production fingerprinted set with fetched_at excluded", () => {
	...
	assert.ok(
		(FINGERPRINTED_STREAMS as readonly string[]).includes(
			"channel_memberships",
		),
...
test("channel_memberships: fetched_at is excluded — a stable membership does NOT re-emit", async () => {
	...
	const run2 = makeHarness({ channel_memberships: priorState }, [
		"channel_memberships",
	]);
	...
	assert.equal(
		run2.emitted.length,
		0,
		"an unchanged membership does not re-emit when only fetched_at moved",
	);
});
...
test("channel_memberships: a newly-added membership DOES emit", async () => {
	...
	assert.equal(
		run2.emitted.length,
		1,
		"only the newly-added membership re-emits",
	);
...
test("channel_memberships: a removed membership is pruned from carry-forward", async () => {
	...
	const run = makeHarness({ channel_memberships: prior }, [
		"channel_memberships",
	]);
	...
	assert.equal(
		post["C1:U1"] !== undefined,
		true,
		"still-present membership retained",
	);
...
test("skipped records still appear in the cursor — STATE carry-forward intact", async () => {
	...
	const run2 = makeHarness({
		users: run1.cursors.get("users")?.toState() ?? {},
	});
	await emitWithFingerprint(run2.deps, "users", userA);
...
test("prune: prior IDs absent from the current run are removed when the stream was requested", async () => {
	...
	await emitWithFingerprint(run1.deps, "users", userB);
	assert.equal(
		Object.keys(run1.cursors.get("users")?.toState() ?? {}).length,
		2,
	);
	...
	assert.equal(
		Object.keys(run2.cursors.get("users")?.toState() ?? {}).length,
		2,
		"carry-forward keeps bob pre-prune",
	);
...
test("prune: streams NOT requested this run keep their full carry-forward", () => {
	...
	assert.equal(
		...
		1,
		"unrequested files stream untouched",
	);
	// Requested users stream had zero observations → its sole prior ID is pruned.
	assert.equal(
		Object.keys(run.cursors.get("users")?.toState() ?? {}).length,
		0,
		"requested+empty stream fully pruned",
	);
});
