...
test("Slack credential normalization preserves URL-encoded d-cookie bytes", () => {
	...
	assert.equal(
		normalizeSlackToken(`  ${VALID_SLACK_TOKEN}  `),
		VALID_SLACK_TOKEN,
	);
...
test("Slack credential normalization rejects empty, control, malformed, and oversized values", () => {
	...
	assert.throws(
		() => normalizeSlackToken("xoxp-not-a-client-token"),
		/slack_token_invalid/,
	);
	assert.throws(
		() => normalizeSlackToken("xoxc-valid\u0000opaque"),
		/slack_token_invalid/,
	);
	assert.throws(
		() => normalizeSlackToken(`xoxc-${"a".repeat(4092)}`),
		/slack_token_invalid/,
	);
	assert.throws(() => normalizeSlackCookie("d= "), /slack_cookie_invalid/);
	assert.throws(
		() => normalizeSlackCookie("xoxd-session-%2"),
		/slack_cookie_invalid/,
	);
	assert.throws(
		() => normalizeSlackCookie("xoxd-valid\u0001opaque"),
		/slack_cookie_invalid/,
	);
	assert.throws(
		() => normalizeSlackCookie(`xoxd-${"a".repeat(4092)}`),
		/slack_cookie_invalid/,
	);
	assert.throws(
		() => normalizeSlackWorkspace("../outside"),
		/slack_workspace_invalid/,
	);
	assert.throws(
		() =>
			extractSlackCredentials({
...
test("slack retry classification treats slackdump exit 6 as resumable", () => {
	assert.equal(
		SLACK_RETRYABLE_FAILURE_RE.test(
			"slackdump failed: slackdump_exit_6: conversations.history 500",
		),
		true,
	);
	assert.equal(
		SLACK_RETRYABLE_FAILURE_RE.test(
			"parser error: unexpected token in archive",
		),
		false,
	);
});
...
test("runSlackdump: detects progress from mtime even when a WAL checkpoint keeps archive bytes flat", async () => {
	...
	const tmpDir = await mkdtemp(
		join(tmpdir(), "pdpp-slackdump-wal-checkpoint-"),
	);
	const fakeSlackdump = join(tmpDir, "fake-slackdump.mjs");
	const sqlitePath = join(tmpDir, "slackdump.sqlite");
	const progressEvents: Array<{ extra: unknown; message: string }> = [];
...
`,
		...
		assert.ok(
			pollCount > 20,
			`expected many concurrent polls during the writer run; got ${pollCount}`,
		);
	} finally {
		await rm(tmpDir, { recursive: true, force: true });
	}
...
test("slack manifest declares no unsupported-in-mode streams (all four gap streams now collect directly)", async () => {
	...
	const unsupported = (manifest.streams || []).filter(
...
test("slack manifest explains the xoxc token and d-cookie fields with the official manual", async () => {
	...
	assert.doesNotMatch(
		token?.help_url ?? "",
		/wiki\/How-to-get-your-Slack-credentials/,
	);
});
	...
	try {
      ...
      `,
				...
				new TextEncoder().encode(
					JSON.stringify({ text: "hello from slack", user: "U0123456789" }),
				),
				1,
			);
		} finally {
...
test("slack connector counts channel-scoped message RECORDs in DONE.records_emitted", async () => {
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-scoped-counter-"),
	);
	try {
      ...
      `);
			insert.run(
				...
				null,
				new TextEncoder().encode(
					JSON.stringify({ text: "included", user: "U0123456789" }),
				),
				1,
			);
			insert.run(
				...
				null,
				new TextEncoder().encode(
					JSON.stringify({ text: "excluded", user: "U0123456789" }),
				),
				1,
			);
		} finally {
...
test("slack connector emits a bounded source-partition diagnostic when a prior channel is missing", async () => {
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-missing-channel-"),
	);
	try {
		...
		const gap = result.messages.find(
			(message): message is Extract<EmittedMessage, { type: "SKIP_RESULT" }> =>
				...
				message.reason === "source_partition_missing",
		);
		assert.ok(gap, "expected source_partition_missing SKIP_RESULT");
		assert.equal(gap.stream, "messages");
		assert.deepEqual(
			(gap.diagnostics as { missing_channel_ids?: string[] })
				.missing_channel_ids,
			["C_MISSING"],
		);
		assert.deepEqual(gap.recovery_hint, {
...
test("slack connector does not emit a missing-partition diagnostic when prior channels remain present", async () => {
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-clean-channel-"),
	);
	try {
	...
	try {
		...
		const result = await runConnectorProtocolSubprocess({
			...
			start: {
				type: "START",
				scope: {
					streams: [{ name: "messages", resources: [scopedChannelId] }],
				},
				state: {
					messages: {
...
test("slack archive resolves next to PDPP_DB_PATH, not HOME (survives container replacement)", async () => {
	...
	const discardedHome = await mkdtemp(
		join(tmpdir(), "pdpp-slack-discarded-home-"),
	);
	try {
		...
		const archiveDir = join(
			resolveConnectorArtifactDir("slack", [workspace], {
				PDPP_DB_PATH: dbPath,
			}).root,
			"archive",
		);
		...
		assert.ok(
			archiveDir.startsWith(durableVolume),
			`expected archive under the durable volume, got ${archiveDir}`,
		);
...
async function writeProgressingSlackdump(
	path: string,
	{
		...
		stepMs,
		thenIdleMs,
	}: { advances: number; stepMs: number; thenIdleMs: number },
): Promise<void> {
	await writeFile(
		path,
		`#!/usr/bin/env node
...
async function withFakeSlackdump<T>(
	fn: (paths: { archive: string; bin: string }) => Promise<T>,
): Promise<T> {
...
test("runSlackdump: a steadily-progressing dump outlives its budget (stall, not total runtime)", async () => {
	await withFakeSlackdump(async ({ archive, bin }) => {
		await writeProgressingSlackdump(bin, {
			advances: 9,
			stepMs: 200,
			thenIdleMs: 0,
		});
...
test("both slackdump timeout shapes are retryable failures", () => {
	assert.equal(
		SLACK_RETRYABLE_FAILURE_RE.test("slackdump failed: slackdump_timeout"),
		true,
	);
	assert.equal(
		SLACK_RETRYABLE_FAILURE_RE.test("slackdump failed: slackdump_max_runtime"),
		true,
	);
});
...
test("runSlackdump: a child that exits with no progress settles immediately, not after the budget", async () => {
	await withFakeSlackdump(async ({ archive, bin }) => {
		...
		assert.ok(
			...
			"expected exit-driven settle, not a budget wait",
		);
	});
});
