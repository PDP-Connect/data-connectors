...
function insertMessage(
	...
	db.prepare(
		...
		text,
		null,
		new TextEncoder().encode(JSON.stringify({ text, user: "U1" })),
		1,
	);
}
...
test("reclaimUploads removes only __uploads/, leaves sqlite + sidecars, reports bytes", async () => {
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-reclaim-unit-"),
	);
	try {
		...
		assert.ok(
			existsSync(join(archiveDir, "__uploads")),
			"precondition: __uploads exists",
		);
...
test("base archive resume is throttled on the 90-minute follow-up without invoking slackdump", async () => {
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-base-throttle-immediate-"),
	);
	try {
		...
		assert.equal(
			(
				messagesState(result).base_archive_resumed_at as
...
test("a failed base archive resume remains owed and retries successfully on the next run", async () => {
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-base-throttle-retry-"),
	);
	try {
		...
		const failed = await runConnectorProtocolSubprocess({
			...options,
			allowFailedDone: true,
		});
		assert.equal(
			failed.messages.findLast((message) => message.type === "DONE")?.status,
			"failed",
		);
		assert.ok(
			!failed.messages.some((message) => message.type === "STATE"),
			"a failed run commits no success cursor",
		);

		const recovered = await runConnectorProtocolSubprocess(options);
		const calls = (await readFile(fakeSlackdump.callLog, "utf8"))
			.trim()
			.split("\n");
		assert.equal(
			calls.length,
			2,
			"the next run retried the failed base resume instead of suppressing it",
		);
		assert.ok(
		...
		assert.ok(
			(
				messagesState(recovered).base_archive_resumed_at as
					| Record<string, string>
					| undefined
			)?.[archiveDir],
...
test("base archive resume runs again after the seven-day lookback expires", async () => {
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-base-throttle-expiry-"),
	);
	try {
		const workspace = "base-throttle-expiry-ws";
		const archiveDir = await seedArchive(artifactRoot, workspace, false);
		const fakeSlackdump = await writeCountingSlackdump(artifactRoot);
		const staleResumedAt = new Date(
		...
		assert.match(
			...
			/^resume /,
			"an expired base cursor resumes again",
		);
		assert.notEqual(
			(
				messagesState(result).base_archive_resumed_at as
...
test("upgrade compatibility: a pre-upgrade successful base archive is throttled on the first post-upgrade run, not replayed", async () => {
	...
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-base-throttle-migration-"),
	);
	try {
		...
		assert.ok(
			migratedAt,
			"a synthetic base_archive_resumed_at fact is derived and committed for the next run",
		);
		...
		const followUp = await runConnectorProtocolSubprocess({
			...
			start: {
				...
				state: {
					messages: {
						...preUpgradeState.messages,
						base_archive_resumed_at: { [archiveDir]: migratedAt },
					},
				},
			},
		});
		...
		assert.equal(
			(
				messagesState(followUp).base_archive_resumed_at as
					| Record<string, string>
					| undefined
			)?.[archiveDir],
			migratedAt,
...
test("upgrade compatibility does NOT seed the throttle from archive existence alone (interrupted/failed prior run)", async () => {
	...
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-base-throttle-no-seed-"),
	);
	try {
...
test("reclaimUploads is a no-op returning 0 when __uploads/ is absent", async () => {
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-reclaim-absent-"),
	);
	try {
		...
		assert.ok(
			existsSync(join(archiveDir, "slackdump.sqlite")),
			"sqlite untouched",
		);
	} finally {
		await rm(artifactRoot, { recursive: true, force: true });
...
function baseArchiveState(
	archivePath: string,
	baseArchiveResumedAt?: string,
): Record<string, unknown> {
	return {
		messages: {
			archive_dir: archivePath,
			...(baseArchiveResumedAt
				? { base_archive_resumed_at: { [archivePath]: baseArchiveResumedAt } }
				: {}),
		},
	};
}
...
test("__uploads reclaim is OFF by default: a normal run leaves __uploads intact", async () => {
	...
	try {
		...
		const done = result.messages.findLast(
			(m): m is Extract<EmittedMessage, { type: "DONE" }> => m.type === "DONE",
		);
		assert.equal(done?.status, "succeeded");
		assert.ok(
			existsSync(join(archiveDir, "__uploads")),
			"__uploads NOT reclaimed without opt-in",
		);
	} finally {
		await rm(artifactRoot, { recursive: true, force: true });
	}
...
test("SLACK_RECLAIM_UPLOADS=1 does NOT reclaim when the run fails (gate honored)", async () => {
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-reclaim-fail-"),
	);
	try {
		...
		const archiveDir = join(
			seedArchiveRoot(artifactRoot, "fail-ws"),
			"archive",
		);
		await mkdir(join(archiveDir, "__uploads", "F1"), { recursive: true });
		await writeFile(
			join(archiveDir, "__uploads", "F1", "a.bin"),
			Buffer.alloc(2048, 3),
		);
		...
		assert.ok(
			existsSync(join(archiveDir, "__uploads")),
			"__uploads intact: reclaim never precedes a durable commit",
		);
	} finally {
		await rm(artifactRoot, { recursive: true, force: true });
	}
...
test("SLACK_RECLAIM_UPLOADS=1 removes __uploads after a successful run, sqlite intact", async () => {
	...
	try {
		...
		assert.equal(
			...
			"succeeded",
			"run succeeded (reclaim runs post-commit)",
		);
		assert.ok(
			!existsSync(join(archiveDir, "__uploads")),
			"__uploads reclaimed after durable commit",
		);
		assert.ok(
			existsSync(join(archiveDir, "slackdump.sqlite")),
			"sqlite (resume state) untouched",
		);
		...
		assert.ok(
			result.stderr.includes("[onDurableCommit]") &&
				result.stderr.includes("reclaim: removed __uploads/"),
			"reports the reclaim as stderr evidence",
		);
		assert.ok(
			result.stderr.includes("one-way"),
			"reclaim evidence states it is one-way/unrecoverable",
		);
	} finally {
		await rm(artifactRoot, { recursive: true, force: true });
	}
...
test("SLACK_RECLAIM_UPLOADS=1 reclaims __uploads/ in every archive the run actually read, not just the base archive", async () => {
	...
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-reclaim-multi-"),
	);
	try {
		const workspace = "reclaim-multi-ws";
		const baseArchiveDir = join(
		...
		const scopedDb = new DatabaseSync(
			join(scopedArchiveDir, "slackdump.sqlite"),
		);
		try {
		...
		assert.equal(
			...
			"succeeded",
			"run healed the missing channel and succeeded",
		);
		assert.ok(
			!existsSync(join(baseArchiveDir, "__uploads")),
			"base archive __uploads/ reclaimed",
		);
		assert.ok(
			!existsSync(join(scopedArchiveDir, "__uploads")),
			"scoped archive __uploads/ ALSO reclaimed",
		);
		assert.ok(
			existsSync(join(baseArchiveDir, "slackdump.sqlite")),
			"base sqlite (resume state) untouched",
		);
		assert.ok(
			existsSync(join(scopedArchiveDir, "slackdump.sqlite")),
			"scoped sqlite (resume state) untouched",
		);
		assert.ok(
			result.stderr.includes(baseArchiveDir) &&
				result.stderr.includes("reclaimed 4096B"),
			"reports the base archive's reclaimed bytes",
		);
		assert.ok(
			result.stderr.includes(scopedArchiveDir) &&
				result.stderr.includes("reclaimed 2048B"),
...
test("SLACK_RECLAIM_UPLOADS=1 reclaims a repair archive that was successfully created/read but recovered no matching channel", async () => {
	...
	// nothing.
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-reclaim-empty-repair-"),
	);
	try {
		...
		const repairDb = new DatabaseSync(
			join(repairArchiveDir, "slackdump.sqlite"),
		);
		try {
		...
		assert.equal(
			...
			"succeeded",
			"run succeeded even though the repair recovered no channel",
		);
		...
		assert.ok(
			result.messages.some(
				(m) =>
					m.type === "SKIP_RESULT" && m.reason === "source_partition_missing",
			),
		...
		assert.ok(
			existsSync(join(baseArchiveDir, "slackdump.sqlite")),
			"base sqlite untouched",
		);
		assert.ok(
			existsSync(join(repairArchiveDir, "slackdump.sqlite")),
			"empty-repair sqlite untouched",
		);
		assert.ok(
			result.stderr.includes(repairArchiveDir) &&
				result.stderr.includes("reclaimed 1024B"),
...
test("SLACK_RECLAIM_UPLOADS=1 does NOT reclaim a repair archive when the repair attempt itself fails before durable", async () => {
	...
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-reclaim-repair-fail-"),
	);
	try {
		...
		assert.equal(
			...
			"succeeded",
			"run still succeeds; the failed repair attempt is non-fatal",
		);
		assert.ok(
			!existsSync(join(baseArchiveDir, "__uploads")),
			"base archive __uploads/ still reclaimed",
		);
		const repairArchiveDir = join(
			seedArchiveRoot(artifactRoot, workspace),
			"archive-scoped",
		...
		assert.ok(
			!existsSync(repairArchiveDir),
			"failed repair attempt created no archive directory to reclaim",
		);
	} finally {
		await rm(artifactRoot, { recursive: true, force: true });
	}
...
test("scoped-archive-reconcile phase timing is reported when source-cache healing runs", async () => {
	...
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-reconcile-timing-"),
	);
	try {
...
test("scoped-archive-reconcile declares 0 selected repair units and does no work when there is nothing to heal", async () => {
	...
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-reconcile-none-"),
	);
	try {
...
test("scoped-archive-reconcile throttles a scoped archive's resume to at most once per lookback window", async () => {
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-reconcile-throttle-"),
	);
	try {
				...
				state: {
					messages: {
						...
						scoped_archive_resumed_at: {
							[scopedArchiveDir]: recentlyResumedAt,
						},
					},
				},
			},
		...
		assert.equal(
			...
			"succeeded",
			"run still succeeds when a scoped archive is throttled",
		);
		const lines = progressLines(result.messages);
		assert.ok(
			lines.some(
...
test("scoped-archive-reconcile resumes a scoped archive again once its lookback throttle window has elapsed", async () => {
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-reconcile-due-"),
	);
	try {
...
test("a failed scoped-archive resume does not advance the success cursor, emits a typed retryable gap, and leaves the archive owed for the next governor-allowed run", async () => {
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-reconcile-resume-fail-"),
	);
...
`,
		...
		assert.equal(
			...
			"succeeded",
			"a failed optional scoped-archive resume is non-fatal to the run",
		);
		...
		const gap = result.messages.find(
			(m): m is Extract<EmittedMessage, { type: "DETAIL_GAP" }> =>
				m.type === "DETAIL_GAP" &&
				(m as { record_key?: unknown }).record_key === scopedArchiveDir,
		);
		...
		assert.equal(
			(gap?.detail_locator as { kind?: unknown } | undefined)?.kind,
			"slack.scoped_archive_resume",
		);
		assert.ok(
...
test("scoped-archive-reconcile emits DETAIL_GAP_RECOVERED when a previously-failed archive resumes successfully", async () => {
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-reconcile-recovered-"),
	);
	try {
		...
		assert.ok(
			recovered,
			"emits DETAIL_GAP_RECOVERED once the archive resumes successfully again",
		);
...
test("a failed NEW-repair attempt for an uncovered missing channel emits the same typed retryable gap and leaves the archive owed", async () => {
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-reconcile-repair-fail-"),
	);
	const priorBin = process.env.SLACKDUMP_BIN;
	try {
...
`,
		...
		assert.equal(
			...
			"succeeded",
			"a failed optional new-repair attempt is non-fatal to the run",
		);
		...
		const gap = result.messages.find(
			(m): m is Extract<EmittedMessage, { type: "DETAIL_GAP" }> =>
				m.type === "DETAIL_GAP" &&
				(m as { record_key?: unknown }).record_key === repairArchiveDir,
		);
...
test("a later successful NEW-repair attempt emits DETAIL_GAP_RECOVERED for a previously-failed repair archive path", async () => {
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-reconcile-repair-recovered-"),
	);
	try {
		...
		try {
			...
			insertMessage(
				...
				"C0MISSING",
				"1714032850.123456",
				"recovered via new-repair attempt",
			);
		} finally {
			repairDb.close();
		}
		...
		assert.ok(
			recovered,
			"emits DETAIL_GAP_RECOVERED once the new-repair attempt succeeds",
		);
