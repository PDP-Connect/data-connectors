	...
	return {
		...
		emit: (msg) => {
			if (msg.type === "DETAIL_COVERAGE") {
				captured.considered.push({
					stream: msg.stream,
					considered: msg.considered ?? 0,
					covered: msg.covered,
				});
			}
...
test("runStarsStream: emits one RECORD per starred item and declares considered", async () => {
	...
	await runStarsStream(
		fakeDeps(db, captured, ["stars"]),
		"xoxc-fake",
		"d-fake",
	);
...
test("runStarsStream: zero stars still completes and declares considered=0", async () => {
	globalThis.fetch = () =>
		Promise.resolve(jsonResponse({ ok: true, items: [] }));
	const db = new DatabaseSync(":memory:");
	const captured: Captured = { considered: [], records: [] };
	await runStarsStream(
		fakeDeps(db, captured, ["stars"]),
...
test("runUserGroupsStream: emits one RECORD per user group", async () => {
	globalThis.fetch = () =>
		Promise.resolve(
			jsonResponse({
				ok: true,
				usergroups: [{ id: "S01", handle: "eng", users: ["U01"] }],
			}),
		);
	const db = new DatabaseSync(":memory:");
	const captured: Captured = { considered: [], records: [] };
	await runUserGroupsStream(
		fakeDeps(db, captured, ["user_groups"]),
		"xoxc-fake",
		"d-fake",
	);
	assert.equal(captured.records.length, 1);
...
test("runRemindersStream: emits one RECORD per reminder", async () => {
	globalThis.fetch = () =>
		Promise.resolve(
			jsonResponse({
				ok: true,
				reminders: [{ id: "Rm01", text: "ping bob", time: 1_714_032_900 }],
			}),
		);
	const db = new DatabaseSync(":memory:");
	const captured: Captured = { considered: [], records: [] };
	await runRemindersStream(
		fakeDeps(db, captured, ["reminders"]),
		"xoxc-fake",
		"d-fake",
	);
	assert.equal(captured.records.length, 1);
...
function seedChannel(
  ...
  `);
	...
	).run(id, null, JSON.stringify(data), 1);
}
...
test("runOptionalStream: a failing optional stream resolves (does not reject) and emits a SKIP_RESULT", async () => {
	...
	await assert.doesNotReject(() =>
		runOptionalStream(emit, "stars", () =>
			Promise.reject(new Error("slack_auth_failed")),
		),
	);
...
test("runOptionalStream: a retryable failure (rate limit) is flagged retryable in the recovery hint", async () => {
	...
	await runOptionalStream(emit, "reminders", () =>
		Promise.reject(new Error("slack_rate_limited")),
	);

	const [msg] = messages;
	const hint = (msg as { recovery_hint?: { retryable?: boolean } })
		.recovery_hint;
...
test("runOptionalStream: a non-retryable failure (auth) is flagged non-retryable in the recovery hint", async () => {
	...
	await runOptionalStream(emit, "stars", () =>
		Promise.reject(new SlackApiAuthError("invalid_auth")),
	);
...
test("runOptionalStream: the recovery_hint diagnostics carries the specific Slack-reported error code", async () => {
	...
	await runOptionalStream(emit, "stars", () =>
		Promise.reject(new SlackApiAuthError("token_revoked")),
	);

	const [msg] = messages;
	const { diagnostics } = msg as {
		diagnostics?: { slack_api_error_code?: string };
	};
...
test("runOptionalStream: classification is by error TYPE, not message text — a plain Error with the same message string is NOT treated as an auth failure", async () => {
	...
	const [msg] = messages;
	const hint = (
		msg as { recovery_hint?: { action?: string; retryable?: boolean } }
	).recovery_hint;
...
function seedUser(db: DatabaseSync, id: string): void {
  ...
  `);
	...
	).run(id, "alice", JSON.stringify({ real_name: "Alice" }), 1);
}
...
test("contrast: a REQUIRED stream's failure is NOT caught by runOptionalStream and propagates", async () => {
	...
	const deps: StreamDeps = {
		...
		emitRecord: (): Promise<void> =>
			Promise.reject(new Error("emitRecord_boom")),
		emittedAt: "2026-07-10T00:00:00.000Z",
		failedStreams: new Set(),
		fingerprintCursors: new Map(),
		progress: () => Promise.resolve(),
