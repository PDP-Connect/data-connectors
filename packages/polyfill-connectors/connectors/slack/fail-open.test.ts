	...
	return openFingerprintCursor(
		{ fingerprints: { stale_id: "prior-fingerprint" } },
		{
			excludeFromFingerprint:
				FINGERPRINT_EXCLUDE[stream as keyof typeof FINGERPRINT_EXCLUDE],
		},
	);
}
...
async function runStream(
	stream: string,
	db: DatabaseSync,
): Promise<{ deps: StreamDeps; messages: EmittedMessage[] }> {
	...
	test(`failed SQLite enumeration: ${stream} stays retryable and uncheckpointed`, async () => {
		...
		try {
			...
			pruneRequestedFingerprintCursors(
				deps.requested,
				deps.failedStreams,
				deps.fingerprintCursors,
			);
			assert.equal(
				deps.fingerprintCursors.get(stream)?.toState().stale_id,
				"prior-fingerprint",
			);
...
function healthyDb(stream: (typeof FAILED_STREAMS)[number]): DatabaseSync {
	...
	if (stream === "workspace") {
		...
		db.prepare("INSERT INTO S_USER VALUES (?, ?, ?, ?)").run(
			"U1",
			"alice",
		...
		db.prepare("INSERT INTO CHANNEL VALUES (?, ?, ?)").run(
			"C1",
			JSON.stringify({}),
			1,
		);
...
test("healthy SQLite enumerations still declare complete coverage for all five streams", async () => {
	for (const stream of FAILED_STREAMS) {
		...
		try {
			const { deps, messages } = await runStream(stream, db);
			assert.equal(
				deps.failedStreams.size,
				0,
				`${stream} healthy run must not be marked failed`,
			);
			...
			assert.equal(
				coverage?.considered,
				1,
				`${stream} healthy run must enumerate one item`,
			);
			assert.equal(
				coverage?.covered,
				1,
				`${stream} healthy run must cover the enumerated item`,
			);
		} finally {
			db.close();
		}
