...
function makeUserDb(
	users: ReadonlyArray<{ id: string; username: string; updated: number }>,
): DatabaseSync {
	...
	const ins = db.prepare(
		"INSERT INTO S_USER (ID, USERNAME, DATA, CHUNK_ID) VALUES (?, ?, ?, ?)",
	);
...
function makeCursors(
	prior: Partial<
		Record<(typeof FINGERPRINTED_STREAMS)[number], Record<string, string>>
	> = {},
...
function makeDeps(
	...
	return {
		...
		requested: new Map<string, StreamScope>(
			requestedStreams.map((n) => [n, { name: n }]),
		),
	};
}
...
function coverageFor(
	...
	const msg = harness.protocolMessages.find(
		(m) =>
			...
			(m as { stream?: string }).stream === stream,
	);
...
test("users: a fresh run declares considered === covered === enumerated (all emitted, a real complete)", async () => {
	...
	assert.equal(
		harness.emitted.filter((r) => r.stream === "users").length,
		3,
		"all three users emitted",
	);
	...
	assert.equal(
		cov?.state_stream,
		"users",
		"self-coverage: state_stream === stream",
	);
	assert.equal(cov?.considered, 3, "considered is the enumerated inventory");
	assert.equal(
		cov?.covered,
		3,
		"covered === considered: every item accounted for (emitted)",
	);
...
test("users: a STEADY-STATE run suppresses every unchanged record yet declares covered === considered", async () => {
	...
	try {
		await runUsersStream(
			makeDeps(db2, h2, makeCursors({ users: priorState }), ["users"]),
		);
	} finally {
		db2.close();
	}
	...
	assert.equal(
		h2.emitted.filter((r) => r.stream === "users").length,
		0,
		"steady-state run emits nothing",
	);
	...
	assert.equal(
		cov?.covered,
		3,
		"all three accounted for as suppressed-unchanged → complete, not a false partial",
	);
});
...
test("users: a one-changed run still declares covered === considered (changed emit + unchanged suppressed both covered)", async () => {
	...
	const db2 = makeUserDb(
		users.map((u, i) => (i === 0 ? { ...u, updated: 200 } : u)),
	);
	const h2 = makeRecordingEmit();
	try {
		await runUsersStream(
	...
	assert.equal(
		h2.emitted.filter((r) => r.stream === "users").length,
		1,
		"only the changed user re-emits",
	);
	const cov = coverageFor(h2, "users");
	...
	assert.equal(
		cov?.covered,
		3,
		"1 emitted + 2 suppressed-unchanged === considered → complete",
	);
});
...
test("channels: NOT requested → no considered/covered declaration (no denominator for an unexercised stream)", async () => {
	...
	try {
		...
		await runChannelsStream(
			makeDeps(db, harness, makeCursors(), ["channel_stats"]),
		);
	} finally {
		db.close();
	}
	...
	assert.equal(
		harness.emitted.filter((r) => r.stream === "channels").length,
		0,
		"no channel entity records",
	);
	assert.equal(
		coverageFor(harness, "channels"),
		null,
		"an unrequested channels pass declares no denominator",
	);
});
...
test("emitWithFingerprint: reports emit vs suppress so covered counts both, never aliasing rows.length", async () => {
	...
	assert.equal(
		firstEmit,
		true,
		"first sight of a record emits (covered via emit)",
	);
	...
	assert.equal(
		suppressed,
		false,
		"an unchanged record is suppressed (covered via suppression, NOT a drop)",
	);

	const changed = await emitWithFingerprint(deps2, "users", {
		...rec,
		updated: 200,
	});
