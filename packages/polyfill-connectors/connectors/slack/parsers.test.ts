...
test("buildChannelMembershipRecord: composite id is channel:user", () => {
	const rec = buildChannelMembershipRecord(
...
test("parseMessageRow: populates sentAt from ts, falls back on missing ts", () => {
	const parsed = parseMessageRow(
		messageRowFromFixture(),
...
test("buildCanvasRecord: overlays channel-canvas-index metadata onto file row", () => {
	const chanRows: ChannelRow[] = [
		{ id: "C1", name: "eng", data: readFixture("channel-with-canvas.json") },
	];
...
test("selectCommittedMaxTs: table-driven", () => {
	const cases: Array<{
		...
		{
			prior: "1700000000.000000",
			run: "1714000000.000100",
			want: "1714000000.000100",
		},
		{
			prior: "1714000000.000100",
			run: "1700000000.000000",
			want: "1714000000.000100",
		},
	];
	for (const c of cases) {
		assert.equal(
			selectCommittedMaxTs(c.prior, c.run),
			c.want,
			`prior=${c.prior} run=${c.run}`,
		);
	}
});

test("extractMessageTimeRange: missing scope → both null; present → passthrough", () => {
	assert.deepEqual(extractMessageTimeRange(undefined), {
		timeFrom: null,
		timeTo: null,
	});
	assert.deepEqual(extractMessageTimeRange({}), {
		timeFrom: null,
		timeTo: null,
	});
	assert.deepEqual(
