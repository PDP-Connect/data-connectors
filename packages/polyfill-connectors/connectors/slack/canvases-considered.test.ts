...
function makeCanvasDb(
	canvasIds: readonly string[],
	otherFileIds: readonly string[] = [],
): DatabaseSync {
...
function canvasCoverage(harness: RecordingEmit): DetailCoverageMessage | null {
	const msg = harness.protocolMessages.find(
		(message): message is DetailCoverageMessage =>
			message.type === "DETAIL_COVERAGE" && message.stream === "canvases",
	);
	return msg ?? null;
}
...
test("runCanvasesStream: declares considered === the enumerated quip inventory (a real complete denominator)", async () => {
	...
	// RECORDs.
	assert.equal(
		harness.emitted.filter((r) => r.stream === "canvases").length,
		3,
	);
	...
	assert.equal(
		c.covered,
		3,
		"every validated canvas in the enumerated snapshot is covered",
	);
...
test("runCanvasesStream: considered counts ONLY quip canvases, never all FILE rows (measured at the query site)", async () => {
	...
	assert.equal(
		cov?.considered,
		2,
		"considered is the quip-filtered enumeration, not the whole FILE table",
	);
});
...
test("runCanvasesStream: emits the considered DETAIL_COVERAGE AFTER the last canvas RECORD", async () => {
	...
	const lastRecordIdx = harness.events.findLastIndex(
		(e) => e.kind === "record" && e.stream === "canvases",
	);
	...
	assert.ok(
		...
		"considered coverage lands after the last canvas RECORD",
	);
});
