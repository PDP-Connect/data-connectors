	...
	return {
		...
		deps: {
			emitRecord: (stream: string, data: RecordData) => {
				if (
					stream === "messages" &&
					!(typeof data.channel_id === "string" && inScope.has(data.channel_id))
				) {
...
test("a row dropped by the channel-scope guard does not advance that channel's watermark", async () => {
	...
	const result = await emitMessagesPass(
		deps,
		[
			messageRow("C_IN", "1700000000.000100"),
			messageRow("C_OUT", "1700000009.000900"),
		],
		null,
	);
	...
	assert.equal(
		result.channelMaxTs.C_IN,
		"1700000000.000100",
		"the emitted channel still advances",
	);
});
...
test("the global last_ts does not advance past a row the scope guard dropped", async () => {
	...
	const result = await emitMessagesPass(
		...
		[
			messageRow("C_IN", "1700000000.000100"),
			messageRow("C_OUT", "1700009999.000900"),
		],
		null,
	);
	...
	assert.equal(
		result.maxMessageTs,
		"1700000000.000100",
		"last_ts reflects the emitted max, not the iterated max",
	);
});
...
test("iterated-max stays observable, and separate from the durable cursor", async () => {
	...
	assert.equal(
		result.channelMaxTs.C_OUT,
		undefined,
		"but it must not leak into the durable cursor",
	);
	assert.notDeepEqual(
		result.channelMaxTs,
		result.iteratedChannelMaxTs,
...
test("a fully out-of-scope pass commits no cursor at all", async () => {
	...
	const result = await emitMessagesPass(
		deps,
		[messageRow("C_A", "1700000001.000000")],
		null,
	);
	...
	assert.deepEqual(
		result.channelMaxTs,
		{},
		"nothing emitted means nothing committed",
	);
...
test("a reactions-only pass does not advance the messages cursor", async () => {
	...
	const result = await emitMessagesPass(
		deps,
		[messageRow("C1", "1700000000.000100")],
		null,
	);
...
function selectRows(
	db: DatabaseSync,
	thresholds: Parameters<typeof buildMessageRowsQuery>[0],
): string[] {
...
test("a channel absent from channel_last_ts does not inherit the global floor", () => {
	...
	try {
		...
		assert.deepEqual(
			got,
			[
				"C_KNOWN:1700000600.000000",
				"C_UNSEEN:1700000100.000000",
				"C_UNSEEN:1700000200.000000",
			],
...
test("a legacy cursor with NO per-channel map still floors every channel", () => {
	...
	try {
		...
		assert.deepEqual(
			got,
			["C_B:1700000900.000000"],
			"the legacy floor still applies when there is no channel map",
		);
	} finally {
		db.close();
	}
...
test("collection_scope.since still composes with the per-channel predicate", () => {
	...
	try {
		...
		assert.deepEqual(
			got,
			["C_UNSEEN:1700000800.000000"],
			"since bounds the unwalked channel even with no cursor",
		);
	} finally {
		db.close();
	}
