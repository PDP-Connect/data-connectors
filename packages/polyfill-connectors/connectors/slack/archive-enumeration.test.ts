...
function insertSession(
	...
	db.prepare(
		"INSERT INTO SESSION (ID, FINISHED, MODE, ARGS) VALUES (?, ?, ?, ?)",
	).run(id, finished, mode, `${mode}|...`);
}
...
test("an archive with no SESSION table at all owes nothing", async () => {
	await withArchive(
		(db) => {
			db.exec(
				"CREATE TABLE CHANNEL (ID TEXT NOT NULL, CHUNK_ID INTEGER NOT NULL);",
			);
		},
		(sqlitePath) => {
			assert.equal(archivePathEnumerationIncomplete(sqlitePath), false);
...
test("a path with no archive on it owes nothing", () => {
	assert.equal(
		archivePathEnumerationIncomplete(
			join(tmpdir(), "slack-enum-does-not-exist", "slackdump.sqlite"),
		),
		false,
	);
});
...
test("pickResumeTarget resumes a discovered archive when enumeration is complete", async () => {
	await withArchive(
		...
		(_sqlitePath, archiveDir) => {
			const { resumeTarget } = pickResumeTarget({}, archiveDir, {
				forceFullArchive: false,
			});
			assert.equal(
				resumeTarget,
				archiveDir,
				"a complete archive is cheap to resume and must be resumed",
			);
		},
	);
});
	...
	await withArchive(
		...
		(_sqlitePath, archiveDir) => {
			...
			assert.equal(
				resumeTarget,
				null,
				"an owed enumeration must run `archive`, not `resume`",
			);
		},
	);
});
