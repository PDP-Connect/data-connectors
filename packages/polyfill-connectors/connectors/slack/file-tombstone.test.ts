...
function makeFileDb(rows: readonly FileRow[]): DatabaseSync {
	...
	db.exec(
		"CREATE TABLE FILE (ID TEXT, FILENAME TEXT, URL TEXT, MODE TEXT, DATA TEXT, CHUNK_ID INTEGER)",
	);
	const insert = db.prepare(
		"INSERT INTO FILE (ID, FILENAME, URL, MODE, DATA, CHUNK_ID) VALUES (?, ?, ?, ?, ?, ?)",
	);
		...
		fingerprintCursors: new Map([
			[
				...
				openFingerprintCursor(
					{ fingerprints: {} },
					{ excludeFromFingerprint: FINGERPRINT_EXCLUDE.files },
				),
			],
		]),
...
test("files: six exact Slack tombstones pass the production shape gate and count as covered", async () => {
	...
	assert.deepEqual(
		recording.skipped,
		[],
		"no tombstone or normal file may become a shape skip",
	);
	const emitted = recording.emitted.filter(
		(record) => record.stream === "files",
	);
	assert.equal(
		emitted.length,
		7,
		"all six tombstones and the normal file are retained",
	);

	const tombstones = emitted.filter(
		(record) => record.data.mode === "tombstone",
	);
	...
	for (const tombstone of tombstones) {
		assert.equal(
			tombstone.data.created,
			0,
			"provider sentinel remains provider metadata",
		);
		assert.equal(
			tombstone.data.created_at,
			null,
			"unset provider time remains absent",
		);
	...
	assert.deepEqual(
		normal.data,
		buildFileRecord(normalRow),
		"the normal transform remains unchanged",
	);
	...
	assert.equal(
		coverage?.covered,
		7,
		"six tombstones plus the normal file are all accounted for",
	);
});
