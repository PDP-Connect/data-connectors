...
test("steam - full-snapshot collection with fingerprint dedup", () => {
	...
	const gameRecord = {
		...
		playtime_linux: null,
		img_icon_url:
			"https://media.steampowered.com/steamcommunity/public/images/apps/730/icon.jpg",
		img_logo_url:
			"https://media.steampowered.com/steamcommunity/public/images/apps/730/logo.jpg",
		has_community_visible_stats: true,
		rtime_last_played: 1_234_567_890,
		content_descriptorids: null,
	};
	...
	assert.strictEqual(
		stateStructure.stream,
		"owned_games",
		"STATE targets owned_games",
	);
	assert(
		stateStructure.cursor.fingerprints,
		"STATE carries fingerprints for dedup",
	);
});
...
test("resolveRawSteamIdFromCredentials - falls back to process.env when absent from the credentials bundle", () => {
	...
	const credentials: Record<string, string | undefined> = {
		STEAM_API_KEY: "fake-api-key",
	};
	const env = { STEAM_USER_ID: "76561198000788935" };

	assert.equal(
		resolveRawSteamIdFromCredentials(credentials, env),
		"76561198000788935",
	);
});
...
test("steam - malformed response handling", () => {
	...
	assert.strictEqual(
		result.ok,
		false,
		"malformed profile (numeric personaname) should fail",
	);
	if (!result.ok) {
		assert(
...
test("classifySteamHttpResponse - other non-2xx status is bounded and sanitized", () => {
	...
	if (result.kind === "error") {
		assert.equal(
			result.message,
			"steam_http_500",
			"message is tagged with the status code only",
		);
		assert.doesNotMatch(
			result.message,
			/secret-api-key|x{20,}/,
			"provider body must not enter durable diagnostics",
		);
	}
});
...
test("STEAM_RETRYABLE_PATTERN - does not match generic bounded HTTP errors", () => {
	assert.doesNotMatch(
		"steam_http_500: internal server error",
		STEAM_RETRYABLE_PATTERN,
	);
...
test("steamCollect - resolves a custom profile name before sending SteamID64 downstream", async () => {
	...
	const requests: URL[] = [];
	const vanityFixture = readFileSync(
		new URL("./__fixtures__/resolve-vanity-url.json", import.meta.url),
		"utf8",
	);
	const profileFixture = readFileSync(
		new URL("./__fixtures__/player-summaries.json", import.meta.url),
		"utf8",
	);
...
test("steamCollect - rejects a vanity resolver response that is not a SteamID64", async () => {
	...
	globalThis.fetch = () =>
		Promise.resolve(
			new Response(
				JSON.stringify({ response: { success: 1, steamid: "display-name" } }),
				{ status: 200 },
			),
		);
	process.env.STEAM_USER_ID = "synthetic-profile";
...
function startSteamFixtureServer(
	requiredEndpoint401: boolean,
	...
	return new Promise((resolveServer, rejectServer) => {
		const server = createServer((req: IncomingMessage, res: ServerResponse) => {
			...
			if (path.endsWith("/GetPlayerSummaries/v0002")) {
				body = {
					response: {
						players: [{ steamid: "76561198012345678", personaname: "UAT" }],
					},
				};
			} else if (
				path.endsWith("/GetOwnedGames/v0001") ||
				path.endsWith("/GetRecentlyPlayedGames/v0001")
			) {
				body = { response: { games: [] } };
			} else {
...
test("Steam protocol execution: optional friends 401 emits exact SKIP_RESULT and succeeds", async () => {
	...
	try {
		const result = await runConnectorProtocolSubprocess({
			...
			env: {
				STEAM_API_BASE_URL: fixture.url,
				STEAM_API_KEY: "uat-key",
		...
		assert.equal(
			result.messages.some(
				(message) => message.type === "STATE" && message.stream === "friends",
			),
			false,
		);
		assert.equal(
			result.messages.some(
				(message) =>
					message.type === "DETAIL_COVERAGE" && message.stream === "friends",
			),
			false,
		);
		const profile = result.messages.find(
			(message) => message.type === "RECORD" && message.stream === "profile",
		);
		assert.ok(
			profile,
			"the valid profile returned by Steam must reach the protocol",
		);
