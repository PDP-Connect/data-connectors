...
async function collectStream(
	...
	globalThis.fetch = (input) => {
		...
		if (
			url.pathname.endsWith("/GetOwnedGames/v0001") ||
			url.pathname.endsWith("/GetRecentlyPlayedGames/v0001")
		) {
	...
	try {
		await steamCollect({
			...
			emit: (msg) => {
				...
				if (
					candidate.type === "DETAIL_COVERAGE" &&
...
test("steam owned_games - a truncated games array reads partial against the declared game_count", async () => {
	...
	const { coverage, recordCount } = await collectStream("owned_games", {
		response: {
			game_count: 10,
			games: [ownedGame(10), ownedGame(20), ownedGame(30)],
		},
	});
	...
	assert.equal(
		coverage?.considered,
		10,
		"denominator must be Steam's declared game_count, not the served length",
	);
	assert.equal(
		coverage?.covered,
		3,
		"covered counts only what was actually served and validated",
	);
	assert.ok(
...
test("steam owned_games - a complete response reads fully covered", async () => {
	...
	assert.equal(
		coverage?.covered,
		2,
		"an untruncated snapshot still proves full coverage",
	);
});
...
test("steam recently_played_games - a truncated array reads partial against total_count", async () => {
	...
	assert.equal(
		coverage?.considered,
		5,
		"denominator must be Steam's declared total_count",
	);
...
test("steam recently_played_games - the documented empty shape proves an empty window", async () => {
	...
	// not a failure.
	const { coverage, recordCount } = await collectStream(
