...
function nextStateFrom(messages: EmittedMessage[]): Record<string, unknown> {
	const state = messages
		.filter((m) => m.type === "STATE" && m.stream === "transactions")
		.at(-1);
	return {
		transactions:
...
test("transactions: re-downloading the same transactions (only fetched_at differs) is fully suppressed", async () => {
	const txns = [
		makeTxn({ fitid: "F1" }),
		makeTxn({ fitid: "F2", name: "GROCERY", amount_cents: -8123 }),
	];
	...
	assert.equal(
		run1.emitted.length,
		2,
		"first run emits both transactions once",
	);
...
test("transactions: QFX acquisition-mode source changes do not re-emit the same transaction", async () => {
	...
	await emitTransactionsStateIfAny(run1.deps);
	assert.equal(
		(run1.emitted[0]?.data as { source?: string } | undefined)?.source,
		"qfx_download_all_2026-04-10",
	);
	...
	assert.equal(
		run2.emitted.length,
		0,
		"same transaction stays silent when only source/fetched_at move",
	);
});
...
test("transactions: a real field move (amount correction) on an existing id re-emits", async () => {
	...
	assert.equal(
		run2.emitted.length,
		1,
		"a corrected amount is a real change and re-emits",
	);
});
...
test("transactions: NO prune — a transaction omitted from a narrower window keeps its fingerprint", async () => {
	...
	assert.equal(
		run2.emitted.length,
		0,
		"F2 unchanged stays silent; F1 not looked at this run",
	);
	...
	assert.equal(
		run3.emitted.length,
		0,
		"re-downloaded F1 stays suppressed because it was never pruned",
	);
});
...
test("transactions: STATE carries BOTH per_account and a transaction-field fingerprints map", async () => {
	...
	const cursorState = nextStateFrom(run.messages).transactions as Record<
		string,
		unknown
	>;
...
test("readPriorTransactionFingerprints: tolerates missing / legacy / malformed / bare-inner state", () => {
	...
	assert.equal(
		readPriorTransactionFingerprints({ transactions: { per_account: {} } })
			.size,
		0,
	...
	const nested = readPriorTransactionFingerprints({
		transactions: { fingerprints: { "A|F1": "fp-1", bad: null } },
	});
	...
	const bare = readPriorTransactionFingerprints({
		fingerprints: { "A|F1": "fp-1" },
	});
