...
function minimalHtml(): string {
	return readFileSync(
		join(FIXTURE_DIR, "current-activity-minimal.html"),
		"utf8",
	);
}
...
function makeDeps(
	...
	const deps: EmitDeps = {
		...
		requested: new Map<string, StreamScope>([
			["current_activity", { name: "current_activity" }],
		]),
		resFilters: new Map(),
...
function nextStateFrom(messages: EmittedMessage[]): Record<string, unknown> {
	const state = messages
		.filter((m) => m.type === "STATE" && m.stream === "current_activity")
		.at(-1);
	return {
		current_activity:
...
test("current_activity: a pending → posted transition on a stable ui_transaction_id re-emits", async () => {
	...
	assert.equal(
		reemitted.length,
		1,
		"only the pending→posted row re-emits; the unchanged payroll row stays silent",
	);
	const [row] = reemitted;
	assert.ok(row, "row was captured");
	assert.equal(
		(row.data as { id: string }).id,
		`${ACCOUNT.internal_id}|txn_20260514_A1`,
	);
...
test("current_activity: a zero-row run carries prior fingerprints forward unchanged (no prune)", async () => {
	...
	const run1 = makeDeps(FROZEN_EMITTED_AT_1, cursor1);
	await runCurrentActivity(run1.deps, minimalHtml(), [ACCOUNT]);
	const before = readPriorCurrentActivityFingerprints(
		nextStateFrom(run1.messages),
	);
	...
	const cursor2 = openCursorFrom(state2);
	const run2 = makeDeps(FROZEN_EMITTED_AT_2, cursor2);
	await runCurrentActivity(run2.deps, minimalHtml(), [
		ACCOUNT,
		{ ...ACCOUNT, internal_id: "INTACC456" },
	]);
	assert.equal(
		run2.emitted.filter((r) => r.stream === "current_activity").length,
		0,
	...
	const after = readPriorCurrentActivityFingerprints(
		nextStateFrom(run2.messages),
	);
	assert.equal(
		after.size,
		2,
		"prior fingerprints carried forward unchanged across the zero-row run",
	);
});

test("readPriorCurrentActivityFingerprints: tolerates missing / legacy / malformed state", () => {
	assert.equal(
		readPriorCurrentActivityFingerprints({}).size,
		0,
		"empty state → empty map",
	);
	assert.equal(
		readPriorCurrentActivityFingerprints({
			current_activity: { fetched_at: "2026-06-01T00:00:00Z" },
		}).size,
		0,
	...
	assert.equal(
		readPriorCurrentActivityFingerprints({
			current_activity: { fingerprints: 5 },
		}).size,
		0,
...
test("current_activity: legacy callers without a cursor still emit unconditionally", async () => {
	...
	assert.equal(
		run.emitted.filter((r) => r.stream === "current_activity").length,
		2,
		"no cursor → emits all rows",
	);
});
