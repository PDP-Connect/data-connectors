...
function makeClickOnlyLocator(
	calls: ClickCall[],
	kind: ClickCall["kind"],
	opts: FakeLocatorOptions,
): Locator {
	const base: Pick<Locator, "click" | "first"> = {
		click(
			_options?: Parameters<Locator["click"]>[0],
		): ReturnType<Locator["click"]> {
			calls.push({ kind });
			if (opts.clickError) {
...
function makePage(opts: { cssClickError?: Error; roleClickError?: Error }): {
	calls: ClickCall[];
	page: Page;
} {
	...
	const fake: Pick<Page, "getByRole" | "locator"> = {
		getByRole(
			...
			return makeClickOnlyLocator(calls, "role", {
				clickError: opts.roleClickError,
			});
		},
		locator(selector: Parameters<Page["locator"]>[0]): Locator {
			assert.equal(selector, "mds-button#download");
			return makeClickOnlyLocator(calls, "css", {
				clickError: opts.cssClickError,
			});
		},
	};
...
test("clickDownloadButton surfaces both failure reasons when the CSS and role click both fail", async () => {
	const cssClickError = new Error(
		"locator.click: Timeout 10000ms exceeded (css)",
	);
	const roleClickError = new Error(
		"locator.click: Timeout 10000ms exceeded (role)",
	);
