...
test("accounts: re-emitting with only a new fetched_at is fully suppressed", async () => {
	...
	assert.equal(
		run2.emitted.length,
		0,
		"unchanged accounts fully suppressed despite new fetched_at",
	);
});
...
test("accounts: a changed account field re-emits; unchanged ones stay silent", async () => {
	...
	assert.equal(
		run2.emitted.length,
		1,
		"a renamed account is a real change and re-emits",
	);
});
...
test("accounts: legacy callers without a cursor emit unconditionally and write no accounts STATE", async () => {
	...
	const accountsState = run.messages.filter(
		(m) => m.type === "STATE" && m.stream === "accounts",
	);
	assert.equal(
		accountsState.length,
		0,
		"no cursor → no accounts STATE written",
	);
});
