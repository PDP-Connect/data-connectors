...
function accountsSelfCoverage(
	...
	return messages.find(
		(m): m is DetailCoverageMessage =>
			...
			m.stream === "accounts" &&
			m.state_stream === "accounts",
	);
}

test("accounts considered: a fresh run declares considered === covered === enumerated, all emitted", async () => {
	const accounts = [
		makeAccount({ internal_id: "A1" }),
		makeAccount({ internal_id: "A2", name: "Freedom Unlimited" }),
	];
	...
	assert.deepEqual(
		cov?.required_keys,
		[],
		"list stream: no detail-hydration required keys",
	);
	assert.deepEqual(
		cov?.hydrated_keys,
		[],
		"list stream: no detail-hydration hydrated keys",
	);
});
...
test("accounts considered: a steady-state run declares covered === considered while collected is 0", async () => {
	...
	assert.equal(
		run2.emitted.length,
		0,
		"steady-state run emits nothing (all suppressed)",
	);
	...
	assert.equal(
		cov?.covered,
		2,
		"covered counts suppressed-unchanged, NOT aliased to collected (0)",
	);
});
...
test("accounts considered: a one-changed run keeps covered === considered", async () => {
	...
	await emitAccountsStream(
		run1.deps,
		[
			makeAccount({ internal_id: "A1", name: "Old Name" }),
			makeAccount({ internal_id: "A2" }),
		],
		cursor1,
	);
	...
	await emitAccountsStream(
		run2.deps,
		[
			makeAccount({ internal_id: "A1", name: "Renamed Card" }),
			makeAccount({ internal_id: "A2" }),
		],
		cursor2,
	);
	...
	assert.equal(
		cov?.covered,
		2,
		"covered (1 emitted + 1 suppressed) === considered, not the collected count of 1",
	);
});
