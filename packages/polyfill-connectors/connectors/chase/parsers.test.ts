...
test("isUsablePdfBuffer: a real PDF (with %PDF magic) is usable", () => {
	assert.equal(
		isUsablePdfBuffer(Buffer.from("%PDF-1.7\n%âãÏÓ\n1 0 obj", "latin1")),
		true,
	);
});
...
test("isUsablePdfBuffer: an HTML error page served as 200 is NOT usable", () => {
	assert.equal(
		isUsablePdfBuffer(Buffer.from("<!DOCTYPE html><html>Error</html>")),
		false,
	);
});
...
test("extractFromQfx: empty / unrecognized shape returns empty", () => {
	...
	assert.deepEqual(extractFromQfx({ OFX: { NOTHING: {} } }), {
		transactions: [],
		balance: null,
	});
});
...
test("parseDashboardAccountsDom: empty page returns []", () => {
	assert.deepEqual(
		parseDashboardAccountsDom("<!doctype html><html><body></body></html>"),
		[],
	);
...
test("parseCurrentActivityDom: extracts pending and posted UI-visible rows", () => {
	const rows = parseCurrentActivityDom(
		readFixture("current-activity-minimal.html"),
		"2026-05-15",
	);
...
test("currentActivityId: prefers UI id and otherwise uses deterministic fallback", () => {
	const [withUiId, withoutUiId] = parseCurrentActivityDom(
		readFixture("current-activity-minimal.html"),
		"2026-05-15",
	);
	assert.ok(withUiId);
...
test("currentActivityId: positional MDS row ids fall back to transaction content", () => {
  ...
  `;
	...
	assert.notEqual(
		currentActivityId("ACC", first),
		currentActivityId("ACC", later),
	);
});

test("parseCurrentActivityDom: ignores ancestor activity containers and emits only leaf rows", () => {
	const rows = parseCurrentActivityDom(
		readFixture("current-activity-wrapped-rows.html"),
		"2026-05-15",
	);
	assert.equal(rows.length, 2);
	assert.deepEqual(
...
test("parseCurrentActivityDom: real dashboard overview shape (committed extract) yields all 5 MDS rows", () => {
	const rows = parseCurrentActivityDom(
		readFixture("current-activity-dashboard-overview-real.html"),
		"2026-05-15",
	);
	...
	for (const r of rows) {
		assert.equal(
			r.ui_transaction_id,
			null,
			"positional MDS row ids must not become stable transaction ids",
		);
	}
});
...
test("parseCurrentActivityDom: QFX download form (broken surface) yields zero rows", () => {
	...
	// silently succeeding.
	const rows = parseCurrentActivityDom(
		readFixture("current-activity-download-form-no-rows.html"),
		"2026-05-15",
	);
...
test("parseCurrentActivityDom: extracts Chase MDS dashboard rows from data-values", () => {
	const rows = parseCurrentActivityDom(
		readFixture("current-activity-mds-overview.html"),
		"2026-05-15",
	);
...
test("parseStatementsListDom: extracts statement rows (tax documents skipped)", () => {
	...
	assert.equal(
		second.rowAnchorId,
		"accountsTable-0-row1-cell3-requestThisDocumentAnchor-download",
	);
...
test("parseStatementsListDom: empty page returns []", () => {
	assert.deepEqual(
		parseStatementsListDom("<!doctype html><html><body></body></html>"),
		[],
	);
});
...
test("resolveAccountIdForRow: null account_reference returns null", () => {
	assert.equal(
		resolveAccountIdForRow(makeRow({ account_reference: null }), []),
		null,
	);
});
...
test("resolveAccountIdForRow: no match returns null", () => {
	const accounts: ChaseAccount[] = [
		makeAccount({ internal_id: "A", name: "Foo", last_four: "0000" }),
	];
...
test("chooseActivity: no hints → all (bootstrap)", () => {
	assert.equal(
		chooseActivity(new Map(), {}, "transactions", "ID", "2026-08-13T12:00:00Z")
			.activity,
		"all",
	);
});
...
test("parseDashboardAccountsDom: local real capture parses ≥1 account", {
	skip: latestLocalRawDir() === null,
}, () => {
	const dir = latestLocalRawDir();
	if (!dir) {
	...
	assert.ok(
		accounts.length >= 1,
		`expected ≥1 account, got ${accounts.length}`,
	);
