...
test("semantic unknown-key tracking stays indexed to semanticSegments after another root array", async () => {
	...
	try {
		...
		assert.deepEqual(
			(semanticValues[0] as Record<string, unknown>).providerFutureField,
			{},
		);
	} finally {
...
test("an oversized UNRELATED trailing root field after the last recognized element does not throw too-large", async () => {
	const segments = Array.from({ length: 200 }, () => ({
		...
		duration: {
			endTimestamp: "2024-06-05T14:00:00.000Z",
			startTimestamp: "2024-06-05T13:45:22.000Z",
		},
	}));
	...
	const unrelatedTrailingValue = "x".repeat(
		maxSingleElementBytes + 1024 * 1024,
	);
	const content = JSON.stringify({
...
test("a large semanticSegments aggregate is normalized incrementally instead of rejected as record_too_large", async () => {
	...
	assert.ok(
		Buffer.byteLength(content, "utf8") > 4 * 1024 * 1024,
	...
	try {
		...
		assert.equal(
			elementCount,
			pathPoints.length,
			"each path point must reach the normalizer exactly once",
		);
	} finally {
		rmSync(dir, { force: true, recursive: true });
	}
...
test("streamed semantic segments match buffered segments in either field order without synthetic duplicates", async () => {
	const segmentBeforePath = {
		duration: {
			startTimestamp: "2024-06-05T13:45:22Z",
			endTimestamp: "2024-06-05T14:00:00Z",
		},
		activity: { activityType: "WALKING" },
		timelinePath: [
			{ point: "geo:37.4219999,-122.0840575", time: "2024-06-05T13:45:22Z" },
		],
	};
	...
	for (const segment of [segmentBeforePath, segmentAfterPath]) {
		...
		try {
			...
			const streamedSegments = streamedValues.flatMap(
				(value) =>
					parseGoogleMapsExportElement("semantic_segments", value).segments,
			);
			const bufferedSegments = parseGoogleMapsExport(
				JSON.parse(content),
			).segments;
...
test("a huge timelinePath stays bounded under a hard heap limit while unrelated fields remain guarded", async () => {
	...
	try {
		const point = JSON.stringify({
			point: "geo:37.4219999,-122.0840575",
			time: "2024-06-05T13:45:22Z",
		});
		const out = createWriteStream(path);
		out.write(
			'{"semanticSegments":[{"duration":{"startTimestamp":"2024-06-05T13:45:22Z"},"timelinePath":[',
		);
		...
		const childPath = new URL(
			"./oversized-element-oracle.test.child.ts",
			import.meta.url,
		);
		...
		assert.equal(
			result.status,
			0,
			`huge path must stream under the heap limit: ${result.stderr}`,
		);
...
test("an oversized PRIMITIVE array element (a bare string, not wrapped in an object) still throws GoogleMapsElementTooLargeError", async () => {
	...
	try {
		...
		assert.equal(
			elementCount,
			0,
			"the oversized primitive must never be emitted as an element",
		);
	} finally {
		rmSync(dir, { force: true, recursive: true });
	}
...
test("object and nested-array elements count across read-buffer boundaries", async () => {
	...
	try {
		const objectBytes = Buffer.byteLength(
			JSON.stringify(objectElement),
			"utf8",
		);
		await streamGoogleMapsExport(objectPath, () => undefined, {
			maxSingleElementBytes: objectBytes,
		});
		await assert.rejects(
			() =>
				streamGoogleMapsExport(objectPath, () => undefined, {
					maxSingleElementBytes: objectBytes - 1,
				}),
			GoogleMapsElementTooLargeError,
		);
	} finally {
	...
	try {
		...
		await streamGoogleMapsExport(arrayPath, () => undefined, {
			maxSingleElementBytes: arrayBytes,
		});
		await assert.rejects(
			() =>
				streamGoogleMapsExport(arrayPath, () => undefined, {
					maxSingleElementBytes: arrayBytes - 1,
				}),
			GoogleMapsElementTooLargeError,
		);
	} finally {
...
test("a recognized key with a malformed non-array value reports unsupported, not too-large", async () => {
	const content = JSON.stringify({
		locations: { note: "not an array" },
		trailing: "x".repeat(4096),
	});
	const { dir, path } = writeTmpFile(content);
	try {
		await assert.rejects(
