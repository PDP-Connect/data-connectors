...
function coverageFor(
	...
	assert.equal(
		all.length,
		1,
		`expected exactly one ${stream} DETAIL_COVERAGE, got ${String(all.length)}`,
	);
...
test("a point element with no timestamp stays in the denominator as considered-but-not-covered", async () => {
	const importRoot = await mkdtemp(
		join(tmpdir(), "pdpp-gm-unaccounted-point-"),
	);
	try {
		...
		const emitted = messages.filter(
			(m) => m.type === "RECORD" && m.stream === "timeline_points",
		).length;
		...
		assert.equal(
			coverage.considered,
			3,
			"the artifact's third element must remain in the denominator",
		);
		assert.equal(
			coverage.covered,
			2,
			"only accounted-for elements may be claimed as covered",
		);
		assert.ok(
			coverage.covered < coverage.considered,
			"an unaccounted element must read partial, never complete",
		);
	} finally {
...
test("an unaccounted point element surfaces an element_unaccounted skip naming the true denominator", async () => {
	...
	try {
		...
		const skip = messages.find(
			(m) =>
				m.type === "SKIP_RESULT" &&
				m.stream === "timeline_points" &&
				m.reason === "element_unaccounted",
		);
		assert.ok(
			skip,
			"a dropped element must be visible as a skip, not swallowed",
		);
		assert.equal(
			(skip as { diagnostics?: { considered?: number; unaccounted?: number } })
				.diagnostics?.unaccounted,
			1,
		);
		assert.equal(
			(skip as { diagnostics?: { considered?: number } }).diagnostics
				?.considered,
			2,
		);
	} finally {
...
test("a segment element with no start time stays in the denominator", async () => {
	const importRoot = await mkdtemp(
		join(tmpdir(), "pdpp-gm-unaccounted-segment-"),
	);
	try {
		...
		assert.equal(
			coverage.considered,
			2,
			"both artifact elements are in the denominator",
		);
...
test("a fully accountable artifact still reconciles as complete on both streams", async () => {
	...
	try {
		...
		assert.equal(
			messages.some(
				(m) => m.type === "SKIP_RESULT" && m.reason === "element_unaccounted",
			),
			false,
