...
function isLikelyTimelineJson(path: string): boolean {
	...
	return (
		lowerPath.includes("location history") ||
		lowerPath.includes("timeline") ||
		lowerPath.endsWith("/records.json")
	);
}
...
function markDiscoveryIncomplete(
	...
	reason: DiscoveryIncompleteReason,
): void {
...
async function walkDiscoveryDirectory(
	dir: string,
	depth: number,
	state: DiscoveryState,
): Promise<void> {
...
async function discoverTimelineFiles(
	importDir: string,
): Promise<TimelineDiscovery> {
...
async function emitRequestedSkip(
	ctx: CollectContext,
	reason: string,
	message: string,
): Promise<void> {
...
type GoogleMapsEmissionError = Error & {
	readonly [GOOGLE_MAPS_EMISSION_ERROR]: true;
};
...
function isGoogleMapsEmissionError(
	error: unknown,
): error is GoogleMapsEmissionError {
	return (
		error instanceof Error &&
		(error as GoogleMapsEmissionError)[GOOGLE_MAPS_EMISSION_ERROR] === true
	);
}
...
async function emitRecordSafely(
	ctx: CollectContext,
	stream: string,
...
async function admitRecord(
	...
	const requested =
		stream === "timeline_points"
			? load.requestedPoints
			: load.requestedSegments;
	if (requested) {
...
async function processPointRecords(
	load: StreamLoadContext,
	points: readonly TimelinePointRecord[],
): Promise<void> {
	for (const point of points) {
		...
		load.summary.latestPoint = maxTimestamp(
			load.summary.latestPoint,
			timestamp,
		);
		if (
			!load.requestedPoints ||
			(load.pointSince && timestamp <= load.pointSince)
		) {
			continue;
		}
...
function recordUnrecognizedSegment(
	summary: LoadSummary,
	...
	summary.unrecognizedKinds.add(
		segment.unrecognized_kind ?? "(no payload key)",
	);
}
...
async function processSegmentRecords(
	...
	for (const segment of segments) {
		...
		const startTime =
			typeof segment.start_time === "string" ? segment.start_time : null;
		...
		load.summary.latestSegment = maxTimestamp(
			load.summary.latestSegment,
			startTime,
		);
		recordUnrecognizedSegment(load.summary, segment);
		if (
			!load.requestedSegments ||
			(load.segmentSince && startTime <= load.segmentSince)
		) {
			continue;
		}
...
async function streamTimelineFile(
	load: StreamLoadContext,
	file: string,
): Promise<void> {
	await streamGoogleMapsExport(file, (event) =>
		processStreamEvent(load, event),
	);
}

async function reportSourceFailure(
	ctx: CollectContext,
	error: unknown,
): Promise<void> {
	...
	if (oversized) {
		...
		message =
			"A Google Maps Timeline export contains a recognized key whose value is not an array";
	}
...
async function loadExports(
	...
	if (!discovery.complete && discovery.incompleteReason) {
		await emitRequestedSkip(
			ctx,
			"source_incomplete",
			discoveryFailureMessage(discovery.incompleteReason),
		);
	}
	...
	if (summary.pointsUnaccounted > 0) {
		await ctx.emit({
			...
			diagnostics: {
				...
				unaccounted: summary.pointsUnaccounted,
			},
		});
	}
	await ctx.emit(
...
async function finishSegments(
	...
	if (summary.segmentsUnaccounted > 0) {
		await ctx.emit({
			...
			diagnostics: {
				...
				unaccounted: summary.segmentsUnaccounted,
			},
		});
		await ctx.emit(
			buildDetailCoverageMessage({
	...
	await ctx.emit(
		buildFullScanCoverageMessage("timeline_segments", summary.segmentCount),
	);
}
...
runConnector({
	...
	async collect(ctx) {
		const importDir =
			process.env.GOOGLE_MAPS_TIMELINE_DIR ||
			join(homedir(), ".pdpp/imports/google_maps");
		...
		if (requestedPoints && summary.complete && summary.pointCount === 0) {
			await ctx.emit({
				...
				message:
					"Google Maps Timeline point records were not found in the configured import directory",
			});
		}
		if (requestedSegments && summary.complete && summary.segmentCount === 0) {
			await ctx.emit({
				...
				message:
					"Google Maps Timeline segment records were not found in the configured import directory",
			});
		}
