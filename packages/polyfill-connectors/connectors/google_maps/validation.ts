...
function detectFormat(
	json: unknown,
): GoogleMapsSourceFormat | "unsupported" | "mixed" {
...
function buildValidationFromCounts(
	...
	fileSha256: string,
	options: Pick<
		GoogleMapsTimelineValidationOptions,
		"existingFileHashes" | "importedThrough"
	>,
): GoogleMapsTimelineValidation {
	...
	if (previousHashes.has(fileSha256)) {
		...
		options.importedThrough &&
		dateRange.end &&
		dateRange.end <= options.importedThrough
	) {
		status = "stale";
	}
 ...
 */
function resolveStreamedFormat(
	counts: StreamedCounts,
