...
type SemanticProjection = Record<string, unknown> & {
	timelinePath: unknown[] | undefined;
};
...
function formatForShapeKey(key: string): GoogleMapsSourceFormat | null {
	return (
		(
			GOOGLE_MAPS_SHAPE_KEY_TO_FORMAT as Readonly<
				Record<string, GoogleMapsSourceFormat | undefined>
			>
		)[key] ?? null
	);
}
...
class RootShapeTracker {
	...
	private onObjectToken(
		frame: Extract<ShapeFrame, { kind: "object" }>,
		token: TokenType,
		value: unknown,
	): void {
	...
	private startObjectValue(
		frame: Extract<ShapeFrame, { kind: "object" }>,
		token: TokenType,
	): void {
		const key = frame.pendingKey;
		const format =
			this.stack.length === 1 && key ? formatForShapeKey(key) : null;
		frame.pendingKey = undefined;
	...
	private onArrayToken(
		frame: Extract<ShapeFrame, { kind: "array" }>,
		token: TokenType,
	): void {
...
class ElementByteTracker {
	...
	consume(text: string): void {
		for (const char of text) {
			this.consumeChar(
				char,
				char.charCodeAt(0) < 0x80 ? 1 : Buffer.byteLength(char, "utf8"),
			);
		}
	}
	...
	private count(bytes: number): void {
		...
		if (
			...
			this.stack.some(
				(candidate) =>
					candidate.kind === "array" && candidate.excludeFromParentElementBytes,
			)
		) {
			return;
		}
...
type GoogleMapsStreamEventHandler = (
...
export async function streamGoogleMapsExport(
	...
	const maxSingleElementBytes =
		options.maxSingleElementBytes ?? DEFAULT_MAX_SINGLE_ELEMENT_BYTES;
	const pending: GoogleMapsStreamEvent[] = [];
