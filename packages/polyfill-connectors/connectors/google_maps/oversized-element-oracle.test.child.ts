...
if (!(path && fileSizeArg)) {
	console.error(
		"usage: oversized-element-oracle.test.child.ts <path> <fileSize>",
	);
	process.exit(2);
}
...
if (mode === "whole-buffer") {
	...
	process.stdout.write(
		`${JSON.stringify({ status: Array.isArray(json.locations) ? "valid" : "unsupported" })}\n`,
	);
	process.exit(0);
} else if (mode === "stream") {
	let elements = 0;
	streamGoogleMapsExport(path, (event) => {
		...
		.then(() => {
			process.stdout.write(
				`${JSON.stringify({ elements, status: "valid" })}\n`,
			);
			process.exit(0);
		})
		.catch((err) => {
