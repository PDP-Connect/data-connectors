...
const MODULE_MOCKS_AVAILABLE =
	typeof (test.mock as { module?: unknown }).module === "function";
	...
	test.mock.module("node:fs/promises", {
		namedExports: {
			...
			readFile: (...args: Parameters<typeof realFsPromises.readFile>) => {
				...
				return (realFsPromises.readFile as typeof realFsPromises.readFile)(
					...args,
				);
			},
		},
	...
	semanticSegments: [
		{
			activity: {
				...
				topCandidate: { probability: 0.9, type: "WALKING" },
			},
			duration: {
...
const VALID_TIMELINE_OBJECTS_WRAPPED = {
	timelineObjects: VALID_TIMELINE_OBJECTS_ROOT_ARRAY,
};
...
test("validateGoogleMapsTimelineArtifactFromFile: identifies unsupported (malformed JSON) artifacts without buffering", async () => {
	...
	try {
		const streamed = await validateGoogleMapsTimelineArtifactFromFile(
			path,
			30,
			{ fileSha256: "x" },
		);
...
test("a single oversized array element reports too_large, not an OOM crash or a silent valid pass", async () => {
	...
	const content = JSON.stringify({
		locations: [{ note: junk, timestampMs: "1717595122000" }],
	});
	const { dir, path } = writeTmpFile(content);
	try {
		const streamed = await validateGoogleMapsTimelineArtifactFromFile(
...
test("validateGoogleMapsTimelineArtifactFromFile: a large sparse export never calls fs/promises.readFile against its own path", {
	...
	try {
		...
		const callsOnThisPath = callsAfter.filter(
			([pathArg]) => String(pathArg) === path,
		);
		assert.deepEqual(
			callsOnThisPath,
			[],
...
test("outcome proof: a 110 MiB sparse Timeline export separates streaming from whole-buffer parsing at one heap limit", async () => {
	...
	const perElementBytes =
		Buffer.byteLength(JSON.stringify(template), "utf8") + 1;
	...
	try {
		...
		assert.ok(
			...
			`fixture must exceed the manifest cap, got ${fileSize} bytes`,
		);
...
test("outcome proof: large wrapped locations and semanticSegments arrays stay below a hard heap limit", async () => {
	...
	const dir = mkdtempSync(
		join(tmpdir(), "pdpp-google-maps-wrapped-heap-oracle-"),
	);
	try {
		const runShape = async (shape: (typeof shapes)[number]): Promise<void> => {
			...
			const itemCount = Math.ceil(
				(TARGET_BYTES - 32) / (Buffer.byteLength(itemText, "utf8") + 1),
			);
			const { createWriteStream } = await import("node:fs");
			const out = createWriteStream(path);
			out.write(`{"${shape.key}":[`);
			...
			assert.ok(
				...
				`${shape.key} fixture must be large enough, got ${fileSize} bytes`,
			);
		...
		await shapes.reduce(
			(previous, shape) => previous.then(() => runShape(shape)),
			Promise.resolve(),
		);
	} finally {
		rmSync(dir, { force: true, recursive: true });
...
test("outcome proof: an oversized primitive element fails before tokenizer buffering can OOM", async () => {
	...
	const dir = mkdtempSync(
		join(tmpdir(), "pdpp-google-maps-primitive-p1-oracle-"),
	);
	const path = join(dir, "Timeline.json");
	try {
		writeFileSync(path, content);
