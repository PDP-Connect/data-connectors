...
test("parseGoogleMapsExport: retains an unmodelled semantic segment as unrecognized", () => {
	...
	assert.equal(
		...
		1,
		"the segment is retained, not dropped",
	);
	assert.equal(result.segments[0]?.segment_kind, "unrecognized");
	assert.equal(
		result.segments[0]?.unrecognized_kind,
		"timelineMemory",
		"provider key preserved verbatim",
	);
...
test("parseGoogleMapsExport: an unrecognized segment does not collide with a path segment", () => {
	...
	const [path] = parseGoogleMapsExport({
		semanticSegments: [
			{
				startTime: "2024-06-05T13:00:00Z",
				timelinePath: [{ point: "geo:1,2", time: "2024-06-05T13:00:00Z" }],
			},
		],
	}).segments;
	...
	assert.notEqual(
		unrecognized?.id,
		path?.id,
		"distinct kinds at the same timestamp get distinct ids",
	);
});
