...
test("FORMAT-CONFORMANCE: synthetic multi-year, multi-source export.xml (~5k records) parses end to end with honest gap reporting", {
	...
	try {
		...
		assert.equal(
			recordRows.length,
			expectedRecords,
			`expected ${expectedRecords} emitted records`,
		);
		assert.equal(
			workoutRows.length,
			stats.workoutsTotal,
			"every workout has a parseable startDate",
		);
		...
		assert.ok(
			recordRows.some(
				(r) =>
					...
					(r.device as string).includes("iPhone"),
			),
		);

		// Workouts: events/statistics/metadata fidelity.
		const withEvents = workoutRows.filter(
			(w) => Array.isArray(w.events) && (w.events as unknown[]).length > 0,
		);
		const withStats = workoutRows.filter(
			(w) =>
				Array.isArray(w.statistics) && (w.statistics as unknown[]).length > 0,
		);
		assert.equal(
			withEvents.length,
			stats.workoutsWithEvents,
			"workout event counts must match ground truth",
		);
		assert.equal(
			withStats.length,
			stats.workoutsWithStatistics,
			"workout statistics counts must match ground truth",
		);
		const zeroDuration = workoutRows.filter((w) => w.duration_minutes === 0);
		assert.equal(
			zeroDuration.length,
			stats.zeroDurationWorkouts,
			"zero-duration workout must still be captured",
		);
		...
		const dstExpected = new Date(
			`${syntheticExportStartYear() + 1}-03-09T01:59:00-08:00`,
		).toISOString();
		const dstRow = recordRows.find((r) => r.start_date === dstExpected);
		assert.ok(
			dstRow,
			`DST-boundary record must parse without throwing or vanishing (expected ${dstExpected})`,
		);
	} finally {
