	...
	async collect({ state, requested, emit, emitRecord, progress }) {
		...
		if (!path) {
			await emit({
				...
				message:
					"Apple Health export data was not found in the configured import directory",
			});
			return;
		}
		...
		await streamParse({
			...
			onRecord: (attrs): Promise<void> =>
				handleRecord(attrs, recordRef, requested, emitRecord),
			onWorkout: (attrs): Promise<void> =>
				handleWorkout(attrs, workoutRef, requested, emitRecord),
		});
