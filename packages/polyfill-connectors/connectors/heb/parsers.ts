...
function parseDetailItem(link: HTMLAnchorElement): DetailItem | null {
	...
	const rawPosition =
		row.getAttribute("data-index") ?? row.getAttribute("aria-posinset");
	const sourcePosition =
		rawPosition !== null && POSITION_RE.test(rawPosition.trim())
			? Number(rawPosition)
			: null;
...
export function orderItemId(
	...
	itemIndex: number,
	siblingItems: readonly Pick<
		DetailItem,
		"productId" | "name" | "sourcePosition"
	>[] = [item],
): string {
	...
	const duplicate = siblingItems.some((sibling, siblingIndex) => {
		...
		const siblingKey =
			sibling.productId ||
			sibling.name.replace(WHITESPACE_RE, " ").trim().toLowerCase() ||
			"unknown";
		return siblingKey === key;
	});
	return duplicate
		? `${orderId}|${key}|${item.sourcePosition ?? itemIndex}`
		: `${orderId}|${key}`;
}
