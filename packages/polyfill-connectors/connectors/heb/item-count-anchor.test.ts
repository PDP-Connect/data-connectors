...
test("tallyOrderItems confirms a fully-collected order", () => {
	const verdict = tallyOrderItems({
		orderId: "o1",
		declaredItemCount: 12,
		collectedItemCount: 12,
	});
...
test("tallyOrderItems catches the real live shortfall", () => {
	...
	const verdict = tallyOrderItems({
		orderId: "HEB20607368035",
		declaredItemCount: 85,
		collectedItemCount: 54,
	});
...
test("tallyOrderItems catches the second real live shortfall", () => {
	...
	const verdict = tallyOrderItems({
		orderId: "HEB20169324473",
		declaredItemCount: 59,
		collectedItemCount: 35,
	});
...
test("summarizeItemCounts reports a fully-reconciled run cleanly", () => {
	const summary = summarizeItemCounts([
		{ orderId: "o1", declaredItemCount: 5, collectedItemCount: 5 },
	]);
...
test("summarizeItemCounts bounds the id sample but keeps the count exact", () => {
	const many: OrderItemTally[] = Array.from(
		{ length: MAX_SHORT_ORDER_IDS_IN_DIAGNOSTIC + 20 },
		(_, i) => ({
			orderId: `o${i}`,
			declaredItemCount: 2,
	...
	assert.equal(
		summary.short,
		MAX_SHORT_ORDER_IDS_IN_DIAGNOSTIC + 20,
		"the COUNT must stay exact",
	);
	assert.equal(
		summary.shortOrderIds.length,
		MAX_SHORT_ORDER_IDS_IN_DIAGNOSTIC,
		"only the id sample is bounded",
	);
});
