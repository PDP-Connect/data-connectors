...
test("parseOrdersListDom parses a comma-separated (thousands-grouped) total (S4 regression)", () => {
	...
	assert.equal(
		order.total,
		"$1,234.56",
		"a thousands-grouped total must not be dropped to null",
	);
...
test("resolveDomMaxPage reads the highest page= from the pagination nav", () => {
	assert.deepEqual(resolveDomMaxPage(fixture("orders-list.html")), {
		kind: "resolved",
		source: "dom",
		value: 3,
	});
...
test("resolveDomMaxPage is absent (not silently 1) when there is no pagination nav", () => {
	assert.deepEqual(resolveDomMaxPage(fixture("orders-list-empty.html")), {
		kind: "absent",
	});
});
...
test("resolveMaxPage falls back to DOM when structured __NEXT_DATA__ is absent", () => {
	assert.deepEqual(resolveMaxPage(fixture("orders-list.html")), {
		kind: "resolved",
		source: "dom",
		value: 3,
	});
...
test("resolveStructuredMaxPage resolves a genuine single page (pages: [{to:'?page=1'}])", () => {
	assert.deepEqual(
		resolveStructuredMaxPage({ page: 1, pages: [{ to: "?page=1" }] }),
		{
			kind: "resolved",
			source: "structured",
			value: 1,
...
test("parseOrdersListStructured extracts every trustworthy row from props.pageProps.orders[]", () => {
	const orders = parseOrdersListStructured(
		fixture("orders-list-nextdata.html"),
	);
	assert.ok(orders);
...
test("parseOrdersListStructured maps status vs status_code as two distinct evidenced fields", () => {
	...
	assert.equal(
		first.status,
		"Delivered",
		"status keeps the human-readable orderStatusMessageShort meaning",
	);
	assert.equal(
		first.statusCode,
		"PAYMENT_RECEIPTED",
		"status_code carries the distinct machine-readable value",
	);
});
...
test("mergeOrdersListPage prefers the structured row's evidenced fields but keeps DOM-only fields", () => {
	...
	assert.equal(
		row.statusCode,
		"PAYMENT_RECEIPTED",
		"structured-only field is preserved",
	);
	assert.equal(
		row.total,
		"$100.00",
		"DOM-only field is carried over onto the preferred structured row",
	);
...
test("mergeOrdersListPage falls back to the DOM row when no structured row shares its order id (per-row fallback)", () => {
	...
	assert.ok(
		structuredRow,
		"a structured-only row with no DOM counterpart is still retained",
	);
});
...
test("parseOrderDetailDom preserves two purchased lines with the same product href", () => {
	...
	assert.equal(
		detail?.items.length,
		2,
		"same-product lines are distinct purchased items",
	);
});
...
test("an Imperva incident report sets the block flag the classifier branches on first", () => {
	const diag = diagnoseEmptyListPage(
		IMPERVA_INCIDENT_PAGE,
		"https://www.heb.com/my-account/order-history",
	);
...
test("isIncapsulaBlocked detects the incidentId+proxyId JSON body that carries no hostName", () => {
	...
	assert.ok(
		...
		"premise: this fixture genuinely has no hostName field",
	);
...
test("hasOrdersEmptyState is true for H-E-B's real 'No past orders' page", () => {
	assert.equal(
		hasOrdersEmptyState(fixture("orders-list-no-past-orders.html")),
		true,
	);
});
...
test("hasOrdersEmptyState is false for a populated order list", () => {
	...
	assert.equal(
		hasOrdersEmptyState(fixture("orders-list-nextdata.html")),
		false,
	);
});
...
test("hasOrdersEmptyState is false when the order-results container is absent entirely", () => {
	...
	assert.equal(
		hasOrdersEmptyState(
			`<html><body><div class="Empty_box__qxVTd"></div></body></html>`,
		),
		false,
	);
});
...
test("diagnoseEmptyListPage reports the real empty page as empty_state with the drift signature", () => {
	...
	assert.equal(
		diag.any_card,
		4,
		"the empty state's own class names are what made this look like drift",
	);
	assert.equal(
		diag.incapsula_block,
		false,
		"the captured page was served, not blocked",
	);
...
test("looksLoggedOut is true for a sign-in URL", () => {
	assert.equal(
		looksLoggedOut("https://www.heb.com/sign-in", fixture("orders-list.html")),
		true,
	);
});
...
test("diagnoseEmptyListPage reports zero order_cards for a legitimate terminal page", () => {
	const diag = diagnoseEmptyListPage(
		fixture("orders-list-empty.html"),
		"https://www.heb.com/my-account/your-orders",
	);
	assert.equal(diag.order_cards, 0);
...
test("orderItemId preserves the legacy provider-backed id for a unique line", () => {
	assert.equal(
		orderItemId("HEB123", { productId: "999", name: "Milk" }, 0),
		"HEB123|999",
	);
});
...
test("orderItemId falls back to a normalized name when product_id is absent", () => {
	assert.equal(
		orderItemId(
			"HEB123",
			{ productId: null, name: "  Organic   Bananas  " },
			0,
		),
		"HEB123|organic bananas",
	);
});
...
test("orderItemId: two same-name, product-id-null items in one order get distinct ids", () => {
	...
	assert.notEqual(
		first,
		second,
		"two null-product-id items with the same name must not collide",
	);
...
test("buildOrderItemRecord maps a parsed detail item into the emitted order_items shape", () => {
	...
	const milk = detail?.items[0];
	assert.ok(milk);
	const record = buildOrderItemRecord(
		"HEB1029384756",
		"2026-07-14",
		milk,
		0,
...
test("the promotional interstitial is not read as a source-authored empty state", () => {
	assert.equal(
		hasOrdersEmptyState(fixture("whats-new-modal-over-orders.html")),
		false,
	);
});
...
test("the promotional interstitial is not read as an Incapsula block", () => {
	assert.equal(
		isIncapsulaBlocked(fixture("whats-new-modal-over-orders.html")),
		false,
	);
});
