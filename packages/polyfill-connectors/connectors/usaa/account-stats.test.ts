...
function nextStateFrom(messages: EmittedMessage[]): Record<string, unknown> {
	const state = messages
		.filter((m) => m.type === "STATE" && m.stream === "accounts")
		.at(-1);
	return {
		accounts:
...
const STATS_OPTS = (observedOn: string) => ({
	emitEntity: true,
	emitStats: true,
	observedOn,
});
...
test("account_stats: a balance-only change does not re-emit the entity but does emit a fresh stat", async () => {
	...
	const accounts2 = [
		makeAccount({ account_id_raw: "A1", balance_cents: 95_000 }),
	];
	...
	assert.equal(
		entity2.length,
		0,
		"a balance-only move does NOT re-version the entity record",
	);
	assert.equal(
		stats2.length,
		1,
		"the new balance is captured on account_stats",
	);
	const [statsRow] = stats2;
	assert.ok(statsRow, "row was captured");
	assert.equal(
		(statsRow.data as { balance_cents?: number }).balance_cents,
		95_000,
	);
});
...
test("account_stats: a real identity/status change re-emits the entity exactly once", async () => {
	const accounts1 = [
		makeAccount({ account_id_raw: "A1", name: "USAA CLASSIC CHECKING" }),
	];
	const run1 = makeHarness();
	const cursor1 = openFingerprintCursor(undefined, {
		excludeFromFingerprint: ["fetched_at"],
	...
	const renamed = [
		makeAccount({ account_id_raw: "A1", name: "USAA PERFORMANCE CHECKING" }),
	];
	const priorState = nextStateFrom(run1.messages);
	const run2 = makeHarness();
	const cursor2 = openAccountsCursor(priorState);
	...
	assert.equal(
		entity2.length,
		1,
		"an identity change re-emits the entity exactly once",
	);
	const [entityRow] = entity2;
	assert.ok(entityRow, "row was captured");
	assert.equal(
		(entityRow.data as { name?: string }).name,
		"USAA PERFORMANCE CHECKING",
	);
});
...
test("account_stats: same-day re-pull is the same key; a later day is a distinct key", () => {
	...
	const sameDayB = buildAccountStatsRecord(
		{ ...a, balance_cents: 95_000 },
		DAY1,
	);
	...
	assert.equal(
		sameDayA.id,
		sameDayB.id,
		"same UTC day → same append key (idempotent / overwrite)",
	);
	assert.notEqual(
		sameDayA.id,
		laterDay.id,
		"a later UTC day → distinct append key (time series)",
	);
});
...
test("account_stats: emitted observation records pass the real zod schema", async () => {
	...
	assert.equal(
		stats.length,
		2,
		"both account_stats records passed the schema (negative balance allowed)",
	);
});
...
test("account_stats: an account_stats-only request emits the stream but no entity record or STATE", async () => {
	...
	assert.equal(
		run.emitted.filter((e) => e.stream === "accounts").length,
		0,
		"no entity record emitted",
	);
	assert.equal(
		run.emitted.filter((e) => e.stream === "account_stats").length,
		1,
		"one stat emitted",
	);
	const entityState = run.messages.filter(
		(m) => m.type === "STATE" && m.stream === "accounts",
	);
	const statsState = run.messages.filter(
		(m) => m.type === "STATE" && m.stream === "account_stats",
	);
	assert.equal(
		entityState.length,
		0,
		"no entity STATE written when the entity is not requested",
	);
	assert.equal(statsState.length, 1, "account_stats STATE written");
	assert.equal(
		(statsState[0] as { cursor?: { observed_on?: string } }).cursor
			?.observed_on,
		DAY1,
