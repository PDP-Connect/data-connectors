...
test("buildAccountTransactionDetailGap: locator carries only the account id, never PII (name/last_four)", () => {
	...
	assert.ok(
		!locatorValues.includes("USAA CLASSIC CHECKING"),
...
test("classifyCsvTransactionResult: a headers-only CSV (dormant account) is no_activity/covered, never a gap", () => {
	...
	const result = classifyCsvTransactionResult(
		"ACCT-DORMANT",
		"2026-01-15",
		"2025-01-01",
		{
	...
	assert.deepEqual(result.outcome, {
		accountId: "ACCT-DORMANT",
		kind: "no_activity",
	});
	assert.equal(
		result.last_date,
		"2026-01-15",
		"the prior watermark is preserved, not advanced or dropped",
	);
});
...
test("classifyCsvTransactionResult: data rows present but none usable is parse trouble → gap csv_no_usable_transactions", () => {
	...
	assert.equal(
		result.last_date,
		"2026-01-15",
		"the cursor never advances past unparsed rows",
	);
});
...
test("dormant headers-only CSV outcome flows to covered coverage with no DETAIL_GAP; unusable-rows outcome flows to gap with one", async () => {
	...
	const dormant = classifyCsvTransactionResult(
		"ACCT-DORMANT",
		"2026-01-15",
		null,
		{
	...
	assert.equal(
		gaps.length,
		1,
		"only the parse-trouble account carries a DETAIL_GAP",
	);
	assert.equal(
		gaps[0]?.type === "DETAIL_GAP" && gaps[0].record_key,
		"ACCT-BROKEN",
	);
	...
	assert.deepEqual(
		...
		["ACCT-DORMANT"],
		"the dormant account is covered",
	);
	...
	assert.equal(
		coverage.covered,
		1,
		"dormant counts as covered; parse trouble does not",
	);
});
...
test("emitTransactionsDetailCoverage: steady-state run — every account verified with zero new records still reports full coverage", async () => {
	...
	assert.deepEqual([...coverage.required_keys].sort(), [
		"ACCT-CHK-0001",
		"ACCT-SAV-0002",
	]);
	assert.deepEqual([...coverage.hydrated_keys].sort(), [
		"ACCT-CHK-0001",
		"ACCT-SAV-0002",
	]);
	...
	assert.equal(
		coverage.covered,
		2,
		"a steady-state verified account counts as covered",
	);
...
test("emitTransactionsDetailCoverage: a completed enumeration with ZERO eligible accounts still emits considered 0 / covered 0", async () => {
	...
	assert.ok(
		coverage,
		"a completed zero-eligible-account enumeration still emits DETAIL_COVERAGE",
	);
...
test("emitTransactionsDetailCoverage: a run cut short by session death before any account was attempted emits nothing", async () => {
	...
	assert.equal(
		coverageOf(messages),
		undefined,
		"an unknown denominator must never be reported as zero",
	);
});
