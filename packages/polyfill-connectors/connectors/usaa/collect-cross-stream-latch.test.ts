...
function makeCollectHarness(): {
	...
	const ctx: BrowserCollectContext = {
		...
		sendInteraction: async (): Promise<never> => {
			throw new Error(
				"sendInteraction must not be called — the fallback ensureUsaaSession throws before reaching it",
			);
		},
		state: {},
	};
...
test("collectUsaa: a session death detected in statements suppresses inbox_messages and credit_card_billing — kills the latchSessionDead-neutered mutant (M6)", async () => {
	await withFastTimers(async () => {
		...
		assert.ok(
			gotoCalls.includes(DOCUMENTS_URL),
			"statements stage ran and hit the session-dead bounce",
		);
		assert.ok(
		...
		const inboxSkips = messages.filter(
			(m) => m.type === "SKIP_RESULT" && m.stream === "inbox_messages",
		);
		assert.equal(
			inboxSkips.length,
			0,
		...
		const statementsSkips = messages.filter(
			(m) => m.type === "SKIP_RESULT" && m.stream === "statements",
		);
		assert.equal(
			statementsSkips.length,
			1,
			"statements itself reports exactly one session-dead SKIP_RESULT",
		);
		assert.equal(
			(statementsSkips[0] as { reason?: string }).reason,
...
test("collectUsaa: a clean run with no session death reaches every requested stream, including inbox_messages and credit_card_billing", async () => {
	await withFastTimers(async () => {
		...
		assert.ok(
			gotoCalls.includes(INBOX_URL),
			"a clean run must still reach inbox_messages",
		);
		const skips = harness.protocolMessages.filter(
			(m) =>
				m.type === "SKIP_RESULT" && m.reason === "session_dead_reauth_failed",
		);
		assert.equal(
			skips.length,
			0,
			"a clean run never reports a session-dead SKIP_RESULT anywhere",
		);
	});
});
