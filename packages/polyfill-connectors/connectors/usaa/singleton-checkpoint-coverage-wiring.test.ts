...
const NEVER_CALLED_SEND_INTERACTION: BrowserCollectContext["sendInteraction"] =
	async () => {
		throw new Error(
			"sendInteraction must not be called on the happy path — no session-death signal was produced",
		);
	};
...
function coverageFor(
	messages: EmittedMessage[],
	stream: string,
): DetailCoverageMessage | undefined {
...
function skipsFor(
	...
	return messages.filter(
		(m): m is Extract<EmittedMessage, { type: "SKIP_RESULT" }> =>
			m.type === "SKIP_RESULT" && m.stream === stream,
	);
}
...
function gapsFor(
	...
	return messages.filter(
		(m): m is DetailGapMessage =>
			m.type === "DETAIL_GAP" && m.stream === stream,
	);
}
...
function makeInboxPage(
	rows: InboxRow[] | (() => InboxRow[]),
	gotoFails = false,
): Page {
...
test("wiring: runInboxStream emits DETAIL_COVERAGE with considered/covered after a successful scrape", async () => {
	await withFastTimers(async () => {
		...
		assert.equal(
			run.emitted.filter((e) => e.stream === "inbox_messages").length,
			2,
			"both rows emitted",
		);
...
test("wiring: runInboxStream emits SKIP_RESULT and NO coverage when page.goto throws", async () => {
	await withFastTimers(async () => {
		const run = makeHarness();
		const page = makeInboxPage(
			[{ status: "Read", date_short: "6/1", preview: "x" }],
			/* gotoFails */ true,
		);
		await runInboxStream(
			run.deps,
		...
		assert.equal(
			run.emitted.filter((e) => e.stream === "inbox_messages").length,
			0,
			"no records on a nav failure",
		);
		const skips = skipsFor(run.messages, "inbox_messages");
		assert.equal(
			skips.length,
			1,
			"a caught navigation failure emits exactly one SKIP_RESULT",
		);
...
test("wiring: runInboxStream emits SKIP_RESULT and NO coverage when the scrape itself throws", async () => {
	await withFastTimers(async () => {
		...
		assert.equal(
			skips.length,
			1,
			"a caught scrape failure emits exactly one SKIP_RESULT",
		);
		assert.equal(
			coverageFor(run.messages, "inbox_messages"),
			undefined,
...
test("wiring: runCreditCardBillingStream emits DETAIL_COVERAGE for both streams after a successful scan", async () => {
	await withFastTimers(async () => {
		const cards = [
			makeCard({
				account_id_raw: "CC1",
				account_url: "/my/credit-card?accountId=CC1",
			}),
			makeCard({
				account_id_raw: "CC2",
				account_url: "/my/credit-card?accountId=CC2",
				last_four: "0002",
			}),
		];
		const { page, billingByUrl } = makeCreditCardPage();
		billingByUrl[`https://www.usaa.com${cards[0]?.account_url}`] = {
			"Current Balance": "$100.00",
		};
		billingByUrl[`https://www.usaa.com${cards[1]?.account_url}`] = {
			"Current Balance": "$200.00",
		};

		const run = makeHarness();
		const fingerprintCursor = openFingerprintCursor(undefined, {
		...
		const statsEmitted = run.emitted.filter(
			(e) => e.stream === "credit_card_billing_stats",
		);
		assert.equal(
		...
		const balances = statsEmitted
			.map(
				(e) =>
					(e.data as { current_balance_cents?: number }).current_balance_cents,
			)
			.sort((a, b) => (a ?? 0) - (b ?? 0));
		assert.deepEqual(
			balances,
		...
		assert.ok(
			entityCov,
			"credit_card_billing declares coverage after a successful scan",
		);
		assert.equal(entityCov?.considered, 2);
		assert.equal(entityCov?.covered, 2);
		assert.ok(
			statsCov,
			"credit_card_billing_stats declares coverage after a successful scan",
		);
...
test("wiring: runCreditCardBillingStream emits DETAIL_GAP_RECOVERED for a card the runtime served a pending gap for, once it scrapes successfully (live regression: gaps from a crashed run stayed pending forever)", async () => {
	await withFastTimers(async () => {
		...
		const billingRecoveries = recoveriesFor(
			run.messages,
			"credit_card_billing",
		);
		const statsRecoveries = recoveriesFor(
			run.messages,
			"credit_card_billing_stats",
		);
		assert.equal(
			billingRecoveries.length,
			1,
			"the successfully-scraped card recovers its credit_card_billing gap",
		);
...
test("wiring: runCreditCardBillingStream does NOT recover a gap for a card the runtime did not serve one for", async () => {
	await withFastTimers(async () => {
		...
		assert.equal(
			recoveriesFor(run.messages, "credit_card_billing").length,
			0,
			"no served gap, no recovery emitted",
		);
		assert.equal(
			recoveriesFor(run.messages, "credit_card_billing_stats").length,
			0,
		);
	});
});
...
test("wiring: runCreditCardBillingStream emits SKIP_RESULT and NO coverage when a scrape throws mid-loop", async () => {
	await withFastTimers(async () => {
		...
		assert.equal(
			skips.length,
			1,
			"a caught mid-loop scrape failure emits exactly one SKIP_RESULT",
		);
		assert.equal(coverageFor(run.messages, "credit_card_billing"), undefined);
		assert.equal(
			coverageFor(run.messages, "credit_card_billing_stats"),
			undefined,
		);
	});
});
...
test("wiring: a card whose navigation fails does NOT scrape/attribute the wrong page — gapped, excluded from covered, siblings still correct", async () => {
	await withFastTimers(async () => {
		...
		const { page, billingByUrl, gotoCalls } = makeCreditCardPage(
			new Set([cc1Url]),
		);
		...
		const entityEmitted = run.emitted.filter(
			(e) => e.stream === "credit_card_billing",
		);
		const statsEmitted = run.emitted.filter(
			(e) => e.stream === "credit_card_billing_stats",
		);
		assert.equal(
			...
			1,
			"only the successfully-navigated card (CC2) emits an entity record",
		);
		...
		assert.equal(
			(cc2Stats.data as { current_balance_cents?: number })
		...
		assert.equal(
			entityGaps.length,
			1,
			"the failed-navigation card gets a DETAIL_GAP, not silent loss",
		);
		assert.equal(entityGaps[0]?.reason, "temporary_unavailable");
		assert.equal(
			statsGaps.length,
			1,
			"the stats stream also gets a DETAIL_GAP for the same card",
		);
		...
		assert.equal(
			...
			2,
			"considered is still the full 2-card boundary",
		);
		assert.equal(
			entityCov?.covered,
			1,
			"covered excludes the failed-navigation card — never a false complete",
		);
		assert.equal(statsCov?.considered, 2);
...
test("wiring: a card's navigation lands on the USAA logon page (session died mid-run) → repair succeeds → retried nav scrapes the real card, not the logon page", async () => {
	await withFastTimers(async () => {
		...
		const cc1Url = `https://www.usaa.com${cc1.account_url}`;
		const { page, billingByUrl, gotoCalls } = makeLogonBouncingCreditCardPage(
			new Set([cc1Url]),
		);
		billingByUrl[cc1Url] = { "Current Balance": "$75.00" };
		...
		assert.equal(
			entityEmitted.length,
			1,
			"the card is scraped normally after the repaired retry",
		);
		const statsEmitted = run.emitted.filter(
			(e) => e.stream === "credit_card_billing_stats",
		);
		assert.equal(
			(statsEmitted[0]?.data as { current_balance_cents?: number } | undefined)
				?.current_balance_cents,
			7500,
			"the card's OWN balance from the post-repair retry, never the logon page's absent/zeroed fields",
		);
		assert.equal(
			gapsFor(run.messages, "credit_card_billing").length,
			0,
			"a repaired session must not gap the card",
		);
	});
});
...
test("wiring: a card's navigation lands on the logon page and repair fails → gapped exactly like a network-error nav failure, never scraped", async () => {
	await withFastTimers(async () => {
		...
		const cc1Url = `https://www.usaa.com${cc1.account_url}`;
		const { page, billingByUrl } = makeLogonBouncingCreditCardPage(
			new Set([cc1Url, cc1Url]),
		);
...
test("wiring: 8 cards, session dead from card 1 onward → exactly ONE reauth attempt across the whole run, not one per card", async () => {
	await withFastTimers(async () => {
		...
		assert.equal(
			...
			true,
			"the loop must latch session death for the caller",
		);
		assert.equal(
			run.emitted.filter((e) => e.stream === "credit_card_billing").length,
			0,
...
test("wiring: card 3 of 5 has a plain nav failure (not a logon bounce) → gapped and the loop CONTINUES to cards 4-5, no reauth attempted at all", async () => {
	await withFastTimers(async () => {
		...
		assert.equal(
			streamState.sessionDeadMidRun,
			false,
			"a plain nav failure is not a session-death signal",
		);
		assert.equal(
			run.emitted.filter((e) => e.stream === "credit_card_billing").length,
			4,
		...
		assert.equal(
			entityCov?.covered,
			4,
			"only the one plain-nav-failure card is excluded",
		);
	});
});
