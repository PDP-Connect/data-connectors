...
test("credit_card_billing: a no-op refresh (only fetched_at differs) is suppressed", () => {
	...
	assert.equal(
		wouldEmit(cursor1, a, billing, RUN1_AT),
		true,
	...
	assert.equal(
		wouldEmit(cursor2, a, billing, RUN2_AT),
		false,
...
test("credit_card_billing: connector fingerprint == compaction fingerprint (excludes fetched_at) over the post-split body", () => {
	...
	const laterBalanceMove = buildCreditCardBillingRecord(
		a,
		makeBilling({ "Current Balance": "$2,000.00" }),
		RUN2_AT,
	);
	const laterLimitMove = buildCreditCardBillingRecord(
		a,
		makeBilling({ "Credit Limit": "$7,500.00" }),
		RUN2_AT,
	);
...
test("readPriorCreditCardBillingFingerprints: tolerates missing / legacy / malformed state", () => {
	assert.equal(
		readPriorCreditCardBillingFingerprints({}).size,
		0,
		"empty state → empty map",
	);
	assert.equal(
		readPriorCreditCardBillingFingerprints({
			credit_card_billing: { fetched_at: "x" },
		}).size,
		0,
	...
	assert.equal(
		readPriorCreditCardBillingFingerprints({
			credit_card_billing: { fingerprints: 5 },
		}).size,
		0,
