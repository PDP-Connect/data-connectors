...
const NEVER_CALLED_SEND_INTERACTION: BrowserCollectContext["sendInteraction"] =
	async () => {
		throw new Error(
			"sendInteraction must not be called — gotoOrRepairSession delegates to deps.reauthenticate",
		);
	};
...
function makeSequencedPage(landedUrls: string[]): {
	calls: string[];
	page: Page;
} {
	const calls: string[] = [];
...
function makeFallbackLoginPage(): {
	...
	const context = {
		cookies: () =>
			Promise.resolve(
				loggedIn ? [{ name: "UsaaMbWebMemberLoggedIn", value: "true" }] : [],
			),
	} as BrowserContext;
	const page = Object.assign({} as Page, {
		...
		click: (selector: string) => {
			if (
				...
				filled.some((entry) => entry.selector === 'input[name="password"]')
			) {
				loggedIn = true;
			}
			return Promise.resolve();
...
test("gotoOrRepairSession: lands on the real page first try → ok, no reauth attempted", async () => {
	...
	assert.equal(
		calls.length,
		1,
		"no retry navigation when the first landing is not the logon page",
	);
});
...
test("gotoOrRepairSession: logon bounce persists after repair → terminal ok:false, exactly one retry", async () => {
	...
	assert.equal(
		reauthCalls,
		1,
		"repair is not retried in a loop after a second logon bounce",
	);
...
test("gotoOrRepairSession: default recovery uses the run-scoped Online ID, not the connector identity or ambient env", async () => {
	...
	try {
		...
		assert.equal(
			fixture.filled.find(
				({ selector }) => selector === 'input[name="password"]',
			)?.value,
			"saved-password",
		);
	} finally {
...
test("transactions recovery: reauthAfterSessionLapse uses the run-scoped Online ID, not ambient env", async () => {
	...
	try {
		...
		assert.equal(
			sessionDead,
			false,
			"stored-credential recovery succeeds before the ladder reports session death",
		);
		assert.equal(
			fixture.filled.find(
				({ selector }) => selector === 'input[name="memberId"]',
...
test("extractAccounts: dashboard bounces to logon, repair succeeds, retried dashboard load parses accounts", async () => {
	await withFastTimers(async () => {
		...
		assert.deepEqual(
			accounts,
			[],
			"post-repair retry proceeds to parse the (empty, in this fixture) dashboard",
		);
		assert.equal(
			streamState.sessionDeadMidRun,
			false,
			"a successful repair must not latch session death",
		);
	});
});
...
test("extractAccounts: repair fails → returns [] with a typed SKIP_RESULT, never scrapes the logon page as accounts, latches session death for later streams", async () => {
	await withFastTimers(async () => {
		...
		assert.deepEqual(
			accounts,
			[],
			"a dead session must never be scraped as zero real accounts",
		);
		assert.equal(
			evaluateCalled,
			false,
			"the logon page's DOM must never be scraped/attributed as account data",
		);
...
test("gotoOrRepairSession: budget already spent by an earlier call this run → refuses immediately after the bounce, no reauth, no retry navigation", async () => {
	...
	const { calls: secondCalls, page: secondPage } = makeSequencedPage([
		LOGON_URL,
	]);
	...
	assert.deepEqual(
		second,
		{ ok: false },
		"a second call this run must be refused, not attempt its own repair",
	);
...
function makeTransactionsAccount(
...
test("cross-path budget: gotoOrRepairSession spends the run's one repair attempt → reauthAfterSessionLapse (via runSingleLadderAttempt) cannot spend it again", async () => {
	...
	assert.equal(
		sessionDead,
		true,
		"the ladder still correctly reports session death to its caller",
	);
	assert.equal(
		reauthCalls,
		1,
...
test("cross-path budget: reauthAfterSessionLapse spends the run's one repair attempt first → a later gotoOrRepairSession call cannot spend it again", async () => {
	...
	const reauthenticate: EmitDeps["reauthenticate"] = () => {
		...
		return Promise.reject(
			new Error(
				"repair fails, proving the SPEND happened even though it didn't help",
			),
		);
	};
	const run = makeHarness(reauthenticate);
	...
	assert.equal(
		reauthCalls,
		1,
		"the transactions path's repair call spends the budget",
	);
	...
	const { calls: statementsCalls, page: statementsPage } = makeSequencedPage([
		LOGON_URL,
	]);
	...
	assert.deepEqual(
		statementsResult,
		{ ok: false },
		"the budget is already spent, so this call must be refused",
	);
	assert.equal(
		reauthCalls,
		1,
...
test("cross-path budget counterweight: a FRESH run's transactions path (reauthAfterSessionLapse) can still spend its one attempt when nothing has touched the budget yet", async () => {
	...
	const reauthenticate: EmitDeps["reauthenticate"] = () => {
		...
		return Promise.reject(
			new Error(
				"repair fails, so the outcome is deterministically session_dead",
			),
		);
	};
	const run = makeHarness(reauthenticate);
	...
	assert.equal(
		streamState.sessionRepairAttempted,
		true,
		"the ladder's own call spends the shared budget",
	);
});
...
test("runStatementsStream: documents nav bounces to logon, repair succeeds → session-alive result, scrape proceeds", async () => {
	await withFastTimers(async () => {
		...
		assert.equal(
			sessionAlive,
			true,
			"repair succeeded — caller must not latch sessionDeadMidRun",
		);
	});
});
...
test("runStatementsStream: documents nav bounces to logon, repair fails → session-dead result, typed SKIP_RESULT, no scrape", async () => {
	await withFastTimers(async () => {
		...
		assert.equal(
			sessionAlive,
			false,
			"caller must latch sessionDeadMidRun so later streams don't also attempt repair",
		);
		assert.equal(
			evaluateCalled,
			false,
			"the logon page must never be scraped as a statements index",
		);
		const skips = run.messages.filter(
			(m) => m.type === "SKIP_RESULT" && m.stream === "statements",
		);
		assert.equal(skips.length, 1);
		assert.equal(
			(skips[0] as { reason?: string }).reason,
...
test("runStatementsStream: a genuine scrape failure (not a logon bounce) still reports session-alive — only a logon bounce means the session died", async () => {
	await withFastTimers(async () => {
		...
		assert.equal(
			sessionAlive,
			true,
			"a non-auth scrape error must not be conflated with session death",
		);
		const skips = run.messages.filter(
			(m) => m.type === "SKIP_RESULT" && m.stream === "statements",
		);
...
test("runInboxStream: inbox nav bounces to logon, repair succeeds → session-alive, scrape proceeds normally", async () => {
	await withFastTimers(async () => {
		...
		Object.assign(page, {
			evaluate: () =>
				Promise.resolve([{ status: "Read", date_short: "6/1", preview: "hi" }]),
		});
...
test("runInboxStream: inbox nav bounces to logon, repair fails → session-dead result surfaces to the caller for cross-stream gating", async () => {
	await withFastTimers(async () => {
		...
		assert.equal(
			sessionAlive,
			false,
			"the caller (collect()) uses this to latch sessionDeadMidRun for later streams",
		);
		assert.equal(
			evaluateCalled,
			false,
			"the logon page must never be scraped as inbox rows",
		);
	});
});
