...
test("detectStatementClosing: credit-card 'Statement Closing Date MM/DD/YY'", () => {
	...
	assert.deepEqual(detectStatementClosing(text), {
		closingMonth: 2,
		closingYear: 2026,
	});
});
...
test("toIso: inherits closing year when month <= closing month", () => {
	assert.equal(
		toIso("02", "14", { closingMonth: 3, closingYear: 2026 }),
		"2026-02-14",
	);
});
...
test("toIso: rolls back to prior year when month > closing month", () => {
	assert.equal(
		toIso("12", "26", { closingMonth: 1, closingYear: 2026 }),
		"2025-12-26",
	);
});
...
test("rowsToTransactions: empty rows or header-only → []", () => {
	...
	assert.deepEqual(
		rowsToTransactions([["Date", "Amount"]], {
			accountId: "A",
			accountName: null,
			fetchedAt: "t",
		}),
...
test("parseModernCheckingEra: ignores lines outside section markers", () => {
	...
	const txns = parseModernCheckingEra(text, {
		closing: { closingMonth: 12, closingYear: 2026 },
	});
	assert.equal(txns.length, 1);
...
test("buildAccountRecord: happy path (entity carries identity/settings only, no balances)", () => {
	...
	assert.equal(
		"available_balance_cents" in rec,
		false,
		"entity drops available_balance_cents",
	);
});
...
test("buildAccountRecord: falls back to hashed raw_text when account_id_raw is null", () => {
	const rec = buildAccountRecord(
		makeAccount({ account_id_raw: null, raw_text: "abc" }),
		"t",
	);
...
test("resolveAccountIdForRef: falls back to name substring match (case-insensitive)", () => {
	...
	assert.equal(
		resolveAccountIdForRef("my total checking document", accounts),
		"A",
	);
});
...
test("resolveAccountIdForRef: empty / unmatched returns null", () => {
	...
	const accounts = [
		makeAccount({ account_id_raw: "A", name: "Foo", last_four: "0000" }),
	];
...
test("parseModernCheckingEra: local statement text parses ≥1 txn (smoke)", {
	...
	const closing = detectStatementClosing(text) ?? {
		closingMonth: 12,
		closingYear: 2026,
	};
