...
function entityWouldEmit(
	...
	const rec: CreditCardBillingRecord = buildCreditCardBillingRecord(
		a,
		billing,
		fetchedAt,
	);
...
test("credit_card_billing_stats: stats builder keys + carries volatile fields; entity keeps settings, drops them", () => {
	...
	assert.equal(
		entity.credit_limit_cents,
		500_000,
		"credit_limit_cents stays on the entity (a rare settings event)",
	);
...
test("credit_card_billing_stats: card_id equals the entity id for the same dashboard account", () => {
	for (const a of [
		...
		makeCardAccount({
			account_id_raw: null,
			last_four: null,
			raw_text: "USAA SECURED *9999 $50.00",
		}),
	]) {
		const entity = buildCreditCardBillingRecord(a, makeBilling(), RUN1_AT);
		const stat = buildCreditCardBillingStatsRecord(a, makeBilling(), DAY1);
		assert.equal(
			stat.card_id,
			entity.id,
			"stats.card_id joins to entity.id across the id fallback chain",
		);
...
test("credit_card_billing_stats: a balance / rewards / status change does NOT re-emit the entity", () => {
	for (const change of [
		...
		assert.equal(
			...
			true,
			"first observation emits the entity",
		);
...
test("credit_card_billing_stats: same-day re-pull is the same key; a later day is distinct", () => {
	const a = makeCardAccount({ account_id_raw: "CC1" });
	const sameDayA = buildCreditCardBillingStatsRecord(
		a,
		makeBilling({ "Current Balance": "$1,200.00" }),
		DAY1,
	);
	const sameDayB = buildCreditCardBillingStatsRecord(
		a,
		makeBilling({ "Current Balance": "$1,500.00" }),
		DAY1,
	);
	...
	assert.notEqual(
		sameDayA.id,
		laterDay.id,
		"later UTC day → distinct append key",
	);
});
...
test("credit_card_billing_stats: emitted observation records pass the real zod schema", async () => {
	...
	for (const a of cards) {
		await deps.emitRecord(
			...
			buildCreditCardBillingStatsRecord(a, makeBilling(), DAY1),
		);
	}
	const stats = harness.emitted.filter(
		(e) => e.stream === "credit_card_billing_stats",
	);
	assert.equal(
		stats.length,
		2,
		"both stats records passed the schema (harness routes through validateRecord)",
	);
	assert.equal(
		harness.skipped.length,
		0,
		"no record was rejected by the schema",
	);
});
