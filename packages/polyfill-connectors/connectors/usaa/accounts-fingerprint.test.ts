...
test("accounts: re-emitting with only a new fetched_at is fully suppressed", async () => {
	...
	assert.equal(
		run2.emitted.length,
		0,
		"unchanged accounts are fully suppressed on the second run",
	);
});
...
test("accounts: a balance-only move does NOT re-emit the entity (balance lives on account_stats)", async () => {
	...
	assert.equal(
		run2.emitted.length,
		0,
		"a balance-only move no longer versions the entity record",
	);
});
...
test("accounts: external account dashboard freshness text does NOT re-emit the entity", async () => {
	...
	assert.equal(
		run1.emitted.length,
		1,
		"first run emits the external account entity",
	);
	...
	assert.equal(
		run2.emitted.length,
		0,
		"dashboard freshness text is normalized before entity fingerprinting",
	);
});
...
test("accounts: a real identity/status move re-emits the entity", async () => {
	...
	assert.equal(
		run2.emitted.length,
		1,
		"an identity move is a fingerprint boundary and re-emits",
	);
	const emittedRec = run2.emitted[0]?.data as { name?: string };
	assert.equal(
		emittedRec.name,
		"USAA PERFORMANCE CHECKING",
		"the re-emitted record carries the new name",
	);
});
...
test("accounts: a disappeared account is pruned so its re-appearance re-emits", async () => {
	const both = [
		makeAccount({ account_id_raw: "A1" }),
		makeAccount({ account_id_raw: "A2", balance_cents: 1 }),
	];
	...
	assert.equal(
		run2.emitted.length,
		0,
		"A1 unchanged → suppressed; A2 absent → not emitted",
	);
	...
	assert.equal(
		fps2.has("A2"),
		false,
		"disappeared account pruned from fingerprint map",
	);
	...
	assert.equal(
		run3.emitted.length,
		1,
		"re-appeared account re-emits exactly once; A1 still suppressed",
	);
	const [row] = run3.emitted;
	assert.ok(row, "row was captured");
	assert.equal(
		(row.data as { id?: string }).id,
		"A2",
		"the re-emit is the re-appeared account",
	);
});
...
test("accounts: legacy callers without a cursor still emit unconditionally", async () => {
	...
	const state = run.messages
		.filter((m) => m.type === "STATE" && m.stream === "accounts")
		.at(-1) as { cursor?: Record<string, unknown> } | undefined;
	assert.ok(
		state?.cursor && "fetched_at" in state.cursor,
		"heartbeat STATE still written",
	);
	assert.equal(
		"fingerprints" in (state?.cursor ?? {}),
		false,
		"no fingerprints map without a cursor",
	);
});

test("readPriorAccountFingerprints: tolerates missing / legacy / malformed state", () => {
	assert.equal(
		readPriorAccountFingerprints({}).size,
		0,
	...
	const ok = readPriorAccountFingerprints({
		accounts: { fingerprints: { A1: "fp-1", bad: null } },
	});
