...
test("transactions: re-downloading the same transactions (only fetched_at differs) is fully suppressed", () => {
	...
	assert.equal(
		run2.length,
		0,
		"re-downloaded unchanged transactions fully suppressed despite new fetched_at",
	);
});
...
test("transactions: a genuinely-new transaction still emits", () => {
	...
	const withNew: readonly [string, string, string][] = [
		...SAMPLE,
		["2026-04-12", "GAS STATION", "-$32.10"],
	];
...
test("transactions: a corrected amount (new tuple → new id) emits", () => {
	...
	emitThrough(
		cursor1,
		csvTxns(RUN1_AT, [["2026-04-10", "COFFEE SHOP", "-$45.99"]]),
	);
	...
	const run2 = emitThrough(
		cursor2,
		csvTxns(RUN2_AT, [["2026-04-10", "COFFEE SHOP", "-$50.00"]]),
	);
	assert.equal(
		run2.length,
		1,
		"a corrected amount produces a new id and emits",
	);
});
...
test("transactions: a balance_after move on a stable id re-emits (real field, not run-clock)", () => {
	...
	const run1Rows = [
		header,
		["2026-04-10", "COFFEE SHOP", "-$45.99", "$1,000.00"],
	];
	const r1 = rowsToTransactions(run1Rows, {
		accountId: "A",
		accountName: "x",
		fetchedAt: RUN1_AT,
	});
	...
	const run2Rows = [
		header,
		["2026-04-10", "COFFEE SHOP", "-$45.99", "$1,050.00"],
	];
	const r2 = rowsToTransactions(run2Rows, {
		accountId: "A",
		accountName: "x",
		fetchedAt: RUN2_AT,
	});
	...
	assert.equal(
		emitted.length,
		1,
		"a balance_after_cents move is a fingerprint boundary and re-emits",
	);
});
...
test("transactions: NO prune — a transaction omitted from a narrower window keeps its fingerprint", () => {
	...
	const run2 = emitThrough(
		cursor2,
		csvTxns(RUN2_AT, [SAMPLE[1] as [string, string, string]]),
	);
	assert.equal(
		run2.length,
		0,
		"the second row unchanged stays silent; the first row not looked at this run",
	);
	...
	assert.equal(
		run3.length,
		0,
		"re-downloaded first row stays suppressed because it was never pruned",
	);
});
...
test("transactions: a transaction seen last run (from either path) is suppressed this run", () => {
	...
	assert.equal(
		fromCsv.length,
		1,
		"CSV path emits the transaction once on the first run",
	);
	...
	const priorState = { transactions: { fingerprints: cursor1.toState() } };
	const cursor2 = openTxnCursor(priorState);
	const fromPdf = emitThrough(
		cursor2,
		csvTxns(RUN2_AT, [["2026-04-10", "COFFEE SHOP", "-$45.99"]]),
	);
	assert.equal(
		fromPdf.length,
		0,
		"the same logical transaction is suppressed across runs on the shared cursor",
	);
});

test("readPriorTransactionFingerprints: tolerates missing / legacy / malformed state", () => {
	assert.equal(
		readPriorTransactionFingerprints({}).size,
		0,
		"empty state → empty map",
	);
	assert.equal(
		readPriorTransactionFingerprints({
			transactions: { "ACCT-1": { last_date: "2026-04-10" } },
		}).size,
		0,
		"legacy cursor (per-account watermarks only, no fingerprints) → empty map",
	);
	assert.equal(
		readPriorTransactionFingerprints({ transactions: { fingerprints: 5 } })
			.size,
		0,
		"malformed fingerprints value → empty map",
	);
	const ok = readPriorTransactionFingerprints({
		transactions: { fingerprints: { id1: "fp-1", bad: null } },
	});
...
test("transactions: STATE round-trips the fingerprints map alongside per-account watermarks", () => {
	...
	assert.equal(
		fps.size,
		2,
		"both transaction fingerprints persisted alongside the watermark",
	);
	// The watermark entry is untouched and still readable as a per-account cursor.
	const watermark = (
		persisted.transactions as Record<string, { last_date?: string }>
	)["ACCT-CHK-0001"];
	assert.equal(
		watermark?.last_date,
		"2026-04-11",
		"per-account watermark survives next to the fingerprints map",
	);
});
