...
test("account_stats: a stats-requested run declares considered === covered === enumerated accounts", async () => {
	...
	assert.equal(
		...
		2,
		"both accounts' stats emitted",
	);
	const cov = selfCoverage(run.messages, "account_stats");
	assert.ok(
		cov,
		"account_stats declares self-coverage — this is the live-bug fix: before it, this was undefined",
	);
	assert.equal(
		cov?.considered,
		2,
		"considered === enumerated account boundary",
	);
	assert.equal(
		cov?.covered,
		2,
		"covered === considered (buildAccountStatsRecord never drops a row)",
	);
});

test("account_stats: zero accounts still proves verified-empty (considered === covered === 0)", async () => {
	const run = makeHarness();
	await emitAccountsStream(run.deps, [], RUN1_AT, undefined, {
		emitStats: true,
		observedOn: DAY1,
	});
	...
	assert.ok(
		cov,
		"a successful zero-account enumeration still declares coverage",
	);
	assert.equal(
		cov?.considered,
		0,
		"considered: 0 is a measured fact, not an absence of measurement",
	);
...
test("creditCardAccounts: no credit cards on the account is a real, measurable zero boundary", () => {
	const accounts = [
		makeAccount({ account_type: "checking" }),
		makeAccount({ account_type: "savings" }),
	];
	assert.equal(
...
test("computeInboxCoverage: every resolved row counts toward covered, fully resolved inbox", () => {
	...
	assert.equal(
		result.considered,
		3,
		"considered === every row the page listed",
	);
	assert.equal(
		result.covered,
		3,
		"covered === considered when every row resolved",
	);
});
...
test("computeInboxCoverage: an unparseable-date row lowers covered below considered (honest partial)", () => {
	...
	assert.equal(
		result.considered,
		3,
		"considered still counts every row the enumeration saw",
	);
	assert.equal(
		result.covered,
		2,
		"the dropped row does not inflate covered — reads partial, not a false complete",
	);
});
