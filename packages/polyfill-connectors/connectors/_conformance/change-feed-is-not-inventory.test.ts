...
function detailCoverageFor(
	messages: readonly EmittedMessage[],
	stream: string,
): DetailCoverageLike[] {
	return messages.filter(
		(message): message is EmittedMessage & DetailCoverageLike =>
			(message as DetailCoverageLike).type === "DETAIL_COVERAGE" &&
			(message as DetailCoverageLike).stream === stream,
	);
}
 ...
 */
async function runAppleContacts(args: {
	readonly priorSyncToken?: string;
}): Promise<readonly EmittedMessage[]> {
...
test("an initial run (empty sync token) DOES prove the full inventory", async () => {
	...
	assert.equal(
		recordCountFor(messages, "contacts"),
		3,
		"the initial run must emit every contact",
	);
	...
	assert.equal(
		coverage.length,
		1,
		"the initial run must emit exactly one contacts coverage claim",
	);
	assert.equal(
		coverage[0]?.considered,
		3,
		"considered must be the measured inventory size",
	);
...
test("a quiet INCREMENTAL run must not report an empty change feed as an empty inventory", async () => {
	...
	assert.equal(
		recordCountFor(messages, "contacts"),
		0,
		"a quiet delta emits no contact records",
	);
...
test("the incremental run still commits its cursor, so withholding coverage does not stall sync", async () => {
	...
	assert.ok(
		...
		"the incremental run must still checkpoint its sync token",
	);
	...
	assert.equal(
		...
		"succeeded",
		"a quiet incremental run is a success, not a failure",
	);
});
