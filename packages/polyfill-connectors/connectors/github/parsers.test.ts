...
function readScrubbedUserFixture(): GitHubUser {
	const parsed = JSON.parse(
		readFileSync(join(SCRUBBED_FIXTURE_DIR, "user.json"), "utf8"),
	) as unknown;
...
const GIST_FIXTURE: GitHubGist = {
	...
	files: {
		"a.md": {
			...
			language: "Markdown",
			size: 42,
			raw_url: "https://example.com/a.md",
		},
		"b.ts": {
			filename: "b.ts",
			language: "TypeScript",
			size: 128,
			raw_url: "https://example.com/b.ts",
		},
	},
	comments: 0,
...
test("assigneeNames: keeps only entries with login", () => {
	assert.deepEqual(assigneeNames([{ login: "a" }, {}, { login: "b" }]), [
		"a",
		"b",
	]);
});
...
test("repoFullFromUrl: strips /repos/ prefix", () => {
	assert.equal(
		repoFullFromUrl(`${API_BASE}/repos/octocat/hello`),
		"octocat/hello",
	);
});
