...
import {
	...
	getDb,
	initDb,
} from "../../../../reference-implementation/server/db.ts";
import {
	drainConnectorInstanceIndexWork,
	ingestRecord,
} from "../../../../reference-implementation/server/records.ts";
import {
...
test("collectUser: records the user cursor on ctx.userCursor as the warm-start carrier", async (t: TestContext) => {
	...
	assert.ok(
		ctx.userCursor,
		"the user cursor is recorded for warm-start persistence",
	);
	assert.equal(
		ctx.userCursor,
...
test("warm-start: pacing fields merged onto the user cursor round-trip through readPersistedPacingInterval", (t: TestContext) => {
	...
	const restored = readPersistedPacingInterval(persistedUserCursor, {
		now: () => now + 1000,
	});
	assert.equal(
		restored,
		480,
		"the next run warm-starts from the interval persisted on the user cursor",
	);
});
...
function starredEntry(id: number, withRepo: boolean): Record<string, unknown> {
	return {
		...
		repo: withRepo
			? { id, full_name: `owner/repo-${String(id)}`, stargazers_count: 1 }
			: undefined,
	};
}
...
test("collectStarred: entries with no repo are counted and surfaced as one bounded SKIP_RESULT", async (t: TestContext) => {
	...
	mockFetch(t, () =>
		Promise.resolve(
			jsonResponse([
				starredEntry(1, true),
				starredEntry(2, false),
				starredEntry(3, false),
			]),
		),
	);
...
test("collectStarred: no drops emits no SKIP_RESULT (run looks complete only when it is)", async (t: TestContext) => {
	mockFetch(t, () =>
		Promise.resolve(
			jsonResponse([starredEntry(1, true), starredEntry(2, true)]),
		),
	);
...
function installPrFetch(
	...
	mockFetch(t, (input: string | URL | Request) => {
		...
		if (url.includes("/search/issues")) {
			return Promise.resolve(
				jsonResponse({ total_count: items.length, items }),
			);
		}
		...
		if (detailMatch) {
			...
			return Promise.resolve(
				jsonResponse({ merged_at: "2026-05-10T00:00:00Z", commits: 3 }),
			);
		}
...
test("collectPullRequests: detail-fetch failures emit one bounded degradation SKIP_RESULT, records still emitted", async (t: TestContext) => {
	const items = [
		prSearchItem(1, "owner/a"),
		prSearchItem(2, "owner/b"),
		prSearchItem(3, "owner/c"),
	];
	...
	assert.match(
		skips[0]?.message ?? "",
		/2 of 3 pull request record\(s\) emitted without detail/,
	);
...
test("collectPullRequests: degradation denominator counts emitted records, not filtered search hits", async (t: TestContext) => {
	...
	assert.match(
		skips[0]?.message ?? "",
		/1 of 2 pull request record\(s\) emitted without detail/,
	);
	assert.deepEqual(skips[0]?.diagnostics, {
		detail_failed: 1,
		total_emitted: 2,
...
test("prCreatedWindows: single year when floor equals current", () => {
	assert.deepEqual(prCreatedWindows(2026, 2026), [
		{ from: "2026-01-01", to: "2026-12-31" },
	]);
});
...
function installWindowedPrFetch(
	...
	mockFetch(t, (input: string | URL | Request) => {
		...
		if (url.endsWith("/user")) {
			return Promise.resolve(
				jsonResponse({ id: 42, login: "octocat", created_at: createdAt }),
			);
		}
		if (url.includes("/search/issues")) {
		...
		if (/\/repos\/[^/]+\/[^/]+\/pulls\/\d+$/.test(url)) {
			return Promise.resolve(
				jsonResponse({ merged_at: "2025-01-01T00:00:00Z", commits: 1 }),
			);
		}
		return Promise.resolve(jsonResponse({}));
	});
...
test("collectPullRequests: full resync partitions by created-year and unions every window", async (t: TestContext) => {
	...
	const prIds = records
		.filter((r) => r.stream === "pull_requests")
	...
	assert.match(
		decodeURIComponent(fetchHandle.searchPaths[0] ?? ""),
		/created:2026-01-01\.\.2026-12-31/,
	);
	assert.match(
		decodeURIComponent(fetchHandle.searchPaths[2] ?? ""),
		/created:2024-01-01\.\.2024-12-31/,
	);
	// No cap tripped → no cap SKIP_RESULT; cursor advances to the newest update.
	assert.equal(skips.length, 0);
	assert.equal(
		(states[0]?.cursor as { last_updated_at?: string })?.last_updated_at,
...
test("collectPullRequests: a window over the search cap fails without a checkpoint", async (t: TestContext) => {
	...
	const fetchHandle = installWindowedPrFetch(
		...
		{
			2026: [
				prSearchItemCreated(1, "owner/a", 2026),
				prSearchItemCreated(2, "owner/b", 2026),
			],
		},
		{ 2026: 1023 },
	);
	...
	assert.deepEqual(capSkip?.recovery_hint, {
	...
	assert.equal(
		states.length,
		0,
		"a search cap gap must not advance the pull-request cursor",
	);
});
...
test("collectPullRequests: windows under the cap emit no cap gap (honest only when truncated)", async (t: TestContext) => {
	...
	assert.equal(
		skips.filter((s) => s.reason === "pr_search_cap_truncated").length,
		0,
	);
});
...
test("collectPullRequests: incremental run (cursor set) issues one unwindowed updated:>= query", async (t: TestContext) => {
	...
	const items = [
		...
		{
			...prSearchItemCreated(2, "owner/b", 2018),
	...
	mockFetch(t, () =>
		Promise.resolve(
			jsonResponse([
				...
				repoItem(2, "2026-05-01T00:00:00Z"),
			]),
		),
	);
...
test("collectIssues: until-filtered issues are considered-not-collected (considered > collected)", async (t: TestContext) => {
	...
	ctx.requested.set("issues", {
		name: "issues",
		time_range: { until: "2026-06-05T00:00:00Z" },
	});
...
test("collectGists: declares considered = gists enumerated", async (t: TestContext) => {
	mockFetch(t, () =>
		Promise.resolve(
			jsonResponse([
				gistItem(1, "2026-06-01T00:00:00Z"),
				gistItem(2, "2026-05-20T00:00:00Z"),
			]),
		),
	);
...
test("declareListConsidered: never aliases considered to the emitted count", async (t: TestContext) => {
	...
	assert.ok(
		cov,
		"an empty enumeration still declares considered: 0 (a fact, not unknown)",
	);
...
test("collectRepositories: zero-changed steady state declares covered === considered (proves complete, not partial)", async (t: TestContext) => {
	...
	mockFetch(t, () =>
		Promise.resolve(jsonResponse([repoItem(1, "2026-01-01T00:00:00Z")])),
	);
	const { ctx, records, coverages } = makeCtx(["repositories"], {
		repositories: { last_pushed_at: "2026-03-01T00:00:00Z" },
	});
...
test("collectRepositories: FIX proof — a page tail past the cursor stop is never counted toward considered", async (t: TestContext) => {
	...
	assert.equal(
		cov?.covered,
		2,
		"both evaluated items were accounted for: one emitted, one the confirming stop match",
	);
});
...
test("collectStarred: covered excludes dropped entries so a drop reads an honest partial under the real oracle", async (t: TestContext) => {
	...
	assert.equal(
		cov?.covered,
		2,
		"the dropped entry (no repo object) was evaluated but never accounted for",
	);
...
test("collectIssues: considered > collected (until-filtered) but fully covered — proves complete, not partial", async (t: TestContext) => {
	...
	assert.equal(
		cov?.considered,
		3,
		"considered > collected: the until filter excluded one in-window issue",
	);
	assert.equal(
		cov?.covered,
		3,
...
function installRepeatingNextFetch(
	t: TestContext,
	stream: string,
): { calls: () => number } {
	let callCount = 0;
...
for (const { collect, name, stream } of githubListCollectors) {
	test(`GitHub ${name}: repeated next link fails with retryable gap and no checkpoint`, async (t: TestContext) => {
		...
		assert.equal(
			handle.calls(),
			stream === "pull_requests" ? 2 : 1,
			"the repeated link must not be fetched twice",
		);
		assert.equal(
			states.length,
			0,
			"a repeated next link must not advance state",
		);
		assert.equal(
			coverages.length,
			0,
			"a repeated next link must not declare complete coverage",
		);
		assert.equal(skips.length, 1);
		assert.equal(skips[0]?.reason, "github_pagination_repeated_next");
		assert.deepEqual(skips[0]?.recovery_hint, {
			action: "retry_by_runtime",
			retryable: true,
		});
	});
...
for (const {
	...
	name,
	stream: entryStream,
} of githubListCollectors.filter(({ stream }) => stream !== "pull_requests")) {
	test(`GitHub ${name}: object list envelope fails closed without coverage or state`, async (t: TestContext) => {
		...
		assert.equal(
			records.length,
			0,
			"a malformed first page is a pre-first-item zero-record failure",
		);
		assert.equal(
			coverages.length,
			0,
			`${entryStream} malformed envelope must not emit coverage`,
		);
		assert.equal(
			states.length,
			0,
			`${entryStream} malformed envelope must not advance state`,
		);
	});
	...
	test(`GitHub ${name}: valid prefix survives a later malformed page without coverage or state`, async (t: TestContext) => {
		...
		assert.equal(
			coverages.length,
			0,
			"later malformed page withholds coverage",
		);
...
test("GitHub repositories: fresh next links stop at the bounded page cap with a retryable gap", async (t: TestContext) => {
	...
	mockFetch(t, (input: string | URL | Request) => {
		...
		next.searchParams.set(
			"page",
			String(Number(current.searchParams.get("page") ?? "1") + 1),
		);
		return Promise.resolve(
			jsonResponse([], { headers: { link: `<${next.href}>; rel="next"` } }),
		);
	});
	...
	assert.equal(
		coverages.length,
		0,
		"a page cap gap must not declare complete coverage",
	);
...
test("GitHub retryability: exhausted transient 503 text remains retryable", () => {
	assert.match(
		"HTTP request got retryable status 503 after retry budget was exhausted",
		GITHUB_RETRYABLE_PATTERN,
	);
	assert.match(
		"HTTP request got retryable status 502 after retry budget was exhausted",
		GITHUB_RETRYABLE_PATTERN,
	);
	assert.match(
		"HTTP request got retryable status 504 after retry budget was exhausted",
		GITHUB_RETRYABLE_PATTERN,
	);
	assert.match(
...
test("collectPullRequests: malformed search 200 fails closed without 0/0 coverage or state", async (t: TestContext) => {
	...
	assert.equal(
		coverages.length,
		0,
		"malformed search must not emit a fabricated 0/0 denominator",
	);
	assert.equal(
		states.length,
		0,
		"malformed search must not advance the PR cursor",
	);
});
...
test("collectPullRequests: positive total_count with empty items fails closed", async (t: TestContext) => {
	...
	await assert.rejects(
		...
		(error: unknown) =>
			error instanceof Error &&
			(error as Error & { code?: string }).code === "github_malformed_response",
	);
	assert.equal(
		records.length,
		0,
		"an impossible empty page must not emit a degraded PR record",
	);
	assert.equal(
		coverages.length,
		0,
		"an impossible empty page must not emit 0/0 coverage",
	);
	assert.equal(
		states.length,
		0,
		"an impossible empty page must not advance the PR cursor",
	);
});
...
test("collectPullRequests: shared Retry-After retry recovers a transient search 429", async (t: TestContext) => {
	...
	assert.equal(
		searchCalls,
		2,
		"Retry-After must trigger one bounded real retry",
	);
	assert.equal(
		sleeps.filter((ms) => ms === 2000).length,
		1,
		"GitHub request seam must honor Retry-After exactly once",
	);
	assert.equal(
		states.length,
		1,
		"state is emitted only after the retried search completes",
	);
});
...
for (const failure of ["429", "transport"] as const) {
	test(`collectPullRequests: ${failure} on a later detail retains only the valid prefix and withholds completion`, async (t: TestContext) => {
				...
				return failure === "429"
					...
					: Promise.reject(
							new Error("fetch failed", {
								cause: new Error("socket reset ECONNRESET"),
							}),
						);
			}
			return Promise.resolve(jsonResponse({}));
		...
		assert.ok(
			detailCalls > 1,
			"the bounded request retry policy may retry the fatal detail",
		);
		assert.deepEqual(
		...
		assert.equal(
			coverages.length,
			0,
			"fatal later detail failure withholds coverage",
		);
		assert.equal(
			states.length,
			0,
			"fatal later detail failure withholds STATE",
		);
		...
		assert.deepEqual(
			readDurablePullRequestRows(connectorInstanceId).map(
				(row) => row.record_key,
			),
			["1"],
		...
		assert.equal(
			new Set(replay.records.map((record) => record.data.id)).size,
			2,
		);
		...
		assert.deepEqual(
			...
			[
				{
					cursor: { last_updated_at: "2026-05-02T00:00:00Z" },
					stream: REPLAY_STREAM,
				},
			],
		...
		assert.ok(
			durableRows.every(
				(row) => row.connector_instance_id === connectorInstanceId,
			),
		);
...
test("collectPullRequests: exhausted transient detail 503 bubbles instead of degrading to a checkpoint", async (t: TestContext) => {
	mockFetch(t, (input: string | URL | Request) => {
		...
		if (url.includes("/search/issues")) {
			return Promise.resolve(
				jsonResponse({ total_count: 1, items: [prSearchItem(1, "owner/a")] }),
			);
		}
		if (/\/repos\/[^/]+\/[^/]+\/pulls\/\d+$/.test(url)) {
	...
	await assert.rejects(
		() => collectPullRequests(ctx),
		(error: unknown) =>
			error instanceof Error && GITHUB_RETRYABLE_PATTERN.test(error.message),
	);
	assert.equal(
		states.length,
		0,
		"an exhausted transient detail failure must not advance state",
	);
});
...
for (const status of [502, 504]) {
	test(`collectPullRequests: exhausted transient detail ${status} bubbles as incomplete`, async (t: TestContext) => {
		...
		assert.equal(
			states.length,
			0,
			`${status} detail failure must not advance state`,
		);
	});
}
...
test("collectPullRequests: PR-detail 429 is terminal without progress, coverage, records, or state", async (t: TestContext) => {
	...
	const { ctx, coverages, records, states, progresses } = makeCtx([
		"pull_requests",
	]);
	...
	await assert.rejects(
		() => collectPullRequests(ctx),
		(error: unknown) =>
			error instanceof Error && error.message === "github_rate_limited",
	);
...
test("collectPullRequests: PR-detail transport failure is terminal without progress, coverage, records, or state", async (t: TestContext) => {
	...
	await assert.rejects(
		() => collectPullRequests(ctx),
		(error: unknown) =>
			error instanceof Error && /fetch failed|ECONNRESET/i.test(error.message),
	);
	assert.equal(records.length, 0);
	assert.equal(coverages.length, 0);
	assert.equal(states.length, 0);
