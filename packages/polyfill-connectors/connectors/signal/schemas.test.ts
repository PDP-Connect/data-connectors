...
test("messages schema rejects a non-UUID id", () => {
	assert.equal(
		messagesSchema.safeParse({ ...MESSAGE_RECORD, id: "not-a-uuid" }).success,
		false,
	);
});
...
test("messages schema rejects a non-ISO sent_at", () => {
	assert.equal(
		messagesSchema.safeParse({ ...MESSAGE_RECORD, sent_at: "1717594922000" })
			.success,
		false,
	);
});
...
test("messages schema rejects a missing sent_at (null must never be emitted for this stream)", () => {
	assert.equal(
		messagesSchema.safeParse({ ...MESSAGE_RECORD, sent_at: null }).success,
		false,
	);
});
...
test("conversations schema rejects an unrecognized type literal", () => {
	assert.equal(
		conversationsSchema.safeParse({ ...CONVERSATION_RECORD, type: "channel" })
			.success,
		false,
	);
});
...
test("conversations schema rejects a non-UUID id", () => {
	assert.equal(
		conversationsSchema.safeParse({ ...CONVERSATION_RECORD, id: "not-a-uuid" })
			.success,
		false,
	);
});
...
test("reactions schema accepts a non-UUID sender (legacy e164/conversation-id-shaped value)", () => {
	const result = reactionsSchema.safeParse({
		...REACTION_RECORD,
		sender: "+15551234567",
	});
	assert.ok(result.success, JSON.stringify(result.error?.issues));
...
test("reactions schema rejects an empty emoji", () => {
	assert.equal(
		reactionsSchema.safeParse({ ...REACTION_RECORD, emoji: "" }).success,
		false,
	);
});
...
test("reactions schema rejects a non-UUID message_id", () => {
	assert.equal(
		reactionsSchema.safeParse({ ...REACTION_RECORD, message_id: "not-a-uuid" })
			.success,
		false,
	);
});
...
test("attachments schema accepts missing/too_large/failed statuses with null size/hash", () => {
	for (const status of ["missing", "too_large", "failed"] as const) {
		...
		assert.ok(
			result.success,
			`${status}: ${JSON.stringify(result.error?.issues)}`,
		);
	}
});
...
test("attachments schema rejects a non-hex id (would leak a raw local path)", () => {
	assert.equal(
		attachmentsSchema.safeParse({
			...ATTACHMENT_HYDRATED,
			id: "/home/tim/.config/Signal/x.jpg",
		}).success,
		false,
	);
});
