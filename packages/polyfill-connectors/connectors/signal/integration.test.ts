...
test("signal reports a failed DONE when check-database fails", async () => {
	...
	try {
		const scriptPath = setupMockSigtop(
			...
			{ messages: [] },
			{
				checkDatabaseExitCode: 1,
				checkDatabaseStdout: "integrity check failed: foo",
			},
		);
		const result = await runConnectorProtocolSubprocess({
			allowFailedDone: true,
...
test("signal derives has_attachments/is_edited from the message json blob", async () => {
	...
	try {
		...
		const result = await runSignal(scriptPath, ["messages"]);
		const msgs = records(result.messages, "messages");
		const withAttachments = msgs.find(
			(m) => m.id === "22222222-2222-2222-2222-222222222222",
		);
		const plain = msgs.find(
			(m) => m.id === "55555555-5555-5555-5555-555555555555",
		);
...
test("signal emits conversations as a full resnapshot with null member_count", async () => {
	...
	try {
		...
		const state = states(result.messages).find(
			(s) => s.stream === "conversations",
		);
...
test("signal derives reactions when only the reactions stream is requested (messages not in scope)", async () => {
	...
	try {
			...
			messages: [
				{
					...
					id: "22222222-2222-2222-2222-222222222222",
					json: JSON.stringify({
						reactions: [{ emoji: "👍", fromId: SENDER, targetTimestamp: t0 }],
					}),
					sentAt: t0,
				},
			],
		...
		assert.equal(
			skips(result.messages).filter((s) => s.stream === "messages").length,
			0,
		);
	...
	try {
		...
		const scriptPath = setupMockSigtop(
			dir,
			{
				...
				messages: [
					{ body: null, conversationId: CONV_A, id: messageId, sentAt: t0 },
				],
			},
			{
				attachments: [
					{ bytes, conversationDir: "Alice", filename: "IMG_0001.jpg" },
				],
			},
		);
...
test("signal: a bounded pass across every default stream reaches an accepted terminal DONE (bounded complete-pass proof)", async () => {
	...
	try {
		...
		for (const stream of SIGNAL_DEFAULT_STREAMS) {
			...
			const streamSkip = skips(result.messages).find(
				(s) => s.stream === stream,
			);
			const streamState = states(result.messages).find(
				(s) => s.stream === stream,
			);
			assert.ok(
				streamRecords.length > 0 || streamSkip || streamState,
				`stream "${stream}" produced no record, SKIP_RESULT, or STATE ` +
					"checkpoint — silently unaccounted for",
			);
		}
...
test("buildSignalExportFixture produces a database queryable by the connector's own column names", async () => {
	...
	try {
		...
		try {
			const rows = db
				.prepare(
					"SELECT id, conversationId, sourceServiceId, sent_at, received_at_ms, body, type, json FROM messages",
				)
				.all();
			assert.equal(rows.length, 1);
			const convRows = db
				.prepare(
					"SELECT id, type, name, e164, serviceId, groupId FROM conversations",
				)
				.all();
