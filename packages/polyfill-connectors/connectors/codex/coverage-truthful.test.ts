...
test("codex coverage truth: populated rollouts show collected status", async () => {
	...
	const messagesRecord = coverageRecs.find(
	...
	assert.equal(
		...
		"collected",
		"messages with emitted records must show collected",
	);
});
...
test("codex: a bounded pass across every default stream reaches an accepted terminal DONE (bounded complete-pass proof)", async () => {
	...
	assert.equal(
		messagesCoverage?.data.status,
		"collected",
		"bounded complete pass must show messages collected",
	);
});
...
test("codex coverage truth: empty rollouts (ENOENT) show collected (complete scan, zero examined)", async () => {
	...
	const emptyHome = join(
		import.meta.dirname,
		"../../fixtures/codex/empty-home-no-rollouts",
	);
	...
	const functionCallsRecord = coverageRecs.find(
		(r) =>
			r.data.store === "derived_function_calls" &&
			r.data.stream === "function_calls",
	);
	...
	assert.equal(
		functionCallsRecord.data.reason,
...
test("codex coverage truth: derived coverage records included in STATE snapshot", async () => {
	...
	const stateSnapshot = (
		coverageStates.at(-1)?.cursor as { stores?: Record<string, unknown> }
	)?.stores;
	...
	const storeNames =
		typeof stateSnapshot === "object" && stateSnapshot !== null
			...
			: [];
	assert(
		storeNames.includes("derived_messages"),
		"STATE snapshot must include derived_messages store",
	);
	assert(
		storeNames.includes("derived_function_calls"),
		"STATE snapshot must include derived_function_calls store",
	);
});
...
test("codex coverage truth: file read error makes coverage unaccounted", async () => {
	...
	const { mkdirSync, writeFileSync, chmodSync, rmSync } = await import(
		"node:fs"
	);
	const { tmpdir } = await import("node:os");
	const tempRoot = join(
		tmpdir(),
		`codex-read-error-${Date.now()}-${Math.random().toString(36).slice(2)}`,
	);
	...
	try {
		...
		const recs = records(result.messages);
		const coverageRecs = recs.filter(
			(r) => r.stream === "coverage_diagnostics",
		);

		const messagesRecord = coverageRecs.find(
			(r) => r.data.store === "derived_messages",
		);
		const functionCallsRecord = coverageRecs.find(
			(r) => r.data.store === "derived_function_calls",
		);
		...
		assert.equal(
			messagesRecord.data.status,
			"unaccounted",
			"file enumeration error must show unaccounted",
		);
		assert.equal(
			functionCallsRecord.data.status,
			"unaccounted",
...
test("codex coverage truth: suppressed-but-populated shows collected (fingerprinted)", async () => {
	...
	const coverageRecs1 = recs1.filter(
		(r) => r.stream === "coverage_diagnostics",
	);
	const messagesRecord1 = coverageRecs1.find(
		(r) => r.data.store === "derived_messages",
	);

	// First run: collected (records were emitted)
	assert.equal(
		messagesRecord1?.data.status,
		"collected",
		"first run should collect records",
	);

	// Extract STATE cursor for second run (to simulate fingerprint suppression)
	const states1 = result1.messages.filter(
		(m): m is Extract<any, { type: "STATE" }> =>
	...
	const coverageRecs2 = recs2.filter(
		(r) => r.stream === "coverage_diagnostics",
	);
	const messagesRecord2 = coverageRecs2.find(
		(r) => r.data.store === "derived_messages",
	);
...
test("codex coverage truth: nested unreadable directory makes coverage unaccounted", async () => {
	...
	const tempRoot = join(
		tmpdir(),
		`codex-unreadable-${Date.now()}-${Math.random().toString(36).slice(2)}`,
	);
	...
	try {
		...
		assert.equal(
			messagesRecord.data.status,
			"unaccounted",
			"nested unreadable must show unaccounted",
		);
		assert.equal(
			functionCallsRecord.data.status,
			"unaccounted",
...
test("codex coverage truth: existing file re-parsed counts as examined", async () => {
	...
	assert.equal(
		messagesRecord1.data.status,
		"collected",
		"first run with emitted records should show collected",
	);
...
test("codex coverage truth: genuine empty (ENOENT store root) shows collected (complete scan, zero examined)", async () => {
	...
	const emptyRoot = join(
		tmpdir(),
		`codex-truly-empty-${Date.now()}-${Math.random().toString(36).slice(2)}`,
	);
