...
test("codex fixture home: inventory-only stores emit metadata without payload", async () => {
	...
	assert(
		recs.some(
			(r) =>
				r.stream === "shell_snapshots" &&
				r.data.relative_path === "shell-snapshots/snapshot-1.sh",
		),
...
test("codex inventory: unchanged history store does not re-version across runs", async () => {
	...
	assert(
		run1Records.length > 0,
		"first run emits the history inventory record",
	);
	...
	assert.equal(
		run2Records.length,
		0,
		"unchanged history store does not re-emit on the second run",
	);
...
test("codex fixture homes: two device homes inventory independently", async () => {
	...
	assert(
		JSON.stringify(bPrompts).includes("Device B"),
		"device B prompt should reflect device B content",
	);
	assert(
		!JSON.stringify(aPrompts).includes("Device B"),
