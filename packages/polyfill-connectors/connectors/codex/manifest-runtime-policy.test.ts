...
test("codex manifest: every runtime inventory_only/defer stream is a non-required accepted-absence policy", () => {
	...
	const runtimeByStream = new Map(
		CODEX_KNOWN_LOCAL_STORES.filter((s) => s.stream).map((s) => [
			s.stream as string,
			s,
		]),
	);
	...
	for (const streamName of CODEX_GATED_INVENTORY_STREAMS) {
		...
		const expectedPolicy =
			RUNTIME_CLASSIFICATION_TO_MANIFEST_POLICY[runtimeStore.classification];
		assert(
			expectedPolicy,
		...
		if (!manifestStream) {
			violations.push(
				`${streamName}: runtime emits it but manifest declares no such stream`,
			);
			continue;
		}
	...
	assert.deepEqual(
		violations,
		[],
		`Codex manifest/runtime policy drift:\n${violations.join("\n")}`,
	);
});
...
test("codex manifest: content-bearing collect streams stay required", () => {
	...
	for (const streamName of contentBearingCollectStreams) {
		...
		assert(
			manifestStream,
			`runtime declares collect stream "${streamName}" but manifest has no such stream`,
		);
		if (manifestStream.required === false) {
			violations.push(
				`${streamName}: content-bearing collect stream must not be manifest required=false`,
			);
		}
	}
