...
const MESSAGE_ID_RE =
	/^([0-9a-f-]{36}|[0-9a-z]{26})(:\d+)?(:output)?$|^call_[A-Za-z0-9]{24}$/;
// Function calls have either OpenAI-style IDs, UUIDs, or composite IDs
const FUNCTION_CALL_ID_RE =
	/^call_[A-Za-z0-9]{24}$|^[0-9a-f-]{36}(:\d+)?(:output)?$|^[0-9a-z]{26}(:\d+)?(:output)?$/;
...
export const messagesSchema = z.object({
	id: z
		...
		.regex(
			MESSAGE_ID_RE,
			"message id must be uuid or uuid:line or uuid:line:output",
		),
	session_id: z
		.string()
		.regex(SESSION_ID_RE, "session_id must be uuid or ulid"),
...
export const functionCallsSchema = z.object({
	id: z
		...
		.regex(
			FUNCTION_CALL_ID_RE,
			"function_call id must be call_* or uuid or composite",
		),
	session_id: z
		.string()
		.regex(SESSION_ID_RE, "session_id must be uuid or ulid"),
