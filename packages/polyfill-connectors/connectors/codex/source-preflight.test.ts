	...
	assert(
		records.some(
			(record) =>
				...
				record.data.relative_path === "history.jsonl",
		),
	);
	assert(
		records.some(
			(record) =>
				record.stream === "session_index" &&
				record.data.relative_path === "session_index.jsonl",
		),
	);
	assert(
		records.some(
...
test("codex memories and context_mode are diagnostics-only, not requestable streams", async () => {
	...
	await writeFile(
		join(codexHome, "context-mode", "local.json"),
		'{"private":"secret-context-payload"}',
	);
	...
	assert(
		!records.some(
			(record) =>
				record.stream === "memories" || record.stream === "context_mode",
		),
	);
	assert(
		!records.some((record) =>
			JSON.stringify(record).includes("private memory"),
		),
	);
	assert(
		!records.some((record) =>
	...
	assert(
		!result.messages.some(
			(msg) =>
				msg.type === "STATE" &&
				["memories", "context_mode"].includes(
