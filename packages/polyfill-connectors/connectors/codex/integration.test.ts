...
test("processRolloutLine: session_meta installs the session id; subsequent messages carry session_id", () => {
	...
	assert.equal(
		msgRec.data.session_id,
		"sess-A",
		"session_id threads through from session_meta",
	);
});
...
test("processRolloutLine: messages disabled — function_calls still flow in the same pass", () => {
	...
	assert.equal(
		emitted.filter((r) => r.stream === "function_calls").length,
		1,
		"paired call emits once on flush",
	);
});
...
test("processRolloutLine: neither messages nor function_calls requested — nothing emits (sessions-only run)", () => {
	...
	assert.equal(
		emitted.length,
		0,
		"no per-line records when both streams are off",
	);
});
...
test("processRolloutLine: function_call + matching output merge into a single record", () => {
	...
	assert.equal(
		calls[0]?.data.arguments,
		"echo hi",
		"arguments from the call side",
	);
	assert.equal(
		calls[0]?.data.output_preview,
		"hi\n",
...
test("processRolloutLine: orphan function_call_output (no matching call) emits its own record immediately", () => {
	...
	const { deps, emitted } = makeHarness();
	drive(deps, [
		sessionMetaLine("sess-F"),
		functionCallOutputLine("orphan-x", "stdout bytes"),
	]);
	const calls = emitted.filter((r) => r.stream === "function_calls");
...
test("processRolloutLine: paired function_calls drain immediately — pendingCalls stays bounded across a large parse", () => {
	...
	for (let i = 0; i < CALLS; i += 1) {
		...
		processRolloutLine({
			obj: functionCallLine(`c${i}`, "shell", "x".repeat(64)),
			state,
			deps,
			file: "r.jsonl",
		});
		processRolloutLine({
			obj: functionCallOutputLine(`c${i}`, "y".repeat(64)),
			state,
			deps,
			file: "r.jsonl",
		});
	...
	assert.ok(
		maxPending <= 1,
		`pendingCalls must stay bounded; saw max ${maxPending} for ${CALLS} paired calls`,
	);
	assert.equal(
		state.pendingCalls.size,
		0,
		"every paired call drained before EOF",
	);
	...
	assert.equal(
		callsBeforeFlush.length,
		CALLS,
		"all paired records landed before any flush",
	);
	// Flush is now a no-op for paired calls — no duplicate emits.
	flushPendingCalls(state, deps);
	const callsAfterFlush = emitted.filter((r) => r.stream === "function_calls");
	assert.equal(
		callsAfterFlush.length,
		CALLS,
		"EOF flush adds nothing for already-paired calls (no double-emit)",
	);
});
...
test("processRolloutLine: an unpaired function_call still flushes once at EOF (no output line)", () => {
	...
	const state = makeRolloutParseState();
	processRolloutLine({
		obj: sessionMetaLine("sess-unpaired"),
		state,
		deps,
		file: "r.jsonl",
	});
	processRolloutLine({
		obj: functionCallLine("lonely", "shell", "sleep 1"),
		state,
		deps,
		file: "r.jsonl",
	});
...
test("processRolloutLine: offset-zero replay caps unmatched function_calls", () => {
	...
	const state = makeRolloutParseState();
	processRolloutLine({
		obj: sessionMetaLine("sess-unmatched"),
		state,
		deps,
	...
	for (let i = 0; i < calls; i += 1) {
		...
		assert.ok(
			state.pendingCalls.size <= 1024,
			"pending calls stay bounded during full replay",
		);
	}

	const emittedBeforeFlush = emitted.filter(
		(r) => r.stream === "function_calls",
	);
	assert.equal(
		emittedBeforeFlush.length,
		calls - 1024,
		"oldest unmatched calls are emitted before EOF",
	);
	assert.equal(
		state.pendingCalls.size,
		1024,
		"only the bounded pending window remains before EOF",
	);

	flushPendingCalls(state, deps);
	const emittedAfterFlush = emitted.filter(
		(r) => r.stream === "function_calls",
	);
	assert.equal(
		emittedAfterFlush.length,
		calls,
		"EOF flush emits the remaining unmatched calls exactly once",
	);
	assert.equal(
		state.pendingCalls.size,
		0,
		"flush clears the bounded pending window",
	);
});
...
test("processRolloutLine: an output for an evicted pending call lands as output-only fallback", () => {
	...
	const state = makeRolloutParseState();
	processRolloutLine({
		obj: sessionMetaLine("sess-evicted-output"),
		state,
		deps,
		file: "rollout-replay.jsonl",
	});
	...
	const evicted = emitted
		.filter((r) => r.stream === "function_calls")
		.find((r) => r.data.call_id === "call_0");
	assert.ok(
		evicted,
		"the oldest call is emitted before EOF when the pending window fills",
	);
	assert.equal(
		evicted.data.output_preview,
		null,
		"the evicted call has not seen its output yet",
	);
	...
	const call0 = emitted.filter(
		(r) => r.stream === "function_calls" && r.data.call_id === "call_0",
	);
	assert.equal(
		call0.length,
		2,
		"late output after eviction lands separately instead of being dropped",
	);
	assert.equal(
		call0[1]?.data.name,
		null,
		"late output fallback cannot recreate evicted call metadata",
	);
	assert.equal(call0[1]?.data.output_preview, "late output");
	assert.ok(
		state.pendingCalls.size <= 1024,
		"late output fallback does not regrow the pending window",
	);
});
...
test("processRolloutLine: messages and paired function_calls emit in file-line order within a session", () => {
	...
	const callIdxs = streams.flatMap((s, i) =>
		s === "function_calls" ? [i] : [],
	);
	...
	const [c0] = callIdxs;
	assert.ok(
		m0 !== undefined && m1 !== undefined && m0 < m1,
		"first < second message",
	);
	assert.ok(
		m1 !== undefined && m2 !== undefined && m1 < m2,
		"second < third message",
	);
	// The merged call record emits on its output line: after message 2, before message 3.
	assert.ok(
		m1 !== undefined && c0 !== undefined && m1 < c0,
		"paired call lands after its preceding (second) message",
	);
	assert.ok(
	...
	assert.equal(
		callRec?.data.name,
		"shell",
		"name from the call side survives the eager emit",
	);
	assert.equal(
		callRec?.data.arguments,
		"ls",
		"arguments from the call side survive",
	);
...
test("processRolloutLine: obj.timestamp threads into each messages record's `timestamp` field", () => {
	const { deps, emitted } = makeHarness();
	drive(deps, [
		sessionMetaLine("sess-H"),
		messageLine("hi", "user", "2026-04-22T01:23:45.000Z"),
	]);
	const msg = emitted.find((r) => r.stream === "messages");
	assert.ok(msg);
	assert.equal(
		msg.data.timestamp,
		"2026-04-22T01:23:45.000Z",
		"line timestamp pinned into the record",
	);
});
...
test("processRolloutLine: driving the same message line twice emits twice (no per-line dedup)", () => {
	...
	assert.equal(
		msgs.length,
		2,
		"two emits for two passes — no dedup at this layer",
	);
	// IDs differ because lineCount advances each pass.
	assert.notEqual(
		msgs[0]?.data.id,
		msgs[1]?.data.id,
		"ids disambiguate duplicate lines via lineCount",
	);
});
...
function makeCursor(
	priorEntries: readonly [string, ThreadFingerprint][] = [],
): CarryForwardCursor<ThreadFingerprint> {
	return openCarryForwardCursor<ThreadFingerprint>(
		new Map<string, ThreadFingerprint>(priorEntries),
	);
}

function makeAggregate(
	overrides: Partial<RolloutAggregate> = {},
): RolloutAggregate {
	return {
...
test("emitSessionsFromMaps: session present in BOTH threads + aggregates emits exactly ONCE (thread-preferred)", () => {
	...
	const threadsMap = new Map<string, ThreadRow>([
		["sess-dup", makeThreadRow("sess-dup")],
	]);
	const aggs = new Map<string, RolloutAggregate>([
		["sess-dup", makeAggregate()],
	]);
	emitSessionsFromMaps({
		threadsMap,
	...
	assert.equal(
		sessions[0]?.data.title,
		"from state_5",
		"thread row wins on title",
	);
	assert.equal(
		sessions[0]?.data.message_count,
		5,
		"aggregate merged in for counts",
	);
...
test("emitSessionsFromMaps: rollout-only sessions (not in threadsMap) emit as fallback with nulls for state_5 fields", () => {
	const { deps, emitted } = makeHarness();
	const aggs = new Map<string, RolloutAggregate>([
		["sess-rollout-only", makeAggregate()],
	]);
	emitSessionsFromMaps({
		threadsMap: new Map(),
		rolloutAggregates: aggs,
	...
	assert.equal(
		sessions[0]?.data.archived,
		null,
		"state_5-only fields null in fallback",
	);
	assert.equal(
		sessions[0]?.data.message_count,
		5,
		"aggregate counts still land",
	);
});
...
test("emitSessionsFromMaps: thread-only and rollout-only sessions both emit; disjoint ids yield two records", () => {
	const { deps, emitted } = makeHarness();
	const threadsMap = new Map<string, ThreadRow>([
		["sess-thread", makeThreadRow("sess-thread")],
	]);
	const aggs = new Map<string, RolloutAggregate>([
		["sess-rollout", makeAggregate()],
	]);
	emitSessionsFromMaps({
		threadsMap,
		rolloutAggregates: aggs,
...
test("shouldReemitThreadSession: new session (no prior fingerprint) — emit", () => {
	assert.equal(
		shouldReemitThreadSession(makeThreadRow("sess-new"), undefined, undefined),
		true,
	);
});

test("shouldReemitThreadSession: this run parsed the rollout — emit (aggregate may have moved counts)", () => {
	const prior: ThreadFingerprint = {
		updated_at: 1_700_000_010,
		message_count: 3,
		function_call_count: 1,
	};
	assert.equal(
		shouldReemitThreadSession(makeThreadRow("sess-x"), makeAggregate(), prior),
		true,
	);
});
...
test("emitSessionsFromMaps: unchanged thread with no aggregate — SKIPS emit but carries fingerprint forward", () => {
	...
	const { deps, emitted } = makeHarness();
	const threadsMap = new Map<string, ThreadRow>([
		["sess-stable", makeThreadRow("sess-stable")],
	]);
	const aggs = new Map<string, RolloutAggregate>();
...
test("emitSessionsFromMaps: thread WITH prior counts but no fresh aggregate — preserves counts (no lossy null overwrite)", () => {
	...
	const threadsMap = new Map<string, ThreadRow>([
		[
			"sess-touched",
			makeThreadRow("sess-touched", {
				updated_at: 1_700_000_999 /* moved forward */,
			}),
		],
	]);
	const aggs = new Map<string, RolloutAggregate>();
	...
	assert.equal(
		sessions[0]?.data.message_count,
		42,
		"prior count preserved, not null",
	);
	assert.equal(
		sessions[0]?.data.function_call_count,
		7,
		"prior count preserved, not null",
	);
...
test("emitSessionsFromMaps: fresh aggregate beats prior fingerprint counts (real updates win)", () => {
	...
	const { deps, emitted } = makeHarness();
	const threadsMap = new Map<string, ThreadRow>([
		["sess-active", makeThreadRow("sess-active")],
	]);
	const aggs = new Map<string, RolloutAggregate>([
	...
	assert.equal(
		sessions[0]?.data.message_count,
		99,
		"aggregate wins over fingerprint",
	);
...
test("emitSessionsFromMaps: first run (no prior fingerprints) — emits everything, populates sink", () => {
	...
	assert.equal(
		cursor.toState()["sess-1"]?.message_count,
		5,
		"aggregate count captured",
	);
	assert.equal(
		cursor.toState()["sess-2"]?.message_count,
		null,
		"no-aggregate thread fingerprint has null count",
	);
});
...
test("shared cursor STATE round-trip: run 2 skips the unchanged thread (no renewed churn)", () => {
	...
	const threadsMap = new Map<string, ThreadRow>([
		[
			"sess-stable",
			makeThreadRow("sess-stable", { updated_at: 1_700_000_010 }),
		],
		[
			"sess-active",
			makeThreadRow("sess-active", { updated_at: 1_700_000_010 }),
		],
	]);
	...
	emitSessionsFromMaps({
		...
		rolloutAggregates: new Map([
			[
				"sess-active",
				makeAggregate({ messageCount: 12, functionCallCount: 3 }),
			],
		]),
...
test("processRolloutLine: forked rollout — first session_meta pins child id; second session_meta (parent) is ignored", () => {
	...
	assert.equal(
		state.sessionId,
		childId,
		"sessionId stays on child after parent session_meta",
	);
	assert.equal(
		state.sessionMeta?.id,
		childId,
		"sessionMeta still describes the child",
	);
	...
	for (const m of msgRecs) {
		...
		assert.ok(
			String(m.data.id).startsWith(`${childId}:`),
			"message record id is prefixed with child session id",
		);
		assert.notEqual(
			m.data.session_id,
			parentId,
			"message must not attribute to fork parent",
		);
	}
	assert.equal(callRecs.length, 1, "the paired function_call lands once");
	assert.equal(
		callRecs[0]?.data.session_id,
		childId,
		"function_call session_id is the child",
	);
	// Counts on state are the child's lifetime within this rollout file.
	assert.equal(
		state.messageCount,
		2,
		"messageCount counts child messages only",
	);
	assert.equal(
		state.functionCallCount,
		1,
		"functionCallCount counts child calls only",
	);
});
...
test("processRolloutLine: forked rollout aggregate writes back under the child id, not the parent id", () => {
	...
	assert.notEqual(
		aggregate.meta.id,
		parentId,
		"aggregate.meta.id is never the fork parent",
	);
...
function makeFileCursor(
	overrides: Partial<RolloutFileCursor> = {},
): RolloutFileCursor {
	return {
		mtime_ms: 1000,
		size_bytes: 500,
...
test("decideRolloutAction: no cursor → full parse (new file)", () => {
	const action = decideRolloutAction({
		cursor: undefined,
		sizeBytes: 100,
		mtimeMs: 1,
		guardMatches: false,
...
test("decideRolloutAction: grown + guard matches → append from committed offset, seeded from cursor", () => {
	...
	if (action.kind === "append") {
		assert.equal(
			action.startOffset,
			480,
			"tails from the prior committed offset",
		);
		...
		assert.equal(
			action.seed.functionCallCount,
			3,
			"seeds cumulative function-call count",
		);
	}
});
...
test("readPriorFileCursors: decodes a well-formed cursor and drops malformed/legacy entries", () => {
	...
	const startMsg = {
		type: "START",
		state: { messages: { file_cursors: fileCursors } },
	} as StartMessage;
	const decoded = readPriorFileCursors(startMsg);
	assert.deepEqual(
		decoded["/rollouts/good.jsonl"],
		good,
		"well-formed cursor survives",
	);
	assert.equal(
		"/rollouts/partial.jsonl" in decoded,
		false,
		"cursor missing offset_bytes is dropped",
	);
	assert.equal(
		"/rollouts/garbage.jsonl" in decoded,
		false,
		"non-object cursor value is dropped",
	);
});
...
test("readPriorFileCursors: legacy state with only file_mtimes yields no rich cursors", () => {
	...
	assert.deepEqual(
		readPriorFileCursors(startMsg),
		{},
		"legacy mtime-only state → empty rich-cursor map",
	);
});
