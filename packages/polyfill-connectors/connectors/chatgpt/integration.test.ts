...
test("normalizeChatGptTerminalError maps pre-progress auth failures to refresh_credentials", () => {
	...
	assert.deepEqual(
		normalizeChatGptTerminalError({
			message:
				"chatgpt_session_failed: apiFetch got 401 on GET /conversation/abc (auth - not retryable)",
			retryable: true,
		}),
		{
			message:
...
for (const code of CHATGPT_MANUAL_ACTION_ERROR_CODES) {
	test(`normalizeChatGptTerminalError classifies exact production code ${code} as manual action`, () => {
		assert.deepEqual(
			normalizeChatGptTerminalError({ message: code, retryable: false }),
			{
...
test("normalizeChatGptTerminalError bounds and redacts parser/runtime diagnostics", () => {
	...
	assert.equal(
		"code" in normalized,
		false,
		"runtime_exception fallback must not invent a code",
	);
	assert.match(
		normalized.message,
		/^chatgpt_preprogress_failure: runtime_exception: /,
	);
	...
	assert.ok(
		normalized.message.length <=
			"chatgpt_preprogress_failure: runtime_exception: ".length + 240,
	);
});
...
test("normalizeChatGptTerminalError redacts relative provider paths, IDs, queries, and cursors but keeps structure", () => {
	...
	assert.match(
		normalized.message,
		/apiFetch got 404/u,
		"status and recovery facts remain visible",
	);
	assert.match(normalized.message, /\[redacted-path\]/u);
	assert.doesNotMatch(
		normalized.message,
		/\$1/u,
		"redaction replacements must not leak capture placeholders",
	);
	for (const secret of [
		...
		assert.equal(
			normalized.message.includes(secret),
			false,
			`diagnostic must redact ${secret}`,
		);
	}
});
...
test("ChatGPT detail fetch fast-opens on bare 429 after the configured attempts, not the full budget", async () => {
	...
	assert.equal(
		calls,
		3,
		"bare-429 detail fetch stops after the fast-open attempt cap",
	);
...
test("Part A: one HTTP request that retries N times causes ONE pacing backoff, not N", async () => {
	...
	const fakePage: Pick<Page, "evaluate" | "goto" | "waitForFunction"> = {
		evaluate: ((_fn: unknown, arg?: unknown): Promise<unknown> => {
			...
			const result =
				fetchResults[Math.min(fetchCallIndex, fetchResults.length - 1)];
			fetchCallIndex += 1;
	...
	assert.equal(
		result.status,
		200,
		"the request ultimately succeeds after retrying",
	);
	assert.equal(
		fetchCallIndex,
		3,
		"the single request made 3 backend fetches (2 retries + success)",
	);
		...
		evaluate: ((fn: unknown, arg?: unknown): Promise<unknown> => {
			if (arg === undefined) {
				...
				assert.doesNotMatch(
					String(fn),
					/__name/,
					"browser expression must not depend on bundler helper symbols",
				);
				return Promise.resolve({
					accessToken:
			...
			return Promise.resolve(
				call.auth?.accessToken === "stale-token"
...
test("createChatGptApi caps repeated-stale-session reauth at one per run, across multiple 401'ing paths", async () => {
	...
	const fakePage: Pick<Page, "evaluate" | "goto" | "waitForFunction"> = {
		evaluate: ((_fn: unknown, arg?: unknown): Promise<unknown> => {
			if (arg === undefined) {
				...
				return Promise.resolve({
					accessToken: `token-${authExtractionCalls}`,
					deviceId: "fake-device",
				});
			}
	...
	assert.equal(
		authExtractionCalls,
		2,
		"one initial auth + exactly one reauth for the first path",
	);
	...
	await assert.rejects(
		() => api.fetch("/custom_gpts"),
		/got 401 on GET \/custom_gpts/,
	);
	assert.equal(
		authExtractionCalls,
		2,
	...
	await assert.rejects(
		() => api.fetch("/gizmos/snorlax/bootstrap"),
		/got 401 on GET \/gizmos\/snorlax\/bootstrap/,
	);
	assert.equal(
		authExtractionCalls,
		2,
		"budget stays spent across every subsequent 401 in the run",
	);

	assert.equal(
		backendCalls.length,
		4,
		"1 initial + 1 repair retry for the first path, then 1 each for the next two",
	);
});
...
test("createChatGptApi.fetchBatch posts capped conversation batch requests", async () => {
	...
	const fakePage: Pick<Page, "evaluate" | "goto" | "waitForFunction"> = {
		evaluate: ((_fn: unknown, arg?: unknown): Promise<unknown> => {
			...
			backendCalls.push(
				arg as { body?: unknown; method?: string; path?: string },
			);
			return Promise.resolve({
				status: 200,
				json: [
...
test("resolveChatGptBackendFetchTimeoutMs supports small test overrides", () => {
	assert.equal(
		resolveChatGptBackendFetchTimeoutMs({
			PDPP_CHATGPT_BACKEND_FETCH_TIMEOUT_MS: "7",
		}),
		7,
	);
	assert.equal(
		resolveChatGptBackendFetchTimeoutMs({
			PDPP_CHATGPT_BACKEND_FETCH_TIMEOUT_MS: "0",
		}),
		45_000,
	);
	assert.equal(
		resolveChatGptBackendFetchTimeoutMs({
			PDPP_CHATGPT_BACKEND_FETCH_TIMEOUT_MS: "invalid",
		}),
		45_000,
	);
});
...
test("resolveChatGptDetailLaneTuning caps probe concurrency at the dataconnect-batch ceiling (5)", () => {
	const tuning = resolveChatGptDetailLaneTuning({
		PDPP_CHATGPT_DETAIL_MAX_CONCURRENCY_PROBE: "50",
	});
...
test("resolveChatGptRateLimitDensityStop honors an explicit positive override", () => {
	assert.equal(
		resolveChatGptRateLimitDensityStop({
			PDPP_CHATGPT_DETAIL_RATE_LIMIT_STOP_AFTER: "3",
		}),
		3,
	);
});
...
test("resolveChatGptRateLimitDensityStop disables the stop on a non-positive value (escape hatch)", () => {
	assert.equal(
		resolveChatGptRateLimitDensityStop({
			PDPP_CHATGPT_DETAIL_RATE_LIMIT_STOP_AFTER: "0",
		}),
		Number.POSITIVE_INFINITY,
	);
	assert.equal(
		resolveChatGptRateLimitDensityStop({
			PDPP_CHATGPT_DETAIL_RATE_LIMIT_STOP_AFTER: "-1",
		}),
		Number.POSITIVE_INFINITY,
	);
});
...
test("resolveChatGptRateLimitDensityStop falls back to the default on invalid (non-integer) input", () => {
	assert.equal(
		resolveChatGptRateLimitDensityStop({
			PDPP_CHATGPT_DETAIL_RATE_LIMIT_STOP_AFTER: "abc",
		}),
		8,
	);
	assert.equal(
		resolveChatGptRateLimitDensityStop({
			PDPP_CHATGPT_DETAIL_RATE_LIMIT_STOP_AFTER: "2.5",
		}),
		8,
	);
	assert.equal(
...
test("ChatGptRateLimitDensityTracker seeds with pre-detail served 429s and trips sooner", () => {
	...
	assert.equal(
		tracker.shouldStop(),
		true,
		"one in-lane 429 trips after the seed",
	);
});
...
test("ChatGptRateLimitDensityTracker seed at/over threshold trips before any in-lane 429", () => {
	...
	assert.equal(
		tracker.shouldStop(),
		true,
		"seed at threshold trips with zero in-lane 429s",
	);
});
...
test("ChatGptRateLimitDensityTracker seed is sanitized (negatives floored, floats truncated)", () => {
	...
	assert.equal(
		new ChatGptRateLimitDensityTracker(5, -4).count,
		0,
		"negative seed floors to 0",
	);
	assert.equal(
		new ChatGptRateLimitDensityTracker(5, 2.9).count,
		2,
		"fractional seed truncates",
	);
	assert.equal(
		new ChatGptRateLimitDensityTracker(5).count,
		0,
		"default seed is 0 (unchanged behavior)",
	);
});
...
test("chatGptBackendFetchInBrowser status-only mode does not parse response JSON", async () => {
		...
		Object.defineProperty(response, "json", {
			value: (): Promise<never> => {
				...
				return Promise.reject(
					new Error("json should not be parsed in status-only mode"),
				);
			},
		});
	...
	try {
		...
		assert.equal(
			parsedJson,
			false,
			"status-only preflight must not parse the body",
		);
	} finally {
		globalThis.fetch = originalFetch;
	}
...
function makeHarness({
	...
	fetchBatch?: (
		ids: readonly string[],
	) => Promise<ChatGptFetchResult[]> | ChatGptFetchResult[];
	...
	const api: ChatGptApi = {
		auth: (): Promise<never> =>
			Promise.reject(new Error("fakeApi.auth() unused in these tests")),
		...
		...(fetchBatch
			? {
					fetchBatch: (ids: readonly string[]): Promise<ChatGptFetchResult[]> =>
						Promise.resolve(fetchBatch(ids)),
				}
			: {}),
	};
...
async function admitFakeProviderBudget(
	providerBudget: ProviderBudgetController,
): Promise<void> {
	...
	let reason: ChatGptPlannedProviderBudgetDeferredError["reason"] =
		"max_detail_fetches";
	...
	throw new ChatGptPlannedProviderBudgetDeferredError(
		"fake provider budget gate closed",
		reason,
		gate,
	);
}
...
test("processConversationDetail: emits 'conversations' record BEFORE any 'messages' records (parent-first)", async () => {
	...
	assert.ok(
		convoIdx < firstMessageIdx,
		"conversation record must emit before the first message record",
	);
});
...
test("processConversationDetail: emits one messages record per mapping node with a role (both branches)", async () => {
	...
	assert.equal(
		msgRecords.length,
		3,
		"u1 + a1 + a2 emit; root is synthetic and skipped",
	);
	const currentFlags = new Map(
		msgRecords.map((r) => [r.data.id, r.data.on_current_branch]),
	);
	assert.equal(
		currentFlags.get("u1"),
		true,
		"u1 sits on the current branch → tip a1",
	);
	assert.equal(currentFlags.get("a1"), true, "a1 is the tip");
	assert.equal(
		currentFlags.get("a2"),
		false,
		"a2 is the off-branch alternative",
	);
});
...
test("processConversationDetail: messages-only scope still runs the emitConversation callback which no-ops", async () => {
	...
	assert.equal(
		...
		0,
		"conversations suppressed by scope",
	);
	assert.ok(
...
test("runMemoriesStream: empty requested scope — caller guards; direct call emits records regardless", async () => {
	...
	assert.equal(
		states.length,
		1,
		"STATE still fires so the stream cursor advances",
	);
});
...
test("runMemoriesStream: http 200 with an unreadable body is a parse_error, not a proven-empty scan", async () => {
	...
	assert.equal(
		...
		0,
		"an unreadable body must not commit a cursor",
	);
	assert.equal(
		memoriesCoverage(messages),
		undefined,
		"an unreadable body must not prove an empty boundary",
	);
	const skip = messages.find(
		(m): m is Extract<EmittedMessage, { type: "SKIP_RESULT" }> =>
			m.type === "SKIP_RESULT",
	...
	assert.equal(
		skip.reason,
		"parse_error",
		"distinct from http_error — the request succeeded, the body did not parse",
	);
 ...
 */
function asWrongShapeChatGptJson(
 ...
 */
const CHATGPT_WRONG_SHAPE_BODIES: ReadonlyArray<
	readonly [string, ChatGptJson]
> = [
	...
	[
		"an object missing the list key",
		asWrongShapeChatGptJson({ unrelated_field: true }),
	],
	[
		"an object whose list key is not an array",
		asWrongShapeChatGptJson({ items: "not-an-array" }),
	],
];
...
for (const [label, body] of CHATGPT_WRONG_SHAPE_BODIES) {
	test(`runMemoriesStream: http 200 with ${label} as the body is a parse_error, not a proven-empty scan`, async () => {
		...
		assert.equal(
			messages.filter((m) => m.type === "STATE").length,
			0,
			`${label}: must not commit a cursor`,
		);
		assert.equal(
			memoriesCoverage(messages),
			undefined,
			`${label}: must not prove an empty boundary`,
		);
		const skip = messages.find(
			(m): m is Extract<EmittedMessage, { type: "SKIP_RESULT" }> =>
				m.type === "SKIP_RESULT",
...
test("runMemoriesStream: COUNTERWEIGHT — genuine empty requires the expected array key present with []", async () => {
	...
	assert.ok(
		coverage,
		"the array key being present and empty IS genuine, provable emptiness",
	);
...
test("runMemoriesStream: successful EMPTY list proves zero coverage (genuine empty stays valid)", async () => {
	...
	assert.equal(
		emitted.length,
		0,
		"an account with no memories emits no records",
	);
	assert.equal(
		messages.filter((m) => m.type === "STATE").length,
		1,
		"a genuine empty list still commits STATE",
	);
	const coverage = memoriesCoverage(messages);
	assert.ok(
		coverage,
		"a successful empty enumeration must emit positive zero-coverage proof",
	);
	assert.equal(coverage.considered, 0);
	assert.equal(coverage.covered, 0);
});
...
test("runMemoriesStream: COUNTERWEIGHT — a populated list still emits records and proves the boundary", async () => {
	...
	assert.equal(
		emitted.filter((r) => r.stream === "memories").length,
		2,
		"positive records are unchanged",
	);
	const coverage = memoriesCoverage(messages);
	assert.ok(coverage);
	assert.equal(
		coverage.considered,
		2,
		"considered counts what the source listed",
	);
...
test("runMemoriesStream: a rejected entry counts toward considered but NOT covered, with a bounded shape_check_failed diagnostic", async () => {
	...
	assert.equal(
		emitted.filter((r) => r.stream === "memories").length,
		1,
		"only the well-formed entry emits",
	);
	const coverage = memoriesCoverage(messages);
	assert.ok(coverage);
...
test("processConversationDetail: detail.status=404 — still emits conversation (list-only) + SKIP on messages", async () => {
	...
	assert.ok(
		convo,
		"conversation record must still emit on http_error so downstream sees the row",
	);
	assert.equal(
...
test("processConversationDetail: detail=200 with missing mapping — list-only fallback + SKIP on messages", async () => {
	...
	const { deps, emitted, messages } = makeHarness();
	const stub: ChatGptFetchResult = {
		status: 200,
		json: { title: "stub but no mapping" },
	};
	await processConversationDetail(
		deps,
		makeConvo(),
...
test("runConversationsAndMessagesStreams: unsafe message content is sanitized to null, message still emits (no shape-skip)", async () => {
	...
	assert.equal(
		...
		0,
		"unsafe text is sanitized to null, not shape-skipped",
	);
	...
	assert.equal(
		...
		2,
		"both messages emit (unsafe with null content, safe with text)",
	);
	...
	assert.equal(
		unsafeRecord?.data.content,
		null,
		"unsafe content is null after sanitization",
	);
	const safeRecord = messageRecords.find((r) => r.data.id === safeNodeId);
	assert.equal(
		safeRecord?.data.content,
		"safe reply",
		"safe sibling content is preserved",
	);
	assert.equal(
	...
	assert.equal(
		stateEvents.length,
		1,
		"cursor commits once at normal end-of-stream",
	);
	const coverageIdx = harness.events.findIndex(
	...
	const lastRecordIdx = harness.events.findLastIndex(
		(e) => e.kind === "record",
	);
	assert.ok(
		coverageIdx > lastRecordIdx,
		"DETAIL_COVERAGE must emit after detail lane records settle",
	);
	assert.ok(stateIdx > coverageIdx, "STATE must emit after DETAIL_COVERAGE");
	assert.ok(
		stateIdx > lastRecordIdx,
		"STATE must remain after all record attempts",
	);
});
...
test("runMessagesAndConversationsWithDetail: fetches detail through adaptive lane with serialized jittered pacing", async () => {
	...
	assert.equal(
		maxActiveFetches,
		1,
		"conversation detail fetches must not overlap",
	);
	...
	assert.deepEqual(
		pauses,
		[],
		"ε-jitter imposes no deterministic launch floor (was a 1500ms manual throttle)",
	);
	...
	assert.deepEqual(
		progressMessages
			.filter((m) => m.message.startsWith("Synced "))
			.map((m) => m.message),
...
test("runMessagesAndConversationsWithDetail: batch detail happy path avoids per-id GET storm", async () => {
	...
	assert.deepEqual(
		fetches,
		[],
		"batch-hydrated conversations must not also hit /conversation/{id}",
	);
...
test("runMessagesAndConversationsWithDetail: batch omissions fall back only for omitted ids", async () => {
	...
	assert.deepEqual(
		...
		"only the omitted id falls back to per-id GET",
	);
	assert.deepEqual(coverage.hydratedKeys, ["convo-1", "convo-2", "convo-3"]);
	assert.deepEqual(coverage.gapKeys, []);
});
...
test("runMessagesAndConversationsWithDetail: 100 conversations use 10 capped batch calls, not 100 GETs", async () => {
	...
	const api: ChatGptApi = {
		...
		fetchBatch: (ids: readonly string[]): Promise<ChatGptFetchResult[]> => {
			...
			assert.ok(
				ids.length <= 10,
				"provider batch requests must never exceed 10 ids",
			);
			return Promise.resolve(ids.map((id) => makeDetailOkForConversation(id)));
		},
	};
	...
	assert.equal(
		batchCalls.length,
		10,
		"100 ids should be fetched as 10 provider-capped batches",
	);
	assert.deepEqual(
...
test("runMessagesAndConversationsWithDetail: intermediate pressure is bounded and redacted", async () => {
	...
	await runMessagesAndConversationsWithDetail(
		deps,
		[
			makeConvo({ id: "sensitive-convo-1" }),
			makeConvo({ id: "sensitive-convo-2" }),
		],
		makeEmitConversation(deps),
		{
			random: () => 0,
	...
	assert.equal(
		maxActiveFetches,
		1,
		"intermediate pressure must not raise detail concurrency",
	);
...
test("runMessagesAndConversationsWithDetail: cumulative 429 density WAITS OUT the account cool-down and continues (SLVP-ideal), losing nothing", async () => {
	...
	assert.deepEqual(
		gaps,
		[],
		"wait-resume hydrates the batch; nothing is deferred under available budget",
	);
...
test("runMessagesAndConversationsWithDetail: pre-detail 429s seed the density stop and trigger the cool-down WAIT sooner (still hydrating the batch)", async () => {
	...
	const coverage = await runMessagesAndConversationsWithDetail(
		...
		makeEmitConversation(deps),
		{
			random: () => 0,
			sleep: () => undefined,
			densityStopThreshold: 3,
			preDetailRateLimited: 2,
		},
	);
	...
	assert.deepEqual(
		gaps,
		[],
		"wait-resume defers nothing under available budget, even with a pre-detail seed",
	);
...
test("resolveChatGptMaxDetailFetchesPerRun: unset/non-positive → no cap; positive int opts into envelope", () => {
	...
	assert.equal(
		resolveChatGptMaxDetailFetchesPerRun({
			PDPP_CHATGPT_MAX_DETAIL_FETCHES_PER_RUN: "-5",
		}),
		Number.POSITIVE_INFINITY,
	);
	assert.equal(
		resolveChatGptMaxDetailFetchesPerRun({
			PDPP_CHATGPT_MAX_DETAIL_FETCHES_PER_RUN: "1.5",
		}),
		Number.POSITIVE_INFINITY,
	...
	assert.equal(
		resolveChatGptMaxDetailFetchesPerRun({
			PDPP_CHATGPT_MAX_DETAIL_FETCHES_PER_RUN: "250",
		}),
		250,
	);
});
...
test("resolveChatGptMaxRunWallClockMs: unset/non-positive → no cap; positive ms opts into envelope", () => {
	...
	assert.equal(
		resolveChatGptMaxRunWallClockMs({
			PDPP_CHATGPT_MAX_RUN_WALL_CLOCK_MS: "0",
		}),
		Number.POSITIVE_INFINITY,
	...
	assert.equal(
		resolveChatGptMaxRunWallClockMs({
			PDPP_CHATGPT_MAX_RUN_WALL_CLOCK_MS: "-1",
		}),
		Number.POSITIVE_INFINITY,
	);
	assert.equal(
		resolveChatGptMaxRunWallClockMs({
			PDPP_CHATGPT_MAX_RUN_WALL_CLOCK_MS: "1800000",
		}),
		1_800_000,
	);
	assert.equal(
		resolveChatGptMaxRunWallClockMs({
			PDPP_CHATGPT_MAX_RUN_WALL_CLOCK_MS: "1800000.9",
		}),
		1_800_000,
...
test("resolveChatGptProviderBudget: defaults enable pacing + circuit breaker + adaptive retry budget", async () => {
	...
	assert.ok(
		defaultBudget.pacing,
		"ChatGPT default enables adaptive inter-request pacing",
	);
	assert.equal(
	...
	assert.ok(
		...
		"retry budget is ON by default — adaptive give-up for wait-out regimes",
	);
	assert.equal(
	...
	assert.ok(
		defaultBudget.circuitBreaker,
		"ChatGPT default enables a circuit breaker",
	);

	// Explicit env override: capacity=5 restores the prior default-ON capacity exactly.
	const retryBudgetReenabled = resolveChatGptProviderBudget({
		PDPP_CHATGPT_RETRY_BUDGET_CAPACITY: "5",
	});
	assert.ok(
		retryBudgetReenabled?.retryBudget,
		"PDPP_CHATGPT_RETRY_BUDGET_CAPACITY overrides capacity",
	);
	assert.equal(
		retryBudgetReenabled.retryBudget.capacity,
		5,
		"capacity overridden to the specified value",
	);

	// initialTokens override via env.
	const customInitial = resolveChatGptProviderBudget({
		PDPP_CHATGPT_RETRY_BUDGET_INITIAL_TOKENS: "3",
	});
	assert.ok(
		customInitial?.retryBudget,
		"retry budget present with custom initialTokens",
	);
	assert.equal(
	...
	const pacingDisabled = resolveChatGptProviderBudget({
		PDPP_CHATGPT_PACING_INITIAL_INTERVAL_MS: "0",
	});
	assert.ok(pacingDisabled instanceof ProviderBudgetController);
	assert.equal(
		pacingDisabled.pacing,
		null,
		"pacing can be disabled independently",
	);
	assert.ok(
		pacingDisabled.retryBudget,
		"retry budget stays ON when only pacing is disabled",
	);
	...
	const retryOnly = resolveChatGptProviderBudget({
		PDPP_CHATGPT_MAX_DETAIL_FETCHES_PER_RUN: "10",
	});
	assert.ok(retryOnly instanceof ProviderBudgetController);
	assert.ok(
		retryOnly.pacing,
		"run cap override preserves default inter-request pacing",
	);
	assert.ok(
		retryOnly.retryBudget,
		"run cap derives a ratio-based retry budget",
	);
...
test("ChatGptRunBudget: no caps never stops; request budget and wall-clock budget each trip with the right reason", () => {
	...
	let nowMs = 1000;
	const clockCapped = new ChatGptRunBudget({
		maxWallClockMs: 500,
		now: () => nowMs,
	});
	assert.equal(
		clockCapped.reason(),
		null,
		"elapsed 0 < 500: open and anchored",
	);
	nowMs = 1400;
...
test("runMessagesAndConversationsWithDetail: a provider-request budget defers the tail as resumable run-cap DETAIL_GAP", async () => {
	...
	assert.equal(
		gaps.some(
			(g) => g.reason === "upstream_pressure" || g.reason === "rate_limited",
		),
		false,
	...
	assert.equal(
		harness.skipped.length,
		0,
		"no hydrated record was dropped by the shape-check",
	);
	const conversationRecords = harness.emitted.filter(
		(r) => r.stream === "conversations",
	);
	assert.equal(
...
test("runMessagesAndConversationsWithDetail: a wall-clock budget defers the tail via an injected clock", async () => {
	...
	assert.equal(
		gaps.every(
			(g) =>
				g.reason === "retry_exhausted" &&
				g.detail?.class === "run_cap_deferred",
		),
		true,
...
test("runMessagesAndConversationsWithDetail: an open upstream-pressure circuit with budget remaining waits out the cool-down and CONTINUES (does not defer-all-and-stop)", async () => {
	...
	assert.equal(
		providerBudget.circuitCooldownMs(),
		resetTimeoutMs,
		"circuit is open with the full cool-down owed",
	);
	...
	assert.deepEqual(
		coverage.gapKeys,
		[],
		"nothing is deferred when the circuit cools down within budget",
	);
...
test("runMessagesAndConversationsWithDetail: genuine wall-clock exhaustion DURING a circuit wait defers the tail (budget exhaustion still stops)", async () => {
	...
	assert.deepEqual(
		fetchedIds,
		["/conversation/convo-1"],
		"no fetch after genuine budget exhaustion",
	);
	...
	assert.equal(
		waitSleeps.length,
		0,
		"an exhausted budget is never waited out as a transient circuit",
	);
	const gaps = harness.protocolMessages.filter(
		(m): m is Extract<EmittedMessage, { type: "DETAIL_GAP" }> =>
			m.type === "DETAIL_GAP",
	...
	assert.ok(
		gaps.length >= 1,
		"the exhausted-budget tail is deferred as durable DETAIL_GAP records",
	);
	assert.equal(
		gaps.every(
			(g) =>
...
test("runMessagesAndConversationsWithDetail: a circuit that NEVER closes converges to a bounded defer (no infinite loop)", async () => {
	...
	assert.ok(
		waitSleeps.length > 0,
		"the run does wait out the circuit while it can",
	);
	assert.ok(
		waitSleeps.length <= 8,
	...
	assert.equal(
		coverage.hydratedKeys.length,
		0,
		"a never-closing circuit hydrates nothing",
	);
	assert.deepEqual(
		coverage.gapKeys,
		["convo-1", "convo-2"],
		"the tail defers durably after the bounded waits",
	);
	const gaps = harness.protocolMessages.filter(
		(m): m is Extract<EmittedMessage, { type: "DETAIL_GAP" }> =>
			m.type === "DETAIL_GAP",
	...
	assert.ok(
		gaps.length >= 1,
		"a never-closing circuit ends with durable DETAIL_GAP records, not an infinite loop",
	);
});
...
test("runMessagesAndConversationsWithDetail: the live 136s/900s shape now runs the full budget instead of exiting early (regression)", async () => {
	...
	const convos = Array.from({ length: 6 }, (_, i) =>
		makeConvo({ id: `convo-${i + 1}` }),
	);
	const coverage = await runMessagesAndConversationsWithDetail(
		deps,
		convos,
	...
	assert.equal(
		coverage.hydratedKeys.length,
		6,
		"the full tail is collected after waiting out the transient circuit",
	);
	assert.deepEqual(
		coverage.gapKeys,
		[],
		"no work is abandoned while budget remains",
	);
	assert.ok(
		fetchedIds.length > 2,
	...
	assert.ok(
		nowMs >= resetTimeoutMs,
		"the run spent real budget waiting out the cool-down, not exiting early",
	);
...
test("runMessagesAndConversationsWithDetail: empty budget leaves a large backlog unbounded", async () => {
	...
	const convos = Array.from({ length: 8 }, (_, i) =>
		makeConvo({ id: `convo-${i + 1}` }),
	);
	const coverage = await runMessagesAndConversationsWithDetail(
		deps,
	...
	assert.equal(
		fetchedIds.length,
		8,
		"every conversation is fetched when no cap is configured",
	);
...
test("runMessagesAndConversationsWithDetail: the GCRA pacing hint is the rate authority, not a launch-jitter floor", async () => {
	...
	assert.equal(
		providerSleeps.length,
		0,
		"controller does not sleep in signal mode (lane owns the single wait)",
	);
...
test("runMessagesAndConversationsWithDetail: emits structured provider-budget circuit transitions", async () => {
	...
	const providerBudgetEvents = harness.protocolMessages.filter(
		(message): message is Extract<EmittedMessage, { type: "PROGRESS" }> =>
			...
			message.provider_budget?.object === "provider_budget_circuit_transition",
	);

	assert.deepEqual(
		providerBudgetEvents.map(
			(message) => message.provider_budget?.circuit.state,
		),
		["half_open", "closed"],
...
test("runConversationsAndMessagesStreams: one run budget bounds the recovery pass AND the forward pass together", async () => {
	...
	assert.equal(
		fetchedIds.length,
		2,
		"the shared budget caps recovery + forward fetches together at 2",
	);
	assert.equal(
	...
	assert.equal(
		gaps.length >= 1,
		true,
		"the conversation past the shared budget is deferred as a resumable gap",
	);
	assert.equal(
		gaps.every(
			(g) =>
...
test("runConversationsAndMessagesStreams: capped forward run covers full listed tail before messages STATE", async () => {
	...
	assert.deepEqual(coverage?.gap_keys, [
		"convo-3",
		"convo-4",
		"convo-5",
		"convo-6",
	...
	const stateIdx = harness.events.findIndex(
		(e) =>
			e.kind === "message" &&
			e.message.type === "STATE" &&
			e.message.stream === "messages",
	);
	assert.ok(coverageIdx !== -1, "detail coverage must emit before state");
	assert.ok(
		stateIdx > coverageIdx,
		"messages STATE must only emit after full detail coverage",
	);
});
...
test("runConversationsAndMessagesStreams: a malformed 200 on the shared /conversations list is a typed parse_error, emits no coverage, and never advances either cursor", async () => {
	...
	const states = harness.protocolMessages.filter(
		(m): m is Extract<EmittedMessage, { type: "STATE" }> =>
			m.type === "STATE" && m.stream === "conversations",
	);
	assert.deepEqual(
...
test("runConversationsAndMessagesStreams: CURSOR SAFETY — a truncated walk leaves the list cursor pinned; a later healthy run reaches the previously-missing tail", async () => {
	...
	const trueMaxUpdateIso = new Date(
		(priorCursorUnix + 1000) * 1000,
	).toISOString();
	...
	const run1ConvoRecords = run1.emitted.filter(
		(r) => r.stream === "conversations",
	);
	assert.equal(
	...
	assert.equal(
		run1ListCoverage,
		undefined,
		"a partial-then-malformed walk must not certify a complete list boundary",
	);
	...
	for (const tailItem of missingTail) {
		assert.ok(
			run2Ids.has(tailItem.id),
			`run 2 must reach ${tailItem.id}, which run 1's truncation could never prove`,
		);
	}
	...
	assert.equal(
		run2Skip,
		undefined,
		"a clean run must not emit a truncation SKIP_RESULT",
	);
...
for (const [label, body] of CHATGPT_WRONG_SHAPE_BODIES) {
	test(`runConversationsAndMessagesStreams: a /conversations 200 body that is ${label} is a typed parse_error, not a proven-empty scan`, async () => {
		...
		assert.deepEqual(
			coverages,
			[],
			`${label}: must not claim coverage for either stream`,
		);
...
test("runConversationsAndMessagesStreams: an http_error on the SECOND /conversations page still withholds coverage and does not advance past the partial prefix", async () => {
	...
	assert.equal(
		conversationsCalls,
		2,
		"sanity: the walk actually reached the failing second page",
	);
	assert.equal(
		harness.emitted.filter((r) => r.stream === "conversations").length,
		100,
	...
	assert.deepEqual(
		coverages,
		[],
		"a list truncated mid-pagination must not claim coverage over its partial prefix",
	);
...
test("runConversationsAndMessagesStreams: hitting the PAGINATION_SAFETY_LIMIT mid-backlog does NOT advance the cursor past the un-listed older tail (data-loss regression)", async () => {
	...
	assert.ok(
		calls > 50,
		"sanity: the walk actually paged past the 5000-item cap",
	);
	assert.equal(
	...
	assert.equal(
		coverage,
		undefined,
		"a cap-truncated list must not claim coverage over its partial prefix",
	);
	const states = harness.protocolMessages.filter(
		(m): m is Extract<EmittedMessage, { type: "STATE" }> => m.type === "STATE",
	);
...
test("resolveChatGptMaxTailDeferralGapsPerRun: default-off, explicit, and fetch-cap-derived", () => {
	...
	assert.equal(
		resolveChatGptMaxTailDeferralGapsPerRun({
			PDPP_CHATGPT_MAX_TAIL_DEFERRAL_GAPS_PER_RUN: "10",
		}),
		10,
	);
	...
	assert.equal(
		resolveChatGptMaxTailDeferralGapsPerRun({
			PDPP_CHATGPT_MAX_TAIL_DEFERRAL_GAPS_PER_RUN: "0",
		}),
		Number.POSITIVE_INFINITY,
	);
	assert.equal(
		resolveChatGptMaxTailDeferralGapsPerRun({
			PDPP_CHATGPT_MAX_TAIL_DEFERRAL_GAPS_PER_RUN: "nope",
		}),
		Number.POSITIVE_INFINITY,
	);
	...
	assert.equal(
		resolveChatGptMaxTailDeferralGapsPerRun({
			PDPP_CHATGPT_MAX_DETAIL_FETCHES_PER_RUN: "300",
		}),
		300,
	);
});
...
test("runMessagesAndConversationsWithDetail: a cap trip over a large tail writes a BOUNDED number of gap rows", async () => {
	...
	assert.equal(
		gaps.length,
		11,
		"bounded tail writes at most chunk (10) per-key gaps + 1 backlog gap, not 200",
	);
	const perKeyGaps = gaps.filter(
		(g) => g.detail_locator.kind === "chatgpt.conversation",
	);
	const backlogGaps = gaps.filter(
		(g) => g.detail_locator.kind === "chatgpt.conversation_backlog",
	);
	assert.equal(
		perKeyGaps.length,
		10,
		"exactly the chunk of newest tail conversations get per-key gaps",
	);
	assert.equal(
		backlogGaps.length,
		1,
		"exactly one durable backlog gap represents the older remainder",
	);
	...
	const watermark = (backlog.detail_locator as { before_update_time?: unknown })
		.before_update_time;
	assert.equal(
		typeof watermark,
		"string",
		"the backlog gap carries an update_time watermark, not an offset",
	);
...
test("cap-tail backlog deferral is NOT source pressure and arms no cooldown", async () => {
	...
	const backlog = gaps.find(
		(g) => g.detail_locator.kind === "chatgpt.conversation_backlog",
	);
	assert.equal(
		backlog?.detail?.network_pressure?.error_class,
		"max_detail_fetches",
	);
	assert.equal(
		backlog?.detail?.http_status,
		undefined,
		"no HTTP status: nothing failed, the run chose to stop",
	);
});
...
test("a follow-up run expands the backlog gap (older window) before any forward work, no offsets", async () => {
	...
	const allConvos = Array.from({ length: 30 }, (_, i) =>
		makeConvo({
			id: `convo-${String(i + 1).padStart(2, "0")}`,
			update_time: 1_700_100_000 - i,
		}),
	);
	...
	while (safety < 12) {
		...
		const deps: StreamDeps = {
			...
			emit: async (msg): Promise<void> => {
				...
				if (msg.type === "STATE" && msg.stream === "messages") {
					const next =
						(msg.cursor as { last_update_time?: string | null } | undefined)
		...
		const hadBacklogBefore = served.some(
			(g) => g.detail_locator?.kind === "chatgpt.conversation_backlog",
		);
		...
		for (const id of harness.emitted
		...
		if (hadBacklogBefore) {
			assert.ok(
				listCalls.length >= 1,
				"a backlog-gap recovery re-lists the parent conversation list",
			);
		}
		const gapsThisRun = harness.protocolMessages.filter(
			(m) => m.type === "DETAIL_GAP",
		);
		assert.ok(
			gapsThisRun.length <= 11,
			"each run writes at most chunk (10) + 1 backlog gap",
		);
	...
	assert.ok(
		safety < 12,
		"the backlog converges to empty in a bounded number of runs",
	);
	...
	assert.equal(
		hydratedAcross.size,
		allConvos.length,
		"all 30 conversations drained oldest-ward with no loss",
	);
	assert.equal(
		isoOf("convo-30") < isoOf("convo-01"),
		true,
		"sanity: convo-30 is the oldest",
	);
});
...
test("runConversationsAndMessagesStreams: detail failure rejects before conversations STATE", async () => {
	...
	await assert.rejects(
		runConversationsAndMessagesStreams(deps, {}),
...
test("runConversationsAndMessagesStreams: isolated recoverable detail exhaustion emits DETAIL_GAP and then STATE", async () => {
	...
	assert.equal(
		harness.emitted.some(
			(r) => r.stream === "conversations" && r.data.id === "convo-gap",
		),
		false,
	...
	assert.equal(
		harness.emitted.some(
			(r) => r.stream === "messages" && r.data.conversation_id === "convo-gap",
		),
		false,
	...
	assert.ok(
		gap,
		"recoverable exhaustion should emit a durable detail-gap signal",
	);
	...
	assert.equal(
		serializedGap.includes("/conversation/convo-gap"),
		false,
		"gap must not expose raw API paths",
	);
	assert.equal(
		gap.detail?.network_pressure?.attempt,
		undefined,
		"gap must not persist attempt counters",
	);
	assert.equal(
		gap.detail?.network_pressure?.max_attempts,
		undefined,
		"gap must not persist max-attempt counters",
	);
	assert.equal(
		gap.last_error?.network_pressure?.attempt,
		undefined,
		"last_error must not persist attempt counters",
	);
	assert.equal(
	...
	assert.equal(
		serializedGap.includes("retry-after-ms"),
		true,
		"gap should expose safe retry-after metadata",
	);
	assert.equal(
		serializedGap.includes("bearer"),
		false,
		"gap must not expose raw error text or tokens",
	);
	assert.equal(
		serializedGap.includes("secret"),
		false,
		"gap must not expose raw error text or tokens",
	);
	...
	const gapIdx = harness.events.findIndex(
		(e) => e.kind === "message" && e.message.type === "DETAIL_GAP",
	);
	const coverageIdx = harness.events.findIndex(
		(e) => e.kind === "message" && e.message.type === "DETAIL_COVERAGE",
	);
...
test("runConversationsAndMessagesStreams: a single recoverable rate-limit on ONE conversation WAITS OUT and the remaining tranche is still fetched (run_1781286755231 regression)", async () => {
	...
	assert.equal(
		harness.emitted.filter(
			(r) => r.stream === "conversations" && r.data.id === "convo-1",
		).length,
		1,
	...
	assert.equal(
		harness.emitted.some(
			(r) => r.stream === "conversations" && r.data.id === "convo-2",
		),
		false,
	...
	const convo2Gap = harness.protocolMessages.find(
		(m): m is Extract<EmittedMessage, { type: "DETAIL_GAP" }> =>
			m.type === "DETAIL_GAP" && m.record_key === "convo-2",
	);
	...
	assert.equal(
		sleepCalls.length,
		8,
		"sleep must be called once per wait-out cycle (8 total)",
	);
});
...
test("runConversationsAndMessagesStreams: retry-exhaustion wait envelope exhausted (8 cycles) still falls back to durable tail-defer — lose-nothing preserved", async () => {
	...
	const api: ChatGptApi = {
		...
		fetch: (path: string): Promise<ChatGptFetchResult> => {
			...
			if (
				path === "/conversation/c-exhaust-1" ||
				path === "/conversation/c-exhaust-2"
			) {
				return Promise.reject(
					new ChatGptRecoverableRetryExhaustedError(
	...
	const detailPaths = fetches
		.filter((path) =>
			new URL(path, "https://chatgpt.test").pathname.startsWith(
				"/conversation/",
			),
		)
		.map((path) => {
	...
	assert.deepEqual(
		...
		Array.from({ length: 9 }, () => ({
			hash: "",
			pathname: "/conversation/c-exhaust-1",
			search: "",
		})),
	...
	assert.equal(
		gaps.length,
		3,
		"all three conversations must emit durable DETAIL_GAP records (lose-nothing)",
	);
	assert.ok(
...
test("runConversationsAndMessagesStreams: 30/278 pressure exhaustion records a durable gap and honest coverage", async () => {
	...
	assert.equal(
		harness.emitted.some(
			(r) => r.stream === "conversations" && r.data.id === pressureItem.id,
		),
		false,
	...
	assert.equal(
		deferredGaps.length,
		248,
		"later same-tranche items should be deferred without detail fetches",
	);
	...
	assert.ok(
		coverage,
		"successful cursor progress must include matching detail coverage",
	);
	...
	assert.ok(
		stateIdx > coverageIdx,
		"cursor STATE must only emit after coverage accounts for the pressure gap",
	);
	...
	const messagesState = harness.protocolMessages.find(
		(m): m is Extract<EmittedMessage, { type: "STATE" }> =>
			m.type === "STATE" && m.stream === "messages",
	);
	assert.ok(
		messagesState,
		"messages STATE cursor must commit even when most details are deferred as gaps",
	);
	const messagesCursor = messagesState.cursor as {
	...
	assert.ok(
		(messagesCursor.last_update_time ?? "") >
			new Date(lastHydratedUpdateTime * 1000).toISOString(),
...
test("runConversationsAndMessagesStreams: recovers pending conversation detail gaps before forward list collection", async () => {
	const harness = makeRecordingEmit(validateRecord);
	const recoveredConvo = makeConvo({
		id: "convo-recover",
		title: "Recover me",
		update_time: 1_700_000_100,
	});
	const forwardConvo = makeConvo({
		id: "convo-forward",
		update_time: 1_700_000_200,
	});
	const fetches: string[] = [];
	...
	const recoveredRecordIdx = harness.events.findIndex(
		(e) =>
			e.kind === "record" &&
			e.stream === "conversations" &&
			e.data.id === "convo-recover",
	);
...
test("runConversationsAndMessagesStreams: drains paged pending message gaps beyond the first 100 in one run", async () => {
	...
	const pages = [
		conversations
			...
			.map((conversation, index) =>
				makeDetailGapFromConvo(`gap_page_2_${index}`, conversation),
			),
		conversations
			.slice(120)
			.map((conversation, index) =>
				makeDetailGapFromConvo(`gap_page_3_${index}`, conversation),
			),
		[],
	];
	const fetches: string[] = [];
	...
	const deps: StreamDeps = {
		...
		detailGaps: conversations
			.slice(0, 60)
			.map((conversation, index) =>
				makeDetailGapFromConvo(`gap_page_1_${index}`, conversation),
			),
		emit: harness.emit,
		emitRecord: harness.emitRecord,
		progress: (): Promise<void> => Promise.resolve(),
	...
	const detailFetches = fetches.filter((path) =>
		path.startsWith("/conversation/"),
	);
	assert.equal(
		detailFetches.length,
		125,
		"all paged pending gaps are fetched in one connector run",
	);
	assert.equal(
		harness.protocolMessages.filter(
			(message) => message.type === "DETAIL_GAP_RECOVERED",
		).length,
		125,
...
test("runConversationsAndMessagesStreams: CONTINUOUS DRAIN — a partially-hydrated page does NOT end the run; the deferred tail is re-attacked and drained to zero in ONE run", async () => {
	...
	const deps: StreamDeps = {
		api,
		detailGaps: [
			makeDetailGapFromConvo("gap_a", convoA),
			makeDetailGapFromConvo("gap_b", convoB),
		],
		emit: harness.emit,
		emitRecord: harness.emitRecord,
		progress: (): Promise<void> => Promise.resolve(),
...
test("runConversationsAndMessagesStreams: a GENUINELY budget-exhausted recovery still defers (no forward walk)", async () => {
	...
	const first = makeConvo({
		id: "convo-budget-first",
		update_time: 1_700_000_100,
	});
	const second = makeConvo({
		id: "convo-budget-second",
		update_time: 1_700_000_200,
	});
	const fetches: string[] = [];
...
test("runConversationsAndMessagesStreams: density during recovery WAITS OUT the cool-down, hydrates the recovery item, and PROCEEDS to the forward walk", async () => {
	...
	assert.deepEqual(
		pressureGaps,
		[],
		"wait-resume hydrates the pressured recovery item instead of deferring it",
	);
	assert.ok(
	...
	assert.ok(
		...
		"the forward walk's list phase runs (advancing the cursor)",
	);
});
...
test("runConversationsAndMessagesStreams: a hot account that SUCCEEDS drains to zero — density wait-resume does NOT give up when successes reset the no-progress counter", async () => {
	...
	const deps: StreamDeps = {
		api,
		detailGaps: recItems.map((c, i) =>
			makeDetailGapFromConvo(`gap-hot-${i + 1}`, c),
		),
		emit: harness.emit,
		emitRecord: harness.emitRecord,
		progress: (): Promise<void> => Promise.resolve(),
	...
	assert.equal(
		fetchedDetail.length,
		12,
		"all 12 conversations fetched — healthy-hot account drains to zero",
	);
	const pressureGaps = harness.protocolMessages.filter(
		(m): m is Extract<EmittedMessage, { type: "DETAIL_GAP" }> =>
			m.type === "DETAIL_GAP" && m.reason === "upstream_pressure",
...
test("runConversationsAndMessagesStreams: a dead account (every fetch fails, no success) exhausts the no-progress counter and defers the tail as durable upstream_pressure gaps", async () => {
						...
						items: [
							{
								id: "fwd-new",
								title: "f",
								create_time: 1_700_001_900,
								update_time: 1_700_001_900,
								current_node: "a1",
							},
						],
					} as ChatGptJson,
	...
	const deps: StreamDeps = {
		api,
		detailGaps: recItems.map((c, i) =>
			makeDetailGapFromConvo(`gap-dead-${i + 1}`, c),
		),
		emit: harness.emit,
		emitRecord: harness.emitRecord,
		progress: (): Promise<void> => Promise.resolve(),
	...
	assert.ok(
		waitOuts.length >= 1,
		"the lane waited out at least one cool-down before giving up",
	);
	assert.ok(
		waitOuts.length <= 16,
	...
	assert.ok(
		...
		"dead account: remaining tail deferred as durable upstream_pressure gaps",
	);
	...
	assert.ok(
		listedCursors.length >= 1,
		"the forward walk list phase runs after the bounded-wait defer",
	);
});
...
test("runConversationsAndMessagesStreams: warm-start round-trip — a run persists its learned interval; the next run restores it", async () => {
	...
	assert.ok(
		learned < coldInterval,
		"the controller learned a faster interval across the run",
	);
	...
	const now = 1_000_000;
	const persistedFields = buildChatGptPacingStateFields(
		providerBudget,
		() => now,
	);
	const nextState = {
		messages: { last_update_time: null, ...persistedFields },
	} as CollectContext["state"];
	const persisted = readChatGptPersistedPacing(nextState);
	assert.ok(
		persisted,
		"the persisted pacing carries the learned interval + recordedAt",
	);
	assert.equal(
		persisted?.intervalMs,
		learned,
		"the persisted interval is the prior run's learned value",
	);
	assert.equal(
		persisted?.recordedAtMs,
		now,
		"the persisted pacing carries WHEN it was learned (for §10-E)",
	);
	...
	const warmBudget = resolveChatGptProviderBudget(
		{},
		{ intervalMs: learned, recordedAtMs: Date.now() },
	);
	assert.equal(
...
test("buildChatGptPacingStateFields: a throttle-inflated interval is CAPPED at cold-start on persist (no seed poisoning)", () => {
	...
	const throttled = resolveChatGptProviderBudget(
		{},
		{ intervalMs: 14_300, recordedAtMs: Date.now() },
	);
	assert.ok(
	...
	assert.ok(
		healthyInterval < coldInterval,
		"the healthy run learned a faster-than-cold interval",
	);
	assert.equal(
...
test("buildChatGptCollectionRateProgress: legible rate state carries no account content", () => {
	...
	assert.ok(
		progress.effective_rate_per_min > 0,
		"an effective rate is reported",
	);
	assert.equal(
		progress.last_backoff?.reason,
		"throttle",
		"the last back-off reason is legible",
	);
...
test("runConversationsAndMessagesStreams: terminal detail http failure remains fail-closed", async () => {
	...
	await assert.rejects(
		runConversationsAndMessagesStreams(deps, {}),
		/required conversation detail convo-terminal-404/,
	);
...
test("runCustomInstructionsStream: 200 → one record + STATE heartbeat", async () => {
	const { deps, emitted, messages } = makeHarness({
		fetchQueue: [
			{
				status: 200,
				json: { about_user_message: "I'm a tester", enabled: true },
			},
		],
	});
	await runCustomInstructionsStream(deps);
	assert.equal(
		emitted.filter((r) => r.stream === "custom_instructions").length,
		1,
	);
...
test("runCustomInstructionsStream: 404 → SKIP_RESULT('not_available'), no record, no STATE", async () => {
	...
	assert.equal(
		messages.filter((m) => m.type === "STATE").length,
		0,
		"no STATE when the stream short-circuits",
	);
});
...
test("runCustomInstructionsStream: http 200 with an unreadable body is a failure, not a synthetic cleared-instructions record", async () => {
	...
	assert.equal(
		emitted.length,
		0,
		"an unreadable body must not synthesize a record",
	);
	assert.equal(
		messages.filter((m) => m.type === "STATE").length,
		0,
...
test("runCustomInstructionsStream: a genuinely cleared/empty body still emits its record and STATE (distinguishable from parse_error)", async () => {
	...
	assert.equal(
		records.length,
		1,
		"a genuine empty body still builds and emits its (all-null) record",
	);
	assert.equal(records[0]?.data.about_user, null);
	assert.equal(records[0]?.data.enabled, null);
	assert.equal(
		messages.filter(
			(m) => m.type === "STATE" && m.stream === "custom_instructions",
		).length,
		1,
	...
	assert.equal(
		skip,
		undefined,
		"a genuinely empty body is not a parse failure",
	);
});
...
test("runCustomInstructionsStream: an unreadable body on a later run does not prune the prior fingerprint (no fabricated clear)", async () => {
	...
	assert.ok(
		priorCursor.fingerprints,
		"the first run persists a fingerprint for the real record",
	);

	const second = makeHarness({ fetchQueue: [{ status: 200, json: null }] });
	await runCustomInstructionsStream(second.deps, {
		custom_instructions: priorCursor,
	});
	assert.equal(
...
test("runCustomInstructionsStream: a successful 200 with a real record proves coverage (considered === covered === 1)", async () => {
	...
	assert.ok(
		coverage,
		"a successful observation must emit coverage — this is the run_1786336482583 regression",
	);
	assert.equal(
		coverage.considered,
		1,
		"the account has exactly one custom-instructions boundary",
	);
	assert.equal(
		coverage.covered,
		1,
		"the boundary was successfully observed and a record emitted",
	);
	assert.equal(
		coverage.state_stream,
		"custom_instructions",
		"singleton full scan: stream is its own state stream",
	);
});
...
test("runCustomInstructionsStream: a fingerprint-suppressed no-op still proves coverage — record count must not gate the proof", async () => {
	...
	assert.ok(
		coverage,
		"zero emitted records must not read as zero coverage — the source was genuinely observed",
	);
	assert.equal(coverage.considered, 1);
	assert.equal(
		coverage.covered,
		1,
		"fingerprint-suppressed-as-unchanged is a genuinely-observed outcome, not a gap",
	);
});
...
test("runCustomInstructionsStream: a genuinely cleared/empty body proves coverage exactly like a populated one", async () => {
	...
	assert.ok(
		coverage,
		"a genuine empty/cleared account still proves its one boundary was observed",
	);
	assert.equal(coverage.considered, 1);
...
test("runCustomGptsStream: paginates gizmos/mine and emits STATE when complete", async () => {
	...
	assert.equal(
		messages.filter((m) => m.type === "STATE" && m.stream === "custom_gpts")
			.length,
		1,
	);
});
...
test("runCustomGptsStream: hitting the GIZMO_MAX_PAGES safety cap with a cursor still present is truncation — no coverage, no STATE", async () => {
	...
	assert.ok(
		calls > 50,
		"sanity: the walk actually reached past the 50-page cap",
	);
	assert.equal(
	...
	assert.equal(
		coverage,
		undefined,
		"a cap-truncated scan must not claim coverage over its partial prefix",
	);
	assert.equal(
		harness.protocolMessages.filter(
			(m) => m.type === "STATE" && m.stream === "custom_gpts",
		).length,
		0,
...
test("runCustomGptsStream: hitting the GIZMO_MAX_PAGES cap exactly when the source also runs out (no next cursor) is honest completion", async () => {
	...
	assert.equal(
		calls,
		51,
		"sanity: the walk fetched exactly through the last, cursor-less page",
	);
	const coverage = customGptsCoverage(harness.protocolMessages);
	assert.ok(
		coverage,
		"the cap coinciding with genuine end-of-list is honest completion, not truncation",
	);
	...
	assert.equal(
		harness.protocolMessages.filter(
			(m) =>
				m.type === "SKIP_RESULT" && m.reason === "pagination_cap_truncated",
		).length,
		0,
	);
});
...
test("runCustomGptsStream: successful EMPTY enumeration proves zero coverage", async () => {
	...
	assert.equal(
		coverage.state_stream,
		"custom_gpts",
		"full scan: the stream is its own state stream",
	);
...
test("runCustomGptsStream: COUNTERWEIGHT — a populated scan still emits records and counts the boundary", async () => {
	...
	assert.equal(
		emitted.filter((r) => r.stream === "custom_gpts").length,
		2,
		"positive records are unchanged",
	);
	const coverage = customGptsCoverage(messages);
	...
	assert.equal(
		coverage.considered,
		2,
		"considered counts what the source listed, measured at the enumeration site",
	);
	assert.equal(coverage.covered, 2);
});
...
test("runCustomGptsStream: considered counts enumerated items, and covered excludes a rejected one (bounded shape_check_failed diagnostic)", async () => {
	...
	assert.equal(
		emitted.filter((r) => r.stream === "custom_gpts").length,
		1,
		"the unusable item yields no record",
	);
	const coverage = customGptsCoverage(messages);
	assert.ok(coverage);
...
test("runCustomGptsStream: an {info,list}-wrapped item (live shape drift, run_1786417045973) is read, not rejected", async () => {
	...
	assert.equal(
		gpts.length,
		1,
		"the {info,list}-wrapped gizmo is emitted, not dropped",
	);
	assert.equal(gpts[0]?.data.id, "g-1");
	...
	assert.equal(
		coverage.covered,
		1,
		"no shape_check_failed reject for a recognized wrapper",
	);
	assert.equal(
		messages.some(
...
test("runCustomGptsStream: a populated cuts body emits records and proves coverage", async () => {
	...
	assert.equal(
		gpts.length,
		2,
		"cuts is read as the item array, same as items/gizmos",
	);
	assert.equal(gpts[0]?.data.id, "g-1");
	assert.equal(gpts[1]?.data.id, "g-2");
	const coverage = customGptsCoverage(messages);
...
test("runCustomGptsStream: an empty cuts:[] body proves zero coverage, not silence", async () => {
	const { deps, emitted, messages } = makeHarness({
		fetchQueue: [
			{
				status: 200,
				json: { cuts: [], workspace_filtered: false, locale: "en-US" },
			},
		],
		requested: ["custom_gpts"],
	});
	...
	assert.equal(
		emitted.length,
		0,
		"an account with no gizmos in cuts emits no records",
	);
	const coverage = customGptsCoverage(messages);
	assert.ok(
		coverage,
		"a genuinely empty cuts array must positively prove zero coverage",
	);
	assert.equal(coverage.considered, 0);
	assert.equal(coverage.covered, 0);
	assert.equal(
...
test("runCustomGptsStream: cuts present but not an array is still a parse_error, not proven-empty", async () => {
	...
	assert.equal(
		customGptsCoverage(messages),
		undefined,
		"a non-array cuts value must not prove an empty boundary",
	);
	assert.equal(messages.filter((m) => m.type === "STATE").length, 0);
	const skip = messages.find(
		(m): m is Extract<EmittedMessage, { type: "SKIP_RESULT" }> =>
...
test("runCustomGptsStream: a body with none of items/gizmos/cuts (the real run_1786336482583 shape) still fails closed", async () => {
	...
	const { deps, emitted, messages } = makeHarness({
		fetchQueue: [
			{ status: 200, json: { workspace_filtered: true, locale: "en-US" } },
		],
		requested: ["custom_gpts"],
	});
	...
	assert.ok(skip);
	assert.equal(skip.reason, "parse_error");
	const diag = skip.diagnostics as
		| { present_keys?: string[]; expected_keys?: string[] }
		| undefined;
...
test("runCustomGptsStream: cuts paginates via cursor exactly like items/gizmos", async () => {
		...
		fetchQueue: [
			{
				...
				json: {
					cursor: "next-page",
					cuts: [{ id: "g-1", display_name: "Planner" }],
				},
			},
	...
	assert.equal(
		gpts.length,
		2,
		"the cursor on a cuts-shaped page is followed the same as items/gizmos",
	);
	assert.equal(gpts[0]?.data.id, "g-1");
	assert.equal(gpts[1]?.data.id, "g-2");
	const coverage = customGptsCoverage(messages);
...
test("runSharedConversationsStream: paginates shared conversations and emits STATE when complete", async () => {
	...
	assert.equal(
		messages.filter(
			(m) => m.type === "STATE" && m.stream === "shared_conversations",
		).length,
		1,
	);
...
test("runSharedConversationsStream: hitting the PAGINATION_SAFETY_LIMIT with more full pages remaining is truncation — no coverage, no STATE, no fingerprint pruning", async () => {
	...
	const deps: StreamDeps = {
		...
		progress: (): Promise<void> => Promise.resolve(),
		requested: new Map(
			["shared_conversations"].map((name) => [name, { name }]),
		),
	};
	...
	assert.equal(
		harness.emitted.filter((r) => r.stream === "shared_conversations").length >
			0,
		true,
...
test("runSharedConversationsStream: 404 → SKIP_RESULT('not_available'), no record", async () => {
	...
	assert.equal(messages.filter((m) => m.type === "STATE").length, 0);
	assert.equal(
		sharedConversationsCoverage(messages),
		undefined,
		"a failed scan must not prove any boundary",
	);
});
...
test("runSharedConversationsStream: successful EMPTY enumeration proves zero coverage", async () => {
	...
	assert.ok(
		coverage,
		"a genuinely empty page must still prove considered === covered === 0",
	);
	assert.equal(coverage.considered, 0);
	assert.equal(coverage.covered, 0);
});
...
test("runSharedConversationsStream: a rejected record counts toward considered but NOT covered, and gets a bounded shape_check_failed diagnostic", async () => {
	...
	assert.equal(
		coverage.considered,
		2,
		"considered counts both listed items, not just the emitted one",
	);
	assert.equal(
		coverage.covered,
		1,
...
test("runSharedConversationsStream: fingerprint-suppressed records on an unchanged re-list still count as covered", async () => {
	...
	await runSharedConversationsStream(second.deps, {
		shared_conversations: priorCursor,
	});
	...
	assert.ok(
		coverage,
		"genuinely observed shares suppressed by the fingerprint gate still prove coverage",
	);
	assert.equal(
		coverage.considered,
		3,
		"all 3 shares were genuinely observed on the wire",
	);
	assert.equal(
		coverage.covered,
		3,
		"fingerprint suppression is not a failure to observe — covered stays 3",
	);
});
...
test("runCustomInstructionsStream: unchanged body on a second run emits zero records but still writes STATE", async () => {
	const body = {
		about_user_message: "I'm a tester",
...
test("runCustomInstructionsStream: a real edit on the second run re-emits the record", async () => {
	...
	const v2 = {
		about_user_message: "I'm a tester now with a different bio",
		enabled: true,
	};

	const first = makeHarness({
		fetchQueue: [{ status: 200, json: v1 }],
		requested: ["custom_instructions"],
	});
	await runCustomInstructionsStream(first.deps, {});
	const priorCursor = lastStateCursor(first.messages, "custom_instructions");
	...
	const second = makeHarness({
		fetchQueue: [{ status: 200, json: v2 }],
		requested: ["custom_instructions"],
	});
	await runCustomInstructionsStream(second.deps, {
		custom_instructions: priorCursor,
	});
	const shares = second.emitted.filter(
		(r) => r.stream === "custom_instructions",
	);
	assert.equal(
		shares.length,
		1,
		"an edited body is a fingerprint boundary → it re-emits",
	);
	assert.equal(
		shares[0]?.data.about_user,
		"I'm a tester now with a different bio",
	);
});
...
test("runSharedConversationsStream: a newly-shared conversation on the second run emits only that one", async () => {
	...
	const shares = second.emitted.filter(
		(r) => r.stream === "shared_conversations",
	);
	assert.equal(
		shares.length,
		1,
		"only the new share is a fingerprint boundary → only it re-emits",
	);
...
test("runSharedConversationsStream: a deleted share is pruned so it does not block a future re-share", async () => {
	...
	assert.ok(
		!("s-1" in fingerprints2),
		"the vanished share is pruned from the carry-forward map",
	);
	...
	await runSharedConversationsStream(third.deps, {
		shared_conversations: cursor2,
	});
	const shares = third.emitted.filter(
		(r) => r.stream === "shared_conversations",
	);
	assert.equal(
		shares.length,
		1,
		"the re-shared conversation re-emits after having been pruned",
	);
...
function makeStatusApi(statuses: readonly number[]): {
	api: ChatGptApi;
	paths: string[];
} {
	const paths: string[] = [];
	...
	const api: ChatGptApi = {
		...
		fetch: (): Promise<never> =>
			Promise.reject(
				new Error("api.fetch() should not run for status-only preflight"),
			),
		fetchStatus: (
			path: string,
...
test("classifyChatGptSourcePressure: a retry-exhausted bare-429 circuit is pressured", async () => {
	const api: ChatGptApi = {
		...
		fetchStatus: (): Promise<Pick<ChatGptFetchResult, "headers" | "status">> =>
			Promise.reject(
				new ChatGptRecoverableRetryExhaustedError(
					"apiFetch got 429 after retry budget exhausted",
					{
						class: "rate_limited",
						httpStatus: 429,
					},
...
test("applyChatGptColdStatePreflight: serial production tuning fires NO probe (byte-for-byte unchanged)", async () => {
	...
	const effective = await applyChatGptColdStatePreflight(
		deps,
		[makeConvo({ id: "convo-1" })],
		SERIAL_TUNING,
	);
	assert.deepEqual(
		effective,
		SERIAL_TUNING,
		"serial tuning returned unchanged",
	);
	assert.deepEqual(
		paths,
		[],
		"no preflight probe was fired in production posture",
	);
	assert.deepEqual(
...
test("applyChatGptColdStatePreflight: cold account lets the requested faster posture through", async () => {
	...
	assert.deepEqual(
		effective,
		FAST_TUNING,
		"cold preflight keeps the requested faster tuning",
	);
	assert.deepEqual(
		paths,
		[
...
test("applyChatGptColdStatePreflight: burst-sensitive pressure forces serial despite clean serial probes", async () => {
	...
	assert.equal(
		...
		1,
		"burst pressure forces serial maxConcurrency",
	);
...
test("runMessagesAndConversationsWithDetail: hot account at probe-concurrency falls back to serial (no overlap)", async () => {
	...
	assert.equal(
		maxActiveFetches,
		1,
		"pressured preflight forced the detail lane to run serially",
	);
});
...
test("runMessagesAndConversationsWithDetail: post-cap emits full tail gaps and stops idle lane launches", async () => {
	...
	assert.equal(
		gaps.length,
		6,
		"every unhydrated listed conversation gets a durable run-cap gap",
	);
...
test("runMessagesAndConversationsWithDetail: no-cap run is byte-for-byte unchanged", async () => {
	...
	assert.deepEqual(
		coverage.gapKeys,
		[],
		"no gaps materialized on an uncapped run",
	);
...
test("runConversationsAndMessagesStreams: recoveryOnly=true runs recovery then returns before list-phase fetch", async () => {
	...
	const recovered = harness.protocolMessages.filter(
		(m) => m.type === "DETAIL_GAP_RECOVERED",
	);
	assert.equal(
		recovered.length,
		1,
		"recovered gap emits exactly one DETAIL_GAP_RECOVERED",
	);
});
...
test("adaptive retry budget: a healthy account refills tokens so the run never depletes (retry-exhausted regime)", async () => {
	...
	assert.ok(
		waitOuts.every(
			(m) =>
				m.message.includes("no-progress waits:") && m.message.includes("/8"),
		),
		"wait messages report the progress-based no-progress counter (N/8, resets on the next successful fetch)",
	);
	assert.ok(
		waitOuts.every((m) =>
			m.message.includes("resets on the next successful fetch"),
		),
...
test("progress-based give-up: a dead account that never succeeds gives up after CHATGPT_CIRCUIT_WAIT_OUT_MAX_CYCLES=8 consecutive wait-outs (retry-exhausted regime)", async () => {
	...
	assert.ok(
		gaps.length > 0,
		"dead account: no-progress give-up triggers the durable lose-nothing defer",
	);
	assert.ok(
	...
	const api: ChatGptApi = {
		...
		fetch: (path: string): Promise<ChatGptFetchResult> => {
			...
			if (!recoveredConvos.has(convoId)) {
				...
				return Promise.reject(
					new ChatGptRecoverableRetryExhaustedError(
						`apiFetch got 429 on GET ${path} after retries fake-secret`,
						{
							class: "rate_limited",
							httpStatus: 429,
						},
...
test("PROGRESS RESET oracle: 5 wait-outs → success → 5 more wait-outs → NEVER gives up (counter resets on success)", async () => {
	...
	const convos = Array.from({ length: 16 }, (_, i) =>
		makeConvo({ id: `reset-${i + 1}` }),
	);
	const coverage = await runMessagesAndConversationsWithDetail(
		deps,
		convos,
...
test("DEAD ACCOUNT oracle: no successes ever → gives up after CHATGPT_CIRCUIT_WAIT_OUT_MAX_CYCLES=8 consecutive wait-outs (lose-nothing)", async () => {
	...
	assert.equal(
		coverage.hydratedKeys.length,
		0,
		"DEAD ACCOUNT: no conversations hydrated",
	);

	// The run must have given up — some conversations deferred durably.
	assert.ok(
		coverage.gapKeys.length > 0,
		"DEAD ACCOUNT: tail deferred as durable DETAIL_GAP records (lose-nothing)",
	);
...
test("adaptive retry budget: no-budget fallback uses consecutiveWaitOutsWithoutSuccess=8 cap (retry-exhausted regime)", async () => {
	...
	assert.ok(
		gaps.length > 0,
		"no-budget fallback: fixed-cycle cap triggers the durable lose-nothing defer",
	);
	assert.ok(
		coverage.gapKeys.length > 0,
		"no-budget fallback: tail conversations deferred after cycle cap",
	);
...
test("regression: ProviderBudgetController present but WITHOUT retryBudget terminates via cycle cap (no infinite loop)", async () => {
	...
	assert.equal(
		...
		false,
		"precondition: no retryBudget on this controller",
	);
	...
	const api: ChatGptApi = {
		...
		fetch: (path: string): Promise<ChatGptFetchResult> =>
			...
			Promise.reject(
				new ChatGptRecoverableRetryExhaustedError(
					`apiFetch got 429 on GET ${path} fake-secret`,
					{
						class: "rate_limited",
						httpStatus: 429,
					},
	...
	assert.ok(
		gaps.length > 0,
		"controller-present-no-retryBudget: tail deferred as DETAIL_GAP after cycle cap",
	);
	assert.ok(
		coverage.gapKeys.length > 0,
		"controller-present-no-retryBudget: gapKeys populated after cycle cap",
	);
});
