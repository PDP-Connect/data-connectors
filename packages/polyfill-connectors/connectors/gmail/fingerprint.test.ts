...
test("buildThreadFingerprint: participant-set insertion order does NOT change the hash", () => {
	...
	const b = makeAgg({
		participant_set: new Set(["bob@example.com", "alice@example.com"]),
	});
...
test("buildThreadFingerprint: flag/attachment changes participate in the hash", () => {
	const base = makeAgg();
	assert.notEqual(
		buildThreadFingerprint(base),
		buildThreadFingerprint({ ...base, unread_count: 1 }),
	);
	assert.notEqual(
		buildThreadFingerprint(base),
		buildThreadFingerprint({ ...base, flagged_count: 1 }),
	);
	assert.notEqual(
		buildThreadFingerprint(base),
		buildThreadFingerprint({ ...base, has_attachments: true }),
	);
});
...
test("readPriorThreadFingerprints: legacy state (no thread_fingerprints) → empty map (one-time re-emit cost)", () => {
	const out = readPriorThreadFingerprints({
		threads: { fetched_at: "2026-05-26T00:00:00.000Z" },
	});
	assert.equal(out.size, 0);
});
...
function persistThreadsState(
	...
	return {
		threads: {
			...
			thread_fingerprints: cursor.toState(),
		},
	};
}

function openCursorFromState(
	state: Record<string, unknown>,
): ReturnType<typeof openFingerprintCursor> {
	return openFingerprintCursor(state, {
...
test("two-pass invariant: unchanged thread aggregates emit on pass 1, zero on pass 2", async () => {
	...
	assert.equal(
		run2.emitted.length,
		0,
		"second pass emits nothing — unchanged threads do not re-emit",
	);
	...
	assert.equal(
		run3.emitted.length,
		0,
		"third pass also no-ops — fingerprints carried forward across skipped runs",
	);
});
...
test("two-pass invariant: thread present in pass 1 but absent in pass 2 is pruned from STATE", async () => {
	...
	assert.equal(
		Object.keys(cursor2.toState()).length,
		2,
		"carry-forward keeps T2 pre-prune",
	);
	cursor2.dropUnseenIds();
	...
	assert.equal(
		post.T2,
		undefined,
		"absent id pruned — future re-creation re-emits",
	);
});
...
test("two-pass invariant: a legacy STATE without thread_fingerprints triggers one-time re-emit, then no-ops", async () => {
	...
	assert.equal(
		run1.emitted.length,
		1,
		"legacy state forces one-time re-emit (cursor migration cost)",
	);
	...
	assert.equal(
		run2.emitted.length,
		0,
		"after the migration emit, subsequent runs no-op",
	);
});
...
test("shared helper hash matches buildThreadFingerprint — wire-compatible with pre-helper STATE", async () => {
	...
	assert.equal(
		helperFp,
		buildThreadFingerprint(agg),
		"hash bytes match the local builder",
	);
});
