...
test("labels: an unchanged mailbox set does not re-emit on the second run", () => {
	...
	assert.deepEqual(
		run2.emitted,
		[],
		"unchanged labels are fully suppressed on the second run",
	);
});
...
test("labels: a newly-created label emits; the unchanged ones stay silent", () => {
	...
	assert.deepEqual(
		run2.emitted,
		["Work/Receipts"],
		"only the new label re-emits",
	);
});

test("labels: a changed flag on an existing label re-emits", () => {
	const run1 = runLabelsPass({}, [
		labelBody({ name: "Promotions", is_system: true }),
	]);
	// Same name, but is_system flipped (e.g. canonicalization changed).
	const run2 = runLabelsPass(run1.nextState, [
		labelBody({ name: "Promotions", is_system: false }),
	]);
	assert.deepEqual(
		run2.emitted,
		["Promotions"],
		"a real field change moves the fingerprint and re-emits",
	);
});
...
test("labels: a deleted mailbox is pruned so it re-emits if re-created", () => {
	...
	const run3 = runLabelsPass(run2.nextState, [
		labelBody({ name: "INBOX" }),
		labelBody({ name: "Temp" }),
	]);
	assert.deepEqual(
		run3.emitted,
		["Temp"],
		"re-created mailbox re-emits after prune",
	);
});
...
test("labels: connector fingerprint (excludes id) == compaction fingerprint over stored body", () => {
	...
	assert.equal(
		connectorFp,
		compactionFp,
		"connector and compaction fingerprints must match byte-for-byte",
	);
});

test("readPriorLabelFingerprints: tolerates missing / legacy / malformed state", () => {
	assert.equal(
		readPriorLabelFingerprints({}).size,
		0,
	...
	assert.equal(
		readPriorLabelFingerprints({ labels: { fingerprints: "not-an-object" } })
			.size,
		0,
	...
	const ok = readPriorLabelFingerprints({
		labels: { fingerprints: { INBOX: "fp-1", bad: 5 } },
	});
