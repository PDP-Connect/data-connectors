...
test("the production wildcard type is replaced, not forwarded", () => {
	...
	assert.equal(
		normalizeAttachmentMimeType("image/*", null),
...
test("a valid type is preserved exactly, including its parameters", () => {
	...
	assert.equal(
		normalizeAttachmentMimeType("text/plain; charset=utf-8", null),
		"text/plain; charset=utf-8",
	);
	assert.equal(
		normalizeAttachmentMimeType("application/vnd.ms-excel", null),
...
test("preference falls to the next candidate only when the earlier one is invalid", () => {
	...
	assert.equal(
		normalizeAttachmentMimeType("image/*", "image/png"),
		"image/png",
	);
	assert.equal(normalizeAttachmentMimeType(null, "image/png"), "image/png");
	assert.equal(
		normalizeAttachmentMimeType(undefined, undefined),
		"application/octet-stream",
	);
	...
	assert.equal(
		normalizeAttachmentMimeType("image/gif", "image/png"),
		"image/gif",
	);
});
