...
test("isImapTransientError: non-Error objects with retryable message", () => {
	...
	assert.ok(
		isImapTransientError(retryableString),
		"string with ETIMEDOUT must be transient",
	);
});
...
test("isImapTransientError: non-Error objects with non-retryable message", () => {
	...
	assert.ok(
		!isImapTransientError(nonRetryableString),
		"string without transient pattern must NOT be transient",
	);
});
...
test("MUTATION: removing TIMEOUT check would break CONNECT_TIMEOUT classification", () => {
	...
	(err as Error & { code?: string }).code = "CONNECT_TIMEOUT";
	assert.ok(
		isImapTransientError(err),
		"CONNECT_TIMEOUT code must be transient regardless of message wording",
	);
	...
	const messageOnly = /ECONN|ETIMEDOUT|fetch failed|EPIPE|timeout/i.test(
		err.message,
	);
	assert.equal(
		messageOnly,
		false,
		"message pattern alone does NOT match 'in required time'",
	);
