...
test("bigintToCursor: safe-range bigint → number", () => {
	...
	assert.equal(
		bigintToCursor(BigInt(Number.MAX_SAFE_INTEGER)),
		Number.MAX_SAFE_INTEGER,
	);
});
...
test("toFlagsArray: Set becomes Array", () => {
	assert.deepEqual(toFlagsArray(new Set(["\\Seen", "\\Flagged"])), [
		"\\Seen",
		"\\Flagged",
	]);
});
...
test("toLabelsArray: Set or Array pass through", () => {
	assert.deepEqual(toLabelsArray(new Set(["\\Important", "Work"])), [
		"\\Important",
		"Work",
	]);
	assert.deepEqual(toLabelsArray(["\\Important", "Work"]), [
		"\\Important",
		"Work",
	]);
});
...
function readBodystructureFixture(): MessageStructureObject {
	return readJsonFixture<MessageStructureObject>(
		"bodystructure-multipart.json",
	);
}
...
test("decodeBodystructureForAttachments: extracts both inline and attachment leaves from fixture", () => {
	...
	const items = decodeBodystructureForAttachments(
		tree,
		"msg-1",
...
test("decodeBodystructureForAttachments: drops body alternatives (inline text without filename)", () => {
	...
	assert.equal(
		items.length,
		0,
		"body alternatives must not be classified as attachments",
	);
});
...
test("decodeBodystructureForAttachments: keeps inline non-text leaves with Content-ID (cid: images)", () => {
	...
	assert.equal(
		items.length,
		1,
		"cid-referenced inline images are real attachments",
	);
	const [item] = items;
...
test("decodeBodyPart: plain utf8 passthrough", () => {
	assert.equal(
		decodeBodyPart(Buffer.from("hello world", "utf8"), null, "utf8"),
		"hello world",
	);
});
...
test("decodeBodyPart: base64 → utf8", () => {
	...
	assert.equal(
		decodeBodyPart(Buffer.from(b64, "ascii"), "base64", "utf8"),
		raw,
	);
});
...
test("parseReferencesHeader: header folding (CRLF + whitespace) unfolds correctly", () => {
	const hdr =
		"References: <first@x>\r\n <second@y>\r\n\t<third@z>\r\nSubject: ok\r\n";
	assert.deepEqual(parseReferencesHeader(hdr), [
		"<first@x>",
		"<second@y>",
		"<third@z>",
	]);
...
test("makeSnippet: drops quoted-reply lines and collapses whitespace", () => {
	...
	assert.equal(
		makeSnippet(input, null, "utf8"),
		"Real reply text Another real line",
	);
});
...
test("makeSnippet: strips BEL/ESC/DEL that whitespace-collapse misses → schema-valid", () => {
	...
	assert.ok(
		snip,
		"snippet should not collapse to null when surrounding text is meaningful",
	);
...
test("makeSnippet: strips C1 controls (U+0080–U+009F) not covered by legacy regex → schema-valid", () => {
	const body = Buffer.from(
		`Order${String.fromCharCode(0x85)}confirmed${String.fromCharCode(0x9f)} today`,
		"utf8",
	);
	const snip = makeSnippet(body, null, "utf8");
...
test("makeSnippet: ordinary snippet is unchanged by sanitization", () => {
	const body = Buffer.from(
		"Your order has shipped — track it at the link below.",
		"utf8",
	);
	const snip = makeSnippet(body, null, "utf8");
...
test("updateThreadAggregate: seeds fresh aggregate on first message", () => {
	const agg = updateThreadAggregate(
		undefined,
		makeParams({ participants: ["a@x", "b@y"] }),
	);
...
test("updateThreadAggregate: subsequent messages expand participants + update dates", () => {
	...
	agg = updateThreadAggregate(
		agg,
		makeParams({
			participants: ["a@x", "c@z"],
			receivedAt: "2024-01-10T00:00:00.000Z",
		}),
	);
...
test("updateThreadAggregate: seen → not unread; flagged increments flagged_count", () => {
	let agg = updateThreadAggregate(
		undefined,
		makeParams({ flagsArr: ["\\Seen"], subject: "s", threadId: "t1" }),
	);
...
test("updateThreadAggregate: any message with attachments turns has_attachments true", () => {
	let agg = updateThreadAggregate(
		undefined,
		makeParams({ hasAttachments: false }),
	);
...
test("updateThreadAggregate: labels accumulated into set", () => {
	let agg = updateThreadAggregate(
		undefined,
		makeParams({ labels: ["INBOX", "Work"] }),
	);
	agg = updateThreadAggregate(
		agg,
		makeParams({ labels: ["Work", "Personal"] }),
	);
...
test("buildMessageBodyRecord: text_plain path populates full fields + bytes", () => {
	...
	assert.equal(
		rec.body_text_bytes,
		Buffer.byteLength("plain body text", "utf8"),
	);
	assert.equal(
		rec.body_html_bytes,
		Buffer.byteLength("<p>html body</p>", "utf8"),
	);
...
test("buildMessageBodyRecord: control-rich canonical fields become null without silent loss", () => {
	...
	const candidates = canonicalBodyBlobCandidates({
		bodyTextFull: bodyText,
		bodyHtmlFull: bodyHtml,
	});
	...
	if (validation.ok) {
		assert.equal(
			validation.data.message_received_at,
...
test("isInTimeRange: only-since / only-until", () => {
	...
	assert.equal(
		isInTimeRange("2023-12-15T00:00:00.000Z", {
			since: "2024-01-01T00:00:00.000Z",
		}),
		false,
	);
	...
	assert.equal(
		isInTimeRange("2024-03-15T00:00:00.000Z", {
			until: "2024-02-01T00:00:00.000Z",
		}),
		false,
	);
...
test("envelopeParticipants: filters missing addresses and handles undefined envelope", () => {
	...
	assert.deepEqual(
		envelopeParticipants({ from: [{ name: "X", address: "" }] }),
		[],
	);
});
