	...
	if (deps.wantBodies) {
		...
		for (const candidate of bodyBlobCandidates) {
			await deps.uploadBodyBlob?.({
				...
				connectorId:
					process.env.PDPP_CONNECTOR_ID || DEFAULT_GMAIL_CONNECTOR_ID,
...
export async function fetchAttachmentPart(
	...
	return {
		...
		expectedSize:
			typeof response.meta?.expectedSize === "number"
				? response.meta.expectedSize
				: attachment.size_bytes,
		mimeType:
...
export async function runAllMailPasses(
	...
	const historicalTargetUid =
		session.messagesBackfill.target_uid ??
		Math.max(0, (session.uidnext ?? session.priorUidnext) - 1);
