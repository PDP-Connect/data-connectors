 ...
 */
function withStravaPages<T>(
	pages: StravaActivityFixture[][],
...
const EPOCH_2026_08_01 = Math.floor(
	new Date("2026-08-01T00:00:00Z").getTime() / 1000,
);
const EPOCH_2026_08_02 = Math.floor(
	new Date("2026-08-02T00:00:00Z").getTime() / 1000,
);
...
test("a walk that exhausts naturally advances the cursor and discloses no truncation", async () => {
	...
	await withStravaPages(
		[
			fullPage(1, "2026-08-01T00:00:00Z"),
			[{ id: 9001, start_date: "2026-08-02T00:00:00Z" }],
		],
	...
	assert.equal(
		records.length,
		PAGE_SIZE + 1,
		"every activity on every fetched page is emitted",
	);
	assert.equal(
		truncationSkip(messages),
		undefined,
		"an exhausted walk must not claim truncation",
	);
	assert.deepEqual(
...
test("an empty first page exhausts the walk without advancing past a prior cursor", async () => {
	...
	assert.equal(
		truncationSkip(messages),
		undefined,
		"an empty page is exhaustion, not truncation",
	);
	assert.deepEqual(
		lastStateCursor(messages, "activities"),
...
test("a capped walk is distinguishable from an exhausted one and holds the cursor at unread history", async () => {
	...
	await withStravaPages(
		[
			fullPage(1, "2026-08-02T00:00:00Z"),
			fullPage(101, "2026-08-03T00:00:00Z"),
		],
	...
	assert.equal(
		records.length,
		PAGE_SIZE,
		"the enumerated prefix may still be emitted",
	);
...
test("a capped walk with no prior cursor holds at zero rather than skipping unread history", async () => {
	...
	assert.ok(
		truncationSkip(messages),
		"a truncated first-ever walk still discloses truncation",
	);
	assert.deepEqual(
		lastStateCursor(messages, "activities"),
		{ last_start_epoch: 0 },
...
test("the page ceiling is what stops the walk — it does not fetch past its budget", async () => {
	...
	const requested = await withStravaPages(
		[
			fullPage(1, "2026-08-02T00:00:00Z"),
			fullPage(101, "2026-08-03T00:00:00Z"),
			fullPage(201, "2026-08-04T00:00:00Z"),
		],
		async (urls) => {
			await collectStrava(ctx, {
				httpGovernor: unpacedGovernor(),
				maxPages: 2,
			});
			return urls;
		},
