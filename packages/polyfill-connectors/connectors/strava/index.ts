...
async function fetchActivitiesPage(
	...
	try {
		const r = await governor.request<
	...
	if (raw.status < 200 || raw.status >= 300) {
		throw new Error(
			`strava_http_${String(raw.status)}: ${raw.body.slice(0, 200)}`,
		);
	}
...
export async function collectStrava(
	ctx: CollectContext,
	...
	const progressWithSignals = progress as (
		message: string,
		extra?: ProgressExtra,
	) => Promise<void>;
	...
	await progressWithSignals("Fetching activities", {
		stream: "activities",
		phase: "start",
	});
	...
	const walk = await walkPagesWithCeiling({
		...
		fetchPage: async (pageNumber) => {
			...
			const acts = await fetchActivitiesPage(
				governor,
				token,
				pageNumber,
				lastEpoch,
