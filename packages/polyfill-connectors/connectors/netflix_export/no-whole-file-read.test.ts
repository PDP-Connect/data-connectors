...
test("extractViewingActivityArtifactFromFile rejects a multi-GB-declared zip entry without ever reading its (sparse) data region", () => {
	...
	try {
		...
		assert.equal(
			result.ok,
			false,
			"a multi-GB-declared entry must be rejected, not read",
		);
		if (!result.ok) {
...
test("boundary: extractViewingActivityArtifactFromFile's CSV size check — exactly MAX_CSV_BYTES succeeds, +1 byte fails", () => {
	...
	try {
		...
		try {
			...
			const atCapResult = extractViewingActivityArtifactFromFile(
				atCapFd,
				MAX_CSV_BYTES,
				"at-cap.csv",
			);
			assert.equal(
				atCapResult.ok,
				true,
				"a CSV exactly at MAX_CSV_BYTES must be accepted",
			);
		} finally {
			closeSync(atCapFd);
		}
		...
		try {
			...
			const before = process.memoryUsage();
			const overCapResult = extractViewingActivityArtifactFromFile(
				overCapFd,
				MAX_CSV_BYTES + 1,
				"over-cap.csv",
			);
			...
			assert.equal(
				overCapResult.ok,
				false,
				"a CSV at MAX_CSV_BYTES + 1 must be rejected",
			);
			if (!overCapResult.ok) {
