 ...
 */
function extractUploadedArtifactFromFile(
	...
	try {
		...
		return {
			ok: false as const,
			code: "unsupported_shape" as const,
			message: "The uploaded file could not be read.",
		};
	}
	try {
		const fileSize = statSync(filePath).size;
		return extractViewingActivityArtifactFromFile(
			fd,
			fileSize,
			uploadedFileName,
		);
...
function loadUploadedArtifactRows(
	canonicalImportDir: string,
	uploadedFileName: string,
): LoadedRows | LoadRowsSkip {
	const artifact = extractUploadedArtifactFromFile(
		canonicalImportDir,
		uploadedFileName,
	);
	if (!artifact.ok) {
...
async function loadLegacyDirectoryRows(
	canonicalImportDir: string,
...
function isLoadRowsSkip(
	result: LoadedRows | LoadRowsSkip,
): result is LoadRowsSkip {
 ...
 */
function hasAmbiguousDirectHistoryDates(
	rows: Record<string, string | undefined>[],
): boolean {
...
function isDateOrderSkip(
	result: { order: DirectHistoryDateOrder | null } | LoadRowsSkip,
): result is LoadRowsSkip {
	return "reason" in result;
}
...
runConnector({
	...
	async collect(ctx) {
		const importDir =
			process.env.NETFLIX_EXPORT_DIR ||
		...
		if (ctx.requested.has("viewing_activity")) {
			await collectViewingActivity(
				ctx,
				importDir,
				typedState?.viewing_activity,
			);
		}
	},
});
