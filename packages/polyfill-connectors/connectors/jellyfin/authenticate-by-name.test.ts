	...
	private route(
		...
		if (path === "/Users/AuthenticateByName") {
			...
			if (path.includes("api_key=") || path.includes("ApiKey=")) {
				res.writeHead(400);
				res.end(
					JSON.stringify({ error: "credential must not be in query params" }),
				);
				return;
			}
			...
			if (
				parsedBody.Username !== "alice" ||
				parsedBody.Pw !== "correct-password"
			) {
				res.writeHead(401);
...
test("e2e: username+password (primary path) authenticates via AuthenticateByName and derives userId from the response", async () => {
	...
	try {
		...
		assert.equal(
			...
			1,
			"should collect using the userId derived from AuthenticateByName",
		);

		const authRequest = server
			.getRequestLog()
			.find((r) => r.path === "/Users/AuthenticateByName");
		...
		for (const req of server.getRequestLog()) {
			assert(
				!req.path.includes("api_key="),
				`request must not carry api_key in query: ${req.path}`,
			);
			assert(
				!req.path.includes("ApiKey="),
				`request must not carry ApiKey in query: ${req.path}`,
			);
			assert(
				!req.path.includes("correct-password"),
				`request path must never expose the password: ${req.path}`,
			);
		}

		// Downstream requests use the returned AccessToken via X-Emby-Token, not
		// the raw password.
		const viewsRequest = server
			.getRequestLog()
			.find((r) => r.path === "/Users/user-alice-1/Views");
		assert.ok(
			viewsRequest,
			"must fetch views for the userId returned by AuthenticateByName",
		);
		assert.equal(
			viewsRequest.headers["x-emby-token"],
...
test("e2e: AuthenticateByName response missing AccessToken fails the run rather than proceeding without a credential", async () => {
	...
	try {
		...
		await assert.rejects(
			() => collect(ctx),
			/jellyfin_authenticate_by_name_no_access_token/,
		);
	} finally {
		await server.stop();
	}
...
test("e2e: AuthenticateByName response missing User.Id fails the run rather than fabricating an identity", async () => {
	...
	try {
		...
		await assert.rejects(
			() => collect(ctx),
			/jellyfin_authenticate_by_name_no_user_id/,
		);
	} finally {
		await server.stop();
	}
...
test("primary path takes precedence: username+password present alongside an api_key still authenticates via AuthenticateByName", async () => {
	...
	try {
		...
		assert.ok(
			authRequest,
			"primary path must be used even when an api_key is also present",
		);
...
test("only one of username/password present is treated as the primary path being absent (never a partial credential)", async () => {
	...
	try {
		...
		const authRequests = server
			.getRequestLog()
			.filter((r) => r.path === "/Users/AuthenticateByName");
		assert.equal(
			authRequests.length,
			0,
			"must not call AuthenticateByName with only half the credential",
		);
	} finally {
		await server.stop();
	}
