	...
	private route(
		...
		if (path.includes("/Users/user-alice-1/Items")) {
			...
			const items = Array.from(
				{ length: Math.min(this.itemsPerPage, totalCount - startIndex) },
				(_, i) => ({
	...
	try {
		...
		const itemsCoverage = messages.find(
			(m) =>
				...
				(m as { stream?: string }).stream === "items",
		);
		assert.equal(
			itemsCoverage,
			undefined,
			"a budget-exhausted run must not emit items coverage (cannot read green)",
		);

		const itemsSkip = messages.find(
			(m) =>
				m.type === "SKIP_RESULT" &&
				(m as { stream?: string }).stream === "items",
		);
		assert.notEqual(
			itemsSkip,
			undefined,
			"a budget-exhausted run must emit an honest SKIP_RESULT for items",
		);
	} finally {
		await server.stop();
	}
...
test("counterweight: a run with exactly one revocation (not repeated) still self-heals and completes with full coverage", async () => {
	...
	class FakeSingleRevocationServer {
		...
		private route(
			...
			if (path.includes("/Users/user-alice-1/Items")) {
				const presented = headers["x-emby-token"] ?? "";
				const shouldReject =
					presented !== this.currentToken ||
					(!this.revokedOnce && this.servedItemsPages === 0);
				if (
					!this.revokedOnce &&
					presented === this.currentToken &&
					this.servedItemsPages === 0
				) {
				...
				const totalCount = 5;
				const items = Array.from(
					{ length: Math.min(500, totalCount - startIndex) },
					(_, i) => ({
	...
	try {
		...
		assert.equal(
			itemRecords.length,
			5,
			"the single stale-token blip must self-heal and still collect every item",
		);

		assert.equal(
			server.authCalls.length,
			2,
			"sign-in plus exactly one self-heal login, no more",
		);
		...
		assert.notEqual(
			itemsCoverage,
			undefined,
			"a fully-recovered run must still emit items coverage (reads green)",
		);
...
test("B5: a mid-run reauth attempt that itself throws (server 500s on AuthenticateByName) still terminates with jellyfin_auth_failed, not a generic error", async () => {
	...
	try {
		...
		assert.notEqual(
			itemsSkip,
			undefined,
			"a failed reauth must still emit an honest SKIP_RESULT for items",
		);
		assert.equal(
			itemsSkip?.reason,
...
test("B5 counterweight: repeated rejection (reauth never throws, just keeps failing the session) still classifies as jellyfin_auth_failed", async () => {
	...
	try {
		...
		assert.equal(
			server.authCalls.length,
			2,
			"sign-in plus exactly one self-heal attempt before giving up",
		);
