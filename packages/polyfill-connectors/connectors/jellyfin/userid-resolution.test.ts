	...
	const urlPromise = new Promise<string>((resolve, reject) => {
		server = createServer((req: IncomingMessage, res: ServerResponse) => {
			...
			if (viewsMatch) {
				...
				res.end(
					JSON.stringify({
						Items: [{ Id: `lib-for-${viewsMatch[1]}`, Name: "Lib" }],
					}),
				);
				return;
			}
...
test("regression: Users 401 must fail the run with the real auth error, not a placeholder user id", async () => {
	...
	try {
		...
		assert.ok(
			threwError,
			"Must fail the run when Users fails, not silently substitute a fallback user id",
		);
		assert.ok(
...
test("regression: Users 400 with a body must fail without persisting that body", async () => {
	...
	const server = await startServer((res) => {
		...
		res.end(
			JSON.stringify({
				Message: "provider secret-api-key: Error processing request for Users",
			}),
		);
	});
	const baseUrl = await server.url;
	...
	try {
		...
		assert.doesNotMatch(
			errorMsg,
			/secret-api-key|Error processing request for Users/,
		);
	} finally {
		await server.stop();
	}
...
test("regression: Users success with no users must fail the run, not substitute a placeholder user id", async () => {
	...
	try {
		...
		assert.ok(
			threwError,
			"Must fail the run when Users returns no users, not fabricate one",
		);
		assert.ok(
			errorMsg.includes("jellyfin_no_users"),
			`Error must name the no-users condition, got: ${errorMsg}`,
		);
	} finally {
		await server.stop();
	}
...
test("regression: Users success with more than one user must fail the run, not guess which one to collect as", async () => {
	...
	try {
		...
		assert.ok(
			threwError,
			"Must fail the run when multiple users exist, not guess one",
		);
		assert.ok(
...
test("regression: Users success with a user entry missing Id must fail the run, not substitute a placeholder user id", async () => {
	...
	try {
		...
		assert.ok(
			threwError,
			"Must fail the run when the sole user has no Id, not fabricate one",
		);
		assert.ok(
...
test("regression: Users success with a non-array body must fail the run, not substitute a placeholder user id", async () => {
	...
	try {
		...
		assert.ok(
			threwError,
			"Must fail the run when Users response is not an array",
		);
		assert.ok(
...
test("regression: successful single-user Users response must still work end-to-end", async () => {
	...
	try {
		...
		assert.equal(
			...
			1,
			"Should fetch libraries using the real resolved user id",
		);
	} finally {
...
test("resolution: owner-supplied user id (matched against Users by Id) is used even with multiple users on the server", async () => {
	...
	try {
		...
		assert.equal(
			...
			"lib-for-user-b",
			"Must collect as the owner-specified user id",
		);
	} finally {
		await server.stop();
	}
...
test("resolution: owner-supplied username (matched against Users by Name, case-insensitive) is used", async () => {
	...
	try {
		...
		assert.equal(
			libraryRecords[0]?.data.id,
			"lib-for-user-b",
			"Must resolve the username to user-b's id",
		);
	} finally {
		await server.stop();
	}
	...
	try {
		...
		const { ctx } = makeContext({
			credentials: {
				base_url: baseUrl,
				secret: "test-key",
				jellyfin_user_id: "nonexistent-user",
			},
			streams: [{ name: "libraries" }],
		});
		...
		assert.ok(
			threwError,
			"Must fail when the configured user id/username matches no user",
		);
		assert.ok(
...
test("resolution: Users/Me returning a concrete user is used when no owner-supplied id is set", async () => {
	...
	try {
		...
		assert.equal(
			libraryRecords[0]?.data.id,
			"lib-for-user-a",
			"Must collect as the Users/Me-resolved user",
		);
	} finally {
		await server.stop();
	}
...
test("resolution: ambiguous-user error names a bounded sample of usernames and the setup field, never user ids", async () => {
	const manyUsers = Array.from({ length: 28 }, (_, i) => ({
		Id: `user-id-${i}`,
		Name: `User${i}`,
	}));
	const server = await startResolutionServer({
	...
	try {
		...
		assert.ok(
			threwError,
			"Must fail with 28 users and no owner-supplied disambiguation",
		);
		assert.ok(errorMsg.includes("jellyfin_ambiguous_user"));
		assert.ok(
			errorMsg.includes("28 users"),
			`Error must state the count, got: ${errorMsg}`,
		);
		assert.ok(
		...
		assert.ok(
			errorMsg.includes("User0"),
			`Error must include a sample username, got: ${errorMsg}`,
		);
		// Bounded sample, not the full 28-user roster.
		assert.ok(
			!errorMsg.includes("User27"),
			`Error must not dump the full user roster, got: ${errorMsg}`,
		);
		// Never leak user ids in the ambiguous-user error — only display names and a count.
		assert.ok(
			!errorMsg.includes("user-id-0"),
			`Error must not include user ids, got: ${errorMsg}`,
		);
	} finally {
		await server.stop();
	}
...
test("resolution: confirm the exactly-one-user guard is load-bearing — Users/Me disabled AND no owner id must still fail on 2+ users", async () => {
	...
	try {
		...
		assert.ok(
			threwError,
			"A 2-user server with no disambiguation must never silently pick one",
		);
		assert.ok(errorMsg.includes("jellyfin_ambiguous_user"));
	} finally {
		await server.stop();
