			...
			this.server = createServer(
				(req: IncomingMessage, res: ServerResponse) => {
					...
					if (path.includes("api_key=")) {
						res.writeHead(400);
						res.end(
							JSON.stringify({ error: "API key must not be in query params" }),
						);
						return;
					}
					...
					if (path.includes("/Users/test-user-123/Items")) {
						...
						if (this.responseMode === "missing_total") {
							...
							res.end(
								JSON.stringify({ Items: [{ Id: "item-1", Name: "Item 1" }] }),
							);
							return;
						}
						...
						if (this.responseMode === "decreasing_total") {
							...
							res.writeHead(200, { "Content-Type": "application/json" });
							res.end(
								JSON.stringify({ Items: items, TotalRecordCount: total }),
							);
							return;
						}
						...
						if (this.responseMode === "repeated_page") {
							...
							res.writeHead(200, { "Content-Type": "application/json" });
							res.end(
								JSON.stringify({ Items: items, TotalRecordCount: 999_999_999 }),
							); // Claims 1 billion
							return;
						}
	...
	try {
		...
		for (const req of requests) {
			assert(
				...
				`Request should not contain api_key query param: ${req.path}`,
			);
			assert(
				!req.path.includes("test-secret-key"),
				`Request should not expose secret: ${req.path}`,
			);
		}
		...
		for (const req of requests) {
			assert(
				...
				`Request path must not be prefixed with /api/: ${req.path}`,
			);
		}
		...
		assert.ok(
			authHeader,
			"At least one request should have X-Emby-Token header",
		);
		assert.equal(
			authHeader.headers["x-emby-token"],
			"test-secret-key-12345",
			"Header should contain correct secret",
		);
	} finally {
		await server.stop();
	}
...
test("e2e: items coverage declares the server-reported TotalRecordCount as the denominator", async () => {
	...
	try {
		...
		assert.equal(
			coverage.covered,
			records.filter((r) => r.stream === "items").length,
		);
...
test("e2e: libraries coverage declares enumerated count as full-scan denominator", async () => {
	...
	try {
		...
		assert.ok(
			records.filter((r) => r.stream === "libraries").length > 0,
			"Should emit at least 1 library",
		);
	} finally {
		await server.stop();
	}
...
test("e2e: libraries coverage with zero enumerated libraries", async () => {
	...
	try {
		...
		assert.ok(
			coverage,
			"expected libraries DETAIL_COVERAGE for empty enumeration",
		);
		...
		assert.equal(
			records.filter((r) => r.stream === "libraries").length,
			0,
			"Should emit no libraries",
		);
	} finally {
...
test("e2e: SSRF protection via origin constraint", async () => {
	...
	try {
		...
		assert.ok(
			records.length > 0,
			"Should successfully fetch with origin-constrained requests",
		);
	} finally {
		await server.stop();
	}
...
test("e2e: subpath-hosted server (base URL with a path segment) is reachable", async () => {
	...
	const server = createServer((req: IncomingMessage, res: ServerResponse) => {
		...
		if (subpath === "/System/Info") {
			res.writeHead(200);
			res.end(
				JSON.stringify({ Id: "test", ServerName: "Test", Version: "10.11.11" }),
			);
			return;
		}
		if (subpath === "/Users") {
	...
	try {
		...
		assert.equal(
			...
			1,
			"Should reach the subpath-hosted server and emit 1 library",
		);
	} finally {
