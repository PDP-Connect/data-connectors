...
function startPagedServer(
	pageForStartIndex: (startIndex: number) => {
...
test("regression: two distinct full pages of equal size must succeed (normal pagination)", async () => {
	...
	try {
		...
		assert.ok(
			!threwError,
			`Should succeed with two distinct equal-sized pages, got error: ${errorMsg}`,
		);
...
test("regression: distinct pages that happen to share a first item ID must still succeed", async () => {
	...
	try {
		...
		assert.equal(
			itemRecords.length,
			1000,
			`Should emit all 1000 items, got ${itemRecords.length}`,
		);
	} finally {
		await server.stop();
	}
