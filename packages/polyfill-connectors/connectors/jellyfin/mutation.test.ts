				...
				(req: IncomingMessage, res: ServerResponse) => {
					...
					if (path.includes("/Users/user-123/Items")) {
						...
						if (this.responseMode === "oversized_no_length") {
							...
							const payload = {
								Items: new Array(100_000).fill(item),
								TotalRecordCount: 0,
							};
						...
						if (this.responseMode === "repeated_page") {
							...
							res.writeHead(200);
							res.end(
								JSON.stringify({ Items: items, TotalRecordCount: 50_000 }),
							);
							return;
						}
	...
	try {
		...
		assert.ok(
			errorMsg.includes("streaming") ||
				...
				errorMsg.includes("retry budget"),
...
test("mutation: repeated-page detection catches non-advancing pagination", async () => {
	...
	try {
		...
		assert.ok(
			threwError,
			"Repeated-page guard must reject non-advancing pagination",
		);
		assert.ok(
			errorMsg.includes("non_advancing"),
			`Expected non-advancing error, got: ${errorMsg}`,
		);
	} finally {
		await server.stop();
	}
...
test("mutation: max-pages guard is testable with injected config", async () => {
				...
				this.server = createServer(
					(req: IncomingMessage, res: ServerResponse) => {
						...
						if (path === "/Users/user-123/Views") {
							res.writeHead(200);
							res.end(
								JSON.stringify({ Items: [{ Id: "lib1", Name: "Lib1" }] }),
							);
							return;
						}
						...
						if (path.includes("/Users/user-123/Items")) {
							...
							res.writeHead(200);
							res.end(
								JSON.stringify({ Items: [], TotalRecordCount: 1_000_000 }),
							);
							return;
						}
	...
	try {
		...
		assert.ok(
			errorMsg.includes("max_pages"),
			`Expected max-pages error, got: ${errorMsg}`,
		);
	} finally {
...
test("mutation: a 200 Views envelope without Items fails before library state or coverage", async () => {
	...
	try {
		...
		assert.equal(
			messages.some(
				(message) => message.type === "STATE" && message.stream === "libraries",
			),
			false,
...
test("mutation: a 200 Items envelope without Items fails before item state or coverage", async () => {
	...
	try {
		...
		assert.equal(
			messages.some(
				(message) => message.type === "STATE" && message.stream === "items",
			),
			false,
