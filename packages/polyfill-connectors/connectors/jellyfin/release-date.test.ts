...
test("normalizeReleaseDate extracts the date portion from Jellyfin's full round-trip datetime shape (the live UAT shape)", () => {
	assert.equal(
		normalizeReleaseDate("1994-09-23T00:00:00.0000000Z"),
		"1994-09-23",
	);
	assert.equal(
		normalizeReleaseDate("2021-01-25T06:00:00.0000000Z"),
		"2021-01-25",
	);
});
...
test("normalizeReleaseDate accepts a real leap day and rejects Feb 29 in a non-leap year (calendar-real, not just regex-shaped)", () => {
	assert.equal(
		normalizeReleaseDate("2020-02-29"),
		"2020-02-29",
		"2020 is a leap year — Feb 29 is real",
	);
	assert.equal(
		normalizeReleaseDate("2021-02-29"),
		null,
		"2021 is not a leap year — Feb 29 does not exist",
	);
	assert.equal(
		normalizeReleaseDate("2020-02-29T00:00:00.0000000Z"),
		"2020-02-29",
	);
...
test("normalizeReleaseDate rejects structurally-shaped but impossible month/day components instead of silently truncating to the leading digits", () => {
	assert.equal(
		normalizeReleaseDate("2021-99-99T00:00:00.0000000Z"),
		null,
		"month 99 / day 99 do not exist",
	);
	assert.equal(
		normalizeReleaseDate("2021-02-30"),
		null,
		"February never has a 30th",
	);
	assert.equal(
		normalizeReleaseDate("2021-13-01"),
		null,
		"month 13 does not exist",
	);
	assert.equal(
		normalizeReleaseDate("2021-00-01"),
		null,
		"month 00 does not exist",
	);
	assert.equal(
		normalizeReleaseDate("2021-01-00"),
		null,
		"day 00 does not exist",
	);
	assert.equal(
		normalizeReleaseDate("2021-04-31"),
		null,
...
test("normalizeReleaseDate does not timezone-shift a real calendar date across the UTC day boundary", () => {
	...
	assert.equal(
		normalizeReleaseDate("1999-12-31T23:59:59.9999999Z"),
		"1999-12-31",
	);
});
...
test("fail-before: the raw live-UAT PremiereDate shape fails the items schema's release_date regex directly (proves the defect without normalization)", () => {
	...
	const result = itemsSchema.safeParse(
		baseItem({ release_date: rawShapeFromProvider }),
	);
	assert.equal(
		result.success,
		false,
		"raw provider datetime shape must fail the bare-date schema pre-normalization",
	);
});
...
test("an unparseable release_date degrades to null and does not discard the rest of an otherwise-valid item record", () => {
	...
	assert.equal(
		result.success,
		true,
		"a null release_date must not fail the record — release_date is nullable",
	);
	if (result.success) {
...
test("fail-before (red-team correction): a naive leading-digits extraction would have produced an invalid calendar date and an unterminated garbage suffix as valid-looking release_date values", () => {
	const naiveLeadingDigitsExtraction = (raw: string): string | null =>
		raw.match(/^(\d{4}-\d{2}-\d{2})/)?.[1] ?? null;
	assert.equal(
