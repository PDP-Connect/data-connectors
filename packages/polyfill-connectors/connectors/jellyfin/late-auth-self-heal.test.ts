...
class FakeJellyfinLateAuthServer {
	 ...
	 */
	constructor(
		revokeAfterPages: number,
		alwaysReject = false,
		sameTokenOnReauth = false,
	) {
	...
	private route(
		...
		if (path.includes("/Users/user-alice-1/Items")) {
			...
			const items = Array.from(
				{ length: Math.min(pageSize, totalCount - startIndex) },
				(_, i) => ({
...
test("fake server sanity check: an unrecognized/stale token 401s on the items endpoint (does not exercise the connector)", async () => {
	...
	try {
		const res = await fetch(
			`${baseUrl}/Users/user-alice-1/Items?StartIndex=2&Limit=2`,
			{
		...
		assert.equal(
			res.status,
			401,
			"the fake server reproduces a 401 on an unrecognized/stale token",
		);
	} finally {
		await server.stop();
	}
...
test("pass-after: a mid-run 401 on a proven-valid token self-heals via one AuthenticateByName retry and the run completes with all records", async () => {
	...
	try {
		...
		assert.equal(
			itemRecords.length,
			600,
			"all 600 items across both pages must be collected despite the mid-run 401",
		);

		assert.equal(
			server.authCalls.length,
			2,
			"must re-authenticate exactly once after the mid-run 401",
		);
		assert.notEqual(
			server.authCalls[0],
			server.authCalls[1],
...
test("a 401 that persists after re-authentication (token genuinely rejected) still fails the run honestly", async () => {
	...
	try {
		...
		assert.equal(
			server.authCalls.length,
			2,
			"must still attempt exactly one re-authentication before giving up",
		);
	} finally {
		await server.stop();
	}
...
test("reauth that hands back the identical (still-revoked) token does not retry the request, and fails after exactly one auth call", async () => {
	...
	try {
		...
		assert.equal(
			server.authCalls[0],
			server.authCalls[1],
			"sanity: the fake server actually returned the same token",
		);
