...
function monthCategoryFixture(
	overrides: Partial<MonthCategoryInput> = {},
): MonthCategoryInput {
	return {
		activity: -42_000,
		balance: 18_000,
...
test("monthCategoryRecord: uses stable budget/month/category primary key", () => {
	const record = monthCategoryRecord(
		monthCategoryFixture(),
		"2026-03-01",
		"budget-main",
	);
...
test("collectMonthCategories: applies range/cutoff gates, emits records, and stores rewound cursor", async () => {
	...
	assert.deepEqual(
		...
		[
			"budget-main:2026-02-01:cat-2026-02-01",
			"budget-main:2026-04-01:cat-2026-04-01",
		],
	);
...
test("collectMonthCategories: empty-delta reconciliation keeps a 24-month detail walk bounded by lastFetchedMonth", async () => {
	...
	assert.equal(
		reconciledDetailCalls,
		13,
		"reconciliation walks only December and the rewound current window",
	);
	assert.deepEqual(reconciliationFact, {
		considered: 0,
		covered: 0,
...
test("collectMonthCategories: omitted nested categories fail before advancing the month checkpoint", async () => {
	...
	await assert.rejects(
		() =>
			collectMonthCategories(ctx, [malformedMonth], {}, () =>
				Promise.resolve(malformedMonth),
			),
	...
	assert.equal(
		messages.some(
			(message) =>
				...
				message.stream === "month_categories",
		),
		false,
...
test("collectMonthCategories: progress omits budget ids and month values", async () => {
	...
	assert.equal(
		serialized.includes("budget-main"),
		false,
		"budget id must not appear in progress",
	);
	assert.equal(
		serialized.includes("2026-04-01"),
		false,
		"month value must not appear in progress",
	);
	assert.equal(
