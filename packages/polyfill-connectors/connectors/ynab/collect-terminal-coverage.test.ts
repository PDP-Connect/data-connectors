...
function fakeRequest(budgets: readonly BudgetFixture[]): {
	...
	const request = <T>(
		...
		requestKnowledge.set(path, [
			...(requestKnowledge.get(path) ?? []),
			options?.knowledge,
		]);

		if (path === "/budgets") {
			return Promise.resolve({
				data: { budgets: budgets.map((b) => budgetSummary(b.id)) },
			} as T);
		}
		...
		if (resource === "accounts") {
			return Promise.resolve({
				data: { accounts: fixture.accounts ?? [], server_knowledge: 100 },
			} as T);
		}
		if (resource === "payee_locations") {
			return Promise.resolve({
				data: { payee_locations: fixture.payeeLocations ?? [] },
			} as T);
		}
		if (resource === "scheduled_transactions") {
			return Promise.resolve({
				data: {
					scheduled_transactions: fixture.scheduledTransactions ?? [],
		...
		if (resource === "categories") {
			return Promise.resolve({
				data: { category_groups: [], server_knowledge: 300 },
			} as T);
		}
		if (resource === "payees") {
			return Promise.resolve({
				data: { payees: [], server_knowledge: 400 },
			} as T);
		}
		if (resource === "transactions") {
			return Promise.resolve({
				data: { transactions: [], server_knowledge: 500 },
			} as T);
		}
		if (resource === "months") {
			return Promise.resolve({
				data: { months: [], server_knowledge: 600 },
			} as T);
		}
		return Promise.reject(
			new Error(`ynab_http_404 [endpoint ${path}]: not_found`),
...
function coherenceFor(
	...
	const envelope: StreamEvidenceEnvelope = {
		...
		...(coverage?.considered === undefined
			? {}
			: { considered: coverage.considered as number }),
		...(coverage?.covered === undefined
			? {}
			: { covered: coverage.covered as number }),
	};
...
test("ynabCollect: multi-budget fresh run proves coverage for every required-by-default stream plus account_stats", async () => {
	const fixtures: BudgetFixture[] = [
		{
			...
			accounts: [
				account(ACCOUNT_1),
				account(ACCOUNT_2, { name: "Savings", type: "savings" }),
			],
	...
	assert.equal(
		accountsCov?.considered,
		3,
		"3 accounts total across both budgets",
	);
	...
	assert.ok(
		statsCov,
		"account_stats declares an explicit disposition, not silence",
	);
	assert.equal(statsCov?.considered, 3);
	assert.equal(
		statsCov?.covered,
		3,
		"singleton_presence: every enumerated account is accounted for",
	);

	const payeeLocCov = coverageFor(messages, "payee_locations");
	assert.ok(payeeLocCov, "payee_locations declares self-coverage");
	assert.equal(
		payeeLocCov?.considered,
		1,
		"only budget A has a payee location",
	);
	...
	for (const [stream, strategy] of [
		...
		assert.equal(
			verdict.proven,
			true,
			`${stream}: coherence evaluator must read this run as proven`,
		);
		assert.equal(
			verdict.reason,
			"enumeration_boundary",
			`${stream}: proof came from a measured boundary`,
		);
	}
});
...
test("ynabCollect: zero-budget run emits no coverage for per-budget streams — a checkpoint alone is not proof", async () => {
	...
	assert.equal(
		coverageFor(messages, "accounts"),
		undefined,
		"no budgets ran → no accounts coverage fabricated",
	);
	...
	assert.equal(
		budgetsVerdict.proven,
		true,
		"budgets: genuine zero is proven-empty",
	);
	...
	for (const [stream, strategy] of [
		...
		assert.equal(
			verdict.proven,
			false,
			`${stream}: zero budgets must not read as proven`,
		);
	}
});
...
test("ynabCollect: established zero streams prove empty only after a fresh provider traversal", async () => {
	...
	const priorState = {
		...
		transactions: {
			[BUDGET_A]: { server_knowledge: 30, since_date: "2026-01-01" },
		},
	...
	for (const stream of streams) {
		...
		assert.ok(
			coverage,
			`${stream}: successful full traversal emits zero proof`,
		);
		assert.equal(
			coverage?.considered,
			0,
			`${stream}: provider returned an empty inventory`,
		);
		assert.equal(
			coverage?.covered,
			0,
			`${stream}: empty inventory is fully covered`,
		);
		assert.equal(
			coherenceFor(messages, stream, "full_inventory").proven,
			true,
			`${stream}: zero is proven`,
		);
	}
	...
	assert.equal(
		knowledgeSeenFor(`/budgets/${BUDGET_A}/transactions`),
		undefined,
	);
	assert.equal(knowledgeSeenFor(`/budgets/${BUDGET_A}/months`), undefined);
	assert.equal(
		knowledgeSeenFor(`/budgets/${BUDGET_A}/scheduled_transactions`),
		undefined,
	);
	assert.deepEqual(requestsFor(`/budgets/${BUDGET_A}/categories`), [
		10,
		undefined,
	]);
	assert.deepEqual(requestsFor(`/budgets/${BUDGET_A}/payees`), [20, undefined]);
	assert.deepEqual(requestsFor(`/budgets/${BUDGET_A}/transactions`), [
		30,
		undefined,
	]);
	assert.deepEqual(requestsFor(`/budgets/${BUDGET_A}/months`), [40, undefined]);
	assert.deepEqual(requestsFor(`/budgets/${BUDGET_A}/scheduled_transactions`), [
		50,
		undefined,
	]);
});
...
test("ynabCollect: a nonempty prior-cursor delta stays incremental", async () => {
	const fixtures: BudgetFixture[] = [
		{ id: BUDGET_A, scheduledTransactions: [scheduledTxn(SCHEDULED_1)] },
	];
	...
	assert.deepEqual(
		requestsFor(`/budgets/${BUDGET_A}/scheduled_transactions`),
		[50],
	);
...
test("ynabCollect: a malformed account/payee_location row leaves covered < considered, never silently aliased to the raw count", async () => {
	const fixtures: BudgetFixture[] = [
		{
			...
			payeeLocations: [
				payeeLocation(PAYEE_LOC_1, PAYEE_1),
				payeeLocation("also-not-a-uuid", PAYEE_1),
			],
			scheduledTransactions: [],
		},
	];
	...
	assert.equal(
		accountsCov?.considered,
		2,
		"both rows were in the API response",
	);
	assert.equal(
		accountsCov?.covered,
		1,
		"only the valid row is covered — never aliased to the raw count of 2",
	);
	...
	const payeeLocVerdict = coherenceFor(
		messages,
		"payee_locations",
		"full_inventory",
	);
...
test("ynabCollect: a run on an established (many-runs-old) connection still proves accounts/account_stats coverage, never sends a knowledge cursor to /accounts", async () => {
	const fixtures: BudgetFixture[] = [
		{ id: BUDGET_A, accounts: [account(ACCOUNT_1)] },
	];
	...
	const stateMsg = first.messages.find(
		(m): m is Extract<EmittedMessage, { type: "STATE" }> =>
			m.type === "STATE" && m.stream === "accounts",
	);
	...
	const second = makeCtx(["accounts", "account_stats"], {
		accounts: stateMsg.cursor as Record<string, unknown>,
	});
	...
	assert.ok(
		secondCov,
		"a run on an established connection still proves accounts coverage",
	);
	assert.equal(secondCov?.considered, 1);
	assert.equal(secondCov?.covered, 1);
	assert.ok(
		coverageFor(second.messages, "account_stats"),
		"account_stats also still proves coverage",
	);

	const accountsVerdict = coherenceFor(
		second.messages,
		"accounts",
		"full_inventory",
	);
	assert.equal(
		accountsVerdict.proven,
		true,
		"an established connection's run must still read as proven",
	);
	...
	const third = makeCtx(["accounts", "account_stats"], {
		accounts: thirdStateMsg.cursor as Record<string, unknown>,
	});
	await ynabCollect(third.ctx, request);
	assert.ok(
		coverageFor(third.messages, "accounts"),
		"a third run on the same connection still proves coverage",
	);
	const thirdVerdict = coherenceFor(
		third.messages,
		"accounts",
		"full_inventory",
	);
...
test("ynabCollect: an account present in run 1 and absent from run 2's full list is pruned from committed accounts STATE", async () => {
	const runOneFixtures: BudgetFixture[] = [
		{ id: BUDGET_A, accounts: [account(ACCOUNT_1), account(ACCOUNT_2)] },
	];
	...
	const firstCursor = firstStateMsg.cursor as Record<
		string,
		{ fingerprints?: Record<string, string> }
	>;
	const firstFingerprints = firstCursor[BUDGET_A]?.fingerprints ?? {};
	assert.ok(
		ACCOUNT_1 in firstFingerprints,
		"run 1's committed STATE fingerprints ACCOUNT_1",
	);
	assert.ok(
		ACCOUNT_2 in firstFingerprints,
		"run 1's committed STATE fingerprints ACCOUNT_2",
	);
	...
	const runTwoFixtures: BudgetFixture[] = [
		{ id: BUDGET_A, accounts: [account(ACCOUNT_1)] },
	];
	const { request: requestTwo } = fakeRequest(runTwoFixtures);

	const second = makeCtx(["accounts"], {
		accounts: firstStateMsg.cursor as Record<string, unknown>,
	});
	...
	const secondCursor = secondStateMsg.cursor as Record<
		string,
		{ fingerprints?: Record<string, string> }
	>;
	...
	const secondCov = coverageFor(second.messages, "accounts");
	assert.equal(
		secondCov?.considered,
		1,
		"run 2's boundary is 1 account — the deleted account is not double-counted",
	);
});
