...
function runPass(
	...
	> =
		priorAccounts &&
		typeof priorAccounts === "object" &&
		!Array.isArray(priorAccounts)
			? {
					...(priorAccounts as Record<
						string,
						{ server_knowledge?: number; fingerprints?: Record<string, string> }
					>),
				}
	...
	return {
		entityEmitted,
		statsEmitted,
		state: { ...priorState, accounts: carry },
	};
}
...
function fingerprintsFor(
	...
	const accounts = state.accounts as Record<
		string,
...
test("accountStatsRecord: date-scoped key carries the day's balances", () => {
	const rec = accountStatsRecord(
		acct({ balance: 123_456 }),
		BUDGET_A,
		"2026-06-03",
	);
...
test("accountRecord: entity record no longer carries balance fields", () => {
	...
	assert.equal(
		"balance" in rec,
		false,
		"balance must not be on the entity record",
	);
...
test("balance-only move: entity does NOT re-emit but account_stats records the new balance", () => {
	...
	const run2 = runPass(
		run1.state,
		BUDGET_A,
		[acct({ balance: 142_000 })],
		"2026-06-03",
		101,
	);
	assert.equal(
		run2.entityEmitted.length,
		0,
		"balance-only change must not version the entity record",
	);
	assert.equal(
		run2.statsEmitted.length,
		1,
		"account_stats still records the observation",
	);
	assert.equal(
		run2.statsEmitted[0]?.id,
		`${A1}:2026-06-03`,
		"same-day key is stable",
	);
	assert.equal(
		run2.statsEmitted[0]?.balance,
		142_000,
		"the new balance is captured",
	);
});
...
test("balance move on a later day appends a new account_stats record", () => {
	...
	assert.equal(
		run2.entityEmitted.length,
		0,
		"still no entity churn across days",
	);
	assert.equal(
		run2.statsEmitted[0]?.id,
		`${A1}:2026-06-04`,
		"next day gets its own key",
	);
});
...
test("identity change (rename) re-emits the entity record exactly once", () => {
	...
	assert.equal(
		run2.entityEmitted.length,
		1,
		"a genuine identity edit versions the entity once",
	);
	...
	assert.equal(
		run3.entityEmitted.length,
		0,
		"no further churn once the rename is recorded",
	);
});
...
test("settings change (closed flag) re-emits the entity record", () => {
	...
	assert.equal(
		run2.entityEmitted.length,
		1,
		"closing an account is a real settings change",
	);
});
...
test("delta omission carries the account forward and does NOT prune it", () => {
	...
	const both = [
		acct({ id: A1 }),
		acct({ id: A2, name: "Savings", type: "savings" }),
	];
	...
	assert.ok(
		fps[A2] !== undefined,
		"A2 carried forward despite being absent from the delta — no prune",
	);
	...
	assert.equal(
		run3.entityEmitted.length,
		0,
		"unchanged A2 no-ops — carry-forward intact across the delta gap",
	);
});
...
test("an account returned with deleted:true re-emits the entity record", () => {
	...
	assert.equal(
		run2.entityEmitted.length,
		1,
		"a deletion is a real field change and re-emits",
	);
...
test("per-budget isolation: a shared account id does not cross-contaminate", () => {
	...
	const runB = runPass(
		seedA.state,
		BUDGET_B,
		[acct({ id: A1, name: "B-Checking" })],
		"2026-06-03",
		50,
	);
	assert.equal(
		runB.entityEmitted.length,
		1,
		"budget-B emits its own A1 despite budget-A having one",
	);
	...
	const accounts = runB.state.accounts as Record<
		string,
...
test("legacy accounts state with only server_knowledge (no fingerprints) yields one full re-emit", () => {
	...
	const run1 = runPass(
		legacyState,
		BUDGET_A,
		[acct({ id: A1 }), acct({ id: A2, name: "Savings" })],
		"2026-06-03",
		43,
	);
	assert.equal(
		run1.entityEmitted.length,
		2,
		"no prior fingerprints → full re-emit",
	);
	...
	assert.equal(
		run2.entityEmitted.length,
		0,
		"subsequent run no-ops once fingerprints are seeded",
	);
});
