...
function runPass(
	...
	const carry: Record<string, { fingerprints?: Record<string, string> }> =
		priorPayeeLocs &&
		typeof priorPayeeLocs === "object" &&
		!Array.isArray(priorPayeeLocs)
			? {
					...(priorPayeeLocs as Record<
						string,
...
test("two identical passes emit each location exactly once — load-bearing churn fix", () => {
	...
	assert.equal(
		run2.emitted.length,
		0,
		"second run with unchanged source emits nothing",
	);
...
test("carry-forward: a skipped location remains in next STATE so a third run still no-ops", () => {
	...
	const after2 = (
		run2.state.payee_locations as Record<
			string,
			{ fingerprints?: Record<string, string> }
		>
	)["budget-A"]?.fingerprints;
	assert.ok(after2);
	assert.equal(
		Object.keys(after2).length,
		2,
		"both ids carried forward despite zero emits this run",
	);
...
test("multi-budget: per-budget cursor isolation — budget-A does not gate budget-B", () => {
	...
	assert.equal(
		runB.emitted.length,
		1,
		"budget-B emits its own L1 despite budget-A having one",
	);

	// Budget-A's fingerprint map survives the budget-B write.
	const aAfter = (
		runB.state.payee_locations as Record<
			string,
			{ fingerprints?: Record<string, string> }
		>
	)["budget-A"]?.fingerprints;
...
test("legacy / malformed prior state is tolerated and yields one full re-emit", () => {
	...
	const legacy1 = runPass(
		{ payee_locations: { "budget-A": {} } },
		"budget-A",
		locations,
	);
	assert.equal(
		legacy1.emitted.length,
		2,
		"empty per-budget entry → full re-emit",
	);

	// Legacy: fingerprints value is the wrong shape (array). Must not throw.
	const legacy2 = runPass(
		{ payee_locations: { "budget-A": { fingerprints: ["bogus"] } } } as Record<
			string,
			unknown
		>,
		"budget-A",
	...
	assert.equal(
		legacy2.emitted.length,
		2,
		"malformed fingerprints shape → full re-emit",
	);
	...
	assert.equal(
		...
		2,
		"all malformed entries treated as missing",
	);
});
...
test("budgets: two identical passes emit each budget exactly once — load-bearing churn fix", () => {
	const budgets = [
		budget({ id: "B1", name: "Personal" }),
		budget({ id: "B2", name: "Business" }),
	];
...
test("budgets: last_month calendar rollover does NOT re-emit", () => {
	...
	assert.equal(
		run2.emitted.length,
		0,
		"calendar rollover of last_month is not a budget change",
	);
});

test("budgets: last_modified_on tick alone does NOT re-emit", () => {
	const initial = [
		budget({ id: "B1", last_modified_on: "2026-01-01T00:00:00+00:00" }),
	];
	const run1 = runBudgetPass({}, initial);
	assert.equal(run1.emitted.length, 1);
	...
	const touched = [
		budget({ id: "B1", last_modified_on: "2026-03-15T12:34:56+00:00" }),
	];
	const run2 = runBudgetPass(run1.state, touched);
	assert.equal(
		run2.emitted.length,
		0,
		"last_modified_on tick without a summary-field change is a no-op",
	);
});
...
test("budgets: a currency-locale change re-emits the budget", () => {
	...
	assert.equal(
		run2.emitted.length,
		1,
		"currency change is a real source fact and re-emits",
	);
});
...
test("budgets: carry-forward keeps a skipped budget gated across a third run", () => {
	...
	const fps = (run2.state.budgets as { fingerprints?: Record<string, string> })
		.fingerprints;
	assert.ok(fps);
	assert.equal(
...
test("budgets: legacy state with only fetched_at (no fingerprints) yields one full re-emit", () => {
	...
	const legacy = runBudgetPass(
		{ budgets: { fetched_at: "2025-12-31T00:00:00.000Z" } },
		budgets,
	);
	assert.equal(
		legacy.emitted.length,
		2,
		"legacy fetched_at-only state → full re-emit",
	);
	...
	assert.equal(
		next.emitted.length,
		0,
		"subsequent run no-ops once fingerprints exist",
	);
});
