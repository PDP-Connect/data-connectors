...
const TRANSACTION_ID_RE =
	/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}(_t)?(_\d{4}-\d{2}-\d{2})?$/;
...
export const budgetsSchema = z.object({
	...
	last_modified_on: z
		.string()
		.regex(ISO_DATETIME_RE, "last_modified_on must be ISO-8601 datetime")
		.nullable(),
	first_month: z
		.string()
		.regex(ISO_DATE_RE, "first_month must be ISO-8601 date")
		.nullable(),
	last_month: z
		.string()
		.regex(ISO_DATE_RE, "last_month must be ISO-8601 date")
		.nullable(),
...
export const accountsSchema = z.object({
	...
	transfer_payee_id: z
		.string()
		.regex(UUID_RE, "transfer_payee_id must be UUID v4")
		.nullable(),
	direct_import_linked: z.boolean().nullable(),
	direct_import_in_error: z.boolean().nullable(),
	last_reconciled_at: z
		.string()
		.regex(ISO_DATETIME_RE, "last_reconciled_at must be ISO-8601 datetime")
		.nullable(),
...
export const accountStatsSchema = z.object({
	id: z
		.string()
		.regex(
			/^[0-9a-f-]{36}:\d{4}-\d{2}-\d{2}$/,
			"id must be {account_id}:{YYYY-MM-DD}",
		),
	account_id: z.string().regex(UUID_RE, "account_id must be UUID v4"),
	budget_id: z.string().regex(UUID_RE, "budget_id must be UUID v4"),
	observed_on: z
		.string()
		.regex(ISO_DATE_RE, "observed_on must be ISO-8601 date"),
...
export const categoriesSchema = z.object({
	...
	category_group_id: z
		.string()
		.regex(UUID_RE, "category_group_id must be UUID v4"),
	...
	goal_creation_month: z
		.string()
		.regex(ISO_DATE_RE, "goal_creation_month must be ISO-8601 date")
		.nullable(),
	goal_target: z.number().int().nullable(),
	goal_target_date: z
		.string()
		.regex(ISO_DATE_RE, "goal_target_date must be YYYY-MM-DD")
		.nullable(),
	...
	goal_snoozed_at: z
		.string()
		.regex(ISO_DATETIME_RE, "goal_snoozed_at must be ISO-8601 datetime")
		.nullable(),
	deleted: z.boolean(),
});
...
export const payeesSchema = z.object({
	...
	transfer_account_id: z
		.string()
		.regex(UUID_RE, "transfer_account_id must be UUID v4")
		.nullable(),
	deleted: z.boolean(),
});
...
export const transactionsSchema = z.object({
	id: z
		...
		.regex(
			TRANSACTION_ID_RE,
			"id must be UUID, optionally with _t and/or _<date> suffix",
		),
	budget_id: z.string().regex(UUID_RE, "budget_id must be UUID v4"),
	account_id: z.string().regex(UUID_RE, "account_id must be UUID v4"),
	...
	category_id: z
		.string()
		.regex(UUID_RE, "category_id must be UUID v4")
		.nullable(),
	...
	transfer_transaction_id: z
		...
		.regex(
			TRANSACTION_ID_RE,
			"transfer_transaction_id must be UUID, optionally with _t and/or _<date> suffix",
		)
		.nullable(),
	matched_transaction_id: z
		...
		.regex(
			TRANSACTION_ID_RE,
			"matched_transaction_id must be UUID, optionally with _t and/or _<date> suffix",
		)
		.nullable(),
...
export const monthCategoriesSchema = z.object({
	id: z
		.string()
		.regex(
			MONTH_CATEGORY_ID_RE,
			"month_categories.id must be budget_id:month:category_id",
		),
	budget_id: z.string().regex(UUID_RE, "budget_id must be UUID v4"),
	month: z.string().regex(ISO_DATE_RE, "month must be ISO-8601 date"),
