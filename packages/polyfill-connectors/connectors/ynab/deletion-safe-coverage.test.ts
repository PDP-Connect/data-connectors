...
test("a deleted payee is a valid, covered record — not a coverage gap", () => {
	...
	assert.equal(
		verdict.ok,
		true,
		"a deleted row must validate like any other row",
	);
});
...
test("deleting a row does not change the considered/covered ratio", () => {
	...
	const afterDeletion = [
		payeeRow({ id: idA }),
		payeeRow({ id: idB }),
		payeeRow({ id: idC, deleted: true }),
	];

	const coverage = (
		rows: Record<string, unknown>[],
	): { considered: number; covered: number } => ({
		considered: rows.length,
		covered: rows.reduce(
			(n, r) => n + (validateRecord("payees", r).ok ? 1 : 0),
			0,
		),
	});
	...
	assert.deepEqual(
		after,
		{ considered: 3, covered: 3 },
		"an upstream deletion must not read as coverage loss",
	);
});
...
test("a malformed row is considered but not covered", () => {
	...
	assert.equal(
		covered,
		1,
		"a row that fails the shape-check must not be counted as covered",
	);
