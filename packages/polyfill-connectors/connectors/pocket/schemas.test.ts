...
test("items schema rejects a non-numeric id (String(item_id) regression)", () => {
	assert.equal(
		itemsSchema.safeParse({ ...ITEM_RECORD, id: "item-229279689" }).success,
		false,
	);
});
...
test("items schema rejects an out-of-range status code (API drift)", () => {
	assert.equal(
		itemsSchema.safeParse({ ...ITEM_RECORD, status: "3" }).success,
		false,
	);
});
...
test("items schema rejects a non-URL url (raw title leaked into url field)", () => {
	assert.equal(
		itemsSchema.safeParse({ ...ITEM_RECORD, url: "The Great Article" }).success,
		false,
	);
});
