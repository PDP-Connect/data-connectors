...
describe("GroupMe two-run dedup test", () => {
	it("run 1: collects all records and builds state", () => {
		...
		const run1Messages = [
			...
			},
			{
				...
				group_id: "g1",
				text: "Hi",
				created_at: "2021-01-01T13:00:00Z",
			},
		];

		const groupCursor = openFingerprintCursor({
			fingerprints: priorState.groups ?? {},
		});
		const msgCursor = openFingerprintCursor({
			fingerprints: priorState.group_messages ?? {},
		});
	...
	it("run 2: reuses state from run 1, emits no duplicates", () => {
		...
		const groupCursor = openFingerprintCursor({
			fingerprints: run1State.groups ?? {},
		});
		const msgCursor = openFingerprintCursor({
			fingerprints: run1State.group_messages ?? {},
		});
		...
		assert.equal(
			emittedGroups.length,
			0,
			"run 2 should emit 0 groups (fingerprints match)",
		);
		assert.equal(
			emittedMessages.length,
			0,
			"run 2 should emit 0 messages (fingerprints match)",
		);
		...
		assert.equal(
			Object.keys(run2State.groups ?? {}).length,
			2,
			"run 2 should carry forward 2 groups",
		);
		assert.equal(
			Object.keys(run2State.group_messages ?? {}).length,
			2,
			"run 2 should carry forward 2 messages",
		);
	});
	...
	it("state namespace: unified under 'groups' stream per runtime merge contract", () => {
		...
		assert.ok(
			stream === "groups",
			"all GroupMe state must be emitted under 'groups' stream",
		);
	...
	it("attachment blob refs added to records without changing id/created_at", () => {
		const msgWithoutBlob = {
			...
			created_at: "2021-01-01T12:00:00Z",
			attachments: [
				{ type: "image", url: "https://i.groupme.com/img.jpg", name: null },
			],
		};
		...
		assert.equal(
			changed,
			true,
			"blob_id addition changes fingerprint and re-emits",
		);
