...
test("collectGroups: clean pass reports failed: false and considered === listed groups", async () => {
	...
	try {
		...
		const outcome = await collectGroups(
			TOKEN,
			cursor,
			noopProgress,
			emitRecord,
...
test("collectGroups: genuine zero groups reports failed: false, considered: 0", async () => {
	...
	try {
		...
		assert.deepEqual(
			outcome,
			{ considered: 0, failed: false },
			"a measured empty boundary is itself the proof",
		);
...
test("collectGroups: fingerprint-suppressed (unchanged) group still counts as considered", async () => {
	...
	try {
		...
		assert.equal(
			emitted.length,
			0,
			"unchanged group is suppressed, not re-emitted",
		);
		assert.equal(
			outcome.considered,
			1,
			"considered counts the listed group even though it was suppressed",
		);
...
test("collectGroups: http error reports failed: true and does not throw (non-auth failure is caught)", async () => {
	...
	try {
		...
		assert.equal(
			outcome.failed,
			true,
			"an http failure must not report a clean pass",
		);
...
test("collectGroups: malformed (unparseable) body reports failed: true, not a proven-empty list", async () => {
	...
	try {
		...
		assert.equal(
			outcome.failed,
			true,
			"an unparseable 200 body is a failed enumeration, not a genuine zero",
		);
		assert.equal(emitted.length, 0);
	} finally {
		restore();
...
test("collectGroups: a rejected/dropped record still counts as considered, not just emitted", async () => {
	const restore = stubFetch({
		response: [group({ id: "group-1" }), group({ id: 999 })],
	});
	try {
		const cursor = openFingerprintCursor(new Map());
		const { emitRecord, emitted } = makeHarness();
		...
		assert.equal(
			...
			1,
			"only the valid record emits",
		);
		assert.equal(
			outcome.considered,
			2,
			"considered counts both listed groups, not just the emitted one",
		);
		assert.equal(outcome.failed, false);
	} finally {
		restore();
...
test("collectGroups: auth failure (401) propagates instead of being swallowed as a stream failure", async () => {
	...
	try {
		...
		await assert.rejects(
			...
			(err: unknown) =>
				err instanceof Error && err.message === "groupme_auth_failed",
		);
	} finally {
		restore();
	}
...
test("collectDirectChats: pages past 200 full pages with no truncation (mutation-killing)", async () => {
	...
	const restore = stubFetchSequence(
		Array.from({ length: totalPages + 1 }, (_pageValue, pageNumber) => {
			...
			const response = Array.from(
				...
				(_itemValue, itemIndex) =>
					directChat({
						id: `chat-p${String(pageNumber)}-${String(itemIndex)}`,
					}),
			);
			return { body: { response } };
		}),
	...
	try {
		const cursor = openFingerprintCursor(new Map());
		const { emitRecord, emitted } = makeHarness();
		const outcome = await collectDirectChats(
			TOKEN,
			cursor,
			noopProgress,
			emitRecord,
...
test("collectGroups: a full-size page contributing zero new ids fails as NonProgressError, does not loop forever", async () => {
	...
	try {
		...
		assert.equal(
			outcome.failed,
			true,
			"a repeated-content page must fail, not loop or falsely report success",
		);
	} finally {
		restore();
	}
...
test("collectGroupMessages: cold start (no prior cursor) walks backward, clean pass across multiple groups sums considered", async () => {
	const restore = stubFetchSequence([
		{
			body: { response: [group({ id: "group-1" }), group({ id: "group-2" })] },
		}, // /groups
		{
			body: {
				response: {
					count: 2,
					messages: [groupMessage({ id: "m1" }), groupMessage({ id: "m2" })],
				},
			},
		}, // group-1 page
		{
			body: { response: { count: 1, messages: [groupMessage({ id: "m3" })] } },
		}, // group-2 page
	]);
	try {
		const cursor = openFingerprintCursor(new Map());
...
test("collectGroupMessages: a declared since bound stops the documented-descending backward walk cleanly, without fetching further pages", async () => {
	...
	globalThis.fetch = ((): Promise<Response> => {
		...
		if (fetchCount === 1) {
			return Promise.resolve(
				new Response(JSON.stringify({ response: [group({ id: "group-1" })] }), {
					status: 200,
				}),
			);
		}
	...
	try {
		...
		assert.deepEqual(
			emitted
				.filter((r) => r.stream === "group_messages")
		...
		assert.equal(
			outcome.failed,
			false,
			"an honest since-bound stop is a clean pass, not a failure",
		);
		assert.equal(
			outcome.considered,
			1,
		...
		assert.equal(
			fetchCount,
			2,
			"the groups list plus exactly one message page",
		);
	} finally {
...
test("collectGroupMessages: a non-descending backward page fails as NonProgressError rather than trusting an unverified before_id", async () => {
	...
	try {
		...
		assert.equal(
			outcome.failed,
			true,
			"a non-descending page must fail the walk, not be silently trusted",
		);
	} finally {
		restore();
	}
...
test("collectGroupMessages: a rejected/dropped record still counts as considered, not just emitted", async () => {
	...
	try {
		...
		assert.equal(
			outcome.considered,
			2,
			"considered counts both listed messages, not just the emitted one",
		);
		assert.equal(outcome.failed, false);
	} finally {
		restore();
...
test("collectGroupMessages: http error partway through a group's pages reports failed: true", async () => {
	...
	try {
		...
		assert.equal(
			outcome.failed,
			true,
			"a mid-walk http failure must not report a clean pass",
		);
		assert.equal(emitted.length, 0);
	} finally {
		restore();
...
test("collectGroupMessages: a group's backward walk pages past 200 full pages with no truncation (mutation-killing)", async () => {
	...
	globalThis.fetch = ((input: RequestInfo | URL): Promise<Response> => {
		...
		return Promise.resolve(
			new Response(
				JSON.stringify({ response: { count: messages.length, messages } }),
				{ status: 200 },
			),
		);
	}) as typeof globalThis.fetch;
...
test("collectDirectChats: clean pass reports failed: false and considered === listed chats", async () => {
	...
	try {
		...
		assert.equal(
			directRecords[1]?.last_message_at,
			null,
			"provider timestamp 0 is absence, never ingest time",
		);
	} finally {
		restore();
	}
...
test("collectDirectChats: fingerprint-suppressed (unchanged) chat still counts as considered", async () => {
	...
	try {
		...
		assert.equal(
			emitted.length,
			0,
			"unchanged chat is suppressed, not re-emitted",
		);
		assert.equal(
			outcome.considered,
			1,
			"considered counts the listed chat even though it was suppressed",
		);
		assert.equal(outcome.failed, false);
	} finally {
		restore();
...
test("collectDirectChatMessages: clean pass across multiple chats sums considered from every chat's pages", async () => {
	const restore = stubFetchSequence([
		{
			body: {
				response: [directChat({ id: "chat-1" }), directChat({ id: "chat-2" })],
			},
		}, // /chats
		{
			body: {
		...
		{
			body: {
				response: { count: 1, direct_messages: [directMessage({ id: "m3" })] },
			},
		}, // chat-2 page
	]);
	try {
		...
		assert.equal(
			emitted.filter((r) => r.stream === "direct_chat_messages").length,
			3,
		);
	} finally {
		restore();
...
test("collectDirectChatMessages: uses the direct-message endpoint keyed by the other user", async () => {
	...
	try {
		...
		assert.equal(
			emitted.find((record) => record.stream === "direct_chat_messages")?.data
				.chat_id,
			"user-2",
		);
		assert.ok(
			!urls.some((url) => url.pathname === "/v3/chats/user-2/messages"),
		);
	} finally {
		globalThis.fetch = original;
	}
...
test("collectDirectChatMessages: a declared since bound filters out-of-scope rows from considered/emitted, without an early stop", async () => {
	...
	try {
		...
		assert.deepEqual(
			emitted
				.filter((r) => r.stream === "direct_chat_messages")
				.map((r) => (r.data as { id: string }).id),
			["d1"],
			"only the in-window message emits",
		);
		assert.equal(
			outcome.failed,
			false,
			"excluding an out-of-scope row is a clean pass, not a failure",
		);
		assert.equal(
			outcome.considered,
			1,
		...
		{ body: { response: { count: 100, direct_messages: fullOldPage } } }, // page 1: entirely out of scope, full page
		{
			body: {
				response: {
					count: 1,
					direct_messages: [
						directMessage({ id: "d-late", created_at: 1_700_000_300 }),
					],
				},
			},
...
test("collectDirectChatMessages: genuine zero chats reports failed: false, considered: 0", async () => {
	...
	try {
		...
		assert.deepEqual(
			outcome,
			{ considered: 0, failed: false },
			"no chats means no messages — a proven-empty walk",
		);
		assert.equal(emitted.length, 0);
	} finally {
		restore();
...
test("collectDirectChatMessages: retries one transient chat page in place without refetching prior chats", async () => {
	...
	globalThis.fetch = ((input: RequestInfo | URL): Promise<Response> => {
		...
		if (url.pathname === "/v3/chats") {
			return Promise.resolve(
				new Response(
					JSON.stringify({
						response: [
							directChat({ id: "chat-1" }),
							directChat({ id: "chat-2" }),
						],
					}),
					{
						status: 200,
		...
		if (messageCalls === 2) {
			return Promise.resolve(
				new Response(JSON.stringify({ error: "transient server error" }), {
					status: 500,
				}),
			);
		}
		...
		return Promise.resolve(
			new Response(
				JSON.stringify({
					response: {
						count: 1,
						direct_messages: [directMessage({ id: messageId })],
					},
				}),
				{
					status: 200,
	...
	try {
		...
		assert.equal(
			messageCalls,
			3,
			"the 500 is retried in place; chat-1 is not refetched",
		);
	} finally {
		globalThis.fetch = original;
	}
...
test("collectDirectChatMessages: persistent 500 remains failed after bounded request attempts", async () => {
	...
	globalThis.fetch = ((input: RequestInfo | URL): Promise<Response> => {
		...
		if (url.pathname === "/v3/chats") {
			return Promise.resolve(
				new Response(
					JSON.stringify({ response: [directChat({ id: "chat-1" })] }),
					{ status: 200 },
				),
			);
		}
		messageCalls += 1;
		return Promise.resolve(
			new Response(JSON.stringify({ error: "persistent server error" }), {
				status: 500,
			}),
		);
	}) as typeof globalThis.fetch;
	try {
		...
		assert.equal(
			messageCalls,
			3,
			"request retries are bounded at three attempts",
		);
	} finally {
		globalThis.fetch = original;
	}
...
test("collectDirectChatMessages: a chat's walk pages past 200 full pages with no truncation (mutation-killing)", async () => {
	...
	globalThis.fetch = ((input: RequestInfo | URL): Promise<Response> => {
		...
		const isLastPage = pageNumber === totalPages;
		const pageLength = isLastPage ? 1 : PAGE_SIZE;
		const direct_messages = Array.from(
			{ length: pageLength },
			(_itemValue, itemIndex) =>
		...
		return Promise.resolve(
			new Response(
				JSON.stringify({
					response: { count: direct_messages.length, direct_messages },
				}),
				{ status: 200 },
			),
		);
...
test("collectDirectChatMessages: a repeated (non-advancing) trailing cursor fails as NonProgressError rather than looping forever", async () => {
	...
	try {
		...
		assert.equal(
			outcome.failed,
			true,
			"a repeated trailing cursor must fail the walk, not loop forever",
		);
	} finally {
		restore();
	}
...
test("collect()-level gating: failed outcome must suppress both STATE and DETAIL_COVERAGE for that stream", async () => {
	...
	try {
		...
		assert.equal(
			messages.length,
			0,
			"a failed collection pass must emit neither STATE nor coverage for its stream",
		);
	} finally {
		restore();
	}
...
test("collect(): successful message walks durably emit complete proof, including verified-empty", async () => {
	...
	try {
		...
		await collect({
			...
			scope: {
				streams: [{ name: "group_messages" }, { name: "direct_chat_messages" }],
			},
		...
		for (const stream of ["group_messages", "direct_chat_messages"]) {
			...
			assert.equal(
				coverage.considered,
				0,
				`${stream} empty enumeration must be explicit`,
			);
			assert.equal(
				coverage.covered,
				0,
				`${stream} verified-empty proof must be complete`,
			);
		}
	} finally {
		restore();
...
test("collect(): failed direct_chat_messages reports failure while preserving successful sibling coverage", async () => {
	...
	try {
		...
		await collect({
			...
			requestDetailGapPage: async () => [],
			scope: {
				streams: [{ name: "groups" }, { name: "direct_chat_messages" }],
			},
			sendInteraction: async () => ({}) as never,
		} satisfies CollectContext);
		...
		assert.ok(
			messages.some(
				(message) =>
					...
					message.stream === "direct_chat_messages",
			),
		...
		assert.ok(
			groupsCoverage,
			"the successful sibling stream must retain its coverage proof",
		);
		...
		assert.equal(
			messages.some(
				(message) =>
					message.type === "STATE" && message.stream === "direct_chat_messages",
			),
			false,
...
test("collectGroupMessages: an empty page short of the provider count marks the boundary unproven", async () => {
	...
	try {
		...
		assert.equal(
			outcome.failed,
			false,
			"nothing errored — only the completeness claim is withheld",
		);
...
test("collectGroupMessages: an empty page whose count AGREES at zero stays a proven walk", async () => {
	...
	try {
		...
		assert.deepEqual(
			outcome.shortfalls,
			[],
			"a coherent zero is a real anchor, not a gap",
		);
	} finally {
		restore();
	}
...
test("collectGroupMessages: GroupMe's documented 304 end-of-history is a PROVEN walk, not a shortfall", async () => {
	...
	try {
		...
		assert.equal(
			outcome.failed,
			false,
			"a 304 terminal page is a clean end, not a failure",
		);
...
test("collect(): an ambiguous empty page is reported as unexplained AND retryable, never as proven-unrecoverable", async () => {
	...
	try {
		...
		const ambiguous = skips.find(
			(s) => s.reason === "history_ended_before_provider_count",
		);

		assert.ok(
			ambiguous,
			"the ambiguous gap must be reported under its own reason",
		);
		...
		assert.ok(
			...
			"recovery_hint must be the structured form",
		);
		...
		const diagnostics = ambiguous.diagnostics as
			| { unexplained_message_total?: number }
			| undefined;
