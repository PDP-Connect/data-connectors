...
function stubFetchByPath(
	...
	globalThis.fetch = ((input: RequestInfo | URL): Promise<Response> => {
		...
		if (route === undefined) {
			throw new Error(
				`unstubbed path in incremental-frontier test: ${url.pathname}`,
			);
		}
...
function stubGroupWalk(opts: {
	groupsListBody?: unknown;
	messagePages: Array<{ body: unknown; status?: number }>;
}): {
	fetchCount: () => number;
	restore: () => void;
} {
	...
	globalThis.fetch = ((input: RequestInfo | URL): Promise<Response> => {
		...
		if (url.pathname === "/v3/groups") {
			return Promise.resolve(
				new Response(
					JSON.stringify({ response: opts.groupsListBody ?? [group()] }),
					{ status: 200 },
				),
			);
		}
		const page =
			opts.messagePages[
				Math.min(messageFetchCount, opts.messagePages.length - 1)
			];
		messageFetchCount += 1;
		return Promise.resolve(
...
test("decodeGroupMessageCursors: absent/malformed state decodes to empty map (cold start)", () => {
	...
	assert.deepEqual(
		decodeGroupMessageCursors({ fingerprints: {} }),
		{},
		"missing cursors field",
	);
	assert.deepEqual(
		decodeGroupMessageCursors({ cursors: "nope" }),
		{},
	...
	return (async () => {
		try {
			...
			assert.equal(
				walk.fetchCount(),
				2,
				"both pages fetched — page 2's shorter length ends the walk naturally",
			);
			assert.equal(
				outcome.considered,
				150,
				"cold run sees the entire history, not a truncated window",
			);
			assert.equal(
			...
			assert.equal(
				...
				"m-149",
				"cursor seeds from the newest message's id",
			);
		} finally {
			walk.restore();
		}
	...
	return (async () => {
		try {
			...
			assert.equal(
				walk.fetchCount(),
				1,
				"a short page ends the forward walk after one fetch",
			);
			...
			assert.equal(
				outcome.nextCursors["group-1"],
				"m-new-2",
				"cursor advances to the newest (last) message this run",
			);
		} finally {
			walk.restore();
		}
	...
	return (async () => {
		try {
			...
			assert.equal(
				walk.fetchCount(),
				2,
				"the full page 1 forces a second forward fetch",
			);
...
function stubGroupWalkPastOldCap(
	...
	globalThis.fetch = ((input: RequestInfo | URL): Promise<Response> => {
		...
		if (url.pathname === "/v3/groups") {
			return Promise.resolve(
				new Response(JSON.stringify({ response: [group()] }), { status: 200 }),
			);
		}
...
test(">old-cap discriminator FAIL-BEFORE proof: temporarily restoring a 200-page cap on the shared list walker demonstrably breaks the >200-page assertion", async () => {
	...
	async function cappedForwardWalk(
		maxPages: number,
	): Promise<{ fetchCount: number; lastId: string | undefined }> {
		let afterId = "m-old-cursor";
		let fetchCount = 0;
		let lastId: string | undefined;
		for (let page = 0; page < maxPages; page += 1) {
			const res = await fetch(
				`https://api.groupme.com/v3/groups/group-1/messages?after_id=${afterId}`,
			);
...
function stubFirstFetchStatus(
	...
	let messageFetchCount = 0;
	const backwardPage = [
		groupMessage({ id: "m-full-1", created_at: 1_700_000_100 }),
	];
	const original = globalThis.fetch;
	const responseInit =
		firstFetchStatus === 429 ? { headers: { "retry-after": "0" } } : {};
	globalThis.fetch = ((input: RequestInfo | URL): Promise<Response> => {
		const url = new URL(typeof input === "string" ? input : input.toString());
		if (url.pathname === "/v3/groups") {
			return Promise.resolve(
		...
		return Promise.resolve(
			new Response(
				JSON.stringify({
					response: { count: backwardPage.length, messages: backwardPage },
				}),
				{
					status: 200,
				},
...
test("deleted/invalid cursor fallback: a 400 on the FIRST resumed fetch falls back to a full backward walk for that group only", async () => {
	...
	try {
		...
		assert.equal(
			outcome.failed,
			false,
			"the fallback backward walk completes cleanly despite the invalid cursor",
		);
		assert.equal(
			emitted.filter((r) => r.stream === "group_messages").length,
			1,
...
test("deleted/invalid cursor fallback: a 404 on the FIRST resumed fetch also falls back to a full backward walk", async () => {
	...
	try {
		...
		assert.equal(
			outcome.failed,
			false,
			"a 404 on first fetch also licenses the fallback",
		);
		assert.equal(
			emitted.filter((r) => r.stream === "group_messages").length,
			1,
...
test("P1 regression: a 401 on the FIRST resumed fetch propagates as groupme_auth_failed, no fallback", async () => {
	...
	try {
		...
		assert.equal(
			stub.fetchCount(),
			1,
			"no fallback fetch was attempted — the whole run is dead on auth failure",
		);
	} finally {
		stub.restore();
	}
...
test("P1 regression: a transient 429 on the FIRST resumed fetch recovers in place, no fallback rescan", async () => {
	...
	try {
		...
		assert.equal(
			outcome.failed,
			false,
			"a transient 429 should recover within the request budget",
		);
		assert.equal(
			stub.fetchCount(),
			2,
			"the retry is in place; no fallback backward walk was attempted",
		);
		assert.equal(
			emitted.filter((r) => r.stream === "group_messages").length,
			1,
...
test("P1 regression: a transient 500 on the FIRST resumed fetch recovers in place, no fallback rescan", async () => {
	...
	try {
		...
		assert.equal(
			outcome.failed,
			false,
			"a transient 500 should recover within the request budget",
		);
		assert.equal(
			stub.fetchCount(),
			2,
...
test("P1 true reproducer: a 422 on the FIRST resumed fetch fails the pass, no fallback rescan (422 is not governor-retried and not 400/404)", async () => {
	...
	try {
		...
		assert.equal(
			stub.fetchCount(),
			1,
			"no fallback backward walk was attempted",
		);
		assert.equal(emitted.length, 0);
	} finally {
		stub.restore();
	...
	globalThis.fetch = ((input: RequestInfo | URL): Promise<Response> => {
		...
		if (messageFetchCount === 1) {
			return Promise.resolve(
				new Response(
					JSON.stringify({
						response: { count: firstPage.length, messages: firstPage },
					}),
					{ status: 200 },
				),
			);
		...
		return Promise.resolve(
			new Response(JSON.stringify({ meta: { code: 400 } }), { status: 400 }),
		);
	}) as typeof globalThis.fetch;
	try {
		const cursor = openFingerprintCursor(new Map());
		...
		assert.equal(
			outcome.failed,
			true,
			"a mid-walk failure fails the pass normally, without a backward fallback",
		);
	} finally {
		globalThis.fetch = original;
	}
...
test("interrupted run via collect(): STATE (including cursors) is withheld end-to-end on a failed pass, prior cursor survives untouched", async () => {
	...
	const run1State = run1.messages.find(
		(m) => m.type === "STATE" && m.stream === "group_messages",
	);
	assert.ok(
		run1State && run1State.type === "STATE",
		"run 1 persists a group_messages STATE checkpoint",
	);
	const run1Cursor = (
		run1State as { cursor: { cursors?: Record<string, string> } }
	).cursor;
	assert.equal(
		run1Cursor.cursors?.["group-1"],
		"gmsg-1",
		"run 1 seeds group-1's cursor",
	);
	...
	const run2State = run2.messages.find(
		(m) => m.type === "STATE" && m.stream === "group_messages",
	);
	assert.ok(
		!run2State,
		"an interrupted run must not emit a replacement STATE — the prior cursor is left untouched",
	);
});
...
test("explicit full_refresh bypass: an established forward cursor is ignored, the group walks backward to its natural end", () => {
	const page1 = Array.from({ length: PAGE_SIZE }, (_, i) =>
		groupMessage({ id: `m-${String(149 - i)}` }),
	);
	const page2 = Array.from({ length: 50 }, (_, i) =>
		groupMessage({ id: `m-${String(49 - i)}` }),
	);
	const walk = stubGroupWalk({
		messagePages: [
			{ body: { response: { count: 100, messages: page1 } } },
	...
	return (async () => {
		try {
			...
			assert.equal(
				walk.fetchCount(),
				2,
				"full_refresh bypasses the cursor — both pages fetched via the backward walk",
			);
			assert.equal(
				outcome.considered,
				150,
				"the entire history is walked and considered under full_refresh",
			);
			assert.equal(
				emitted.filter((r) => r.stream === "group_messages").length,
				150,
