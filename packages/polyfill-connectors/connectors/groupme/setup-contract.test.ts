...
describe("GroupMe setup contract", () => {
	...
	it("credential_capture declares access_token kind with password field", () => {
		...
		assert.strictEqual(
			capture.kind,
			"access_token",
			"credential_capture.kind must be 'access_token'",
		);
		...
		assert.strictEqual(
			field.type,
			"password",
			"first field type must be 'password'",
		);
		assert.strictEqual(field.secret, true, "first field secret must be true");
		assert.ok(
			field.env?.includes("GROUPME_ACCESS_TOKEN"),
			"field env must include GROUPME_ACCESS_TOKEN",
		);
	});
	...
	it("live: accepts valid GROUPME_ACCESS_TOKEN via X-Access-Token header", {
		skip: process.env.GROUPME_ACCESS_TOKEN
			? false
			: "GROUPME_ACCESS_TOKEN unset",
	}, async () => {
		const token = process.env.GROUPME_ACCESS_TOKEN;
		assert.ok(
			token && token.length > 0,
			"GROUPME_ACCESS_TOKEN must be non-empty",
		);
