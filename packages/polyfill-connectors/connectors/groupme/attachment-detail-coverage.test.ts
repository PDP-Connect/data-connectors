...
test("recordAttachmentCoverage: zero attachments considered this run yields a proven-empty 0/0 claim", () => {
	...
	assert.equal(
		msg.considered,
		0,
		"a genuinely empty run must prove considered: 0, not omit it",
	);
...
test("recordAttachmentCoverage: every attachment hydrated reports considered === covered (nonzero, full coverage)", () => {
	...
	assert.equal(
		msg.covered,
		2,
		"a fully hydrated run must prove full coverage, not merely nonzero collection",
	);
...
test("recordAttachmentCoverage: a partial hydration failure reports covered < considered — must not be laundered into full coverage", () => {
	...
	assert.equal(
		msg.considered,
		3,
		"the failed attachment was still considered — it must not shrink the denominator",
	);
	assert.equal(
		msg.covered,
		2,
		"only the two genuinely hydrated attachments count toward covered",
	);
	assert.deepEqual(msg.required_keys, ["a1", "a2", "a3"]);
	assert.deepEqual(
		msg.hydrated_keys,
		["a1", "a3"],
		"the failed attachment must not appear in hydrated_keys",
	);
});
...
function stubFetchByPath(
	...
	globalThis.fetch = ((input: RequestInfo | URL): Promise<Response> => {
		...
		if (route === undefined) {
			throw new Error(
				`unstubbed path in attachment-detail-coverage test: ${url.pathname}`,
			);
		}
...
function attachmentsCoverageMessages(
	messages: readonly EmittedMessage[],
): EmittedMessage[] {
	return messages.filter(
		(m) => m.type === "DETAIL_COVERAGE" && m.stream === "attachments",
	);
}
...
test("collect(): zero attachments across both parent streams proves a verified-empty 0/0 attachments claim with STATE", async () => {
	...
	try {
		...
		const state = messages.find(
			(m) => m.type === "STATE" && m.stream === "attachments",
		);

		assert.equal(
			coverage.length,
			2,
			"each independently checkpointed parent must carry its own report",
		);
		...
		assert.ok(
			state,
			"attachments STATE must be emitted when parent streams succeed cleanly",
		);
	} finally {
		restore();
	}
...
test("collect(): nonzero attachments with no blob-upload backend configured proves considered > 0, covered: 0 — never false-complete, and emits STATE", async () => {
	...
	const restore = stubFetchByPath({
		...
		"/v3/direct_messages": {
			count: 1,
			direct_messages: [directMessageWithAttachment("dmsg-1")],
		},
	});
	try {
		const { ctx, messages } = makeCtx();
...
test("collect(): a failed requested parent emits no report for that parent while preserving the successful sibling", async () => {
	...
	try {
		const { ctx, messages } = makeCtx({
			attachments: { fingerprints: "prior-state" },
		});
		await collect(ctx);

		const coverage = attachmentsCoverageMessages(messages);
		const attachmentsState = messages.find(
			(m) => m.type === "STATE" && m.stream === "attachments",
		);
		...
		assert.equal(
			coverage.length,
			1,
			"the successful direct-message parent keeps its independent coverage report",
		);
		assert.equal(
		...
		assert.ok(
			attachmentsState,
			"the successful parent's attachment fingerprints remain reusable",
		);
	} finally {
		restore();
	}
...
test("collect(): a parent stream that was never requested does not block attachments coverage for the requested parent that succeeded", async () => {
	...
	try {
		...
		assert.equal(
			coverage?.type === "DETAIL_COVERAGE" && coverage.considered,
			1,
		);
	} finally {
		restore();
...
test("collect(): a transient attachment failure stays uncovered so the parent cursor can be withheld", async () => {
	...
	const restoreFirst = stubFetchByPath({
		...
		"/v3/groups/group-1/messages": {
			count: 1,
			messages: [groupMessageWithAttachment("retry-me")],
		},
	});
	try {
		...
		assert.equal(
			firstCoverage?.type === "DETAIL_COVERAGE" && firstCoverage.covered,
			0,
		);
		assert.deepEqual(
			firstCoverage?.type === "DETAIL_COVERAGE" && firstCoverage.hydrated_keys,
			[],
		);
		assert.equal(
			firstCoverage?.type === "DETAIL_COVERAGE" && firstCoverage.gap_keys,
			undefined,
		);
		assert.equal(
			secondCoverage?.type === "DETAIL_COVERAGE" && secondCoverage.covered,
			1,
		);
		assert.deepEqual(
			secondCoverage?.type === "DETAIL_COVERAGE" &&
				secondCoverage.hydrated_keys,
			secondCoverage?.type === "DETAIL_COVERAGE" &&
				secondCoverage.required_keys,
		);
		assert.deepEqual(
			evaluateStreamCoherence(
				{
					...
					collected: 1,
					considered:
						firstCoverage?.type === "DETAIL_COVERAGE"
							? (firstCoverage.considered ?? null)
							: null,
					covered:
						firstCoverage?.type === "DETAIL_COVERAGE"
							? (firstCoverage.covered ?? null)
							: null,
			...
			evaluateStreamCoherence(
				{
					...
					considered:
						secondCoverage?.type === "DETAIL_COVERAGE"
							? (secondCoverage.considered ?? null)
							: null,
					covered:
						secondCoverage?.type === "DETAIL_COVERAGE"
							? (secondCoverage.covered ?? null)
							: null,
					pending_detail_gaps: 0,
					skipped: null,
				},
