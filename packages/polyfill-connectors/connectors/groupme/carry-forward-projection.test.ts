...
function stubFetchByPath(
	...
	globalThis.fetch = ((input: RequestInfo | URL): Promise<Response> => {
		...
		if (route === undefined) {
			throw new Error(
				`unstubbed path in carry-forward projection test: ${url.pathname}`,
			);
		}
...
test("collect(): two real runs through the runtime's STATE projection suppress all four cursor-backed streams on run 2", async () => {
	...
	try {
		...
		assert.deepEqual(
			run1.skipped,
			[],
			"no record should fail schema validation in run 1",
		);
		assert.equal(
			run1.emitted.filter((r) => r.stream === "groups").length,
			1,
			"run 1 emits the group",
		);
		assert.equal(
			run1.emitted.filter((r) => r.stream === "group_messages").length,
			1,
			"run 1 emits the group message",
		);
		assert.equal(
			run1.emitted.filter((r) => r.stream === "direct_messages").length,
			1,
			"run 1 emits the direct chat",
		);
		assert.equal(
		...
		const stateStreams = new Set(
			run1.messages.filter((m) => m.type === "STATE").map((m) => m.stream),
		);
		assert.deepEqual(
			[...stateStreams].sort(),
		...
		assert.equal(
			run2.emitted.filter((r) => r.stream === "groups").length,
			0,
			"unchanged group suppressed on run 2",
		);
		assert.equal(
...
test("collect(): a failed stream's prior cursor survives untouched into the next run (no unified blob required)", async () => {
	...
	const projectedState = projectState(run1.messages);
	assert.ok(
		projectedState.direct_messages,
		"run 1 persisted a direct_messages cursor to carry forward",
	);
	...
	const run2StateStreams = new Set(
		run2.messages.filter((m) => m.type === "STATE").map((m) => m.stream),
	);
	assert.ok(
		!run2StateStreams.has("direct_messages"),
		"failed stream must not emit a replacement STATE this run",
	);
	assert.ok(
		!run2StateStreams.has("direct_chat_messages"),
		"direct_chat_messages was not requested this run's scope",
	);
