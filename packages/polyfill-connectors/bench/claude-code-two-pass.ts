...
const baseDir =
	process.env.CLAUDE_CODE_PROJECTS_DIR ||
...
const silentEmit = (): ((msg: EmittedMessage) => Promise<void>) => () =>
	Promise.resolve();
...
async function runLegacyBaseline(): Promise<RunResult> {
	...
	await legacyEmitSessionsFromAccumulators({
		emitRecord,
		requested,
		sessionAccumulators,
	});
...
async function main(): Promise<void> {
	...
	// asymmetry too.
	process.stderr.write(
		`[bench] Interleaved ${ITERATIONS} iterations of (legacy baseline, two-pass)\n`,
	);
	...
	const regression =
		((twoPass.medianMs - single.medianMs) / single.medianMs) * 100;
	...
	process.stdout.write(
		`legacy baseline median (pre-Tranche-C, commit 0cf0f51^): ${single.medianMs.toFixed(0)}ms\n`,
	);
	process.stdout.write(
		`current two-pass median:                                 ${twoPass.medianMs.toFixed(0)}ms\n`,
	);
	process.stdout.write(
		`  pass 1 (build):   ${twoPass.pass1MedianMs?.toFixed(0)}ms\n`,
	);
	process.stdout.write(
		`  emit sessions:    ${twoPass.emitSessionsMedianMs?.toFixed(0)}ms\n`,
	);
	process.stdout.write(
		`  pass 2 (emit):    ${twoPass.pass2MedianMs?.toFixed(0)}ms\n`,
	);
	process.stdout.write(
	...
	if (regression > 25) {
		...
		process.stdout.write(
			"Verdict: acceptable with caveat (10-25% regression).\n",
		);
	} else {
