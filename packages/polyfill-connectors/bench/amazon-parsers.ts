...
function bench(label: string, fn: () => unknown, iters: number): number {
	...
	console.log(
		`${label.padEnd(40)} ${iters} iters ${elapsed.toFixed(0).padStart(5)}ms → ${perCall.toFixed(2)}ms/call`,
	);
	return perCall;
}
...
const [listPath, detailPath] = liveDir
	...
	: ([
			join(SYNTHETIC_DIR, "orders-list-minimal.html"),
			join(SYNTHETIC_DIR, "order-detail-minimal.html"),
		] as const);

if (liveDir && !detailPath) {
	console.log(
		"[bench] live captures present but no order-detail html found; using synthetic detail",
	);
}
...
const listMs = bench(
	"parseOrdersListDom",
	() => parseOrdersListDom(listHtml),
	50,
);
const detailMs = bench(
	"parseOrderDetailDom",
	() => parseOrderDetailDom(detailHtml),
	50,
);
...
console.log(
	"        (Playwright nav is ~1-2s/page, so parsing is <1% of wall-clock.)",
);
