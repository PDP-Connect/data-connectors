...
test("RetryBudget: consumes retry tokens and refills on successes", () => {
	...
	assert.equal(
		budget.consume(),
		false,
		"fractional token is not enough for a retry",
	);
	budget.recordSuccess();
	assert.equal(budget.remaining, 1);
	assert.equal(budget.consume(), true);
...
test("retryBudgetCapacityFromRequestCap: derives ratio-based capacity and preserves unbounded runs", () => {
	...
	assert.equal(
		retryBudgetCapacityFromRequestCap({ maxRequests: 3, minCapacity: 1 }),
		1,
	);
	assert.equal(
		retryBudgetCapacityFromRequestCap({
...
test("CircuitBreaker: honors minimum throughput before opening", () => {
	...
	assert.equal(
		breaker.state,
		"closed",
		"minimum throughput prevents false-open on tiny samples",
	);
...
test("CircuitBreaker: open blocks requests, half-open probes after timeout, success closes", () => {
	...
	assert.equal(breaker.state, "open");
	assert.equal(
		breaker.beforeRequest().ok,
		false,
		"open circuit fast-fails before reset timeout",
	);
	...
	assert.equal(
		breaker.beforeRequest().ok,
		true,
		"reset timeout allows a half-open probe",
	);
...
test("CircuitBreaker: remainingCooldownMs reports the exact wait until half-open, 0 when not open", () => {
	...
	assert.equal(
		breaker.remainingCooldownMs(),
		0,
		"closed circuit owes no cool-down",
	);
	...
	assert.equal(
		breaker.remainingCooldownMs(),
		1000,
		"freshly opened circuit owes the full reset timeout",
	);
	...
	assert.equal(
		breaker.remainingCooldownMs(),
		400,
		"cool-down shrinks as the clock advances",
	);
	...
	assert.equal(
		breaker.remainingCooldownMs(),
		0,
		"no cool-down owed once the reset timeout has elapsed",
	);
	...
	assert.equal(
		breaker.remainingCooldownMs(),
		0,
		"half-open circuit owes no cool-down",
	);
});
...
test("ProviderBudgetController: circuitCooldownMs surfaces the breaker's remaining cool-down (0 when no breaker)", () => {
	...
	assert.equal(
		budget.circuitCooldownMs(),
		0,
		"no cool-down owed before the circuit opens",
	);
	budget.recordFailure();
	assert.equal(
		budget.circuitCooldownMs(),
		5000,
		"open circuit surfaces the full cool-down",
	);
	nowMs = 2000;
	assert.equal(
		budget.circuitCooldownMs(),
		3000,
		"cool-down shrinks with elapsed time",
	);

	const noBreaker = new ProviderBudgetController({
		retryBudget: { capacity: 1 },
	});
	assert.equal(
		noBreaker.circuitCooldownMs(),
		0,
		"a controller without a circuit breaker owes no cool-down",
	);
});
...
test("ProviderBudgetController: drains generic circuit state transitions", async () => {
	...
	assert.deepEqual(
		budget.drainCircuitTransitions(),
		[],
		"drain clears the durable-transition handoff buffer",
	);
	...
	assert.deepEqual(
		budget
			...
			.map(({ previousState, reason, state, trigger }) => ({
				previousState,
				reason,
				state,
				trigger,
...
test("ProviderBudgetController: gates by request budget before pacing", async () => {
	...
	assert.deepEqual(
		sleeps,
		[1000],
		"second request did not pay pacing wait after budget exhaustion",
	);
});
...
test("ProviderBudgetController: wall-clock budget returns planned defer metadata", async () => {
	let nowMs = 10_000;
	const budget = new ProviderBudgetController({
		runBudget: { maxWallClockMs: 500, now: () => nowMs },
	});
...
test("ProviderBudgetController: throttle ratchets pacing without double-paying already slept Retry-After", async () => {
	...
	assert.notEqual(
		sleeps.at(-1),
		5000,
		"Retry-After already slept by retry layer is not paid again by pacing",
	);
});
