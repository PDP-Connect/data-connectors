 ...
 */
const PASSKEY_ENROLLMENT_URL_RE =
	/^https:\/\/accounts\.heb\.com\/interaction\/[^/]+\/passkey_registration\b/i;
/**
 ...
 */
const PASSKEY_DECLINE_SELECTOR =
 ...
 */
const VERIFICATION_CODE_RE =
	/\b(verification code|security code|one[- ]time code|code sent)\b/i;
const CAPTCHA_RE = /\b(captcha|verify you are human|security check)\b/i;
const AUTHENTICATED_ORDERS_EVIDENCE_RE =
	/data-qe-id="orderResults"|data-testid="no-orders-message"/i;
 ...
 */
const INTERSTITIAL_DIALOG_SELECTOR =
	'[data-component="modal-content"][role="dialog"][aria-modal="true"]';
/**
 ...
 */
const LOGIN_INTERACTION_URL_RE =
	/^https:\/\/accounts\.heb\.com\/interaction\/[^/]+\/login\/?(?:[?#]|$)/i;
 ...
 */
async function countsMatchLoginForm(
	root: Locator,
	...
	const submitCount = await countUsableCandidates(
		root.locator(SUBMIT_SELECTOR),
	);
	if (submitCount !== 1) {
		return false;
	}
	...
	const presentPasswordCount = await passwordLocator
		.count()
		.catch((): number => 0);
	...
	const codeCount = await countUsableCandidates(
		root.locator(VERIFICATION_CODE_SELECTOR),
	);
	return codeCount === 0;
}
...
async function resolveUniqueVerificationCodeFormRoot(
	page: Page,
 ...
 */
async function resolvePasskeyDeclineControl(
	page: Page,
): Promise<Locator | null> {
...
async function hasUsableVerificationCodeInput(page: Page): Promise<boolean> {
	const digitCount = await countUsableCandidates(
		page.locator(VERIFICATION_CODE_SELECTOR),
	);
...
async function hasDismissibleInterstitial(page: Page): Promise<boolean> {
	...
	return (
		(await countUsableCandidates(page.locator(INTERSTITIAL_CLOSE_SELECTOR))) > 0
	);
}
...
async function dismissInterstitials(
	...
	for (
		let attempt = 0;
		attempt < MAX_INTERSTITIAL_DISMISS_ATTEMPTS;
		attempt += 1
	) {
		...
		const clicked = await clickWhenUsable(
			page,
			page.locator(INTERSTITIAL_CLOSE_SELECTOR),
			{
 ...
 */
function passkeyEnrollmentDeclineError(
	reason: "control_unavailable" | "still_on_enrollment_page",
): string {
...
async function observeUnknownSurface(
	page: Page,
...
async function inspectPostSubmitAuthSurface(
	page: Page,
): Promise<HebAuthSurface> {
	const url = page.url();
	const html = await page.content().catch((): string => "");
	if (!html) {
		return "unknown";
...
async function probeOrdersPage(
	page: Page,
	checkpoint?: SessionCheckpointFn,
): Promise<HebAuthSurface> {
	await page
...
async function reProbeAfterManualAction(
	page: Page,
	checkpoint?: SessionCheckpointFn,
): Promise<boolean> {
...
type PostSubmitAuthOutcome =
	...
	| {
			kind: "challenge";
			surface: Exclude<HebAuthSurface, "live" | "login_form" | "unknown">;
	  }
	| {
			kind: "passkey_enrollment_decline_failed";
			reason: "control_unavailable" | "still_on_enrollment_page";
	  }
	...
	while (clock.now() - startedAt <= POST_SUBMIT_TIMEOUT_MS) {
		...
		if (surface === "passkey_enrollment") {
			...
			if (declineAttempts > MAX_PASSKEY_DECLINE_ATTEMPTS) {
				return {
					kind: "passkey_enrollment_decline_failed",
					reason: "still_on_enrollment_page",
				};
			}
...
async function handleVerifiedLoginFormSubmission({
	...
	const submitted = await submitVerifiedLoginForm(
		loginFormRoot,
		page,
		username,
		password,
	...
	if (
		postSubmitSurface.kind === "challenge" &&
		postSubmitSurface.surface === "verification_code"
	) {
...
async function fillSplitVerificationCodeDigits(
	...
	for (let i = 0; i < digitCount; i += 1) {
		const filled = await fillWhenUsable(
			page,
			verificationCode.nth(i),
			digits[i] ?? "",
			{ timeout: 3000 },
		);
		if (!filled) {
...
async function clickVerifySubmit(
	page: Page,
	submit: Locator,
): Promise<boolean> {
...
async function submitVerificationCodeForm(
	root: Locator,
	page: Page,
	code: string,
): Promise<boolean> {
	...
	const codeFilled =
		codeCount > 1
			? await fillSplitVerificationCodeDigits(
					page,
					verificationCode,
					code,
					codeCount,
...
async function handleVerificationCodeSubmission({
	...
	const resp = await sendInteraction({
		kind: "otp",
		message:
			"H-E-B sent a verification code. Reply with the code to continue signing in.",
		schema: {
			type: "object",
	...
	const verificationCodeRoot = await waitForUniqueVerificationCodeFormRoot(
		page,
		waitClock,
	);
	...
	const submitted = await submitVerificationCodeForm(
		verificationCodeRoot,
		page,
		code,
	);
	if (!submitted) {
...
export async function probeHebSession(
	page: Page,
	checkpoint?: SessionCheckpointFn,
): Promise<boolean> {
	return (await probeOrdersPage(page, checkpoint)) === "live";
...
async function waitForUniqueVerificationCodeFormRoot(
	page: Page,
	clock: PostSubmitWaitClock,
): Promise<Locator | null> {
