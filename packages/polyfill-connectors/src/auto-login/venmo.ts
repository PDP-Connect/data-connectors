...
export const VENMO_POST_SUBMIT_PROBE_TRANSPORT_ERROR =
...
export function isVenmoSignInUrl(url: string): boolean {
	...
	return (
		parsed.protocol === "https:" && parsed.hostname === VENMO_IDENTITY_HOST
	);
}
/**
...
const SMS_OFFER_REMEMBER_DEVICE_SELECTOR =
	'.venmoRemeberMeDevice input[type="checkbox"]';
const SMS_OFFER_SEND_CODE_SELECTOR =
	'button[data-testid="button-next"][name="Send code"][type="submit"]';
...
const LOGIN_LOCATOR_PROBES: LocatorProbe[] = [
	...
	},
	{
		...
		description:
			"Venmo's two-step identifier ('Next') control; absence means a one-screen or unrecognized form.",
	},
	{
		id: "captcha",
		kind: "css",
		...
		description:
			"Human-verification frames; presence explains a stalled password step but never blocks a healthy one.",
	},
];
...
async function submitVenmoSmsOffer(page: Page): Promise<void> {
	...
	if (
		!(
			(await locatorIsVisible(rememberDevice)) &&
			(await rememberDevice.isEnabled().catch((): boolean => false))
		)
	) {
		throw new Error(
			`${VENMO_SMS_OFFER_CHECKBOX_UNAVAILABLE}: Remember this device is not usable`,
		);
	}
	const remembered = await rememberDevice.isChecked().catch((): null => null);
	if (remembered === null) {
		throw new Error(
			`${VENMO_SMS_OFFER_CHECKBOX_UNAVAILABLE}: could not read Remember this device`,
		);
	}
	if (!remembered) {
		await rememberDevice.click().catch((error: unknown): never => {
			throw new Error(
				`${VENMO_SMS_OFFER_CHECKBOX_UNAVAILABLE}: could not select Remember this device`,
				{
					cause: error,
				},
			);
		...
		if (!(await rememberDevice.isChecked().catch((): boolean => false))) {
			throw new Error(
				`${VENMO_SMS_OFFER_CHECKBOX_UNAVAILABLE}: Remember this device did not stay selected`,
			);
		}
	}
	...
	if (
		!(
			(await locatorIsVisible(sendCode)) &&
			(await sendCode.isEnabled().catch((): boolean => false))
		)
	) {
		throw new Error(
			`${VENMO_SMS_OFFER_SEND_CODE_UNAVAILABLE}: Send code is not usable`,
		);
	}
	await sendCode.click().catch((error: unknown): never => {
		throw new Error(
			`${VENMO_SMS_OFFER_SEND_CODE_UNAVAILABLE}: could not send the code`,
			{ cause: error },
		);
	});
 ...
 * exact.
 */
async function clickVenmoLoginSubmit(
	page: Page,
 ...
 */
async function pageLoadedInvisibleScoringChallenge(
	page: Page,
): Promise<boolean> {
	return await page
		.locator(
			'script[src*="recaptchav3"], iframe[src*="grcenterprise_v3"], script[src*="grcenterprise_v3"]',
		)
		.first()
		.count()
 ...
 */
async function waitForVenmoPasswordScreen(
	page: Page,
	timeoutMs: number,
): Promise<boolean> {
	return await page
 ...
 */
async function waitForActionableHandoffSurface(
	page: Page,
	timeoutMs: number,
): Promise<boolean> {
	return await page
...
> & {
	...
	const painted = await waitForActionableHandoffSurface(
		page,
		HANDOFF_PAINT_TIMEOUT_MS,
	);
	if (!painted) {
 ...
 */
async function withSuppressedManualPrompts<T>(
	run: () => Promise<T>,
): Promise<T> {
...
> & {
	...
	return await waitForManualLogin({
		...
		...(replaySignIn
			? {
					resumeFill: async () => {
						await withSuppressedManualPrompts(() =>
							resumeSignInAfterChallenge(replaySignIn),
						);
					},
				}
			: {}),
 ...
 */
function challengeHandoffMessage(
	reason: string,
...
async function handleVenmoOtpIfPresent(
	...
	const smsOfferPresent =
		SMS_OFFER_TEXT_RE.test(bodyText) &&
		(await locatorIsVisible(
			page.locator(SMS_OFFER_SEND_CODE_SELECTOR).first(),
		));
	if (
		!(usableOtpInput || OTP_PROMPT_TEXT_RE.test(bodyText) || smsOfferPresent)
	) {
		return null;
	}
...
> & {
	...
	// clothes.
	const settleOutcome = await pollForLiveSessionAfterSubmit(
		page,
		postSubmitSettleTimeoutMs,
	);
	if (!settleOutcome.settled) {
		...
		throw new Error(
			`venmo_login_did_not_complete: no sign-in outcome within ${postSubmitSettleTimeoutMs}ms`,
		);
	}
