...
function makePostSubmitWaitClock(page: Page): {
	now: () => number;
...
function controlLocator(
	...
	function triggerSubmit(): void {
		...
		const canSucceed =
			...
			Boolean(codeValue || (form.values.email && form.values.password));
		...
		fill: (value: string): Promise<void> => {
			...
			if (kind === "email") {
				...
				if (form.codeControls.length > 1) {
					const digits =
						form.values.codeDigits ??
						new Array(form.codeControls.length).fill("");
...
function controlListLocator(
	...
	> = {
		...
		first(): Locator {
			return controls[0]
				? controlLocator(form, formIndex, kind, 0)
				: emptyLocator();
		},
		innerText: async (): Promise<string> => "",
		inputValue: async (): Promise<string> => "",
	...
	> = {
		...
		first(): Locator {
			return state.buttons[0]
				? buttonLocator(state.buttons[0])
				: emptyLocator();
		},
		innerText: async (): Promise<string> => "",
		inputValue: async (): Promise<string> => "",
		isEnabled: async (): Promise<boolean> =>
			state.buttons.some((b) => b.enabled && b.visible),
		isVisible: async (): Promise<boolean> =>
			state.buttons.some((b) => b.visible),
		locator(): Locator {
			return emptyLocator();
		},
		press: (): Promise<void> => Promise.resolve(),
...
function pageControlsLocator(kind: ControlKind): Locator {
	...
	function at(index: number): Locator {
		...
		return entry
			? controlLocator(entry.form, entry.formIndex, kind, entry.controlIndex)
			: emptyLocator();
	}
	const locator: Pick<
		Locator,
	...
	> = {
		...
		inputValue: async (): Promise<string> => "",
		isEnabled: async (): Promise<boolean> =>
			entries().some(({ control }) => control.enabled && control.visible),
		isVisible: async (): Promise<boolean> =>
			entries().some(({ control }) => control.visible),
		locator(): Locator {
			return emptyLocator();
		},
		press: (): Promise<void> => Promise.resolve(),
...
function formsLocator(): Locator {
	...
	> = {
		...
		inputValue: async (): Promise<string> => "",
		isEnabled: async (): Promise<boolean> =>
			state.forms.some((form) => form.enabled && form.visible),
		isVisible: async (): Promise<boolean> =>
			state.forms.some((form) => form.visible),
		locator(): Locator {
			return emptyLocator();
		},
		press: (): Promise<void> => Promise.resolve(),
...
function makePage(initial: FakePageInit = {}): Page {
	...
	state = {
		...
		postSubmitOutcomes:
			initial.postSubmitOutcomes ??
			(initial.postSubmitOutcome ? [initial.postSubmitOutcome] : []),
		submitClicks: 0,
		title: initial.title ?? "",
...
test("ensureHebSession declines the post-submit passkey-enrollment upsell and continues WITHOUT any OTP prompt", async () => {
	await withHebCredentials(async () => {
		...
		const harness = makeInteractionHarness({
			makeSessionLiveOnManualAction: false,
		});
		...
		assert.equal(
			...
			0,
			"the enrollment upsell must never prompt the owner",
		);
		assert.equal(
		...
		assert.equal(
			state.declineClicks,
			1,
			"the decline control must be clicked exactly once",
		);
...
test("ensureHebSession fails honestly when the passkey decline click does not take effect — bounded, no OTP, no spin", async () => {
	await withHebCredentials(async () => {
		...
		assert.equal(
			state.enrollClicks,
			0,
			"PDPP must never click Add passkey, even while retrying",
		);
		// Bounded: retries stop rather than spinning for the full timeout.
		assert.ok(state.declineClicks >= 1);
		assert.ok(
			state.declineClicks <= 4,
			`decline retries must be bounded, saw ${state.declineClicks}`,
		);
		assert.equal(state.live, false);
	});
});
...
test("a genuine verification-code surface on the accounts.heb.com interaction host STILL prompts for OTP", async () => {
	await withHebCredentials(async () => {
		...
		assert.equal(
			harness.requests[0]?.kind,
			"otp",
			"real challenges must be unaffected by the passkey fix",
		);
...
test("the login-method chooser never prompts for an OTP — a radio label offering a code is not a challenge", async () => {
	await withHebCredentials(async () => {
		const page = makePage({
			...
			forms: [
				createForm({ codeControls: [], submitControls: [createControl(true)] }),
			],
		...
		assert.equal(
			ok,
			true,
			"the already-selected password path must carry the sign-in through",
		);
		assert.equal(state.live, true);
	});
});
...
test("prose mentioning a one-time code with no code input never prompts — text alone is not evidence", async () => {
	await withHebCredentials(async () => {
		...
		const handoffs = harness.requests.filter(
			(req) => req.kind === "manual_action",
		);
		assert.equal(
			handoffs.length,
			1,
			"the honest path is a browser handoff, not an invented secret",
		);
...
test("passkey-enrollment detection does not fire on a lookalike URL or on enrollment copy alone", async () => {
	...
	assert.equal(
		state.declineClicks,
		0,
		"URL is required — marketing copy alone must not trigger a decline",
	);
		...
		const ok = await ensureHebSession({
			...
			onCredentialSubmit: () => {
				...
				assert.equal(
					state.submitClicks,
					1,
					"the marker must fire after the submit click, never before it",
				);
			},
			page,
			postSubmitWaitClock: makePostSubmitWaitClock(page),
...
test("ensureHebSession does not fast-path a loading orders page as a live session", async () => {
	...
	assert.match(
		harness.requests[0]?.message ?? "",
		/did not render the expected login form|open the secure browser/i,
	);
});
...
test("ensureHebSession does not treat orders URL login/loading/challenge bodies as live", async () => {
	await withHebCredentials(async () => {
		...
		for (const { expectedMessage, postSubmitOutcomes } of cases) {
			...
			assert.doesNotMatch(
				harness.requests[0]?.message ?? "",
				/owner@example\.com|synthetic-password/,
			);
			assert.equal(state.gotoEvents.length, 2);
			assert.ok(
				state.gotoEvents[1]?.atMs !== undefined &&
					state.gotoEvents[1].atMs >= 200,
			);
		}
	});
});
		...
		const harness = makeInteractionHarness({
			responseForRequest: (req: InteractionRequest): InteractionResponse => {
				...
				state.onWaitForTimeout = () => {
					state.forms = [
						createForm({
							codeControls: [createControl(true)],
							submitControls: [],
						}),
					];
...
function splitCodeForm(): FakeFormState {
	return createForm({
		...
		submitControls: [
			createControl(true, true, "Verify"),
			createControl(true, true, "Back"),
		],
	});
}
	...
	function domLocator(selector: string, root?: Element): Locator {
		...
		function makeFor(index: number): Locator {
			const locator: Partial<Locator> = {
					...
					if (el) {
						...
						if (el.getAttribute("data-qe-id") === "modalClose") {
							const cover = doc.querySelector(
								'[data-component="modal-cover-core"]',
							);
				...
				inputValue: async (): Promise<string> => "",
				isEnabled: async (): Promise<boolean> =>
					!nthEl(index)?.hasAttribute("disabled"),
				isVisible: async (): Promise<boolean> => isElementVisible(nthEl(index)),
				locator: (sub: string): Locator =>
					domLocator(sub, nthEl(index) ?? undefined),
				press: (): Promise<void> => Promise.resolve(),
			};
		...
		const listLocator: Partial<Locator> = {
			...
			inputValue: async (): Promise<string> => "",
			isEnabled: async (): Promise<boolean> =>
				matches().some((el) => !el.hasAttribute("disabled")),
			isVisible: async (): Promise<boolean> =>
				matches().some((el) => isElementVisible(el)),
...
test("promotional interstitial over a live session is dismissed without involving the owner", async () => {
	...
	assert.ok(
		domState.clicks.includes("modalClose"),
		"expected the close control to be clicked",
	);
	assert.ok(
		checkpoints.includes("heb-interstitial-dismissing"),
		"expected a dismissal checkpoint",
	);
	assert.ok(
		!checkpoints.includes("heb-unclassified-surface"),
		"a dismissed promo must not be an unclassified surface",
	);
});
...
test("an OTP-only login form (no password input at all) classifies as login_form, not as an unknown surface", async () => {
	...
	const message = String(
		(harness.requests[0] as { message?: string }).message ?? "",
	);
...
test("a genuinely unrecognized surface reports what was observed instead of asserting a cause", async () => {
	...
	assert.ok(
		checkpoints.includes("heb-unclassified-surface"),
		"expected an unclassified-surface diagnostic",
	);
});
...
test("an interstitial that cannot be dismissed degrades to an honest unknown, never a false login claim", async () => {
	...
	const undismissable = WHATS_NEW_MODAL_HTML.replace(
		/data-qe-id="modalClose"/,
		'data-qe-id="notTheCloseButton"',
	);
	const { page } = makeDomPage({
		html: undismissable,
		title: "Your orders | HEB.com",
