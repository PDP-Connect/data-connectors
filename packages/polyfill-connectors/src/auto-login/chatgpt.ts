 ...
 */
export function resolveChatGptPushApprovalTimeoutMs(
...
function pushApprovalPollAttempts(
	env: NodeJS.ProcessEnv = process.env,
): number {
	return Math.max(
		1,
		Math.ceil(
			resolveChatGptPushApprovalTimeoutMs(env) / PUSH_APPROVAL_POLL_INTERVAL_MS,
		),
	);
}

export function resolveChatGptBrowserLoginTimeoutMs(
	env: NodeJS.ProcessEnv = process.env,
): number {
...
function browserLoginPollAttempts(
	env: NodeJS.ProcessEnv = process.env,
): number {
	return Math.max(
		1,
		Math.ceil(
			resolveChatGptBrowserLoginTimeoutMs(env) / BROWSER_LOGIN_POLL_INTERVAL_MS,
		),
	);
}

export function chatGptPushApprovalAssistance(
	env: NodeJS.ProcessEnv = process.env,
): AssistanceRequest {
	return {
...
export function chatGptBrowserLoginAssistance(
	env: NodeJS.ProcessEnv = process.env,
): AssistanceRequest {
	return {
...
export function interactionResponseCode(
	resp: InteractionResponse,
): string | null {
	return resp.data?.code ?? resp.value ?? null;
}

function chatGptAuthProbeDiagnosticMessage(
	diagnostic: ChatGptAuthProbeDiagnostic,
): string {
...
function sessionAssistanceHooks(
	hooks: SessionAssistanceHookInput,
): SessionAssistanceHooks {
...
export function chatGptAllowsInteractiveAuthRepair(
	env: NodeJS.ProcessEnv = process.env,
): boolean {
...
async function checkLoggedInViaDOMDetails(
	page: Page,
): Promise<ChatGptDomSessionProbe> {
	try {
...
export function isLikelyChatGptPushApprovalText(text: string): boolean {
	...
	const hasDeviceCopy =
		normalized.includes("chatgpt app") || normalized.includes("your devices");
	const hasResendPrompt = normalized.includes("resend prompt");
	const hasEmailFallback = normalized.includes("try with email");
	return (
		(hasApproveHeading &&
			(hasNotificationCopy || hasDeviceCopy || hasResendPrompt)) ||
...
async function isChatGptSessionActiveFromContext(page: Page): Promise<boolean> {
	try {
		const response = await page
			.context()
			.request.get("https://chatgpt.com/api/auth/session", { timeout: 10_000 });
		if (!response.ok()) {
			return false;
		}
		const body: unknown = await response.json();
		return Boolean(
			body && typeof body === "object" && "user" in body && body.user,
		);
	} catch {
		return false;
	}
...
async function isChatGptSessionActive(
	page: Page,
	options: { allowOriginProbe?: boolean } = {},
): Promise<boolean> {
	if (await isChatGptSessionActiveOnPage(page)) {
		return true;
	}
	if (
		!options.allowOriginProbe ||
		page.url().startsWith(CHATGPT_HOME_URL) ||
		!(await isChatGptSessionActiveFromContext(page))
	) {
		return false;
	}
	// The context request shares the browser cookies and proves the session
	// without creating a competing tab. Once live, return the owner page to
	// ChatGPT so collection starts on its own origin.
	await page
		.goto(CHATGPT_HOME_URL, { waitUntil: "domcontentloaded", timeout: 30_000 })
		.catch((): undefined => undefined);
	return await isChatGptSessionActiveOnPage(page);
}

async function verifyChatGptSessionAfterOwnerAction(
	page: Page,
): Promise<boolean> {
	// The owner may finish device approval on auth.openai.com. Navigation is
	// safe only after the explicit manual action has completed.
	if (!page.url().startsWith(CHATGPT_HOME_URL)) {
		await page
...
async function navigateAndProbeSession(
	...
	await progress?.(
		chatGptAuthProbeDiagnosticMessage({
			...
			decision: apiSessionUser
				? "accepted_by_api_session"
...
async function pollSessionReadiness({
	...
	for (let attempt = 0; attempt < attempts; attempt += 1) {
		...
		if ((attempt + 1) % 12 === 0) {
			await checkpoint?.(
				`${waitingCheckpointDetailPrefix ?? waitingCheckpointPrefix}-${String(attempt + 1)}`,
			);
		}
		const allowOriginProbe = attempt === 0 || (attempt + 1) % 6 === 0;
...
> & {
	...
	await page.waitForTimeout(3000);
	return await verifyChatGptSessionAfterOwnerAction(page);
}
...
>): Promise<boolean> {
	...
	if (
		...
		if (assistanceRequestId && completeAssistance) {
			await completeAssistance(assistanceRequestId, "resolved", {
				message:
					"The external approval completed and the connector is continuing.",
			});
		}
	...
	if (assistanceRequestId && completeAssistance) {
		await completeAssistance(assistanceRequestId, "escalated", {
			message:
				"External approval did not complete automatically; falling back to browser confirmation.",
		});
	}
...
> & {
	...
	const hooks = sessionAssistanceHooks({
		assist,
		capture,
		checkpoint,
		completeAssistance,
...
> & {
	...
	return await handleBrowserLoginAssistance({
		...
		...(credentialReason
			? {
					message: `${credentialReason} ${CHATGPT_BROWSER_LOGIN_ASSISTANCE_MESSAGE}`,
				}
			: {}),
		page,
		...(progress ? { progress } : {}),
