...
test("interactionResponseCode reads orchestrator otp responses", () => {
	assert.equal(
		interactionResponseCode(response({ data: { code: "123456" } })),
		"123456",
	);
});
...
test("interactionResponseCode preserves legacy value responses", () => {
	assert.equal(
		interactionResponseCode(response({ value: "654321" })),
		"654321",
	);
});
...
test("isLikelyChatGptPushApprovalText ignores ordinary login copy", () => {
	assert.equal(
		isLikelyChatGptPushApprovalText("Log in to ChatGPT Continue with password"),
		false,
	);
});
...
test("ChatGPT push approval copy separates external approval from browser control", () => {
	assert.match(
		CHATGPT_PUSH_APPROVAL_PROGRESS_MESSAGE,
		/continue automatically/i,
	);
	assert.doesNotMatch(
		CHATGPT_PUSH_APPROVAL_PROGRESS_MESSAGE,
		/streaming companion|run interaction controls/i,
	);
	assert.match(
		CHATGPT_PUSH_APPROVAL_FALLBACK_MESSAGE,
		/open the ChatGPT app to approve it/i,
	);
});
...
test("ChatGPT push approval checkpoints while polling so the session watchdog sees progress", async () => {
	...
	const page = {
		...
		locator: (selector: string) => {
			...
			if (
				selector.includes('input[name="code"]') ||
				selector.includes('input[type="tel"]')
			) {
	...
	const ok = await ensureChatGptSession({
		...
		sendInteraction: (req) =>
			Promise.resolve(
				response({ request_id: req.request_id ?? "interaction_1" }),
			),
	});
...
test("ChatGPT initial auth probe survives ONE transient /api/auth/session blip on an otherwise-valid session", async () => {
	...
	const page = {
		evaluate: (fn: (...args: never[]) => unknown) => {
			...
			if (source.includes("/api/auth/session")) {
				...
				// never actually dead.
				return Promise.resolve(
					sessionProbeCount === 1 ? null : { user: { id: "owner" } },
				);
			}
			if (source.includes("querySelectorAll")) {
				return Promise.resolve({
		...
		getByRole: () => {
			loginOpened = true;
			throw new Error(
				"login path should not be reached — the retry should have recovered the session",
			);
		},
		goto: (url: string) => {
			if (url.includes("/auth/login")) {
	...
	assert.equal(
		ok,
		true,
		"a session that recovers within the retry budget must count as active",
	);
	assert.equal(
		loginOpened,
		false,
		"the credential-login UI must never be opened for a transient blip",
	);
	assert.equal(
		assistanceRequested,
		false,
		"no owner assistance should ever be requested for a healthy session",
	);
	assert.equal(
		sessionProbeCount,
		2,
...
test("ChatGPT auth repair policy only allows owner-started manual runs by default", () => {
	...
	assert.equal(
		chatGptAllowsInteractiveAuthRepair({ PDPP_RUN_TRIGGER_KIND: "manual" }),
		true,
	);
	assert.equal(
		chatGptAllowsInteractiveAuthRepair({ PDPP_RUN_TRIGGER_KIND: "scheduled" }),
		false,
	);
	assert.equal(
		chatGptAllowsInteractiveAuthRepair({ PDPP_RUN_TRIGGER_KIND: "retry" }),
		false,
	);
	assert.equal(
		chatGptAllowsInteractiveAuthRepair({ PDPP_RUN_TRIGGER_KIND: "webhook" }),
		false,
	);
});
	...
	await withEnvValues(
		...
		async () => {
			...
			const page = {
				...
				getByRole: () => {
					throw new Error(
						"scheduled auth repair must not open or click the login form",
					);
				},
				goto: (url: string) => {
				...
				locator: () => {
					throw new Error(
						"scheduled auth repair must not inspect credential form fields",
					);
				},
				url: () => "https://chatgpt.com/",
				waitForTimeout: async () => undefined,
			...
			const page = {
				evaluate: (fn: (...args: never[]) => unknown) => {
					...
					if (source.includes("/api/auth/session")) {
						...
						// becoming active.
						return Promise.resolve(
							sessionProbeCount >= 5 ? { user: { id: "owner" } } : null,
						);
					}
					if (source.includes("querySelectorAll")) {
						return Promise.resolve({
		...
		async () => {
			...
			const page = {
				...
				getByText: (text: RegExp) => ({
					...
					waitFor: () =>
						passwordSubmitted &&
						text.test("Incorrect email address or password")
							? Promise.resolve()
			...
			await assert.rejects(
				ensureChatGptSession({
					assist: async () => {
						await Promise.resolve();
						throw new Error(
							"rejected stored password must not request owner assistance",
						);
					},
					context: {} as never,
					credentials: {
					...
					onCredentialSubmit: () => {
						...
						assert.equal(
							passwordSubmitted,
							true,
							"marker must fire only after the password submit click",
						);
					},
					page: page as never,
					progress: () => Promise.resolve(),
					sendInteraction: async () => {
						await Promise.resolve();
						throw new Error(
							"rejected stored password must not ask for push approval or OTP",
						);
					},
				}),
			...
			assert.equal(
				markerCount,
				1,
				"one stored-credential attempt — the runtime must hear exactly one submit",
			);
		},
	);
});
...
test("resolveChatGptPushApprovalTimeoutMs honors a positive env override and falls back otherwise", () => {
	...
	assert.equal(
		resolveChatGptPushApprovalTimeoutMs({
			PDPP_CHATGPT_PUSH_APPROVAL_TIMEOUT_MS: "60000",
		}),
		60_000,
	);
	...
	assert.equal(
		resolveChatGptPushApprovalTimeoutMs({
			PDPP_CHATGPT_PUSH_APPROVAL_TIMEOUT_MS: "0",
		}),
		900_000,
	);
	assert.equal(
		resolveChatGptPushApprovalTimeoutMs({
			PDPP_CHATGPT_PUSH_APPROVAL_TIMEOUT_MS: "nope",
		}),
		900_000,
	);
});
...
test("chatGptPushApprovalAssistance derives timeout_seconds from the configured budget", () => {
	...
	const assistance = chatGptPushApprovalAssistance({
		PDPP_CHATGPT_PUSH_APPROVAL_TIMEOUT_MS: "300000",
	});
