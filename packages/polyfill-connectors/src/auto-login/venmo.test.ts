...
function makeProbePage(initialLive: boolean): {
	gotoUrls: string[];
	page: Page;
...
function makeFillRecordingLocator(
	...
	const fake: Pick<
		Locator,
		"click" | "count" | "fill" | "first" | "innerText" | "isVisible" | "waitFor"
	> = {
		click: (): Promise<void> => Promise.resolve(),
		count: (): Promise<number> => Promise.resolve(count),
...
function makePageWithWorkingLoginForm(): {
...
async function withVenmoCredentials(run: () => Promise<void>): Promise<void> {
	await withVenmoCredentialValues(
		{ password: "test-password", username: "test-user" },
		run,
	);
}

async function withoutVenmoCredentials(
	run: () => Promise<void>,
): Promise<void> {
...
test("ensureVenmoSession: a live session is reused with zero interactions and no form fill", async () => {
	...
	assert.equal(
		requests.length,
		0,
		"a live session must not prompt the owner at all",
	);
...
test("ensureVenmoSession: hands off to manual_action when no credentials are saved, with no password/username leaked", async () => {
	await withoutVenmoCredentials(async () => {
		...
		assert.match(
			requests[0]?.message ?? "",
			/No optional Venmo sign-in details/,
		);
...
test("ensureVenmoSession: the owner's response triggers NO navigation — the authenticated page they just created is never destroyed (run_1787918248525)", async () => {
	await withoutVenmoCredentials(async () => {
		...
		assert.notEqual(
			gotosAtResponse,
			-1,
			"the manual_action handoff must actually have been requested",
		);
		assert.equal(
			gotoUrls.length,
			gotosAtResponse,
...
test("ensureVenmoSession: no page-context probe runs after the owner responds (the probe's fetch is what forced the navigation)", async () => {
	await withoutVenmoCredentials(async () => {
		...
		assert.equal(
			evaluatesAfterResponse,
			0,
			"the post-handoff liveness probe must be gone, not merely made careful",
		);
	});
});
...
test("ensureVenmoSession: captures a locator probe of the login page, recording which selector candidates matched", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.ok(
			probeCalls.length > 0,
			"expected at least one locator-probe capture during login",
		);
		assert.ok(
...
test("ensureVenmoSession: fills saved credentials and completes login without an OTP prompt when none renders", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.equal(
			requests.length,
			0,
			"no OTP interaction when Venmo never rendered one",
		);
	});
});
...
test("ensureVenmoSession: onCredentialSubmit fires exactly once when the saved password is submitted", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.equal(
			markerCount,
			1,
			"one login attempt submitted one credential — the runtime must hear about it once",
		);
	});
});
...
test("mutation-kill twin: omitting progress from ensureVenmoSession still succeeds (the hook is additive, not load-bearing for login control flow)", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.equal(
			result?.live,
			true,
			"ensureVenmoSession must not require a progress callback to function",
		);
	});
});
...
test("ensureVenmoSession: an OTP input drives sendInteraction with kind=otp, never asking for the password again, and reports the owner-handoff sentinel rather than a fabricated probe", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.equal(
			probeCount,
			1,
			"only the initial pre-submit probe may run; nothing may probe after the OTP response",
		);
	});
});
...
test("ensureVenmoSession: a code-bearing OTP response triggers no page-context probe or navigation after it resolves", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.equal(
			result,
			null,
			"a submitted OTP code must report the owner-handoff sentinel",
		);
		assert.equal(requests[0]?.kind, "otp");
		assert.deepEqual(
			filledCodes,
			["123456"],
			"the OTP locator must be filled with the owner-supplied code",
		);
		assert.equal(
		...
		assert.equal(
			evaluatesAfterResponse,
			0,
			"no page-context probe may run once the OTP response resolves",
		);
		assert.equal(
			gotosAfterResponse,
			0,
			"no navigation may run once the OTP response resolves",
		);
	});
});
...
test("ensureVenmoSession: cancelling/dismissing the OTP prompt triggers no page-context probe or navigation, and reports the owner-handoff sentinel rather than fabricating proof", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.equal(
			evaluatesAfterResponse,
			0,
			"no page-context probe may run once the cancel/dismiss response resolves",
		);
		assert.equal(
			gotosAfterResponse,
			0,
			"no navigation may run once the cancel/dismiss response resolves",
		);
	});
});
...
function makeOtpDecisionPage({
	...
	let currentUrl = "https://venmo.com/";
	const submit = makeLocator();
	const otpLocator = (): Locator =>
		makeLocator({
			count: otpCount,
			enabled: otpEnabled,
			visible: state.otpVisible,
		});
	const page: Pick<
		Page,
		| "evaluate"
...
function makeCapturedSmsOfferPage({
	...
	const rememberDevice: Pick<
		Locator,
		"click" | "count" | "first" | "isChecked" | "isEnabled" | "isVisible"
	> = {
		click: (): Promise<void> => {
			checkboxClicks += 1;
		...
		count: (): Promise<number> =>
			Promise.resolve(stage === "offer" && offerVisible ? 1 : 0),
		first(): Locator {
			return rememberDevice as Locator;
		},
		isChecked: (): Promise<boolean> => Promise.resolve(rememberDeviceChecked),
		isEnabled: (): Promise<boolean> =>
			Promise.resolve(stage === "offer" && offerVisible),
		isVisible: (): Promise<boolean> =>
			Promise.resolve(stage === "offer" && offerVisible),
	};
	const sendCode: Pick<
		Locator,
		"click" | "count" | "first" | "isEnabled" | "isVisible"
	> = {
		click: (): Promise<void> => {
			sendCodeClicks += 1;
	...
	> = {
		...
		locator(selector: string): Locator {
			...
			if (
				selector.includes('data-testid="button-next"') &&
				selector.includes('[name="Send code"]')
			) {
				return sendCode as Locator;
			}
			if (selector.includes("otp") || selector.includes("code")) {
...
test("ensureVenmoSession: the captured SMS offer selects Remember this device, sends its code, then uses the existing OTP interaction", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.equal(
			result,
			null,
			"a submitted OTP code must report the owner-handoff sentinel, not a probed result",
		);
		assert.equal(
			fake.checkboxClicks,
			1,
			"the unchecked captured control must be selected before code dispatch",
		);
		assert.equal(
			fake.isRememberDeviceChecked(),
			true,
			"Remember this device must stay selected",
		);
		assert.equal(
			fake.sendCodeClicks,
			1,
			"the real captured Send code control must dispatch exactly once",
		);
		assert.deepEqual(
			fake.filledCodes,
			["654321"],
...
test("ensureVenmoSession: the already-checked captured Remember this device control is never clicked off", async () => {
	await withVenmoCredentials(async () => {
		const fake = makeCapturedSmsOfferPage({
			rememberDeviceInitiallyChecked: true,
		});
		const result = await ensureVenmoSession({
			credentials: {
				VENMO_PASSWORD: "test-password",
		...
		assert.equal(
			fake.checkboxClicks,
			0,
			"a checked Remember this device control must not be clicked back off",
		);
...
test("ensureVenmoSession: a Send code button without the captured verification offer copy does not dispatch or fabricate an OTP request", async () => {
	await withVenmoCredentials(async () => {
		const fake = makeCapturedSmsOfferPage({
			bodyText: "Account settings Send code",
		});
		const { requests, sendInteraction } = recordingSendInteraction();
		...
		assert.equal(
			fake.sendCodeClicks,
			0,
			"a lookalike button is not enough evidence to send a real SMS",
		);
		assert.deepEqual(
...
test("ensureVenmoSession: a dispatched SMS offer with no rendered code input fails with its named reason and never prompts", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.equal(
			fake.sendCodeClicks,
			1,
			"the failure occurs after exactly one real SMS dispatch attempt",
		);
		assert.deepEqual(
			requests.filter((request): boolean => request.kind === "otp"),
			[],
...
function makeTwoStepVenmoPage({
	...
	const roleSubmit = semanticSubmitVisible
		? makeClickRecordingLocator(() => {
				advance(
					onPasswordScreen
						? "role-submit-password-screen"
						: "role-submit-identifier-screen",
				);
			})
	...
	> = {
		...
		locator(selector: string): Locator {
			if (selector === "body") {
				return makeLocator({
					innerText: "Log in Enter email, mobile, or username Next Sign up",
				});
			}
			...
			if (
				selector.includes("recaptchav3") ||
				selector.includes("grcenterprise_v3")
			) {
				// Present but never visible — v3 renders no widget at all.
				return makeLocator({
					count: scoringChallengeLoaded ? 1 : 0,
					visible: false,
				});
			}
			if (
				selector.includes("recaptcha") ||
				selector.includes("paypalobjects")
			) {
				return makeLocator({
					count: captchaVisible ? 1 : 0,
...
function makeClickRecordingLocator(onClick: () => void): Locator {
	const fake: Pick<
		Locator,
		"click" | "count" | "fill" | "first" | "isEnabled" | "isVisible" | "waitFor"
	> = {
		click: (): Promise<void> => {
			onClick();
			return Promise.resolve();
...
test("ensureVenmoSession: the live two-step page (login_email -> Next -> password) completes login with no owner handoff", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.equal(
			result?.live,
			true,
			"a recognized two-step Venmo sign-in must complete, not hand off",
		);
		assert.equal(
			fillCalls.username,
			"test-user",
			"the login_email identifier field must actually be filled",
		);
		assert.equal(
			fillCalls.password,
			"test-password",
			"the password must be filled on the SECOND screen",
		);
		assert.deepEqual(
			requests,
			[],
		...
		assert.equal(
			clicks.length,
			2,
			"exactly two submits: one to advance past the identifier, one to send the password",
		);
	});
});
...
test("ensureVenmoSession: a password screen slower than the old fixed sleep is still awaited and filled, not misread as absent", async () => {
	await withVenmoCredentials(async () => {
		const { fillCalls, page } = makeTwoStepVenmoPage({
			passwordScreenDelayMs: 2500,
		});
		const { requests, sendInteraction } = recordingSendInteraction();
		const result = await ensureVenmoSession({
			credentials: {
		...
		assert.equal(
			result?.live,
			true,
			"a slow-but-arriving password screen is a success, not a handoff",
		);
		assert.equal(
			fillCalls.password,
			"test-password",
			"the password must be filled once the screen actually renders",
		);
		assert.deepEqual(
			requests,
			[],
			"the owner must not be interrupted for a screen that simply took a moment",
		);
	});
});
...
test("ensureVenmoSession: an unnamed step-one control is advanced via #btnNext, in preference to the generic submit selector", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.deepEqual(
			requests,
			[],
			"an id-addressable step-one control needs no owner handoff",
		);
	});
});
...
test("ensureVenmoSession: a session that goes live only after the submit request settles is signed in, not handed off", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.equal(
			fillCalls.password,
			"test-password",
			"the password was accepted; only the cookie was late",
		);
		assert.deepEqual(
			requests,
			[],
...
test("ensureVenmoSession: a login that never settles fails as incomplete, never as a rejected credential", async () => {
	await withVenmoCredentials(async () => {
		...
		await assert.rejects(
			...
			(error: Error) => {
				...
				assert.match(
					error.message,
					/2000ms/,
					"the message must carry the bound the owner actually waited",
				);
				return true;
			},
		);
		assert.deepEqual(
			requests,
			[],
			"an unsettled login is not a challenge — there is nothing for the owner to do",
		);
	});
});
...
test("ensureVenmoSession: onCredentialSubmit fires exactly once on a two-step flow — at the password submit, not the identifier submit", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.equal(result?.live, true);
		assert.equal(
			markerAtClickCount.length,
			1,
			"two submits, but only ONE of them sent a credential",
		);
		assert.equal(
...
test("ensureVenmoSession: a two-step flow that stalls before the password screen never fires onCredentialSubmit", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.equal(
			markerCount,
			0,
			"no password was ever submitted, so nothing may be marked as a credential submission",
		);
	});
});
...
test("ensureVenmoSession: a password screen that never renders fails with a bounded, named error and never fabricates an OTP prompt", async () => {
	await withVenmoCredentials(async () => {
		...
		await assert.rejects(
			...
			(err: unknown) => {
				...
				assert.match(
					err.message,
					/venmo_password_screen_timeout/,
					"the failure must name its own cause",
				);
				return true;
			},
		);
		...
		assert.ok(
			...
			"the wait must respect its own bound rather than spin",
		);
		assert.deepEqual(
			requests.filter((req): boolean => req.kind === "otp"),
			[],
...
test("ensureVenmoSession: a captcha blocking the password screen hands off to the owner instead of spinning or failing silently", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.ok(
			Date.now() - startedAt < 20_000,
			"a captcha must not turn into an unbounded wait",
		);
		const manual = requests.filter(
			(req): boolean => req.kind === "manual_action",
		);
		assert.equal(
			manual.length,
			1,
			"a blocking captcha must reach the owner exactly once",
		);
		assert.match(
			manual[0]?.message ?? "",
...
test("ensureVenmoSession: a page with no username field still hands off with the existing 'sign-in form did not render' message", async () => {
	await withVenmoCredentials(async () => {
		const { fillCalls, page } = makeTwoStepVenmoPage({
			identifierSelectorMatches: false,
		});
		const { requests, sendInteraction } = recordingSendInteraction();
		// The handoff happened. It no longer probes, so it resolves with `null`
		// rather than throwing — a session that did not really sign in now fails
		...
		assert.equal(
			manual.length,
			1,
			"an unrecognized sign-in page must still reach the owner",
		);
		assert.match(
			manual[0]?.message ?? "",
		...
		assert.equal(
			fillCalls.username,
			undefined,
			"nothing may be typed into a form the connector does not recognize",
		);
...
test("ensureVenmoSession: a one-screen sign-in still completes with a single submit (COUNTERWEIGHT)", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.deepEqual(
			requests,
			[],
			"a one-screen form needs no owner involvement",
		);
	});
});
...
test("probeVenmoAccount: navigates to venmo.com first when the page starts on about:blank", async () => {
	...
	assert.equal(
		result.live,
		true,
		"the probe must actually run the fetch after navigating, not read the blank page",
	);
	assert.deepEqual(
		gotoUrls,
		["https://venmo.com/"],
		"a fresh about:blank page must navigate to venmo.com first",
	);
});
...
test("probeVenmoAccount: a goto that resolves without leaving about:blank throws a diagnosable origin-navigation fault, not a bare fetch failure", async () => {
	...
	await assert.rejects(probeVenmoAccount(page as Page), (err: unknown) => {
		...
		assert.match(
			err.message,
			/venmo_probe_transport_error/,
			"still wrapped in the probe's own phase-aware fault name",
		);
		return true;
	});
	assert.deepEqual(
		gotoUrls,
		["https://venmo.com/"],
		"the navigation must still be attempted exactly once",
	);
});
...
test("probeVenmoAccount: does not re-navigate when the page is already on venmo.com", async () => {
	...
	assert.equal(
		gotoCalls,
		0,
		"already being on the venmo.com origin must not trigger a re-navigation",
	);
});
...
test("probeVenmoAccount: a transport fault carrying a bearer token cannot reach the thrown error text", async () => {
	const page: Pick<Page, "evaluate" | "goto" | "url"> = {
		async evaluate(): Promise<unknown> {
			return await Promise.reject(
				new Error(
					"Failed to fetch https://api.venmo.com/v1/account (Authorization: Bearer abc123SECRETTOKEN)",
				),
			);
		},
		goto(): ReturnType<Page["goto"]> {
	...
	await assert.rejects(probeVenmoAccount(page as Page), (err: unknown) => {
		...
		assert.doesNotMatch(
			err.message,
			/abc123SECRETTOKEN/,
			"a bearer token must not survive",
		);
		assert.match(err.message, /\[redacted-authorization]/);
		assert.match(
			err.message,
			/Failed to fetch/,
			"the safe transport class must remain visible",
		);
		return true;
	});
});
...
test("probeVenmoAccount: a transport fault carrying a query-string token, cookie, and email cannot reach the thrown error text", async () => {
	...
	await assert.rejects(probeVenmoAccount(page as Page), (err: unknown) => {
		...
		assert.match(
			err.message,
			/ECONNRESET/,
			"the safe transport code must remain visible",
		);
		return true;
	});
});
 ...
 */
function makePageWhosePostSubmitProbeThrows(): {
	fillCalls: Record<string, string>;
	page: Page;
} {
	const fillCalls: Record<string, string> = {};
...
test("ensureVenmoSession: a transport fault in the post-submit probe throws a distinctly-named, non-retryable error", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.equal(
			fillCalls.username,
			"test-user",
			"the automated form fill did happen — this IS the post-submit case",
		);
		assert.equal(fillCalls.password, "test-password");
	});
});
...
test("B4 oracle: repeated-dispatch simulation — a retry-if-retryable caller submits the password exactly once total across N dispatch attempts", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.equal(
			submitCount,
			1,
			"exactly one password submission across every dispatch attempt, never up to three",
		);
		assert.match(
			lastError?.message ?? "",
			/venmo_post_submit_probe_transport_error/,
...
test("B4 counterweight: a pre-submit transport fault keeps its retryable name and never touches the credential form", async () => {
	await withVenmoCredentials(async () => {
		...
		const { sendInteraction } = recordingSendInteraction();
		await assert.rejects(
			ensureVenmoSession({ page: page as Page, sendInteraction }),
			(err: unknown) => {
				assert.ok(err instanceof Error);
				assert.match(err.message, /^venmo_probe_transport_error/);
				assert.equal(
...
test("probeVenmoAccount: a probe whose in-page fetch never resolves throws a bounded transport error instead of hanging", async () => {
	...
	await assert.rejects(
		probeVenmoAccount(page as Page, "pre_submit", { evaluateTimeoutMs: 50 }),
		(err: unknown) => {
			assert.ok(err instanceof Error);
			assert.match(
				err.message,
				new RegExp(`^${VENMO_PROBE_TRANSPORT_ERROR}: `),
			);
			return true;
		},
	);
	assert.ok(
		...
		"the probe must resolve within its own bound",
	);
});
...
test("probeVenmoAccount: a hung POST-submit probe keeps the non-retryable name, so a stall never resubmits a password (B4)", async () => {
	...
	await assert.rejects(
		probeVenmoAccount(page as Page, "post_submit", { evaluateTimeoutMs: 50 }),
		(err: unknown) => {
			assert.ok(err instanceof Error);
			assert.match(
				err.message,
				new RegExp(`^${VENMO_POST_SUBMIT_PROBE_TRANSPORT_ERROR}: `),
			);
...
function makeRedirectingPage(
	...
	const page: Pick<Page, "goto" | "url"> = {
		goto(url: string): ReturnType<Page["goto"]> {
			gotoUrls.push(url);
			currentUrl =
				typeof finalUrl === "function" ? finalUrl(gotoUrls.length) : finalUrl;
			return Promise.resolve(null);
		},
		url(): string {
			return currentUrl;
...
test("isVenmoFamilyUrl: every legitimate Venmo host is accepted", () => {
	for (const url of [
		...
		assert.equal(
			isVenmoFamilyUrl(url),
			true,
			`${url} is a real Venmo host and must be recognized`,
		);
	}
});
...
test("isVenmoFamilyUrl: lookalike hosts a suffix match would admit are rejected", () => {
	for (const url of [
		...
		assert.equal(
			isVenmoFamilyUrl(url),
			false,
			`${url} is NOT Venmo and must never be treated as the family`,
		);
	}
});
...
test("ensureVenmoOrigin: a post-sign-in page on id.venmo.com re-navigates to venmo.com instead of failing the run", async () => {
	...
	const { gotoUrls, page } = makeRedirectingPage(
		"https://id.venmo.com/signin",
		(attempt) =>
...
test("ensureVenmoOrigin: the confirmed post-login rest origin https://account.venmo.com/ is accepted, not rejected (run_1787151856448)", async () => {
	...
	assert.equal(
		...
		"https://venmo.com",
		"and must end on the CORS-granted origin",
	);
...
test("ensureVenmoOrigin: a page stuck on a Venmo family host that never reaches venmo.com still fails loudly", async () => {
	...
	assert.equal(
		gotoUrls.length,
		2,
		"bounded: exactly one re-navigation, never an unbounded loop",
	);
});
...
test("ensureVenmoOrigin: landing on a genuinely unexpected origin fails with the named error and does not echo the hostile host", async () => {
	const { gotoUrls, page } = makeRedirectingPage(
		"about:blank",
		"https://evil-venmo.com/harvest?session=SECRETVALUE",
	);
	await assert.rejects(ensureVenmoOrigin(page), (err: unknown) => {
		assert.ok(err instanceof Error);
		assert.match(
			err.message,
			/venmo_origin_navigation_failed/,
			"an unexpected landing must still fail loudly",
		);
		assert.doesNotMatch(
			err.message,
			/evil-venmo/,
			"an attacker-chosen hostname must not be echoed to the owner",
		);
		assert.doesNotMatch(
			err.message,
			/SECRETVALUE/,
			"no path/query may ride along into connector_error_json",
		);
		return true;
	});
	assert.equal(
		gotoUrls.length,
		1,
		"a non-family landing must NOT get a second navigation attempt",
	);
});
...
test("probeVenmoAccount: normal live and dead sessions are unaffected by the bound (COUNTERWEIGHT)", async () => {
	...
	const live = await probeVenmoAccount(
		makePage({ kind: "live", ownerId: "42" }),
		"pre_submit",
		{
	...
	const dead = await probeVenmoAccount(
		makePage({ kind: "dead" }),
		"pre_submit",
		{ evaluateTimeoutMs: 50 },
	);
...
function makeVenmoStepOneWithUnnamedPasswordPage({
	...
	> = {
		...
		getByRole: (): Locator =>
			makeClickRecordingLocator(() =>
				advance(
					onPasswordScreen ? "role-submit-password" : "role-submit-identifier",
				),
			),
		goto(url: string): ReturnType<Page["goto"]> {
			onGoto?.(url);
...
test("ensureVenmoSession: step one's UNNAMED password input must not be mistaken for a one-screen form (production run_1787319714987)", async () => {
	await withVenmoCredentials(async () => {
		...
		assert.equal(
			fillCalls.username,
			"test-user",
			"the identifier must still be filled on screen one",
		);
		assert.equal(
			fillCalls.password,
			"test-password",
			"the password must be typed on screen two, after Next",
		);
		assert.equal(
			result?.live,
			true,
			"a correct two-step sign-in establishes a live session",
		);
		assert.deepEqual(
			requests,
			[],
			"a sign-in the connector can complete must not involve the owner at all",
		);
	});
});
...
test("ensureVenmoSession: a captcha handoff preserves the live id.venmo.com challenge instead of navigating away from it", async () => {
	await withVenmoCredentials(async () => {
		...
		const captchaRequest = requests.find((req): boolean =>
			/CAPTCHA|verification challenge/i.test(req.message ?? ""),
		);
		...
		assert.equal(
			gotoUrls.filter((url): boolean => url === "https://venmo.com/login")
				.length,
			1,
...
test("isVenmoSignInUrl: only the https identity host counts as a live sign-in screen (COUNTERWEIGHT)", () => {
	assert.equal(
		isVenmoSignInUrl("https://id.venmo.com/signin?country.x=US"),
		true,
	);
	...
	assert.equal(
		isVenmoSignInUrl("https://venmo.com/"),
		false,
		"the logged-out home page is not a sign-in screen",
	);
	assert.equal(isVenmoSignInUrl("https://account.venmo.com/"), false);
	assert.equal(
		isVenmoSignInUrl("about:blank"),
		false,
		"a fresh run's blank page must still be navigated (F2)",
	);
	assert.equal(
		isVenmoSignInUrl("http://id.venmo.com/signin"),
		false,
		"a downgraded origin is not trusted",
	);
	assert.equal(
		isVenmoSignInUrl("https://id.venmo.com.evil.example/signin"),
		false,
 ...
 */
const SIGN_IN_URL =
	"https://id.venmo.com/signin?country.x=US&ctxId=AAEkSlyLMSWJ8X310aVa0Sbj";
 ...
 */
function makeBlankHandoffPage({
	paintAfterMs,
}: {
	paintAfterMs: number | "never";
}): {
	gotoUrls: string[];
	page: Page;
} {
...
test("waitForManualLogin: a sign-in surface that has not painted yet is awaited before the owner is asked to act", async () => {
	...
	await ensureVenmoSession({
		...
		sendInteraction: async (req) => {
			...
			surfacePaintedAtHandoff = (await page
				.locator("input")
				.first()
				.isVisible()) satisfies boolean;
...
test("waitForManualLogin: a surface that never paints still reaches the owner, with an honest message", async () => {
	...
	assert.equal(
		requests.length,
		1,
		"a blank surface must still hand off rather than fail silently",
	);
	assert.match(
		requests[0]?.message ?? "",
...
test("a bot-check handoff resumes by SUBMITTING the revealed form, not by filling and stopping", async () => {
	const { fillCalls, page, reveal, submitClicks } =
		makePageWithChallengeThenForm();
	...
	assert.equal(
		fillCalls.password,
		"pw-stored",
		"the stored password must be entered too; the owner does not know it",
	);
...
test("a transport fault after the challenge-resume submit is post_submit, not a retryable pre_submit", async () => {
	const { page, reveal, submitClicks } = makePageWithChallengeThenForm({
		transportFaultAfterSubmit: true,
	});
	const sendInteraction = (
	...
	assert.ok(
		submitClicks.length > 0,
		"precondition: the replay must have submitted before the fault",
	);
	assert.match(
...
test("a post-submit failure inside the replay keeps its own diagnosis instead of flattening", async () => {
	...
	assert.ok(
		submitClicks.length > 0,
		"precondition: the replay must have submitted before the failure",
	);
	assert.match(
		(thrown as Error | null)?.message ?? "",
		/venmo_login_did_not_complete/,
...
test("a submitted password in one run does not misclassify the next run's pre-submit fault", async () => {
	...
	assert.ok(
		first.submitClicks.length > 0,
		"precondition: run one must actually submit a password",
	);
	...
	const secondPage = {
		evaluate: (): Promise<unknown> =>
			Promise.resolve({ kind: "transport_error", message: "socket hang up" }),
	...
	assert.match(
		message,
		/venmo_probe_transport_error/,
		"a fault before any password is typed stays RETRYABLE",
	);
	assert.doesNotMatch(
		message,
		/venmo_post_submit_probe_transport_error/,
...
test("suppression preserves the manual fallback: an owner who signs in by hand still completes, unverified", async () => {
	...
	assert.equal(
		submitClicks.length,
		0,
		"precondition: with no form revealed, nothing may be submitted",
	);
	assert.equal(
		promptCount,
		1,
		"the manual path still asks the owner exactly once",
	);
	assert.equal(
		result,
		null,
