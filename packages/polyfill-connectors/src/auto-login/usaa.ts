 ...
 */
const TEXT_CODE_CHOICE_SELECTOR =
	':text-matches("Text security code to:", "i")';
...
const LOGIN_NAVIGATION_INTERVENTION_ERROR =
	/page\.goto: net::ERR_(HTTP2_PROTOCOL_ERROR|CONNECTION_RESET|FAILED)\b/i;
...
export function classifyUsaaLoginStepFailure(
	bodyText: string,
): "source_unavailable" | "password_field_missing" {
...
function sourceUnavailableLoginRecoveryDiagnostic(
	outcome: SourceUnavailableLoginRecoveryOutcome,
): string {
...
async function captureSourceUnavailableLoginActionStructure(
	...
	await capture
		?.captureLocatorProbe?.(page, "usaa-source-unavailable-login-action", [
			{
				description:
					"Exact accessible-name candidate; captures only structural locator evidence.",
...
async function sourceUnavailableLoginActions(
	page: Page,
): Promise<SourceUnavailableLoginAction[]> {
...
async function attemptSourceUnavailableLoginRecovery(
	...
	return page
		...
		.catch(
			(): SourceUnavailableLoginRecoveryOutcome => "member_id_form_missing",
		);
}
...
async function requestOtp(
	sendInteraction: EnsureUsaaSessionArgs["sendInteraction"],
	attempt: number,
): Promise<string> {
...
async function completeOtpChallenge({
	...
	for (let attempt = 1; attempt <= MAX_OTP_ATTEMPTS; attempt += 1) {
		...
		if (
			!(
				(await isStillOnOtpChallenge(page)) &&
...
async function handlePasswordFieldStall(
	...
	if (initialClassification === "source_unavailable") {
		recoveryOutcome = await attemptSourceUnavailableLoginRecovery(
			capture,
			page,
		);
...
export async function ensureUsaaSession({
	...
	try {
		...
		if (LOGIN_NAVIGATION_INTERVENTION_ERROR.test(message)) {
			const reason = firstLine(message).replace(
				STACK_TRACE_LOCATION_SUFFIX_RE,
				"",
			);
			if (
			...
			throw new Error(
				`USAA login page navigation failed (${reason}); manual action did not establish a session`,
				{
					cause: err,
				},
			);
