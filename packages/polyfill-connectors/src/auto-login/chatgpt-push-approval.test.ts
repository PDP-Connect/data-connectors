...
const visibleLocator: Pick<
	Locator,
	"click" | "count" | "fill" | "first" | "waitFor"
> = {
...
function makePushApprovalPage(opts: PushApprovalPageOptions): Page {
	...
	> = {
		...
		evaluate(fn: unknown): Promise<unknown> {
			...
			if (
				typeof fn === "function" &&
				fn.toString().includes("querySelectorAll")
			) {
...
test("push-approval poll checkpoints each tick so a long wait does NOT trip the session watchdog", async () => {
	...
	try {
		...
		assert.equal(
			tripped,
			false,
			"checkpointing poll must not trip the session-establishment watchdog",
		);
...
test("readiness during the non-blocking poll resolves the assistance and emits NO interaction", async () => {
	...
	try {
		...
		assert.equal(
			interactions.length,
			0,
			"auto-resume MUST NOT emit an INTERACTION",
		);
		assert.equal(
			assistCalls,
			1,
			"the non-blocking act_elsewhere assistance was requested once",
		);
		assert.deepEqual(
			completes,
			[{ id: "asst_1", status: "resolved" }],
			"assistance resolved, not escalated",
		);
	} finally {
		clearBudgetEnv(prior);
	}
...
test("push-approval poll checks ChatGPT origin when approval page does not redirect", async () => {
	...
	try {
		...
		assert.equal(
			interactions.length,
			0,
			"origin-proved readiness MUST NOT require a Continue click",
		);
		assert.deepEqual(
			primaryNavigations,
			["https://chatgpt.com/"],
...
test("budget exhausted: assistance is escalated BEFORE the blocking manual_action is emitted", async () => {
	...
	try {
		...
		assert.equal(
			interactions.length,
			1,
			"exactly one fallback interaction after the budget",
		);
...
test("budget exhausted with no readiness even after the fallback returns false", async () => {
	...
	try {
		...
		assert.equal(
			result,
			false,
			"no readiness anywhere → handler reports not-resumed",
		);
		assert.equal(
			interactions.length,
			1,
			"the blocking fallback was still attempted once",
		);
	} finally {
		clearBudgetEnv(prior);
	}
...
test("browser-login assistance resolves when readiness appears and emits NO interaction", async () => {
	...
	try {
		...
		assert.equal(
			interactions.length,
			0,
			"auto-resume MUST NOT wait for an owner INTERACTION response",
		);
		assert.equal(
			assistanceRequests.length,
			1,
			"browser-login assistance was requested once",
		);
		assert.equal(assistanceRequests[0]?.owner_action, "operate_attachment");
		assert.deepEqual(assistanceRequests[0]?.attachments, [
...
test("browser-login budget exhaustion escalates before blocking manual_action fallback", async () => {
	...
	try {
		...
		assert.equal(
			interactions.length,
			1,
			"fallback interaction is emitted only after the assistance budget",
		);
...
test("chatGptBrowserLoginAssistance carries browser attachment and timeout", () => {
	const assistance = chatGptBrowserLoginAssistance({
		PDPP_CHATGPT_BROWSER_LOGIN_TIMEOUT_MS: "120000",
	});
	...
	assert.deepEqual(assistance.attachments, [
		{ kind: "browser_surface", role: "streaming_companion" },
	]);
});
