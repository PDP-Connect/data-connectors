...
function makeContext(
	...
	const fake: Pick<BrowserContext, "cookies"> = {
		cookies(
			...
			const batch =
				cookieBatches[Math.min(calls, Math.max(cookieBatches.length - 1, 0))] ??
				[];
			calls += 1;
...
function makePasswordStepFailurePage(
	bodyText: string,
	...
	const bodyLocator: Pick<Locator, "innerText"> = {
		innerText: (): Promise<string> =>
			Promise.resolve(
				currentUrl === DASHBOARD_URL && dashboardBodyText !== undefined
					? dashboardBodyText
					: bodyText,
			),
	};
...
function makeSourceUnavailableRecoveryPage(
	fixture: SourceUnavailableRecoveryFixture,
): {
	actionClicks: number;
	...
	const bodyLocator: Pick<Locator, "innerText"> = {
		innerText: (): Promise<string> => {
			...
			return Promise.resolve(
				"We are unable to complete your request. Our system is currently unavailable.",
			);
		},
	};
	const otpInputLocator: Pick<
		Locator,
		"fill" | "first" | "isEnabled" | "isVisible"
	> = {
	...
	const textCodeChoiceLocator: Pick<
		Locator,
		"click" | "first" | "isEnabled" | "isVisible"
	> = {
		click: (): Promise<void> => {
	...
	fake.click = ((selector: string): Promise<void> => {
		...
		if (
			selector === 'button[type="submit"], #next-button' &&
			otpChallengeActive
		) {
	...
	fake.waitForLoadState = ((): Promise<void> =>
		Promise.resolve()) as Page["waitForLoadState"];
	fake.waitForSelector = ((selector: string): Promise<unknown> => {
		...
		if (
			selector === 'input[name="memberId"]' &&
			actionClicked &&
			fixture === "member_id_form_missing"
		) {
...
test("ensureUsaaSession uses the resolved USAA_USERNAME, never connector identity or ambient env", async () => {
	...
	try {
		...
		const context = makeContext([
			[],
			[makeCookie("UsaaMbWebMemberLoggedIn", "true")],
		]);
		...
		const ok = await ensureUsaaSession({
			context,
			credentials: {
				USAA_PASSWORD: "saved-password",
				USAA_USERNAME: "saved-online-id",
			},
		...
		assert.equal(
			fixturePage.filledValues.find(
				({ selector }) => selector === 'input[name="memberId"]',
			)?.value,
			"saved-online-id",
		);
		assert.notEqual(
...
async function withUsaaCredentials(run: () => Promise<void>): Promise<void> {
	await withUsaaCredentialValues(
...
test("ensureUsaaSession self-resolves the no-credentials manual handoff via assist/completeAssistance when the connector's own probe proves the session is live, without ever sending a manual_action interaction", async () => {
	await withoutUsaaCredentials(async () => {
		...
		assert.deepEqual(completions, [
			{ id: "assist_req_usaa_1", status: "resolved" },
		]);
	});
});
...
test("ensureUsaaSession emits manual_action when USAA login navigation trips HTTP/2 bot failure", async () => {
	await withUsaaCredentials(async () => {
		...
		const { gotoCalls, page } = makePage(
			new Error(
				`page.goto: net::ERR_HTTP2_PROTOCOL_ERROR at ${LOGIN_URL}\nCall log:`,
			),
		);
		const interactions = makeInteractionHarness();
...
test("ensureUsaaSession does not convert ordinary DNS navigation errors into manual_action", async () => {
	await withUsaaCredentials(async () => {
		const context = makeContext([[]]);
		const { page } = makePage(
			new Error(`page.goto: net::ERR_NAME_NOT_RESOLVED at ${LOGIN_URL}`),
		);
		const interactions = makeInteractionHarness();
...
test("classifyUsaaLoginStepFailure distinguishes source downtime from selector drift", () => {
	...
	assert.equal(
		classifyUsaaLoginStepFailure("Member Account Login Username Next"),
...
test("ensureUsaaSession follows exactly one visible semantic login action, then authenticates through OTP", async () => {
	await withUsaaCredentials(async () => {
		...
		const context = makeContext(
			[[]],
			() => fixturePage.otpResponseAuthenticated,
		);
		const interactions = makeInteractionHarness("success", { code: "123456" });
		...
		assert.equal(ok, true);
		assert.equal(
			fixturePage.actionClicks,
			1,
			"recovery follows the semantic action at most once",
		);
		assert.deepEqual(
		...
		assert.deepEqual(filledSelectors, [
			'input[name="memberId"]',
			'input[name="memberId"]',
			'input[name="password"]',
		]);
		...
		assert.equal(
			fixturePage.otpResponseAuthenticated,
			true,
			"the OTP response authenticates the recovered session",
		);
...
for (const [fixture, expectedOutcome] of [
	...
	test(`ensureUsaaSession preserves manual_action when source-unavailable login recovery is ${fixture}`, async () => {
		await withUsaaCredentials(async () => {
			...
			assert.equal(
				fixturePage.actionClicks,
				fixture === "ambiguous" || fixture === "action_error" ? 0 : 1,
			);
		});
	});
}
...
test("ensureUsaaSession still routes a delayed USAA source-unavailable page after member-id submit to genuine manual_action, with an owner-visible diagnostic note", async () => {
	...
	await withUsaaCredentials(async () => {
		...
		assert.match(
			interactions.requests[0]?.message ?? "",
			/source_unavailable_login_action=absent/,
		);
	});
});
...
test("ensureUsaaSession routes a lockout/challenge page containing 'try again later' to manual_action without the source-unavailable note", async () => {
	...
	await withUsaaCredentials(async () => {
		...
		assert.doesNotMatch(
			interactions.requests[0]?.message ?? "",
			/USAA's page reported its own system as unavailable/,
		);
	});
});
...
test("ensureUsaaSession classifies USAA source-unavailable page rendered after password submit in the thrown diagnostic, without a false-certainty retryable claim", async () => {
	...
	await withUsaaCredentials(async () => {
		...
		assert.match(
			thrown.message,
			/USAA login completed but no verified authenticated dashboard session was detected/,
		);
...
test("ensureUsaaSession calls onCredentialSubmit exactly once, immediately after the password click and before any post-submit read/capture", async () => {
	await withUsaaCredentials(async () => {
		...
		page.click = ((
			...args: Parameters<Page["click"]>
		): ReturnType<Page["click"]> => {
			calls.push("click");
		...
		page.locator = ((
			...
			const real = (originalLocator as Page["locator"]).call(
				page,
				selector,
				options,
			);
		...
		assert.equal(
			credentialSubmitCount,
			1,
			"onCredentialSubmit must fire exactly once for one login attempt",
		);
		...
		assert.ok(
			markerIndex > 0,
			"onCredentialSubmit must fire after at least one click",
		);
		assert.equal(
			calls[markerIndex - 1],
			"click",
			"onCredentialSubmit must immediately follow a click call",
		);
		assert.ok(
...
function makeTextCodeDispatchPage(options: DispatchFixtureOptions = {}): {
	...
	> = {
		...
		isEnabled: (): Promise<boolean> =>
			Promise.resolve(choicePresent && choiceEnabled),
		isVisible: (): Promise<boolean> => Promise.resolve(choicePresent),
	};
	const positionalLocator: Pick<
		Locator,
		"click" | "first" | "isEnabled" | "isVisible"
	> = {
		click: (): Promise<void> => {
	...
	const codeEntryUsable = (): boolean =>
		dispatchClicks > 0 && (codeEntrySurvivesSubmit || !codeSubmitted);
	const otpInputLocator: Pick<
		Locator,
		"fill" | "first" | "isEnabled" | "isVisible"
	> = {
	...
	fake.locator = ((selector: string): Locator => {
		...
		if (
			selector === "#next-button:not([disabled])" ||
			selector === 'input[name="memberId"]'
		) {
	...
	fake.waitForSelector = ((selector: string): Promise<unknown> => {
		if (selector === 'input[name="password"]') {
			return passwordSubmitted
				? Promise.resolve({})
				: Promise.reject(new Error("password field unavailable"));
		}
...
test("ensureUsaaSession does not dispatch a security code when the chooser copy matches but no usable delivery control is present", async () => {
	await withUsaaCredentials(async () => {
		...
		assert.equal(
			fixturePage.dispatchClicks,
			0,
			"no delivery control may be clicked: that click is a real SMS",
		);
		assert.equal(
...
test("ensureUsaaSession treats a visible but DISABLED delivery control as not usable and refuses to dispatch", async () => {
	await withUsaaCredentials(async () => {
		...
		assert.equal(
			fixturePage.dispatchClicks,
			0,
			"a disabled control is not evidence the chooser is ready",
		);
	});
});
...
test("ensureUsaaSession never clicks the removed hardcoded positional selector, even when the delivery control is absent", async () => {
	await withUsaaCredentials(async () => {
		...
		assert.equal(
			fixturePage.positionalClicks,
			0,
			"the positional fallback must never fire",
		);
		assert.ok(
...
test("ensureUsaaSession hands off to the owner's browser when the delivery control cannot be located, and succeeds if they sign in", async () => {
	await withUsaaCredentials(async () => {
		...
		assert.equal(
			fixturePage.dispatchClicks,
			0,
			"handoff replaces the guess; it never dispatches",
		);
...
test("ensureUsaaSession dispatches exactly once and re-prompts only on positive rejection evidence", async () => {
	await withUsaaCredentials(async () => {
		...
		const fixturePage = makeTextCodeDispatchPage({
			postSubmitBody: "Please wait while we redirect you.",
		});
		const interactions = makeInteractionHarness("success", { code: "123456" });
		const context = makeContext([[]]);
		...
		assert.equal(
			fixturePage.dispatchClicks,
			1,
			"the delivery control is clicked exactly once: one SMS, not three",
		);
		assert.equal(
			interactions.requests.filter((request) => request.kind === "otp").length,
			1,
