...
const PAGE_BLAMING =
	/did not render|failed to load|did not finish signing in|could not finish sign-in/i;
...
test("blank and whitespace-only credentials count as absent, not as a login attempt", () => {
	...
	for (const blank of ["", "   ", "\t\n"]) {
		...
		assert.equal(
			resolved.kind,
			"absent",
			`"${blank}" must not be treated as a password`,
		);
	}
});
...
test("every migrated connector resolves its pair from the runtime credentials, not process.env", () => {
	...
	for (const connector of ["amazon", "chase", "chatgpt", "heb"]) {
		const source = readFileSync(
			join(AUTO_LOGIN_DIR, `${connector}.ts`),
			"utf8",
		);
		assert.match(
			source,
