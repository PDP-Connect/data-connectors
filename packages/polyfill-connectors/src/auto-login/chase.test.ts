...
function makePage({
	...
	const fake: Pick<Page, "getByText" | "goto" | "isClosed"> = {
		getByText(
			_text: Parameters<Page["getByText"]>[0],
			_options?: Parameters<Page["getByText"]>[1],
		): Locator {
...
function makeLiveLocator(isLive: () => boolean): Locator {
	const fake: Pick<Locator, "first" | "waitFor"> = {
		...
		waitFor: (): Promise<void> =>
			isLive()
				? Promise.resolve()
				: Promise.reject(new Error("Chase dashboard is not authenticated")),
	};
...
function makeLivePage(isLive: () => boolean): FakePage {
	...
	const fake: Pick<
		Page,
		"close" | "context" | "getByText" | "goto" | "isClosed"
	> = {
 ...
 */
async function withoutChaseCredentials(
	run: () => Promise<void>,
): Promise<void> {
...
function otpControlLocator(state: FakeOtpPageState, index: number): Locator {
	...
	> = {
		...
		waitFor: (): Promise<void> =>
			usable()
				? Promise.resolve()
				: Promise.reject(new Error("chase otp input not visible")),
	};
	return fake as Locator;
}
 ...
 */
function deliveryOptionLocator(
	state: FakeOtpPageState,
	onClick: () => void,
): Locator {
	...
	const fake: Pick<
		Locator,
		"click" | "count" | "first" | "isEnabled" | "isVisible" | "nth" | "waitFor"
	> = {
		click: (): Promise<void> => {
			onClick();
		...
		isEnabled: (): Promise<boolean> =>
			Promise.resolve(present() && (state.deliveryOptionEnabled ?? true)),
		isVisible: (): Promise<boolean> => Promise.resolve(present()),
		nth: (): Locator => fake as Locator,
		waitFor: (): Promise<void> =>
			present()
				? Promise.resolve()
				: Promise.reject(new Error("option absent")),
	};
	return fake as Locator;
}
...
function makeOtpPage(
	...
	{
		onAfterSignInClick,
	}: { onAfterSignInClick?: (state: FakeOtpPageState) => void } = {},
): FakeOtpPage {
	...
	const filledValues: { password: string[]; username: string[] } = {
		password: [],
		username: [],
	};
...
test("ensureChaseSession hands off when optional credentials are absent", async () => {
	await withoutChaseCredentials(async () => {
		...
		assert.doesNotMatch(
			requests[0]?.message ?? "",
			/synthetic-password|synthetic-user/u,
		);
	});
});
...
test("the identity-challenge copy alone never dispatches a code when the delivery option is absent", async () => {
	...
	await withChaseCredentials(async () => {
		...
		assert.equal(
			fake.deliveryClicks,
			0,
			"no delivery option may be clicked: that click is a real code dispatch",
		);
		assert.deepEqual(
...
test("a visible but DISABLED delivery option is not evidence the chooser is ready", async () => {
	...
	await withChaseCredentials(async () => {
		...
		assert.equal(
			fake.deliveryClicks,
			0,
			"a disabled option must not be clicked",
		);
	});
});
...
test("a genuine method chooser still dispatches exactly once and reaches the code prompt", async () => {
	await withChaseCredentials(async () => {
		...
		assert.equal(
			fake.deliveryClicks,
			1,
			"the genuine chooser dispatches exactly once, as before",
		);
		assert.deepEqual(
