...
test("a complete pair resolves", () => {
	...
	assert.equal(
		result.kind === "resolved" && result.username,
...
test("an absent pair names the credential, never the page", () => {
	...
	assert.deepEqual(result.kind === "absent" && result.missing, [
		"HEB_USERNAME",
		"HEB_PASSWORD",
	]);
...
test("a half pair is absent, not a half login", () => {
	...
	const result = resolveLoginCredentials(
		{ HEB_USERNAME: "owner@example.invalid" },
		HEB_FIELDS,
		"heb",
	);
	assert.equal(result.kind, "absent");
...
test("aliases resolve first-non-empty", () => {
	const fields = {
		...
		username: ["GITHUB_EMAIL", "GITHUB_USERNAME"],
	} as const;
	const result = resolveLoginCredentials(
		{
	...
	assert.equal(
		preferred.kind === "resolved" && preferred.username,
...
test("two connections of one connector resolve their own distinct credentials", () => {
	const connectionA = {
		HEB_PASSWORD: "synthetic-pw-a",
		HEB_USERNAME: "owner-a@example.invalid",
	};
	const connectionB = {
		HEB_PASSWORD: "synthetic-pw-b",
		HEB_USERNAME: "owner-b@example.invalid",
	};
	...
	assert.notEqual(
		a.kind === "resolved" ? a.password : null,
		b.kind === "resolved" ? b.password : null,
	);
});
...
test("resolution ignores process.env entirely", () => {
	...
	try {
		...
		assert.equal(
			absent.kind,
			"absent",
			"ambient process.env must not satisfy a missing stored credential",
		);
		...
		assert.equal(
			stored.kind === "resolved" && stored.username,
