 ...
 * it is the one `connector_instance_config_store.propose()` (reference
 * implementation, PDP-Connect/pdpp) actually trusts -- a manifest cannot
 * widen its own eligibility by mislabeling a collection-shaping knob as
 * "transport" (which self-activates without owner confirmation) instead of
 * "collection_scope" (which does not).
 *
 ...
 * conventions). `.github/scripts/cross-repo-integrity/check-connector-option-kind-drift.mjs`
...
const PLATFORM_OPTION_KINDS: Readonly<Record<string, ConnectorOptionKinds>> =
	Object.freeze({
		claude_code: Object.freeze({
			// Both are substring filters over the project folders that get
			// scanned (connectors/claude_code/parsers.ts applyProjectDirScope),
			// so each one decides which sources are collected at all.
			CLAUDE_CODE_PROJECT_EXCLUDE: "collection_scope",
			CLAUDE_CODE_PROJECT_INCLUDE: "collection_scope",
		}),
		google_messages: Object.freeze({
			// Caps how many conversations are scanned after a recency sort, so
			// it decides WHICH chats are collected (connectors/google_messages/
			// index.ts resolveMaxChats).
			GMCLI_MAX_CHATS: "collection_scope",
			// Caps messages fetched per chat, bounding how much history is
			// collected.
			GMCLI_MESSAGES_PER_CHAT_LIMIT: "collection_scope",
			// Subprocess timeout for one helper invocation; changes no
			// selection.
