...
interface LocatorProbeLocator {
	ariaSnapshot: (
		options?: Parameters<ReturnType<Page["locator"]>["ariaSnapshot"]>[0],
	) => Promise<string>;
	count: () => Promise<number>;
	first: () => LocatorProbeLocator;
	isEnabled: (
		options?: Parameters<ReturnType<Page["locator"]>["isEnabled"]>[0],
	) => Promise<boolean>;
...
export interface CaptureSession {
	...
	setTraceCheckpointHook?: (
		hook: ((label: string) => Promise<void>) | null,
	) => void;
}
...
function requireProbeMethod<K extends keyof LocatorProbePage>(
	...
	return (method as (...args: readonly unknown[]) => unknown).bind(
		page,
	) as NonNullable<LocatorProbePage[K]>;
}

function locatorForProbe(
	page: LocatorProbePage,
	probe: LocatorProbe,
): LocatorProbeLocator {
	switch (probe.kind) {
		...
		case "role": {
			const name = probe.namePattern
				? new RegExp(probe.namePattern, probe.nameFlags ?? "i")
				: probe.name;
		...
		default:
			throw new Error(
				`unsupported locator probe kind: ${(probe as { kind?: string }).kind ?? "unknown"}`,
			);
	}
}
...
async function capturePageMetadata(
	...
	try {
		...
		const message = err instanceof Error ? err.message : String(err);
		process.stderr.write(
			`[capture] page metadata write failed for ${label}: ${message}\n`,
		);
	}
}
...
async function captureAriaSnapshot(
	...
	try {
		...
		const message = err instanceof Error ? err.message : String(err);
		process.stderr.write(
			`[capture] aria snapshot failed for ${label}: ${message}\n`,
		);
	}
}
	...
	try {
		...
		const mask =
			typeof page.locator === "function"
				? [page.locator(SCREENSHOT_MASK_SELECTOR)]
				: null;
		if (!mask) {
			process.stderr.write(
				`[capture] screenshot skipped for ${label}: page cannot build a credential mask\n`,
			);
			return;
		}
		...
		const message = err instanceof Error ? err.message : String(err);
		process.stderr.write(
			`[capture] screenshot write failed for ${label}: ${message}\n`,
		);
	}
}
...
async function runLocatorProbe(
	...
	try {
		...
		if (result.count > 0) {
			...
			result.enabled = await first
				.isEnabled({ timeout: LOCATOR_PROBE_TIMEOUT_MS })
				.catch((): boolean => false);
...
async function writeLocatorProbeReport(
	...
	try {
		...
		const message = err instanceof Error ? err.message : String(err);
		process.stderr.write(
			`[capture] locator probe write failed for ${label}: ${message}\n`,
		);
	}
}
	...
	return {
		...
		finalize(): void {
			...
			try {
				...
				process.stderr.write(
					`[capture] run succeeded; raw capture deleted (${baseDir})\n`,
				);
			} catch (err) {
				const message = err instanceof Error ? err.message : String(err);
				process.stderr.write(
					`[capture] cleanup failed for ${baseDir}: ${message}\n`,
				);
			}
		},
		recordRecord(msg): void {
			try {
				...
				appendFileSync(
					file,
					`${redactKnownSecrets(JSON.stringify(msg.data), [...secrets])}\n`,
				);
			} catch (err) {
				const message = err instanceof Error ? err.message : String(err);
		...
		async captureDom(page, label): Promise<void> {
			...
			if (traceCheckpointHook) {
				await traceCheckpointHook(label).catch((err: unknown): undefined => {
					const message = err instanceof Error ? err.message : String(err);
					process.stderr.write(
						`[capture] trace checkpoint failed for ${label}: ${message}\n`,
					);
				});
			}
		},
		async captureLocatorProbe(page, label, probes): Promise<void> {
			...
			const results = await Promise.all(
				probes.map((probe) => runLocatorProbe(page, probe, knownSecrets)),
			);
			await writeLocatorProbeReport(
				page,
				baseDir,
				label,
				safe,
		...
		captureHttp(label, body, meta = {}): void {
			try {
				...
				writeFileSync(
					file,
					redactKnownSecrets(JSON.stringify(payload, null, 2), [...secrets]),
				);
			} catch (err) {
				const message = err instanceof Error ? err.message : String(err);
				process.stderr.write(
