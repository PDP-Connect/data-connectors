...
async function createOwnerTokenServer({
	requireOwnerCookie,
}: {
	requireOwnerCookie: boolean;
}): Promise<{
	close: () => Promise<void>;
	state: {
	...
	const server = createServer(async (req, res) => {
		if (req.method === "POST" && req.url === "/owner/login") {
			...
			res.setHeader(
				"set-cookie",
				"pdpp_owner_session=test-session; Path=/; HttpOnly",
			);
		...
		if (req.method === "POST" && req.url === "/device/approve") {
			...
			if (
				requireOwnerCookie &&
				state.approveCookie !== "pdpp_owner_session=test-session"
			) {
		...
		if (req.method === "POST" && req.url === "/oauth/token") {
			sendJson(res, 200, {
				access_token: `token_${state.subjectId ?? "missing_subject"}`,
			});
			return;
		}
