...
test("parseManifestStreams: extracts every stream.name in declaration order", () => {
	...
	assert.deepEqual(parseManifestStreams(json).declared, [
		"orders",
		"order_items",
		"shipments",
	]);
...
test("parseManifestStreams: empty manifest → empty list", () => {
	...
	assert.deepEqual(
		parseManifestStreams(JSON.stringify({ streams: [] })).declared,
		[],
	);
});
...
test("parseSchemaStreams: extracts SCHEMAS registry keys (shorthand)", () => {
  ...
  `;
	assert.deepEqual(parseSchemaStreams(src).registered, [
		"orders",
		"order_items",
	]);
});
