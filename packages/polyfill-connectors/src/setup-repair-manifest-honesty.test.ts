...
function setupAndPolicyText(
	...
	push(
		...
		manifest.setup?.credential_capture?.description,
	);
	push(
		"setup.manual_or_upload.description",
		manifest.setup?.manual_or_upload?.description,
	);
	push(
		"setup.manual_or_upload.help_text",
		manifest.setup?.manual_or_upload?.help_text,
	);
	push(
		"setup.manual_or_upload.large_file_fallback",
		manifest.setup?.manual_or_upload?.large_file_fallback,
	);
	push(
		"capabilities.refresh_policy.rationale",
		manifest.capabilities?.refresh_policy?.rationale,
	);

	for (const [index, method] of (
		manifest.setup?.manual_or_upload?.acquisition_methods ?? []
	).entries()) {
		push(
			`setup.manual_or_upload.acquisition_methods[${index}].label`,
			method.label,
		);
		push(
			`setup.manual_or_upload.acquisition_methods[${index}].detail`,
			method.detail,
		);
	}
	for (const [index, expectation] of (
		manifest.setup?.manual_or_upload?.validation_expectations ?? []
	).entries()) {
		push(
			`setup.manual_or_upload.validation_expectations[${index}]`,
			expectation,
		);
	}
...
test("first-party setup and refresh-policy copy does not claim live repair state", () => {
	...
	for (const name of MANIFEST_NAMES) {
		...
		for (const { path, value } of setupAndPolicyText(manifest)) {
			const pattern = LIVE_STATE_PATTERNS.find((candidate) =>
...
test("static-secret manifests declare stable credential capture shape only", () => {
	...
	for (const name of MANIFEST_NAMES) {
		...
		if (
			manifest.setup?.modality !== "static_secret" ||
			!kind ||
			!hasSecretField
		) {
			offenders.push(
...
test("automatic human-interaction manifests are explicitly session-reuse/assisted policy, not generic background auth", () => {
	...
	for (const name of MANIFEST_NAMES) {
		...
		const claimsAutomatic =
			policy?.recommended_mode === "automatic" ||
			policy?.background_safe === true;
...
test("human_interaction and interaction_posture stay coarse stable hints", () => {
	...
	for (const name of MANIFEST_NAMES) {
		...
		if (Array.isArray(interactions)) {
			for (const interaction of interactions) {
				if (
					...
					!allowedInteractions.has(interaction)
				) {
					offenders.push(
						`${name}:capabilities.human_interaction=${String(interaction)}`,
					);
				}
			}
		}
		...
		if (typeof posture !== "string" || !allowedPostures.has(posture)) {
			offenders.push(
				`${name}:capabilities.refresh_policy.interaction_posture=${String(posture)}`,
			);
		}
	}
	...
	assert.deepEqual(
		offenders,
		[],
		"manifest interaction declarations must stay at coarse capability/posture altitude",
	);
});
