	...
	const page: MockPage = {
		url: () =>
			typeof opts.url === "function"
				? opts.url()
				: (opts.url ?? "https://example.test/login"),
		title: () => {
			...
			newCDPSession: (_page: unknown) => {
				...
				return Promise.resolve({
					send: (_method: string) => {
						...
						return Promise.resolve({
							targetInfo: { targetId, type: targetType },
						});
					},
...
function makeControllableTitle(): {
	...
	const fetchImpl: typeof fetch = (input, init) => {
		...
		if (next === undefined) {
			return Promise.reject(
				new Error(
					`fakeFetch: no response queued for request #${String(cursor)} (${method} ${url})`,
				),
			);
		}
...
test("resolveWsUrlForExactPage throws when CDP target type is not 'page'", async () => {
	const page = makeMockPage({
		session: { targetType: "iframe", targetId: "X" },
	});
	...
	const result = await prepareManualAction({
		...
		resolveStreamingRegistration: () =>
			Promise.resolve({
				runId: "run_x",
				register: () =>
					Promise.reject(new Error("register should not be called")),
				unregister: () =>
					Promise.reject(new Error("unregister should not be called")),
			}),
	});
	assert.equal(result.registered, false);
});
...
test("prepareManualAction PUTs the composed wsUrl + page metadata when env is fully set", async () => {
	...
	const result = await prepareManualAction({
		...
		resolveStreamingRegistration: (resolveEnv) => {
			...
			const token =
				resolveEnv?.[STREAMING_REGISTRATION_TOKEN_ENV] ??
				resolveEnv?.[LOCAL_DEVICE_TOKEN_ENV];
	...
	const parsed = sent.body
		? (JSON.parse(sent.body) as Record<string, string>)
		: null;
	assert.ok(parsed);
	...
	assert.equal(
		parsed.ws_url,
		"ws://127.0.0.1:44763/devtools/page/FAKETARGET01",
	);
...
test("prepareManualAction registers managed n.eko descriptor with lease-scoped CDP details", async () => {
	...
	const result = await prepareManualAction({
		...
		resolveWsUrl: () => {
			...
			return Promise.reject(
				new Error("CDP resolver must not be called for managed n.eko"),
			);
		},
		resolveStreamingRegistration: () =>
			Promise.resolve({
	...
	assert.equal(
		JSON.stringify(args).includes("REMOTE_CDP"),
		false,
		"CDP env key must not be registered",
	);
});
...
test("prepareManualAction does not fall back to CDP registration when managed n.eko descriptor env is incomplete", async () => {
	...
	const result = await prepareManualAction({
		...
		resolveWsUrl: () => {
			resolveWsUrlCalled = true;
			return Promise.resolve(
				"ws://127.0.0.1:44763/devtools/page/SHOULD_NOT_REGISTER",
			);
		},
		resolveStreamingRegistration: () =>
			Promise.resolve({
...
test("prepareManualAction returns { registered: false } and does NOT throw when newCDPSession throws (page closed)", async () => {
	...
	const page = makeMockPage({
		newCDPSessionShouldThrow: new Error(
			"browserContext.newCDPSession: page: no object with guid page@abc",
		),
	});
	...
	assert.equal(
		registerCalled,
		false,
		"register must not be called when wsUrl resolution fails",
	);
	assert.match(result.interactionId, /^int_\d+_[0-9a-f]{8}$/);
});
...
test("manualAction unregisters its exact target after response or send failure", async () => {
	...
	const success = await manualAction(
		{
			env,
			message: "Continue after the response.",
			page,
			resolveStreamingRegistration,
		},
		(req) =>
	...
	assert.equal(
		unregistered.length,
		2,
		"failed interaction must still unregister its exact target",
	);
...
test("manualAction starts fixture capture before notifying and does not wait for capture completion", async () => {
	...
	const sendInteraction: SendInteraction = (req) => {
		assert.equal(
			captureStarted,
			true,
			"capture should be started before sendInteraction",
		);
		notified = true;
		return Promise.resolve({
			type: "INTERACTION_RESPONSE",
	...
	assert.match(
		captured[0]?.label ?? "",
		/^manual-action-2fa-int_\d+_[0-9a-f]{8}$/,
	);
...
test("manualBrowserLogin keeps watching after the initial fast window and self-resolves without a Continue interaction", async () => {
	...
	assert.equal(
		probeCalls,
		2,
		"the readiness watcher must survive past the former 15-second cap",
	);
...
test("manualAction still emits the interaction (with URL) when page.title() hangs forever", async () => {
	...
	try {
		...
		assert.equal(
			seen.length,
			1,
			"interaction must still be emitted despite hung title",
		);
