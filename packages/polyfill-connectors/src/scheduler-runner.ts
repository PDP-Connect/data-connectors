...
export function millisecondsUntilNextNightlySummary(
	now: Date,
	hour = 7,
	minute = 0,
): number {
		...
		timer = setTimer(() => {
			Promise.resolve()
				...
				.catch((err: unknown) => {
					const message = err instanceof Error ? err.message : String(err);
					console.error(
						`[scheduler] failed to build overnight summary: ${message}`,
					);
					scheduleNext();
				});
		}, delayMs);
...
export async function startPolyfillScheduler({
	...
	const schedulerStore =
		await resolveSchedulerPersistenceStore(schedulerStoreOption);
