...
export const BOUNDED_READ_EXCEPTIONS: readonly BoundedReadException[] = [
	...
	},
	{
		...
		reason:
			"Reads one Takeout JSON sidecar per call; streaming migration is deferred until large fixtures justify it.",
	},
	{
		connector: "ical",
		file: "index.ts",
...
function declaresFilesystemBinding(manifest: unknown): boolean {
	...
	const runtimeRequirements = (manifest as { runtime_requirements?: unknown })
	...
	return (
		...
		filesystem !== null &&
		(filesystem as { required?: unknown }).required === true
	);
}
