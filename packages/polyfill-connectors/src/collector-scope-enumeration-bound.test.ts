...
const CLAUDE_EXCLUDED_SENTINEL = `${JSON.stringify({
	...
	message: {
		content: [{ text: "PDPP_SENTINEL_MUST_NOT_BE_READ", type: "text" }],
		role: "user",
	},
 ...
 */
async function seedClaudeHome(): Promise<{
	claudeHome: string;
	projectsDir: string;
}> {
	...
	await writeFile(
		join(projectsDir, "-home-u-code-excluded", "session.jsonl"),
		CLAUDE_EXCLUDED_SENTINEL,
		"utf8",
	);
...
function runConnectorChild(input: {
	...
	const streamScope = input.streams.map((name) => ({
		...
		...(input.scope?.source_roots
			? { source_roots: input.scope.source_roots }
			: {}),
	}));
	return new Promise((resolve, reject) => {
		const child = spawn(
			process.execPath,
			["--import", "tsx", join(CONNECTORS_DIR, input.connector, "index.ts")],
			{
				env: {
					...process.env,
		...
		child.stdin.end(
			`${JSON.stringify({ scope: { streams: streamScope }, type: "START" })}\n`,
		);
	});
}
...
test("claude_code discovery prunes out-of-root projects before any file is opened", async () => {
	...
	const bounded = await discoverClaudeJsonlSources(projectsDir, noopEmit, {
		source_roots: ["/home/u/code/included"],
	});
	assert.ok(
...
test("claude_code: a root matching zero projects surfaces an actionable skip, not a silent empty pass", async () => {
	...
	assert.ok(
		out.includes("scope_matched_no_sources"),
		"a zero-match scope must be reported, never silently accepted",
	);
	assert.ok(
...
test("claude_code: a natural absolute owner path collects the project it names", async () => {
	...
	assert.equal(
		out.includes("scope_matched_no_sources"),
		false,
		"the natural path must match, not read as a typo",
	);
	assert.ok(
		out.includes('"type":"RECORD"'),
		"the named project must actually be collected",
	);
});
