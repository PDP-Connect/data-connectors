...
export function applyScrubRules(
	...
	for (const rule of rules) {
		...
		if (typeof rule.replacement === "string") {
			...
			out = out.replace(rule.pattern, (substring: string, ...args: string[]) =>
				replacement(substring, ...args),
			);
		}
	}
	return out;
...
export function parseStructuredRedactionPlan(
	value: unknown,
): StructuredRedactionPlan {
...
export function applyStructuredRedactionPlan(
	content: string,
	plan: StructuredRedactionPlan,
): string {
	let out = content;
...
export async function loadConnectorScrubRules(
	...
	)) as ConnectorScrubRuleModule;
	const rules = mod.scrubRules ?? mod.default ?? [];
	if (!Array.isArray(rules)) {
		console.warn(
			`${rulesFile}: expected scrubRules array; got ${typeof rules}`,
		);
		return [];
	}
...
function parseStructuredRedaction(
	value: unknown,
	index: number,
): StructuredRedaction {
	if (!value || typeof value !== "object") {
	...
	if (
		...
		throw new Error(
			`redaction ${index} replacement must be a [REDACTED_*] placeholder`,
		);
	}
