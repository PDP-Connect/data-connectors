 ...
 *   is what lets the config surface reach every connector without a
 ...
 *
 * This file mirrors PDP-Connect/pdpp's
 * `packages/polyfill-connectors/src/connector-options-schema.ts`
 * semantically (contract, not bytes -- see this repo's own formatting
 * conventions), and is what
 * `reference-implementation/server/routes/owner-connection-config.ts` (pdpp)
 * imports for `GET .../config`'s `options_schema` field.
 * `.github/scripts/cross-repo-integrity/check-connector-option-kind-drift.mjs`
 * enforces that the two stay in agreement.
 */
...
function readOptionType(
	...
	if (!isRecord(items) || items.type !== "string") {
		throw new ConnectorOptionsSchemaError(
			`${where}: an array option must declare items.type "string"`,
		);
	}
	return "string_array";
}
...
function readDefaultValue(
	...
	if (raw === undefined) {
		throw new ConnectorOptionsSchemaError(
			`${where}: a default is required, so a form can render an unset option`,
		);
	}
	switch (type) {
		case "boolean": {
			if (typeof raw !== "boolean") {
				throw new ConnectorOptionsSchemaError(
					`${where}: default must be a boolean`,
				);
			}
			return raw;
		}
		case "integer": {
			if (typeof raw !== "number" || !Number.isInteger(raw)) {
				throw new ConnectorOptionsSchemaError(
					`${where}: default must be an integer`,
				);
			}
			return raw;
		}
		case "string": {
			if (typeof raw !== "string") {
				throw new ConnectorOptionsSchemaError(
					`${where}: default must be a string`,
				);
			}
			return raw;
		}
		default: {
			if (
				...
				throw new ConnectorOptionsSchemaError(
					`${where}: default must be an array of strings`,
				);
			}
...
function readBound(
	...
	if (typeof raw !== "number" || !Number.isInteger(raw)) {
		throw new ConnectorOptionsSchemaError(
			`${where}: ${field} must be an integer when declared`,
		);
	}
	return raw;
}
...
function readEnumValues(where: string, raw: unknown): readonly string[] | null {
	...
	if (
		...
		throw new ConnectorOptionsSchemaError(
			`${where}: enum must be a non-empty array of strings when declared`,
		);
	}
	return Object.freeze([...(raw as string[])]);
}
...
function readDescription(where: string, raw: unknown): string {
	if (typeof raw !== "string" || raw.trim().length === 0) {
		throw new ConnectorOptionsSchemaError(
			`${where}: description must be a non-empty string`,
		);
	}
	return raw;
}
...
function resolveOption(
	...
	if (minimum !== null && maximum !== null && minimum > maximum) {
		throw new ConnectorOptionsSchemaError(
			`${where}: minimum ${minimum} exceeds maximum ${maximum}`,
		);
	}
...
export function resolveOptionsSchemaFromManifest(
	...
	if (!isRecord(raw)) {
		throw new ConnectorOptionsSchemaError(
			`${file}: options_schema must be an object`,
		);
	}
	if (raw.type !== "object") {
		throw new ConnectorOptionsSchemaError(
			`${file}: options_schema.type must be "object"`,
		);
	}
	const { properties } = raw;
	if (!isRecord(properties)) {
		throw new ConnectorOptionsSchemaError(
			`${file}: options_schema.properties must be an object`,
		);
	}
	...
	for (const optionKey of Object.keys(properties).sort()) {
		...
		if (!isRecord(prop)) {
			throw new ConnectorOptionsSchemaError(
				`${file}: options_schema.properties.${optionKey} must be an object`,
			);
		}
		options.push(
			resolveOption(
				connectorKey,
				`${file}: options_schema.${optionKey}`,
				optionKey,
				prop,
			),
		);
 ...
 */
export function connectorOptionsSchemas(): Readonly<
...
export function connectorOptionsSchema(
