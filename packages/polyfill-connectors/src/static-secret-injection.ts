...
function freezeStaticSecretDescriptor(
	descriptor: StaticSecretConnectorDescriptor,
): StaticSecretConnectorDescriptor {
...
function mappingFromGenerated(
	generated: GeneratedStaticSecretDescriptor,
	...
	return {
		...
		...(generated.optionalSecretBundleFields
			? {
					optionalSecretBundleFields: new Set(
						generated.optionalSecretBundleFields,
					),
				}
			: {}),
		...(generated.secretEnvVars
			? { secretEnvVars: generated.secretEnvVars }
			: {}),
		...(generated.secretFieldEnvVars
			? { secretFieldEnvVars: generated.secretFieldEnvVars }
			: {}),
		...(generated.setupFieldEnvVars
			? { setupFieldEnvVars: generated.setupFieldEnvVars }
			: {}),
	};
}
 ...
 */
const LEGACY_CREDENTIAL_KIND_MIGRATIONS: Readonly<
	Record<string, readonly StaticSecretInjectionMapping[]>
> = Object.freeze({
	jellyfin: [
		{
 ...
 */
export const STATIC_SECRET_CONNECTOR_REGISTRY: Readonly<
	Record<string, StaticSecretConnectorDescriptor>
> = Object.freeze(
	Object.fromEntries(
		Object.entries(GENERATED_STATIC_SECRET_REGISTRY).map(
			([connectorId, generated]) => {
				const acceptedCredentialVariants =
					LEGACY_CREDENTIAL_KIND_MIGRATIONS[connectorId];
				return [
					connectorId,
...
export function isStaticSecretCaptureOptional(connectorId: string): boolean {
	return (
		STATIC_SECRET_CONNECTOR_REGISTRY[connectorId]?.captureRequired === false
	);
}

function setupFieldsFromSourceBinding(
...
function injectionMappingForRecoveredSecret(
	...
	if (
		!recovered ||
		typeof recovered.secret !== "string" ||
		recovered.secret.length === 0
	) {
		throw new StaticSecretInjectionError(
	...
	const expectedKinds = [
		...
		...(descriptor.acceptedCredentialVariants ?? []).map(
			(v) => v.credentialKind,
		),
	];
...
export function buildConnectionScopedSecretEnv(
	...
	const mapping = injectionMappingForRecoveredSecret(
		connectorId,
		descriptor,
		recovered,
	);
