...
test("authOptional: a missing credential yields no credentials WITHOUT ever prompting the owner", async () => {
	...
	try {
		...
		assert.deepEqual(
			credentials,
			{},
			"a missing credential must resolve to an empty credential set, not throw",
		);
	} finally {
		restore();
	}
...
test("authOptional silences credential resolution WITHOUT closing the interaction channel", async () => {
	...
	try {
		...
		assert.deepEqual(
			delivered,
			[],
			"credential resolution must be silent for a session-first connector",
		);
		...
		assert.equal(
			manual.status,
			"success",
			"a manual_action interaction must still reach the owner",
		);
		assert.deepEqual(
			delivered,
			["manual_action"],
			"only the credential question is skipped",
		);
	} finally {
		restore();
	}
...
test("authOptional stays OFF by default: a missing credential still PROMPTS and still fails closed", async () => {
	...
	assert.deepEqual(
		kinds,
		["credentials"],
		"a required credential must still be requested from the owner",
	);
});
...
test("authOptional never fabricates credentials: a STORED credential still resolves and never prompts", async () => {
	...
	assert.equal(
		prompted,
		0,
		"a fully-stored credential must never raise an interaction",
	);
});
...
test("the chatgpt connector opts in — the production connection that failed is covered", async () => {
	...
	assert.match(
		source,
		/authOptional:\s*true/,
		"connectors/chatgpt/index.ts must set authOptional: true",
	);
});
...
test("the INSTALLED protocol build honours authOptional — not just the source we intended to ship", async () => {
	...
	assert.deepEqual(
		calls,
		[],
		"the installed protocol build still prompts: vendor/ tarball or lock integrity is stale",
	);
});
...
test("the credentials prompt makes no false persistence promise", async () => {
	...
	assert.doesNotMatch(
		message,
		/\.env\.local/,
		"the prompt must not tell the owner to edit .env.local",
	);
	assert.doesNotMatch(
		message,
		/persistence/i,
		"the prompt must not promise persistence it does not provide",
	);
	assert.match(
		message,
		/CHATGPT_USERNAME/,
		"the prompt must still name what it needs",
	);
});
