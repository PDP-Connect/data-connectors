...
function classify(raw: RawResponse): {
	status: number;
	headers?: Record<string, string | undefined>;
	value: unknown;
} {
...
const TEST_PROFILE = {
	pacingMinIntervalMs: DEFAULT_PACING_MIN_INTERVAL_MS,
} as const;
...
test("connector governor: Retry-After double-pay guard — the server interval is slept once, not stacked on backoff", async () => {
	...
	assert.deepEqual(
		slept,
		[3000],
		"no double-pay: backoff is not added on top of Retry-After",
	);
});
	...
	await assert.rejects(
		...
		(err: unknown) => {
			...
			assert.equal(
				...
				"github_rate_limited",
				"matches the existing retryablePattern token",
			);
			return true;
		},
	);
...
test("connector governor: ratio-based retry budget caps retry volume across requests", async () => {
	...
	assert.equal(
		calls,
		2,
		"1 initial + 1 budgeted retry, then the retry budget stops the run (not 10 attempts)",
	);
});
	...
	await assert.rejects(
		...
		(err: unknown) => {
			assert.ok(
				...
				"a 5xx exhaustion is not a rate-limit terminal",
			);
			assert.ok(!(err instanceof ConnectorRateLimitedError));
			return true;
		},
...
test("connector governor: per-provider isolation — two governors throttle independently", async () => {
	...
	assert.notEqual(
		a.governor,
		b.governor,
		"distinct governor instances per provider",
	);
});
...
test("connector governor: pacing is the single pre-flight gate (one wait source per attempt)", async () => {
	...
	assert.equal(
		probe.count,
		1,
		"exactly one pre-flight wait — the single pacing governor",
	);
});
...
test("adaptive default: a profiled governor cold-starts at the shared discovery seed and exposes a snapshot", () => {
	...
	assert.ok(
		snap,
		"a profiled governor has live rate state by default (pacing is on)",
	);
	assert.equal(
		snap.intervalMs,
		DEFAULT_PACING_INITIAL_INTERVAL_MS,
	...
	assert.equal(
		snap.lastBackoff,
		null,
		"no back-off has fired on a fresh governor",
	);
});
...
test("adaptive default: sustained success ACCELERATES the profiled governor toward the ceiling (the proven adaptive behavior)", async () => {
	...
	assert.ok(
		after < before,
		`interval shrank under success: ${after}ms < ${before}ms (the rate accelerated)`,
	);
	assert.ok(
		after >= DEFAULT_PACING_MIN_INTERVAL_MS,
		"acceleration never crosses the rate ceiling",
	);
});
...
test("adaptive default: a throttle BACKS OFF the profiled governor and the back-off is legible in the snapshot", async () => {
	...
	const statuses = [429, 200];
	await g.request(
		() => ({ status: statuses.shift() ?? 200, body: null }),
		classify,
	);
	const snap = g.snapshot();
	assert.ok(
		snap?.lastBackoff,
		"the throttle is recorded as a legible back-off event",
	);
	assert.equal(
		snap.lastBackoff.reason,
		"throttle",
		"back-off reason surfaces for the operator",
	);
	assert.ok(
...
test("adaptive default: pacingInitialIntervalMs:0 opts OUT (no snapshot, byte-identical pre-convergence path)", async () => {
	...
	assert.equal(
		g.snapshot(),
		null,
		"opting out disables the rate controller entirely",
	);
	await g.request(() => ({ status: 200, body: null }), classify);
	assert.equal(
		probe.count,
		0,
		"no pre-flight wait when pacing is off — the old conservative path",
	);
});
...
test("warm-start seam: round-trip — a run persists its learned interval; the next run restores it via the shared helpers", async () => {
	...
	assert.ok(
		learned < DEFAULT_PACING_INITIAL_INTERVAL_MS,
		"run 1 learned a faster interval than the cold seed",
	);
	...
	const restored = readPersistedPacingInterval(persistedSlice, {
		now: () => now + 1000,
	});
	assert.equal(
		restored,
		learned,
		"warm-start restores the learned interval, not the cold seed",
	);
...
test("warm-start seam: a throttle-inflated interval is CAPPED at cold-start on persist (no seed poisoning)", () => {
	...
	const throttled = createConnectorHttpGovernorWithInterval(
		"seed-poison",
		14_300,
	);
	assert.ok(
		(throttled.snapshot()?.intervalMs ?? 0) >
			DEFAULT_PACING_INITIAL_INTERVAL_MS,
	...
	assert.ok(
		(healthy.snapshot()?.intervalMs ?? 0) < DEFAULT_PACING_INITIAL_INTERVAL_MS,
	);
	assert.equal(
		buildPacingStateFields(healthy, { now: () => 1_000_000 })
			.pacing_interval_ms,
		400,
...
test("warm-start seam: a stale persisted interval is discarded (cold start against a possibly-reset quota)", () => {
	const now = 1_000_000;
	const persistedSlice = buildPacingStateFields(
		createConnectorHttpGovernorWithInterval("oura", 500),
		{
			now: () => now,
		},
	);
	...
	const restored = readPersistedPacingInterval(persistedSlice, {
		now: () => now + 100 * 60 * 60 * 1000,
	});
	assert.equal(
		restored,
		null,
		"a long-idle resume discards the stale learned rate and cold-starts conservatively",
	);
});
...
test("warm-start seam: an absent / malformed state slice yields null (cold start)", () => {
	...
	assert.equal(
		readPersistedPacingInterval({}),
		null,
		"no persisted fields → cold start",
	);
	assert.equal(
		readPersistedPacingInterval({
			pacing_interval_ms: "nan",
			pacing_recorded_at_ms: Date.now(),
		}),
		null,
...
test("warm-start staleness window: a learned interval persisted 1h ago is restored (within 6h window)", () => {
	...
	const restored = readPersistedPacingInterval(slice, {
		now: () => persistedAt + ONE_HOUR_MS,
	});
	assert.equal(
		restored,
		750,
...
test("warm-start staleness window: a learned interval persisted 7h ago is discarded (beyond 6h window)", () => {
	...
	const restored = readPersistedPacingInterval(slice, {
		now: () => persistedAt + SEVEN_HOURS_MS,
	});
	assert.equal(
		restored,
		null,
...
test("observability: buildCollectionRateProgress turns a profiled governor's snapshot into legible rate state carrying no account content", () => {
	...
	assert.equal(
		rate.effective_rate_per_min,
		Math.round(60_000 / DEFAULT_PACING_INITIAL_INTERVAL_MS),
	);
	assert.equal(
		rate.ceiling_rate_per_min,
		Math.round(60_000 / DEFAULT_PACING_MIN_INTERVAL_MS),
	);
...
function createConnectorHttpGovernorWithInterval(
	name: string,
	intervalMs: number,
) {
