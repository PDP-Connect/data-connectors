...
function importConnectorInChild(
...
function runConnectorEntrypointWithClosedStdin(
	connectorRelPath: string,
...
function runInteractionWithClosedStdin(): Promise<EntrypointResult> {
	return new Promise((resolvePromise) => {
		const runtimeUrl = pathToFileURL(
			join(PACKAGE_ROOT, "src/connector-runtime.ts"),
		).href;
...
test("importing claude_code/index.ts (non-browser connector) in a child process exits cleanly without firing runConnector", {
	...
	const result = await importConnectorInChild(
		"connectors/claude_code/index.ts",
	);
	assert.equal(result.timedOut, false, `child hung: stderr=${result.stderr}`);
	assert.equal(
		result.code,
...
test("a connector entrypoint with closed stdin fails closed instead of hanging before START", {
	...
	const result = await runConnectorEntrypointWithClosedStdin(
	...
	assert.equal(
		result.code,
		1,
		`expected fail-closed exit code 1: stdout=${result.stdout} stderr=${result.stderr}`,
	);
	assert.match(
		result.stdout,
		/"type":"DONE"/,
		"missing START should emit a terminal DONE envelope",
	);
	assert.match(
		result.stdout,
		/"status":"failed"/,
		"missing START should fail the DONE envelope",
	);
	assert.match(result.stdout, /Missing START message before stdin closed/);
});

test("an interaction with closed stdin settles instead of hanging", {
	timeout: IMPORT_TIMEOUT_MS + 5000,
}, async () => {
