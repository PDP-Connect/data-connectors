...
const HOME_ROOTED_READ_EXEMPTIONS: Readonly<Record<string, string>> = {
	...
	claude_code:
		"reads ~/.claude (CLAUDE_CODE_HOME), a foreign tool's own directory",
	...
	twitter_archive:
		"reads the user's Twitter archive drop-box (TWITTER_ARCHIVE_DIR)",
	apple_health:
		"reads the user's Apple Health export drop-box (APPLE_HEALTH_EXPORT_DIR)",
	apple_photos:
		"reads the user's Apple Photos export drop-box (APPLE_PHOTOS_EXPORT_DIR)",
};
...
test("no connector derives a durable artifact path from the home directory", () => {
	...
	for (const connector of connectorNames()) {
		...
		for (const file of connectorSourceFiles(connector)) {
			...
			if (
				/\bhomedir\s*\(\s*\)/.test(source.replace(/^\s*(?:\/\/|\*).*$/gm, ""))
			) {
...
test("every exemption names a connector that still exists", () => {
	...
	for (const connector of Object.keys(HOME_ROOTED_READ_EXEMPTIONS)) {
		assert.ok(
			present.has(connector),
			`exemption names a missing connector: ${connector}`,
		);
	}
});
