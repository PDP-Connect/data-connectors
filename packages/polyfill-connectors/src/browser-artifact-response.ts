...
export function suggestedFilenameFromHeaders(
...
export function isLikelyPdfResponseBody(
	body: Buffer,
	headers: Record<string, string>,
): boolean {
	if (body.length === 0) {
	...
	return (
		contentType.includes("application/pdf") || disposition.includes(".pdf")
	);
}
...
export function attachBodyResponseQueue(
	page: Page,
	options: BodyResponseQueueOptions,
): BodyResponseQueue {
	...
	const truncateMessageLength =
		options.truncateMessageLength ?? DEFAULT_ERROR_SLICE;
	let detached = false;
	...
	const onCdpRequestWillBeSent = (event: {
	...
	const onCdpLoadingFailed = (event: {
		errorText?: string;
		requestId?: string;
	}): void => {
		if (!event.requestId) {
