...
export const LOCAL_DEVICE_CONNECTOR_PROFILES: Readonly<
	Record<string, LocalDeviceConnectorProfile>
> = {
	[CODEX_CONNECTOR_ID]: {
...
export function resolveLocalDeviceConnectorProfile(
	connectorId: string,
): LocalDeviceConnectorProfile {
	const profile = LOCAL_DEVICE_CONNECTOR_PROFILES[connectorId];
	if (!profile) {
		const known = Object.keys(LOCAL_DEVICE_CONNECTOR_PROFILES).join(", ");
		throw new Error(
			`unsupported local-device connector "${connectorId}" (known: ${known})`,
		);
	}
	return profile;
}
...
export async function enrollLocalDevice(
	config: LocalDeviceEnrollmentConfig,
): Promise<EnrollmentExchangeResponse> {
 ...
 * connector instance.
 */
export async function runLocalDeviceExporter(
	config: LocalDeviceRuntimeConfig,
): Promise<LocalDeviceRuntimeResult> {
	const profile = resolveLocalDeviceConnectorProfile(
		config.connectorId ?? CODEX_CONNECTOR_ID,
	);
	...
	await client.heartbeat({
		...
		records_pending: (await queue.list()).filter(
	...
	const diagnosticRecords = allRecords.filter(
		(record) => record.stream === LOCAL_COVERAGE_DIAGNOSTICS_STREAM,
	);
	const substantiveRecords = allRecords.filter(
		(record) => record.stream !== LOCAL_COVERAGE_DIAGNOSTICS_STREAM,
	);
	const sampledSubstantive =
		sampleLimit !== undefined && sampleLimit >= 0
			? substantiveRecords.slice(0, sampleLimit)
			: substantiveRecords;
	const truncatedBySample =
		sampledSubstantive.length < substantiveRecords.length;
...
export function runCodexLocalDeviceExporter(
	config: LocalDeviceRuntimeConfig,
): Promise<LocalDeviceRuntimeResult> {
...
export function buildLocalDeviceStartMessage(
	streams: readonly string[],
): StartMessage {
	return {
...
export function buildCodexStartMessage(
	streams: readonly string[] = DEFAULT_CODEX_STREAMS,
): StartMessage {
...
function nextBatchSeq(
	items: readonly LocalDeviceQueueItem[],
	sourceInstanceId: string,
): number {
	return (
		Math.max(
