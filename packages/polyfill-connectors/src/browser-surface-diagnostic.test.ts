...
test("browser surface diagnostic maps only runtime launch posture", () => {
	...
	assert.equal(
		browserSurfaceManagedState("legacy_remote_cdp"),
		"legacy_remote",
	);
	assert.equal(browserSurfaceManagedState("isolated_local"), "isolated");
	assert.equal(
		browserSurfaceManagedState("https://private.example/?token=secret"),
		"unknown",
	);
});
...
test("browser surface diagnostic rejects free text, URLs, identifiers, invalid enums, and fixture references", () => {
	const input = chaseInput({
		fixture: {
			reference: "usaa/accounts/account-12345678.html",
			sha256: "a".repeat(64),
		},
	...
	assert.doesNotMatch(
		...
		/alice|private\.example|account-12345678|raw-secret/i,
	);
});
...
test("browser surface diagnostic has a fixed finite structural shape", () => {
	const diagnostic = buildBrowserSurfaceDiagnostic(
		chaseInput({
			parserCount: Number.POSITIVE_INFINITY,
			readCount: 1.5,
			targetCount: 9_999_999,
		}),
	);
