...
function makeFakeFetch(
	...
	const fetchImpl: typeof fetch = async (input, init) => {
		...
		if (next === undefined) {
			throw new Error(
				`fakeFetch: no response queued for request #${cursor} (${method} ${url})`,
			);
		}
...
test("createRegistrationClient throws when baseUrl is missing", () => {
	assert.throws(
		() => createRegistrationClient({ baseUrl: "", deviceToken: "token" }),
		/baseUrl required/,
	);
});
...
test("register PUTs to /admin/runs/:runId/interactions/:interactionId/streaming-target with bearer header and ws_url body", async () => {
	...
	assert.equal(
		sent.url,
		`http://127.0.0.1:7662${REGISTRATION_PATH("run_123", "int_456")}`,
	);
...
test("register POSTs n.eko descriptor to /admin/runs/:runId/interactions/:interactionId/streaming-target", async () => {
	...
	assert.equal(
		sent.url,
		`http://127.0.0.1:7662${REGISTRATION_PATH("run_neko", "int_neko")}`,
	);
...
test("register URL-encodes runId AND interactionId in the path", async () => {
	...
	assert.equal(
		seen[0]?.url.endsWith(
			"/admin/runs/run%2Fwith%2Fslashes/interactions/int%20with%20spaces/streaming-target",
		),
		true,
	);
});
...
test("register returns false and does NOT call fetch when wsUrl host is not allowed", async () => {
	...
	assert.equal(
		seen.length,
		0,
		"fetch must not be called for a disallowed wsUrl host",
	);
...
test("register returns false (does not throw) on 4xx with body", async () => {
	const { fetchImpl } = makeFakeFetch([
		new Response(
			'{"error":{"code":"run_target_invalid_url","message":"bad"}}',
			{ status: 400 },
		),
	]);
	const client = createRegistrationClient({
...
test("unregister DELETEs to /admin/runs/:runId/interactions/:interactionId/streaming-target with bearer header", async () => {
	...
	const ok = await client.unregister({
		runId: "run_999",
		interactionId: "int_888",
	});
	...
	assert.equal(
		seen[0]?.url,
		`http://127.0.0.1:7662${REGISTRATION_PATH("run_999", "int_888")}`,
	);
...
test("unregister returns false silently when runId or interactionId is empty", async () => {
	...
	assert.equal(
		await client.unregister({ runId: "", interactionId: "int_a" }),
		false,
	);
	assert.equal(
		await client.unregister({ runId: "run_a", interactionId: "" }),
		false,
	);
	assert.equal(seen.length, 0);
});
...
test("resolveStreamingRegistrationFromEnv prefers PDPP_STREAMING_REGISTRATION_TOKEN over PDPP_LOCAL_DEVICE_TOKEN", async () => {
	...
	assert.ok(
		hooks,
		"hooks should be returned when all three pieces are present",
	);
	...
	const baseUrl =
		env.PDPP_REFERENCE_BASE_URL ??
		assert.fail("PDPP_REFERENCE_BASE_URL is set in fixture");
	const client = createRegistrationClient({
		baseUrl,
		// Mirror the resolver's precedence: registration token wins over device token.
		deviceToken:
			env[STREAMING_REGISTRATION_TOKEN_ENV] ||
			env[LOCAL_DEVICE_TOKEN_ENV] ||
			"",
		fetch: fetchImpl,
	});
	await client.register({
