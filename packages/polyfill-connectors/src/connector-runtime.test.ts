...
import {
	manualBrowserLogin,
	type PrepareBrowserInteractionTargetArgs,
...
test("a self-resolved browser handoff registers before emission and unregisters after terminal completion", async () => {
	...
	const lifecycle = createBrowserSurfaceAssistanceLifecycle({
		...
		prepareTarget: ({ interactionId }: PrepareBrowserInteractionTargetArgs) => {
			...
			assert.ok(
				interactionId,
				"the lifecycle supplies the generated assistance ID to the registration target",
			);
	...
	await manualBrowserLogin({
		...
		message:
			"Finish sign-in in the secure browser. PDPP continues automatically.",
		page: ownerPage,
	...
	assert.deepEqual(completions, [
		{ id: "assist_streamed_login", status: "resolved" },
	]);
...
test("the recording env-var names mirrored in connector-runtime still match browser-launch's exported source of truth", async () => {
	...
	const { HAR_RECORD_PATH_ENV, STORAGE_STATE_RECORD_PATH_ENV } = await import(
		"./browser-launch.ts"
	);
	...
	assert.equal(
		shouldCloseBrowserPageAfterRun({}, true, {
			[HAR_RECORD_PATH_ENV]: "/tmp/x.har",
		}),
		false,
	);
	assert.equal(
		shouldCloseBrowserPageAfterRun({}, true, {
			[STORAGE_STATE_RECORD_PATH_ENV]: "/tmp/x.json",
		}),
		false,
	);
});
...
test("shouldCloseBrowserPageAfterRun keeps the page open when --record-har is active, regardless of preserve flags or outcome", () => {
	...
	const storageStateEnv = {
		PDPP_SCENARIO_STORAGE_STATE_RECORD_PATH: "/tmp/run1.storage-state.json",
	};
	...
	assert.equal(
		shouldCloseBrowserPageAfterRun({}, true, storageStateEnv),
		false,
	);
