...
export function isLocalJsonlPhysicalCursorV1(
	value: unknown,
): value is LocalJsonlPhysicalCursorV1 {
	if (!isRecord(value)) {
		return false;
	}
	return (
...
async function verifyStablePath(
	...
	if (
		...
		throw new LocalJsonlUnstableSourceError(
			"local JSONL path changed while scanning",
		);
	}
	if (afterHandle.size < snapshot.size) {
		throw new LocalJsonlUnstableSourceError(
			"local JSONL source shrank while scanning",
		);
	}
	if (
		afterHandle.size === snapshot.size &&
		afterHandle.mtimeMs !== snapshot.mtimeMs
	) {
		throw new LocalJsonlUnstableSourceError(
			"local JSONL source mutated while scanning",
		);
	}
}
...
async function proveCommittedPrefix(input: {
	...
	if (first !== input.expectedSha256 || second !== input.expectedSha256) {
		throw new LocalJsonlUnstableSourceError(
			"local JSONL committed prefix changed while scanning",
		);
	}
...
export async function scanLocalJsonl({
	...
	try {
		...
		if (
			!(prior && isLocalJsonlPhysicalCursorV1(prior)) ||
		...
		while (position < snapshot.size) {
			...
			if (bytesRead !== buffer.length) {
				throw new LocalJsonlUnstableSourceError(
					"local JSONL source ended during a snapshot read",
				);
			}
			position += bytesRead;
			pending =
		...
		return {
			cursor,
			decision:
				linesDelivered === 0 && decision.kind === "append"
					? { kind: "verified_noop" }
					: decision,
