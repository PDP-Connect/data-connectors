...
const AURORA_MANIFEST: ProviderAuthManifestLike = Object.freeze({
	capabilities: {
		auth: {
			...
			deployment_config: [
				{
					env_alias: "AURORA_OAUTH_CLIENT_ID",
...
test("initiateAuthorization builds an authorization URL from the manifest and applies only manifest-declared authorization_params, no implicit defaults", async () => {
	...
	assert.equal(
		...
		"https://auth.aurora.example/o/oauth2/v2/auth",
	);
	...
	assert.equal(
		...
		"https://api.aurora.example/scopes/notes.readonly",
	);
...
test("initiateAuthorization applies no authorization_params at all when the manifest declares none", async () => {
	...
	assert.equal(
		...
		null,
		"no implicit access_type default",
	);
	assert.equal(
		url.searchParams.get("prompt"),
		null,
		"no implicit prompt default",
	);
	assert.equal(
		url.searchParams.get("include_granted_scopes"),
		null,
		"no implicit include_granted_scopes default",
	);
});
...
test("exchangeCode resolves deployment values via the injected resolver by logical_key, never a literal env var", async () => {
	...
	try {
		...
		assert.ok(
			seenArgs.every(
				(a) =>
					a.envAlias === "AURORA_OAUTH_CLIENT_ID" ||
					a.envAlias === "AURORA_OAUTH_CLIENT_SECRET",
			),
		);
	} finally {
		fetchStub.restore();
...
test("runInventoryOrTest resolves identity via the manifest-declared userinfo_url", async () => {
	...
	assert.ok(adapter);
	const fetchStub = makeFetch([
		jsonResponse({ email: "owner@example.com", id: "aurora-user-1" }),
	]);
	...
	try {
		...
		assert.equal(
			...
			"https://api.aurora.example/userinfo",
		);
		assert.equal(
			result.persistenceContext,
			undefined,
			"the generic adapter carries no persistence context",
		);
	} finally {
		fetchStub.restore();
	}
