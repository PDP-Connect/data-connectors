...
test("resolveTsxBinary resolves a real tsx from this package (regression: the live ENOENT)", () => {
	...
	assert.ok(
		resolved,
		"expected the monorepo's tsx to be resolvable from the package root",
	);
	assert.ok(resolved.endsWith(join("node_modules", ".bin", "tsx")));
	assert.ok(
		resolved.startsWith("/"),
		"resolved tsx must be an absolute path, not a bare command",
	);
});

test("resolveConnectorCommand rewrites a bare tsx to the resolved absolute path", () => {
	const resolved = resolveConnectorCommand(
		"tsx",
		() => "/opt/tools/node_modules/.bin/tsx",
	);
	...
	assert.throws(
		...
		(error: unknown) => {
			...
			assert.match(
				error.message,
				/tsx/,
				"the error must name the missing binary",
			);
			assert.doesNotMatch(
				error.message,
				/ENOENT/,
				"must not surface a bare spawn ENOENT",
			);
			return true;
		},
	);
