...
export const LOCAL_COVERAGE_STORES_BY_CONNECTOR = Object.entries(
	LOCAL_COVERAGE_STORE_DESCRIPTORS_BY_CONNECTOR,
).reduce<Record<string, readonly string[]>>((acc, [connector, stores]) => {
	acc[connector] = stores.map((store) => store.store);
	return acc;
}, {}) as LocalCoverageStoreNamesByConnector;

export type LocalCoverageConnector =
	keyof typeof LOCAL_COVERAGE_STORE_DESCRIPTORS_BY_CONNECTOR;
...
export function expectedLocalCoverageStores(
	connectorId: string,
...
export function expectedLocalCoverageStoreDescriptors(
	...
	return normalized in LOCAL_COVERAGE_STORE_DESCRIPTORS_BY_CONNECTOR
		? LOCAL_COVERAGE_STORE_DESCRIPTORS_BY_CONNECTOR[
				normalized as LocalCoverageConnector
			]
		: null;
}
...
export function localCoverageStreamsMissingDescriptors(
	...
	const declaredStreams = new Set(
		(descriptors ?? []).map((descriptor) => descriptor.stream).filter(Boolean),
	);
	return requiredStreams
		.filter((stream) => !declaredStreams.has(stream))
		.sort();
}
 ...
 */
function assertExpectedLocalCoverageStores(
	tool: string,
	...
	const actual = stores
		.map((store) => `${store.store}\u0000${store.stream ?? ""}`)
		.sort();
	const expectedSet = new Set(
		expectedDescriptors.map(
			(store) => `${store.store}\u0000${store.stream ?? ""}`,
		),
	);
	if (
		new Set(actual).size !== actual.length ||
		actual.some((store) => !expectedSet.has(store))
	) {
		throw new Error(
			`${tool} local coverage declaration diverges from its authoritative expected-store set`,
		);
	}
}
...
export function buildDerivedCoverageRecord(input: {
	...
	const descriptor = selectLocalCoverageDerivedDescriptor(
		input.connectorId,
		input.stream,
	);
	const status: CoverageStatus | "unaccounted" = input.scanComplete
		? "collected"
		: "unaccounted";
	return {
		...
		...(input.scopeFingerprint
			? { collection_scope: input.scopeFingerprint }
			: {}),
	};
}
...
export function selectLocalCoverageDerivedDescriptor(
	...
	if (!descriptors) {
		throw new Error(
			`${connectorId} has no authoritative local coverage inventory`,
		);
	}
...
const SAFE_COVERAGE_DIAGNOSTIC_STATUSES = new Set<
...
const COVERAGE_DIAGNOSTICS_STATE_ENTRY_KEYS = [
	"status",
	"store",
	"stream",
] as const;
 ...
 */
const COVERAGE_DIAGNOSTICS_STATE_ENTRY_KEYS_WITH_SCOPE = [
	"collection_scope",
	"status",
	"store",
	"stream",
...
function parseCoverageDiagnosticStateEntry(rawEntry: unknown): {
	...
	const store =
		typeof entry.store === "string" && entry.store ? entry.store : null;
	const { status } = entry;
	if (
		!store ||
...
export function parseCoverageDiagnosticsStateSnapshot(
	...
	const expectedByStore = new Map(
		expected.map((entry) => [entry.store, entry]),
	);
	...
	for (const rawEntry of stores) {
		...
		rows.push({
			...(entry.collection_scope
				? { collection_scope: entry.collection_scope }
				: {}),
			store,
	...
	const hasCommittedSnapshot =
		!malformed && duplicateStores.length === 0 && missingStores.length === 0;
	return {
...
export async function buildLocalSourceInventory(
	...
	for (const { pathMeta, store } of storesWithMeta) {
		...
		if (
			...
			(store.classification !== "inventory_only" &&
				store.classification !== "defer")
		) {
			continue;
		}
...
export const INVENTORY_FINGERPRINT_EXCLUDE_KEYS = [
	"mtime_epoch",
	"size_bytes",
] as const;
...
export function openInventoryFingerprintCursor(
	priorState: unknown,
): FingerprintCursor {
