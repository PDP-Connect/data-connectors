...
export class ProviderPacing {
	...
	constructor(options: PacingOptions) {
		const configuredInitialIntervalMs =
		...
		this.initialIntervalMs = Math.max(
			configuredInitialIntervalMs,
			this.minIntervalMs,
		);
		this.burstToleranceMs =
			options.burstToleranceMs ?? 2 * this.initialIntervalMs;
		this.additiveIncreaseMs =
			options.additiveIncreaseMs ?? DEFAULT_ADDITIVE_INCREASE_MS;
		...
		const postStepIntervalMs = Math.max(
			1,
			this.initialIntervalMs - this.additiveIncreaseMs,
		);
		this.additiveRateStepPerMin =
			MS_PER_MIN / postStepIntervalMs - MS_PER_MIN / this.initialIntervalMs;
		...
		this.recoveryGain =
			typeof options.recoveryGain === "number" &&
			Number.isFinite(options.recoveryGain) &&
			options.recoveryGain >= 0
		...
		this.maxIntervalMs =
			typeof options.maxIntervalMs === "number" &&
			Number.isFinite(options.maxIntervalMs) &&
	...
	private static resolveInitialInterval(
		...
		if (
			options.restoredAtMs !== undefined &&
			options.maxWarmStartAgeMs !== undefined
		) {
	...
	private nextTatAfterIdleCredit(nowMs: number): number {
		const maxIdleCreditMs = Math.max(
			0,
			2 * this._currentIntervalMs - this.minIntervalMs,
		);
		const effectiveBurstToleranceMs = Math.min(
			this.burstToleranceMs,
			maxIdleCreditMs,
		);
	...
	private additiveStepMs(): number {
		...
		const elapsedMs =
			this.lastSuccessAtMs === null
				? this.initialIntervalMs
				: Math.max(0, this.now() - this.lastSuccessAtMs);
		...
		const recoverWeight = Math.min(
			this.elapsedRecoveryCap,
			Math.max(1, elapsedMs / this.initialIntervalMs),
		);
		...
		return (
			baseStepMs + Math.floor(this.recoveryGain * overBackoffMs * recoverWeight)
		);
	}
	...
	recordThrottle(signal?: ThrottleSignal): void {
		...
		const reason: PacingBackoffReason = hasRetryAfter
			? "retry_after"
			: "throttle";
		if (hasRetryAfter) {
