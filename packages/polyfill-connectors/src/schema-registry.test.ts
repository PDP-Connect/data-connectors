...
test("an unmodeled enum value RETAINS the record instead of skipping it", () => {
	...
	assert.equal(
		result.ok,
		true,
		"a record whose only defect is vocabulary drift must not be skipped",
	);
});
...
test("the unrecognized enum value is preserved VERBATIM, never coerced or dropped", () => {
	...
	const attachments = result.data.attachments as Array<{
		type: string;
		url: string | null;
	}>;
	assert.equal(
		attachments[0]?.type,
		"linked_image",
		"the value the vendor sent must survive intact",
	);
...
test("the drift is reported as an anomaly so it is visible, not silently tolerated", () => {
	...
	assert.equal(
		anomaly?.value,
		"poll",
		"the diagnostic reports the actual value, for widening the schema later",
	);
...
test("every one of GroupMe's 10 observed attachment types is retained", () => {
	...
	for (const type of observed) {
		assert.equal(
			validate("messages", messageWith(type)).ok,
			true,
...
test("a missing required id is STILL skipped", () => {
	...
	assert.equal(
		result.ok,
		false,
		"a structurally malformed record must still be skipped",
	);
});
...
test("an enum drift COMBINED with a real fault is skipped on the fault", () => {
	...
	const result = validate("messages", {
		text: null,
		attachments: [{ type: "mentions", url: null }],
	});
	assert.equal(
		result.ok,
		false,
		"drift must not launder a genuinely malformed record through",
	);
	if (result.ok) {
		return;
	}
...
test("an unknown stream still passes through untouched", () => {
	const result = validate("streams_without_a_schema", {
		id: "x",
		anything: true,
	});
	assert.equal(result.ok, true);
