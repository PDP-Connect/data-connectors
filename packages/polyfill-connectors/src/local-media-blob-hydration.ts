...
export function resolveMaxMediaBytes(
	envVar: string,
...
export type MediaHydrationStatus =
	| "failed"
	| "hydrated"
	| "skipped_too_large"
	| "unavailable";
...
export async function hydrateMediaBytes(args: {
	...
	try {
		const { bytes, sizeBytes, tooLarge } = await readBoundedMediaBytes(
			args.filePath,
			args.maxBytes,
		);
		if (tooLarge || !bytes) {
		...
		try {
			...
			return {
				blobRef: null,
				contentSha256,
				hydrationError: sanitizeHydrationError(
