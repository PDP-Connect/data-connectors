...
export function isFullyBundledStaticSecretCredentialKind(
	kind: string,
): boolean {
...
function normalizeField(
	...
	const env = Array.isArray(raw.env)
		? raw.env.filter((value): value is string => cleanString(value) !== null)
		: [];
...
export function normalizeStaticSecretCredentialCapture(
	...
	const kind =
		cleanString(capture.credential_kind) ?? cleanString(capture.kind);
	if (!kind) {
		return null;
	}
