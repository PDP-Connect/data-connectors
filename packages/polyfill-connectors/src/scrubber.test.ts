...
test("structured redaction plans replace exact free-text targets", () => {
	...
	const scrubbed = applyStructuredRedactionPlan(
		"Alice Example wrote: Birthday gift for Mara",
		plan,
	);
...
test("structured redaction plans fail closed when a target is missing", () => {
	...
	assert.throws(
		() => applyStructuredRedactionPlan("No matching text here", plan),
		/redaction target not found/,
	);
});
