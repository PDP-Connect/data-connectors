...
test("millisecondsUntilNextNightlySummary targets the next local nightly slot", () => {
	assert.equal(
		millisecondsUntilNextNightlySummary(new Date("2026-04-29T06:30:00"), 7, 0),
		30 * 60 * 1000,
	);
	assert.equal(
		millisecondsUntilNextNightlySummary(new Date("2026-04-29T07:00:00"), 7, 0),
		24 * 60 * 60 * 1000,
	);
});

test("notifyOvernightSummarySafely keeps ntfy failures non-fatal", async () => {
	await notifyOvernightSummarySafely(summary, () =>
		Promise.reject(new Error("ntfy unavailable")),
	);
});
...
test("resolveSchedulerPersistenceStore loads the default store when omitted", async () => {
	...
	const store = await resolveSchedulerPersistenceStore(undefined, () =>
