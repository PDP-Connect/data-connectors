...
test("captureDom writes html, aria, page metadata, and screenshot in raw local capture mode", async () => {
	...
	try {
		const page: Pick<
			Page,
			"ariaSnapshot" | "content" | "locator" | "screenshot" | "title" | "url"
		> = {
			ariaSnapshot: () =>
				Promise.resolve('- document:\n  - button "Submit" [ref=e1]'),
			content: () =>
				Promise.resolve("<html><title>Fixture</title><body>ok</body></html>"),
		...
		const pageMeta = JSON.parse(
			readFileSync(`${capture.baseDir}/pages/${safe}.json`, "utf8"),
		);
		...
		assert.equal(
			readFileSync(`${capture.baseDir}/screenshots/${safe}.png`, "utf8"),
			"png",
		);
	} finally {
...
test("captureLocatorProbe calls getBy* helpers as methods, not detached (production run_1787537596833)", async () => {
	...
	try {
		...
		const report = JSON.parse(
			readFileSync(
				`${capture.baseDir}/locators/venmo-login-after-submit.json`,
				"utf8",
			),
		) as {
			probes: readonly {
		...
		for (const probe of report.probes) {
			assert.equal(
				probe.error,
				undefined,
				`probe ${probe.id} must not lose its receiver: ${probe.error ?? ""}`,
			);
			assert.equal(
				probe.count,
				1,
				`probe ${probe.id} must resolve a real count`,
			);
		}
	} finally {
		rmSync(capture.baseDir, { force: true, recursive: true });
...
test("captureLocatorProbe writes locator counts and first-match state", async () => {
	...
	try {
		...
		const report = JSON.parse(
			readFileSync(`${capture.baseDir}/locators/download_form.json`, "utf8"),
		);
...
test("createCaptureSession returns null when raw capture mode is disabled", () => {
	withEnv(
		{ PDPP_CAPTURE_FIXTURES: undefined, PDPP_CAPTURE_ON_FAILURE: undefined },
		() => {
			assert.equal(
				createCaptureSession(
					`fixture_capture_disabled_${process.pid}_${Date.now()}`,
				),
				null,
			);
		},
	...
	withEnv(
		{
			...
			PDPP_CAPTURE_ON_FAILURE: undefined,
			PDPP_CAPTURE_ROOT_DIR: root,
		},
		() => {
			...
			try {
				assert.equal(
					capture.baseDir.startsWith(join(root, connectorName, "raw")),
					true,
				);
...
test("createCaptureSession honors PDPP_CAPTURE_ON_FAILURE=1 with keepOnSuccess=false", () => {
	withEnv(
		{ PDPP_CAPTURE_FIXTURES: undefined, PDPP_CAPTURE_ON_FAILURE: "1" },
		() => {
			const capture = createCaptureSession(
				`fixture_capture_on_failure_${process.pid}_${Date.now()}`,
			);
			assert.ok(capture);
			try {
...
test("PDPP_CAPTURE_FIXTURES wins over PDPP_CAPTURE_ON_FAILURE (always retain)", () => {
	withEnv({ PDPP_CAPTURE_FIXTURES: "1", PDPP_CAPTURE_ON_FAILURE: "1" }, () => {
		const capture = createCaptureSession(
			`fixture_capture_both_${process.pid}_${Date.now()}`,
		);
		assert.ok(capture);
		try {
...
test("PDPP_CAPTURE_ON_FAILURE finalize() deletes raw dir on success", async () => {
	await withEnv(
		...
		async () => {
			const capture = createCaptureSession(
				`fixture_capture_on_failure_success_${process.pid}_${Date.now()}`,
			);
			assert.ok(capture);
			const page: Pick<
				Page,
			...
			assert.equal(existsSync(capture.baseDir), true);
			assert.equal(
				existsSync(`${capture.baseDir}/dom/before-success.html`),
				true,
			);
...
test("PDPP_CAPTURE_ON_FAILURE finalize() retains raw dir when markSucceeded was not called", async () => {
	await withEnv(
		...
		async () => {
			const capture = createCaptureSession(
				`fixture_capture_on_failure_fail_${process.pid}_${Date.now()}`,
			);
			assert.ok(capture);
			try {
				const page: Pick<
				...
				assert.equal(
					existsSync(`${capture.baseDir}/dom/before-fail.html`),
					true,
				);
...
test("PDPP_CAPTURE_FIXTURES finalize() retains raw dir on success (always-retain)", () => {
	withEnv(
		{ PDPP_CAPTURE_FIXTURES: "1", PDPP_CAPTURE_ON_FAILURE: undefined },
		() => {
			const capture = createCaptureSession(
				`fixture_capture_always_retain_${process.pid}_${Date.now()}`,
			);
			assert.ok(capture);
			try {
				capture.markSucceeded();
