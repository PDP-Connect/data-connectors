...
async function seedClaudeCode(): Promise<ConnectorFixture> {
	...
	await writeFile(
		...
		`${JSON.stringify({
			cwd: "/home/u/code/old",
			message: {
				content: [{ text: "should never be read", type: "text" }],
				role: "user",
			},
...
function scopeState(
	scope: ConnectorFixture["declaredScope"],
...
for (const connectorId of ["claude_code", "codex"] as const) {
	...
	test(`${connectorId}: resume after an interruption is idempotent and eventually commits coverage`, async () => {
		...
		const firstHarness = await startRunHarness(
...
test("claude_code: a declared `since` does not prune a file by its mtime — every in-root file is still opened", async () => {
	...
	await writeFile(
		...
		`${JSON.stringify({
			...
			message: {
				content: [{ text: "content behind a stale mtime", type: "text" }],
				role: "user",
			},
	...
	try {
		const args = [
			"--import",
			"tsx",
			join(CONNECTORS_DIR, "claude_code", "index.ts"),
		];
		...
		assert.equal(
			result.done?.status,
			"succeeded",
			"the run must complete, not skip the only file it has",
		);
		assert.equal(
