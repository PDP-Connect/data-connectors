...
function isDetailCoverageMessage(
	message: EmittedMessage,
): message is DetailCoverageMessage {
...
function latestRecordEmittedAt(
...
export function buildRunSummary(
	...
	for (const stream of Object.values(streams)) {
		if (stream.first_record_at && stream.last_record_at) {
			const span =
				Date.parse(stream.last_record_at) - Date.parse(stream.first_record_at);
