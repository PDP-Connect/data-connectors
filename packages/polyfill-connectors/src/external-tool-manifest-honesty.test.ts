...
test("connectors that reference known external tools declare them in manifests", () => {
	...
	for (const name of readdirSync(CONNECTORS_DIR).sort()) {
		...
		const referencedTools = KNOWN_EXTERNAL_TOOLS.filter((tool) =>
		...
		assert.equal(
			...
			true,
			`${name} references external tools but has no manifest`,
		);
	...
	assert.deepEqual(
		missing,
		[],
		"external subprocess tool references must be declared in manifest metadata",
	);
});
