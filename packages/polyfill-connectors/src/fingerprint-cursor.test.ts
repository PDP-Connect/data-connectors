...
test("recordFingerprint: excluded keys do not shift the hash", () => {
	...
	assert.notEqual(
		recordFingerprint(a),
		recordFingerprint(b),
		"without exclusion the hash moves",
	);
	assert.equal(
...
test("identical second run: cursor reports no changes for every record", () => {
	...
	for (const r of records) {
		assert.equal(
			second.shouldEmit(r),
			false,
			"identical second run does not re-emit",
		);
	}
...
test("run-clock exclusion: only `fetched_at` moved → no re-emit", () => {
	...
	const second = openFingerprintCursor(
		{ fingerprints: first.toState() },
		{ excludeFromFingerprint: ["fetched_at"] },
	);
	const advanced = { ...workspace, fetched_at: "2026-05-26T13:00:00Z" };
	assert.equal(
		second.shouldEmit(advanced),
		false,
		"fetched_at-only delta does not re-emit",
	);
...
test("source-field change: exactly the changed record re-emits", () => {
	...
	assert.equal(
		second.shouldEmit({ id: "U1", name: "alice", updated: 1000 }),
		false,
		"unchanged",
	);
	assert.equal(
		second.shouldEmit({ id: "U2", name: "bob", updated: 3000 }),
		true,
		"changed",
	);
});
...
test("pruneStale: id present in prior but absent from this run is dropped", () => {
	...
	assert.equal(
		second.toState().U2,
		first.toState().U2,
		"carry-forward keeps bob until prune",
	);
...
test("pruneStale: seen-but-unchanged ids survive", () => {
	...
	assert.equal(
		second.toState().U1,
		firstFp,
		"fingerprint carried through prune",
	);
});
...
test("anonymous records (no id) pass through and never touch cursor state", () => {
	...
	assert.equal(
		cursor.size(),
		1,
		"alice retained after prune despite anonymous emits",
	);
});
...
test("openCarryForwardCursor: prior() reports the prior value; note() does not change it", () => {
	...
	assert.deepEqual(
		cursor.prior("a"),
		{ updated_at: 1, count: 5 },
		"prior is the prior run's value",
	);
	assert.deepEqual(
		cursor.toState().a,
		{ updated_at: 2, count: 9 },
		"toState reflects this run's note",
	);
