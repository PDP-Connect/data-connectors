...
const trackedRegistryPath = join(
	packageDir,
...
test("static-secret-registry.generated.ts has not drifted from what regenerating from the manifests on disk would produce", () => {
	const scratchDir = mkdtempSync(
		join(tmpdir(), "static-secret-registry-drift-"),
	);
	try {
		const scratchPath = join(scratchDir, "static-secret-registry.generated.ts");
		execFileSync(
			"node",
			["--experimental-strip-types", generatorScript, scratchPath],
			{
				cwd: packageDir,
				stdio: "pipe",
			},
...
function writeScratchManifests(
	scratchDir: string,
...
function generateScratchRegistry(scratchManifestsDir: string): {
	GENERATED_STATIC_SECRET_REGISTRY: unknown;
} {
	...
	try {
		execFileSync(
			"node",
			["--experimental-strip-types", generatorScript, outPath],
			{
				cwd: packageDir,
				env: {
					...process.env,
					PDPP_POLYFILL_MANIFESTS_DIR: scratchManifestsDir,
				},
				stdio: "pipe",
			},
		);
		...
		const match = source.match(
			/GENERATED_STATIC_SECRET_REGISTRY[^{]*Object\.freeze\(([\s\S]*)\);\s*$/,
		);
		assert.ok(
			match,
			"could not locate GENERATED_STATIC_SECRET_REGISTRY literal in generator output",
		);
...
function probeManifest(): Record<string, unknown> {
	return {
		...
		display_name:
			"Static Secret Onboarding Probe (test fixture, not a real connector)",
...
test("counterweight: a brand-new static-secret manifest is picked up by the generated runtime registry with no hand-edit — reproduces the venmo onboarding gap as a failing case first", () => {
	const scratchDir = mkdtempSync(
		join(tmpdir(), "static-secret-registry-onboarding-probe-"),
	);
	try {
		writeScratchManifests(scratchDir, {
			[`${PROBE_CONNECTOR_KEY}.json`]: probeManifest(),
		});
		const { GENERATED_STATIC_SECRET_REGISTRY } = generateScratchRegistry(
			scratchDir,
		) as {
...
test("fail-closed: a manifest with credential_capture.fields but no secret:true field is not injectable (missing secret field)", () => {
	const scratchDir = mkdtempSync(
		join(tmpdir(), "static-secret-registry-missing-secret-"),
	);
	try {
		const manifest = probeManifest();
		(
			manifest.setup as {
				credential_capture: { fields: Record<string, unknown>[] };
			}
		...
		writeScratchManifests(scratchDir, {
			[`${PROBE_CONNECTOR_KEY}.json`]: manifest,
		});
		const { GENERATED_STATIC_SECRET_REGISTRY } = generateScratchRegistry(
			scratchDir,
		) as {
...
test("fail-closed: a manifest with an empty credential_capture (no kind) is not injectable (partial declaration)", () => {
	const scratchDir = mkdtempSync(
		join(tmpdir(), "static-secret-registry-partial-"),
	);
	try {
		const manifest = probeManifest();
		(manifest.setup as { credential_capture: unknown }).credential_capture = {
			fields: [],
		};
		writeScratchManifests(scratchDir, {
			[`${PROBE_CONNECTOR_KEY}.json`]: manifest,
...
test("fail-closed: buildConnectionScopedSecretEnv refuses a connector absent from the generated registry instead of inventing env vars", () => {
	...
	assert.throws(
		() =>
			buildConnectionScopedSecretEnv(PROBE_CONNECTOR_KEY, {
				credentialKind: "api_key",
				secret: "x",
			}),
		(err: unknown) =>
...
test("fail-closed: buildConnectionScopedSecretEnv refuses a recovered credential kind mismatch for every generated connector, not just gmail", () => {
	const mismatches: {
		...
		{
			connectorId: "github",
			recovered: { credentialKind: "app_password", secret: "x" },
		},
		{
			connectorId: "venmo",
			recovered: { credentialKind: "personal_access_token", secret: "x" },
		},
		{
			connectorId: "steam",
			recovered: { credentialKind: "username_password", secret: "x" },
		},
	];
