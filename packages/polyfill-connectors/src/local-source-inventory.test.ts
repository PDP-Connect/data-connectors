...
for (const [name, state] of [
	...
	[
		"missing top-level fetched_at",
		(({ fetched_at: _fetchedAt, ...rest }) => rest)(coverageState()),
	],
	[
		"missing top-level stores",
		(({ stores: _stores, ...rest }) => rest)(coverageState()),
	],
	...
	[
		...
		coverageState({
			stores: [
				{ ...coverageStores()[0], secret_path: "/private/home" },
				...coverageStores().slice(1),
			],
		}),
	],
	[
		...
		coverageState({
			stores: [
				{ ...coverageStores()[0], stream: "conflicting" },
				...coverageStores().slice(1),
			],
		}),
	],
...
test("size-only growth on a file inventory record is a no-op emit", () => {
	...
	const run2 = openInventoryFingerprintCursor(state);
	assert.equal(
		run2.shouldEmit(fileRecord(2048, 1_717_009_999)),
		false,
		"size growth + mtime tick must not re-emit",
	);
});
...
test("a classification change re-emits", () => {
	...
	assert.equal(
		run2.shouldEmit(
			inventoryRecord({ classification: "defer", mtime_epoch: 1_717_009_999 }),
		),
		true,
...
test("skipped record carries its fingerprint forward into the next STATE", () => {
	...
	assert.deepEqual(
		state2.fingerprints,
		state1.fingerprints,
		"skipped record's fingerprint is carried forward",
	);

	const run3 = openInventoryFingerprintCursor(state2);
	assert.equal(
		run3.shouldEmit(inventoryRecord({ mtime_epoch: 1_717_020_000 })),
		false,
		"still a no-op on run 3",
	);
});
...
test("a store that disappears is pruned and re-emits on re-appearance", () => {
	...
	const state1 = { fingerprints: run1.toState() };
	assert.ok(
		"backups:abc123" in state1.fingerprints,
		"present store stays in cursor",
	);
	...
	const state2 = { fingerprints: run2.toState() };
	assert.equal(
		Object.keys(state2.fingerprints).length,
		0,
		"absent store is pruned",
	);
	...
	const run3 = openInventoryFingerprintCursor(state2);
	assert.equal(
		run3.shouldEmit(inventoryRecord()),
		true,
		"re-appeared store re-emits",
	);
});
...
test("legacy cursor (no fingerprints field) re-emits everything once", () => {
	...
	const cursor = openInventoryFingerprintCursor({
		fetched_at: "2026-06-01T00:00:00Z",
	});
	assert.equal(
		cursor.shouldEmit(inventoryRecord()),
		true,
		"legacy cursor re-emits",
	);
});
...
test("buildDerivedCoverageRecord: a completed scan with zero examined is collected, not missing", () => {
	...
	assert.equal(
		record.status,
		"collected",
		"an empty-but-complete scan is collected, not missing/unaccounted",
	);
...
test("buildDerivedCoverageRecord: the emitted store id is exactly what the authority table declares for this (connector, stream) pair", () => {
	for (const [connectorId, stream, expectedStore] of [
		...
		assert.equal(
			record.id,
			`coverage:${expectedStore}`,
			"record id must be derived from the selected store",
		);
	}
});
...
test("buildDerivedCoverageRecord: carries an optional scopeFingerprint as collection_scope, omits it when absent", () => {
	...
	assert.equal(
		(withScope as { collection_scope?: string }).collection_scope,
		"fp-abc",
	);
...
function isLocalCoverageManifest(
	manifest: unknown,
): manifest is LocalCoverageManifest {
	if (!manifest || typeof manifest !== "object") {
		return false;
	}
	const candidate = manifest as { connector_key?: unknown; streams?: unknown };
	return (
		typeof candidate.connector_key === "string" &&
...
function shippedManifestsWithDescriptorAuthority(): readonly LocalCoverageManifest[] {
	return readPolyfillManifests()
		...
		.filter(
			(manifest) =>
				expectedLocalCoverageStoreDescriptors(manifest.connector_key) !== null,
		);
}
...
test("every shipped manifest with a descriptor-table entry has a descriptor for each required stream", () => {
	...
	for (const manifest of manifests) {
		const required = manifest.streams
			.filter(
				(stream) =>
					stream.required !== false && stream.name !== "coverage_diagnostics",
			)
			.map((stream) => stream.name);
		const missing = localCoverageStreamsMissingDescriptors(
...
test("descriptor authority carries no store whose stream is absent from its own connector's manifest", () => {
	...
	for (const manifest of manifests) {
		...
		const descriptors =
			LOCAL_COVERAGE_STORE_DESCRIPTORS_BY_CONNECTOR[
				manifest.connector_key.replace(
					/-/g,
					"_",
				) as keyof typeof LOCAL_COVERAGE_STORE_DESCRIPTORS_BY_CONNECTOR
			];
		const orphaned = descriptors
			.filter(
				(descriptor) =>
					descriptor.stream !== null && !manifestStreams.has(descriptor.stream),
			)
