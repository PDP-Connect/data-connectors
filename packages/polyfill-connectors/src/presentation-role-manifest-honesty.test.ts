	...
	for (const file of manifestFiles()) {
		...
		for (const stream of manifest.streams ?? []) {
			...
			if (roleEntries.length === 0) {
				violations.push(
					`${connectorKey}.${streamName}: no schema.properties[field].x_pdpp_role declarations`,
				);
				continue;
			}
			...
			for (const { field, role } of roleEntries) {
				if (!FIELD_ROLES.has(role)) {
					violations.push(
						`${connectorKey}.${streamName}.${field}: unknown x_pdpp_role="${role}"`,
					);
				}
				const types = schemaTypes(stream.schema?.properties?.[field] ?? {});
				const stringCompatible =
					types.includes("string") ||
					(role === "actor" && types.includes("array"));
				if (STRING_ROLES.has(role) && !stringCompatible) {
					violations.push(
						`${connectorKey}.${streamName}.${field}: x_pdpp_role="${role}" requires a string field`,
					);
				}
			}

			const primaryTitleCount = roleEntries.filter(
				({ role }) => role === "primary-title",
			).length;
			if (primaryTitleCount > 1) {
				violations.push(
					`${connectorKey}.${streamName}: declares ${primaryTitleCount} primary-title fields`,
				);
			}
		}
	}
