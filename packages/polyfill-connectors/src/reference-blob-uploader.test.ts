...
} from "./reference-blob-uploader.ts";
...
function requestBody(
	...
	assert.ok(
		init?.body,
		"test fixture fetch must receive a streaming RequestInit.body",
	);
...
function composedHydrator(
	...
	return makeAttachmentHydrator({
		connectorId: "https://registry.pdpp.dev/connectors/test",
		fetchAttachment: () =>
			Promise.resolve({ content, expectedSize: null, mimeType: "text/plain" }),
		maxBytes,
		uploadBlob,
	});
}
...
test("makeReferenceBlobUploader: extracts the real message from the RI host's object-shaped error envelope", async () => {
	...
	await assert.rejects(
		...
		(err) => {
			...
			assert.match(
				err.message,
				/blob storage backend is temporarily unavailable/,
			);
...
test("makeReferenceBlobUploader: classifies invalid successful responses and integrity mismatches", async () => {
	const invalidResponseUpload = makeReferenceBlobUploader({
		fetchFn: async () =>
			new Response(JSON.stringify({ object: "not-a-blob" }), { status: 200 }),
		ownerToken: "test-token",
		rsUrl: "https://pdpp.example.test",
	});
	await assert.rejects(
	...
	const upload = makeReferenceBlobUploader({
		fetchFn: async (input, init) => {
			...
			const received = Buffer.from(
				await new Response(requestBody(init)).arrayBuffer(),
			);
...
test("makeReferenceBlobUploader: classifies source stream failures separately from blob transport", async () => {
	...
	const brokenContent: AsyncIterable<Buffer> = {
		[Symbol.asyncIterator]: () => ({
			next: () => Promise.reject(sourceFailure),
		}),
	};
	const upload = makeReferenceBlobUploader({
		fetchFn: async (_input, init) => {
...
test("makeAttachmentHydrator + makeReferenceBlobUploader: preserves streamed size-policy, source, cancellation, and transport boundaries", async () => {
	...
	assert.match(
		sourceFailed.record.hydration_error ?? "",
		/IMAP source iterator failed/,
	);
	...
	const cancelled = await composedHydrator(
		Readable.from([Buffer.from("content")]),
		cancelThenReject,
	)({} as never, composedAttachment);
	assert.equal(cancelled.failure?.stage, "blob_upload_transport_failed");
	assert.match(
		cancelled.record.hydration_error ?? "",
		/socket failure after cancellation/,
	);
	...
	const rejected = await composedHydrator(
		Readable.from([Buffer.from("content")]),
		async () => Promise.reject(new Error("socket failure before consumption")),
	)({} as never, composedAttachment);
	assert.equal(rejected.failure?.stage, "blob_upload_transport_failed");
	assert.match(
		rejected.record.hydration_error ?? "",
		/socket failure before consumption/,
	);

	const transportCause = new Error("immediate socket rejection");
	const sourceFailureAfterTransport = new Error(
		"late independent source failure",
	);
	...
	const immediateTransport = await composedHydrator(
		racingSource,
		async (_input, init) => {
	...
	assert.equal(
		immediateTransport.failure?.stage,
		"blob_upload_transport_failed",
	);
	assert.match(
		immediateTransport.record.hydration_error ?? "",
		/immediate socket rejection/,
	);
...
test("makeReferenceBlobUploader: source proximity never replaces an independent transport cause", async () => {
	...
	const sourceAlreadyRecordedContent: AsyncIterable<Buffer> = {
		[Symbol.asyncIterator]: () => ({
			next: () => Promise.reject(sourceAlreadyRecorded),
		}),
	};
