...
export type BrowserSurfaceKind =
	| "chase_current_activity"
	| "usaa_transaction_export";
export type BrowserSurfacePosture =
	| "recognized"
	| "verified_empty"
	| "parser_zero"
	| "unexpected";
export type BrowserSurfaceManagedState =
	| "isolated"
	| "legacy_remote"
	| "managed"
	| "unknown";
export type BrowserSurfaceRoute = "expected" | "interstitial" | "unknown";
export type BrowserSurfaceWaitOutcome =
	| "not_needed"
	| "resolved"
	| "timed_out"
	| "unknown";
...
export function browserSurfaceManagedState(
...
function isKind(value: unknown): value is BrowserSurfaceKind {
	return (
		value === "chase_current_activity" || value === "usaa_transaction_export"
	);
}
...
function isRoute(value: unknown): value is BrowserSurfaceRoute {
	return (
		value === "expected" || value === "interstitial" || value === "unknown"
	);
}
 ...
 */
export function buildBrowserSurfaceDiagnostic(
	input: BrowserSurfaceDiagnosticInput,
): BrowserSurfaceDiagnostic | null {
	if (
		!(
			isKind(input.kind) &&
	...
	return {
		...
		phase:
			input.kind === "chase_current_activity"
				? "final_snapshot"
