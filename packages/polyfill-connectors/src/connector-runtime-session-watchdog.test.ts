...
test("watchdog does NOT trip while establishment keeps checkpointing past the deadline", async () => {
	...
	assert.equal(
		tripped,
		false,
		"steadily-checkpointing flow must not be killed",
	);
});
...
test("watchdog is paused while nonblocking running assistance is open", async () => {
	...
	const completeAssistance = async (
		_assistanceRequestId: string,
		_status: AssistanceCompletionStatus,
	): Promise<void> => undefined;
	const wrappedAssist = watchdog.wrapAssist(assist);
	const wrappedCompleteAssistance =
		watchdog.wrapCompleteAssistance(completeAssistance);
	...
	assert.equal(
		tripped,
		false,
		"open nonblocking assistance must pause the watchdog",
	);
});
...
test("watchdog is paused while a blocked no-response browser handoff watches for late readiness", async () => {
	...
	const wrappedAssist = watchdog.wrapAssist(
		async (): Promise<string> => "assist_browser_login",
	);
	const wrappedCompleteAssistance = watchdog.wrapCompleteAssistance(
	...
	assert.equal(
		tripped,
		false,
		"a blocked no-response handoff must outlive the normal watchdog deadline",
	);
});
...
test("watchdog re-arms after an interaction resolves and trips if progress then stalls", async () => {
	...
	const rejection = assert.rejects(
		watchdog.run(work),
		/amazon_session_establish_timeout/,
	);
...
function makeEstablishArgs(
	...
	return {
		assist: (() =>
			Promise.reject(new Error("assist must not be called"))) as never,
		capture: null,
		...
		sendInteraction: (() =>
			Promise.reject(new Error("sendInteraction must not be called"))) as never,
	};
}
...
test("HEB regression: a code-shaped ensureSession throw (heb_verification_code_not_provided) survives as TerminalError.code, not just the free-form message", async () => {
	await assert.rejects(
		...
		(err: unknown) => {
			...
			assert.match(
				err.message,
				/heb_session_failed: heb_verification_code_not_provided/,
			);
		...
		establishSession(
			{
				ensureSession: () => {
					...
					throw new Error(
						"source_unavailable: USAA reported its login system is currently unavailable",
					);
				},
				probeSession: undefined,
			},
...
test("the recovered code round-trips through the actual redaction the connector_error_message column applies (proves the fix, not just the plumbing)", async () => {
	...
	await assert.rejects(
		...
		(err: unknown) => {
			...
			// useless.
			assert.equal(
				boundConnectorErrorMessage(typedErr.message),
			...
			assert.equal(
				boundConnectorErrorCode(typedErr.code),
				"heb_verification_code_not_provided",
			);
			return true;
		},
...
test("captureBrowserPage returns within its deadline when captureDom hangs (wedged renderer)", async () => {
	...
	assert.ok(
		elapsed < 1000,
		`captureBrowserPage should return promptly on a hang (took ${String(elapsed)}ms)`,
	);
});
...
test("captureBrowserPage skips a closed page without invoking captureDom", async () => {
	...
	await captureBrowserPage(
		fakeCapture as CaptureSession,
		fakeClosedPage as Page,
		"runtime-error",
		20,
	);
...
test("resolveSessionEstablishWatchdogMs falls back to default on missing/invalid", () => {
	...
	assert.equal(
		resolveSessionEstablishWatchdogMs({
			PDPP_SESSION_ESTABLISH_WATCHDOG_MS: "0",
		}),
		120_000,
	);
	assert.equal(
		resolveSessionEstablishWatchdogMs({
			PDPP_SESSION_ESTABLISH_WATCHDOG_MS: "nope",
		}),
		120_000,
	);
});
