...
test("aria snapshot never writes a filled password value to disk", async () => {
	await withCaptureEnv(async () => {
		const capture = createCaptureSession(
			`redact_aria_${process.pid}_${Date.now()}`,
		);
		assert.ok(capture);
		...
		const written = readFileSync(
			`${capture.baseDir}/aria/login.aria.yml`,
			"utf8",
		);
...
test("redaction preserves that a field existed and whether it was filled", () => {
	...
	assert.ok(
		!out.includes("[ref=e9]: [REDACTED]"),
		"an empty field must not be reported as filled",
	);
...
test("an otp field value is redacted and keeps its quoting", () => {
	...
	assert.match(
		out,
		/textbox "One-time code" \[active\] \[ref=e5\]: "\[REDACTED\]"/,
	);
});
...
test("a known credential is redacted even in a field nobody labelled secret", () => {
	...
	assert.equal(
		redactAriaSnapshot(snapshot, [SECRET]),
		'  - textbox "Nickname" [ref=e8]: [REDACTED]',
	);
...
test("dom capture never writes a password value attribute to disk", async () => {
	await withCaptureEnv(async () => {
		const capture = createCaptureSession(
			`redact_dom_${process.pid}_${Date.now()}`,
		);
		assert.ok(capture);
		...
		assert.ok(
			!written.includes(SECRET),
			`password value leaked into dom capture:\n${written}`,
		);
		assert.match(
			written,
...
test("dom redaction keeps the field present and leaves non-sensitive values alone", () => {
	...
	assert.match(
		out,
		/<input name="password" type="password" value="\[REDACTED\]">/,
	);
	assert.match(
		out,
		/<input name="username" type="text" value="tim@example\.com">/,
	);
});
...
test("an unlabelled password input is redacted by its type alone", () => {
	...
	const out = redactDomHtml(
		`<input name="field1" id="f1" type="password" value="${SECRET}">`,
	);

	assert.equal(
		out,
		'<input name="field1" id="f1" type="password" value="[REDACTED]">',
	);
});
...
test("every separator spelling of a one-time-code field name reads as secret", () => {
	...
	for (const name of [
		...
		assert.equal(
			...
			true,
			`${name} SHALL be treated as a secret field name`,
		);
	}
	...
	for (const name of ["username", "email", "account name", "display_name"]) {
		assert.equal(
			isSecretFieldName(name),
			false,
			`${name} SHALL NOT be redacted as a secret`,
		);
	}
});
