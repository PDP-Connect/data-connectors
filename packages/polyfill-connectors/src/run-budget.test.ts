...
test("RunBudget: no caps — never stops, tripReason always null", () => {
	...
	assert.equal(
		budget.tripReason(),
		null,
		"uncapped budget trip reason is null",
	);
});
...
test("RunBudget: maxWallClockMs trips when elapsed >= cap", () => {
	...
	assert.equal(
		budget.tripReason(),
		null,
		"elapsed 0 < 500: open and clock anchored",
	);
	nowMs = 1400;
...
test("RunBudget: request cap takes priority when both caps trip simultaneously", () => {
	...
	assert.equal(
		budget.tripReason(),
		"max_requests",
		"request cap takes priority over wall-clock",
	);
});
...
test("RunBudget: remainingWallClockMs is Infinity when no wall-clock cap is set", () => {
	...
	assert.equal(
		...
		"uncapped time = unbounded wait budget",
	);
});
...
test("RunBudget: remainingWallClockMs returns the full budget before any request, then shrinks with elapsed", () => {
	...
	assert.equal(
		budget.remainingWallClockMs(),
		500,
		"full budget remains at anchor",
	);
	nowMs = 1300;
	assert.equal(
		budget.remainingWallClockMs(),
		200,
		"elapsed 300 → 200 remaining",
	);
	nowMs = 1500;
	assert.equal(
		budget.remainingWallClockMs(),
		0,
		"elapsed 500 → 0 remaining (cap reached)",
	);
	nowMs = 1700;
	assert.equal(
		budget.remainingWallClockMs(),
		0,
		"never negative once the cap is passed",
	);
});
