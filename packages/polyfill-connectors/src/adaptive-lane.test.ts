...
test("adaptive lane never exceeds effective max concurrency", async () => {
	...
	assert.equal(
		active,
		2,
		"third task may start only after one active task completes",
	);
	gates[1]?.resolve("b");
	gates[2]?.resolve("c");
...
test("adaptive lane respects bounded Retry-After cooldown with fake sleep", async () => {
	...
	const lane = createAdaptiveLane<{ status: number }>({
		...
		classifyOutcome: ({ result: outcomeResult }) =>
			outcomeResult?.status === 429
				? { kind: "rate_limited", retryAfterMs: 20_000 }
				: { kind: "ok" },
		emitTelemetry: (event) => {
			events.push(event);
		},
	...
	assert.equal(
		events.some(
			(event) => event.type === "cooldown" && event.retryAfterMs === 20_000,
		),
		true,
	);
});
...
test("adaptive lane reportPressure delays next queued task even when current task succeeds", async () => {
	...
	assert.deepEqual(
		sleeps,
		[5000],
		"reported pressure sets a capped cooldown before the next launch",
	);
	assert.equal(
		lane.snapshot().concurrency,
		1,
		"reported pressure must not raise max-limited concurrency",
	);
	assert.equal(
		events.some(
			(event) =>
...
test("adaptive lane absorbed pressure reduces concurrency but does not mirror a launch cooldown", async () => {
	...
	assert.equal(
		lane.snapshot().concurrency,
		1,
		"absorbed rate_limited pressure still clamps concurrency to min",
	);
...
test("adaptive lane repeated rate limits keep backing off across attempts", async () => {
	...
	assert.equal(
		calls,
		4,
		"each 429 is retried until success within the attempt budget",
	);
	assert.deepEqual(
		sleeps,
		[5000, 5000, 5000],
		"every repeated 429 still backs off by the bounded Retry-After",
	);
	assert.equal(
		events.filter(
			(event) => event.type === "cooldown" && event.outcome === "rate_limited",
		).length,
		3,
...
test("adaptive lane reportPressure uses pressure bounds without changing ordinary pacing", async () => {
	...
	assert.deepEqual(
		sleeps,
		[1500, 1500, 45_000],
		"only reported pressure uses the wider cooldown bounds",
	);
});
...
test("adaptive lane rate limits reduce cap and clean successes increase only within max", async () => {
	...
	const lane = createAdaptiveLane<{ status: number }>({
		...
		classifyOutcome: ({ result }) =>
			result?.status === 429 ? { kind: "rate_limited" } : { kind: "ok" },
		emitTelemetry: (event) => {
			events.push(event);
		},
		random: () => 0,
	...
	assert.equal(
		lane.snapshot().concurrency,
		1,
		"one clean success is not enough to increase",
	);
	await lane.run(() => ({ status: 200 }));
	...
	assert.equal(
		events.filter((event) => event.type === "concurrency_increased").length,
		2,
	);
});
...
test("adaptive lane simulator handles hidden and changing quota pressure deterministically", async () => {
	...
	assert.ok(
		maxActive <= 3,
		"simulated hidden quota never exceeds lane max concurrency",
	);
	assert.equal(
		attemptsByItem.get(3),
		2,
		"pathological first attempt is retried once",
	);
	assert.ok(
		sleeps.includes(1000),
		"Retry-After controls a deterministic cooldown",
	);
	assert.equal(
		events.some(
			(event) =>
				event.type === "concurrency_decreased" &&
...
test("adaptive lane cancellation rejects queued and sleeping retry work", async () => {
	...
	const lane = createAdaptiveLane<{ status: number }>({
		...
		minDelayMs: 1000,
		classifyOutcome: ({ result }) =>
			result?.status === 503 ? { kind: "retryable" } : { kind: "ok" },
		random: () => 0,
	...
	assert.equal(
		attempts,
		1,
		"scheduled retry does not launch after cancellation",
	);
});
...
test("adaptive lane runAll cancels queued work after first rejection", async () => {
	...
	assert.equal(
		attempts,
		1,
		"queued work must not start after required work fails",
	);
});
...
test("adaptive lane telemetry summarizes errors without carrying raw error objects", async () => {
	...
	await assert.rejects(
		lane.run(() =>
			Promise.reject(new Error("secret-url=https://example.test/token")),
		),
	);

	const terminalEvent = events.find(
		(event) => event.type === "completed" && event.errorName === "Error",
	);
	assert.ok(terminalEvent, "expected a safe error-name-only terminal event");
	assert.equal(
		Object.hasOwn(terminalEvent, "error"),
		false,
		"raw errors must not be exposed in lane telemetry",
	);
