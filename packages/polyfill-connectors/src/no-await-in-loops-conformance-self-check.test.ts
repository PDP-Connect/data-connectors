...
import {
	countByLocationKey,
	locationKey,
} from "../scripts/check-no-await-in-loops-conformance.ts";
...
test("countByLocationKey: unique locations each count 1, no key is dropped", () => {
	...
	assert.equal(
		counts.size,
		3,
		"three distinct locations must produce three keys",
	);
...
test("countByLocationKey: a duplicated (path, line, column) reports count 2, not silently collapsed to 1", () => {
	...
	assert.equal(
		counts.size,
		2,
		"two distinct location keys, even though three entries were provided",
	);
	assert.equal(
...
test("mutation: the real checked-in allowlist has zero duplicate location keys (regression guard)", async () => {
	const { NO_AWAIT_IN_LOOPS_ALLOWLIST } = await import(
