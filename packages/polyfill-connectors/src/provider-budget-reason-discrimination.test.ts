 ...
 */
const SOURCE_PRESSURE_GAP_REASONS: ReadonlySet<string> = new Set([
	"rate_limited",
	"upstream_pressure",
]);
...
test("DISCRIMINATION: the two reason taxonomies are fully disjoint", () => {
	...
	const intersection = [...SOURCE_PRESSURE_GAP_REASONS].filter((r) =>
		budget.has(r),
	);
	assert.deepEqual(
		intersection,
		[],
		"source-pressure and budget-exhaustion reason sets share no member",
	);
});

test("DISCRIMINATION: every controller stop carries a budget-exhaustion (non-pressure) reason", () => {
	// Run cap trip → max_requests.
	const capController = new ProviderBudgetController({
	...
	const retryController = new ProviderBudgetController({
		retryBudget: {
			capacity: retryBudgetCapacityFromRequestCap({ maxRequests: 5 }),
			refillPerSuccess: 0,
		},
	});
	// Drain the retry budget, then the next consume returns a `retry_budget` stop.
	let stop: { ok: true } | { ok: false; reason: ProviderBudgetDeferReason } = {
		ok: true,
	};
	...
	if (!stop.ok) {
		...
		assert.equal(
			...
			false,
			"retry-budget stop is not source pressure",
		);
	}
});
