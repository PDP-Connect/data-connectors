...
export const QUERY_AFFORDANCE_ALLOWLIST: Readonly<
	Record<string, AffordanceJustification>
> = Object.freeze({
	...
	// privacy control.
	"uber.trips.pickup_address.lexical":
		"addresses intentionally non-searchable for privacy",
	"uber.trips.dropoff_address.lexical":
		"addresses intentionally non-searchable for privacy",
	...
	"claude-code.skills.mtime_epoch.group_by_time":
		"integer epoch, not a server-supported time-bucket schema",
	"claude-code.memory_notes.mtime_epoch.group_by_time":
		"integer epoch, not a server-supported time-bucket schema",
	"claude-code.slash_commands.mtime_epoch.group_by_time":
		"integer epoch, not a server-supported time-bucket schema",
	"codex.prompts.mtime_epoch.group_by_time":
		"integer epoch, not a server-supported time-bucket schema",
	"codex.skills.mtime_epoch.group_by_time":
		"integer epoch, not a server-supported time-bucket schema",
	...
	"ynab.categories.goal_snoozed_at.range":
		"category goal-snooze state timestamp; not a useful owner filter axis",
