 ...
 */
function isCredentialsMissingError(
	...
	return {
		assist: async (request) => {
			const assistanceRequestId =
				request.assistance_request_id ?? nextAssistanceRequestId();
			if (
		...
		close: async () => {
			await Promise.all(
				[...openAssistanceIds].map((interactionId) =>
...
const STORAGE_STATE_RECORD_PATH_ENV_MIRROR =
 ...
 */
function browserRecordingRequested(
	env: NodeJS.ProcessEnv = process.env,
): boolean {
	return Boolean(
		env[HAR_RECORD_PATH_ENV_MIRROR]?.trim() ||
...
export function makeTracer(
	 ...
	 */
	const credentialsInPlay = (): boolean =>
		capture?.hasRegisteredSecrets?.() === true;
