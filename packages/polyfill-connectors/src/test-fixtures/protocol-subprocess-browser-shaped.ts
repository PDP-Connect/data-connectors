...
const validateRecord: ValidateRecord = (stream: string, data: RecordData) => {
	if (
		...
		data.source === "browser_fixture"
	) {
		return { ok: true, data };
	}
	return {
		ok: false,
		issues: [{ path: "id", message: "expected a browser fixture order id" }],
	};
};
...
runConnector({
	...
	async collect({ emit, emitRecord }) {
		await emit({
			type: "PROGRESS",
			stream: "orders",
			message: "replaying browser-shaped fixture data",
		});
		...
		await emit({
			...
			message:
				"detail-pane collection intentionally skipped in no-browser fixture",
		});
		await emit({
			type: "STATE",
			stream: "orders",
			cursor: { last_order_id: "order-1" },
		});
	},
});
