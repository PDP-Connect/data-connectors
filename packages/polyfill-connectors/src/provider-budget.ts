...
export type ProviderBudgetDeferReason =
	| RunBudgetTrip
	| "circuit_open"
	| "retry_budget";
...
export class RetryBudget {
	...
	constructor(options: RetryBudgetOptions = {}) {
		this.capacity = sanitizeNonNegative(
			options.capacity ?? Number.POSITIVE_INFINITY,
		);
		this.refillPerSuccess = sanitizeNonNegative(options.refillPerSuccess ?? 1);
		this.tokens = Math.min(
			sanitizeNonNegative(options.initialTokens ?? this.capacity),
			this.capacity,
		);
	}
...
export class CircuitBreaker {
	...
	constructor(options: CircuitBreakerOptions = {}) {
		...
		this.minimumThroughput = Math.max(
			1,
			Math.floor(options.minimumThroughput ?? 5),
		);
		this.now = options.now ?? Date.now;
		this.resetTimeoutMs = Math.max(
			0,
			Math.floor(options.resetTimeoutMs ?? 60_000),
		);
		this.windowSize = Math.max(
			this.minimumThroughput,
			Math.floor(options.windowSize ?? 10),
		);
	}
	...
	beforeRequest(): ProviderBudgetGate {
		...
		const elapsedOpenMs =
			this.openedAt === null ? 0 : Math.max(0, this.now() - this.openedAt);
...
export class ProviderBudgetController implements SendDelayHint {
	...
	recordThrottle(
		signal: ThrottleSignal & { retryAfterAlreadySlept?: boolean } = {},
	): void {
	...
	currentStop(reason: ProviderBudgetDeferReason): ProviderBudgetStop {
		return {
			...
			retryTokensRemaining:
				this.retryBudget?.remaining ?? Number.POSITIVE_INFINITY,
		};
	}
	...
	private recordCircuitTransition(
		...
		this.circuitTransitions.push({
			elapsedMs: this.runBudget?.elapsedMs() ?? 0,
			previousState,
			reason: providerBudgetTransitionReason(
				trigger,
				previousState,
...
export function retryBudgetCapacityFromRequestCap({
	...
	return Math.max(
		...
		Math.ceil(Math.max(0, maxRequests) * clampRatio(ratio)),
	);
}
...
function resolveCircuitBreaker(
	value: ProviderBudgetOptions["circuitBreaker"],
): CircuitBreaker | null {
...
function resolveProviderPacing(
	value: ProviderBudgetOptions["pacing"],
): ProviderPacing | null {
	if (value === false || value === undefined) {
		return null;
	}
...
function resolveRetryBudget(
	value: ProviderBudgetOptions["retryBudget"],
): RetryBudget | null {
	if (value === false || value === undefined) {
		return null;
	}
...
function resolveRunBudget(
	value: ProviderBudgetOptions["runBudget"],
): RunBudget | null {
	if (value === false || value === undefined) {
		return null;
	}
