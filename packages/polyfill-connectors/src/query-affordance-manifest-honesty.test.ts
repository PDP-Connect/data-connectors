 ...
 */
const INGEST_TIME_FIELD =
	/^(fetched_at|ingested_at|collected_at|synced_at|observed_at|retrieved_at)$/;
...
function isIntegerOrNumberSchema(schema: JsonSchema | undefined): boolean {
	const types = nonNullTypes(schema);
	return (
		types.length === 1 && (types[0] === "integer" || types[0] === "number")
	);
}
...
function isScalarSchema(schema: JsonSchema | undefined): boolean {
	...
	return (
		only !== undefined &&
		["boolean", "integer", "number", "string"].includes(only)
	);
}
...
function isAllowlisted(
	...
	return Object.hasOwn(
		QUERY_AFFORDANCE_ALLOWLIST,
		allowKey(connector, stream, field, affordance),
	);
}
 ...
 */
function isRangeRequiredTimeField(
	field: string,
	schema: JsonSchema,
 ...
 */
function isGroupByTimeRequiredField(
	field: string,
	schema: JsonSchema,
	stream: ManifestStream,
): boolean {
	...
	for (const file of manifestFiles()) {
		...
		for (const stream of manifest.streams ?? []) {
			...
			const rangeFields = new Set(
				Object.keys(stream.query?.range_filters ?? {}),
			);
...
test("connector manifests declare group_by_time for useful owner-facing time fields", () => {
	...
	for (const file of manifestFiles()) {
		...
		for (const stream of manifest.streams ?? []) {
			const streamName = stream.name ?? "<unnamed>";
			const properties = stream.schema?.properties ?? {};
			const groupByTime = new Set(
				stream.query?.aggregations?.group_by_time ?? [],
			);
...
test("group_by_time declarations target only server-supported date/date-time string fields", () => {
	...
	for (const file of manifestFiles()) {
		...
		for (const stream of manifest.streams ?? []) {
			...
			if (
				(groupByTime.length > 0 || (aggregations?.group_by ?? []).length > 0) &&
				aggregations?.count !== true
			) {
				violations.push(
					`${connectorKey}.${streamName}: declares grouping without aggregations.count: true`,
				);
			}
	...
	for (const file of manifestFiles()) {
		...
		for (const stream of manifest.streams ?? []) {
			...
			for (const field of Object.keys(stream.query?.range_filters ?? {})) {
				...
				if (!schema) {
					violations.push(
						`${connectorKey}.${streamName}.${field}: range_filters targets an absent field`,
					);
					continue;
				}
			...
			for (const field of stream.query?.aggregations?.group_by ?? []) {
				...
				if (!schema) {
					violations.push(
						`${connectorKey}.${streamName}.${field}: group_by targets an absent field`,
					);
					continue;
				}
				if (!isScalarSchema(schema)) {
					violations.push(
						`${connectorKey}.${streamName}.${field}: group_by requires a scalar field`,
					);
				}
			}
		}
...
test("query affordance allowlist entries are non-stale and not contradicted by manifests", () => {
	...
	for (const [entryKey, reason] of Object.entries(QUERY_AFFORDANCE_ALLOWLIST)) {
		assert.ok(
			reason.length > 0,
			`${entryKey}: allowlist entries require a justification reason`,
		);
		...
		if (!(affordance && field)) {
			stale.push(
				`${entryKey}: malformed allowlist key (expected connector.stream.field.affordance)`,
			);
			continue;
		}
		...
		switch (affordance) {
			case "range":
				...
				break;
			case "group_by_time":
				declared = (stream.query?.aggregations?.group_by_time ?? []).includes(
					field,
				);
				break;
			case "group_by":
			...
			case "lexical":
				...
				break;
			case "semantic":
				declared = (stream.query?.search?.semantic_fields ?? []).includes(
					field,
				);
				break;
			default:
		...
		if (declared) {
			stale.push(
				`${entryKey}: affordance is declared in the manifest; remove the allowlist entry`,
			);
		}
	}
