...
test("every publicly-listed connector is in the production-ready roster, and vice versa", () => {
	...
	const listedButNotRostered = listedKeys.filter(
		(k) => !rosterKeys.includes(k),
	);
	const rosteredButNotListed = rosterKeys.filter(
		(k) => !listedKeys.includes(k),
	);
...
test("every production-ready roster entry names a test file that exists", () => {
	...
	assert.deepEqual(
		missing,
		[],
		`roster entries name a test file that does not exist: ${missing.join("; ")}`,
	);
});

test("known scaffold connectors are not in the production-ready roster", () => {
	const overlap = KNOWN_SCAFFOLD_CONNECTORS.filter(
		(key) => key in PRODUCTION_READY_CONNECTORS,
	);
	assert.deepEqual(
		overlap,
		[],
...
test("known scaffold connectors do not appear in REAL_UNLISTED_CONNECTORS", () => {
	const overlap = KNOWN_SCAFFOLD_CONNECTORS.filter(
		(key) => key in REAL_UNLISTED_CONNECTORS,
	);
	assert.deepEqual(
		overlap,
		[],
