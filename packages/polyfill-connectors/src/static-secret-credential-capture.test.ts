...
test("password-without-secret: type:'password' with no explicit secret:true is still classified secret (closes F1)", () => {
	...
	assert.ok(
		normalized,
		'a type:"password" field with no secret:true must still make the connector static-secret',
	);
...
test("missing-label: a secret field with no label throws instead of silently vanishing or silently shipping (closes F2)", () => {
	...
	assert.throws(
		...
		(err: unknown) =>
			err instanceof StaticSecretCredentialCaptureError &&
			err.code === "static_secret_field_label_required",
	);
});
...
test("empty-env: a secret field with env declared as an empty array throws (closes F4)", () => {
	...
	assert.throws(
		...
		(err: unknown) =>
			err instanceof StaticSecretCredentialCaptureError &&
			err.code === "static_secret_field_env_required",
	);
});
...
test("empty-env: a secret field with no env key at all (undefined) throws the same as an empty array", () => {
	const capture: StaticSecretCredentialCaptureLike = {
		fields: [
			{ label: "Probe token", name: "token", required: true, secret: true },
		],
		kind: "api_key",
		label: "Probe",
	};
...
test("required/optional: required defaults true, and required:false is preserved for a multi-secret-field bundle", () => {
	...
	const normalized = normalizeStaticSecretCredentialCapture(
		"probe_required_optional",
		capture,
	);
	assert.ok(normalized);
	const byName = Object.fromEntries(
		normalized.fields.map((field) => [field.name, field]),
	);
...
test("required defaults to true when omitted entirely (not false, not undefined)", () => {
	const capture: StaticSecretCredentialCaptureLike = {
		fields: [
			{ env: ["PROBE_TOKEN"], label: "Token", name: "token", secret: true },
		],
		kind: "api_key",
		label: "Probe",
	};
	const normalized = normalizeStaticSecretCredentialCapture(
		"probe_required_default",
		capture,
	);
...
test("block-level required defaults to true when the manifest omits it entirely", () => {
	...
	const normalized = normalizeStaticSecretCredentialCapture(
		"probe_block_required_default",
		capture,
	);
	assert.equal(
		normalized?.required,
		true,
		"omitting the block-level fact must default to required, not optional",
	);
});
...
test("block-level required: false is preserved (Venmo's shape) — independent of each field's own required flag", () => {
	...
	const normalized = normalizeStaticSecretCredentialCapture(
		"probe_block_optional",
		capture,
	);
	assert.equal(
		normalized?.required,
		false,
		"an explicit block-level false must be preserved",
	);
...
test("no credential_capture at all returns null, not an error", () => {
	assert.equal(
		normalizeStaticSecretCredentialCapture("probe_none", null),
		null,
	);
	assert.equal(
		normalizeStaticSecretCredentialCapture("probe_none", undefined),
		null,
	);
});
...
test("credential_capture with a kind but no secret field returns null, not an error", () => {
	...
	assert.equal(
		normalizeStaticSecretCredentialCapture("probe_no_secret_field", capture),
		null,
	);
});
...
test("credential_capture with fields but no kind returns null, not an error", () => {
	...
	assert.equal(
		normalizeStaticSecretCredentialCapture("probe_no_kind", capture),
		null,
	);
});
