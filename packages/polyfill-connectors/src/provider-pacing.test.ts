...
test("ProviderPacing: inverted initial/min configuration cold-starts at the declared floor", async () => {
	...
	assert.equal(
		...
		10_000,
		"cold-start state cannot be faster than minIntervalMs",
	);
	assert.equal(
		pacing.snapshot().initialIntervalMs,
		10_000,
		"snapshot reports the effective cold-start interval",
	);
	...
	await Promise.all(
		cases.map(async ({ burstToleranceMs, expected }) => {
			...
			assert.deepEqual(
				admittedAtMs,
				expected,
				`long-idle sequence for burstToleranceMs=${burstToleranceMs}`,
			);
			for (let i = 2; i < admittedAtMs.length; i += 1) {
				assert.ok(
					(admittedAtMs[i] as number) - (admittedAtMs[i - 1] as number) >=
						minIntervalMs,
...
test("ProviderPacing: one-way error ratchet — throttle after success raises interval; success after throttle only raises fill rate", () => {
	...
	pacing.recordThrottle();
	assert.ok(
		pacing.currentIntervalMs > beforeThrottle,
		"throttle raises interval",
	);
	...
	pacing.recordSuccess();
	assert.ok(
		pacing.currentIntervalMs < afterThrottle,
		"success reduces interval after throttle",
	);
...
test("ProviderPacing Part B: a Retry-After is a one-shot wait, NOT a steady-state interval", async () => {
	...
	assert.equal(
		learnedInterval,
		818,
		"warmed to a faster learned interval (rate-space second step)",
	);
	...
	assert.equal(
		pacing.currentIntervalMs,
		learnedInterval,
		"Retry-After does NOT inflate the steady-state interval",
	);
	...
	assert.equal(
		spy.calls.at(-1),
		100_000,
		"one-shot wait honored exactly on next admit",
	);
	...
	assert.equal(
		pacing.snapshot().lastBackoff?.reason,
		"retry_after",
		"retry-after back-off reason recorded",
	);
});
...
test("ProviderPacing Part B: a plain throttle (no Retry-After) still increases the interval (soft 1.5× step)", () => {
	...
	assert.equal(
		pacing.currentIntervalMs,
		1500,
		"plain throttle multiplies the interval by 1.5 (soft step)",
	);
	assert.equal(
		pacing.snapshot().lastBackoff?.reason,
		"throttle",
		"plain throttle reason recorded",
	);
});
...
test("ProviderPacing: provider isolation — two instances throttled independently", () => {
	...
	assert.equal(
		pacingA.currentIntervalMs,
		1500,
		"A throttled (1000 × 1.5 = 1500)",
	);
...
test("ProviderPacing: sustained success monotonically DECREASES the interval toward the ceiling (throughput rises)", () => {
	...
	for (let i = 1; i < trajectory.length; i += 1) {
		...
		assert.ok(
			curr <= prev,
			`interval must never rise under sustained success: ${prev} -> ${curr}`,
		);
	}
	...
	assert.ok(
		sampled < first,
		"interval decreased overall over the sampled window (throughput rose)",
	);
	assert.equal(
		sampled,
		429,
	...
	assert.equal(
		pacing.currentIntervalMs,
		ceilingMs,
		"interval converged to the ceiling under sustained success",
	);
});
...
test("ProviderPacing: injected throttle multiplicatively INCREASES the interval, then success recovers it", () => {
	...
	assert.ok(
		beforeThrottle < 1000,
		"controller discovered a faster interval before pressure",
	);
	// Inject 429 pressure.
	pacing.recordThrottle();
	assert.ok(
		pacing.currentIntervalMs > beforeThrottle,
		"throttle multiplicatively slowed the rate",
	);
	const afterThrottle = pacing.currentIntervalMs;
	...
	assert.ok(
		pacing.currentIntervalMs < afterThrottle,
		"success recovers the interval after a back-off",
	);
	assert.equal(
...
test("ProviderPacing: the interval NEVER crosses the ceiling under any success volume", () => {
	...
	for (let i = 0; i < 1000; i += 1) {
		...
		assert.ok(
			...
			"interval never crosses the ceiling (faster than allowed)",
		);
	}
	assert.equal(
		pacing.currentIntervalMs,
		ceilingMs,
		"rests exactly at the ceiling",
	);
});
...
test("ProviderPacing: warm-start restores the prior run's learned interval, clamped to the ceiling", () => {
	...
	assert.equal(
		warm.currentIntervalMs,
		320,
		"warm-start resumes at the restored learned interval, not the cold 1000ms",
	);
	...
	assert.equal(
		clamped.currentIntervalMs,
		250,
		"warm-start never restores faster than the ceiling",
	);
	...
	assert.equal(
		cold.currentIntervalMs,
		1000,
		"no warm-start → cold discovery seed",
	);
});
...
test("ProviderPacing: snapshot() exposes interval, ceiling, and last back-off for legibility", () => {
	...
	assert.equal(
		snap.intervalMs,
		snap.lastBackoff?.atIntervalMs,
		"snapshot interval matches the back-off it just took",
	);
	assert.equal(
		snap.lastBackoff?.reason,
		"throttle",
		"plain throttle reason recorded",
	);
...
test("ProviderPacing: deep transient back-off recovers to the ceiling fast (rate-space subsumes the old flat tail; recoveryGain still helps)", () => {
	...
	assert.ok(
		withBoost <= 40,
		`deep recovery reaches the ceiling in ≤40 successes, got ${withBoost}`,
	);
});
...
test("ProviderPacing §9-C2 IDEAL: near the ceiling the per-success RATE gain is ~CONSTANT (rate-space AIMD), NOT super-accelerating", () => {
	...
	const calibratedRateStep =
		ratePerMin(initialIntervalMs - baseStep) - ratePerMin(initialIntervalMs);
	...
	while (pacing.currentIntervalMs > ceilingMs) {
		pacing.recordSuccess();
		const dRate: number =
			ratePerMin(pacing.currentIntervalMs) - ratePerMin(prevInterval);
...
test("ProviderPacing: distance-proportional recovery still NEVER crosses the ceiling", () => {
	...
	assert.equal(
		pacing.currentIntervalMs,
		ceilingMs,
		"rests exactly at the ceiling, never below",
	);
});
...
test("ProviderPacing: the step is ADDITIVE, not multiplicative — each step is a deterministic fixed increment (rate-space base + interval-space deep boost)", () => {
	...
	const calibratedRateStep =
		msPerMin / (initialIntervalMs - baseStep) - msPerMin / initialIntervalMs;
	...
	const rateSpaceBaseStepMs = (interval: number): number => {
		const newInterval = Math.max(
			1,
			Math.round(msPerMin / (msPerMin / interval + calibratedRateStep)),
		);
	...
	for (let i = 0; i < 5; i += 1) {
		...
		const expectedDelta =
			rateSpaceBaseStepMs(prev) +
			Math.floor(gain * Math.max(0, prev - initialIntervalMs));
		assert.equal(
			observedDelta,
			expectedDelta,
		...
		assert.ok(
			Number.isInteger(pacing.currentIntervalMs),
			`interval stays integer-valued (i=${i})`,
		);
...
test("ProviderPacing: recordSuccess without suppressAdditiveIncrease (or false) still decreases interval normally", () => {
	...
	assert.equal(
		pacing.currentIntervalMs,
		900,
		"plain recordSuccess() still decreases interval (calibrated first step)",
	);
...
test("ProviderPacing §10-E: warm-start exactly AT the staleness boundary is still treated as fresh", () => {
	...
	assert.equal(
		boundary.currentIntervalMs,
		320,
		"warm-start exactly at the age boundary is still treated as fresh",
	);
});
...
test("ProviderPacing Fix1: elapsed weighting — deep interval recovers MUCH faster with large elapsed gap", () => {
	...
	const calibratedRateStep =
		msPerMin / (initialIntervalMs - additiveIncreaseMs) -
		msPerMin / initialIntervalMs;
	const rateSpaceBaseStepMs = (interval: number): number => {
		const newInterval = Math.max(
			1,
			Math.round(msPerMin / (msPerMin / interval + calibratedRateStep)),
	...
	assert.ok(
		intervalAfterFirst > initialIntervalMs,
		"still deep above initial after first success",
	);
	...
	const weight = Math.min(
		elapsedRecoveryCap,
		Math.max(1, bigElapsedMs / initialIntervalMs),
	);
	const weightedStep =
		baseStep + Math.floor(recoveryGain * overBackoff * weight);
	...
	assert.equal(
		actualStep,
		weightedStep,
		`step = rateSpaceBase + floor(gain*overBackoff*weight) = ${weightedStep}`,
	);
	assert.ok(
...
test("ProviderPacing Fix1: gentle-near-ceiling PRESERVED — huge elapsed gap at tiny overBackoff does NOT collapse to floor", () => {
	...
	const calibratedRateStep =
		msPerMin / (initialIntervalMs - 100) - msPerMin / initialIntervalMs;
	const rateSpaceBaseStepMs = (interval: number): number => {
		const newInterval = Math.max(
			1,
			Math.round(msPerMin / (msPerMin / interval + calibratedRateStep)),
	...
	assert.ok(
		pacing2.currentIntervalMs > initialIntervalMs,
		"pushed above initial",
	);
	...
	const expectedStep =
		rateSpaceBaseStepMs(intervalAboveInitial) +
		Math.floor(0.1 * (intervalAboveInitial - initialIntervalMs) * 8);
	const actualStep = intervalAboveInitial - pacing2.currentIntervalMs;
	assert.equal(
		actualStep,
		expectedStep,
		`step is rateSpaceBase + weighted-overBackoff: ${expectedStep}`,
	);
	assert.notEqual(
...
test("ProviderPacing Fix1: backward-compat at operating point — normal-cadence success recovers at weight=1", () => {
	...
	const expectedUnweighted =
		rateSpaceBaseStepMs(beforeFirst) +
		Math.floor(0.1 * (beforeFirst - initialIntervalMs));
	assert.equal(
		stepFirst,
		expectedUnweighted,
	...
	const expectedSecond =
		rateSpaceBaseStepMs(beforeSecond) +
		Math.floor(0.1 * (beforeSecond - initialIntervalMs));
	assert.equal(
		stepSecond,
		expectedSecond,
...
test("ProviderPacing Fix2: plain recordThrottle multiplies by 1.5 (1000 → 1500), not ×2", () => {
	...
	assert.equal(
		pacing.currentIntervalMs,
		1500,
		"default softThrottleGain=0.5 → 1000 × 1.5 = 1500",
	);
});
...
test("ProviderPacing Fix2: repeated throttles are clamped at maxIntervalMs", () => {
	...
	assert.equal(
		pacing.currentIntervalMs,
		maxIntervalMs,
		"interval rests exactly at maxIntervalMs after saturation",
	);
});
...
test("ProviderPacing Fix2: maxIntervalMs does not affect retry-after one-shot (retry-after path unchanged)", async () => {
	...
	assert.equal(
		spy.calls.at(-1),
		90_000,
		"retry-after one-shot honored exactly even when > maxIntervalMs",
	);
	...
	assert.equal(
		pacing.currentIntervalMs,
		1000,
		"retry-after does not change the sustained interval",
	);
});
...
test("ProviderPacing §9-C2: additive increase is RATE-space — per-success rate gain is ~CONSTANT across the ramp, not super-accelerating near the floor", () => {
	...
	const rateStep =
		ratePerMin(initialIntervalMs - baseStep) - ratePerMin(initialIntervalMs); // 6.667/min
	...
	assert.equal(
		pacing.currentIntervalMs,
		floorMs,
		"rate-space law rests at exactly the floor (steady state unchanged)",
	);
	assert.equal(
		steps,
		27,
		"cold-start ramp 1000→250 is 27 successes (cautious rate-space approach, not the old 8)",
	);
