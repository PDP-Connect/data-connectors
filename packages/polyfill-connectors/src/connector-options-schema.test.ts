 ...
 * is written. Those are stated as "however the manifest is written" -- each
 * hostile fixture writes `declared_option_kind: "transport"` a different
 * way and every one must still resolve to `collection_scope`.
 *
 * None of this repo's shipped manifests declare an `options_schema` yet
 * (unlike PDP-Connect/pdpp's `slack.json`, the worked example its own
 * suite reads from disk), so every fixture here is synthetic rather than a
 * real manifest read from `../manifests/`. The parser and guardrail logic
 * are exercised identically either way.
 */
...
function optionOf(
	schema: { options: readonly { optionKey: string }[] } | null,
	key: string,
) {
...
test("a manifest cannot make a registry-classified collection_scope option self-activating", () => {
	...
	assert.equal(
		platformOptionKind("slack", "CHANNEL_ALLOWLIST"),
		"collection_scope",
	);
...
test("an option the registry does not know fails closed to collection_scope", () => {
	...
	assert.equal(
		option.platformClassified,
		false,
		"an unclassified key must say so, not pose as a platform decision",
	);
});
...
test("a synthetic slack-shaped manifest resolves the fields its code reads", () => {
	const resolved = resolveOptionsSchemaFromManifest(
		{
			connector_key: "slack",
			options_schema: {
				properties: {
					...
					LOOKBACK_DAYS: {
						...
						default: 7,
						description:
							"How many days of message history to fetch per channel.",
						type: "integer",
					},
					SKIP_FILES: {
						declared_option_kind: "transport",
						default: true,
						description: "Skip downloading file attachments.",
						type: "boolean",
					},
				},
				type: "object",
	...
	assert.deepEqual(
		...
		["CHANNEL_TYPES", "LOOKBACK_DAYS", "SKIP_FILES"],
	);
	...
	assert.deepEqual(channelTypes.defaultValue, [
		"public",
		"private",
		"im",
		"mpim",
	]);
	assert.deepEqual(channelTypes.enumValues, [
		"public",
		"private",
		"im",
		"mpim",
...
test("connectorOptionsSchemas() over this repo's real shipped manifests: none currently declare options_schema", () => {
	// None of this repo's manifests declare `options_schema` yet (see the
	// module doc comment); this pins that fact so the day a manifest adds
	// one, this test's failure is the signal to replace it with a positive
	// assertion, the same shape as pdpp's own "every shipped manifest that
	// declares an options_schema parses" test.
...
> = [
	[
		"a non-object options_schema",
		{ connector_key: "slack", options_schema: [] },
		/must be an object/,
	],
	[
	...
	[
		"an unsupported type",
		{
			connector_key: "slack",
			options_schema: {
				properties: { X: { default: 1, description: "d", type: "number" } },
				type: "object",
			},
		},
		/type must be one of/,
	...
	[
		"a missing default",
		{
			connector_key: "slack",
			options_schema: {
				properties: { X: { description: "d", type: "boolean" } },
				type: "object",
			},
		},
		/default is required/,
	...
	[
		"a default of the wrong type",
		{
			...
			options_schema: {
				properties: {
					X: { default: "yes", description: "d", type: "boolean" },
				},
				type: "object",
			},
		},
	...
	[
		"a non-integer default for an integer option",
		{
			connector_key: "slack",
			options_schema: {
				properties: { X: { default: 1.5, description: "d", type: "integer" } },
				type: "object",
			},
		},
	...
	[
		"a missing description",
		{
			connector_key: "slack",
			options_schema: {
				properties: { X: { default: true, type: "boolean" } },
				type: "object",
			},
		},
	...
	[
		"an empty enum",
		{
			...
			options_schema: {
				properties: {
					X: { default: "a", description: "d", enum: [], type: "string" },
				},
				type: "object",
			},
		},
...
for (const [label, manifest, expected] of malformed) {
	test(`rejects ${label}`, () => {
		assert.throws(
			...
			(err: unknown) =>
				err instanceof ConnectorOptionsSchemaError &&
				expected.test((err as Error).message),
...
test("a scratch manifests dir is picked up, so the resolver reads real files not a hardcoded list", () => {
	...
	writeFileSync(
		...
		JSON.stringify(
			manifestClaiming("scratch_connector", "SOME_KNOB", "transport"),
		),
	);
	...
	try {
		...
		assert.equal(
			optionOf(schemas.scratch_connector ?? null, "SOME_KNOB").optionKind,
			"collection_scope",
		);
	} finally {
