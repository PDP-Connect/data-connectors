...
test("PreflightWaitProbe: counts only non-zero pre-flight waits", async () => {
	...
	assert.deepEqual(
		slept,
		[0, 0, 500, 250],
		"the underlying sleep is always called",
	);
	probe.reset();
...
test("STACKING REGRESSION: a converged ChatGPT-shaped request path has exactly ONE pre-flight wait source", async () => {
	...
	assert.equal(
		probe.count,
		1,
		"first request waits through exactly one pre-flight source",
	);
	...
	assert.equal(
		probe.count,
		1,
		"second request still has exactly one pre-flight wait source — no stacking",
	);
});
...
test("STACKING DETECTOR: the legacy preflight mode IS detectably a second gate", async () => {
	...
	assert.equal(
		probe.count,
		2,
		"legacy preflight mode + lane launch delay = TWO pre-flight waits (the anti-pattern)",
	);
});
...
test("ProviderBudgetController: signal mode performs no pre-flight wait in beforeRequest", async () => {
	...
	assert.equal(
		controller.pacingDelayHint(),
		2500,
		"the hint carries the GCRA interval",
	);
});
...
test("ProviderBudgetController: preflight mode still sleeps in beforeRequest (legacy parity)", async () => {
	...
	assert.equal(
		controller.pacingDelayHint(),
		0,
		"preflight mode yields no hint (would double-count)",
	);
});
