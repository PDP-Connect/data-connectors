...
function fieldShouldBeSemantic(field: string, schema: JsonSchema): boolean {
	return (
		isSearchableStringSchema(schema) &&
		!NON_TEXT_FIELD_NAME.test(field) &&
		(SEMANTIC_FIELD_NAMES.has(field) ||
			(typeof schema.maxLength === "number" && schema.maxLength >= 1024))
	);
}
	...
	for (const file of manifestFiles()) {
		...
		for (const stream of manifest.streams ?? []) {
			...
			const semanticFields = new Set(
				stream.query?.search?.semantic_fields ?? [],
			);
			...
			for (const [field, schema] of Object.entries(properties)) {
				...
				if (
					fieldShouldBeSemantic(field, schema) &&
