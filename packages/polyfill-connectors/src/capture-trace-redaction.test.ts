...
function assertZipFreeOfSecret(zipPath: string, secret: string): void {
	...
	assert.deepEqual(
		leaking,
		[],
		`secret found inside ${zipPath} entries: ${leaking.join(", ")}`,
	);
}
 ...
 */
async function runTracedLogin(options: {
	hasSecrets: boolean;
}): Promise<{ dir: string; traces: string[] }> {
...
test("trace zip hides the secret from grep — why these tests are zip-aware", async (t) => {
	...
	assert.equal(
		readFileSync(zipPath).includes(SECRET),
		false,
		"precondition: the secret is compressed, not plaintext",
	);
...
test("a run that registered credentials persists no trace zip at all", async (t) => {
	...
	assert.deepEqual(
		traces,
		[],
		`expected no retained traces, found: ${traces.join(", ")}`,
	);
...
test("suppression leaves a breadcrumb explaining the missing trace", async (t) => {
	...
	const notes = existsSync(dir)
		? readdirSync(dir).filter((name) => name.endsWith(".suppressed.json"))
		: [];
	assert.equal(
		notes.length,
		1,
		`expected one suppression note, found: ${notes.join(", ")}`,
	);
	const note = JSON.parse(
		readFileSync(join(dir, notes[0] as string), "utf8"),
	) as { reason?: string };
...
test("a run with no registered credentials keeps its trace for diagnostics", async (t) => {
	...
	assert.ok(
		traces.length > 0,
		"expected a trace to be retained when no credential was registered",
	);
});
...
test("registering a credential arms the trace gate on the real session", () => {
	withCaptureEnv(() => {
		...
		assert.equal(
			capture.hasRegisteredSecrets(),
			false,
			"a fresh session must not suppress traces",
		);
		capture.registerSecrets([SECRET]);
		assert.equal(
			capture.hasRegisteredSecrets(),
			true,
			"a registered credential must arm the gate",
		);
...
test("a credential too short to match still arms the trace gate", () => {
	withCaptureEnv(() => {
		const capture = createCaptureSession(
			`gate_short_${process.pid}_${Date.now()}`,
		);
		assert.ok(capture);
		...
		assert.equal(
			capture.hasRegisteredSecrets(),
			true,
			"a sub-floor credential is still a credential",
		);
		rmSync(capture.baseDir, { force: true, recursive: true });
	});
});
...
test("http capture redacts a credential in a url or response body", () => {
	withCaptureEnv(() => {
		...
		capture.captureHttp(
			...
			{
				method: "GET",
				path: `https://venmo.test/cb?code=${SECRET}`,
				status: 200,
			},
		);
		...
		assert.ok(
			...
			`credential leaked into http capture:\n${written}`,
		);
...
test("record capture redacts a credential echoed back by an api", () => {
	withCaptureEnv(() => {
		...
		capture.recordRecord({
			stream: "profile",
			data: { id: "u1", session_token: SECRET },
		});

		const written = readFileSync(
			join(capture.baseDir, "records", "profile.jsonl"),
			"utf8",
		);
		assert.ok(
			!written.includes(SECRET),
			`credential leaked into records capture:\n${written}`,
		);
...
test("no screenshot is written when the page cannot build a credential mask", async () => {
	await withCaptureEnv(async () => {
		...
		await capture.captureDom(
			page as unknown as Parameters<CaptureSession["captureDom"]>[0],
			"unmaskable",
		);
...
test("screenshot capture masks a one-time code rendered as readable text", async () => {
	...
	try {
		...
		const png = readFileSync(
			join(captured, "screenshots", "otp-after-submit.png"),
		);
		...
		assert.ok(
			magenta > 0,
			"expected a mask overlay covering the credential fields in the screenshot",
		);
