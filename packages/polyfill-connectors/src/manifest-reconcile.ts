...
const EMIT_OBJ_RE =
	/type\s*:\s*['"]RECORD['"][^}]*?stream\s*:\s*['"]([a-zA-Z_][a-zA-Z0-9_]*)['"]/g;
...
export function reconcile(args: {
	...
	const missing_emit = declared.filter(
		(s) => !(emittedSet.has(s) || registeredSet.has(s)),
	);
	...
	return {
		...
		missing_emit,
		ok:
			missing_manifest.length === 0 &&
			missing_schema.length === 0 &&
			missing_emit.length === 0,
	};
}
...
export function reconcileFromDisk(
	args: ReconcileFromDiskArgs,
): ReconcileReport {
	const manifest = parseManifestStreams(
		readFileSync(args.manifestPath, "utf8"),
	);
	const schema = args.schemaPath
		? parseSchemaStreams(readFileSync(args.schemaPath, "utf8"))
		: { registered: [] };
