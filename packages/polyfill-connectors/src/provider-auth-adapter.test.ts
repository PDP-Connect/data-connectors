...
} from "./provider-auth-adapters.ts";
...
test("resolveProviderAuthAdapter eagerly loads the fixed adapter module list and resolves the real registered kinds", async () => {
	...
	assert.ok(
		oauth2Generic,
		"oauth2_generic is registered from provider-auth-adapters.ts's adapter list",
	);

	const googleDataPortability = await resolveProviderAuthAdapter(
