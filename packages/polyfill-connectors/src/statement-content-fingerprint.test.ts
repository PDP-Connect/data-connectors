...
test("hasStatementContentFingerprint: false when pdf_page_count zero", () => {
	assert.ok(
		!hasStatementContentFingerprint({ ...CONTENT_BEARING, pdf_page_count: 0 }),
	);
});
...
test("statementFingerprintExcludeKeys: content-bearing → full content-gated exclusion", () => {
	...
	assert.deepEqual(
		[...keys].sort(),
		[...STATEMENT_CONTENT_GATED_EXCLUDE_KEYS].sort(),
	);
...
test("statementFingerprintExcludeKeys: content-less → conservative run-clock-only", () => {
	...
	assert.deepEqual(
		[...keys].sort(),
		[...STATEMENT_RUN_CLOCK_EXCLUDE_KEYS].sort(),
	);
...
test("AC-1: blob-only churn with identical content fields → same fingerprint", () => {
	const fp1 = recordFingerprint(
		CONTENT_BEARING,
		statementFingerprintExcludeKeys(CONTENT_BEARING),
	);
	const fp2 = recordFingerprint(
		BLOB_CHURNED,
		statementFingerprintExcludeKeys(BLOB_CHURNED),
	);
	assert.equal(
		fp1,
		fp2,
		"blob churn should produce identical fingerprints when content fields match",
	);
});
...
test("AC-2: different pdf_text_sha256 → distinct fingerprint", () => {
	...
	const fp2 = recordFingerprint(
		changed,
		statementFingerprintExcludeKeys(changed),
	);
	assert.notEqual(
		fp1,
		fp2,
		"different pdf_text_sha256 must change the fingerprint",
	);
});
...
test("AC-2: different pdf_page_count → distinct fingerprint", () => {
	...
	assert.notEqual(
		fp1,
		fp2,
		"different pdf_page_count must change the fingerprint",
	);
});
...
test("AC-3: account_id null→value is always a fingerprint boundary", () => {
	...
	const fp1 = recordFingerprint(
		nullAccountId,
		statementFingerprintExcludeKeys(nullAccountId),
	);
	const fp2 = recordFingerprint(
		CONTENT_BEARING,
		statementFingerprintExcludeKeys(CONTENT_BEARING),
	...
	assert.notEqual(
		fp1,
		fp2,
		"account_id null→value must change the fingerprint (never excluded)",
	);
});
...
test("AC-4: content-less version and content-bearing version have distinct fingerprints", () => {
	const fp1 = recordFingerprint(
		CONTENT_LESS,
		statementFingerprintExcludeKeys(CONTENT_LESS),
	);
	const fp2 = recordFingerprint(
		CONTENT_BEARING,
		statementFingerprintExcludeKeys(CONTENT_BEARING),
	...
	assert.notEqual(
		fp1,
		fp2,
		"a content-less version must not collapse into a content-bearing version",
	);
});
...
test("AC-4: two content-less versions with same body are same fingerprint (conservative behavior preserved)", () => {
	...
	assert.equal(
		fp1,
		fp2,
		"content-less: only fetched_at excluded, same body → same fingerprint",
	);
});
...
test("AC-5: statementContentFingerprintFromText: same text with jitter → same sha256", () => {
	const fp1 = statementContentFingerprintFromText(
		"Hello   World\nFoo\t\tBar",
		2,
	);
...
test("AC-5: statementContentFingerprintFromText: NFC normalization is applied", () => {
	...
	assert.equal(
		fp1.pdf_text_sha256,
		fp2.pdf_text_sha256,
		"NFC must unify precomposed and decomposed forms",
	);
});
