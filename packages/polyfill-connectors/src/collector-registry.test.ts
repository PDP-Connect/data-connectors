...
test("every local collector definition is well-formed and filesystem-class", () => {
	...
	for (const def of LOCAL_COLLECTOR_DEFINITIONS) {
		...
		assert.equal(
			ids.has(def.connector_id),
			false,
			`duplicate connector_id ${def.connector_id}`,
		);
		...
		// path shape.
		assert.ok(
			def.entry.length > 0 && !def.entry.includes("/"),
			`entry must be a bare segment: ${def.entry}`,
		);
		assert.equal(
			def.bindings.filesystem?.required,
			true,
			`${def.connector_id} must require the filesystem binding`,
		);
		assert.ok(
			Array.isArray(def.streams) && def.streams.length > 0,
			`${def.connector_id} must declare streams`,
		);
	}
});
...
test("every default stream is declared in the connector's manifest (no undeclared stream requested)", async () => {
	for (const def of LOCAL_COLLECTOR_DEFINITIONS) {
		...
		for (const stream of def.streams) {
			assert.ok(
				declared.has(stream),
				`${def.connector_id} default stream '${stream}' is not declared in the manifest`,
			);
		}
	}
});
