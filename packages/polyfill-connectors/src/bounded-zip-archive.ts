...
const EOCD_SCAN_WINDOW_BYTES =
	ZIP_EOCD_MAX_COMMENT_LENGTH + ZIP_EOCD_MIN_LENGTH;
...
const ZIP_CENTRAL_DIRECTORY_MAX_RECORD_LENGTH =
...
export class ZipPolicyViolationError extends Error {
	readonly code:
		...
		| "unsafe_entry_name";
	constructor(
		code: ZipPolicyViolationError["code"],
		message: string,
...
function findEndOfCentralDirectory(
	source: BytesSource,
): { offset: number; tail: Buffer; tailStart: number } | null {
	...
	for (
		let offset = tail.length - ZIP_EOCD_MIN_LENGTH;
		offset >= min;
		offset -= 1
	) {
...
function readCentralDirectoryRecord(
	...
	const unixMode =
		versionMadeByHost === ZIP_VERSION_MADE_BY_HOST_UNIX
			? Math.floor(externalFileAttributes / 65_536)
			: 0;
...
function resolveCompressedSlice(
	source: BytesSource,
...
function resolveEffectiveCap(
	...
	const totalBudgetIsBinding =
		budget.remainingTotalBytes < policy.maxEntryUncompressedBytes;
	return {
		bytes: totalBudgetIsBinding
			? budget.remainingTotalBytes
			: policy.maxEntryUncompressedBytes,
		violationCode: totalBudgetIsBinding ? "total_too_large" : "entry_too_large",
	};
}

function inflateOrStoreEntry(
	compressed: Buffer,
	record: CentralDirectoryRecord,
	cap: EffectiveCap,
): Buffer {
	const boundDescription =
		cap.violationCode === "total_too_large"
			? "the shared total"
			: "the per-entry cap";
 ...
 */
function readZipEntriesFromSource(
	source: BytesSource,
	policy: ZipReadPolicy,
): ZipEntry[] {
	...
	// policy.maxEntries.
	const maxPlausibleCentralDirSize =
		entryCount * ZIP_CENTRAL_DIRECTORY_MAX_RECORD_LENGTH;
	...
	for (let i = 0; i < entryCount; i += 1) {
		...
		if (
			UNSAFE_ZIP_ENTRY_NAME_RE.test(record.name) ||
			WHITESPACE_PADDED_DOT_DOT_SEGMENT_RE.test(record.name)
		) {
			throw new ZipPolicyViolationError(
				"unsafe_entry_name",
