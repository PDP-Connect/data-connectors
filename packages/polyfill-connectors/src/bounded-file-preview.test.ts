...
import {
	BOUNDED_PREVIEW_MAX_BYTES,
	readBoundedFilePreview,
} from "./bounded-file-preview.ts";
...
test("file larger than maxBytes → bounded buffer + truncated flag", async () => {
	...
	assert.equal(
		got.bytesRead,
		max,
		"must retain at most maxBytes regardless of file size",
	);
});
...
test("UTF-8 multibyte sequence cut at the byte boundary is not misclassified as binary", async () => {
	...
	assert.equal(
		got.bytesRead,
		prefixAscii.length,
		"incomplete trailing code point trimmed",
	);
	const preview = safeTextPreview(got.buffer, 500);
	assert.equal(
		preview.kind,
		"text",
		"trimmed prefix must decode as text, not binary",
	);
	assert.equal(preview.preview, prefixAscii);
	// Sanity: the untrimmed raw prefix WOULD have been classified binary.
	const rawPrefix = Buffer.from(body, "utf8").subarray(
		0,
		prefixAscii.length + 1,
	);
...
test("complete multibyte sequence at the exact boundary is retained", async () => {
	...
	assert.equal(
		got.bytesRead,
		totalBytes,
		"complete trailing code point not trimmed",
	);
...
test("binary content within the prefix is still detected as binary", async () => {
	...
	const path = await writeBytes(
		"binary.txt",
		Buffer.from([0x61, 0x62, 0x00, 0x63]),
	);
	const got = await readBoundedFilePreview(path);
	assert.ok(got);
	const preview = safeTextPreview(got.buffer, 500);
