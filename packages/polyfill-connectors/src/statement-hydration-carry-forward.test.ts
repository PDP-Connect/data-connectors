...
function priorMapFrom(
	state: Record<string, unknown>,
): ReadonlyMap<string, StatementHydration> {
...
test("isHydrated: only a complete pointer triple counts as hydrated", () => {
	...
	assert.equal(
		isHydrated({ ...HYD, pdf_path: null }),
		false,
		"partial triple is not hydrated",
	);
});
...
test("resolveOnFailure: previously-hydrated statement carries prior pointers forward", () => {
	...
	assert.deepEqual(
		carried,
		HYD,
		"failure on a previously-hydrated statement carries prior pointers",
	);
	...
	assert.deepEqual(
		cursorB.toState().S1,
		HYD,
		"carried pointers survive into the next STATE",
	);
});
...
test("resolveOnFailure: never-hydrated statement stays all-null (honest index-only)", () => {
	...
	assert.deepEqual(
		resolved,
		NEVER_HYDRATED,
		"a statement with no prior hydration stays all-null",
	);
...
test("note: a successful (re)hydration overwrites the carried pointers", () => {
	...
	assert.deepEqual(
		cursorB.toState().S1,
		rehydrated,
		"a fresh hydration replaces the prior pointers",
	);
});
...
test("readPriorStatementHydration: tolerates missing / legacy / malformed state", () => {
	assert.equal(
		readPriorStatementHydration({}).size,
		0,
		"empty state → empty map",
	);
	assert.equal(
		readPriorStatementHydration({ fetched_at: "x" }).size,
		0,
		"legacy cursor (no hydration) → empty map",
	);
	assert.equal(
		readPriorStatementHydration({ hydration: 5 }).size,
		0,
		"malformed hydration value → empty map",
	);
	assert.equal(
		readPriorStatementHydration(null).size,
		0,
		"null state → empty map",
	);
	assert.equal(
		readPriorStatementHydration({
			hydration: {
				S1: { document_url: null, pdf_path: null, pdf_sha256: null },
			},
		}).size,
		0,
		"an all-null prior entry is not carriable and is dropped",
	);
	const ok = readPriorStatementHydration({
		hydration: { S1: HYD, bad: { pdf_path: 5 } },
	});
