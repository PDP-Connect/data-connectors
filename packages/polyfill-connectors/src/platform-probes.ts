...
export async function detectCloudflareChallenge(
	...
	const countMatches = (selector: string): Promise<number> =>
		safe(() => page.locator(selector).count(), 0);
	...
	if (
		(await countMatches(
			'script[src*="challenge-platform"], script[src*="/cdn-cgi/challenge-platform/"]',
		)) > 0
	) {
	...
	if (
		(await countMatches(
			"#cf-challenge-running, #challenge-running, #cf-error-details",
		)) > 0
	) {
	...
	if (navResponse) {
		...
		if (
			navResponse.status() === 403 &&
			(headers["cf-ray"] !== undefined || CF_SERVER_RE.test(server))
		) {
	...
	return {
		isChallenge,
		signals,
		confidence: isChallenge ? "confirmed" : "none",
	};
}
