...
type AdaptiveLaneEventInput = Omit<
	AdaptiveLaneEvent,
	keyof AdaptiveLaneSnapshot
>;
...
export class AdaptiveLaneAttemptTimeoutError extends Error {
	...
	constructor(name: string, attempt: number, timeoutMs: number) {
		super(
			`adaptive lane ${name} attempt ${attempt} timed out after ${timeoutMs}ms`,
		);
...
function normalizeOutcome(
	outcome: AdaptiveLaneOutcome | AdaptiveLaneOutcomeKind,
): AdaptiveLaneOutcome {
	return typeof outcome === "string" ? { kind: outcome } : outcome;
}

export function createAdaptiveLane<T>(
	options: AdaptiveLaneOptions<T>,
): AdaptiveLane<T> {
	...
	const initialConcurrency = clampInt(
		options.initialConcurrency,
		minConcurrency,
		maxConcurrency,
	);
	const maxAttempts = Math.max(1, Math.floor(options.maxAttempts ?? 1));
	const successWindow = Math.max(
		1,
		Math.floor(options.successWindow ?? Math.max(3, maxConcurrency)),
	);
	const random = options.random ?? Math.random;
	const sleep = options.sleep ?? defaultSleep;
	const pressureMinDelayMs = Math.max(
		0,
		Math.floor(options.pressureMinDelayMs ?? options.minDelayMs),
	);
	const pressureMaxDelayMs = Math.max(
		pressureMinDelayMs,
		Math.floor(options.pressureMaxDelayMs ?? options.maxDelayMs),
	);
	...
	const boundedExplicitDelay = (delayMs: number): number =>
		clampInt(delayMs, pressureMinDelayMs, pressureMaxDelayMs);
	...
	const runAttempt = (
		...
		return Promise.race([
			...
			wait(options.attemptTimeoutMs, signal).then(() => {
				throw new AdaptiveLaneAttemptTimeoutError(
					options.name,
					attempt,
	...
	const runAttempts = async (
		...
		for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
			try {
				...
				if (
					(await handlePressure(outcome, attempt, signal, error)) === "stop"
				) {
					throw error;
				}
			}
		...
		throw new Error(
			`adaptive lane ${options.name} exhausted without terminal outcome`,
		);
	};
	...
	const run = (
		...
		if (
			...
			const error = new AdaptiveLaneQueueFullError(
				options.name,
