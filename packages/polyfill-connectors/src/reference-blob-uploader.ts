...
function getUploadIterator(
	content: ReferenceBlobUploadContent,
): AsyncIterator<Buffer | Uint8Array | string> {
	...
	return {
		...
		return: (value?: unknown) => {
			...
			return Promise.resolve({
				...
				value: value as Buffer | Uint8Array | string,
			});
		},
	};
}

function createHashingUploadBody(
	content: ReferenceBlobUploadContent,
): HashingUploadBody {
	...
	let resolveDigest: (value: { sha256: string; sizeBytes: number }) => void =
		() => undefined;
	let rejectDigest: (reason?: unknown) => void = () => undefined;
	const digest = new Promise<{ sha256: string; sizeBytes: number }>(
		(resolve, reject) => {
	...
	const failTransportDigest = (reason: unknown): void => {
		...
		settled = true;
		rejectDigest(
			new ReferenceBlobUploadFailure("transport", errorMessage(reason), reason),
		);
	};
	...
	return {
		body,
		digest,
		sourceFailure: () => ({ failed: sourceFailed, reason: sourceFailure }),
	};
}
...
async function validatedBlobUploadResponse(
	response: Response,
	upload: HashingUploadBody,
): Promise<ReferenceBlobRef> {
	...
	if (!response.ok) {
		const message =
			body && typeof body === "object" && !Array.isArray(body)
				? (extractErrorFieldMessage((body as Record<string, unknown>).error) ??
					response.statusText)
				: response.statusText;
		throw new ReferenceBlobUploadFailure(
	...
	if (!isBlobUploadResponse(body)) {
		throw new ReferenceBlobUploadFailure(
			"invalid_response",
			"blob upload returned an invalid response",
		);
	}
	...
	if (
		body.sha256 !== localHash.sha256 ||
		body.size_bytes !== localHash.sizeBytes
	) {
		throw new ReferenceBlobUploadFailure(
			"integrity_mismatch",
			"blob upload hash/size mismatch",
		);
	}
	return {
...
export function runtimeBlobUploadAvailable(
	...
	return async ({
		...
		const response = await fetchUploadResponse({
			...
			url: makeBlobUploadUrl({
				connectorId,
				connectorInstanceId:
					connectorInstanceId ?? args.connectorInstanceId ?? null,
				mimeType,
				recordKey,
				rsUrl: args.rsUrl,
				stream,
