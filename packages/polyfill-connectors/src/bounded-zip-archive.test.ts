...
test("readZipEntries reads a flat single-entry zip", () => {
	const zip = buildZip([
		{ content: Buffer.from("hello world"), name: "hello.txt" },
	]);
...
test("zipBasename strips directory components", () => {
	assert.equal(
		zipBasename("CONTENT_INTERACTION/ViewingActivity.csv"),
		"ViewingActivity.csv",
	);
...
test("readZipEntries rejects an archive declaring more entries than maxEntries", () => {
	...
	assert.throws(() => readZipEntries(zip, policy), {
...
test("LYING declared size cannot bypass the per-entry actual-bytes cap — inflation itself is bounded", () => {
	...
	const zip = buildZip([
		{ content: bomb, declaredUncompressedSize: 1, name: "lying.bin" },
	]);
	const policy: ZipReadPolicy = {
		maxEntries: 10,
...
test("LYING declared sizes across many small-declared entries cannot bypass the shared total actual-bytes budget", () => {
	...
	assert.ok(
		thirdThrew instanceof ZipPolicyViolationError,
		"third extraction must be rejected by the shared budget",
	);
...
function buildStoredZip(
	content: Buffer,
	declaredSize: number = content.length,
): Buffer {
...
test("boundary: STORE-method actual size — exactly maxEntryUncompressedBytes succeeds, +1 byte fails", () => {
	...
	const policy: ZipReadPolicy = {
		maxEntries: 10,
		maxEntryUncompressedBytes: cap,
		maxTotalUncompressedBytes: cap,
	};

	const atCapEntries = readZipEntries(
		buildStoredZip(Buffer.alloc(cap, 91), 1),
		policy,
	);
	assert.equal(
		atCapEntries[0]?.data().length,
		cap,
		"STORE actual bytes exactly at the cap must succeed",
	);

	const overCapEntries = readZipEntries(
		buildStoredZip(Buffer.alloc(cap + 1, 91), 1),
		policy,
	);
...
test("malformed central-directory metadata (out-of-bounds offsets) is rejected, not crashed on", () => {
	...
	assert.equal(
		entries.length,
		1,
		"entry is still listed from valid central-directory metadata",
	);
	assert.throws(
		() => entries[0]?.data(),
		/zip_entry_local_header_invalid|zip_entry_data_out_of_bounds/,
	);
});
...
test("malformed central-directory length fields that would overflow past the buffer are rejected", () => {
	...
	assert.equal(
		entries.length,
		0,
		"corrupt-length entry must be dropped, not crash the reader",
	);
});
...
function withTempZipFile<T>(
	zip: Buffer,
	fn: (fd: number, fileSize: number) => T,
): T {
 ...
 * settles.
 */
async function withTempZipFileAsync<T>(
	zip: Buffer,
	fn: (fd: number, fileSize: number) => Promise<T>,
): Promise<T> {
...
test("readZipEntriesFromFile enforces the same declared-size fast-reject as the buffer API", () => {
	...
	withTempZipFile(zip, (fd, fileSize) => {
		assert.throws(() => readZipEntriesFromFile(fd, fileSize, policy), {
...
test("readZipEntriesFromFile handles a multi-GB-scale archive using a SPARSE fixture (no real large allocation)", () => {
	...
	try {
		...
		assert.equal(
			entries.length,
			0,
			"listing throws before returning any entries",
		);
	} catch (err) {
...
test("readZipEntriesFromFile rejects a LYING centralDirSize that claims far more than entryCount plausibly needs (B1 — unbounded central-directory read)", () => {
	...
	try {
		...
		assert.ok(
			threw instanceof ZipPolicyViolationError,
			`expected a ZipPolicyViolationError, got: ${threw}`,
		);
		assert.equal((threw as ZipPolicyViolationError).code, "too_many_entries");
		assert.match(
			(threw as ZipPolicyViolationError).message,
			/central directory/i,
		);
...
test("boundary: maxEntries — exactly maxEntries entries succeeds, maxEntries+1 fails", () => {
	...
	assert.throws(() => readZipEntries(buildWithCount(6), policy), {
		code: "too_many_entries",
	});
});
...
test("boundary: central-directory plausibility ceiling — exactly at the ceiling succeeds, ceiling+1 fails", () => {
	...
	assert.throws(() => readZipEntries(overCeiling, GENEROUS_POLICY), {
		code: "too_many_entries",
	});
});
...
test("boundary: per-entry DECLARED uncompressed_size — exactly maxEntryUncompressedBytes succeeds, +1 byte fails", () => {
	...
	assert.equal(
		atCap.length,
		1,
		"declared size exactly at the cap must pass the fast-reject",
	);
	assert.equal(atCap[0]?.data().length, cap);

	assert.throws(() => readZipEntries(buildWithDeclared(cap + 1), policy), {
		code: "entry_too_large",
	});
});
...
test("boundary: per-entry ACTUAL inflated size — exactly maxEntryUncompressedBytes succeeds, +1 real byte fails", () => {
	...
	assert.equal(
		atCapEntries[0]?.data().length,
		cap,
		"actual inflated bytes exactly at the cap must succeed",
	);

	const overCapZip = buildZip([
		{
			content: Buffer.alloc(cap + 1, 66),
			declaredUncompressedSize: 1,
			name: "a.bin",
		},
	]);
...
test("boundary: running total DECLARED uncompressed_size — exactly maxTotalUncompressedBytes across entries succeeds, +1 byte fails", () => {
	const buildTwoEntries = (secondDeclaredSize: number) =>
		buildZip([
			...
			{
				content: Buffer.alloc(secondDeclaredSize, 68),
				declaredUncompressedSize: secondDeclaredSize,
				name: "b.bin",
			},
		]);
	const total = 4096;
	...
	assert.equal(
		atCap.length,
		2,
		"declared sizes summing to exactly the total budget must pass the fast-reject",
	);

	assert.throws(
		() => readZipEntries(buildTwoEntries(total - 2000 + 1), policy),
...
test("boundary: running total ACTUAL inflated bytes — exactly maxTotalUncompressedBytes across data() calls succeeds, +1 real byte fails", () => {
	...
	const atCapZip = buildZip([
		{
			content: Buffer.alloc(firstReal, 69),
			declaredUncompressedSize: 1,
			name: "a.bin",
		},
		{
			content: Buffer.alloc(secondReal, 70),
			declaredUncompressedSize: 1,
			name: "b.bin",
		},
	]);
	...
	const overCapZip = buildZip([
		...
		{
			content: Buffer.alloc(secondReal + 1, 70),
			declaredUncompressedSize: 1,
			name: "b.bin",
		},
	]);
...
test("readZipEntries rejects a parent-directory-traversal entry name", () => {
	const zip = buildZip([
		{ content: Buffer.from("x"), name: "../../etc/passwd" },
	]);
	assert.throws(() => readZipEntries(zip, GENEROUS_POLICY), {
...
test("readZipEntries rejects a backslash parent-directory-traversal entry name", () => {
	const zip = buildZip([
		{ content: Buffer.from("x"), name: "..\\..\\windows\\system32\\config" },
	]);
	assert.throws(() => readZipEntries(zip, GENEROUS_POLICY), {
		code: "unsafe_entry_name",
	});
...
test("readZipEntries counterweight: a name merely containing '..' as part of a longer segment is NOT flagged as traversal", () => {
	const zip = buildZip([
		{ content: Buffer.from("x"), name: "..hidden-file/foo..bar.txt" },
	]);
	const entries = readZipEntries(zip, GENEROUS_POLICY);
	assert.equal(entries.length, 1);
...
test("readZipEntries rejects a Windows drive-letter entry name", () => {
	const zip = buildZip([
		{ content: Buffer.from("x"), name: "C:\\Windows\\system.ini" },
	]);
	assert.throws(() => readZipEntries(zip, GENEROUS_POLICY), {
		code: "unsafe_entry_name",
	});
...
test("readZipEntries rejects a UNC-root entry name", () => {
	const zip = buildZip([
		{ content: Buffer.from("x"), name: "\\\\attacker-host\\share\\payload" },
	]);
	assert.throws(() => readZipEntries(zip, GENEROUS_POLICY), {
		code: "unsafe_entry_name",
	});
...
test("readZipEntries does NOT treat a regular file as a symlink when version-made-by is NOT Unix", () => {
	...
	assert.equal(
		entries.length,
		1,
		"non-Unix-authored archive must not misread external attrs as a symlink",
	);
...
test("readZipEntries preserves ordinary Unicode and deeply nested harmless names", () => {
	...
	const zip = buildZip(
		names.map((name, i) => ({ content: Buffer.from(`content-${i}`), name })),
	);
	const entries = readZipEntries(zip, GENEROUS_POLICY);
...
test("readZipEntriesFromFile enforces the same dangerous-name and symlink rejections as the buffer API", () => {
	const traversalZip = buildZip([
		{ content: Buffer.from("x"), name: "../escape.txt" },
	]);
	withTempZipFile(traversalZip, (fd, fileSize) => {
		assert.throws(() => readZipEntriesFromFile(fd, fileSize, GENEROUS_POLICY), {
			code: "unsafe_entry_name",
		});
	});
});
...
async function withTempDestPath<T>(
	fn: (destPath: string) => Promise<T>,
): Promise<T> {
...
test("streamZipEntryToFile extracts a DEFLATE entry to disk byte-for-byte", async () => {
	const content = Buffer.from(
		'<HealthData><Record type="HKQuantityTypeIdentifierStepCount"/></HealthData>'.repeat(
			5000,
		),
	);
...
test("streamZipEntryToFile rejects a DECLARED-oversized entry before inflating, and leaves no partial file", async () => {
	...
	const tinyPolicy: ZipReadPolicy = {
		maxEntries: 10,
	...
	await withTempZipFileAsync(zip, async (fd, fileSize) => {
		await withTempDestPath(async (destPath) => {
			...
			assert.equal(
				existsSync(destPath),
				false,
				"no partial file left behind on a declared-size rejection",
			);
		});
	});
});
...
test("streamZipEntryToFile rejects on ACTUAL inflated bytes exceeding the cap even when declared_size lies", async () => {
	...
	const zip = buildZip([
		{ content, declaredUncompressedSize: 10, name: "export.xml" },
	]);
	const policy: ZipReadPolicy = {
		maxEntries: 10,
	...
	await withTempZipFileAsync(zip, async (fd, fileSize) => {
		await withTempDestPath(async (destPath) => {
			await assert.rejects(
				() =>
					streamZipEntryToFile(fd, fileSize, "export.xml", destPath, policy),
				{
					code: "entry_too_large",
				},
			);
...
test("streamZipEntryToFile handles content spanning many read-chunk boundaries without corruption", async () => {
	...
	const content = Buffer.from(
		Array.from(
			{ length: 300_000 },
			(_, i) =>
				`<Record id="${i}" type="HKQuantityTypeIdentifierStepCount" value="${i}"/>`,
		).join("\n"),
	);
	assert.ok(
		content.length > 3 * 1024 * 1024,
		"fixture must span multiple 1 MiB read-chunk windows",
	);
	const zip = buildZip([{ content, name: "export.xml" }]);
	const policy: ZipReadPolicy = {
		maxEntries: 10,
