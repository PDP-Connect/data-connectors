...
test("a walker may descend through an ancestor to reach a selected root", () => {
	...
	assert.equal(
		shouldDescendIntoDirectory("a/b/d", scope),
		false,
		"an unrelated sibling subtree is pruned unlisted",
	);
...
test("date-encoded directories prune whole years and months before listing them", () => {
	...
	assert.equal(
		dateDirectoryInRange({ year: "2025" }, scope),
		false,
		"a whole prior year is pruned",
	);
	assert.equal(dateDirectoryInRange({ year: "2027" }, scope), true);
	assert.equal(
		dateDirectoryInRange({ month: "05", year: "2026" }, scope),
		false,
	);
	assert.equal(
		dateDirectoryInRange({ month: "07", year: "2026" }, scope),
		true,
	);
});
...
test("the boundary day itself is never pruned — it straddles the instant", () => {
	...
	assert.equal(
		dateDirectoryInRange({ day: "16", month: "06", year: "2026" }, scope),
		true,
	);
});
...
test("dot-segment traversal is resolved before containment is decided", () => {
	...
	assert.equal(
		isPathWithinSourceRoots("/a/proj/./sub/file.jsonl", scope),
		true,
		"`.` segments are inert",
	);
	assert.equal(
...
test("the boundary is read off the stream scopes the runtime already threads through", () => {
	const requested = new Map([
		...
		[
			...
			{
				source_roots: ["proj-a"],
				time_range: { since: "2026-06-01T00:00:00.000Z" },
			},
		],
	]);
	...
	assert.equal(
		readEnumerationScope(new Map([["sessions", {}]]), ["sessions"]),
		null,
	);
});
...
test("scopeBoundsEnumeration distinguishes a bounded run from a merely filtered one", () => {
	...
	assert.equal(
		scopeBoundsEnumeration({ since: null, source_roots: [] }),
		false,
	);
	assert.equal(
		scopeBoundsEnumeration({ since: "2026-06-01T00:00:00.000Z" }),
		true,
	);
...
test("a natural absolute path selects its flattened project directory", () => {
	...
	assert.equal(
		projectDirMatchesSourceRoots("-home-u-code-other", scope),
		false,
	);
});
...
test("both dot encodings are accepted, because both appear in real corpora", () => {
	...
	assert.equal(
		projectDirMatchesSourceRoots("-home-u--tmp-proj", scope),
		true,
		"dot folded into a dash",
	);
	assert.equal(
		projectDirMatchesSourceRoots("-home-u-.tmp-proj", scope),
		true,
...
test("a bare project name is accepted as the final segment only", () => {
	...
	assert.equal(
		projectDirMatchesSourceRoots("-home-u-code-pdpp-worktree", scope),
		false,
	);
});
...
test("a bare root never widens onto a longer sibling name", () => {
	assert.equal(
		projectDirMatchesSourceRoots("-home-u-code-real", {
			source_roots: ["rea"],
		}),
		false,
	...
	assert.equal(
		projectDirMatchesSourceRoots("-home-u-code-proj-secrets", {
			source_roots: ["proj"],
		}),
		false,
	...
	assert.equal(
		projectDirMatchesSourceRoots("-home-u-code-mypdpp", {
			source_roots: ["pdpp"],
		}),
		false,
...
test("an absolute root never widens onto a longer sibling path", () => {
	assert.equal(
		projectDirMatchesSourceRoots("-home-u-code-pdpp2", {
			source_roots: ["/home/u/code/pdpp"],
		}),
		false,
...
test("no declared roots leaves every project directory in scope", () => {
	...
	assert.equal(
		projectDirMatchesSourceRoots("-anything", { source_roots: [] }),
		true,
	);
});
