...
async function tempOutboxPath(): Promise<string> {
	return join(
		await mkdtemp(join(tmpdir(), "pdpp-auto-compact-")),
		"outbox.sqlite",
	);
}
...
test("PDPP_COLLECTOR_AUTO_COMPACT=0 disables the run-time compact", () => {
	for (const value of ["0", "false", "off", "no", "FALSE"]) {
		const policy = resolveCollectorAutoCompactPolicy(undefined, {
			PDPP_COLLECTOR_AUTO_COMPACT: value,
		});
...
test("auto-compact reclaims the freelist and shrinks the file on a bloated, quiet lane", async () => {
	...
	try {
		...
		assert.ok(
			before.reclaimableBytes > 1024 * 1024,
			"prune must leave >1 MiB reclaimable",
		);
		...
		assert.equal(
			outbox.pageStats().freelistPages,
			0,
			"freelist is emptied by the rebuild",
		);
...
test("auto-compact defers when the lane is not quiet (unsent work present)", async () => {
	...
	try {
		assert.ok(
			outbox.pageStats().reclaimableBytes > 1024 * 1024,
			"freelist is over threshold",
		);
		...
		assert.equal(
			statSync(path).size,
			sizeAfterPrune,
			"file must not change while work is unsent",
		);
		assert.ok(
			outbox.pageStats().freelistPages > 0,
			"freelist is preserved for a later quiet run",
		);
	} finally {
		outbox.close();
	}
...
test("auto-compact is a no-op when disabled, never opening a write transaction", async () => {
	...
	try {
		...
		assert.equal(
			statSync(path).size,
			sizeAfterPrune,
			"disabled policy must touch nothing",
		);
	} finally {
		outbox.close();
	}
