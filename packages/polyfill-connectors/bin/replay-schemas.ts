...
async function replayConnector(
	...
	const filtered = rows.filter(
		(r) => connectorIdToSlug(r.connector_id) === slug,
	);
...
function reportToSummary(report: ConnectorReport): ReplaySummary {
	return {
		...
		streams: Object.fromEntries(
			Object.entries(report.streams).map(([k, v]) => [
				k,
				{ total: v.total, passed: v.passed, failed: v.failed },
			]),
		),
	};
}

function writeAndPrintReport(
	...
	const reportPath = join(
		REPORT_DIR,
		`schema-replay-${report.connector}-${ts}.json`,
	);
	writeFileSync(reportPath, JSON.stringify(report, null, 2));
	console.log(
		`# ${report.connector}  ${totals.passed}/${totals.total} pass  (${totals.failed} fail)  → ${reportPath}`,
	);
