...
function normalizeType(schema: JsonSchema, path: string): NormalizedType {
	...
	if (type === undefined) {
		throw new Error(
			`missing "type" at ${path}: every schema node in the supported subset must declare a type`,
		);
	}
	...
	if (nonNull.length === 0) {
		throw new Error(
			`"type" at ${path} is only ["null"] — no non-null type to validate against`,
		);
	}
	if (nonNull.length > 1) {
		throw new Error(
...
function compileNode(schema: JsonSchema, path: string): string {
	...
	if (schema.enum !== undefined) {
		...
		const literals = (nonNullMembers as string[])
	...
	switch (primary) {
		...
		case "array": {
			if (!schema.items) {
				throw new Error(
					`array at ${path} has no "items": this spike requires typed array items`,
				);
			}
...
function generateForConnector(connector: string): void {
	...
	for (const stream of manifest.streams) {
		if (typeof stream.name !== "string" || stream.name.length === 0) {
			throw new Error(
				`${manifestPath}: a stream is missing a non-empty "name"`,
			);
		}
		if (!stream.schema) {
			throw new Error(
				`${manifestPath}: stream "${stream.name}" has no "schema"`,
			);
		}
		const schemaTypes = Array.isArray(stream.schema.type)
			? stream.schema.type
			: [stream.schema.type];
		if (!schemaTypes.includes("object")) {
			throw new Error(
				`${manifestPath}: stream "${stream.name}" schema root must be type "object"`,
			);
		}
...
`;
	...
	console.log(
		`wrote ${outPath} (${manifest.streams.length} stream schema(s), digest ${digest})`,
	);
}
...
if (connectors.length === 0) {
	console.error(
		"usage: tsx bin/generate-validators.ts <connector> [connector...]",
	);
	process.exit(2);
}
