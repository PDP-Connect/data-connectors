...
function usageAndExit(code: number): never {
	process.stderr.write(
		"Usage: observe-schema <connector> [--records <dir-or-file>...] [--manifest <path>]\n",
	);
	process.exit(code);
}
...
export function resolveStreamFiles(
	...
	for (const source of sources) {
		if (!existsSync(source)) {
			process.stderr.write(
				`[observe-schema] WARN: --records source not found: ${source}\n`,
			);
			continue;
		}
...
export function computeDivergences(
	...
	for (const [path, obs] of observed) {
		...
		const admitsIntegerAsNumber =
			admitted.has("number") && !admitted.has("integer");
		...
		if (nonAdmitted.length > 0) {
			const nonAdmittedSorted = nonAdmitted.toSorted((a, b) =>
				a.localeCompare(b),
			);
			const admittedSorted = [...admitted].toSorted((a, b) =>
				a.localeCompare(b),
			);
			divergences.push({
				kind: "type-mismatch",
	...
	for (const [path] of declared) {
		...
		if (!observedPaths.has(path)) {
			...
			divergences.push({
				kind: "unobserved-field",
				path,
				detail: "declared but never observed in any sample",
			});
		}
	}
...
function hasOpaqueDeclaredAncestor(
	path: string,
	declared: Map<string, DeclaredField>,
): boolean {
...
function collectEnumViolations(
	...
	for (const [path, decl] of enumPaths) {
		const allowed = new Set(
			(decl.enumValues ?? []).map((v) => JSON.stringify(v)),
		);
		const observation = fields.get(path);
		if (!observation) {
			continue;
...
function renderFieldTable(
	...
	lines.push(
		`  ${"field".padEnd(pathWidth)}  ${"type(s)".padEnd(24)}  presence`,
	);
	for (const path of sortedPaths) {
		...
		lines.push(
			`  ${path.padEnd(pathWidth)}  ${types.padEnd(24)}  ${presencePct(obs.count, sampleCount)}`,
		);
	}
	return lines.join("\n");
}
...
function renderDivergences(divergences: readonly Divergence[]): string {
	...
	return divergences
		.map((d) => `  [${order[d.kind]}] ${d.path}: ${d.detail}`)
		.join("\n");
}
...
function printStreamReport(
	...
	console.log(
		`  samples: ${records.length}${records.length >= SAMPLE_CAP ? " (capped)" : ""}`,
	);
...
function main(): void {
	...
	if (streams.length === 0) {
		console.log(
			`[observe-schema] connector "${args.connector}" declares no streams in its manifest`,
		);
	...
	const sources =
		args.recordSources.length > 0
			? args.recordSources
			: [defaultRecordsDir(args.connector)];
