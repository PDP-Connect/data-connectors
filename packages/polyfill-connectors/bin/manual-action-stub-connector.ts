...
function describeWsUrl(args: StreamingTargetRegisterArgs): string {
	return args.backend === "neko"
		? `neko:${JSON.stringify(args.descriptor)}`
		: args.wsUrl;
}
...
async function main(): Promise<void> {
	...
	process.stderr.write(
		`[stub] starting (stubOnly=${String(stubOnly)}, headless=${String(headless)})\n`,
	);
	process.stderr.write(
		`[stub] env PDPP_RUN_ID=${process.env.PDPP_RUN_ID ?? ""}\n`,
	);
	process.stderr.write(
		`[stub] env PDPP_REFERENCE_BASE_URL=${process.env.PDPP_REFERENCE_BASE_URL ?? ""}\n`,
	);
	process.stderr.write(
	...
	if (stubOnly) {
		registration = {
			...
			unregister: (args: UnregisterArgs): Promise<boolean> => {
				process.stderr.write(
					`[stub] unregister called: runId=${args.runId} interactionId=${args.interactionId}\n`,
				);
				return Promise.resolve(true);
			},
		};
		...
		if (!real) {
			process.stderr.write(
				"[stub] resolveStreamingRegistrationFromEnv returned undefined; aborting.\n",
			);
			process.exit(2);
		}
		...
		registration = {
			...
			unregister: async (args: UnregisterArgs): Promise<boolean> => {
				process.stderr.write(
					`[stub] unregister attempting: runId=${args.runId} interactionId=${args.interactionId}\n`,
				);
	...
	process.stderr.write(
		"[stub] browser acquired, sleeping 3s for steady state\n",
	);
