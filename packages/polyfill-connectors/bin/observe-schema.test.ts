...
test("observe-schema CLI: detects every divergence class against a crafted temp manifest + JSONL", () => {
	...
	try {
		...
		assert.equal(
			result.code,
			0,
			`observe-schema always exits 0; stderr=${result.stderr}`,
		);
		...
		assert.match(
			result.stdout,
			/\[ENUM-VIOLATION] status: observed value outside declared enum: "deleted"/,
		);
		// Observed-but-undeclared: extra_field never appears in the schema.
		assert.match(
			result.stdout,
			/\[OBSERVED-BUT-UNDECLARED] extra_field: observed but not declared in schema/,
		);
