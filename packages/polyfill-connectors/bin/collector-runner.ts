...
async function main(): Promise<void> {
	...
	if (options.command === "status" || options.command === "recover") {
		...
		const outboxPath = scopedDefaultQueuePath(
			options.queuePath,
			DEFAULT_QUEUE_PATH,
...
export function readOutboxStatus(
	...
	try {
		...
		const deadLetterErrors = outbox.deadLetterErrorSummary({
			sourceInstanceId,
		});
...
export function parseArgs(args: string[]): CliOptions {
	...
	if (
		...
		throw new Error(
			"usage: collector-runner <enroll|run|advertise|status|recover> --base-url <url> [options]",
		);
	}
...
function parseProtocolCapabilities(
	value: string,
	...
	return capabilities.filter(
		(capability): capability is ConnectorProtocolCapability =>
			CONNECTOR_PROTOCOL_CAPABILITIES.some((known) => known === capability),
	);
}
