	...
	return page.evaluate(
		(): FormStateSnapshot => ({
			...
			headings: [...document.querySelectorAll<HTMLElement>("h1, h2, h3")]
				...
				.filter(Boolean)
				.slice(0, 20),
			selects: [...document.querySelectorAll<HTMLSelectElement>("select")].map(
				(s) => ({
					name: s.name,
					id: s.id,
					options: [...s.options]
...
async function main(): Promise<void> {
	console.error(
		"[request-export] launching isolated patchright browser (profile=amazon)",
	);
	...
	try {
		...
		const postSnap: PostSnap = await page.evaluate((): PostSnap => {
			const buttonText = (
				b: HTMLButtonElement | HTMLInputElement | HTMLAnchorElement,
			): string => {
			...
			return {
				...
				headings: [...document.querySelectorAll<HTMLElement>("h1, h2, h3")]
					...
					.filter(Boolean)
					.slice(0, 20),
				forms: [...document.querySelectorAll<HTMLFormElement>("form")].map(
					(f) => ({
						id: f.id,
						action: f.action,
						method: f.method,
						buttons: [
							...f.querySelectorAll<HTMLButtonElement | HTMLInputElement>(
