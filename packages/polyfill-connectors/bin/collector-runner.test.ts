...
test("CLI local-agent defaults request safe inventory and coverage streams", () => {
	...
	assert(
		!claude.streams.includes("context_mode"),
		"Claude context_mode remains diagnostics-only",
	);
	assert(
		!codex.streams.includes("context_mode"),
		"Codex context_mode remains diagnostics-only",
	);
	assert(
		!codex.streams.includes("memories"),
		"Codex memories remain diagnostics-only",
	);
});

test("CLI run --connector google_takeout uses its own LocalCollectorDefinition streams, not a hand-maintained CLI table", () => {
	...
	// other registered local-collector connector failed with "run requires
	// --streams" before spawn, even though its own `LocalCollectorDefinition`
	// (src/collector-registry.ts) already declares a default stream set.
	// Reproduced pre-fix; this pins the fix.
	//
	// Upstream (pdpp ab415be6cc) pinned this with `signal`. This repo's
	// registry deliberately excludes signal until data-connect's
	// local-collector vendors it (see src/collector-registry.ts), so this
	// uses google_takeout instead — equally absent from the old hand-copied
	// table, so it exercises the same drift.
	const options = parseArgs([
		"run",
		"--base-url",
		"http://127.0.0.1:7662",
...
test("default collector queue path is scoped by connection id", () => {
	assert.equal(
		scopedDefaultQueuePath(
			"/tmp/collector-runner-queue.json",
			"/tmp/collector-runner-queue.json",
			"conn/a b",
		),
...
test("status: a dead-lettered record reports dead_letter lifecycle and names the error class", async () => {
	...
	seedOneDeadLetter(
		outboxPath,
		"conn-1",
		"boom: mime_type must be a valid media type",
	);
	const report = readOutboxStatus(outboxPath, "conn-1");
