...
const lastStatePathFor = (connector: string): string =>
	join(PACKAGE_ROOT, "runs", connector, "last-state.json");

/** Every connector id this suite runs the CLI against. `connector-dev.ts`
 *  writes two things under PACKAGE_ROOT keyed by connector id: each run's
 *  summary and committed cursor under `runs/<connector>/`, and — for a run
 *  with capture-on-failure retention and no `PDPP_CAPTURE_ROOT_DIR` override —
 *  its browser evidence under the package-local `fixtures/<connector>/`
 *  default. A suite that does not remove both leaves build-tree litter behind
 *  for whatever runs next; the stray `fixtures/**` JSON in particular is then
 *  reported by `npm run check`'s Biome pass, so `npm test` followed by
 *  `npm run verify` fails on artifacts rather than on code. Individual tests
 *  still clean up their own run directory where the assertion needs a
 *  known-empty state; this is the backstop for the ones whose assertions do
 *  not, and for any test that fails before reaching its own cleanup. */
const RUN_DIR_CONNECTORS = [
...
after(() => {
	for (const connector of RUN_DIR_CONNECTORS) {
		rmSync(join(PACKAGE_ROOT, "runs", connector), {
			force: true,
			recursive: true,
		});
	...
	// Remove the now-empty `runs/` parent, but only if this suite is what
	// created it — a non-empty directory here means a real developer run's
	// output is present, which is not this suite's to delete.
	try {
		rmdirSync(join(PACKAGE_ROOT, "runs"));
	} catch {
		// ENOTEMPTY (a developer's own run output) or ENOENT (never created).
	}
});
...
test("connector-dev CLI: succeeding fixture streams RECORD/PROGRESS/STATE lines, exits 0, writes a matching summary", () => {
	...
	try {
		...
		assert.match(
			result.stdout,
			/PROGRESS\s+\[items] collecting synthetic items/,
		);
		// STATE commit line.
		...
		assert.match(
			result.stdout,
			new RegExp(`summary written to: ${summaryPath}`),
		);
...
test("connector-dev CLI: failing fixture exits non-zero, prints the failure kind, and writes a failed summary", () => {
	...
	try {
		...
		assert.ok(
			...
			"summary file must still be written on failure",
		);
		const summary = JSON.parse(readFileSync(summaryPath, "utf8")) as RunSummary;
...
test("connector-dev CLI: default summary path is under runs/<connector>/ when --summary-out is omitted", () => {
	...
	assert.equal(result.code, 0, `expected exit 0; stderr=${result.stderr}`);
	const match =
		/summary written to: (.+runs\/connector-dev-cli-fixture\/.+-summary\.json)/.exec(
			result.stdout,
		);
	assert.ok(
		match,
		`expected default summary path in stdout; got: ${result.stdout}`,
	);
	const writtenPath = match?.[1]?.trim();
	assert.ok(
		writtenPath && existsSync(writtenPath),
		"default-path summary file must exist",
	);
	if (writtenPath) {
...
test("connector-dev CLI: --answer <index>=<value> completes an INTERACTION and the run succeeds", () => {
	...
	try {
		...
		assert.match(
			result.stdout,
			/PROMPT\s+needs otp: Enter the verification code/,
		);
		assert.match(result.stdout, /STATE\s+\[items] checkpoint committed/);
		assert.match(result.stdout, /STATUS succeeded/);
...
test("connector-dev CLI: no --answer and no TTY fails loudly, naming the unanswered prompt", () => {
	...
	try {
		...
		assert.notEqual(
			result.code,
			0,
			"an unanswered interaction with no TTY must fail non-zero",
		);
		assert.match(
			result.stdout,
			/PROMPT\s+needs otp: Enter the verification code/,
...
test("connector-dev CLI: a succeeded DONE followed by a nonzero exit is reported as a failure, not a success", () => {
	const tmpDir = mkdtempSync(
		join(tmpdir(), "connector-dev-done-then-exit1-test-"),
	);
	const summaryPath = join(tmpDir, "summary.json");
	try {
		const result = runCli([
		...
		assert.match(
			result.stdout,
			/FAILED\s+protocol_violation: nonzero_exit_after_done/,
		);
...
test("connector-dev CLI: --streams subsets START.scope — the fixture only sees and emits for the named streams", () => {
	...
	try {
		...
		assert.match(
			result.stdout,
			new RegExp(`START ${connector} — streams: items$`, "m"),
		);
		...
		assert.match(
			result.stdout,
			/PROGRESS\s+\[items] requested streams: items$/m,
		);
...
test("connector-dev CLI: --streams naming an unknown stream fails, listing the fixture's actual stream names", () => {
	...
	try {
		...
		assert.ok(
			!existsSync(summaryPath),
			"no summary should be written for a pre-flight arg failure",
		);
	} finally {
		rmSync(tmpDir, { recursive: true, force: true });
		rmSync(join(PACKAGE_ROOT, "runs", connector), {
...
test("connector-dev CLI: --seed-last-state round-trips a prior run's committed cursor into the next run's START.state", () => {
	...
	try {
		...
		assert.equal(
			result1.code,
			0,
			`run 1 failed; stdout=${result1.stdout} stderr=${result1.stderr}`,
		);
		assert.doesNotMatch(
			result1.stdout,
			/^SEEDED/m,
			"run 1 has no --seed-last-state, so no SEEDED line",
		);

		assert.ok(
			existsSync(lastStatePath),
			"last-state.json must exist after a DONE that emitted STATE",
		);
		const lastStateAfterRun1 = JSON.parse(
			readFileSync(lastStatePath, "utf8"),
		) as LastState;
		assert.equal(lastStateAfterRun1.connector, connector);
		assert.deepEqual(lastStateAfterRun1.state, {
			items: { seen: 1 },
			extras: { seen: 1 },
		});
		...
		assert.equal(
			result2.code,
			0,
			`run 2 failed; stdout=${result2.stdout} stderr=${result2.stderr}`,
		);
		assert.match(
			result2.stdout,
			new RegExp(
				`^SEEDED   state from ${lastStatePath.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")} \\(run of .+\\)$`,
				"m",
			),
		);

		const lastStateAfterRun2 = JSON.parse(
			readFileSync(lastStatePath, "utf8"),
		) as LastState;
		assert.deepEqual(lastStateAfterRun2.state, {
			items: { seen: 2 },
			extras: { seen: 2 },
		});

		const summary2 = JSON.parse(
			readFileSync(summaryPath2, "utf8"),
		) as RunSummary;
...
test("connector-dev CLI: --seed-last-state with no prior run fails clearly, naming the missing file", () => {
	...
	const tmpDir = mkdtempSync(
		join(tmpdir(), "connector-dev-seed-missing-test-"),
	);
	const summaryPath = join(tmpDir, "summary.json");
	const lastStatePath = lastStatePathFor(connector);
	assert.ok(
		!existsSync(lastStatePath),
		"precondition: no prior last-state.json for this fresh connector name",
	);
	try {
		const result = runCli([
			connector,
		...
		assert.notEqual(
			result.code,
			0,
			"--seed-last-state with no prior state must fail non-zero",
		);
		assert.match(
			result.stdout,
			new RegExp(
...
test("resolveCaptureOnFailureEnv: --no-capture disables regardless of environment", () => {
	...
	assert.equal(
		resolveCaptureOnFailureEnv(true, { PDPP_CAPTURE_ON_FAILURE: "1" }),
		undefined,
	);
});
...
test("resolveCaptureOnFailureEnv: an explicit 0 already in the environment is respected, not overridden", () => {
	assert.equal(
		resolveCaptureOnFailureEnv(false, { PDPP_CAPTURE_ON_FAILURE: "0" }),
		"0",
	);
});
...
test("resolveCaptureOnFailureEnv: an explicit 1 already in the environment stays 1", () => {
	assert.equal(
		resolveCaptureOnFailureEnv(false, { PDPP_CAPTURE_ON_FAILURE: "1" }),
		"1",
	);
});
...
test("connector-dev CLI: default run sets PDPP_CAPTURE_ON_FAILURE=1 for the subprocess and prints the policy line", () => {
	...
	assert.match(
		result.stdout,
		/^capture: on-failure \(default; --no-capture to disable\)$/m,
	);
...
test("connector-dev CLI: an explicit PDPP_CAPTURE_ON_FAILURE=0 already in the environment is passed through untouched", () => {
	...
	assert.match(
		result.stdout,
		/^capture: disabled \(PDPP_CAPTURE_ON_FAILURE=0 already set in environment\)$/m,
	);
...
test("classifyFailureEnvironment: a real-origin checkpoint with a degenerate-small DOM -> viewport_zero", () => {
	...
	assert.deepEqual(classifyFailureEnvironment(checkpoints), {
		kind: "viewport_zero",
	});
});
...
test("connector-dev CLI: a failure with capture disabled prints evidence:none and does not invent a diagnosis", () => {
	...
	assert.match(
		result.stdout,
		/evidence: none retained for this run \(capture disabled/,
	);
...
test("connector-dev CLI: a failure with capture enabled but no browser checkpoints prints the evidence dir and no false classification", () => {
	const tmpCaptureRoot = mkdtempSync(
		join(tmpdir(), "connector-dev-capture-root-"),
	);
	try {
		const result = spawnSync(
			process.execPath,
		...
		assert.match(
			result.stdout,
			new RegExp(
				`evidence: ${tmpCaptureRoot.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`,
			),
		);
