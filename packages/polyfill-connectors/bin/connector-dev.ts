...
const VALUE_FLAG_CONSUMERS: Record<
	string,
	(argv: readonly string[], i: number, into: MutableCliArgs) => number
> = {
	"--summary-out": (argv, i, into) =>
		consumeValueFlag(argv, i, into, "summaryOut"),
...
export function buildPreAnsweredResponse(
	requestId: string,
	value: string,
): InteractionResponse {
	return {
 ...
 */
export function buildUnansweredResponse(
	requestId: string,
	message: string,
): InteractionResponse {
	return {
...
export function buildStartScope(streams: readonly ManifestStream[]): {
	streams: StreamScope[];
} {
	return {
		streams: streams.map((stream): StreamScope => ({ name: stream.name })),
	};
}
 ...
 */
export function finalStateFromMessages(
 ...
 *  checkpoint. */
export function writeLastState(
	connector: string,
	startedAt: string,
	state: Record<string, unknown>,
): void {
...
function printCapturePolicyNote(resolvedValue: string | undefined): void {
	...
	if (resolvedValue === "0") {
		printLine(
			"capture: disabled (PDPP_CAPTURE_ON_FAILURE=0 already set in environment)",
		);
		return;
	}
...
function renderMessage(message: EmittedMessage): void {
	switch (message.type) {
		...
		case "PROGRESS": {
			...
			const countTag =
				message.count === undefined
					? ""
					: ` (${message.count}${message.total ? `/${message.total}` : ""})`;
		...
		case "DETAIL_GAP": {
			printLine(
				`  WARN     [${message.stream}] detail gap: ${message.reason} (record ${String(message.record_key)})`,
			);
			return;
		}
		...
		case "ASSISTANCE": {
			printLine(
				`  PROMPT   assistance needed (${message.owner_action}): ${message.message}`,
			);
			return;
		}
		case "ASSISTANCE_STATUS": {
			printLine(
				`  PROMPT   assistance ${message.status}${message.message ? `: ${message.message}` : ""}`,
			);
			return;
		}
		case "DONE": {
 ...
 * settled Promise.
 */
export function runAndStream(
	entrypoint: string,
	start: object,
	options: RunAndStreamOptions = {},
): Promise<RunResult> {
	...
	return new Promise((resolvePromise, rejectPromise) => {
		const child = spawn(process.execPath, ["--import", "tsx", entrypoint], {
			...
			env: {
				...
				...(options.captureOnFailure === undefined
					? {}
					: { PDPP_CAPTURE_ON_FAILURE: options.captureOnFailure }),
			},
		...
		const answerInteraction = (
			parsed: Extract<EmittedMessage, { type: "INTERACTION" }>,
		): void => {
			...
			if (preAnswered !== undefined) {
				child.stdin?.write(
					stringifyForJsonl(
						buildPreAnsweredResponse(parsed.request_id, preAnswered),
					),
				);
				return;
			}
			if (!isTty) {
				printLine(
					`  PROMPT   FAILED (no --answer, no TTY): ${parsed.kind} — ${parsed.message}`,
				);
				...
				child.stdin?.write(
					stringifyForJsonl(
						buildUnansweredResponse(parsed.request_id, parsed.message),
					),
				);
				return;
			}
			const schema: InteractionMessage["schema"] = parsed.schema as
		...
		const parseLine = (line: string): void => {
			...
			try {
				...
				finish(() =>
					rejectPromise(
						new Error(
							`connector emitted invalid JSONL: ${reason}; line=${line}`,
						),
					),
				);
				return;
		...
		child.on("close", (code, signal) => {
			...
			const succeededDone = messages.find(
			...
			const nonzeroExitAfterSucceededDone =
				Boolean(succeededDone) && (code !== 0 || Boolean(signal));
...
function printBrowserRendererNote(env: NodeJS.ProcessEnv): void {
	...
	if (hasDisplay) {
		if (headlessOverride === "1") {
			...
			printLine(
				"BROWSER  DISPLAY detected — browser-class connectors will launch headed (visible) by default.",
			);
		}
		return;
	}
...
function printRunSummaryBody(summary: RunSummary, outPath: string): void {
	...
	for (const [name, streamSummary] of streamEntries) {
		const took =
			streamSummary.elapsed_ms === undefined
				? ""
				: `  took=${formatElapsed(streamSummary.elapsed_ms)}`;
		printLine(
	...
	if (summary.done.coverage) {
		printLine(
			`  coverage: considered=${summary.done.coverage.considered} covered=${summary.done.coverage.covered}`,
		);
	}
	if (summary.done.latest_record_emitted_at) {
		printLine(
			`  latest_record_emitted_at: ${summary.done.latest_record_emitted_at}`,
		);
	}
...
async function main(): Promise<void> {
	...
	printLine(
		`START ${connector} — streams: ${streams.map((s) => s.name).join(", ") || "(none declared)"}`,
	);
	printBrowserRendererNote(process.env);
	const captureOnFailure = resolveAndAnnounceCapturePolicy(
		args.noCapture,
		process.env,
	);
	...
	const outPath = summaryOut
		? resolve(summaryOut)
		: defaultSummaryPath(connector, startedAt);
	...
	if (result.unansweredInteractions.length > 0) {
		for (const unanswered of result.unansweredInteractions) {
			printLine(
				`  unanswered prompt: ${unanswered.kind} — ${unanswered.message} (request_id=${unanswered.requestId})`,
			);
		}
	}
...
function printFailureDiagnostics(
	...
	const captureDir =
		captureOnFailure === undefined
			? undefined
			: findLatestCaptureDir(connector, env);
 ...
 */
export function findLatestCaptureDir(
	connector: string,
	env: Record<string, string | undefined>,
): string | undefined {
...
export function readCheckpointEvidence(
	captureDir: string,
): CheckpointEvidence[] {
	...
	for (const file of files) {
		...
		const domFile = join(
			captureDir,
			"dom",
			file.replace(JSON_EXTENSION_RE, ".html"),
		);
...
export type FailureEnvironmentClassification =
...
export function classifyFailureEnvironment(
	...
	const hasRealOrigin = checkpoints.some(
		(c) => c.url !== undefined && c.url !== "about:blank" && c.url !== "",
	);
...
function environmentClassificationLine(
	classification: FailureEnvironmentClassification,
): string {
...
function diagnosticNextSteps(captureDir: string | undefined): string[] {
	...
	if (failingCheckpoint) {
		const domPart =
			failingCheckpoint.domBytes === undefined
				? "unknown-byte"
				: `${failingCheckpoint.domBytes}-byte`;
		lines.push(
