...
test("same-version GitHub PDPP maintained source drift fails check mode", () => {
  ...
  try {
    ...
    assert.match(
      `${result.stdout}\n${result.stderr}`,
      /github-pdpp@0\.5\.2 maintained source changed without a version bump: src\/connector\/index\.ts/,
    );
  } finally {
...
test("same-version ChatGPT PDPP artifact configuration drift fails check mode", () => {
  ...
  try {
    ...
    assert.match(
      `${result.stdout}\n${result.stderr}`,
      /chatgpt-pdpp@0\.1\.2 maintained source changed without a version bump: artifact\.json/,
    );
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
...
test("same-version GitHub PDPP provenance drift fails check mode", () => {
  ...
  try {
    ...
    assert.match(
      `${result.stdout}\n${result.stderr}`,
      /github-pdpp@0\.5\.2 source changed without a version bump/,
    );
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
...
test("release-assets refresh preserves every PDPP artifact when generated dist is absent", () => {
  ...
  try {
    ...
    assert.equal(
      pdpp.artifactUrl,
      "https://github.com/PDP-Connect/data-connectors/releases/download/connectors-release-test/github-pdpp-0.5.2.tgz",
    );
    assert.equal(
      pdpp.artifactSha256,
    ...
    assert.equal(
      chatgpt.artifactUrl,
      "https://github.com/PDP-Connect/data-connectors/releases/download/connectors-release-test/chatgpt-pdpp-0.1.2.tgz",
    );
