...
| claude_code | `claude_code.prompts` | The prompts you typed into Claude Code, with timestamp and project, derived from your local command history. Pasted attachment bodies are excluded. | [JSON Schema](connectors/anthropic/schemas/claude_code.prompts.json) | None | claude-code-local (experimental) |
| claude_code | `claude_code.sessions` | Per-session metadata across all of your Claude Code projects: project path, git branch, CLI version, start/end time, message counts, models used, and the session title. Message bodies are not exported. | [JSON Schema](connectors/anthropic/schemas/claude_code.sessions.json) | None | claude-code-local (experimental) |
| claude_code | `claude_code.trajectories` | The full step-by-step trace of each Claude Code session: your prompts, the assistant's reasoning and replies, every tool call with its input, and every tool result with its output, in order. This includes source code, file paths, and command output from your machine. Credential-shaped strings are redacted and long step bodies are truncated. | [JSON Schema](connectors/anthropic/schemas/claude_code.trajectories.json) | None | claude-code-local (experimental) |
| claude_code | `claude_code.usage` | Aggregate Claude Code usage: total sessions and messages, per-model token consumption and cost, day-by-day activity, hour-of-day distribution, and your longest session. | [JSON Schema](connectors/anthropic/schemas/claude_code.usage.json) | None | claude-code-local (experimental) |
