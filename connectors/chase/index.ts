...
import { ensureChaseSession } from "../../packages/polyfill-connectors/src/auto-login/chase.ts";
import {
	attachBodyResponseQueue,
	type BodyResponseQueue,
	isLikelyPdfResponseBody as isLikelyPdfResponseBodyShared,
} from "../../packages/polyfill-connectors/src/browser-artifact-response.ts";
import {
...
import type {
	CaptureSession,
	LocatorProbe,
} from "../../packages/polyfill-connectors/src/fixture-capture.ts";
...
} from "../../packages/polyfill-connectors/src/statement-content-fingerprint.ts";
import {
	isHydrated,
...
} from "../../packages/polyfill-connectors/src/statement-hydration-carry-forward.ts";
import {
	ACTIVITY_LABELS,
	accountSlug,
	chooseActivity,
...
const DOWNLOAD_RESPONSE_HINT_RE =
	/filename|attachment|octet-stream|x-ofx|qfx/iu;
const CHASE_DOWNLOAD_ROUTE_RE =
	/downloadAccountTransactions|confirmDownloadAccountActivity/iu;
const NO_ACTIVITY_CONFIRMATION_RE =
	/we couldn't find any activity that matched the date range you chose/iu;
...
const CHASE_CURRENT_ACTIVITY_TABLE_MARKER_RE =
	/<table\b[^>]*\bclass=["'][^"']*\bmds-activity-table\b[^"']*["']/giu;
const CHASE_DASHBOARD_MARKER_RE =
	/\baccounts-name-link-button-[a-z0-9_-]+\b/giu;
...
export const CHASE_QFX_FILE_TYPE_SELECT_SELECTOR =
 ...
 */
export function classifyChaseAccountsSurface(
	diagnostics: DashboardDiagnostics | null,
): ChaseAccountsSurface {
	if (!diagnostics) {
		return "unknown";
	}
	try {
...
export function chaseNoAccountsDiagnosticMessage(
	surface: ChaseAccountsSurface,
): string {
...
export function redactChaseDashboardDiagnostics(
	diagnostics: DashboardDiagnostics | null,
): DashboardDiagnostics | null {
	return diagnostics
		? {
				body_preview: "",
				income_capture_description_count:
					diagnostics.income_capture_description_count,
...
export function classifyChaseCurrentActivityRoute(
	value: string,
): BrowserSurfaceDiagnostic["route"] {
	try {
...
function chaseCurrentActivityDiagnostic(
	...
	if (!diagnostic) {
		throw new Error(
			"closed Chase browser-surface diagnostic input was invalid",
		);
	}
	return diagnostic;
}
...
export async function snapshotDashboardHtmlForCurrentActivity(
	...
	const immediateParserCount = parseCurrentActivityDom(
		immediateHtml,
		referenceDateIso,
	).length;
	const immediateDashboardMarkerCount = countStructuralMarker(
		immediateHtml,
		CHASE_DASHBOARD_MARKER_RE,
	);
	const immediateTableMarkerCount = countStructuralMarker(
		immediateHtml,
		CHASE_CURRENT_ACTIVITY_TABLE_MARKER_RE,
	);
	...
	let dashboardMarkerCount = countStructuralMarker(
		finalHtml,
		CHASE_DASHBOARD_MARKER_RE,
	);
	let activityTableMarkerCount = countStructuralMarker(
		finalHtml,
		CHASE_CURRENT_ACTIVITY_TABLE_MARKER_RE,
	);
	let readCount = 2;
	...
	if (parserCount === 0 && page.goto) {
		...
		dashboardMarkerCount = countStructuralMarker(
			finalHtml,
			CHASE_DASHBOARD_MARKER_RE,
		);
		activityTableMarkerCount = countStructuralMarker(
			finalHtml,
			CHASE_CURRENT_ACTIVITY_TABLE_MARKER_RE,
		);
		readCount = 3;
...
async function clickFileTypeControl(page: Page): Promise<void> {
	try {
		await page
			.locator(CHASE_QFX_FILE_TYPE_SELECT_SELECTOR)
			.first()
			.click({ timeout: CLICK_TIMEOUT_MS });
	} catch (selectorErr) {
		try {
...
function isLikelyQfxResponseBody(
	body: Buffer,
...
export function isLikelyChaseQfxResponse(
	headers: Record<string, string>,
	url = "",
): boolean {
	const hint = `${headers["content-disposition"] ?? ""} ${headers["content-type"] ?? ""} ${url}`;
	return (
		DOWNLOAD_RESPONSE_HINT_RE.test(hint) || CHASE_DOWNLOAD_ROUTE_RE.test(url)
	);
}
...
async function waitForQfxDownloadArtifact(
	...
	try {
		...
		const result = await Promise.any([
			...
			qfxResponsePromise,
			waitForNoActivityConfirmation(page, {
		...
		if (result.kind === "no_activity") {
			...
			await capturePageCheckpoint(
				capture,
				page,
				`download-qfx-${account.internal_id}-${activity}-no-activity`,
			);
		...
		if (result.kind === "playwright_download") {
			try {
				...
				capture?.captureHttp(
					`download-qfx-${account.internal_id}-${activity}-qfx-response-after-download-save-failed`,
					{
						...
						downloadError: truncate(
							errMessage(downloadErr),
							ERROR_MESSAGE_SLICE_LONG,
						),
			...
			capture?.captureHttp(
				...
				{
					bytes: result.response.body.length,
...
async function waitForNoActivityConfirmation(
	...
	await page.waitForFunction(
		(messagePattern) => {
			...
			return (
				location.href.includes("confirmDownloadAccountActivity") &&
				new RegExp(messagePattern, "iu").test(bodyText)
			);
		},
...
async function downloadQfx(
	...
	await capturePageCheckpoint(
		capture,
		page,
		`download-qfx-${account.internal_id}-${activity}-form-loaded`,
	);
	...
	if (activity !== "current") {
		try {
			...
			await capturePageCheckpoint(
				capture,
				page,
				`download-qfx-${account.internal_id}-${activity}-activity-selected`,
			);
		} catch (err) {
			await capturePageCheckpoint(
				capture,
		...
		if (activity === "date_range") {
			...
			await capturePageCheckpoint(
				capture,
				page,
				`download-qfx-${account.internal_id}-${activity}-date-range-filled`,
			);
		}
	}
	...
	try {
		...
		await capturePageCheckpoint(
			capture,
			page,
			`download-qfx-${account.internal_id}-${activity}-file-type-selected`,
		);
	} catch (err) {
		await capturePageCheckpoint(
			capture,
	...
	try {
		...
		await capturePageCheckpoint(
			capture,
			page,
			`download-qfx-${account.internal_id}-${activity}-download-click-failed`,
		);
	...
	try {
		return await waitForQfxDownloadArtifact(
			page,
			account,
			activity,
			tmpDir,
...
const STATEMENT_ROOT = resolveConnectorArtifactDir("chase", [
	"statements",
]).root;
...
async function downloadStatementPdf(
	...
	if (!exists) {
		await capturePageCheckpoint(
			capture,
			page,
			`statement-${row.rowAnchorId}-anchor-not-found`,
		);
	...
	await capturePageCheckpoint(
		capture,
		page,
		`statement-${row.rowAnchorId}-before-download-click`,
	);
	const downloadQueue = attachDownloadQueue(page);
	...
	try {
		...
		await capturePageCheckpoint(
			capture,
			page,
			`statement-${row.rowAnchorId}-download-click-failed`,
		);
		downloadQueue.detach();
	...
	let result: Awaited<
		typeof playwrightDownloadPromise | typeof pdfResponsePromise
	>;
	try {
		...
		await capturePageCheckpoint(
			capture,
			page,
			`statement-${row.rowAnchorId}-download-event-timeout`,
		);
		captureBodyResponseDiagnostics(
			capture,
			page,
	...
	try {
		if (result.kind === "pdf_response") {
			...
			try {
				...
				await rm(tmpPdfDir, { recursive: true, force: true }).catch(
					(): undefined => undefined,
				);
			}
		}
		...
		await capturePageCheckpoint(
			capture,
			page,
			`statement-${row.rowAnchorId}-download-save-failed`,
		);
		captureBodyResponseDiagnostics(
			capture,
			page,
	...
	if (!isUsablePdfBuffer(buffer)) {
		await capturePageCheckpoint(
			capture,
			page,
			`statement-${row.rowAnchorId}-download-empty-pdf`,
		);
...
export function chaseLocatorProbesForLabel(
	label: string,
): readonly LocatorProbe[] {
	const probes: LocatorProbe[] = [];
	if (label.includes("dashboard")) {
		probes.push({
			description:
				"Structural account affordance currently used to discover Chase account ids.",
	...
	if (label.includes("income-capture-interstitial")) {
		probes.push(
			{
				description:
					"Observed authenticated Chase income-capture heading; this is not an account surface.",
				exact: true,
			...
			{
				description:
					"Observed income-capture explanatory text; no action selector was retained in the evidence.",
				exact: true,
	...
	if (label.includes("download-qfx")) {
		probes.push(
			...
			{
				description:
					"Whether the activity select has a stable accessible combobox name.",
			...
			{
				description:
					"Whether the file-type select has a stable accessible combobox name.",
			...
			{
				description:
					"Structural download button currently used for the QFX click.",
	...
	if (label.includes("statement") || label.includes("statements-list")) {
		probes.push(
			...
			{
				description:
					"Statement download anchors currently used for PDF hydration.",
 ...
 */
export function readPriorCurrentActivityFingerprints(
	state: Record<string, unknown>,
): Map<string, string> {
...
export function readPriorTransactionFingerprints(
	...
	const streamState = (state.transactions ?? state ?? {}) as Record<
		string,
		unknown
	>;
	const raw = streamState.fingerprints;
...
export async function emitStatementDetailCoverage(
	...
	const hydratedKeys = outcomes
		.filter((outcome) => outcome.kind === "hydrated")
 ...
 */
export async function emitTransactionsStateIfAny(
	deps: EmitDeps,
): Promise<void> {
	if (
		!(deps.wantsTransactions && Object.keys(deps.maxSeenByAccount).length > 0)
	) {
		return;
	}
	const cursor: Record<string, unknown> = {
		per_account: deps.maxSeenByAccount,
	};
...
export async function emitBalancesDetailCoverage(
	...
	const { requiredKeys, hydratedKeys, gapKeys } =
		accountDetailCoverageKeys(outcomes);
	await deps.emit(
		buildDetailCoverageMessage({
			stream: "balances",
			stateStream: "accounts",
...
function readDashboardDiagnostics(
	...
	return page
		.evaluate(
			({ description, heading }): DashboardDiagnostics => {
				...
				return {
					...
					body_preview: (document.body?.innerText || "")
						.replace(WS, " ")
						.slice(0, 500),
					income_capture_heading_count: countExactText(
						"h1, h2, h3, h4, h5, h6",
						heading,
					),
			...
			{
				description: CHASE_INCOME_CAPTURE_DESCRIPTION,
				heading: CHASE_INCOME_CAPTURE_HEADING,
			},
		)
		.catch((): DashboardDiagnostics | null => null);
}

async function emitNoAccountsDiagnostic(
	diagnostics: DashboardDiagnostics | null,
	emit: EmitFn,
): Promise<void> {
...
export async function collectChaseAccountInventory({
	...
	emit,
	page,
}: Pick<BrowserCollectContext, "capture" | "emit" | "page">): Promise<
	ChaseAccount[]
> {
	...
	if (accounts.length === 0) {
		...
		await capturePageCheckpoint(
			capture,
			page,
			surface === "income_capture_interstitial"
...
function accountProgressLabel(accountProgress?: {
	index: number;
	total: number;
}): string {
	return accountProgress
		? `${accountProgress.index}/${accountProgress.total}`
		: "?/?";
}

function accountProgressDiagnostic(accountProgress?: {
	index: number;
	total: number;
}): {
...
export function classifyNoActivityOutcome(
	accountId: string,
	activity: ActivityKind,
): AccountDetailOutcome {
	if (activity === "all") {
		return {
			kind: "gap",
			accountId,
...
export async function runCurrentActivity(
	...
	const emitted = await emitCurrentActivityForAccount(
		deps,
		account,
		dashboardHtml,
		fingerprintCursor,
	...
	if (emitted === 0) {
		...
		await deps.emit({
			...
			diagnostics: {
				account_count: 1,
				...(surfaceDiagnostic ? { browser_surface: surfaceDiagnostic } : {}),
			},
		});
	} else {
		await deps.emit({
...
async function processStatementRow(
	...
	try {
		...
		const dlResult = await downloadStatementPdf(
			page,
			row,
			accountId,
			deps.capture,
		...
		await deps.emit({
			...
			diagnostics: buildChaseDurableFailureDiagnostic(
				"statement",
				"row_exception",
				rowErr,
			),
			...
			try {
				...
				if (droppedAccountCursors.length > 0) {
					...
					await emit({
						...
						message:
							"An account with a saved position is no longer listed on the dashboard",
						diagnostics: {
							dropped_account_count: droppedAccountCursors.length,
						},
					});
				...
				if (deps.wantsAccounts) {
					const accountsFingerprintCursor = openFingerprintCursor(
						startState.accounts,
						{
							excludeFromFingerprint: ["fetched_at"],
				...
				if (deps.wantsStatements) {
					const statementsFingerprintCursor = openFingerprintCursor(
						startState.statements,
						{
