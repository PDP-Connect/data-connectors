...
function stateWithCursor(maxSeenDate: string): TransactionsStateShape {
	return {
		per_account: {
			[ACCOUNT]: { max_seen_date: maxSeenDate } as TransactionCursor,
		},
	};
}
...
test("chooseActivity honours an explicit time_range over any cursor", () => {
	const requested = new Map([
		[
			"transactions",
			{ time_range: { since: "2026-05-01", until: "2026-07-31" } },
		],
	]);
	const choice = chooseActivity(
		requested,
		stateWithCursor("2026-08-19"),
		"transactions",
		ACCOUNT,
		RUN_DATE,
	);
...
test("prunePerAccountCursors drops cursors for undiscovered accounts", () => {
	const { kept, dropped } = prunePerAccountCursors(
		CURSORS,
		new Set(["acct-1", "acct-2"]),
	);
...
test("prunePerAccountCursors keeps every cursor when all accounts are present", () => {
	const { kept, dropped } = prunePerAccountCursors(
		CURSORS,
		new Set(["acct-1", "acct-2", "acct-gone"]),
	);
