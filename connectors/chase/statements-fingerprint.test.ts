...
test("statements: re-emitting with only a new fetched_at is fully suppressed", async () => {
	...
	assert.equal(
		run2.emitted.length,
		0,
		"unchanged statement fully suppressed despite a new fetched_at",
	);
});
...
test("statements: a changed statement field re-emits", async () => {
	...
	await emitOneStatement(
		...
		run1.deps.emit,
		"S1",
		makeRow({ title: "April 2026 Statement" }),
		cursor1,
	);
	...
	assert.equal(
		run2.emitted.length,
		1,
		"a retitled statement is a real change and re-emits",
	);
});
...
test("AC-1: previously hydrated + transient download failure carries pointers forward (no flap)", async () => {
	...
	const carried = readPriorStatementHydration(
		nextStateFrom(b.messages).statements,
	);
	assert.equal(
		carried.get(STMT_ID)?.pdf_path,
		PDF,
...
test("AC-2: never-hydrated failure stays all-null (honest), and the body has null pointers", async () => {
	...
	assert.equal(
		readPriorStatementHydration(nextStateFrom(run.messages).statements).size,
		0,
		"nothing to carry",
	);
});
...
test("AC-6: a delisted statement is pruned, never carried forward as a phantom", async () => {
	...
	assert.equal(
		carried.has("S2"),
		false,
		"a delisted statement is pruned, never carried as a phantom",
	);
	assert.equal(
		carried.has("S1"),
		true,
