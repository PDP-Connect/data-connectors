...
test("statement enumeration failure is not an empty complete run and preserves cursors", async () => {
	...
	const hydration = openStatementHydrationCursor(
		new Map([["S1", PRIOR_HYDRATION]]),
	);

	await runStatements(
		deps,
		fakeStatementsPage(async () =>
			Promise.reject(new Error("fixture DOM failure")),
		),
		[],
		[],
		null,
	...
	assert.ok(
		skip,
		"statement scrape failure must surface through Chase's own failure path",
	);
	...
	assert.equal(
		messages.some(
			(message) => message.type === "STATE" && message.stream === "statements",
		),
		false,
	...
	assert.equal(
		fingerprints.size(),
		1,
		"failed enumeration must not prune hydration fingerprints",
	);
	assert.equal(
		hydration.size(),
		1,
		"failed enumeration must not prune hydration pointers",
	);
...
test("healthy empty statement enumeration still reports measured complete 0/0", async () => {
	...
	await runStatements(
		...
		fakeStatementsPage(
			async () =>
				"<!doctype html><html><body><div class=documents-page></div></body></html>",
		),
		[],
		[],
		null,
	...
	assert.equal(
		fingerprints.size(),
		0,
		"a successful full scan may prune a stale fingerprint",
	);
	assert.equal(
		hydration.size(),
		0,
		"a successful full scan may prune stale hydration",
	);
