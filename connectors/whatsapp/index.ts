...
function isSupportedExportFile(fileName: string): boolean {
	...
	return SUPPORTED_EXPORT_EXTENSIONS.some((extension) =>
...
class AttachmentMessageLinkIndex {
	...
	constructor(attachments: readonly ParsedWhatsAppAttachment[]) {
		this.attachmentFilenamesLower = attachments.map((a) =>
	...
	observe(message: ParsedWhatsAppMessage): void {
		...
		for (const filenameLower of this.attachmentFilenamesLower) {
			if (
				!this.byFilenameLower.has(filenameLower) &&
				contentLower.includes(filenameLower)
			) {
...
async function emitMessageRecords(
	...
	await streamWhatsAppChatMessagesAsync(lines, chatId, async (m) => {
		...
		if (processed % MESSAGE_PROGRESS_INTERVAL === 0) {
			await progress(
				`Processed ${processed} WhatsApp messages from export ${exportOrdinal} of ${exportTotal}.`,
				{
					count: processed,
					stream: "messages",
					total: processed,
...
function testOnlyTxtToctouBarrier(): void {
	...
	while (!existsSync(releasePath)) {
		if (Date.now() >= deadline) {
			throw new Error(
				"timed out waiting for the WhatsApp .txt TOCTOU test barrier release",
			);
		}
...
async function parseTxtExportFile(fileName: string): Promise<TxtParseOutcome> {
	...
	try {
		const summary = await scanWhatsAppChatIdentityStream(
			basename(fileName),
			sniffedLines(pinnedFdLines(fd)),
		);
		...
		return {
			...
			source: {
				...
				linesForPass: () => {
					...
					if (
						current.size !== identitySnapshot.size ||
						current.mtimeMs !== identitySnapshot.mtimeMs
					) {
...
async function parseZipExportOutcome(
	...
	const zipResult = await parseZipExportFile(
		fileName,
		emit,
		exportOrdinal,
		exportTotal,
	...
	try {
		summary = scanWhatsAppChatIdentity(
			zipResult.source.fileTitle,
			zipResult.source.linesForPass() as Iterable<string>,
		);
	} catch (err) {
	...
	return {
		closeSource: zipResult.closeSource,
		result: { source: zipResult.source, summary },
	};
}
...
async function parseTxtExportOutcome(
	...
	return {
		closeSource: noopClose,
		result: { source: txtOutcome.source, summary: txtOutcome.summary },
	};
}
...
async function parseExportFile(
	...
	await progress(
		`Reading WhatsApp export ${exportOrdinal} of ${exportTotal}.`,
		{
			count: exportOrdinal,
			total: exportTotal,
		},
...
async function emitAttachmentRecords(
	...
	for (let index = 0; index < attachments.length; index += 1) {
		...
		if (
			processed % ATTACHMENT_PROGRESS_INTERVAL === 0 ||
	...
	return {
		covered: attachments.length - skipped,
		emitted,
		processed: attachments.length,
	};
}
...
function decodeAliasList(raw: unknown): ChatIdentityAlias[] {
	...
	for (const entry of raw) {
		if (
			...
			Array.isArray(
				(entry as { fingerprint_sample?: unknown }).fingerprint_sample,
			)
		) {
			aliases.push({
				chatId: (entry as { chat_id: string }).chat_id,
				fingerprintSample: (
					entry as { fingerprint_sample: unknown[] }
				).fingerprint_sample.filter((f): f is string => typeof f === "string"),
			});
		}
	}
	return aliases;
...
function mergeAliasMapFrom(
	map: Map<string, ChatIdentityAlias[]>,
	cursorState: unknown,
): void {
	if (!cursorState || typeof cursorState !== "object") {
		return;
	}
	const chatIdentity = (cursorState as { chat_identity?: unknown })
		.chat_identity;
	...
	for (const [identityKey, aliasListRaw] of Object.entries(
 ...
 */
function decodeAliasesByIdentityKey(
	state: Record<string, unknown>,
): Map<string, ChatIdentityAlias[]> {
...
export function openChatIdentityCursor(
	state: Record<string, unknown>,
): ChatIdentityCursor {
	...
	return {
		resolve(identityKey, provisionalChatId, messageFingerprints) {
			...
			const matched = existingAliases.find((alias) =>
				alias.fingerprintSample.some((f) => incomingSet.has(f)),
			);
			if (matched) {
				...
				const grownAliases = existingAliases.map((alias) =>
					alias.chatId === matched.chatId
						? { chatId: alias.chatId, fingerprintSample: grownSample }
						: alias,
				);
			...
			const newChatId =
				existingAliases.length === 0
					? provisionalChatId
					: mintChatId(identityKey, String(existingAliases.length));
			aliasesByKey.set(identityKey, [
				...existingAliases,
				{
					chatId: newChatId,
					fingerprintSample: sampleUniform(
						messageFingerprints,
						MESSAGE_FINGERPRINT_SAMPLE_SIZE,
					),
				},
...
async function discoverImportFilesOrThrow(
	importDir: string,
): Promise<string[]> {
	try {
...
async function emitNoExports(
	emit: EmitEvent,
	skipStream: string,
): Promise<void> {
	await emit({
		message:
			"No WhatsApp .txt or .zip exports are available for this source. Add an export and run again.",
...
async function emitParsedExport(
	...
	if (requested.has("chats")) {
		...
		records += 1;
		await progress(
			`Imported chat metadata for WhatsApp export ${exportOrdinal} of ${exportTotal}.`,
			{
				count: exportOrdinal,
				stream: "chats",
				total: exportTotal,
...
function pruneRequestedCursors(
	requested: RequestedStreams,
	cursors: WhatsAppCursors,
): void {
...
async function emitRequestedState(
	...
	for (const [stream, cursor] of Object.entries(cursors)) {
		...
		if (stream === "messages") {
			await emitStateForCursor(stream, cursor, emit, {
				chat_identity: identityCursor.toState(),
			});
	...
	if (!identityStateWritten) {
		...
		const fallbackStream = STREAM_PRIORITY.find((stream) =>
...
runConnector({
	...
	async collect({ requested, state, emit, emitRecord, progress }) {
		...
		await progress(
			`Found ${files.length} WhatsApp export file(s) to inspect.`,
			{
				count: files.length,
				total: files.length,
			},
		...
		for (let index = 0; index < files.length; index += 1) {
			...
			const { closeSource, result } = await parseExportFile(
				f,
				emit,
				progress,
				exportOrdinal,
