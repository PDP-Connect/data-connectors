...
import { steamPacingProfile } from "../../packages/polyfill-connectors/src/provider-profile.ts";
...
function steamDeclaredTotal(
	...
	if (typeof value !== "number" || !Number.isInteger(value) || value < 0) {
		throw new Error(
			`steam_response_malformed: ${field} must be a nonnegative integer`,
		);
	}
 ...
 */
export const STEAM_RETRYABLE_PATTERN =
	/ECONN|ETIMEDOUT|fetch failed|steam_rate_limited|retryable status \d+/i;
 ...
 */
export function classifySteamHttpResponse(
	status: number,
	_body: string,
): SteamHttpClassification {
	if (status === 401) {
	...
	if (status === 403) {
		return {
			kind: "error",
			message: "steam_forbidden_auth_or_visibility",
			status,
		};
	}
	if (status === 429) {
...
async function steamApiRequest<T>(
	...
	try {
		...
		const classification = classifySteamHttpResponse(
			result.status,
			result.body,
		);
		...
		if (
			error instanceof SteamHttpError &&
			error.message === "steam_auth_failed"
		) {
			await progress?.("Steam API authentication failed", extra);
		} else if (
			error instanceof SteamHttpError &&
			error.message === "steam_forbidden_auth_or_visibility"
		) {
			await progress?.(
				"Steam API request forbidden; authentication or profile visibility may be involved",
				extra,
			);
		}
		throw error;
...
const STEAM_PROFILE_ID_URL_PATTERN =
	/^https?:\/\/steamcommunity\.com\/profiles\/(7656119\d{10})\/?$/i;
const STEAM_VANITY_URL_PATTERN =
...
async function resolveSteamId(
	...
	if (!trimmed) {
		throw new Error(
			"steam_setup_invalid: enter a SteamID64 or custom Steam profile name",
		);
	}
	...
	if (STEAM_HTTP_URL_PATTERN.test(trimmed) && vanityUrl === trimmed) {
		throw new Error(
			"steam_setup_invalid: use a SteamID64 or a steamcommunity.com/id/<name> profile URL",
		);
	}
	const res = await steamApiRequest<ResolveVanityUrlResponse>(
	...
	if (
		response.success === 1 &&
		typeof response.steamid === "string" &&
		isSteamId64(response.steamid)
	) {
		return response.steamid;
	}
	throw new Error(
		`steam_vanity_url_not_found: could not resolve "${vanityUrl}" to a SteamID64`,
	);
}
...
function recentlyPlayedRecord(
	game: SteamRecentlyPlayed,
	steamid: string,
): RecordData {
	return {
		id: `${steamid}:${game.appid}`,
...
async function collectOwnedGames(
	...
	const games = requireSteamArray<SteamOwnedGame>(
		response.games,
		"response.games",
	);
	...
	const declaredTotal = steamDeclaredTotal(
		response.game_count,
		"response.game_count",
		games.length,
	);
	await deps.progress("Fetched owned games", {
		stream: "owned_games",
		count: games.length,
	});

	const gamesCursor = openFingerprintCursor(
		(newState.owned_games as unknown) ?? {},
	);
	...
	newState.owned_games = {
		fetched_at: nowIso(),
		fingerprints: gamesCursor.toState(),
	};
	await deps.emit({
		type: "STATE",
		stream: "owned_games",
		cursor: newState.owned_games,
	});
...
async function collectRecentlyPlayed(
	...
	await deps.progress("Fetching recently played games", {
	...
	const recentGames =
		response.games === undefined
			? []
			: requireSteamArray<SteamRecentlyPlayed>(
					response.games,
					"response.games",
				);
	...
	const recentCursor = openFingerprintCursor(
		(newState.recently_played_games as unknown) ?? {},
	);
	const coverage = await emitSteamSnapshotRecords(
	...
	newState.recently_played_games = {
		fetched_at: nowIso(),
		fingerprints: recentCursor.toState(),
	};
	await deps.emit({
		type: "STATE",
		stream: "recently_played_games",
		cursor: newState.recently_played_games,
	});
...
async function collectFriends(
	...
	try {
		...
		if (error instanceof SteamHttpError && error.status === 401) {
			await deps.emit({
				...
				message:
					"Steam friends are unavailable because this account restricts the friends list.",
	...
	const friends = requireSteamArray<SteamFriend>(
		friendsList.friends,
		"friendslist.friends",
	);
	await deps.progress("Fetched friends list", {
		stream: "friends",
	...
	newState.friends = {
		fetched_at: nowIso(),
		fingerprints: friendsCursor.toState(),
	};
	await deps.emit({
		type: "STATE",
		stream: "friends",
		cursor: newState.friends,
	});
...
>): Promise<void> {
	...
	const rawSteamId = resolveRawSteamIdFromCredentials(
		credentials,
		process.env,
	)?.trim();
	if (!rawSteamId) {
