...
const CONNECTOR_INDEXES = [
	...
	"imessage",
	"twitter_archive",
].map((name) => join(import.meta.dirname, "..", name, "index.ts"));
...
const UNSAFE_OPERATOR_MESSAGE_PATTERNS: [RegExp, string][] = [
	...
	[
		/message:\s*`[^`]*(?:\.jsonl|\.json|\.js|\.xml|\.db)[^`]*`/,
		"message includes a concrete source filename",
	],
	[
		/progress\(\s*`[^`]*(?:\.jsonl|\.json|\.js|\.xml|\.db)[^`]*`/,
		"progress includes a concrete source filename",
	],
];
