...
async function makePopulatedHome(): Promise<string> {
	const claudeHome = await mkdtemp(
		join(tmpdir(), "pdpp-claude-coverage-truthful-"),
	);
...
test("claude_code coverage truth: a run emitting real messages/attachments/memory_notes records also emits coverage_diagnostics rows for those streams", async () => {
	...
	const messagesCoverage = coverageRecs.find(
		(r) => r.data.stream === "messages",
	);
	const attachmentsCoverage = coverageRecs.find(
		(r) => r.data.stream === "attachments",
	);
	const memoryNotesCoverage = coverageRecs.find(
		(r) => r.data.stream === "memory_notes",
	);
	...
	const streamNames = Object.values(stateSnapshot)
		.filter(
			(v) =>
				typeof v === "object" &&
				v !== null &&
				"stream" in (v as Record<string, unknown>),
		)
		.map((v) => (v as { stream?: unknown }).stream);

	for (const stream of ["messages", "attachments", "memory_notes"]) {
		assert(
			streamNames.includes(stream),
			`STATE snapshot must include a '${stream}' coverage row`,
		);
	}
});
...
test("claude_code coverage truth: genuinely empty projects dir (no messages/attachments/memory_notes) still shows collected", async () => {
	const claudeHome = await mkdtemp(
		join(tmpdir(), "pdpp-claude-coverage-truthful-empty-"),
	);
	...
	for (const stream of ["messages", "attachments", "memory_notes"]) {
		assert(
			coveredStreams.has(stream),
			`coverage_diagnostics must include a row for '${stream}' even when empty`,
		);
	}
	const messagesCoverage = coverageRecs.find(
		(r) => r.data.stream === "messages",
	...
	assert.equal(
		...
		"collected",
		"an empty-but-complete scan is collected, not missing",
	);
});
...
test("claude_code: a bounded pass across every default stream reaches an accepted terminal DONE (bounded complete-pass proof)", async () => {
	...
	for (const stream of ["messages", "attachments", "memory_notes"]) {
		assert(
			coveredStreams.has(stream),
			`coverage_diagnostics must cover '${stream}' in a bounded complete pass`,
		);
	}
});
...
function coverageReasonFor(
	recs: Extract<EmittedMessage, { type: "RECORD" }>[],
	stream: string,
): string | undefined {
	const row = recs.find(
		(r) => r.stream === "coverage_diagnostics" && r.data.stream === stream,
	);
...
test("claude_code coverage truth: a message-only transcript reports attachments as 0 examined, not the message line count", async () => {
	const claudeHome = await mkdtemp(
		join(tmpdir(), "pdpp-claude-coverage-truthful-message-only-"),
	);
	const project = join(claudeHome, "projects", "-tmp-demo");
	const sessionDir = join(project, SESSION_ID);
	...
	const recs = records(result.messages);
	assert.equal(
		recs.filter((r) => r.stream === "messages").length,
		5,
		"fixture must emit exactly 5 message records",
	);
	assert.equal(
		recs.filter((r) => r.stream === "attachments").length,
		0,
		"fixture must emit zero attachment records",
	);
...
test("claude_code coverage truth: an attachment-only transcript reports messages as 0 examined, not the attachment line count", async () => {
	const claudeHome = await mkdtemp(
		join(tmpdir(), "pdpp-claude-coverage-truthful-attachment-only-"),
	);
	const project = join(claudeHome, "projects", "-tmp-demo");
	const sessionDir = join(project, SESSION_ID);
	await mkdir(sessionDir, { recursive: true });
	...
	assert.equal(
		recs.filter((r) => r.stream === "messages").length,
		1,
		"fixture emits exactly the one pinning message",
	);
