...
import {
	...
	openArtifactCapture,
} from "../../packages/polyfill-connectors/src/artifact-capture.ts";
...
export const CLAUDE_CODE_KNOWN_LOCAL_STORES: KnownLocalStore[] = [
	...
	},
	{
		...
		classification: "inventory_only",
		reason:
			"user-specific local convention; diagnostics only, not a general Claude Code stream",
	},
	{
		store: "cache",
		relativePath: "cache",
 ...
 */
export function observeJsonlFields(
	obj: JsonlObject,
 ...
 */
export async function processJsonlLine({
	buildOnly,
	deps,
	obj,
	obs,
}: ProcessJsonlLineArgs): Promise<void> {
	const { sessionId } = obs;
	if (!sessionId) {
		return;
	}
...
async function processToolResultEntry(
	ent: Dirent,
	args: ProcessToolResultArgs,
): Promise<void> {
...
async function processSlashCommandFile(
	args: ProcessSlashCommandArgs,
): Promise<void> {
...
async function emitProjectMemoryNotes({
	...
	const files = await readFilesRecursively(
		memoryDir,
		(ent) =>
			(ent.isFile() || ent.isSymbolicLink()) && MD_FILE_RE.test(ent.name),
	);
...
function readSessionAccumulator(
	value: unknown,
): SessionAccumulator | undefined {
	if (!isRecord(value)) {
		return;
	}
	const textFields = [
...
function readChildFileCursors(
	...
	for (const [path, cursor] of Object.entries(value)) {
		if (
			...
			out[path] = {
				...cursor,
				current_session_id: cursor.current_session_id as string | null,
			};
		}
	}
	return out;
...
function readSessionAggregates(value: unknown): {
	aggregates: Record<string, SessionAccumulator>;
	valid: boolean;
} {
	if (!isRecord(value)) {
...
async function emitLocalJsonlTelemetry(
	emit: CollectContext["emit"],
	telemetry: LocalJsonlTelemetry,
): Promise<void> {
	await emit({
...
export async function discoverClaudeJsonlSources(
	...
	for (const projectDir of projectDirs) {
		...
		for (const entry of entries
			.filter(
				(item) => item.isDirectory() && SESSION_DIR_PREFIX_RE.test(item.name),
			)
			.sort((a, b) => a.name.localeCompare(b.name))) {
...
async function scanSessionSource(input: {
	...
	cursor: ClaudeSessionFileCursorV1;
	rebuilt: boolean;
...
async function assertRequestedClaudeSources(input: {
	...
	if (
		input.requested.has("skills") &&
		!(await isReadableDirectory(join(input.claudeHome, "skills")))
	) {
		missing.push(
			`CLAUDE_CODE_HOME skills directory=${join(input.claudeHome, "skills")}`,
		);
	}
	if (
		input.requested.has("slash_commands") &&
		!(await isReadableDirectory(join(input.claudeHome, "commands")))
	) {
		missing.push(
			`CLAUDE_CODE_HOME commands directory=${join(input.claudeHome, "commands")}`,
		);
	}
	if (missing.length > 0) {
		throw new Error(
	...
	if (requested.has("skills")) {
		if (skillsScanned) {
			...
			await emit({
				...
				message:
					"The Claude Code skills directory could not be enumerated, so its contents are unknown for this run",
			});
		}
	}
	if (requested.has("slash_commands")) {
		if (slashCommandsScanned) {
			...
			await emit({
				...
				reason: "source_unreadable",
				message:
					"The Claude Code commands directory could not be enumerated, so its contents are unknown for this run",
			});
		}
	}
	if (requested.has("usage")) {
...
function matchesLegacyJsonlMtime(
	mtimes: Map<string, Set<number>>,
	path: string,
