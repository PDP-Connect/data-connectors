...
} from "../../packages/polyfill-connectors/src/local-device-runtime.ts";
...
async function collectSkills(
	home: string,
): Promise<Extract<EmittedMessage, { type: "RECORD" }>[]> {
...
test("claude-code: two source homes emit the SAME connector-local skill key", async () => {
	const [aSkills, bSkills] = await Promise.all([
		collectSkills(DEVICE_A_HOME),
		collectSkills(DEVICE_B_HOME),
	]);
...
test("claude-code: per-source-home envelopes stay isolated despite identical record keys", async () => {
	...
	assert.ok(
		aShared && bShared,
		"both homes must produce an envelope for the shared skill key",
	);
	...
	assert.equal(
		aShared.record_key,
		bShared.record_key,
		"record key collision is expected",
	);
	...
	const dedupKey = (e: {
		...
		stream: string;
		record_key: string;
	}): string => JSON.stringify([e.source_instance_id, e.stream, e.record_key]);
	const slots = new Set([dedupKey(aShared), dedupKey(bShared)]);
	assert.equal(
		slots.size,
		2,
		"shared record key must map to two distinct per-instance slots",
	);
	...
	assert.ok(
		JSON.stringify(bShared.data).includes("Device B"),
		"device B body must be preserved",
	);
});
