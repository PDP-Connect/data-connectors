...
import { slackApiPacingProfile } from "../../packages/polyfill-connectors/src/provider-profile.ts";
import type {
...
export const SLACK_API_RETRYABLE_FAILURE_RE =
	/slack_rate_limited|ECONN|ETIMEDOUT|timeout/i;
 ...
 */
export function buildSlackSessionCookieHeader(
	cookie: string,
...
function parseSlackApiResponse<T extends { error?: string; ok: boolean }>(
	raw: SlackApiRawResponse,
): T {
	if (raw.status === 401) {
	...
	if (raw.status < 200 || raw.status >= 300) {
		throw new Error(
			`slack_api_http_${String(raw.status)}: ${raw.body.slice(0, 200)}`,
		);
	}
	let parsed: T;
	try {
...
export async function fetchAllStars(
	...
	do {
		...
		const resp = await slackApiPost<SlackStarsListResponse>(
			"stars.list",
			token,
			cookie,
			params,
...
export async function fetchAllUserGroups(
	...
	const resp = await slackApiPost<SlackUserGroupsListResponse>(
		"usergroups.list",
		token,
		cookie,
		{
...
export async function fetchAllReminders(
	...
	const resp = await slackApiPost<SlackRemindersListResponse>(
		"reminders.list",
		token,
		cookie,
		{},
...
export async function fetchDmReadStates(
	...
	for (const channelId of channelIds) {
		const resp = await slackApiGet<SlackConversationInfoResponse>(
			"conversations.info",
			token,
			cookie,
			{
