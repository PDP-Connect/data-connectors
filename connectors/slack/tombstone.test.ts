...
test("exact Slack tombstone marker emits a delete, while ordinary records do not", () => {
	assert.equal(
		isSlackMessageTombstone("messages", { id: "m1", is_tombstone: true }),
		true,
	);
	assert.equal(
		isSlackMessageTombstone("messages", { id: "m1", is_tombstone: false }),
		false,
	);
	assert.equal(
		isSlackMessageTombstone("reactions", { id: "m1", is_tombstone: true }),
		false,
	);
});
