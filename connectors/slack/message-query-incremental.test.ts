...
function referenceQuery(thresholds: Thresholds): {
	params: string[];
	sql: string;
} {
...
function runRows(
	...
	return rows
		.map((r) => `${r.CHANNEL_ID}|${r.TS}|${String(r.DATA)}`)
		.sort((a, b) => a.localeCompare(b));
}
...
const SHAPES: Array<{ name: string; thresholds: Thresholds }> = [
	...
	{
		name: "legacy-only cursor",
		thresholds: {
			channelLastTs: {},
			legacyLastTs: "180.000000",
			sinceTs: null,
		},
	},
	{
		...
		thresholds: {
			channelLastTs: { C1: "100.000001", C2: "250.000001" },
			legacyLastTs: null,
			sinceTs: null,
		},
	},
...
for (const shape of SHAPES) {
	test(`incremental message query is emit-identical to reference: ${shape.name}`, () => {
		...
		assert.deepEqual(
			actual,
			expected,
			`emitted set diverged for shape: ${shape.name}`,
		);
		db.close();
	});
}
...
test("a newer higher-CHUNK_ID row for a past-cursor TS wins the dedup", () => {
	...
	assert.ok(
		rows.includes("C1|300.000001|c1-300-resumed-latest"),
		"latest chunk for 300 must win",
	);
	assert.ok(
		!rows.includes("C1|300.000001|c1-300-only"),
		"stale chunk for 300 must not be emitted",
	);
	db.close();
});
