...
async function scopedArchiveDigest(
...
test("scoped-archive fold across 2+ archives: each (state_stream, stream) DETAIL_COVERAGE pair emits exactly once, with the summed considered", async () => {
	const artifactRoot = await mkdtemp(
		join(tmpdir(), "pdpp-slack-scoped-coverage-"),
	);
	try {
		...
		const scopedArchiveDirA = join(
			seedArchiveRoot(artifactRoot, workspace),
			"archive-scoped",
			scopedDigestA,
		);
		const scopedArchiveDirB = join(
			seedArchiveRoot(artifactRoot, workspace),
			"archive-scoped",
			scopedDigestB,
		);
		...
		const scopedDbA = new DatabaseSync(
			join(scopedArchiveDirA, "slackdump.sqlite"),
		);
		try {
		...
		const scopedDbB = new DatabaseSync(
			join(scopedArchiveDirB, "slackdump.sqlite"),
		);
		try {
		...
		const messagesCoverage = coverage.filter(
			(m) => m.state_stream === "messages" && m.stream === "messages",
		);
		const reactionsCoverage = coverage.filter(
			(m) => m.state_stream === "messages" && m.stream === "reactions",
		);
		const attachmentsCoverage = coverage.filter(
			(m) =>
				m.state_stream === "messages" && m.stream === "message_attachments",
		);
		...
		assert.equal(
			messagesCoverage[0]?.considered,
			10,
			"considered is the SUMMED total across every archive folded",
		);
		...
		assert.equal(
			...
			0,
			"reactions counts reaction records, never message rows",
		);
		assert.equal(reactionsCoverage[0]?.covered, 0);
		assert.equal(
			attachmentsCoverage[0]?.considered,
			0,
			"attachments count attachment records, never message rows",
		);
		...
		for (const streamName of ["reactions", "message_attachments"]) {
			...
			assert.equal(
				"state_stream" in stream,
				false,
				`${streamName} must not use static state_stream inheritance`,
			);
		}
	} finally {
