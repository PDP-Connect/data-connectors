...
const ENTRYPOINT = connectorEntrypoint("google_maps");
...
function pointsCoverage(
	...
	const coverage = messages.find(
		(
			...
			message.stream === "timeline_points",
	);
...
test("google_maps declares the merged point boundary as the timeline_points denominator", async () => {
	const importRoot = await mkdtemp(
		join(tmpdir(), "pdpp-google-maps-coverage-"),
	);
	try {
		await writeFile(
			join(importRoot, "Records.json"),
			JSON.stringify(EXPORT_FIXTURE),
		);
		const { messages } = await runImport(importRoot);

		const emitted = messages.filter(
			(message) =>
				message.type === "RECORD" && message.stream === "timeline_points",
		).length;
...
test("google_maps dedupes duplicate archive files and proves both declared streams", async () => {
	...
	try {
		...
		await writeFile(
			join(importRoot, "location-history.json"),
			JSON.stringify(EXPORT_FIXTURE),
		);
		await writeFile(
		...
		const segmentRecords = messages.filter(
			(message) =>
				message.type === "RECORD" && message.stream === "timeline_segments",
		);
		assert.equal(
			pointRecords.length,
			2,
			"duplicate archive files must not duplicate stable point IDs",
		);
		assert.equal(
			segmentRecords.length,
			1,
			"the semantic segment should be emitted once",
		);
...
test("google_maps imports a large semantic segment on both streams without terminal record_too_large gaps", async () => {
	const importRoot = await mkdtemp(
		join(tmpdir(), "pdpp-google-maps-large-segment-"),
	);
	try {
		...
		assert.ok(
			Buffer.byteLength(exportContent, "utf8") > 4 * 1024 * 1024,
			"fixture must exceed the old element bound",
		);
			...
			assert.equal(
				messages.some(
					(message) =>
						...
						message.reason === "record_too_large",
				),
				false,
		...
		assert.ok(
			messages.some(
				(message) =>
					message.type === "STATE" && message.stream === "timeline_points",
			),
		);
		assert.ok(
			messages.some(
				(message) =>
					message.type === "STATE" && message.stream === "timeline_segments",
			),
		);
	} finally {
		await rm(importRoot, { force: true, recursive: true });
...
test("google_maps with a malformed archive does not emit STATE or coverage", async () => {
	...
	try {
		...
		assert.ok(
			messages.some(
				(message) =>
					message.type === "SKIP_RESULT" && message.reason === "invalid_json",
			),
...
test("google_maps rejects trailing non-whitespace after a valid archive", async () => {
	const importRoot = await mkdtemp(
		join(tmpdir(), "pdpp-google-maps-trailing-"),
	);
	try {
		await writeFile(
			join(importRoot, "Records.json"),
			`${JSON.stringify(EXPORT_FIXTURE)}${" ".repeat(65_536)}trailing`,
		);
		const { messages } = await runImport(importRoot);
...
test("google_maps rejects recognized non-array keys without state or coverage on either stream", async () => {
	...
	const runCase = async ([
		key,
		value,
	]: (typeof cases)[number]): Promise<void> => {
				...
				assert.ok(
					messages.some(
						(message) =>
							message.type === "SKIP_RESULT" &&
							message.stream === stream &&
							message.reason === "unsupported_shape",
					),
	...
	await cases.reduce(
		(previous, entry) => previous.then(() => runCase(entry)),
		Promise.resolve(),
	);
});
...
test("google_maps rejects scalar, empty, and unknown-key-only roots without zero-coverage claims", async () => {
	...
	const runCase = async (content: string): Promise<void> => {
		const importRoot = await mkdtemp(
			join(tmpdir(), "pdpp-google-maps-root-shape-"),
		);
		try {
				...
				assert.equal(
					messages.some(
						(message) =>
							message.type === "SKIP_RESULT" &&
							message.stream === stream &&
							message.reason.endsWith("_not_found"),
					),
					false,
					content,
				);
...
test("google_maps keeps a recognized empty array as a valid empty counterweight", async () => {
	const importRoot = await mkdtemp(
		join(tmpdir(), "pdpp-google-maps-empty-shape-"),
	);
	try {
		await writeFile(
			join(importRoot, "Timeline.json"),
			JSON.stringify({ timelineObjects: [] }),
		);
		const { messages } = await runImport(importRoot);
		...
		assert.ok(
			messages.some(
				(message) =>
					message.type === "SKIP_RESULT" &&
					message.reason === "timeline_points_not_found",
			),
		);
		assert.ok(
			messages.some(
...
test("google_maps with a schema-rejected unknown segment withholds its cursor and coverage", async () => {
	const importRoot = await mkdtemp(
		join(tmpdir(), "pdpp-google-maps-schema-rejection-"),
	);
	try {
...
test("google_maps fails closed when discovery misses the source boundary", async () => {
	const missingRoot = join(
		tmpdir(),
		`pdpp-google-maps-missing-${String(process.pid)}-${Date.now()}`,
	);
	const readErrorRoot = join(
		tmpdir(),
		`pdpp-google-maps-not-a-directory-${String(process.pid)}-${Date.now()}`,
	);
	...
	try {
		...
		await deepPaths.reduce(
			(previous, path) => previous.then(() => mkdir(path)),
			Promise.resolve(),
		);
		await Promise.all(
			Array.from({ length: 2001 }, (_, index) =>
				writeFile(
					join(cappedRoot, `unrelated-${String(index).padStart(4, "0")}.txt`),
					"",
				),
			),
		);
		...
		await [missingRoot, readErrorRoot, deepRoot, cappedRoot].reduce(
			(previous, importRoot) =>
				previous.then(() => runDiscoveryCase(importRoot)),
			Promise.resolve(),
		);
	} finally {
