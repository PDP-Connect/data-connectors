...
function parseLegacyRecords(json: Record<string, unknown>): ParseResult {
	const locations = Array.isArray(json.locations)
		? (json.locations as LegacyLocationPoint[])
		: [];
	const points: TimelinePointRecord[] = [];
	for (const loc of locations) {
		const timestamp =
			parseTimestamp(loc.timestamp) ?? parseTimestampMs(loc.timestampMs);
...
function parseTimelinePathPoint(
	...
	const obj = asObject(value);
	const timestamp =
		parseTimestamp(obj?.time) ?? parseTimestamp(obj?.timestamp) ?? fallbackTime;
...
function parseSemanticSegment(segment: Record<string, unknown>): ParseResult {
	...
	const topActivity =
		asObject(activity?.topCandidate) ?? asObject(activity?.topActivity);
	const visitLocation = parseLatLon(
		topVisit?.placeLocation ??
			topVisit?.location ??
			visit?.placeLocation ??
			visit?.location,
	);
	...
	const activityType =
		asString(topActivity?.type) ?? asString(activity?.activityType);
	const placeId =
		asString(topVisit?.placeID) ??
		asString(topVisit?.placeId) ??
		asString(visit?.placeId);
	const semanticType =
		asString(topVisit?.semanticType) ?? asString(visit?.semanticType);
	const probability = safeProbability(
		topVisit?.probability ??
	...
	if (startTime) {
		const seg = buildSegment({
			...
			probability,
			unrecognizedKind:
				segmentKind === "unrecognized" ? unrecognizedSegmentKey(segment) : null,
		});
		segmentId = seg.id;
		segments.push(seg);
	}
...
function parsePlaceVisitObject(
	placeVisit: Record<string, unknown>,
): ParseResult {
...
function parseActivitySegmentObject(
	activitySegment: Record<string, unknown>,
): ParseResult {
...
function mergeResults(results: ParseResult[]): ParseResult {
	...
	return {
		points: [...pointMap.values()].sort((a, b) =>
			a.timestamp.localeCompare(b.timestamp),
		),
		segments: [...segmentMap.values()].sort((a, b) =>
...
export function parseGoogleMapsExportElement(
	format: GoogleMapsSourceFormat,
	value: unknown,
): ParseResult {
