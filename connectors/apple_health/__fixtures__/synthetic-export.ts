...
const SOURCES = [
	{
		...
		version: "17.5",
		device:
			"<<HKDevice: 0x1>, name:iPhone, manufacturer:Apple, model:iPhone, hardware:iPhone16,2, software:17.5>",
	},
	{
		name: "Tim’s Apple Watch",
		version: "10.5",
		device:
			"<<HKDevice: 0x2>, name:Apple Watch, manufacturer:Apple, model:Watch, hardware:Watch7,1, software:10.5>",
	},
...
const CATEGORY_TYPES: { type: string; values: string[] }[] = [
	...
	},
	{
		...
		values: ["HKCategoryValueNotApplicable"],
	},
];
	...
	for (let i = 0; i < recordCount; i += 1) {
		...
		if (rnd() < 0.15) {
			...
			const sleepEnd = new Date(
				d.getTime() + (cat.type.includes("Sleep") ? 3 * 3_600_000 : 600_000),
			);
			lines.push(
...
function buildEdgeCaseRecordLines(startYear: number): string[] {
	...
	lines.push(
		`<Record type="HKQuantityTypeIdentifierStepCount" sourceName="iPhone" value="5"/>`,
	);
...
function buildWorkoutSection(
	...
	for (let i = 0; i < workoutCount; i += 1) {
		...
		lines.push(
			buildWorkoutLine(d, durationMin, wType, src, {
				hasChildren,
				hasEvents,
				hasStats,
			}),
 ...
 */
export function buildSyntheticExportXml(recordCount: number): {
	stats: SyntheticExportStats;
	xml: string;
} {
	...
	const {
		...
		quantityRecords,
		categoryRecords,
	} = buildRecordLines(recordCount, startYear, rnd);
...
export function buildSyntheticExportDir(recordCount: number): {
	dir: string;
	stats: SyntheticExportStats;
} {
