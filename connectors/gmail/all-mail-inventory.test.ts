...
async function runPass(
	...
	return {
		...
		inventory: protocolMessages.find(
			(m) => m.type === "PROGRESS" && m.all_mail_inventory !== undefined,
		),
		state: protocolMessages.find(
...
test("gmail all mail: the server-declared EXISTS is bound and disclosed, not discarded", async () => {
	...
	assert.ok(
		inventory,
		"a run must disclose the mailbox total the server handed it",
	);
...
test("gmail all mail: EXISTS is measured at the provider boundary, not derived from what was emitted", async () => {
	...
	const { inventory } = await runPass({ exists: 1200 });
	const disclosed = inventory?.all_mail_inventory as {
		all_mail_exists: number;
	};

	assert.equal(
		disclosed.all_mail_exists,
		1200,
		"the total is the server's count, independent of the 2 emitted",
	);
	assert.notEqual(
		disclosed.all_mail_exists,
		2,
		"the total must never collapse to the emitted count",
	);
});
...
test("gmail all mail: the EXISTS total is carried on STATE so the next run can compare it", async () => {
	...
	const allMail = (
		state?.cursor as Record<string, Record<string, unknown>> | undefined
	)?.all_mail;
	...
	assert.equal(
		allMail.exists,
		1200,
		"the cursor persists the epoch's inventory size",
	);
	assert.equal(
		allMail.uidvalidity,
		123,
		"the count is only meaningful alongside the epoch it was measured in",
	);
});
...
test("gmail all mail: a non-numeric EXISTS fails closed", async () => {
	await assert.rejects(
		() => runPass({ exists: "1200" }),
		/gmail_all_mail_exists_not_number/,
	);
});

test("gmail all mail: a non-finite EXISTS fails closed", async () => {
	await assert.rejects(
		() => runPass({ exists: Number.POSITIVE_INFINITY }),
		/gmail_all_mail_exists_not_finite/,
	);
});
...
test("gmail all mail: a fractional EXISTS fails closed", async () => {
	await assert.rejects(
		() => runPass({ exists: 12.5 }),
		/gmail_all_mail_exists_not_integer/,
	);
});
...
test("gmail all mail: a negative EXISTS fails closed", async () => {
	await assert.rejects(
		() => runPass({ exists: -1 }),
		/gmail_all_mail_exists_negative/,
	);
});
...
test("gmail all mail: a growing mailbox within one UID epoch is normal and does not throw", async () => {
	...
	assert.equal(
		(inventory.all_mail_inventory as { all_mail_exists: number })
			.all_mail_exists,
		1200,
...
test("gmail all mail: a UIDVALIDITY re-key does not read a lower count as loss", async () => {
	...
	assert.equal(
		(inventory.all_mail_inventory as { uidvalidity: number }).uidvalidity,
		123,
	);
});
...
test("gmail all mail: the messages page coverage denominator stays per-page", async () => {
	...
	const coverage = messages.find(
		(m) => m.type === "DETAIL_COVERAGE" && m.stream === "messages",
	);
	...
	assert.equal(
		coverage.considered,
		2,
		"the page denominator is the page, not the mailbox",
	);
	assert.equal(coverage.covered, 2);
	assert.notEqual(
		coverage.considered,
		1200,
		"the mailbox total must not be substituted here",
	);
});
