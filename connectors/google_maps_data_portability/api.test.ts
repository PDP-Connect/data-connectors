...
test("Google Maps Data Portability resource set is Maps-only and excludes Timeline", () => {
	assert.ok(
		GOOGLE_MAPS_DATA_PORTABILITY_RESOURCE_GROUPS.includes(
			"maps.starred_places",
		),
	);
	assert.ok(
	...
	assert.ok(
		GOOGLE_MAPS_DATA_PORTABILITY_RESOURCE_GROUPS.includes("myactivity.maps"),
	);
	assert.ok(
		GOOGLE_MAPS_DATA_PORTABILITY_RESOURCE_GROUPS.includes("mymaps.maps"),
	);
	assert.equal(
		GOOGLE_MAPS_DATA_PORTABILITY_RESOURCE_GROUPS.some((item) =>
			/timeline|location/i.test(item),
		),
		false,
	);
});
...
test("OAuth scopes are derived from documented Data Portability resource groups only", () => {
	...
	assert.equal(
		GOOGLE_MAPS_DATA_PORTABILITY_OAUTH_SCOPES.some((scope) =>
			/gmail|userinfo|timeline/i.test(scope),
		),
		false,
	);
});
...
test("initiateArchive posts resource groups and optional time range", async () => {
	...
	assert.equal(
		transport.calls[0]?.url,
		"https://dp.example/v1/portabilityArchive:initiate",
	);
...
test("getArchiveState calls the documented archive state endpoint", async () => {
	...
	assert.equal(
		transport.calls[0]?.url,
		"https://dp.example/v1/archiveJobs/job-123/portabilityArchiveState",
	);
