...
type DetailGapRecovered = Extract<
	EmittedMessage,
...
function findDetailGapRecovered(
	messages: EmittedMessage[],
): DetailGapRecovered[] {
	return messages.filter(
		(m): m is DetailGapRecovered => m.type === "DETAIL_GAP_RECOVERED",
	);
}
...
function noopSendInteraction(): ReturnType<
...
function makePageStub(opts: {
	...
	let gotoCalls = 0;
	const url =
		opts.url ?? "https://www.heb.com/my-account/order-history/HEB1000000001";
	return new Proxy(
		{},
		{
				...
				if (prop === "evaluate") {
					if (opts.evaluateError) {
						return (): Promise<never> =>
							Promise.reject(new Error("synthetic evaluate failure"));
					}
					return (): Promise<unknown> => {
						const hasItem = opts.content.includes(
							'data-qe-id="itemRowDetailsName"',
						);
						const rows = hasItem
							? [
									{
...
const LIVE_ORDERS_HTML =
	'<html><body><main><div data-qe-id="orderResults"></div></main></body></html>';
const STILL_DEAD_HTML =
	'<html><body><main><form><input type="password"></form></main></body></html>';
 ...
 */
function makeSessionRepairPageStub(opts: {
	detailHtmlAfterRepair?: string;
	htmlSequence: string[];
}): Page {
	...
	return new Proxy(
		{},
		{
			get(_target, prop): unknown {
				if (prop === "goto") {
					return (navUrl: string): Promise<null> => {
						...
						if (next !== undefined) {
							...
							navUrl.includes("order-history") &&
							opts.detailHtmlAfterRepair
						) {
				...
				throw new Error(
					`unexpected page.${String(prop)} in session-repair test stub`,
				);
			},
		},
	) as Page;
...
test("processListOrder: a malformed order date records a 'gap' coverage outcome backed by DETAIL_GAP when order_items is in scope", async () => {
	...
	assert.deepEqual(
		coverage.required,
		["HEB1000000001"],
		"the order must still join the required denominator",
	);
	assert.deepEqual(
		coverage.gap,
		["HEB1000000001"],
		"parse failure is an actionable gap",
	);
	assert.deepEqual(coverage.hydrated, []);
	assert.equal(
		emitted.length,
		0,
		"no order/item record can emit without a parsed order_date",
	);
	assert.ok(
...
test("processListOrder: a malformed order date emits no DETAIL_GAP when order_items is out of scope (wantsItems: false)", async () => {
	...
	assert.deepEqual(
		coverage.required,
		[],
		"orderItemsCoverage is not written when wantsItems: false",
	);
	...
	assert.equal(
		gaps.length,
		0,
		"no DETAIL_GAP when order_items is out of scope",
	);
});
...
test("collect-path regression guard: orders scoped alone (order_items out of scope) still emits an orders DETAIL_COVERAGE", async () => {
	...
	await processListOrder(
		...
		deps,
		makeRunFlags(),
		makeListOrder({ orderId: "HEB1000000001" }),
	);
	await processListOrder(
		NEVER_CALLED_PAGE,
		deps,
		makeRunFlags(),
		makeListOrder({ orderId: "HEB1000000002" }),
	);
	await emitOrdersCoverage(deps, ordersCoverage);

	const coverageMessages = protocolMessages.filter(
		(m): m is DetailCoverage => m.type === "DETAIL_COVERAGE",
	);
...
test("processListOrder: a password-form detail response latches sessionRepairRequired and prevents a second detail navigation", async () => {
	...
	assert.equal(
		...
		true,
		"a password-form response latches the run into session repair",
	);
...
test("fetchOrderDetail: a single transient navigation failure retries and then succeeds (not immediately exhausted)", async () => {
	...
	assert.equal(
		result.status,
		"hydrated",
		"one transient failure must not be reported as exhausted",
	);
});
...
test("fetchOrderDetail: a non-retryable navigation error (e.g. page closed) is not mislabeled navigation_retry_exhausted and makes exactly one attempt", async () => {
	...
	const page = new Proxy(
		{},
		{
			get(_target, prop): unknown {
				...
				throw new Error(
					`unexpected page.${String(prop)} in non-retryable-error test stub`,
				);
			},
		},
	) as Page;
	...
	assert.equal(
		gotoCalls,
		1,
		"a non-retryable error must not consume the retry budget",
	);
	assert.equal(result.status, "failed");
	assert.notEqual(
		result.failureKind,
...
test("classifyHebDetailFailure: a non-retryable navigation failure classifies as connector_defect, not a retryable class", () => {
	assert.equal(
		classifyHebDetailFailure("navigation_failed_non_retryable"),
		"connector_defect",
	);
});
...
test("hebAllowsInteractiveAuthRepair: true when PDPP_RUN_TRIGGER_KIND is 'manual'", () => {
	assert.equal(
		hebAllowsInteractiveAuthRepair({ PDPP_RUN_TRIGGER_KIND: "manual" }),
		true,
	);
});
...
test("hebAllowsInteractiveAuthRepair: false for a scheduled/retry/webhook trigger kind", () => {
	assert.equal(
		hebAllowsInteractiveAuthRepair({ PDPP_RUN_TRIGGER_KIND: "scheduled" }),
		false,
	);
	assert.equal(
		hebAllowsInteractiveAuthRepair({ PDPP_RUN_TRIGGER_KIND: "retry" }),
		false,
	);
	assert.equal(
		hebAllowsInteractiveAuthRepair({ PDPP_RUN_TRIGGER_KIND: "webhook" }),
		false,
	);
});
...
function fakeSendInteraction(
	response: Partial<
		import("../../packages/polyfill-connectors/src/connector-runtime.ts").InteractionResponse
	> = {},
): BrowserCollectContext["sendInteraction"] {
	return () =>
		Promise.resolve({
...
test("resolveOrderDetail: unattended run — sessionRepairRequired latches immediately with zero interaction, no repair spent", async () => {
	...
	assert.equal(result.failureKind, "session_repair_required");
	assert.equal(
		flags.manualRepairAttempted,
		false,
		"an unattended run must never spend the repair attempt",
	);
	assert.equal(
		flags.sessionRepairRequired,
		true,
		"the latch stays set for the rest of the run",
	);
});
...
test("resolveOrderDetail: owner-started manual run — successful repair retries the affected detail once and succeeds", async () => {
	...
	assert.equal(
		flags.manualRepairAttempted,
		true,
		"the one shared attempt is now spent",
	);
	assert.equal(
		result.status,
		"hydrated",
		"the retried detail fetch must succeed after a recovered session",
	);
	assert.equal(
		flags.sessionRepairRequired,
		false,
		"a successful repair+retry clears the latch",
	);
	assert.equal(
		repairRetryWaits,
		1,
		"the repair retry waits exactly once after a recovered session",
	);
});
...
test("resolveOrderDetail: owner-started manual run — manualAction itself fails (sendInteraction errors) latches and defers", async () => {
	...
	assert.equal(
		flags.manualRepairAttempted,
		true,
		"the attempt is consumed even though it failed",
	);
	assert.equal(result.status, "deferred");
	assert.equal(result.failureKind, "session_repair_required");
	assert.equal(
		flags.sessionRepairRequired,
		true,
		"latches after a failed repair, same as the unattended path",
	);
});
...
test("resolveOrderDetail: owner-started manual run — the one shared attempt is spent only once across repeated failures", async () => {
	...
	const first = await resolveOrderDetail(
		page,
		flags,
		"HEB1000000001",
		repairDeps,
	...
	const second = await resolveOrderDetail(
		NEVER_CALLED_PAGE,
		flags,
		"HEB1000000002",
		repairDeps,
...
test("resolveOrderDetail: without repairDeps, an owner-started manual run still latches and defers (repairDeps is optional)", async () => {
	...
	assert.equal(
		flags.manualRepairAttempted,
		false,
		"no repairDeps means no attempt can be spent",
	);
});
...
test("runForwardScan: an order id repeated across two list pages is only processed once", async () => {
			...
			get(_target, prop): unknown {
				...
				if (prop === "content") {
					return (): Promise<string> =>
						Promise.resolve(pages[currentPage] ?? "");
				}
				if (prop === "url") {
					return (): string =>
						`https://www.heb.com/my-account/your-orders?page=${currentPage}`;
				}
	...
	const orderIds = emitted
		.filter((r) => r.stream === "orders")
		.map((r) => r.data.id);
	assert.deepEqual(
		orderIds.sort(),
...
test("runForwardScan: item-enriched scan (wantsItems: true) still fetches page 2 and dedupes the boundary repeat", async () => {
	...
	const page = new Proxy(
		{},
		{
			get(_target, prop): unknown {
				...
				if (prop === "url") {
					return (): string =>
						"https://www.heb.com/my-account/order-history/current";
				}
				throw new Error(
					`unexpected page.${String(prop)} in item-enriched pagination test stub`,
				);
			},
		},
	) as Page;
	...
	const itemOrderIds = emitted
		.filter((r) => r.stream === "order_items")
		.map((r) => r.data.order_id);
	assert.deepEqual(
		itemOrderIds.sort(),
		["HEB1000000000", "HEB1000000001", "HEB1000000002"],
	...
	const page = new Proxy(
		{},
		{
			get(_target, prop): unknown {
				...
				if (prop === "url") {
					return (): string =>
						"https://www.heb.com/my-account/your-orders?page=2";
				}
				throw new Error(
					`unexpected page.${String(prop)} in failed-nav test stub`,
				);
			},
		},
	) as Page;
	...
	const page = new Proxy(
		{},
		{
			get(_target, prop): unknown {
				if (prop === "goto") {
					return (url: string): Promise<null> => {
						const pageNum = Number(
							/your-orders\?page=(\d+)/.exec(url)?.[1] ?? 1,
						);
						if (pageNum === 1) {
							lastContent = page1Html;
							return Promise.resolve(null);
						...
						return Promise.reject(
							new Error("private provider path and stale page"),
						);
					};
				}
				if (prop === "waitForSelector") {
				...
				throw new Error(
					`unexpected page.${String(prop)} in stale-card test stub`,
				);
			},
		},
	) as Page;
...
test("runForwardScan: a falsy goto rejection cannot parse stale page content", async () => {
	...
	assert.deepEqual(
		protocolMessages.find((message) => message.type === "SKIP_RESULT")
			?.diagnostics,
		{
			error_class: "unknown",
		},
...
test("runForwardScan: a genuine single-page result (maxPage: 1, affirmatively asserted) completes without requesting page 2", async () => {
	...
	const page = new Proxy(
		{},
		{
			get(_target, prop): unknown {
				...
				throw new Error(
					`unexpected page.${String(prop)} in genuine-single-page test stub`,
				);
			},
		},
	) as Page;

	const { newestOrderDate } = await runForwardScan(
		page,
		deps,
		makeRunFlags(),
		null,
	...
	assert.equal(
		gotoCount,
		1,
		"a genuine single-page result must never request page 2",
	);
	assert.equal(
		newestOrderDate,
		"2026-07-13",
		"page 1's order must still be processed normally",
	);
	const orderIds = emitted
		.filter((r) => r.stream === "orders")
		.map((r) => r.data.id);
	...
	const page = new Proxy(
		{},
		{
			get(_target, prop): unknown {
				...
				throw new Error(
					`unexpected page.${String(prop)} in empty-before-maxPage test stub`,
				);
			},
		},
	) as Page;
	...
	const page = new Proxy(
		{},
		{
			get(_target, prop): unknown {
				...
				throw new Error(
					`unexpected page.${String(prop)} in empty-before-maxPage-with-nav test stub`,
				);
			},
		},
	) as Page;
...
test("runForwardScan: H-E-B's real 'No past orders' page completes the scan instead of aborting as selector_drift", async () => {
	...
	const { newestOrderDate: newest } = await runForwardScan(
		page,
		deps,
		makeRunFlags(),
		null,
	...
	assert.ok(
		coverage,
		"the orders stream must still report coverage on an empty run",
	);
...
test("classifyEmptyListPage: an auth/challenge page keeps its own reason even when prior orders exist", () => {
	...
	assert.deepEqual(
		classifyEmptyListPage(
			{ ...EMPTY_STATE_DIAG, incapsula_block: true },
			1,
			RESOLVED_MAX_PAGE,
			{
				hasPriorOrders: true,
	...
	assert.deepEqual(
		classifyEmptyListPage(
			{ ...EMPTY_STATE_DIAG, password_form: true },
			1,
			RESOLVED_MAX_PAGE,
			{
				hasPriorOrders: true,
...
test("classifyEmptyListPage: prior orders INSIDE the retention window still escalate", () => {
	// The negative case that keeps the prior-orders guard alive. A recent
	// checkpoint means H-E-B should still be listing these orders, so an empty
	// page is a genuine contradiction — a purge or a degraded session — and must
	// NOT be laundered into a proven-empty result by the retention exception.
	assert.deepEqual(
		classifyEmptyListPage(
			EMPTY_STATE_DIAG,
			1,
...
test("priorOrdersAreBeyondRetention: the floor is a strict boundary measured in real days", () => {
	...
	assert.equal(
		priorOrdersAreBeyondRetention(
			{
				hasPriorOrders: true,
				newestPriorOrderDate: daysBefore(HEB_ORDER_RETENTION_FLOOR_DAYS - 5),
			},
			now,
		),
		false,
	...
	assert.equal(
		priorOrdersAreBeyondRetention(
			{ hasPriorOrders: false, newestPriorOrderDate: "2000-01-01" },
			now,
		),
		false,
	);
...
test("priorOrdersEvidenceFromState: the checkpoint date is carried, not just its existence", () => {
	...
	assert.deepEqual(
		priorOrdersEvidenceFromState({ checkpoint: AGED_OUT_CHECKPOINT }),
		{
			hasPriorOrders: true,
			newestPriorOrderDate: AGED_OUT_CHECKPOINT,
		},
...
test("runForwardScan: H-E-B's real 'No past orders' page aborts when this connection already collected orders", async () => {
	...
	await assert.rejects(
		() =>
			runForwardScan(page, deps, makeRunFlags(), "2026-08-01", {
				hasPriorOrders: true,
			}),
	...
	assert.equal(ordersCoverage.covered.length, 0);
	assert.equal(
		findDetailCoverage(protocolMessages),
		undefined,
		"an aborted run must claim no coverage at all",
	);
});
...
test("runForwardScan: the empty-history abort emits no records and no STATE cursor", async () => {
	...
	assert.equal(
		emitted.length,
		0,
		"no records may be written on a contradicted-empty run",
	);
	assert.equal(
...
): BrowserCollectContext["detailGaps"][number] {
	return {
		detail_locator: {
...
function makeLegacyGapWithoutDate(
	orderId: string,
): BrowserCollectContext["detailGaps"][number] {
	return {
...
test("recoverPendingOrderItemDetailGaps: hydrates a pending order_items gap and emits DETAIL_GAP_RECOVERED", async () => {
	...
	assert.ok(
		emitted.some(
			(r) => r.stream === "order_items" && r.data.order_id === "HEB1000000001",
		),
	...
	assert.equal(
		...
		0,
		"a recovered gap must not re-emit a new pending gap",
	);
});
...
test("recoverPendingOrderItemDetailGaps: a legacy gap with no order_date does NOT fabricate a date — it retains the pending gap", async () => {
	...
	assert.equal(
		result.recovered,
		0,
		"a dateless legacy gap must not count as recovered",
	);
	assert.equal(result.stoppedWithPending, true);
	assert.equal(
	...
	assert.equal(
		gaps.length,
		1,
		"the gap must be retained (re-emitted as pending), not dropped",
	);
...
test("recoverPendingOrderItemDetailGapsBeforeForwardRun: order_items out of scope skips recovery entirely", async () => {
	...
	assert.equal(
		result.suppressForward,
		false,
		"a plain out-of-scope run must not suppress the forward walk",
	);
});
...
test("reasonForDetailFailure and classifyHebDetailFailure cover every current DetailFailureKind", () => {
	...
	const exactUnion: [MissingKinds, UnexpectedKinds] extends [never, never]
		? true
		: false = true;
...
function makeCeilingPage(advertisedMaxPage: number): {
	page: Page;
	pagesFetched: () => number[];
} {
...
test("runForwardScan: hitting MAX_LIST_PAGES does not report the orders stream complete", async () => {
	...
	assert.equal(
		pagesFetched().length,
		HEB_MAX_LIST_PAGES,
		"the ceiling bounds the walk at MAX_LIST_PAGES pages",
	);
...
test("runForwardScan: an honest completion (maxPage under the ceiling) still reports the orders stream complete", async () => {
	...
	assert.equal(
		pagesFetched().length,
		3,
		"an honest walk fetches exactly the advertised page count",
	);
	assert.ok(
		ordersCoverage.covered.length >= ordersCoverage.considered.length &&
			ordersCoverage.considered.length === 3,
...
test("runForwardScan: the ceiling exit reports truncated, the honest exit does not", async () => {
	...
	assert.equal(
		...
		true,
		"stopping at the page ceiling must report truncated",
	);
	...
	assert.equal(
		honestScan.truncated,
		false,
		"reaching the advertised maxPage must NOT report truncated",
	);
});
...
test("buildOrdersStateCursor: a truncated scan must not advance the orders checkpoint past unread pages", () => {
	...
	const truncated = buildOrdersStateCursor(
		"2026-07-14",
		priorState,
		undefined,
		true,
	...
	const honest = buildOrdersStateCursor(
		"2026-07-14",
		priorState,
		undefined,
		false,
	...
	return new Proxy(
		{},
		{
			get(_target, prop): unknown {
				...
				if (prop === "evaluate") {
					return (): Promise<unknown> => {
						...
						const lastVisibleItem = Math.min(
							firstVisibleItem + viewportItems,
							itemsRendered,
						);
						const rows = Array.from(
							{ length: lastVisibleItem - firstVisibleItem },
							(_, offset) => {
						...
						if (scrolls <= maxScrolls && itemsRendered < totalItems) {
							...
							firstVisibleItem = Math.min(
								Math.max(itemsRendered - viewportItems, 0),
				...
				if (prop === "url") {
					return (): string =>
						"https://www.heb.com/my-account/order-history/HEB123";
				}
...
test("fetchOrderDetail: scrolls to load lazy-rendered items before parsing", async () => {
	...
	assert.equal(
		itemsCollected,
		59,
		"virtualized rows observed across scroll snapshots are all collected",
	);
});
...
test("fetchOrderDetail: real inner scrollport fixture proves virtualization and load-more are honored", async () => {
	...
	try {
		...
		const fixtureHtml = readFileSync(
			join(FIXTURES_DIR, "order-detail-inner-scrollport.html"),
			"utf8",
		);
		await page.route(
			"https://www.heb.com/my-account/order-history/HEB-INNER-SCROLL",
			async (route) => {
		...
		assert.equal(
			result.detail?.items.filter(
				(item) =>
					item.productUrl === "https://www.heb.com/product-detail/repeated/900",
			).length,
			2,
...
test("fetchOrderDetail: an incomplete bounded surface becomes an observable gap, not partial hydrated data", async () => {
	...
	assert.equal(result.status, "failed");
	assert.ok(
		result.failureKind === "detail_surface_timeout" ||
			result.failureKind === "detail_surface_incomplete",
...
test("fetchOrderDetail: a visible no-op control fails closed when no declared count exists", async () => {
	...
	try {
		const page = await browser.newPage();
		const fixtureHtml = readFileSync(
			join(FIXTURES_DIR, "order-detail-no-op-control.html"),
			"utf8",
		);
		await page.route(
			"https://www.heb.com/my-account/order-history/HEB-NO-OP",
			async (route) => {
				await route.fulfill({ body: fixtureHtml, contentType: "text/html" });
			},
		);
...
test("fetchOrderDetail: a visible no-op control blocks the declared-count settlement branch", async () => {
	...
	try {
		...
		await page.route(
			"https://www.heb.com/my-account/order-history/HEB-NO-OP-DECLARED",
			async (route) => {
				await route.fulfill({ body: fixtureHtml, contentType: "text/html" });
			},
		);
...
test("fetchOrderDetail: a delayed control response is observed before settlement", async () => {
	...
	try {
		const page = await browser.newPage();
		const fixtureHtml = readFileSync(
			join(FIXTURES_DIR, "order-detail-delayed-control.html"),
			"utf8",
		);
		await page.route(
			"https://www.heb.com/my-account/order-history/HEB-DELAYED",
			async (route) => {
				await route.fulfill({ body: fixtureHtml, contentType: "text/html" });
			},
		);
...
test("fetchOrderDetail: a row without provider or positional identity fails closed", async () => {
	...
	try {
		const page = await browser.newPage();
		const fixtureHtml = readFileSync(
			join(FIXTURES_DIR, "order-detail-missing-row-identity.html"),
			"utf8",
		);
		await page.route(
			"https://www.heb.com/my-account/order-history/HEB-MISSING-IDENTITY",
			async (route) => {
				await route.fulfill({ body: fixtureHtml, contentType: "text/html" });
			},
		);
...
test("fetchOrderDetail: an unanchored remounting window fails closed instead of using document order", async () => {
	...
	try {
		const page = await browser.newPage();
		const fixtureHtml = readFileSync(
			join(FIXTURES_DIR, "order-detail-unanchored-remount.html"),
			"utf8",
		);
		await page.route(
			"https://www.heb.com/my-account/order-history/HEB-UNANCHORED-REMOUNT",
			async (route) => {
				await route.fulfill({ body: fixtureHtml, contentType: "text/html" });
			},
		);
...
test("fetchOrderDetail: a tall fully mounted static list uses document order only with a matching declared count", async () => {
	...
	try {
		...
		await page.route(
			"https://www.heb.com/my-account/order-history/HEB-STATIC-TALL",
			async (route) => {
				await route.fulfill({ body: fixtureHtml, contentType: "text/html" });
			},
		);
...
test("fetchOrderDetail: a fully mounted small order uses stable document order when the provider exposes no position", async () => {
	...
	try {
		const page = await browser.newPage();
		const fixtureHtml = readFileSync(
			join(FIXTURES_DIR, "order-detail.html"),
			"utf8",
		);
		await page.route(
			"https://www.heb.com/my-account/order-history/HEB-STATIC-DETAIL",
			async (route) => {
				await route.fulfill({ body: fixtureHtml, contentType: "text/html" });
			},
		);
...
test("fetchOrderDetail: small orders (fit in viewport) parse without scroll", async () => {
	...
	const result = await fetchOrderDetail(page, "HEB8", {
		waitForHydration: immediateWait,
	});
	...
	assert.equal(
		result.detail?.items.length,
		8,
		"all 8 items collected without scroll",
	);
});
...
test("fetchOrderDetail: a declared-count match does not settle on a transient snapshot and waits for stability after remount", async () => {
	...
	try {
		const page = await browser.newPage();
		const fixtureHtml = readFileSync(
			join(FIXTURES_DIR, "order-detail-declared-count-remount.html"),
			"utf8",
		);
		await page.route(
			"https://www.heb.com/my-account/order-history/HEB-DECLARED-REMOUNT",
			async (route) => {
				await route.fulfill({ body: fixtureHtml, contentType: "text/html" });
			},
		);
		...
		assert.ok(
			snapshots >= 4,
			`expected multiple polls for stability, got diagnostic=${result.diagnostic}`,
		);
	} finally {
		await browser.close();
	}
...
test("fetchOrderDetail: a remount that changes identity scheme does not accumulate duplicate rows", async () => {
	...
	try {
		const page = await browser.newPage();
		const fixtureHtml = readFileSync(
			join(FIXTURES_DIR, "order-detail-identity-scheme-switch.html"),
			"utf8",
		);
		await page.route(
			"https://www.heb.com/my-account/order-history/HEB-SCHEME-SWITCH",
			async (route) => {
				await route.fulfill({ body: fixtureHtml, contentType: "text/html" });
			},
		);
		...
		assert.equal(
			result.detail?.items.length,
			2,
			`expected exactly 2 rows, diagnostic=${result.diagnostic}`,
		);
		assert.deepEqual(
			result.detail?.items.map((item) => item.name),
			["Settled item 1", "Settled item 2"],
...
test("fetchOrderDetail: continuous remounts with matching count fail closed without consecutive stable polls", async () => {
	...
	try {
		const page = await browser.newPage();
		const fixtureHtml = readFileSync(
			join(FIXTURES_DIR, "order-detail-declared-count-unstable.html"),
			"utf8",
		);
		await page.route(
			"https://www.heb.com/my-account/order-history/HEB-DECLARED-UNSTABLE",
			async (route) => {
				await route.fulfill({ body: fixtureHtml, contentType: "text/html" });
			},
		);
