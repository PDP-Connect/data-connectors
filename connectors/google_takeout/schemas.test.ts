...
test("youtube_watch_history schema accepts a parser-built record", () => {
	const entry: WatchHistoryEntry = {
		...
		subtitles: [
			{ name: "A Channel", url: "https://www.youtube.com/channel/UC123" },
		],
	};
...
test("matchSidecarFilename finds the exact legacy sidecar", () => {
	const match = matchSidecarFilename("IMG_1234.jpg", [
		"IMG_1234.jpg.json",
		"other.json",
	]);
...
test("matchSidecarFilename finds a truncated supplemental-metadata sidecar by prefix", () => {
	const match = matchSidecarFilename(
		"a_very_long_original_filename_from_a_phone.jpg",
