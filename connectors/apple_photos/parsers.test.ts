...
import type { MediaHydrationResult } from "../../packages/polyfill-connectors/src/local-media-blob-hydration.ts";
import {
	advanceCursor,
	buildPhotoRecord,
	detectMimeType,
...
test("buildPhotoRecord: fallback id (no content hash) still differs across distinct files", () => {
	const fileA: DiscoveredFile = {
		path: "/tmp/export/x.jpg",
		sizeBytes: 10,
		mtimeIso: "2024-06-05T00:00:00.000Z",
	};
	const fileB: DiscoveredFile = {
		path: "/tmp/export/y.jpg",
		sizeBytes: 20,
		mtimeIso: "2024-06-06T00:00:00.000Z",
	};
...
test("advanceCursor: undefined prev → takes next", () => {
	assert.equal(
		advanceCursor(undefined, "2024-06-05T00:00:00.000Z"),
