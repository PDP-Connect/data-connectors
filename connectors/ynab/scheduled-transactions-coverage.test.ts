...
test("YNAB production pacing remains configured while fixture tests use the request seam", () => {
	...
	assert.notEqual(
		ctx.request,
		ynab,
		"fixture coverage must not route through production pacing",
	);
});
	...
	const restore = stubFetch({
		data: {
			...
			scheduled_transactions: [
				scheduledTxn(),
				scheduledTxn({ id: "88888888-8888-4888-8888-888888888888" }),
			],
		},
	});
	try {
		...
		assert.equal(
			fact.considered,
			2,
			"considered === records returned by the fresh call",
		);
		assert.equal(
			fact.covered,
			2,
			"every row validated + emitted, so covered === considered",
		);
		assert.equal(
			fact.enumeratedFresh,
			true,
			"no prior cursor -> fresh enumeration",
		);
		assert.equal(
			emitted.filter((r) => r.stream === "scheduled_transactions").length,
			2,
		);
	} finally {
		restore();
...
test("collectScheduledTransactions: fresh call with genuine zero reports enumeratedFresh with considered === covered === 0", async () => {
	const restore = stubFetch({
		data: { server_knowledge: 200, scheduled_transactions: [] },
	});
	try {
		const { ctx, emitted } = makeCtx(BUDGET_A, {});
		const fact = await collectScheduledTransactions(ctx);
		...
		assert.equal(
			fact.enumeratedFresh,
			true,
			"genuine zero is still a fresh, boundary-measuring call",
		);
...
test("collectScheduledTransactions: a fresh call where one row fails shape-check reports covered < considered, never aliased to the raw response length", async () => {
	...
	try {
		...
		assert.equal(
			emitted.filter((r) => r.stream === "scheduled_transactions").length,
			1,
			"only the valid row emits",
		);
		assert.equal(
			fact.considered,
			2,
			"considered is the full response length — the rejected row was still returned",
		);
		assert.equal(
			fact.covered,
			1,
...
test("collectScheduledTransactions: delta-only zero triggers a fresh reconciliation", async () => {
	const restore = stubFetch({
		data: { server_knowledge: 201, scheduled_transactions: [] },
	});
	try {
		const priorState = {
			scheduled_transactions: { [BUDGET_A]: { server_knowledge: 200 } },
		};
		...
		assert.equal(
			fact.enumeratedFresh,
			true,
			"a delta-only zero must be reconciled before it proves emptiness",
		);
		...
		assert.deepEqual(stateMsgs[0]?.cursor, {
			[BUDGET_A]: { server_knowledge: 201 },
		});
	} finally {
		restore();
	}
...
test("collectScheduledTransactions: a nonempty prior-cursor run stays incremental", async () => {
	...
	try {
		...
		assert.equal(
			fact.enumeratedFresh,
			false,
			"nonempty delta response remains incremental",
		);
		assert.equal(
			emitted.filter((r) => r.stream === "scheduled_transactions").length,
			1,
...
test("collect(): failed fetch never reaches the coverage emit (no DETAIL_COVERAGE, no STATE for scheduled_transactions)", async () => {
	...
	try {
		...
		assert.equal(
			messages.length,
			0,
			"a failed fetch emits neither STATE nor DETAIL_COVERAGE",
		);
	} finally {
		restore();
	}
