...
import { ynabPacingProfile } from "../../packages/polyfill-connectors/src/provider-profile.ts";
...
function requireYnabObject<T extends object = Record<string, unknown>>(
	value: unknown,
	field: string,
): T {
...
function parseCategoriesResponse(res: YnabCategoriesResponse): {
	...
	const categoryGroups = requireYnabArray<YnabCategoryGroup>(
	...
	for (const [groupIndex, group] of categoryGroups.entries()) {
		if (!isObjectRecord(group)) {
			throw new Error(
				`ynab_response_malformed: data.category_groups[${String(groupIndex)}] must be an object`,
			);
		}
		requireYnabArray<YnabCategory>(
			group.categories,
			`data.category_groups[${String(groupIndex)}].categories`,
		);
	}
	return {
...
function endpointLabel(path: string): string {
	return path
		.replace(BUDGET_ID_PATH_SEGMENT, "/budgets/{budget_id}")
		.replace(MONTH_PATH_SEGMENT, "/months/{month}");
}
...
export function createYnabRequest(
	governor: Pick<ConnectorHttpGovernor, "request">,
): YnabRequest {
	return async function request<T>(
		...
		try {
			...
			if (error instanceof Error) {
				throw new Error(`${error.message} [endpoint ${endpointLabel(path)}]`, {
					cause: error,
				});
			}
			throw error;
 ...
 */
export const BUDGET_FINGERPRINT_EXCLUDE = [
	"last_month",
	"last_modified_on",
] as const;
 ...
 */
export function openBudgetCursor(
	state: Record<string, unknown>,
): FingerprintCursor {
...
export function accountStatsRecord(
	a: YnabAccount,
	budgetId: string,
	observedOn: string,
): RecordData {
...
export function payeeLocationRecord(
...
export function monthCategoryRecord(
	c: YnabCategory,
	month: string,
	budgetId: string,
): RecordData {
...
function coverageForRecords(
	...
	return {
		...
		covered: records.reduce(
			(count, record) => count + (validateRecord(stream, record).ok ? 1 : 0),
			0,
		),
		enumeratedFresh,
	};
...
function aggregateCoverageFacts(
 ...
 */
export function openAccountCursor(
	state: Record<string, unknown>,
	budgetId: string,
): FingerprintCursor {
...
async function emitAccountsAndStats({
	...
	wantsEntity,
	wantsStats,
}: EmitAccountsAndStatsArgs): Promise<{
	accountStatsCovered: number;
	accountsCovered: number;
}> {
	...
	if (wantsEntity) {
		...
		const accounts =
			(newState.accounts as
				| Record<
						string,
						{ server_knowledge: number; fingerprints?: Record<string, string> }
				  >
				| undefined) ?? {};
		accounts[budgetId] = {
 ...
 * zero budgets ran).
 */
export function aggregateAccountsCoverage(
	facts: readonly AccountsBudgetFact[],
): {
	...
	const accounts =
		accountsFacts.length > 0
			? {
					...
					covered: accountsFacts.reduce(
						(sum, f) => sum + (f.accountsCovered ?? 0),
						0,
					),
				}
			: null;
	...
	const accountStats =
		statsFacts.length > 0
			? {
					...
					covered: statsFacts.reduce(
						(sum, f) => sum + (f.accountStatsCovered ?? 0),
						0,
					),
				}
			: null;
...
export async function collectCategoriesAndGroups(ctx: BudgetCtx): Promise<{
	...
	if (
		!enumeratedFresh &&
		(parsed.categoryGroups.length === 0 || parsed.categoryCount === 0)
	) {
		res = await request<YnabCategoriesResponse>(
			`/budgets/${budgetId}/categories`,
			token,
			{},
			progress,
	...
	return {
		categoryGroups: coverageForRecords(
			"category_groups",
			categoryGroupRecords,
			enumeratedFresh,
		),
		categories: coverageForRecords(
			"categories",
			categoryRecords,
			enumeratedFresh,
		),
...
async function collectPayees(ctx: BudgetCtx): Promise<CoverageFact> {
	...
	if (!enumeratedFresh && res.data.payees.length === 0) {
		res = await request<YnabPayeesResponse>(
			`/budgets/${budgetId}/payees`,
			token,
			{},
			progress,
 ...
 * maps.
 */
export function openPayeeLocationCursor(
	state: Record<string, unknown>,
	budgetId: string,
): FingerprintCursor {
...
async function collectPayeeLocations(
	ctx: BudgetCtx,
): Promise<PayeeLocationsBudgetFact> {
	const {
		budgetId,
		budgetOrdinal = 0,
		request,
	...
	const res = await request<YnabPayeeLocationsResponse>(
...
async function collectTransactions(ctx: BudgetCtx): Promise<CoverageFact> {
	...
	try {
		const aRes = await request<YnabAccountsResponse>(
			`/budgets/${budgetId}/accounts`,
			token,
			{},
			progress,
	...
	if (!enumeratedFresh && res.data.transactions.length === 0) {
		res = await request<YnabTransactionsResponse>(
			`/budgets/${budgetId}/transactions`,
			token,
			{},
			progress,
...
export async function collectScheduledTransactions(
	ctx: BudgetCtx,
): Promise<ScheduledTransactionsBudgetFact> {
	const {
		budgetId,
		budgetOrdinal = 0,
		request,
...
async function fetchMonthsIfNeeded(
	...
	if (!enumeratedFresh && res.data.months.length === 0) {
		res = await request<YnabMonthsResponse>(
			`/budgets/${budgetId}/months`,
			token,
			{},
			progress,
...
async function collectMonthStreams(ctx: BudgetCtx): Promise<MonthStreamFacts> {
	...
	const monthFetch = await fetchMonthsIfNeeded(
		ctx,
		Boolean(monthsStream || monthCategoriesStream),
	);
	if (!monthFetch) {
		return {};
	}
...
async function fetchMonthDetail(
	...
	const monthRes = await request<YnabMonthDetailResponse>(
...
export async function collectMonthCategories(
	...
	const priorCutoff = state.month_categories as
	...
	const activeMonths = monthList.filter((m) => {
		...
		return (
			fullScanForDetails ||
			lastFetchedMonth === undefined ||
			m.month >= lastFetchedMonth
		);
	});
	...
	let highestMonth: string | null = fullScanForDetails
		? scopeSince || null
		: lastFetchedMonth || scopeSince || null;
	const records: RecordData[] = [];
	for (let i = 0; i < activeMonths.length; i += 1) {
		...
		await progress("Fetching YNAB month categories window", {
			...
			total_seen: i,
			cursor_present: Boolean(
				(fullScanForDetails ? scopeSince : lastFetchedMonth) || scopeSince,
			),
		});
		...
		const monthCategories = requireYnabArray<YnabCategory>(
			monthObject.categories,
			"data.month.categories",
		);
...
async function collectForBudget(
	ctx: BudgetCtx,
): Promise<CollectForBudgetFacts> {
	const { requested } = ctx;
 ...
 * only coverage axis.
 */
export async function emitBudgetsStream(
	deps: BudgetsStreamDeps,
): Promise<void> {
...
function appendBudgetFacts(
	collection: YnabCoverageFacts,
	facts: CollectForBudgetFacts,
): void {
	if (facts.accounts) {
...
async function emitYnabCoverage(
	...
	if (requested.has("accounts") || requested.has("account_stats")) {
		const { accounts, accountStats } = aggregateAccountsCoverage(
			facts.accounts,
		);
		await emitWholeStreamCoverage(
			emit,
			"accounts",
			requested.has("accounts") ? accounts : null,
			true,
		);
		await emitWholeStreamCoverage(
			emit,
			"account_stats",
			requested.has("account_stats") ? accountStats : null,
			true,
		);
	}
	...
	if (requested.has("payee_locations")) {
		await emitWholeStreamCoverage(
			emit,
			"payee_locations",
			aggregatePayeeLocationsCoverage(facts.payeeLocations),
			true,
		);
	}
	if (requested.has("category_groups")) {
		await emitWholeStreamCoverage(
			emit,
			"category_groups",
			aggregateCoverageFacts(facts.categoryGroups),
		);
	}
	if (requested.has("categories")) {
		await emitWholeStreamCoverage(
			emit,
			"categories",
			aggregateCoverageFacts(facts.categories),
		);
	}
	if (requested.has("payees")) {
		await emitWholeStreamCoverage(
			emit,
			"payees",
			aggregateCoverageFacts(facts.payees),
		);
	}
	if (requested.has("transactions")) {
		await emitWholeStreamCoverage(
			emit,
			"transactions",
			aggregateCoverageFacts(facts.transactions),
		);
	}
	if (requested.has("months")) {
		await emitWholeStreamCoverage(
			emit,
			"months",
			aggregateCoverageFacts(facts.months),
		);
	}
	if (requested.has("month_categories")) {
		await emitWholeStreamCoverage(
			emit,
			"month_categories",
			aggregateCoverageFacts(facts.monthCategories),
		);
	}
	if (requested.has("scheduled_transactions")) {
		await emitWholeStreamCoverage(
			...
			facts.scheduledTransactions.length > 0 &&
				facts.scheduledTransactions.every((fact) => fact.enumeratedFresh),
		);
	}
}
...
export async function ynabCollect(
	...
	await progressWithCounters("Fetching budgets", {
		stream: "budgets",
		phase: "fetch",
		cursor_present: false,
	});
	const budgetsRes = await request<YnabBudgetsResponse>(
		"/budgets",
		token,
		{},
		progressWithCounters,
		{
			stream: "budgets",
			phase: "fetch",
			cursor_present: false,
	...
	for (
		let budgetOrdinal = 0;
		budgetOrdinal < budgetIds.length;
		budgetOrdinal += 1
	) {
...
if (isMainModule(import.meta.url)) {
	runConnector({
		...
		auth: {
			kind: "env",
			required: [["YNAB_PERSONAL_ACCESS_TOKEN", "YNAB_PAT"]],
		},
		validateRecord,
		collect: ynabCollect,
	});
