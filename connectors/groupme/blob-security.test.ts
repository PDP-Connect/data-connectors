...
function fakeAttachmentResponse(
...
function providerXmlError(
	status: 403 | 404,
	code: "AccessDenied" | "NoSuchKey",
): Response {
...
describe("GroupMe blob attachment security (production seam)", () => {
	describe("fetchAttachmentBlobOutcome", () => {
		...
		it("classifies a bounded provider AccessDenied response as unavailable", async () => {
			...
			const result = await fetchAttachmentBlobOutcome(
				"https://i.groupme.com/gone.jpg",
				"gone-403",
			);
		...
		for (const status of [403, 404]) {
			it(`keeps a bare HTTP ${status} response as an unproven failure`, async () => {
				...
				deepStrictEqual(result, {
					kind: "failed",
					reason: `attachment_http_${status}`,
				});
			});
		}
		...
		for (const [label, body] of [
			...
			[
				"nested Code",
				"<Error><Message><Code>NoSuchKey</Code></Message></Error>",
			],
			[
				"comment",
			...
			[
				"multiple roots",
				"<Error><Code>NoSuchKey</Code></Error><Error><Code>NoSuchKey</Code></Error>",
			],
			["attribute", '<Error><Code source="guess">NoSuchKey</Code></Error>'],
			[
				"processing instruction",
				"<Error><?guess value?><Code>NoSuchKey</Code></Error>",
			],
			[
				"nested field",
				"<Error><Message><Value>gone</Value></Message><Code>NoSuchKey</Code></Error>",
			],
			[
				"duplicate field",
				"<Error><Code>NoSuchKey</Code><Code>NoSuchKey</Code></Error>",
		...
		it("rejects a malformed Content-Length instead of accepting its integer prefix", async () => {
			...
			(global as any).fetch = async () =>
				providerXmlResponse(404, body, `${Buffer.byteLength(body)}junk`);
			const result = await fetchAttachmentBlobOutcome(
				"https://i.groupme.com/gone.jpg",
				"bad-length",
			);
		...
		it("retries an unproven attachment on the next invocation and hydrates it when the provider recovers", async () => {
			...
			const first = await fetchAttachmentBlobOutcome(
				"https://i.groupme.com/retry.jpg",
				"retry-first",
			);
			const second = await fetchAttachmentBlobOutcome(
				"https://i.groupme.com/retry.jpg",
				"retry-second",
			);
			...
			strictEqual(
				attempts,
				2,
				"the transient failure is not cached or converted into terminal absence",
			);
		});
	});
	...
	describe("validateAttachmentUrl (origin validation)", () => {
		...
		it("rejects https://i.groupme.com:8080 (non-default port)", () => {
			const result = validateAttachmentUrl(
				"https://i.groupme.com:8080/image.jpg",
			);
			strictEqual(result.valid, false);
		...
		it("allows https://i.groupme.com:443 (explicit default HTTPS port, URL-equivalent)", () => {
			...
			const result = validateAttachmentUrl(
				"https://i.groupme.com:443/image.jpg",
			);
			strictEqual(result.valid, true);
		});
		...
		it("rejects https://user:pass@i.groupme.com (userinfo present)", () => {
			const result = validateAttachmentUrl(
				"https://user:pass@i.groupme.com/image.jpg",
			);
			strictEqual(result.valid, false);
	...
	describe("fetchAttachmentBlob (streaming + size validation)", () => {
		...
		it("rejects a body whose actual byte count differs from Content-Length", async () => {
			(global as any).fetch = async () =>
				fakeAttachmentResponse({ "content-length": "5" }, Buffer.from("test"));
	...
	describe("fetchAttachmentBlob content-type capture", () => {
		...
		it("returns null contentType when the header is missing", async () => {
			(global as any).fetch = async () =>
				fakeAttachmentResponse({}, Buffer.alloc(10));
			const result = await fetchAttachmentBlob(
				"https://i.groupme.com/a.bin",
				"missing-header",
			);
		...
		it("returns null contentType for a malformed/unrecognized header value", async () => {
			(global as any).fetch = async () =>
				fakeAttachmentResponse(
					{ "content-type": "text/html; <script>evil()</script>" },
					Buffer.alloc(10),
				);
			const result = await fetchAttachmentBlob(
				"https://i.groupme.com/a.jpg",
			...
			strictEqual(
				result.contentType,
				null,
				"unrecognized/unsafe header values must not pass through",
			);
		});
		...
		it("does not fetch the attachment twice to determine content-type (single fetch call)", async () => {
			...
			strictEqual(
				callCount,
				1,
				"must resolve content-type from the same fetch that downloads the body",
			);
		});
	});
	...
	describe("normalizeAttachmentContentType (safe header normalization)", () => {
		...
		it("accepts application/pdf and application/octet-stream for file attachments", () => {
			strictEqual(
				normalizeAttachmentContentType("application/pdf"),
				"application/pdf",
			);
			strictEqual(
				normalizeAttachmentContentType("application/octet-stream"),
		...
		it("strips parameters and lowercases before matching", () => {
			strictEqual(
				normalizeAttachmentContentType("IMAGE/PNG; charset=binary"),
				"image/png",
			);
		});
		...
		it("returns null for an unrecognized MIME type not in the safe set", () => {
			strictEqual(
				normalizeAttachmentContentType("application/x-shellscript"),
				null,
			);
