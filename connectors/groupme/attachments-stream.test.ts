...
describe("GroupMe attachments stream (production seam)", () => {
	describe("attachmentRecordId (record_key uniqueness)", () => {
		it("derives distinct record keys for two different attachments in the same message", () => {
			const first = attachmentRecordId(
				"msg.1",
				0,
				"https://i.groupme.com/a.jpg",
			);
			const second = attachmentRecordId(
				"msg.1",
				1,
				"https://i.groupme.com/b.jpg",
			);
			assert.notStrictEqual(
				first,
				second,
				"distinct attachments must not collapse onto one record_key",
			);
		});

		it("derives distinct record keys for the same-position attachment across different messages", () => {
			const messageOne = attachmentRecordId(
				"msg.1",
				0,
				"https://i.groupme.com/a.jpg",
			);
			const messageTwo = attachmentRecordId(
				"msg.2",
				0,
				"https://i.groupme.com/a.jpg",
			);
			assert.notStrictEqual(
				messageOne,
				messageTwo,
				"record_key must be scoped to the owning message",
			);
		});
		...
		it("is stable (reproducible) for the same message/index/url across runs", () => {
			...
			strictEqual(
				a,
				b,
				"re-collecting the same message must derive the same record_key (dedup-safe)",
			);
		});
		...
		it("never reproduces the pre-fix constant-per-type record_key shape", () => {
			...
			assert.notStrictEqual(
				id,
				"attachment:image",
				"must not regress to the old type-only record_key",
			);
		});
	});
	...
	describe("attachmentContentType (MIME type correctness)", () => {
		...
		it("does NOT send the bogus 'image/file' MIME type for file attachments", () => {
			const mimeType = attachmentContentType({
				type: "file",
				url: "https://i.groupme.com/a.pdf",
			});
	...
	describe("resolveUploadMimeType (prefer observed Content-Type over the guess)", () => {
		it("prefers a real observed PNG content-type over the image/jpeg guess", () => {
			strictEqual(
				resolveUploadMimeType("image/png", "image/jpeg"),
				"image/png",
			);
		});
		...
		it("prefers a real observed GIF content-type over the image/jpeg guess", () => {
			strictEqual(
				resolveUploadMimeType("image/gif", "image/jpeg"),
				"image/gif",
			);
		});
		...
		it("falls back to the guess when the observed header failed normalization (malformed/invalid)", () => {
			strictEqual(
				resolveUploadMimeType(null, "application/octet-stream"),
	...
	describe("normalizeOneAttachment / normalizeAttachments (attachments stream emission)", () => {
		it("emits an 'attachments' record for an image attachment when the stream is requested", async () => {
			...
			deepStrictEqual(record.blob_ref, {
				blob_id: "blob_1",
				mime_type: "image/jpeg",
				sha256: "sha1",
				size_bytes: 10,
		...
		it("does not emit an attachments record when the stream is not requested (emitAttachmentRecord undefined)", async () => {
			...
			strictEqual(
				result.blob_id,
				"blob_1",
				"inline blob_id embedding is preserved regardless of stream request",
			);
		});
		...
		it("records hydration_status=failed with a preserved error when the uploader throws", async () => {
			...
			const uploader = () =>
				Promise.reject(
					new Error(
						"blob upload failed (404): Stream 'attachments' not found for connector groupme",
					),
				);
			...
			strictEqual(
				record.blob_id,
				undefined,
				"normalized attachment must not claim a blob_id on failure",
			);
			assert.match(
				String(record.hydration_error),
				/Stream 'attachments' not found/,
			);
		});
		...
		it("preserves the legacy null uploader result as deferred hydration", async () => {
			...
			await normalizeOneAttachment(
				{
					type: "image",
					url: "https://i.groupme.com/deferred.jpg",
					name: null,
				},
				0,
				"msg.1",
		...
		it("emits one distinct attachments record per attachment in a multi-attachment message", async () => {
			...
			strictEqual(
				new Set(ids).size,
				2,
				"each attachment must get a distinct record id",
			);
		});
	});
});
