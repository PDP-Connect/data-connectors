...
): Parameters<typeof providerMessageCount>[0] {
	return { id: GROUP_ID, ...overrides } as Parameters<
		typeof providerMessageCount
	>[0];
}
...
test("providerMessageCount prefers the nested count when both shapes are present", () => {
	assert.equal(
		providerMessageCount(
			group({ messages: { count: 142 }, messages_count: 7 }),
		),
		142,
	);
});
...
test("providerMessageCount rejects a malformed count rather than coercing it", () => {
	...
	assert.equal(
		providerMessageCount(group({ messages_count: Number.NaN })),
		null,
	);
	assert.equal(
		providerMessageCount(group({ messages_count: Number.POSITIVE_INFINITY })),
		null,
	);
});
...
test("groupMessageShortfall reports ok on an exact match", () => {
	assert.equal(
		groupMessageShortfall(group({ messages: { count: 320 } }), 320).kind,
		"ok",
	);
});
...
test("groupMessageShortfall does NOT report a gap when holdings exceed the provider count", () => {
	...
	assert.equal(
		groupMessageShortfall(group({ messages: { count: 300 } }), 320).kind,
		"ok",
	);
});
...
test("groupMessageShortfall treats a provider zero as a real anchor, not as unanchored", () => {
	assert.equal(
		groupMessageShortfall(group({ messages: { count: 0 } }), 0).kind,
		"ok",
	);
});
...
test("groupMessageShortfall reports a gap for an empty walk against a non-zero count", () => {
	...
	assert.equal(verdict.kind, "short");
	assert.equal(
		verdict.kind === "short" ? verdict.shortfall.providerCount : null,
		12,
	);
});
...
test("partitionGroupMessageShortfalls preserves every message of the gap across both buckets", () => {
	...
	const before = shortfalls.reduce(
...
test("pageEndedShortOfProviderCount flags an empty page served against a non-zero count", () => {
	assert.equal(
		pageEndedShortOfProviderCount({ count: 29, messages: [] }),
		true,
	);
});
...
test("pageEndedShortOfProviderCount accepts an empty page whose count agrees at zero", () => {
	...
	assert.equal(
		pageEndedShortOfProviderCount({ count: 0, messages: [] }),
		false,
	);
});
...
test("pageEndedShortOfProviderCount never flags a page that actually served messages", () => {
	...
	>[0]["messages"];
	assert.equal(
		pageEndedShortOfProviderCount({ count: 900, messages: served }),
		false,
	);
	assert.equal(
		pageEndedShortOfProviderCount({ count: 2, messages: served }),
		false,
	);
});
...
test("pageEndedShortOfProviderCount treats a missing/non-numeric count as no shortfall", () => {
	...
	const noCount = { messages: [] } as unknown as Parameters<
		typeof pageEndedShortOfProviderCount
	>[0];
	assert.equal(pageEndedShortOfProviderCount(noCount), false);
	const nullCount = { count: null, messages: [] } as unknown as Parameters<
		typeof pageEndedShortOfProviderCount
	>[0];
...
test("groupMessageShortfall carries the unproven-boundary flag into the shortfall", () => {
	...
	const verdict = groupMessageShortfall(
		group({ messages: { count: 29 } }),
		0,
		true,
	);
	assert.equal(verdict.kind, "short");
	assert.equal(
		verdict.kind === "short" ? verdict.shortfall.unprovenBoundary : null,
		true,
	);
});
