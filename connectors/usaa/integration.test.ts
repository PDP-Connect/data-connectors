...
import type { BodyResponseDiagnostics } from "../../packages/polyfill-connectors/src/browser-artifact-response.ts";
...
function makeSlowRenderingSelectPage(): {
	page: Page;
	selectCountCalls: () => number;
	selectWaitCount: () => number;
} {
			...
			if (selector === '[role="dialog"]') {
				return {
					...
					locator(childSelector: string) {
						return childSelector.includes("selectionType")
							? selectLocator
							: dateLocator;
					},
				};
	...
	return {
		page,
		selectCountCalls: () => selectCountCalls,
		selectWaitCount: () => selectWaitCount,
	};
}
...
test("driveExport treats a slow-but-real date-range select render as ready, not a structure change", async () => {
	...
	assert.equal(
		probe.selectWaitCount(),
		1,
		"the select readiness wait must run",
	);
	assert.equal(
		probe.selectCountCalls(),
		1,
		"the rendered select must be counted after waiting",
	);
});
