...
import type { StatementHydration } from "../../packages/polyfill-connectors/src/statement-hydration-carry-forward.ts";
import {
...
const NULL_POINTERS: StatementHydration = {
	document_url: null,
	pdf_path: null,
	pdf_sha256: null,
};
...
function nonCandidate(
	id: string,
	pointers: StatementHydration = NULL_POINTERS,
): StatementCoverageRow {
...
test("fully hydrated run: required === hydrated, no gaps", () => {
	const result = computeStatementCoverage([
		candidate("s1", HYDRATED),
		candidate("s2", HYDRATED),
	]);
	...
	assert.equal(
		result.coverage.gapKeys,
		undefined,
		"no gap_keys when fully hydrated",
	);
...
test("required === hydrated ∪ gap for every run (runtime coverage-completeness invariant)", () => {
	...
	for (const key of coverage.requiredKeys) {
		assert.ok(
			union.has(key),
			`required key ${String(key)} must be hydrated or a gap`,
		);
	}
});
...
test("non-statement rows (disclosures/agreements) are excluded from the denominator", () => {
	...
	assert.deepEqual(
		result.coverage.requiredKeys,
		["stmt"],
		"only the statement is a candidate",
	);
...
test("zero statement candidates => candidateCount 0 with empty key sets (the pure compute step, not the emit decision)", () => {
	const result = computeStatementCoverage([
		nonCandidate("disclosure"),
		nonCandidate("agreement"),
	]);
...
test("duplicate ids de-duplicate; a later hydrated occurrence upgrades a gap", () => {
	...
	const result = computeStatementCoverage([
		candidate("dup", NULL_POINTERS),
		candidate("dup", HYDRATED),
	]);
	assert.deepEqual(
		result.coverage.requiredKeys,
		["dup"],
		"id appears once in required_keys",
	);
...
test("redaction: no title or account text ever appears in coverage or gap output", () => {
	...
	assert.doesNotMatch(
		serialized,
		/CHECKING|SAVINGS|\bUSAA\b|\*\d{4}|\b[A-Z][a-z]+ \d{4}\b/,
	);
