...
const MODERN_ROW_BREAK_RE =
	/^\s*(Page\s+\d+\s+of\s+\d+|--\s*\d+\s+of\s+\d+\s*--|Date\s+Description\s.*Balance)\s*$/i;
...
export function safeAccountSlug(
 ...
 */
export function currencyToCentsFromStatement(
...
function resolveClosingContext(
	ctx: ClosingContext,
...
function csvRowToTransaction({
	...
	const original =
		idx.idxOrig >= 0
			? (row[idx.idxOrig] ?? "").trim() || description
			: description;
	...
	const categoryRaw =
		idx.idxCat >= 0 ? (row[idx.idxCat] ?? "").trim() || null : null;
 ...
 */
export function parseModernCheckingEra(
	text: string,
	{ closing }: EraParseArgs,
): ParsedStatementTxn[] {
 ...
 */
export function parseCreditCardEra(
	text: string,
	{ closing }: EraParseArgs,
): ParsedStatementTxn[] {
	const lines = text.split(STMT_LINE_SPLIT_RE);
...
function stableAccountName(a: DashboardAccount): string | null {
	...
	const stable = a.name
		.replace(EXTERNAL_ACCOUNT_FRESHNESS_SUFFIX_RE, "")
		.trim();
 ...
 */
export function buildAccountStatsRecord(
	a: DashboardAccount,
	observedOn: string,
): AccountStatsRecord {
