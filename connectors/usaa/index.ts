...
import { ensureUsaaSession } from "../../packages/polyfill-connectors/src/auto-login/usaa.ts";
import {
...
import {
	attachDownloadQueue,
	type DownloadQueue,
} from "../../packages/polyfill-connectors/src/download-queue.ts";
import {
	type FingerprintCursor,
	openFingerprintCursor,
} from "../../packages/polyfill-connectors/src/fingerprint-cursor.ts";
import { readPlaywrightDownloadBufferDetailed } from "../../packages/polyfill-connectors/src/playwright-download.ts";
import { statementFingerprintExcludeKeys } from "../../packages/polyfill-connectors/src/statement-content-fingerprint.ts";
import {
 ...
 */
async function recoverServedStatementGaps(
	deps: EmitDeps,
	hydratedKeys: readonly (string | number)[],
): Promise<void> {
 ...
 */
export function resolveUsaaOfferInterstitialEscape(
	finalUrl: string,
	accountUrl: string,
): string | null {
	let parsed: URL;
 ...
 */
async function settleOnAccountPage(
	page: Page,
	accountUrl: string,
...
async function openExportDialog(
	...
	const selectCount = rendered
		? await selectLocator.count().catch((): number => 0)
		: 0;
	...
	const hasActionableDateRange =
		(await isVisibleActionableField(startInput)) &&
 ...
 */
export function buildServedCreditCardGapLookups(
	detailGaps: readonly BrowserCollectContext["detailGaps"][number][],
): {
...
async function processAccountTransactions(
	...
	if (!csvPath) {
		...
		const errorClass =
			lastDiag && isFatalDiagPhase(lastDiag)
				? "source_structure_changed"
				: "export_no_download";
		return {
			last_date: priorLastDate,
	...
	try {
		...
		for (const a of cards) {
			...
			if (outcome === "ok") {
				...
				await recoverServedCreditCardGaps(
					deps,
					cardId,
					deps.servedCreditCardGaps,
					{ emitEntity, emitStats },
				);
				continue;
			}
