...
import {
	...
	readPriorStatementFingerprints,
} from "./index.ts";
...
function nextStateFrom(messages: EmittedMessage[]): Record<string, unknown> {
	const state = messages
		.filter((m) => m.type === "STATE" && m.stream === "statements")
		.at(-1);
	return {
		statements:
			(state as { cursor?: Record<string, unknown> } | undefined)?.cursor ?? {},
	};
}
...
test("statements: re-emitting with only a new fetched_at is fully suppressed", async () => {
	const indexRows = [
		makeIndexRow({ id: "S1" }),
		makeIndexRow({ id: "S2", rowIndex: 1, title: "May 2026 STATEMENT" }),
	];
	const hydration = new Map<number, HydrationResult>([
		[
			0,
			makeHydrationOk({
				pdfPath: "/tmp/usaa/2026-04-aaaa.pdf",
		...
		[
			1,
			makeHydrationOk({
				pdfPath: "/tmp/usaa/2026-05-bbbb.pdf",
	...
	assert.equal(
		run2.emitted.length,
		0,
		"unchanged statements are fully suppressed on the second run",
	);
});
...
test("statements: a newly-hydrated statement re-emits (index-only → hydrated is a real change)", async () => {
	...
	const failed = new Map<number, HydrationResult>([
		[0, { err: "download failed" }],
	]);
	const run1 = makeHarness();
	const cursor1 = openFingerprintCursor(undefined, {
		excludeFromFingerprint: ["fetched_at"],
	...
	assert.equal(
		run2.emitted.length,
		1,
		"hydration appearing is a real change and re-emits",
	);
});
...
test("readPriorStatementFingerprints: tolerates missing / legacy / malformed state", () => {
	assert.equal(
		readPriorStatementFingerprints({}).size,
		0,
	...
	const ok = readPriorStatementFingerprints({
		statements: { fingerprints: { S1: "fp-1", bad: null } },
	});
...
function openCursors(priorState: Record<string, unknown>): {
	...
	return {
		...
		hydration: openStatementHydrationCursor(
			readPriorStatementHydration(
				(priorState as { statements?: unknown }).statements,
			),
		),
	};
}
...
async function runEmit(
	...
	return {
		...
		nextState: nextStateFrom(harness.messages),
	};
}
...
test("AC-1: previously hydrated + transient failure carries pointers forward (no flap)", async () => {
	...
	assert.equal(
		b.emitted.length,
		0,
		"run B emits NO new version — prior pointers carried forward",
	);
	...
	assert.equal(
		carried.get("S1")?.pdf_path,
		"/tmp/usaa/p.pdf",
		"carried pdf_path survives the failed run",
	);
...
test("AC-2: never-hydrated statement that fails stays all-null (honest), then first hydration versions once", async () => {
	...
	const aRec = a.emitted.find((r) => r.stream === "statements")?.data as Record<
		string,
		unknown
	>;
	...
	assert.equal(
		readPriorStatementHydration(a.nextState.statements).size,
		0,
		"nothing to carry",
	);
	...
	assert.equal(
		b.emitted.length,
		1,
		"first hydration is a real change and versions exactly once",
	);
});
...
test("AC-3: a genuine identity/title change still re-versions (carry-forward never masks it)", async () => {
	...
	const a = await runEmit(
		[makeIndexRow({ id: "S1", title: "April 2026 STATEMENT" })],
		ok,
		{},
	);
	...
	const b = await runEmit(
		[makeIndexRow({ id: "S1", title: "April 2026 STATEMENT (amended)" })],
		ok,
		a.nextState,
	);
	assert.equal(
		b.emitted.length,
		1,
		"a retitled statement is real history and re-versions",
	);
});
...
test("AC-4: flap-back is idempotent — hydrate, fail, re-hydrate identical → one version total", async () => {
	...
	assert.equal(
		c.emitted.length,
		0,
		"run C re-hydrates the identical PDF — still no new version",
	);
});
...
test("AC-6: a terminal removal is NOT hidden as a carried-forward success", async () => {
	...
	const b = await runEmit(
		rowsB,
		new Map([[0, makeHydrationOk()]]),
		a.nextState,
	);
	const carriedAfterB = readPriorStatementHydration(b.nextState.statements);
	assert.equal(
		carriedAfterB.has("S2"),
		false,
		"a delisted statement is pruned, never carried forward as a phantom",
	);
	assert.equal(
		carriedAfterB.has("S1"),
		true,
		"the still-listed statement is retained",
	);
});
