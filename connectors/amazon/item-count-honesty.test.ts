...
test("amazon item_count: an unfetched detail page reports unknown, not an empty order", () => {
	...
	assert.equal(
		rec.item_count,
		null,
		"no surface saw an item and none was fetched — the count is unknown",
	);
	assert.notEqual(
		rec.item_count,
		0,
		"0 would claim a proven-empty order we never actually looked at",
	);
});
...
test("amazon item_count: a fetched detail page with no items is a proven-empty order", () => {
	...
	assert.equal(
		rec.item_count,
		0,
		"a successful detail fetch that found nothing proves zero",
	);
});
...
test("amazon item_count: the list card alone still counts when no detail was fetched", () => {
	...
	assert.equal(
		rec.item_count,
		2,
		"observed items are observed regardless of which surface showed them",
	);
});
...
test("amazon item_count: the larger of the two surfaces is kept", () => {
	const rec = build(
		...
		makeDetail({
			items: [makeDetailItem({ asin: "B01" }), makeDetailItem({ asin: "B02" })],
		}),
	);
	...
	assert.equal(
		rec.item_count,
		2,
		"the detail page saw more than the list card",
	);
});
...
test("amazon item_count: a null count is accepted by the orders schema", () => {
	...
	assert.equal(
		result.ok,
		true,
		`a null item_count must validate: ${JSON.stringify(result.ok ? [] : result.issues)}`,
	);
});
