...
import { PreflightWaitProbe } from "../../packages/polyfill-connectors/src/send-governor.ts";
...
async function runDetailPass(convoCount: number): Promise<RunResult> {
	...
	const providerBudget = new ProviderBudgetController({
		pacing: {
			...
			minIntervalMs: 250,
			now,
			sleep: (ms) => Promise.resolve(sleep(ms)),
		},
		pacingMode: "signal",
	});
	...
	const convos = Array.from({ length: convoCount }, (_, i) =>
		makeConvo(`convo-${i + 1}`),
	);
...
test("send governor: exactly one pre-flight wait source per request (no stacking)", async () => {
	...
	const result = await runDetailPass(3);
	assert.equal(
		result.preflightWaitCount,
		3,
		"one pre-flight wait per request, no second gate",
	);
});
...
test("send governor: resolveChatGptProviderBudget always yields signal mode (no flag)", () => {
	...
	const controller = resolveChatGptProviderBudget({
		PDPP_CHATGPT_DETAIL_RATE_LIMIT_STOP_AFTER: "5",
	});
	assert.ok(controller instanceof ProviderBudgetController);
	assert.equal(
		controller.pacingMode,
		"signal",
		"controller always in signal mode — lane owns the wait",
	);
});
