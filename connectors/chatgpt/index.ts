...
} from "../../packages/polyfill-connectors/src/auto-login/chatgpt.ts";
import { browserConfigPreservationFor } from "../../packages/polyfill-connectors/src/browser-surface-policy.ts";
import {
...
function scrubChatGptTerminalDiagnostic(message: string): string {
	return message
		...
		.replace(
			/access[_-]?token["']?\s*[:=]\s*["']?[A-Za-z0-9._~+/=-]+/giu,
...
async function getAuthFromPage(page: Page): Promise<ChatGptAuth> {
	...
	const auth = (await page.evaluate(
		CHATGPT_AUTH_EXTRACTION_EXPRESSION,
	)) as ChatGptAuth;
...
const CHATGPT_RATE_LIMIT_DENSITY_STOP_ENV =
 ...
 */
export function resolveChatGptRateLimitDensityStop(
 ...
 */
export function resolveChatGptMaxDetailFetchesPerRun(
	env: NodeJS.ProcessEnv = process.env,
): number {
 ...
 * of a single run.
 */
export function resolveChatGptMaxRunWallClockMs(
	env: NodeJS.ProcessEnv = process.env,
): number {
 ...
 */
export function resolveChatGptMaxTailDeferralGapsPerRun(
	env: NodeJS.ProcessEnv = process.env,
): number {
...
function resolveChatGptRetryBudgetCapacity(
...
function resolveChatGptRetryBudgetInitialTokens(
	env: NodeJS.ProcessEnv,
): number {
	...
	return Number.isFinite(parsed) && parsed > 0
		? Math.floor(parsed)
		: CHATGPT_DEFAULT_RETRY_BUDGET_INITIAL_TOKENS;
}
 ...
 */
function resolveChatGptPacingRecoveryGain(
	env: NodeJS.ProcessEnv,
): number | null {
...
function resolveChatGptPacingMaxIntervalMs(env: NodeJS.ProcessEnv): number {
	const override = resolvePositiveFiniteMs(
		env,
		CHATGPT_PACING_MAX_INTERVAL_MS_ENV,
	);
...
function chatGptWarmStartPacingFields(
	persistedPacing?:
		| { intervalMs: number; recordedAtMs?: number }
		| number
		| null,
): {
	...
	const persisted =
		typeof persistedPacing === "number"
			? { intervalMs: persistedPacing }
			: (persistedPacing ?? null);
	if (
		persisted === null ||
		!Number.isFinite(persisted.intervalMs) ||
		persisted.intervalMs <= 0
	) {
		return {};
	}
...
export function resolveChatGptProviderBudget(
	...
	const pacingInitialOverride =
		env[CHATGPT_PACING_INITIAL_INTERVAL_MS_ENV]?.trim();
	...
	const retryBudgetCapacity = resolveChatGptRetryBudgetCapacity(
		env,
		maxRequests,
	);
	...
	const minIntervalMs =
		resolvePositiveFiniteMs(env, CHATGPT_PACING_MIN_INTERVAL_MS_ENV) ??
		CHATGPT_DEFAULT_PACING_MIN_INTERVAL_MS;
	const burstToleranceMs =
		initialIntervalMs === null
			? null
			: (resolvePositiveFiniteMs(env, CHATGPT_PACING_BURST_TOLERANCE_MS_ENV) ??
				2 * initialIntervalMs);
...
export function readChatGptPersistedPacing(
	state: CollectContext["state"] | undefined,
): ChatGptPersistedPacing | null {
	const messages = (state as { messages?: Record<string, unknown> } | undefined)
		?.messages;
	if (!messages || typeof messages !== "object") {
		return null;
	}
	const intervalMs = (messages as Record<string, unknown>)[
		CHATGPT_PACING_STATE_INTERVAL_KEY
	];
	const recordedAtMs = (messages as Record<string, unknown>)[
		CHATGPT_PACING_STATE_RECORDED_AT_KEY
	];
	if (
...
export function buildChatGptPacingStateFields(
	...
	const persistedIntervalMs = Math.min(
		snapshot.intervalMs,
		snapshot.initialIntervalMs,
	);
	return {
...
export class ChatGptRunBudget {
	...
	constructor(
		...
		this.inner = new RunBudget({
			...(options.maxFetches === undefined
				? {}
				: { maxRequests: options.maxFetches }),
...
export const CHATGPT_RETRYABLE_ERROR_PATTERN =
	/ECONN|ETIMEDOUT|fetch failed|429|retry budget exhausted/i;
const CHATGPT_BACKEND_FETCH_TIMEOUT_ENV =
...
const CHATGPT_CONVERSATION_DETAIL_PATH_PATTERN =
	/^\/conversation\/[^/?#]+(?:[?#].*)?$/;
const URL_QUERY_OR_FRAGMENT_PATTERN = /[?#].*$/;

export type ChatGptRunCapReason =
	| "circuit_open"
	| "max_detail_fetches"
	| "max_wall_clock"
	| "provider_retry_budget";

export type ChatGptRetryExhaustedClass =
	| "rate_limited"
...
export class ChatGptPlannedProviderBudgetDeferredError extends Error {
	...
	constructor(
		message: string,
		reason: ChatGptRunCapReason,
		gate: (ProviderBudgetGate & { ok: false }) | null = null,
	) {
		super(message);
...
export function resolveChatGptBackendFetchTimeoutMs(
	env: NodeJS.ProcessEnv = process.env,
): number {
 ...
 */
function isChatGptTransientCircuitDefer(
	err: unknown,
): err is ChatGptPlannedProviderBudgetDeferredError {
	return (
		err instanceof ChatGptPlannedProviderBudgetDeferredError &&
		err.reason === "circuit_open"
	);
}

function classifyRetryExhaustedStatus(
	status: number | null,
): ChatGptRetryExhaustedClass {
	if (status === 429) {
		return "rate_limited";
	}
...
function providerBudgetRetryTokensForProgress(
	value: number,
...
function providerBudgetTransitionProgress(
	transition: ProviderBudgetCircuitTransition,
): ProviderBudgetProgress {
	const retryTokensRemaining = providerBudgetRetryTokensForProgress(
		transition.retryTokensRemaining,
	);
...
function makeChatGptNetworkPressureDiagnostic({
	...
	const response =
		cause && typeof cause === "object"
			? (cause as { headers?: Record<string, string>; status?: number })
			: {};
	const status =
	...
	return {
		...
		...(attempts === undefined
			? {}
			: { attempt: attempts, max_attempts: attempts }),
		...(status === undefined ? {} : { status }),
		...(retryAfterMs === null
			? {}
			: {
					retry_after_ms: retryAfterMs,
					safe_headers: { "retry-after-ms": retryAfterMs },
				}),
	};
}
...
export function createChatGptApi({
	...
	function fetchWithRetry(
		...
		let plannedProviderBudgetDefer: ChatGptPlannedProviderBudgetDeferredError | null =
			null;
		...
		return retryHttp({
			...
			beforeAttempt: async () => {
				plannedProviderBudgetDefer =
					await admitChatGptProviderBudgetAttempt(providerBudget);
				await emitChatGptProviderBudgetTransitions({ emit, providerBudget });
			...
			onRetry: async ({
				...
				const recordPacingThrottle =
					!pacingThrottleRecordedForThisRequest &&
					isChatGptRetryableStatus(response?.status);
			...
			request: async () => {
				try {
					...
					if (
						captureResult &&
						capture &&
						!isChatGptRetryableStatus(result.status)
					) {
			...
			if (err instanceof TerminalHttpStatusError) {
				throw new Error(
					`apiFetch got ${err.status} on ${method} ${path} (auth - not retryable)`,
				);
			}
	...
	return {
		...
		async fetchBatch(ids: readonly string[]): Promise<ChatGptFetchResult[]> {
			...
			if (result.status !== 200 || !Array.isArray(json)) {
				throw new Error(
					`chatgpt_batch_detail_unavailable: status=${result.status}`,
				);
			}
			return json
				.filter(
...
function isChatGptSideEffectProbeEnabled(
	env: NodeJS.ProcessEnv = process.env,
): boolean {
...
function sameOrder(
	a: ChatGptConversationProbeItem[],
	b: ChatGptConversationProbeItem[],
): boolean {
	return (
		a.map((item) => item.id).join("\n") === b.map((item) => item.id).join("\n")
	);
}
...
export function summarizeChatGptSideEffectProbe(
	result: ChatGptSideEffectProbeResult,
): string {
	if (!result.ok) {
	...
	const { after1Target, after2Target, beforeTarget, targetId } =
		getProbeTargets(result);
	...
	const updateChanged = probeValueChanged(
		beforeUpdate,
		after1Update,
		after2Update,
	);
...
export async function runMemoriesStream(deps: StreamDeps): Promise<void> {
	...
	const classified = classifyChatGptListPage<RawMemoryEntry>(
		await deps.api.fetch(MEMORIES_PATH),
		{
			stream: "memories",
	...
	deps.emit(
		buildChatGptRejectAwareFullScanCoverageMessage(
			"memories",
			entries.length,
			covered,
		),
...
export async function runCustomInstructionsStream(
	...
	const record = buildCustomInstructionsRecord(
		res.json as RawCustomInstructionsBody,
	);
 ...
 */
function findDanglingBranchParent(
	mapping: Record<string, ChatGptNode>,
	currentNode: string,
): string | null {
...
export async function runSharedConversationsStream(
	...
	if (!sawError) {
		...
		deps.emit({
			...
			cursor: {
				fetched_at: nowIso(),
				fingerprints: fingerprintCursor.toState(),
			},
		});
...
async function selectConversationListsForRequestedStreams({
	...
	if (wantsConversations && wantsMessages) {
		const sharedCursor = oldestConversationCursor(
			priorConversationsCursor,
			priorMessagesCursor,
		);
...
const CHATGPT_DETAIL_INITIAL_CONCURRENCY_ENV =
	"PDPP_CHATGPT_DETAIL_INITIAL_CONCURRENCY_PROBE";
const CHATGPT_DETAIL_MAX_CONCURRENCY_ENV =
	"PDPP_CHATGPT_DETAIL_MAX_CONCURRENCY_PROBE";
const CHATGPT_DETAIL_PAUSE_MIN_MS_ENV =
	"PDPP_CHATGPT_DETAIL_PAUSE_MIN_MS_PROBE";
const CHATGPT_DETAIL_PAUSE_MAX_MS_ENV =
...
function resolvePositiveMsOverride(
	raw: string | undefined,
	fallback: number,
): number {
	const trimmed = raw?.trim();
 ...
 */
export function resolveChatGptDetailLaneTuning(
	env: NodeJS.ProcessEnv = process.env,
): ChatGptDetailLaneTuning {
	...
	const pauseMinMs = resolvePositiveMsOverride(
		env[CHATGPT_DETAIL_PAUSE_MIN_MS_ENV],
		CONVO_DETAIL_PAUSE_MIN_MS,
	);
	const pauseMaxMs = Math.max(
		pauseMinMs,
		resolvePositiveMsOverride(
			env[CHATGPT_DETAIL_PAUSE_MAX_MS_ENV],
			CONVO_DETAIL_PAUSE_MAX_MS,
		),
	);
...
async function classifyChatGptBurstPressure(
	...
	const results = await Promise.allSettled(
		ids.map((id) => fetchChatGptPressureProbeStatus(deps, id)),
	);
	let rateLimited = 0;
...
export async function classifyChatGptSourcePressure(
	...
	const burst = await classifyChatGptBurstPressure(
		deps,
		probeIds,
		burstConcurrency,
	);
...
export async function applyChatGptColdStatePreflight(
	...
	const probeIds = convosToSync
		.slice(0, CHATGPT_PREFLIGHT_PROBE_COUNT)
		.map((c) => c.id);
	if (!probeIds.length) {
		return requestedTuning;
	}
...
function shouldEmitConversationDetailLaneProgress(
	...
	if (
		...
		event.reason === "upstream_pressure_deferred"
	) {
		return false;
	}
	return true;
...
function formatConversationDetailLaneProgress(
	event: AdaptiveLaneEvent,
): string {
	const parts = [
...
function safeConversationListItemHint(
	c: ConversationListItem,
	...
	return {
		...
		create_time:
			typeof c.create_time === "string" || typeof c.create_time === "number"
				? c.create_time
				: null,
		update_time:
			typeof c.update_time === "string" || typeof c.update_time === "number"
				? c.update_time
				: null,
...
function conversationListItemFromGap(
	...
	const id =
		typeof locator.conversation_id === "string"
...
function makeDeferredConversationDetailGap(
	...
	return buildDetailGap({
		...
		error: {
			...
			...(observedPressure.httpStatus === null
				? {}
				: { httpStatus: observedPressure.httpStatus }),
			...(networkPressure === undefined ? {} : { networkPressure }),
		},
	});
}
 ...
 */
function makeRateLimitDensityPressureError(
	observedRateLimited: number,
 ...
 */
function chatGptRunCapReasonFromProviderGate(
	gate: ProviderBudgetGate & { ok: false },
): ChatGptRunCapReason {
...
const CHATGPT_CONVERSATION_BACKLOG_RECORD_KEY =
	"__chatgpt_conversation_backlog__";
const CHATGPT_CONVERSATION_BACKLOG_LOCATOR_KIND =
	"chatgpt.conversation_backlog";
...
function backlogGapBeforeUpdateTime(
	gap: CollectContext["detailGaps"][number],
): string | null {
	const locator = gap.detail_locator;
	...
	const before = (locator as { before_update_time?: unknown })
		.before_update_time;
...
function makeConversationListCoverage(
	considered: number,
	covered = considered,
): DetailCoverageMessage {
	return buildDetailCoverageMessage({
...
export async function runMessagesAndConversationsWithDetail(
	...
	const tuning = await applyChatGptColdStatePreflight(
		deps,
		convosToSync,
		requestedTuning,
	);
	const coverage: ConversationDetailCoverage = {
		gapKeys: [],
		hydratedKeys: [],
	};
	...
	const seededRateLimited =
		pacing.preDetailRateLimited ?? deps.preDetailPressure?.rateLimited ?? 0;
	...
	const tailGapBound =
		pacing.tailGapBound ?? resolveChatGptMaxTailDeferralGapsPerRun();
	...
	let observedRecoverablePressure: ChatGptRecoverableRetryExhaustedError | null =
		null;
	...
	function cachedBatchDetailId(
		detail: ChatGptFetchResult,
		chunkIdSet: ReadonlySet<string>,
	): string | null {
	...
	async function emitRunCapTailConversationDetailGaps(
		...
		const tail = convosToSync.slice(start);
		const perKey = Number.isFinite(tailGapBound)
			? tail.slice(0, tailGapBound)
			: tail;
		const backlog = Number.isFinite(tailGapBound)
			? tail.slice(tailGapBound)
			: [];
		for (const item of perKey) {
			await emitConversationDetailGapOnce(
				item,
				makeRunCapDeferredConversationDetailGap(item, capReason),
			);
		}
		if (backlog.length) {
			...
			if (watermark) {
				...
				await deps.emit(
					makeRunCapBacklogConversationDetailGap(
						watermark,
						backlog.length,
						capReason,
					),
	...
	async function maybeWaitOutRateLimitDensity(
		from: ConversationListItem,
	...
	async function maybeDeferForRunBudget(
		from: ConversationListItem,
	): Promise<ChatGptFetchResult | null> {
	...
	async function maybeDeferForFetchError(
		from: ConversationListItem,
		err: unknown,
	): Promise<ChatGptFetchResult | null> {
		...
		if (err instanceof ChatGptRecoverableRetryExhaustedError) {
			...
			const waitBudgetExhausted =
				runBudget.shouldStop() ||
				remainingRunBudgetMs <= 0 ||
				consecutiveWaitOutsWithoutSuccess >=
					CHATGPT_CIRCUIT_WAIT_OUT_MAX_CYCLES;
			...
			return emitTailConversationDetailGaps(from, (item, index) =>
				index === 0
					? makeConversationDetailGap(item, err)
					: makeDeferredConversationDetailGap(item, err),
			);
		}
		return null;
	}
...
async function recoverPendingConversationDetailGaps(
	...
	while (page.length > 0) {
		const result = await recoverPendingConversationDetailGapPage(
			deps,
			page,
			emitConversation,
			pacing,
...
async function recoverPendingConversationDetailGapPage(
	...
	if (recoveryItems.length) {
		...
		stoppedWithPending =
			hydrated.size < recoveryItems.length ||
			coverage.backlogGapKey !== undefined;
	...
	const backlogGap = messagesGaps.find(
		(gap) => backlogGapBeforeUpdateTime(gap) !== null,
	);
	if (backlogGap) {
		...
		if (beforeUpdateTime) {
			...
			return {
				...
				stoppedWithPending: result.expanded,
			};
		}
	}
...
async function recoverPendingMessageDetailGapsBeforeForwardRun(
	...
	const recovery = await recoverPendingConversationDetailGaps(
		deps,
		emitConversation,
		pacing,
	);
...
if (isMainModule(import.meta.url)) {
	runConnector({
		...
		browser: {
			...browserConfigPreservationFor("chatgpt"),
			profileName: "chatgpt",
		},
		async ensureSession({
			assist,
