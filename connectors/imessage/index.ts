...
export function resolveAttachmentsRoot(
	...
	return (
		env[ATTACHMENTS_ROOT_ENV] || join(homedir(), "Library/Messages/Attachments")
	);
}
...
function queryParticipantRows(
	db: DatabaseSync,
): IterableIterator<ParticipantRow> {
	return db
		.prepare(
			`
...
function queryAttachmentRows(
	db: DatabaseSync,
	hasChatJoin: boolean,
): IterableIterator<AttachmentRow> {
	const chatIdSelect = hasChatJoin
		? "cmj.chat_id as chat_id"
		: "NULL as chat_id";
	const chatJoin = hasChatJoin
		? "LEFT JOIN chat_message_join cmj ON cmj.message_id = m.ROWID"
		: "";
	return db
		.prepare(
...
async function resolveAttachmentHydration(
	...
	if (!(safe.ok && safe.path)) {
		return {
			...
			contentSha256: null,
			hydrationError:
				"attachment file is missing, unreadable, or outside the trusted attachments root.",
			hydrationStatus: "missing",
			sizeBytes: null,
		};
	}
