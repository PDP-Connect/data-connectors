# GitHub PDPP Collection Profile

`src/connector/` is the upstream GitHub connector copied verbatim except for
the mechanical import seam used by the explicit build. It was initially copied
from PDPP commit `597cc012611df90d07edbed187ba3e3212dbf258`; later connector
changes may be made here without a synchronization framework. Generic PDPP
runtime source is deliberately not checked in: the build resolves that closure
from the pinned commit object in `--pdpp-root` and records every runtime input
hash plus every bundled `node_modules` input by package, version,
package-manifest hash, per-file hashes, and closure hash. Dirty or untracked
files in that worktree are never build inputs.

`collection-profile.json` matches PDPP's canonical GitHub manifest at the
source commit recorded in `provenance.json`, except for its local artifact
version. The builder rejects all other manifest changes. `dist/collection-profile.mjs` is ignored
working-tree build output; its exact generated bytes are committed in the
artifact tarball and digested in the index and provenance. `provenance.json`
is generated. **Do not hand-edit either generated file.** Rebuild explicitly
with `npm run github-pdpp:build -- --pdpp-root /path/to/pdpp`, then run
`node scripts/generate-connector-index.mjs` to refresh the tarball and index.
While an unpublished draft artifact is deliberately being amended, a source
change may use `node scripts/generate-connector-index.mjs
--allow-unpublished-rebuild`; the default generator and every `--check` run
still reject same-version source drift.
The ordinary `npm run github-pdpp:test` suite is hermetic; it tests the
checked-in source, provenance, and tarball. To additionally compare a pinned
PDPP repository and prove a dirty worktree cannot affect a byte-stable rebuild,
set `PDPP_GITHUB_SOURCE_ROOT=/path/to/pdpp` for that test command.

The connector declares and uses only the `network` runtime binding. The pinned
generic runtime retains browser paths for source fidelity, but the build replaces
only their unreachable browser package with a tiny virtual fail-fast module.
GitHub's normal API collection path does not invoke it.
