	...
	for (let hop = 0; hop <= MAX_REDIRECT_HOPS; hop += 1) {
		...
		if (
			...
			if (!isSafeRedirectTarget(currentUrl, nextUrl, trustedOrigins)) {
				throw new CardDavStructuralError(
					`carddav_unsafe_redirect: ${originOf(currentUrl)} -> ${originOf(nextUrl)}`,
				);
			}
			currentUrl = nextUrl;
			continue;
		...
		if (outcome.kind !== "ok") {
			throw new CardDavStructuralError(
				`carddav_response_too_large: ${describeBoundedReadRejection(outcome)}`,
			);
		}
...
export async function listAddressBooks(args: {
	...
	if (res.status < 200 || res.status >= 300) {
		throw new Error(
			`carddav_list_addressbooks_failed: status=${String(res.status)}`,
		);
	}
...
export async function syncCollectionReport(args: {
	...
	const { authHeader, bookUrl, fetchImpl, priorSyncToken, trustedOrigins } =
		args;
	const res = await davRequest(
		fetchImpl,
		bookUrl,
	...
	if (res.status < 200 || res.status >= 300) {
		throw new Error(
			`carddav_sync_collection_failed: status=${String(res.status)}`,
		);
	}
...
export async function addressbookMultiget(args: {
	...
	for (let start = 0; start < hrefs.length; start += MULTIGET_CHUNK_SIZE) {
		...
		if (res.status < 200 || res.status >= 300) {
			throw new Error(
				`carddav_addressbook_multiget_failed: status=${String(res.status)}`,
			);
		}
		for (const block of extractAllHrefBlocks(res.text)) {
			const href = extractTag(block, "href");
...
export async function addressbookQueryAll(args: {
	...
	if (res.status < 200 || res.status >= 300) {
		throw new Error(
			`carddav_addressbook_query_failed: status=${String(res.status)}`,
		);
	}
	const resources: VCardResource[] = [];
	for (const block of extractAllHrefBlocks(res.text)) {
...
function decodeNumericXmlEntity(
	...
	if (
		!(Number.isInteger(codePoint) && codePoint >= 0 && codePoint <= 0x10_ff_ff)
	) {
		return entity;
	}
