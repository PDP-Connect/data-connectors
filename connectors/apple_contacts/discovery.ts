...
export const nativeFetchAdapter: DiscoveryFetch = (url, init) =>
	fetch(url, init);
...
export class CardDavRedirectOriginError extends Error {
	constructor(fromOrigin: string, toOrigin: string) {
		super(
			`carddav_discovery_unsafe_redirect: refused to follow ${fromOrigin} -> ${toOrigin} (origin not trusted)`,
		);
...
async function propfindFollowingRedirects(
	...
	for (let hop = 0; hop <= MAX_REDIRECT_HOPS; hop += 1) {
		...
		if (
			...
			if (!location) {
				throw new CardDavDiscoveryError(
					`carddav_discovery_redirect_missing_location: ${currentUrl}`,
				);
			}
			const nextUrl = new URL(location, currentUrl).toString();
			if (!isSafeRedirectTarget(currentUrl, nextUrl, trustedOrigins)) {
				throw new CardDavRedirectOriginError(
					originOf(currentUrl),
					originOf(nextUrl),
				);
			}
			...
			if (hop === MAX_REDIRECT_HOPS) {
				throw new CardDavDiscoveryError(
					`carddav_discovery_too_many_redirects: started at ${startUrl}`,
				);
			}
			continue;
		}
		...
		if (outcome.kind !== "ok") {
			throw new CardDavDiscoveryError(
				`carddav_discovery_response_too_large: ${describeBoundedReadRejection(outcome)}`,
			);
		}
		return {
			finalUrl: currentUrl,
...
async function retryPrincipalAtOriginRoot(
	...
	step: Awaited<ReturnType<typeof propfindFollowingRedirects>>;
	href: string | null;
} | null> {
	...
	return {
		step: rootStep,
		href: extractHref(rootStep.text, "current-user-principal"),
	};
}
...
export async function discoverCardDav(args: {
	...
	const principalUrl = new URL(
		principalHref,
		principalStep.finalUrl,
	).toString();
	if (
		!isSafeRedirectTarget(principalStep.finalUrl, principalUrl, trustedOrigins)
	) {
		throw new CardDavRedirectOriginError(
			originOf(principalStep.finalUrl),
			originOf(principalUrl),
		);
	}
	...
	if (homeSetStep.status < 200 || homeSetStep.status >= 300) {
		throw new CardDavDiscoveryError(
			`carddav_discovery_home_set_failed: status=${String(homeSetStep.status)}`,
		);
	}
	...
	if (
		!isSafeRedirectTarget(
			homeSetStep.finalUrl,
			addressBookHomeUrl,
			trustedOrigins,
		)
	) {
		throw new CardDavRedirectOriginError(
			originOf(homeSetStep.finalUrl),
			originOf(addressBookHomeUrl),
		);
	}
